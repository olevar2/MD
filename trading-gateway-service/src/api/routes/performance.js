...
router.get('/returns', async (req, res) => {
  try {
    ...
    console.error('Error generating returns analysis:', error);
    res.status(500).json({ error: 'Failed to generate returns analysis' });
  }
});
...
router.get('/drawdowns', async (req, res) => {
  try {
    ...
    console.error('Error generating drawdown analysis:', error);
    res.status(500).json({ error: 'Failed to generate drawdown analysis' });
  }
});

// Risk-adjusted returns endpoint
router.get('/risk-adjusted', async (req, res) => {
  try {
    // Generate metrics
    const metrics = [
      {
    ...
    console.error('Error generating risk-adjusted returns:', error);
    res.status(500).json({ error: 'Failed to generate risk-adjusted returns' });
  }
});
...
router.get('/trading-stats', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to generate trading statistics' });
  }
});
...
router.get('/comparison', async (req, res) => {
  try {
    const { benchmark, timeRange } = req.query;
    
    // Generate dates based on time range
    const dates = generateDatesByTimeRange(timeRange || '1y');
    const dataPoints = dates.length;
    ...
    console.error('Error generating portfolio comparison:', error);
    res.status(500).json({ error: 'Failed to generate portfolio comparison' });
  }
});
...
function generateDatesByTimeRange(timeRange) {
  const now = new Date();
  const dates = [];
