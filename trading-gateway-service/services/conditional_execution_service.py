...
class ConditionalExecutionService(BaseExecutionService):
    ...
    def place_order(self, order: OrderRequest, broker_name: Optional[str]=
        ...
        if order.order_type != OrderType.CONDITIONAL:
            error_msg = (
                f'Invalid order type for ConditionalExecutionService: {order.order_type}'
                )
        ...
        if mode_report is not None:
            ...
            self.orders[order_id] = {'order': order, 'broker': broker_name or
                ...
                'timestamp': datetime.utcnow(), 'condition': kwargs[
                'condition']}
        ...
        try:
            ...
            order_with_condition.extra_params = (order_with_condition.
                extra_params or {})
            order_with_condition.extra_params['condition'] = kwargs['condition'
                ]
            execution_report = asyncio.run(broker_adapter.place_order(
            ...
            self.orders[order_id] = {'order': order, 'broker': broker_name,
                ...
                .utcnow(), 'condition': kwargs['condition']}
            self._trigger_callbacks('order_placed', self.orders[order_id])
            ...
            self.logger.error(error_msg)
            rejection_report = ExecutionReport(order_id=str(uuid.uuid4()),
                client_order_id=order.client_order_id, instrument=order.
                instrument, status=OrderStatus.REJECTED, direction=order.
                direction, order_type=order.order_type, quantity=order.
                quantity, filled_quantity=0.0, price=order.price,
            ...
            self.orders[order_id] = {'order': order, 'broker': broker_name,
                ...
                .utcnow(), 'condition': kwargs['condition'], 'error': str(e)}
            self._trigger_callbacks('order_rejected', self.orders[order_id])
            self._trigger_callbacks('execution_error', {'order_id':
                order_id, 'error': str(e), 'order_info': self.orders[order_id]}
                )
    ...
    def cancel_order(self, order_id: str) ->ExecutionReport:
        ...
        try:
            ...
            self.logger.error(error_msg)
            rejection_report = ExecutionReport(order_id=order_id,
                client_order_id=execution_report.client_order_id,
                instrument=execution_report.instrument, status=OrderStatus.
                REJECTED, direction=execution_report.direction, order_type=
                execution_report.order_type, quantity=execution_report.
                quantity, filled_quantity=execution_report.filled_quantity,
                price=execution_report.price, rejection_reason=error_msg)
