...
class TestAnalysisEngineAdapter(unittest.TestCase):
    ...
    def test_detect_market_regime(self):
        """Test detect_market_regime method."""
        # Mock response
        mock_response = MagicMock()
        mock_response.json.return_value = {"regime": "trending", "confidence": 0.8}
        self.adapter.client.post.return_value = mock_response

        # Call method
        result = asyncio.run(self.adapter.detect_market_regime(
            symbol="EURUSD",
            timeframe="1h",
            lookback_bars=100
        ))
        ...
        # Verify result
        self.assertEqual(result, {"regime": "trending", "confidence": 0.8})
        self.adapter.client.post.assert_called_once()
    ...
    def test_get_technical_indicators(self):
        ...
        # Mock response
        mock_response = MagicMock()
        mock_response.json.return_value = {"indicators": {"sma": [1.0, 2.0, 3.0]}}
        self.adapter.client.post.return_value = mock_response

        # Call method
        result = asyncio.run(self.adapter.get_technical_indicators(
            symbol="EURUSD",
            timeframe="1h",
        ...
        # Verify result
        self.assertEqual(result, {"indicators": {"sma": [1.0, 2.0, 3.0]}})
        self.adapter.client.post.assert_called_once()
