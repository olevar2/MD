...
class BrokerScore:
    ...
    def total_score(self) -> float:
        ...
        return (self.price_score + self.speed_score + 
                self.liquidity_score + self.reliability_score) / 4.0
...
class SmartOrderRoutingAlgorithm(BaseExecutionAlgorithm):
    ...
    def __init__(self, 
        ...
        # Minimum number of brokers to use
        self.min_brokers = self.config.get('min_brokers', 1)
        
        # Maximum number of brokers to use
        self.max_brokers = self.config.get('max_brokers', len(broker_adapters))
        
        # Minimum order size per broker
        self.min_order_size = self.config.get('min_order_size', 1000)
    ...
    async def execute(self, order: OrderRequest) -> ExecutionResult:
        ...
        try:
            ...
            if not broker_scores:
                self.logger.error("No suitable brokers found for order execution")
                return ExecutionResult(
                    algorithm_id=self.algorithm_id,
                    original_order_id=order.client_order_id,
                    status='FAILED',
                    metrics={'error': 'No suitable brokers found'}
                )
            
            # Determine order allocation
            ...
            if not allocations:
                ...
                return ExecutionResult(
                    ...
                    original_order_id=order.client_order_id,
                    status='FAILED',
                    metrics={'error': 'Failed to calculate allocations'}
                )
            
            # Create and place child orders
            tasks = []
            for broker_name, quantity in allocations.items():
                tasks.append(self._place_child_order(order, broker_name, quantity))
            ...
            for result in child_results:
                if isinstance(result, Exception):
                    self.logger.error(f"Error placing child order: {str(result)}")
            ...
            result = ExecutionResult(
                ...
                metrics={
                    ...
                    'broker_scores': {b.broker_name: b.total_score for b in broker_scores},
                    'success_rate': success_count / len(allocations) if allocations else 0
                }
            )
    ...
    async def cancel(self) -> bool:
        ...
        if cancel_tasks:
            ...
            self._trigger_callbacks('cancelled', {
                ...
                'cancelled_orders': success_count,
                'total_orders': len(cancel_tasks),
                'timestamp': datetime.utcnow().isoformat()
            })
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'completion_percentage': (filled_quantity / total_quantity * 100) if total_quantity > 0 else 0,
    ...
    async def _score_brokers(self, order: OrderRequest) -> List[BrokerScore]:
        ...
        for broker_name, adapter in self.broker_adapters.items():
            if hasattr(adapter, 'get_quote') and callable(adapter.get_quote):
                quote_tasks.append(self._get_broker_quote(broker_name, adapter, order))
            else:
                self.logger.warning(f"Broker {broker_name} does not support quotes")
        ...
        # First pass to find best values for normalization
        for quote in quotes:
            ...
            if price is not None:
                if best_price is None or (
                    order.direction.value.upper() == 'BUY' and price < best_price or
                    order.direction.value.upper() == 'SELL' and price > best_price
                ):
                    best_price = price
        ...
        for quote in quotes:
            ...
            price = quote.get('price')
            liquidity = quote.get('liquidity', 0)
            speed = quote.get('response_time', 1000)  # ms
            ...
            if best_price is not None and price is not None:
                ...
                price_score = 0.5  # Neutral score if no comparison available
                
            # Speed score (lower is better)
            speed_score = 1.0 - min(1.0, speed / 1000)  # Normalize to 0-1
            
            # Liquidity score
            liquidity_score = liquidity / best_liquidity if best_liquidity > 0 else 0
            
            # Create broker score
            score = BrokerScore(
                broker_name=broker_name,
                price_score=price_score * self.price_weight,
                speed_score=speed_score * self.speed_weight,
                liquidity_score=liquidity_score * self.liquidity_weight,
                reliability_score=reliability * self.reliability_weight
            )
    ...
    async def _get_broker_quote(self, 
                              broker_name: str, 
                              adapter: BrokerAdapterInterface, 
                              order: OrderRequest) -> Dict[str, Any]:
        """
        ...
        try:
            # Check if the adapter has a get_quote method
            if hasattr(adapter, 'get_quote') and callable(adapter.get_quote):
                ...
                liquidity = quote.get('liquidity', {}).get(
                    'ask' if order.direction.value.upper() == 'BUY' else 'bid', 0
                )
                ...
                return {
                    ...
                    'reliability': 0.9,  # Default high reliability
                    'spread': quote.get('spread'),
                    'timestamp': quote.get('timestamp')
                }
            else:
                # Fallback to a simple price check
                if hasattr(adapter, 'get_price') and callable(adapter.get_price):
                    ...
                    return {
                        ...
                        'liquidity': 0,  # Unknown
                        'response_time': response_time,
                        'reliability': 0.7,  # Lower reliability for fallback
                        'timestamp': datetime.utcnow().isoformat()
                    }
                else:
                    return {
                        'broker_name': broker_name,
                        'error': 'Broker does not support quotes or prices'
                    }
        except Exception as e:
            self.logger.error(f"Error getting quote from broker {broker_name}: {str(e)}")
            return {
                'broker_name': broker_name,
                'error': str(e)
            }
    ...
    def _calculate_allocations(self, 
                             order: OrderRequest, 
                             broker_scores: List[BrokerScore]) -> Dict[str, float]:
        """
        ...
        # Determine how many brokers to use
        num_brokers = min(
            self.max_brokers,
            max(self.min_brokers, len(broker_scores))
        )
        
        # Use only the top N brokers
        ...
        if total_score <= 0:
            ...
            for i, broker in enumerate(top_brokers):
                # Last broker gets all remaining quantity
                if i == len(top_brokers) - 1:
                    allocation = remaining_quantity
                else:
                    # Allocate proportionally to score
                    allocation = total_quantity * (broker.total_score / total_score)
                    
                    # Round to avoid tiny orders
    ...
    async def _place_child_order(self, 
                               ...
                               broker_name: str, 
                               quantity: float) -> ExecutionReport:
        """
        ...
        # Create child order
        child_order_id = f"{parent_order.client_order_id}_SOR_{broker_name}_{uuid.uuid4().hex[:8]}"
        ...
        # Place the order
        self.logger.info(
            f"Placing child order {child_order_id} with broker {broker_name}: "
        ...
        try:
            ...
            self.logger.error(f"Error placing child order with broker {broker_name}: {str(e)}")
            raise
