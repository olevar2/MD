...
class BaseExecutionService(ABC):
    ...
    def set_default_broker(self, broker_name: str) -> bool:
        ...
        if broker_name not in self.broker_adapters:
            self.logger.error(f"Cannot set default broker: '{broker_name}' not registered")
            return False
    ...
    def _handle_execution_error(self, error: Exception, order_id: str = None,
                            client_order_id: str = None, instrument: str = None,
                            direction = None, order_type = None, quantity: float = 0.0,
                            price = None, error_prefix: str = "Error in execution") -> ExecutionReport:
        """
        ...
        rejection_report = ExecutionReport(
            ...
            direction=direction,  # Type will be corrected by ExecutionReport.__post_init__
            order_type=order_type,  # Type will be corrected by ExecutionReport.__post_init__
            quantity=quantity,
            filled_quantity=0.0,
            price=price,
        ...
        # If we have an order ID, update the order information
    ...
    def _create_dummy_execution_report(self, order: OrderRequest, status: OrderStatus) -> ExecutionReport:
        ...
        # For filled orders, set the executed price to the order price
        executed_price = order.price if status == OrderStatus.FILLED else None

        # For filled orders, set the filled quantity to the order quantity
    ...
    def update_execution_status(self, order_id: str, status_update: Dict[str, Any]) -> bool:
        ...
        # Update the execution report with the new status information
        for key, value in status_update.items():
            if hasattr(execution_report, key):
                setattr(execution_report, key, value)

        # Update the order info with the updated execution report
        order_info["execution_report"] = execution_report
        self.orders[order_id] = order_info

        # Trigger appropriate callbacks based on the new status
