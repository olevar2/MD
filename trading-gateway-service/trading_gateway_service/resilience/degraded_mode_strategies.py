...
class OrderPriority(int, Enum):
    ...
    @classmethod
    def from_order_type(cls, order_type: OrderType, is_closing: bool=False
        ) ->'OrderPriority':
        """Determine priority based on order type and context"""
        if (order_type == OrderType.STOP_LOSS or order_type == OrderType.
            STOP_MARKET):
            return cls.CRITICAL
        elif is_closing:
            return cls.HIGH
...
class OrderQueueManager:
    ...
    def __init__(self):
        ...
        self._queue_stats = {'total_queued': 0, 'total_processed': 0,
            'total_failed': 0, 'last_success_time': None,
            'degraded_mode_incidents': 0, 'current_queue_size': 0}
                ...
                if self._event_bus:
                    try:
                        event = create_event(event_type=EventType.
                            SERVICE_STATUS_CHANGED, source_service=
                            'trading-gateway-service', data={'status':
                            'DEGRADED', 'reason': reason, 'timestamp': self
                            ._degraded_mode_start_time.isoformat(),
                            'details': {'queue_stats': self._queue_stats}})
                        self._event_bus.publish(event)
                        logger.info(f'Published degraded mode event: {reason}')
                    except Exception as e:
                        logger.error(
                            f'Failed to publish degraded mode event: {e}')
                logger.warning(
                    f'Trading Gateway entered DEGRADED mode: {reason}')
        ...
        with self._lock:
            if self._is_degraded:
                ...
                if self._degraded_mode_start_time:
                    duration = (datetime.utcnow() - self.
                        _degraded_mode_start_time).total_seconds()
                if self._event_bus:
                    try:
                        event = create_event(event_type=EventType.
                            SERVICE_STATUS_CHANGED, source_service=
                            'trading-gateway-service', data={'status':
                            'HEALTHY', 'reason': 'Connectivity restored',
                            'timestamp': datetime.utcnow().isoformat(),
                            'details': {'degraded_duration_seconds':
                            duration, 'queue_stats': self._queue_stats}})
                        self._event_bus.publish(event)
                        logger.info('Published service recovery event')
                    except Exception as e:
                        logger.error(
                            f'Failed to publish service recovery event: {e}')
                logger.info(
                    f'Trading Gateway exited DEGRADED mode (duration: {duration}s)'
                    )
                self._degraded_mode_start_time = None
    ...
    def get_status(self) ->Dict[str, Any]:
        ...
        """
        with self._lock:
            return {'is_degraded': self._is_degraded,
                'degraded_mode_start_time': self._degraded_mode_start_time.
                isoformat() if self._degraded_mode_start_time else None,
                'queue_size': len(self._queue), 'queue_stats': self.
                _queue_stats.copy()}

    def queue_order(self, order_id: str, order_data: Dict[str, Any],
        ...
        with self._lock:
            ...
            prioritized_order = PrioritizedOrder(priority=priority.value,
                timestamp=time.time(), order_id=order_id, order_data=order_data
                )
            ...
            logger.info(
                f'Queued order {order_id} with priority {priority.name}')

    def set_order_processor(self, processor: Callable[[str, Dict[str, Any]],
        bool]) ->None:
        """
    ...
    """
        ...
        thread = threading.Thread(target=processing_loop, daemon=True, name
            ='order-queue-processor')
        thread.start()
        logger.info(
            f'Started order queue processor (interval: {check_interval}s)')
    ...
    def _process_queue(self) ->int:
        ...
        with self._lock:
            ...
            while self._queue:
                ...
                try:
                    success = self._order_processor(order.order_id, order.
                        order_data)
                    if success:
                        processed_count += 1
                        self._queue_stats['total_processed'] += 1
                        self._queue_stats['last_success_time'
                            ] = datetime.utcnow().isoformat()
                        logger.info(f'Processed queued order {order.order_id}')
                        if self._event_bus:
                            try:
                                event = create_event(event_type=EventType.
                                    ORDER_PROCESSED_FROM_QUEUE,
                                    source_service=
                                    'trading-gateway-service', data={
                                    'order_id': order.order_id,
                                    'queue_delay_seconds': time.time() -
                                    order.timestamp, 'priority':
                                    OrderPriority(order.priority).name})
                                self._event_bus.publish(event)
                            except Exception as e:
                                logger.error(
                                    f'Failed to publish order processed event: {e}'
                                    )
                    else:
                        failed_orders.append(order)
                        self._queue_stats['total_failed'] += 1
                        logger.warning(
                            f'Failed to process queued order {order.order_id}')
                except Exception as e:
                    failed_orders.append(order)
                    self._queue_stats['total_failed'] += 1
                    logger.error(
                        f'Error processing queued order {order.order_id}: {e}')
    ...
    @with_broker_api_resilience('get_queue_size')
    def get_queue_size(self) ->int:
        """Get number of orders in queue"""
        with self._lock:
            return len(self._queue)

    @with_broker_api_resilience('get_queue_summary')
    def get_queue_summary(self) ->Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {'total': len(self._queue), 'priorities': priorities,
                'oldest': datetime.fromtimestamp(oldest).isoformat() if
                oldest else None, 'newest': datetime.fromtimestamp(newest).
...
class RiskControlManager:
    ...
    def set_conservative_limits(self, max_position_size: Dict[str, float]=
        None, max_order_size: Dict[str, float]=None, max_daily_loss: Dict[
        str, float]=None, max_open_positions: Dict[str, int]=None) ->None:
        """
    ...
    def set_default_limits(self, account_id: str, account_balance: float
        ) ->None:
        """
        ...
        with self._lock:
            ...
            common_pairs = ['EURUSD', 'GBPUSD', 'USDJPY', 'AUDUSD',
    ...
    def update_position(self, account_id: str, symbol: str, size: float
        ) ->None:
        """
    ...
    @with_broker_api_resilience('update_pnl')
    ...
    def check_order(self, account_id: str, symbol: str, direction: str,
        size: float, is_closing: bool=False) ->Tuple[bool, str]:
        """
        ...
        with self._lock:
            ...
            if size > max_order:
                return (False,
                    f'Order size {size} exceeds degraded mode limit {max_order} for {symbol}'
                    )
            daily_pnl = self._daily_pnl.get(account_id, 0)
            max_loss = self._max_daily_loss.get(account_id, -1000)
            if daily_pnl < max_loss:
                return (False,
                    f'Daily loss {daily_pnl} exceeds degraded mode limit {max_loss} for account {account_id}'
                    )
            current_positions = len(self._position_sizes.get(account_id, {}))
            max_positions = self._max_open_positions.get(account_id, 5)
            if current_positions >= max_positions:
                return (False,
                    f'Open positions ({current_positions}) at degraded mode limit {max_positions}'
                    )
            current_size = self._position_sizes.get(account_id, {}).get(symbol,
                0)
            ...
            if abs(new_size) > max_position:
                return (False,
                    f'Position size {abs(new_size)} would exceed degraded mode limit {max_position} for {symbol}'
                    )
            return True, ''

    @with_risk_management_resilience('get_risk_status')
    def get_risk_status(self, account_id: str) ->Dict[str, Any]:
        """Get current risk status"""
        with self._lock:
            return {'daily_pnl': self._daily_pnl.get(account_id, 0),
                'max_daily_loss': self._max_daily_loss.get(account_id, 0),
                'open_positions': len(self._position_sizes.get(account_id,
                {})), 'max_open_positions': self._max_open_positions.get(
                account_id, 0), 'position_sizes': self._position_sizes.get(
                account_id, {}), 'degraded_mode_active': True}


def submit_order_degraded_broker(original_submit_order_func: Callable,
    order_id: str, order_data: Dict[str, Any]):
    """
...
def check_risk_degraded(original_check_risk_func: Callable, account_id: str,
    symbol: str, direction: str, size: float, is_closing: bool=False):
    """
    ...
    return risk_manager.check_order(account_id, symbol, direction, size,
        is_closing)


@with_exception_handling
def configure_trading_gateway_degraded_mode(submit_order_func: Callable,
    check_risk_func: Callable, process_queue_func: Callable,
    ...
    """
    fallback_for('broker', submit_order_func)(lambda *args, **kwargs:
        submit_order_degraded_broker(submit_order_func, *args, **kwargs))
    fallback_for('risk-management', check_risk_func)(lambda *args, **kwargs:
        check_risk_degraded(check_risk_func, *args, **kwargs))
    queue_manager = OrderQueueManager()
    ...
    def handle_degraded_status(dependency_name: str, is_degraded: bool):
        ...
        if dependency_name == 'broker':
            ...
            if is_degraded:
                logger.warning(
                    'Risk management service unavailable, using conservative limits'
                    )
                try:
