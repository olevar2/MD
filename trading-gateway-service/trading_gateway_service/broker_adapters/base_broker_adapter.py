...
from ..interfaces.broker_adapter import BrokerAdapter, OrderRequest, ExecutionReport, PositionUpdate, AccountUpdate, OrderStatus
...
class BaseBrokerAdapter(BrokerAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._heartbeat_interval = config_manager.get('heartbeat_interval', 30)
    ...
    @retry_with_policy(max_attempts=3, base_delay=4.0, max_delay=10.0,
    ...
    async def _make_request(self, method: str, endpoint: str, **kwargs) ->Any:
        ...
        async with self.session.request(method, endpoint, **kwargs
            ) as response:
    ...
    async def _start_heartbeat(self) ->None:
        ...
        if self._heartbeat_task is None or self._heartbeat_task.done():
            self._heartbeat_task = asyncio.create_task(self.
                _heartbeat_monitor())
    ...
    async def _handle_connection_loss(self) ->None:
        ...
        retry_count = 0
        max_retries = self.config_manager.get('max_retries', 5)
        base_delay = self.config_manager.get('retry_delay', 1)
        while retry_count < max_retries and not self._is_connected:
            try:
                ...
                logger.error(
                    f'Reconnection attempt {retry_count + 1} failed: {str(e)}')
                retry_count += 1
    ...
    async def _reconcile_positions(self) ->None:
        ...
        try:
            ...
            logger.info(
                f'Position reconciliation completed. Found {len(broker_positions)} positions.'
                )
        except Exception as e:
