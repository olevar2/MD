...
class MetaTraderAdapter(BaseBrokerAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._symbol_map = {}  # Maps standard symbols to MT5 symbols
        self._position_map = {}  # Maps MT5 position tickets to our position IDs

    def _get_auth_headers(self) -> Dict[str, str]:
        """Get authentication headers for API requests."""
        # Not used for MT5 as it uses direct connection
        return {}
    ...
    def _standardize_symbol(self, mt5_symbol: str) -> str:
        ...
        # Remove any special prefixes/suffixes used by the broker
    ...
    async def place_order(self, order_request: OrderRequest) -> ExecutionReport:
        ...
        try:
            ...
            request = {
                "action": mt5.TRADE_ACTION_DEAL if order_request.order_type == OrderType.MARKET
                         else mt5.TRADE_ACTION_PENDING,
                "symbol": mt5_symbol,
                "volume": order_request.quantity,
                "type": self._convert_order_type(order_request.order_type, order_request.direction),
                "price": order_request.price or mt5.symbol_info_tick(mt5_symbol).ask,
                "deviation": self.config.get("price_deviation", 10),
                "magic": 123456,  # Custom identifier
                "comment": f"ID:{order_request.client_order_id}",
            ...
            return ExecutionReport(
                ...
                client_order_id=order_request.client_order_id,
                instrument=order_request.instrument,
                status=OrderStatus.FILLED if result.volume == order_request.quantity
    ...
    def _convert_order_type(self, order_type: OrderType, direction: OrderDirection) -> int:
        """Convert our order type to MT5 order type."""
        if order_type == OrderType.MARKET:
            return mt5.ORDER_TYPE_BUY if direction == OrderDirection.BUY else mt5.ORDER_TYPE_SELL
        elif order_type == OrderType.LIMIT:
            return mt5.ORDER_TYPE_BUY_LIMIT if direction == OrderDirection.BUY else mt5.ORDER_TYPE_SELL_LIMIT
        elif order_type == OrderType.STOP:
            return mt5.ORDER_TYPE_BUY_STOP if direction == OrderDirection.BUY else mt5.ORDER_TYPE_SELL_STOP
    ...
    async def cancel_order(self, client_order_id: str) -> ExecutionReport:
        ...
        """
        try:
            # Find the order by client_order_id in the comment field
            ...
            request = {
                ...
                "order": target_order.ticket,
            }

            result = mt5.order_send(request)
            if result.retcode != mt5.TRADE_RETCODE_DONE:
                return ExecutionReport(
                    broker_order_id=str(target_order.ticket),
                    client_order_id=client_order_id,
                    instrument=target_order.symbol,
                    status=OrderStatus.REJECTED,
                    rejection_reason=f"MT5 Error: {result.comment}"
                )
    ...
    async def modify_order(self, client_order_id: str, modifications: Dict[str, Any]) -> ExecutionReport:
        ...
        try:
            ...
            request = {
                ...
                "order": target_order.ticket,
                "price": modifications.get("price", target_order.price_open),
                "sl": modifications.get("stop_loss", target_order.sl),
                "tp": modifications.get("take_profit", target_order.tp)
            }
        ...
        try:
            ...
            for pos in positions:
                ...
                result.append(PositionUpdate(
                    instrument=self._standardize_symbol(pos.symbol),
    ...
    async def close_position(self, position_id: str, quantity: Optional[float] = None) -> ExecutionReport:
        ...
        try:
            ...
            return ExecutionReport(
                ...
                client_order_id="",
                instrument=position.symbol,
                status=OrderStatus.FILLED if result.volume == close_volume
                       else OrderStatus.PARTIALLY_FILLED,
                filled_quantity=result.volume,
                average_price=result.price
            )
    ...
    async def _monitor_updates(self) -> None:
        ...
        while self._is_connected:
            try:
                # Check for new trades/orders every second
                trades = mt5.history_deals_get(
                    from_date=last_position_check
                )
                if trades:
                    for trade in trades:
                        if self._execution_callback:
                            await self._execution_callback(self._convert_trade_to_execution(trade))
                
                # Update positions and account info every 5 seconds
                now = datetime.utcnow()
    ...
    def _convert_trade_to_execution(self, trade) -> ExecutionReport:
        ...
        return ExecutionReport(
            broker_order_id=str(trade.order),
            client_order_id=self._extract_client_id(trade.comment),
            instrument=self._standardize_symbol(trade.symbol),
            status=OrderStatus.FILLED,
            filled_quantity=trade.volume,
