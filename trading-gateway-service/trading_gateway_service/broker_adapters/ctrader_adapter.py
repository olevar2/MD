...
class CTraderAdapter(BaseBrokerAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Track subscribed symbols for market data
    ...
    async def _authenticate(self) -> bool:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(f"{self.api_url}/connect/token", data=auth_data) as response:
                    ...
                    self._token_expires = datetime.utcnow().timestamp() + expires_in
                    return True
            ...
            while self._is_connected:
                ...
                if msg_type == "execution":
                    if self._execution_callback:
                        await self._execution_callback(self._convert_execution(data))
                elif msg_type == "position":
                    if self._position_callback:
                        await self._position_callback(self._convert_position(data))
                elif msg_type == "account":
                    if self._account_callback:
                        await self._account_callback(self._convert_account(data))
    ...
    def _convert_position(self, data: Dict) -> PositionUpdate:
        ...
        return PositionUpdate(
            ...
            average_price=data["openPrice"],
            unrealized_pl=data["unrealizedPL"],
    ...
    def _convert_account(self, data: Dict) -> AccountUpdate:
        ...
        return AccountUpdate(
            ...
            equity=data["equity"],
            margin_used=data["marginUsed"],
            margin_available=data["marginAvailable"],
    ...
    async def place_order(self, order_request: OrderRequest) -> ExecutionReport:
        ...
        try:
            ...
            request_data = {
                "symbol": order_request.instrument,
                "type": order_request.order_type.value,
                "side": order_request.direction.value,
            ...
            async with aiohttp.ClientSession(headers=self._get_auth_headers()) as session:
                async with session.post(f"{self.api_url}/v1/orders", json=request_data) as response:
                    data = await response.json()
                    ...
                    if response.status != 201:
                        return ExecutionReport(
                            ...
                            rejection_reason=data.get("message", "Unknown error")
                        )
                ...
                async with session.delete(
                    ...
                    if response.status != 200:
                        ...
                        return ExecutionReport(
                            ...
                            instrument="",
                            status=OrderStatus.REJECTED,
                            rejection_reason=data.get("message", "Cancel failed")
                        )
        ...
        try:
            async with aiohttp.ClientSession(headers=self._get_auth_headers()) as session:
                async with session.patch(
                    f"{self.api_url}/v1/orders/{client_order_id}",
                    json=modifications
                ) as response:
                    data = await response.json()
                    ...
                    if response.status != 200:
                        return ExecutionReport(
                            ...
                            instrument="",
                            status=OrderStatus.REJECTED,
                            rejection_reason=data.get("message", "Modification failed")
                        )
    ...
    async def get_positions(self) -> List[PositionUpdate]:
        ...
        try:
            async with aiohttp.ClientSession(headers=self._get_auth_headers()) as session:
                async with session.get(f"{self.api_url}/v1/positions") as response:
                    if response.status != 200:
                        return []
    ...
    async def get_account_info(self) -> AccountUpdate:
        ...
        try:
            async with aiohttp.ClientSession(headers=self._get_auth_headers()) as session:
                async with session.get(f"{self.api_url}/v1/accounts/{self.account_id}") as response:
                    if response.status != 200:
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        return ExecutionReport(
                            ...
                            instrument="",
                            status=OrderStatus.REJECTED,
                            rejection_reason=data.get("message", "Close failed")
                        )
