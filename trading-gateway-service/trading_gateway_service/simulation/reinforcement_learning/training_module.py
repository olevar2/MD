...
# Deep Learning frameworks - optional dependencies
try:
    import torch
...
# Stable Baselines - optional dependency
try:
...
# Local imports
from trading_gateway_service.simulation.reinforcement_learning.curriculum_learning_framework import (
    CurriculumLearningFramework, DifficultyLevel
)
from trading_gateway_service.simulation.reinforcement_learning.environment_generator import (
    ForexTradingEnvironment, EnvConfiguration, EnvironmentGenerator, ActionType
)
from trading_gateway_service.simulation.reinforcement_learning.agent_benchmarking import (
    RLAgentBenchmark, BenchmarkCategory
)
...
if TORCH_AVAILABLE:
    class DQNNetwork(nn.Module):
        ...
        def forward(self, x):
            ...
            # Reshape input to flatten window and features
            x = x.view(x.size(0), -1)
...
class DQNAgent:
    ...
    def load(self, path: str):
        ...
        if (checkpoint["input_shape"] != self.input_shape or 
            checkpoint["num_actions"] != self.num_actions):
            logger.warning("Model architecture mismatch. Creating new networks.")
...
class StableBaselineAgent:
    ...
    def __init__(
        ...
        if not SB3_AVAILABLE:
            raise ImportError("stable-baselines3 is required for StableBaselineAgent")
        ...
        if algorithm == "PPO":
            self.model = PPO(
                policy=custom_policy or policy,
                env=self.vec_env,
                verbose=verbose,
                **self.hyperparams
            )
...
class CurriculumTrainer:
    ...
    def __init__(
        self,
        framework: CurriculumLearningFramework,
        ...
        self.output_dir = output_dir or os.path.join(os.getcwd(), "training_results")
    ...
    def evaluate_agent(
        ...
        return {
            "returns_pct": np.mean(all_returns),
            "sharpe_ratio": np.mean(all_sharpe),
            "max_drawdown_pct": np.mean(all_drawdowns),
            "num_trades": np.mean(all_trade_counts),
    ...
    def train_on_scenario(
        ...
        env = EnvironmentGenerator.create_environment(
            market_simulator=self.framework.market_generator.market_simulator,
        ...
        if hasattr(self.agent, "learn"):
            # Agent has built-in learning method
            try:
                train_results = self.agent.learn(env, num_episodes, steps_per_episode)
                if isinstance(train_results, dict) and "rewards" in train_results:
                    training_metrics["episode_rewards"] = train_results["rewards"]
            except Exception as e:
                logger.error(f"Error during agent training: {e}")
        else:
            # Manual training loop for agents without learn method
            for episode in range(1, num_episodes + 1):
                state = env.reset()
                episode_reward = 0
                ...
                if episode % eval_frequency == 0:
                    ...
                    logger.info(f"Episode {episode}/{num_episodes} | "
                               f"Return: {eval_metrics['returns_pct']:.2f}% | "
    ...
    def train_current_level(
        ...
        for i, scenario in enumerate(scenarios):
            logger.info(f"Training on scenario {i+1}/{len(scenarios)}: "
                      f"{scenario.market_condition} condition")
            
            # Create environment config
            env_config = EnvConfiguration(
                symbols=[scenario.symbol],
                timeframe="1h",
                max_episode_steps=int(scenario.duration.total_seconds() / 3600)
            )
            
            # Train on this scenario
            scenario_results = self.train_on_scenario(
                scenario=scenario,
                env_config=env_config,
                num_episodes=episodes_per_scenario
            )
            
            # Report results to the curriculum framework
            self.framework.report_training_results(
                scenario_index=i,
    ...
    def run_curriculum_training(
        ...
        # Track agent model path if saved
        checkpoint_paths = []
        ...
        while self.framework.current_level <= max_levels:
            ...
            level_results = self.train_current_level(
            ...
            if save_checkpoints and hasattr(self.agent, "save"):
                ...
                logger.info(f"Saved agent checkpoint for level {level} to {checkpoint_path}")
            ...
            # If we haven't advanced and we're not done yet, we need more training
            if previous_level == current_level and level < max_levels:
                logger.info(f"Continuing training on level {level}")
                
                # One more round of training on the same level
                continue
    ...
    def _generate_training_report(self, results: Dict[str, Any]) -> None:
        ...
        # Extract performance metrics by level
        ...
        for level in levels:
            ...
            # Find final metrics from scenarios
            all_returns = []
            all_sharpes = []
            all_drawdowns = []
            all_win_rates = []
            ...
            for scenario, scenario_data in level_data.items():
                ...
                if "final_metrics" in scenario_data:
                    ...
                    all_drawdowns.append(final_metrics.get("max_drawdown_pct", 0.0))
                    all_win_rates.append(final_metrics.get("win_rate", 0.0))
            
            # Average metrics for this level
            if all_returns:
        ...
        if levels and returns_by_level:
            # Plot performance metrics by level
            ...
            # Find first level with episode rewards
            for level in levels:
                level_data = results["levels"][level]
                for scenario, scenario_data in level_data.items():
                    if scenario == "summary":
                    ...
                    if "episode_rewards" in scenario_data and scenario_data["episode_rewards"]:
                        plt.plot(scenario_data["episode_rewards"], 
                                label=f"Level {level[-1]} - {scenario}")
                        break
...
def run_experiment():
    ...
    output_dir = "d:/MD/forex_trading_platform/experiments/rl_curriculum_learning"
    ...
    # Create market condition generator
    market_gen = EnhancedMarketConditionGenerator(
        broker_simulator=broker_sim,
        base_volatility_map={"EUR/USD": 0.0007, "GBP/USD": 0.0009}
    )
    ...
    framework = CurriculumLearningFramework(
        ...
        num_levels=3,  # Simplified to 3 levels for the experiment
        symbols=["EUR/USD"],
        consecutive_successes_required=2,
        session_duration=timedelta(hours=24)
    )
    
    # Define agent factory function
    def create_dqn_agent():
        input_shape = (20, 100)  # (window_size, features)
        num_actions = 4  # BUY, SELL, CLOSE, HOLD for one symbol
    ...
    results = trainer.run_curriculum_training(
        scenarios_per_level=2,
    ...
    benchmark_summary = benchmark.get_benchmark_summary(list(benchmark.results.keys())[0])
    
    # Print final results
    logger.info("=== Experiment Complete ===")
    logger.info(f"Final agent saved to: {final_agent_path}")
    logger.info(f"Curriculum progress: {results['final_progress']['curriculum_completion'] * 100:.1f}% complete")
    logger.info(f"Benchmark overall score: {benchmark_summary['overall_score']:.4f}")
    
    # Save experiment summary
    with open(os.path.join(output_dir, "experiment_summary.json"), 'w') as f:
        json.dump({
            "curriculum_completion": results['final_progress']['curriculum_completion'],
            "benchmark_score": benchmark_summary['overall_score'],
            "category_scores": benchmark_summary['category_scores'],
