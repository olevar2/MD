...
class NewsAwareStrategy:
    ...
    def __init__(self):
        ...
        # Risk parameters
        self.normal_size = 0.1  # 0.1 lot standard position size
        self.news_size = 0.05   # 0.05 lot during news (reduced risk)
        self.normal_stop_pips = 20
        self.news_stop_pips = 40  # Wider stops during news
    ...
    def execute(
        ...
        **kwargs
        ...
        for instrument, df in data.items():
            ...
            # Iterate through each bar (excluding the warmup period)
            for i in range(max(self.slow_ma_normal, self.rsi_period), len(df)):
                ...
                current = {
                    ...
                    'fast_ma_normal': self.ma_data[instrument]['fast_normal'][i],
                    'slow_ma_normal': self.ma_data[instrument]['slow_normal'][i],
                    'fast_ma_news': self.ma_data[instrument]['fast_news'][i],
                    'slow_ma_news': self.ma_data[instrument]['slow_news'][i],
                    'rsi': self.ma_data[instrument]['rsi'][i],
                    'prev_fast_ma_normal': self.ma_data[instrument]['fast_normal'][i-1],
                    'prev_slow_ma_normal': self.ma_data[instrument]['slow_normal'][i-1],
                    'prev_fast_ma_news': self.ma_data[instrument]['fast_news'][i-1],
                    'prev_slow_ma_news': self.ma_data[instrument]['slow_news'][i-1],
                    'prev_rsi': self.ma_data[instrument]['rsi'][i-1]
                }
                ...
                high_impact_news = [
                    event for event in active_news
                    if event.impact_level in [NewsImpactLevel.HIGH, NewsImpactLevel.CRITICAL]
                ]
                
                # Log when high impact news is active
                if high_impact_news:
                ...
                # Decide whether to use news-sensitive parameters
                ...
                self._check_entries(
                    instrument, timestamp, current, backtester, use_news_params, high_impact_news
                )
                
        # Return strategy information
        return {
            "strategy_name": "News-Aware MA Crossover with RSI Filter",
            "parameters": {
                "fast_ma_normal": self.fast_ma_normal,
                "slow_ma_normal": self.slow_ma_normal,
                "fast_ma_news": self.fast_ma_news,
                "slow_ma_news": self.slow_ma_news,
                ...
                "stop_loss": {
                    "normal_pips": self.normal_stop_pips,
                    "news_pips": self.news_stop_pips
                }
            }
        }
    ...
    def _calculate_indicators(self, instrument: str, df: pd.DataFrame) -> None:
        ...
        # Calculate news-adjusted MAs (shorter periods)
        ...
        self.ma_data[instrument] = {
            'fast_normal': data['fast_ma_normal'].values,
            'slow_normal': data['slow_ma_normal'].values,
            'fast_news': data['fast_ma_news'].values,
            'slow_news': data['slow_ma_news'].values,
            'rsi': data['rsi'].values
        }
    ...
    def _check_entries(
        ...
        backtester: NewsEventBacktester,
        use_news_params: bool,
        high_impact_news: List[Any]
    ) -> None:
        ...
        # Select which MA to use based on news presence
        if use_news_params:
            ...
            position_size = self.news_size  # Smaller size during news
            stop_pips = self.news_stop_pips  # Wider stop during news
        else:
        ...
        # Skip if RSI is extreme (overbought/oversold)
        is_overbought = rsi > self.rsi_overbought
        is_oversold = rsi < self.rsi_oversold
        
        # Check for news-specific entry filters
        if high_impact_news:
            # Additional checks during high-impact news
            # For example, only take signals aligned with news sentiment
            news_sentiment = self._determine_news_sentiment(high_impact_news)
            signal_aligned_with_news = False
            
            # Check for bullish crossing with bullish news or bearish crossing with bearish news
            if news_sentiment > 0 and prev_fast_ma <= prev_slow_ma and fast_ma > slow_ma:
                signal_aligned_with_news = True  # Bullish cross with bullish news
            elif news_sentiment < 0 and prev_fast_ma >= prev_slow_ma and fast_ma < slow_ma:
                signal_aligned_with_news = True  # Bearish cross with bearish news
                
            # Skip if signal and news sentiment don't align
        ...
        if prev_fast_ma <= prev_slow_ma and fast_ma > slow_ma and not is_overbought:
            ...
            # Calculate stop loss (fixed pip distance)
            ...
            position = backtester.open_position(
                ...
                stop_loss=stop_loss,
                metadata={
                    "signal_type": "ma_crossover_long",
                    "during_news": use_news_params,
            ...
            position = backtester.open_position(
                ...
                stop_loss=stop_loss,
                metadata={
                    "signal_type": "ma_crossover_short",
                    "during_news": use_news_params,
                    "news_count": len(high_impact_news),
                    "rsi": rsi
                }
    ...
    def _check_exits(
        ...
        # Skip if we don't have a position for this instrument
        ...
        # Check for opposing crossover (exit signal)
        exit_signal = False
        ...
        # Exit position if we have a signal or stop was hit
...
def run_news_aware_backtest(
    ...
    news_events = news_collector.load_or_download_news_for_period(
        start_date, end_date, source='sample'
    )
    ...
    backtester = NewsEventBacktester(
        ...
        commission=0.0001,  # 1 pip per round trip
        base_slippage=0.2,  # 0.2 pips
        base_spread=0.5     # 0.5 pips
    )
    
    # Add news events
    backtester.news_events = news_events
    
    # Pre-calculate news impacts
    ...
    # Analyze performance during news vs. non-news periods
...
def _generate_sample_price_data(
    ...
    for instrument in instruments:
        # Set base parameters per instrument
        ...
        # Add volume (not used in this simple strategy)
...
def _generate_summary_report(
    ...
    with open(report_path, 'w') as f:
        ...
        # Determine if strategy performs better during news or normal periods
...
if __name__ == "__main__":
    ...
    results = run_news_aware_backtest(
