...
from trading_gateway_service.simulation.news_sentiment_simulator import (
    NewsEvent, NewsImpactLevel, NewsEventType, SentimentLevel
)
...
class NewsDataCollector:
            ...
            with open(csv_path, 'r', encoding='utf-8') as f:
                ...
                for i, row in enumerate(reader):
                    try:
                        ...
                        # Determine currencies affected
                        currency = row.get('currency', '')
                        if not currency:
                            continue
                            
                        # Convert currency to affected pairs
                        ...
                        # Calculate price and volatility impact based on deviation from forecast
                        price_impact, volatility_impact = self._calculate_impact(
                        ...
                        event = NewsEvent(
                            ...
                            timestamp=date_obj,
                            currencies_affected=pairs,
                            title=event_title,
                            description=f"Historical {event_title}",
                            ...
                            duration_minutes=self._get_duration_for_impact(impact_level)
                        )
    ...
    def load_or_download_news_for_period(
        self, 
        start_date: datetime,
        end_date: datetime,
        source: str = 'local',
        ...
        """
        # Generate cache filename based on period
        cache_filename = f"news_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.json"
        ...
        # No cache or forced reload - collect data
        events = []
        
        if source == 'local':
            # Look for local CSV files with naming pattern YYYY_MM.csv
            year_months = set()
            ...
            for year, month in year_months:
                csv_path = os.path.join(self.cache_dir, f"{year}_{month:02d}.csv")
            ...
            # Create some high-impact events during the period
            days_range = (end_date - start_date).days
            if days_range <= 0:
                days_range = 1
                
            # Create a few major events
            major_events = []
            for i in range(min(days_range // 7 + 1, 5)):
                # Add a major news event roughly every week
                event_date = start_date + timedelta(days=i*7 + np.random.randint(0, 7))
                
                # Make it during trading hours
                hour = np.random.randint(8, 16)
                event_date = event_date.replace(hour=hour, minute=0)
                
                # Create an NFP or rate decision event
                if i % 2 == 0:
                    event = NewsEvent(
                        event_id=f"sample_major_{i}",
                        event_type=NewsEventType.ECONOMIC_DATA,
                        impact_level=NewsImpactLevel.HIGH,
                        timestamp=event_date,
                        currencies_affected=["EUR/USD", "GBP/USD", "USD/JPY"],
                        title="US Non-Farm Payrolls",
                        description="Major employment report",
                        expected_value=200.0,
                        actual_value=180.0,
                        previous_value=190.0,
                        sentiment_impact=SentimentLevel.SLIGHTLY_BEARISH,
                    ...
                    event = NewsEvent(
                        ...
                        timestamp=event_date,
                        currencies_affected=["EUR/USD", "EUR/GBP", "EUR/JPY"],
            ...
            # Create some medium impact events
            medium_events = []
            for i in range(min(days_range, 10)):
                # Spread medium events throughout the period
                ...
                event = NewsEvent(
                    event_id=f"sample_medium_{i}",
                    event_type=NewsEventType.ECONOMIC_DATA,
                    impact_level=NewsImpactLevel.MEDIUM,
                    timestamp=event_date,
                    currencies_affected=["GBP/USD"],
    ...
    def _currency_to_pairs(self, currency: str) -> List[str]:
        ...
        # Extract the currency code (usually 3 letters)
        match = re.search(r'([A-Z]{3})', currency)
        if match:
            currency = match.group(1)
        
        # Return pairs for this currency
    ...
    def _determine_event_type(self, event_title: str) -> NewsEventType:
        ...
        if any(x in title_lower for x in ['rate', 'central bank', 'fed', 'ecb', 'boe', 'rba', 'boj', 'fomc', 'minutes']):
            return NewsEventType.CENTRAL_BANK
        elif any(x in title_lower for x in ['election', 'vote', 'referendum', 'war', 'conflict', 'tariff', 'trade war']):
            return NewsEventType.GEOPOLITICAL
        elif any(x in title_lower for x in ['earthquake', 'hurricane', 'tsunami', 'volcano', 'flood', 'disaster']):
            return NewsEventType.NATURAL_DISASTER
        elif any(x in title_lower for x in ['sentiment', 'confidence', 'outlook', 'survey']):
            return NewsEventType.MARKET_SENTIMENT
        else:
            return NewsEventType.ECONOMIC_DATA  # Default to economic data
    ...
    def _calculate_impact(
        ...
        base_factors = {
            NewsImpactLevel.LOW: 0.0005,
            NewsImpactLevel.MEDIUM: 0.001,
            NewsImpactLevel.HIGH: 0.002,
            NewsImpactLevel.CRITICAL: 0.005
        }
        
        # Base volatility multipliers
        base_volatility = {
            NewsImpactLevel.LOW: 1.2,
            NewsImpactLevel.MEDIUM: 1.5,
            NewsImpactLevel.HIGH: 2.0,
            NewsImpactLevel.CRITICAL: 3.0
        }
        
        base_factor = base_factors[impact_level]
        volatility = base_volatility[impact_level]
        
        # If we have actual and forecast, calculate surprise factor
    ...
    def _determine_sentiment_impact(
        ...
        # Reverse for certain indicators where lower is better
        title_lower = event_title.lower()
        if any(x in title_lower for x in ['unemployment', 'jobless', 'deficit', 'debt', 'inflation']):
            surprise_factor = -surprise_factor
            
        # Map surprise to sentiment
    ...
    def _get_duration_for_impact(self, impact_level: NewsImpactLevel) -> int:
        ...
        durations = {
            NewsImpactLevel.LOW: 60,
            NewsImpactLevel.MEDIUM: 120,
            NewsImpactLevel.HIGH: 240,
            NewsImpactLevel.CRITICAL: 480
        }
        return durations.get(impact_level, 120)


# Function to convert single-currency CSV to correct format
    ...
    try:
        ...
        if missing:
            ...
            for col in df.columns:
                ...
                elif 'event' in col.lower() or 'title' in col.lower() or 'name' in col.lower():
                    col_map[col] = 'event'
        ...
        if 'date' in df.columns:
            # Try to parse dates in a common format
            try:
