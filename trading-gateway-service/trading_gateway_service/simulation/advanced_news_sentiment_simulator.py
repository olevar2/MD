...
class NewsCategory(str, Enum):
    ...
    ECONOMIC_DATA = "economic_data"               # e.g., GDP, CPI, employment
    CENTRAL_BANK = "central_bank"                 # e.g., Fed decisions, ECB announcements
    GEOPOLITICAL = "geopolitical"                 # e.g., elections, conflicts
    CORPORATE = "corporate"                       # e.g., earnings, mergers
    MARKET_SENTIMENT = "market_sentiment"         # e.g., risk-on/risk-off shifts
    NATURAL_DISASTER = "natural_disaster"         # e.g., weather events, earthquakes
    REGULATORY = "regulatory"                     # e.g., policy changes
    COMMODITY = "commodity"                       # e.g., oil price movements


class NewsImpactLevel(str, Enum):
    """Impact level of news events."""
    LOW = "low"           # Minor price movements (±0.1-0.2%)
    MEDIUM = "medium"     # Moderate price movements (±0.2-0.5%)
    HIGH = "high"         # Significant price movements (±0.5-1.0%)
    CRITICAL = "critical" # Major price movements (>1.0%)


class SentimentLevel(str, Enum):
    """Market sentiment levels."""
    VERY_BEARISH = "very_bearish"     # Strong selling pressure
    BEARISH = "bearish"               # Moderate selling pressure
    SLIGHTLY_BEARISH = "slightly_bearish"  # Mild selling pressure
    NEUTRAL = "neutral"               # Balanced market
    SLIGHTLY_BULLISH = "slightly_bullish"  # Mild buying pressure
    BULLISH = "bullish"               # Moderate buying pressure
    VERY_BULLISH = "very_bullish"     # Strong buying pressure


class TechnicalImpact(str, Enum):
    """Technical impact of news events on market conditions."""
    TREND_REVERSAL = "trend_reversal"       # Reverses current trend
    TREND_ACCELERATION = "trend_acceleration"  # Speeds up current trend
    VOLATILITY_SPIKE = "volatility_spike"   # Increases volatility
    LIQUIDITY_REDUCTION = "liquidity_reduction"  # Reduces market liquidity
    RANGE_BREAKOUT = "range_breakout"       # Breaks support/resistance
    GAP_EVENT = "gap_event"                 # Creates price gap
    NONE = "none"                           # No significant technical impact
...
class NewsEvent:
    ...
    def __init__(
        ...
        sentiment_impact: Dict[str, SentimentLevel] = None,
        technical_impact: TechnicalImpact = TechnicalImpact.NONE,
        price_impact_factor: float = 0.0,
        volatility_impact_factor: float = 1.0,
        liquidity_impact_factor: float = 1.0,
    ...
    def calculate_remaining_impact(self, current_time: datetime) -> float:
        ...
        if current_time < self.release_time:
            ...
            remaining_pct = 1.0 - min(1.0, elapsed / max(1, self.duration_minutes))
            
            # Impact peaks quickly then declines gradually
            if elapsed < self.duration_minutes * 0.1:
                # Rapid rise in first 10% of duration
                rise_pct = elapsed / (self.duration_minutes * 0.1)
                return rise_pct
            else:
                # Gradual decline in remaining time
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sentiment_impact": {k: v.value for k, v in self.sentiment_impact.items()},
            "technical_impact": self.technical_impact.value,
            "price_impact_factor": self.price_impact_factor,
            "volatility_impact_factor": self.volatility_impact_factor,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'NewsEvent':
        ...
        return cls(
            ...
            sentiment_impact={k: SentimentLevel(v) for k, v in data.get('sentiment_impact', {}).items()},
            technical_impact=TechnicalImpact(data.get('technical_impact', 'none')),
            price_impact_factor=data.get('price_impact_factor', 0.0),
            volatility_impact_factor=data.get('volatility_impact_factor', 1.0),
            liquidity_impact_factor=data.get('liquidity_impact_factor', 1.0),
...
class NewsCalendar:
    ...
    def get_events_in_window(
        ...
        impact_level: Optional[NewsImpactLevel] = None
    ) -> List[NewsEvent]:
        """
    ...
    def get_active_events(self, current_time: datetime) -> List[NewsEvent]:
        ...
        for event in self.events:
            ...
            end_time = event.release_time + timedelta(minutes=event.duration_minutes)
...
class MarketSentimentTracker:
    ...
    def __init__(self, base_sentiment: Dict[str, SentimentLevel] = None):
        ...
        """
        self.base_sentiment = base_sentiment or {
            "USD": SentimentLevel.NEUTRAL,
            "EUR": SentimentLevel.NEUTRAL,
            "GBP": SentimentLevel.NEUTRAL,
            "JPY": SentimentLevel.NEUTRAL,
            "AUD": SentimentLevel.NEUTRAL,
            "CAD": SentimentLevel.NEUTRAL,
            "CHF": SentimentLevel.NEUTRAL,
            "NZD": SentimentLevel.NEUTRAL
        }
        
        self.sentiment_history = {
            currency: [] for currency in self.base_sentiment
        }
    ...
    def sentiment_to_value(self, sentiment: SentimentLevel) -> float:
        ...
        mapping = {
            ...
            SentimentLevel.BEARISH: -0.66,
            SentimentLevel.SLIGHTLY_BEARISH: -0.33,
            SentimentLevel.NEUTRAL: 0.0,
            SentimentLevel.SLIGHTLY_BULLISH: 0.33,
            SentimentLevel.BULLISH: 0.66,
    ...
    def update_sentiment(self, currency: str, event: NewsEvent, event_impact: float) -> None:
        """
        ...
        # Get sentiment impact from event
        impact_sentiment = event.sentiment_impact.get(currency, SentimentLevel.NEUTRAL)
        impact_value = self.sentiment_to_value(impact_sentiment)
        
        # Scale by event impact factor
        scaled_impact = impact_value * event_impact
        
        # Add to active impacts
        self.active_sentiment_impacts.append({
            "currency": currency,
            "event_id": event.event_id,
            "impact_value": scaled_impact,
            "expiry": datetime.now() + timedelta(minutes=event.duration_minutes)
        })
    ...
    def get_current_sentiment(self, currency: str) -> SentimentLevel:
        ...
        # Start with base sentiment
        base_value = self.sentiment_to_value(self.base_sentiment[currency])
        
        # Add active impacts
        current_time = datetime.now()
        active_impacts = [
            impact for impact in self.active_sentiment_impacts
            if impact["currency"] == currency and impact["expiry"] > current_time
        ]
    ...
    def get_pair_sentiment(self, base_currency: str, quote_currency: str) -> SentimentLevel:
        """
        ...
        """
        # Get individual currency sentiments
        base_sentiment = self.sentiment_to_value(self.get_current_sentiment(base_currency))
        quote_sentiment = self.sentiment_to_value(self.get_current_sentiment(quote_currency))
        
        # Calculate pair sentiment (positive = bullish for base currency)
    ...
    def get_historical_sentiment(
        ...
        # Return the most recent 'lookback' entries
    ...
    def record_current_sentiment(self) -> None:
        ...
        for currency in self.base_sentiment:
            ...
            if len(self.sentiment_history[currency]) > max_history:
                self.sentiment_history[currency] = self.sentiment_history[currency][-max_history:]
...
class NewsAndSentimentSimulator:
    ...
    def __init__(
        ...
        sentiment_tracker: Optional[MarketSentimentTracker] = None,
        event_frequency: float = 0.05,  # Events per hour
    ...
    def update(self, current_time: datetime) -> List[NewsEvent]:
        ...
        time_elapsed = (current_time - self.last_update_time).total_seconds() / 3600.0  # hours
        
        # Check for random event generation
        ...
        # Apply impacts of newly processed events
    ...
    def _apply_event_impact(self, event: NewsEvent) -> None:
        ...
        # Update sentiment for affected currencies
        for currency in event.affected_currencies:
            self.sentiment_tracker.update_sentiment(currency, event, 1.0)
            
        # Create and add market event to broker simulator
        market_event = self._create_market_event_from_news(event, affected_symbols)
        self.broker_simulator.add_market_event(market_event)
        
    def _create_market_event_from_news(
        self, 
        ...
        # Map news category to market event type
        event_type_mapping = {
            NewsCategory.ECONOMIC_DATA: MarketEventType.ECONOMIC_RELEASE,
            NewsCategory.CENTRAL_BANK: MarketEventType.CENTRAL_BANK_DECISION,
            NewsCategory.GEOPOLITICAL: MarketEventType.GEOPOLITICAL_EVENT,
            NewsCategory.NATURAL_DISASTER: MarketEventType.GEOPOLITICAL_EVENT,
            NewsCategory.MARKET_SENTIMENT: MarketEventType.TECHNICAL_BREAKOUT,
            NewsCategory.REGULATORY: MarketEventType.CENTRAL_BANK_DECISION,
            NewsCategory.CORPORATE: MarketEventType.ECONOMIC_RELEASE,
            NewsCategory.COMMODITY: MarketEventType.GEOPOLITICAL_EVENT
        }
        
        event_type = event_type_mapping.get(news_event.category, MarketEventType.ECONOMIC_RELEASE)
        
        # Map impact level to magnitude
        impact_magnitude_mapping = {
            NewsImpactLevel.LOW: 0.2,
            NewsImpactLevel.MEDIUM: 0.5,
            NewsImpactLevel.HIGH: 0.8,
            NewsImpactLevel.CRITICAL: 1.0
        }
        
        impact_magnitude = impact_magnitude_mapping.get(news_event.impact_level, 0.5)
    ...
    def _generate_random_event(self, current_time: datetime) -> NewsEvent:
        ...
        """
        # Generate event with random parameters
        event_id = str(uuid.uuid4())
        
        # Random category with weighted probabilities
        category_weights = {
            NewsCategory.ECONOMIC_DATA: 0.4,
            NewsCategory.CENTRAL_BANK: 0.2,
            NewsCategory.GEOPOLITICAL: 0.15,
            NewsCategory.CORPORATE: 0.05,
            NewsCategory.MARKET_SENTIMENT: 0.1,
            NewsCategory.NATURAL_DISASTER: 0.03,
            NewsCategory.REGULATORY: 0.05,
            NewsCategory.COMMODITY: 0.02
        }
        ...
        # Random impact level with weighted probabilities
        impact_weights = {
            NewsImpactLevel.LOW: 0.5,
            NewsImpactLevel.MEDIUM: 0.3,
            NewsImpactLevel.HIGH: 0.15,
            NewsImpactLevel.CRITICAL: 0.05
        }
        ...
        # Generate sentiment impact
        sentiment_impact = {}
        for currency in affected_currencies:
            # Random sentiment impact
            sentiment_value = random.uniform(-1.0, 1.0)
            sentiment_level = self.sentiment_tracker.value_to_sentiment(sentiment_value)
            sentiment_impact[currency] = sentiment_level
        
        # Random impact factors based on impact level
        impact_mapping = {
            NewsImpactLevel.LOW: (0.0001, 0.0003, 1.1, 0.9, 30),
            NewsImpactLevel.MEDIUM: (0.0003, 0.0008, 1.3, 0.8, 60),
            NewsImpactLevel.HIGH: (0.0008, 0.0015, 1.8, 0.7, 120),
            NewsImpactLevel.CRITICAL: (0.0015, 0.0030, 2.5, 0.5, 180)
        }
        
        min_price, max_price, vol_factor, liq_factor, duration = impact_mapping[impact_level]
        
        price_impact_factor = random.uniform(min_price, max_price)
        if random.random() < 0.5:
            price_impact_factor *= -1  # 50% chance of negative impact
            
        # Random technical impact
        ...
        event = NewsEvent(
            ...
            liquidity_impact_factor=liq_factor,
    ...
    def _get_random_titles(self, category: NewsCategory) -> List[str]:
        ...
        title_templates = {
            ...
            NewsCategory.REGULATORY: [
                ...
                "{country} reforms {policy_type} regulations"
            ],
            NewsCategory.COMMODITY: [
                "Oil prices {direction} amid {economic_condition}",
        ...
        templates = title_templates.get(category, ["Economic news impacts markets"])
        filled_templates = []
        
        for template in templates:
            filled = template.format(
                country=random.choice(["US", "EU", "UK", "Japan", "China", "Australia", "Canada", "Switzerland"]),
                country2=random.choice(["US", "EU", "UK", "Japan", "China", "Russia", "Brazil", "India"]),
                indicator=random.choice(["GDP", "Inflation", "CPI", "PPI", "Unemployment", "Retail Sales", "PMI", "Industrial Production"]),
                direction=random.choice(["rises", "falls", "jumps", "plummets", "increases", "decreases"]),
                value=random.randint(1, 10) / 10.0,
                beats_misses=random.choice(["beats", "misses", "matches"]),
                adj=random.choice(["strong", "weak", "moderate", "robust", "sluggish", "unexpected", "anticipated"]),
                central_bank=random.choice(["Fed", "ECB", "BoE", "BoJ", "RBA", "BoC", "SNB", "PBOC"]),
                action=random.choice(["raises", "cuts", "maintains", "increases", "decreases", "expands", "reduces"]),
                escalate_ease=random.choice(["escalate", "ease", "intensify", "improve"]),
                policy_type=random.choice(["fiscal", "monetary", "trade", "tax", "economic"]),
                industry=random.choice(["banking", "tech", "energy", "retail", "automotive", "healthcare", "manufacturing"]),
                company=random.choice(["Major bank", "Tech giant", "Energy firm", "Retail leader", "Auto manufacturer"]),
                company2=random.choice(["competitor", "industry leader", "global player", "market entrant"]),
                economic_condition=random.choice(["inflation concerns", "growth outlook", "recession fears", "recovery hopes", "stability expectations"]),
                sentiment=random.choice(["bullish", "bearish", "optimistic", "pessimistic", "cautious"]),
                appetite_aversion=random.choice(["appetite", "aversion"]),
                disaster=random.choice(["Earthquake", "Hurricane", "Flood", "Drought", "Wildfire", "Pandemic"]),
                regulatory_body=random.choice(["Central Bank", "SEC", "CFTC", "FSA", "Government", "Treasury"]),
                commodity=random.choice(["Oil", "Gold", "Natural Gas", "Copper", "Agricultural products", "Metals"]),
                timeframe=random.choice(["yearly", "monthly", "weekly", "daily"]),
                high_low=random.choice(["high", "low"]),
                currency=random.choice(["USD", "EUR", "GBP", "JPY", "AUD", "CAD", "CHF"])
            )
    ...
    def _get_random_descriptions(self, category: NewsCategory) -> List[str]:
        ...
        """
        # Simple implementation with generic descriptions
        descriptions = [
    ...
    def generate_economic_calendar(
        ...
        # Generate regular high-impact events
    ...
    def _generate_regular_economic_events(self, start_date: datetime, end_date: datetime) -> None:
        """
        ...
        Args:
            ...
            end_date: End date
        """
        # US Fed meetings (typically every 6 weeks)
        fed_meetings = [
            (2025, 1, 29),  # Example dates for 2025
            (2025, 3, 19),
            (2025, 4, 30),
            (2025, 6, 11),
            (2025, 7, 30),
        ...
        for year, month, day in fed_meetings:
            meeting_date = datetime(year, month, day, 14, 0)  # 2:00 PM ET
            ...
            if start_date <= meeting_date <= end_date:
                event = NewsEvent(
                    ...
                    description="The Federal Reserve announces its monetary policy decision.",
                    release_time=meeting_date,
                    affected_currencies=["USD"],
                    expected_value=None,
                    actual_value=None,
                    previous_value=None,
                    sentiment_impact={"USD": SentimentLevel.NEUTRAL},  # Will be determined at event time
                    technical_impact=TechnicalImpact.VOLATILITY_SPIKE,
                    price_impact_factor=0.0,  # Will be determined at event time
                    volatility_impact_factor=2.0,
                    liquidity_impact_factor=0.8,
        ...
        while current_month <= end_date:
            ...
            first_day = current_month.weekday()  # Monday is 0
            days_until_friday = (4 - first_day) % 7  # Friday is 4
            ...
            if first_friday <= end_date:
                nfp_date = first_friday.replace(hour=8, minute=30)  # 8:30 AM ET
                ...
                event = NewsEvent(
                    ...
                    actual_value=None,
                    previous_value=None,
                    sentiment_impact={"USD": SentimentLevel.NEUTRAL},
                    technical_impact=TechnicalImpact.VOLATILITY_SPIKE,
                    price_impact_factor=0.0,
                    volatility_impact_factor=2.2,
                    liquidity_impact_factor=0.7,
                    duration_minutes=90
                )
                self.news_calendar.add_event(event)
    ...
    def add_scheduled_event(
        ...
        sentiment_impact = {}
        for currency in affected_currencies:
            sentiment_impact[currency] = SentimentLevel.NEUTRAL  # Will be determined at event time
        ...
        event = NewsEvent(
            ...
            technical_impact=technical_impact,
            price_impact_factor=price_impact_factor,
            volatility_impact_factor=volatility_impact_factor,
            liquidity_impact_factor=1.0 - (volatility_impact_factor - 1.0) * 0.5,  # Inverse relationship with volatility
    ...
    def get_sentiment_data(
        ...
        for symbol in symbols:
            ...
            current_sentiment = self.sentiment_tracker.get_pair_sentiment(base, quote)
            current_value = self.sentiment_tracker.sentiment_to_value(current_sentiment)
            
            # Get historical sentiment for base and quote
            base_history = self.sentiment_tracker.get_historical_sentiment(base, lookback)
            quote_history = self.sentiment_tracker.get_historical_sentiment(quote, lookback)
            
            # Calculate pair sentiment history
            pair_sentiment_history = []
            for i in range(min(len(base_history), len(quote_history))):
                base_value = self.sentiment_tracker.sentiment_to_value(base_history[i])
                quote_value = self.sentiment_tracker.sentiment_to_value(quote_history[i])
                pair_value = base_value - quote_value
                pair_value = max(-1.0, min(1.0, pair_value))
                pair_sentiment_history.append(self.sentiment_tracker.value_to_sentiment(pair_value))
            
            # Store results
            sentiment_data[symbol] = {
                "current_sentiment": current_sentiment.value,
                "current_value": current_value,
                "sentiment_history": [s.value for s in pair_sentiment_history],
                "base_currency_sentiment": self.sentiment_tracker.get_current_sentiment(base).value,
                "quote_currency_sentiment": self.sentiment_tracker.get_current_sentiment(quote).value,
            }
    ...
    def get_upcoming_events(
        self, 
        hours_ahead: int = 24,
        impact_filter: Optional[List[NewsImpactLevel]] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        for event in events:
            event_dict = {
                ...
                "time_until_minutes": int((event.release_time - self.current_time).total_seconds() / 60)
            }
