...
class SimulatedBroker:
    ...
    def __init__(
        ...
        spread_config: Optional[Dict[str, float]] = None,
        ...
        # Default spread configuration (in pips)
        ...
        self.slippage_model = slippage_model or {
            "base_slippage": 0.1,  # Base slippage in pips
            "volume_impact": 0.02,  # Additional pips per 1M volume
            "volatility_impact": 0.5  # Multiplier for volatility effect
        }
    ...
    def place_order(
        ...
        **kwargs
        ...
        validation = self._validate_order(
            symbol, order_type, direction, size, price
        )
    ...
    def get_account_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'margin_level': (self.equity / self.margin_used * 100
                           if self.margin_used > 0 else 100),
    ...
    def _execute_market_order(self, order: Dict[str, Any]) -> None:
        ...
        price = (self.market_data[symbol]['ask']
                ...
                else self.market_data[symbol]['bid'])
    ...
    def _update_position(self, symbol: str) -> None:
        ...
        if position['direction'] == 'buy':
            position['unrealized_pnl'] = (
                (current_bid - position['average_price']) *
                position['size']
            )
        else:
            position['unrealized_pnl'] = (
                (position['average_price'] - current_ask) *
                position['size']
            )
        ...
        if position['stop_loss']:
            if (position['direction'] == 'buy' and
                current_bid <= position['stop_loss']):
                self._close_position(symbol, current_bid)
            elif (position['direction'] == 'sell' and
                  current_ask >= position['stop_loss']):
                self._close_position(symbol, current_ask)
                
        # Check take profit
        if position['take_profit']:
            if (position['direction'] == 'buy' and
                current_bid >= position['take_profit']):
                self._close_position(symbol, current_bid)
            elif (position['direction'] == 'sell' and
                  current_ask <= position['take_profit']):
                self._close_position(symbol, current_ask)
    ...
    def _update_account_metrics(self) -> None:
        ...
        self.margin_used = sum(
            self._calculate_required_margin(p['symbol'], p['size'])
    ...
    def _apply_slippage(
        ...
        volume_pips = (size / 1_000_000) * self.slippage_model['volume_impact']
        slippage_pips += volume_pips
        
        # Add volatility-based slippage if available
        if (symbol in self.market_data and
            self.market_data[symbol].get('volatility')):
            volatility = self.market_data[symbol]['volatility']
            volatility_pips = (
                volatility *
                self.slippage_model['volatility_impact']
            )
