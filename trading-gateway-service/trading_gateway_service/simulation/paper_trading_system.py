...
class PaperTradingSystem:
    ...
    def __init__(
        ...
        # Initialize monitoring first for performance tracking
        self.monitoring = TradingGatewayMonitoring(
    ...
    def _update_risk_metrics(self, market_data: Dict[str, Dict[str, Any]]) -> None:
        """Update risk metrics with new market data."""
        # Calculate current prices for all positions
        current_prices = {
            symbol: data['bid'] if pos.direction == 'buy' else data['ask']
            for symbol, data in market_data.items()
        ...
        metrics = {
            ...
            'margin_used': self.broker.margin_used,
            'unrealized_pnl': sum(
                pos.calculate_pnl(current_prices.get(pos.symbol, pos.entry_price))
                for pos in self.broker.positions.values()
            )
        }
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'stats': self.stats,
            'account': self.broker.get_account_summary(),
            'active_circuit_breakers': self.circuit_breaker.get_active_circuit_breakers(),
            'risk_metrics': self.risk_manager.metrics_history[-1] if self.risk_manager.metrics_history else None
        }
