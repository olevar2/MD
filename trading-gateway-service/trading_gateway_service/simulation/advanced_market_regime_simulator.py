...
from trading_gateway_service.simulation.forex_broker_simulator import (
    ForexBrokerSimulator, MarketRegimeType, Order
)
...
class MarketCondition(str, Enum):
    ...
    NORMAL = "normal"  # Regular market behavior
    HIGH_VOLATILITY = "high_volatility"  # Elevated volatility without trend
    ...
    RANGING_NARROW = "ranging_narrow"  # Tight range
    RANGING_WIDE = "ranging_wide"  # Wide range
    BREAKOUT_BULLISH = "breakout_bullish"  # Bullish breakout
    BREAKOUT_BEARISH = "breakout_bearish"  # Bearish breakout
    REVERSAL_BULLISH = "reversal_bullish"  # Bullish reversal
    REVERSAL_BEARISH = "reversal_bearish"  # Bearish reversal
    LIQUIDITY_GAP = "liquidity_gap"  # Thin market with potential gaps
    FLASH_CRASH = "flash_crash"  # Sudden, severe drop
    FLASH_SPIKE = "flash_spike"  # Sudden, severe spike
    NEWS_REACTION = "news_reaction"  # Post-news increased activity
...
class LiquidityProfile(str, Enum):
    ...
    VERY_HIGH = "very_high"  # Major pairs during peak hours
    HIGH = "high"  # Major pairs during regular hours
    MEDIUM = "medium"  # Cross pairs or off-peak hours
    LOW = "low"  # Minor pairs or illiquid periods
    VERY_LOW = "very_low"  # Exotic pairs or major news events
...
class SimulationScenario:
    ...
    def __init__(
        ...
        mean_reversion_strength: float = 0.0,  # 0.0 (none) to 1.0 (strong)
        price_jump_probability: float = 0.0,
        price_jump_magnitude: float = 0.0,
    ...
    def __str__(self) -> str:
        ...
        return (f"Scenario: {self.name} ({self.symbol}, {self.duration}), "
               f"Condition: {self.market_condition.value}, "
               f"Liquidity: {self.liquidity_profile.value}")
...
class AdvancedMarketRegimeSimulator:
    ...
    def __init__(
        ...
        self.active_conditions = {}  # Symbol -> MarketCondition
        self.session_schedule = self._create_default_session_schedule()
        self.historical_regimes = {}  # Symbol -> List of (timestamp, regime)
        
        # Session-specific spread and volatility multipliers
        self.session_liquidity_map = {
            MarketSession.SYDNEY: 0.8,  # Lower liquidity
            MarketSession.TOKYO: 0.9,
            MarketSession.LONDON: 1.2,  # Higher liquidity
            MarketSession.NEWYORK: 1.2,
            MarketSession.OVERLAP_TOKYO_LONDON: 1.1,
            MarketSession.OVERLAP_LONDON_NEWYORK: 1.3,  # Highest liquidity
        }
        
    def _create_default_session_schedule(self) -> Dict[MarketSession, Tuple[int, int]]:
        """
        ...
        """
        return {
            MarketSession.SYDNEY: (20, 5),       # 20:00-05:00 UTC
            MarketSession.TOKYO: (0, 9),         # 00:00-09:00 UTC
            MarketSession.LONDON: (8, 17),       # 08:00-17:00 UTC
            MarketSession.NEWYORK: (13, 22),     # 13:00-22:00 UTC
    ...
    def get_liquidity_factor(self, timestamp: datetime) -> float:
        ...
        if not active_sessions:
            return 0.7  # Baseline when no major sessions are active
            
        # Get the maximum liquidity among active sessions
        return max(self.session_liquidity_map.get(session, 1.0) 
                  for session in active_sessions)
                  
    def apply_scenario(self, scenario: SimulationScenario, start_time: datetime) -> None:
        """
        ...
        # Get the appropriate regime type from the condition
        ...
        # Configure price impact parameters based on the scenario
        impact_params = {
        ...
        logger.info(f"Applied scenario: {scenario.name} starting at {start_time}")
        
    def _condition_to_regime(self, condition: MarketCondition) -> MarketRegimeType:
        """Map detailed market condition to broader regime type."""
        if condition in [MarketCondition.TRENDING_BULLISH, 
                        MarketCondition.TRENDING_BEARISH]:
            return MarketRegimeType.TRENDING
        elif condition in [MarketCondition.RANGING_NARROW, 
                          MarketCondition.RANGING_WIDE]:
            return MarketRegimeType.RANGING
        elif condition in [MarketCondition.HIGH_VOLATILITY, 
    ...
    def _configure_price_impacts(self, symbol: str, params: Dict[str, float]) -> None:
        """Configure price impact parameters for the broker simulator."""
        # This would interface with the broker simulator's price impact model
        # For now, we'll assume it has methods to configure these parameters
        
        # Example implementation (would need to match actual broker simulator interface):
        ...
        if hasattr(self.broker_simulator, 'set_jump_parameters'):
            self.broker_simulator.set_jump_parameters(
                params['jump_probability'], 
                params['jump_magnitude']
            )
            
    def _add_special_event(
        self, 
        ...
        if 'time_offset_minutes' in event_config:
            event_time = base_time + timedelta(
                minutes=event_config['time_offset_minutes'])
        else:
            event_time = base_time + timedelta(
                minutes=random.randint(5, int(self.current_scenario.duration.total_seconds() / 60) - 5))
                
        # Create and add the event
        event = NewsEvent(
            event_id=f"scenario_{self.current_scenario.name}_{len(self.news_simulator.events)}",
            event_type=event_config.get('event_type'),
            impact_level=event_config.get('impact_level', NewsImpactLevel.HIGH),
            timestamp=event_time,
            currencies_affected=[self.current_scenario.symbol],
            title=event_config.get('title', 'Scenario Event'),
            description=event_config.get('description', ''),
            expected_value=event_config.get('expected_value', 0),
            actual_value=event_config.get('actual_value', 0),
            previous_value=event_config.get('previous_value', 0),
            sentiment_impact=event_config.get('sentiment_impact', SentimentLevel.NEUTRAL),
            volatility_impact=event_config.get('volatility_impact', 1.0),
            price_impact=event_config.get('price_impact', 0.0),
            duration_minutes=event_config.get('duration_minutes', 30)
        )
    ...
    def generate_market_data(
        ...
        if timeframe.endswith('m'):
            ...
            periods = int((end_time - start_time).total_seconds() / (minutes * 60)) + 1
        elif timeframe.endswith('h'):
            hours = int(timeframe[:-1])
            periods = int((end_time - start_time).total_seconds() / (hours * 3600)) + 1
        elif timeframe.endswith('d'):
            days = int(timeframe[:-1])
            periods = int((end_time - start_time).total_seconds() / (days * 86400)) + 1
        else:
            raise ValueError(f"Unsupported timeframe: {timeframe}")
        
        # Generate timestamps
        timestamps = [start_time + timedelta(minutes=i * minutes) 
                     for i in range(periods)]
        
        # Get the current active condition or default to normal
        condition = self.active_conditions.get(symbol, MarketCondition.NORMAL)
        
        # Generate price data based on condition
    ...
    def _get_base_price(self, symbol: str) -> float:
        ...
        # Check if broker simulator has current prices
        try:
        ...
        # Default prices by currency pair
        default_prices = {
            'EUR/USD': 1.1000,
            'GBP/USD': 1.2500,
            'USD/JPY': 110.00,
    ...
    def _get_volatility(self, symbol: str) -> float:
        ...
        # Use custom volatility map if provided
        if symbol in self.base_volatility_map:
            return self.base_volatility_map[symbol]
            
        # Default volatilities by currency pair (daily)
        default_volatility = {
            'EUR/USD': 0.005,  # 0.5% daily volatility
            'GBP/USD': 0.007,
            'USD/JPY': 0.006,
            'USD/CHF': 0.006,
            'AUD/USD': 0.008,
    ...
    def _generate_price_series(
        ...
        # Assuming timestamps are uniformly spaced
        if len(timestamps) > 1:
            minutes_diff = (timestamps[1] - timestamps[0]).total_seconds() / 60
            # Scale from daily volatility (assumed in _get_volatility)
            timeframe_factor = np.sqrt(minutes_diff / (24 * 60))
            adjusted_volatility = volatility * timeframe_factor
        else:
            adjusted_volatility = volatility / np.sqrt(252 * 24 * 60)  # Assuming daily vol
            
        # Further adjust volatility by condition factor
        ...
        for i in range(1, periods):
            # Get session liquidity impact
            liquidity = liquidity_factors[i]
            
            # Calculate drift component
            drift = params['trend'] * base_price * 0.0001  # Scale trend to reasonable magnitude
            
            # Calculate mean reversion if enabled
            if params['mean_reversion'] > 0 and i > 1:
                # Mean revert to the average of recent prices
                lookback = min(20, i)
                mean_price = np.mean(prices[i-lookback:i])
                mean_reversion = params['mean_reversion'] * (mean_price - prices[i-1]) * 0.1
            else:
                mean_reversion = 0
                
            # Calculate random component (scaled by volatility and liquidity)
            random_change = np.random.normal(0, adjusted_volatility / np.sqrt(liquidity))
            
            # Calculate price jump if applicable
            if np.random.random() < params['jump_probability']:
                jump_direction = 1 if np.random.random() > 0.5 else -1
                jump_size = np.random.exponential(params['jump_size']) * base_price * jump_direction
            else:
                jump_size = 0
                
            # Calculate the total price change
            price_change = drift + mean_reversion + random_change * prices[i-1] + jump_size
    ...
    def _get_condition_parameters(self, condition: MarketCondition) -> Dict[str, float]:
        ...
        condition_params = {
            ...
            MarketCondition.TRENDING_BULLISH: {
                'trend': 0.5,
                'mean_reversion': -0.1  # Anti-mean reversion
            },
            MarketCondition.TRENDING_BEARISH: {
                'trend': -0.5,
                'mean_reversion': -0.1
        ...
        # Update with condition-specific parameters
    ...
    def _generate_ohlcv(
        ...
        # Generate random intrabar movements
        n = len(prices)
        
        opens = prices.copy()
        closes = prices.copy()
        
        # Add some randomness to opens (except first candle)
        opens[1:] = opens[1:] * (1 + np.random.normal(0, 0.0002, n-1))
        
        # Generate candle body and wicks
        ...
        # Generate spreads based on liquidity and volatility
        base_spread = self.base_spread_map.get(symbol, self._get_default_spread(symbol))
        liquidity_factors = np.array([self.get_liquidity_factor(ts) for ts in timestamps])
        volatility_factor = np.std(prices) / (prices[0] * 0.001)  # Normalized volatility
        
        # Higher spreads during low liquidity and high volatility
        spreads = base_spread * (1.5 - 0.5 * liquidity_factors) * (0.8 + 0.4 * volatility_factor)
    ...
    def generate_scenario_library(self) -> Dict[str, SimulationScenario]:
        ...
        scenarios['normal_market'] = SimulationScenario(
            name='normal_market',
            symbol='EUR/USD',
            duration=timedelta(days=1),
            market_condition=MarketCondition.NORMAL,
            liquidity_profile=LiquidityProfile.HIGH,
            description='Standard market conditions with normal liquidity and volatility'
        )
        
        # Trending scenarios
        scenarios['strong_uptrend'] = SimulationScenario(
            name='strong_uptrend',
            symbol='EUR/USD',
            duration=timedelta(days=1),
        ...
        scenarios['strong_downtrend'] = SimulationScenario(
        ...
        # Ranging scenarios
        scenarios['tight_range'] = SimulationScenario(
            name='tight_range',
            symbol='EUR/USD',
            duration=timedelta(days=1),
            market_condition=MarketCondition.RANGING_NARROW,
            liquidity_profile=LiquidityProfile.MEDIUM,
            volatility_factor=0.6,
            mean_reversion_strength=0.7,
            description='Tight consolidation with strong mean reversion'
        )
        
        scenarios['wide_range'] = SimulationScenario(
            name='wide_range',
            symbol='EUR/USD',
            duration=timedelta(days=1),
            market_condition=MarketCondition.RANGING_WIDE,
            liquidity_profile=LiquidityProfile.MEDIUM,
            volatility_factor=1.3,
            mean_reversion_strength=0.5,
            description='Wide range consolidation with elevated volatility'
        )
        
        # Breakout scenarios
        scenarios['bullish_breakout'] = SimulationScenario(
            ...
            description='Bullish breakout with increased volatility and volume'
        )
        
        # Volatility scenarios
        scenarios['high_volatility'] = SimulationScenario(
        ...
        scenarios['nfp_release'] = SimulationScenario(
            ...
            description='High impact news event with significant price reaction'
        )
        
        # Low liquidity scenarios
        scenarios['low_liquidity'] = SimulationScenario(
            name='low_liquidity',
            symbol='EUR/USD',
            ...
            price_jump_magnitude=0.002,
            description='Very low liquidity condition with wide spreads and potential gaps'
        )
        
        # Extreme events
        scenarios['flash_crash'] = SimulationScenario(
            name='flash_crash',
            symbol='EUR/USD',
            ...
            description='Sudden market crash with extreme volatility and poor liquidity'
        )
        
        return scenarios
    
    def get_random_scenario(self, symbol: Optional[str] = None) -> SimulationScenario:
        """
        ...
        if not scenarios:
            # Create a default scenario if none match
    ...
    def create_curriculum(
        ...
        scenario_duration: timedelta = timedelta(hours=8)
        ...
        # Level 1 (Easiest): Normal and simple trending markets
        curriculum[1] = [
            SimulationScenario(
                name='level1_normal',
                symbol=symbol,
            ...
            SimulationScenario(
                name='level1_mild_trend',
                symbol=symbol,
                duration=scenario_duration,
                market_condition=MarketCondition.TRENDING_BULLISH,
                liquidity_profile=LiquidityProfile.HIGH,
                trend_strength=0.3,
                description='Level 1: Mild bullish trend'
            )
        ]
        
        # Level 2: Introduce ranging markets and reversals
        curriculum[2] = [
            SimulationScenario(
                name='level2_ranging',
                symbol=symbol,
                ...
                description='Level 2: Wide ranging market'
            ),
            SimulationScenario(
                name='level2_reversal',
                symbol=symbol,
                duration=scenario_duration,
                market_condition=MarketCondition.REVERSAL_BULLISH,
                liquidity_profile=LiquidityProfile.MEDIUM,
                volatility_factor=1.2,
                description='Level 2: Market reversal with increased volatility'
            )
        ]
        
        # Level 3: Higher volatility, breakouts, and mild news events
        curriculum[3] = [
            SimulationScenario(
                name='level3_volatility',
                symbol=symbol,
                duration=scenario_duration,
                market_condition=MarketCondition.HIGH_VOLATILITY,
                liquidity_profile=LiquidityProfile.MEDIUM,
                volatility_factor=1.8,
                description='Level 3: Higher volatility market'
            ),
            SimulationScenario(
                name='level3_breakout',
                symbol=symbol,
                duration=scenario_duration,
                market_condition=MarketCondition.BREAKOUT_BULLISH,
                liquidity_profile=LiquidityProfile.MEDIUM,
                trend_strength=0.7,
                price_jump_probability=0.03,
                description='Level 3: Bullish breakout'
            ),
            SimulationScenario(
                name='level3_news',
                symbol=symbol,
                ...
                description='Level 3: Medium-impact news event'
            )
        ]
        
        # Level 4: Low liquidity, stronger news events, and trend changes
        curriculum[4] = [
            SimulationScenario(
                name='level4_liquidity',
                symbol=symbol,
                duration=scenario_duration,
                market_condition=MarketCondition.LIQUIDITY_GAP,
                liquidity_profile=LiquidityProfile.LOW,
                spread_factor=2.0,
                price_jump_probability=0.05,
                description='Level 4: Low liquidity conditions'
            ),
            SimulationScenario(
                name='level4_major_news',
                symbol=symbol,
                duration=scenario_duration,
                market_condition=MarketCondition.NEWS_REACTION,
                liquidity_profile=LiquidityProfile.LOW,
                ...
                description='Level 4: High-impact news event'
            ),
            SimulationScenario(
                name='level4_trend_change',
                symbol=symbol,
                duration=scenario_duration,
                market_condition=MarketCondition.TRENDING_BULLISH,
                liquidity_profile=LiquidityProfile.MEDIUM,
                ...
                special_events=[{
                    ...
                    'time_offset_minutes': int(scenario_duration.total_seconds() / 60 / 2),
                    'description': 'Sudden trend reversal',
                    'price_impact': -0.005
                }],
                description='Level 4: Trend with sudden reversal'
            )
        ]
        
        # Level 5 (Hardest): Extreme events, flash crashes, and complex scenarios
        curriculum[5] = [
            SimulationScenario(
                name='level5_flash_crash',
                symbol=symbol,
                duration=scenario_duration,
                market_condition=MarketCondition.FLASH_CRASH,
                liquidity_profile=LiquidityProfile.VERY_LOW,
                ...
                special_events=[
                    {
                        'event_type': 'ECONOMIC_DATA',
                        'impact_level': NewsImpactLevel.HIGH,
                        'time_offset_minutes': int(scenario_duration.total_seconds() / 60 / 3),
                        'volatility_impact': 3.0,
                        'price_impact': 0.006
                    },
                    {
                        ...
                        'impact_level': NewsImpactLevel.HIGH,
                        'time_offset_minutes': int(scenario_duration.total_seconds() / 60 * 2/3),
                        'volatility_impact': 2.5,
                        'price_impact': -0.008
                    }
                ],
                description='Level 5: Complex scenario with multiple high-impact events'
            ),
            SimulationScenario(
                name='level5_extreme_volatility',
                symbol=symbol,
                duration=scenario_duration,
                market_condition=MarketCondition.HIGH_VOLATILITY,
                liquidity_profile=LiquidityProfile.VERY_LOW,
