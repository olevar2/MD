...
FOREX_TRADING_RUNBOOKS = {'market_data': {'title':
    'Market Data Incident Response', 'description':
    'Procedures for handling market data issues in forex trading',
    'incidents': [{'title': 'Price Feed Interruption', 'severity': 'high',
    'description': 'Loss of price feed from primary data source',
    'immediate_actions': ['Identify affected currency pairs and timeframes',
    ...
    'Check system logs for connection errors'], 'resolution_steps': [
    'Restore primary data connection',
    'Verify data consistency before resuming trading',
    'Gradually re-enable automated trading',
    'Document incident and update monitoring if needed'], 'prevention': [
    'Implement redundant data sources',
    'Add data quality monitoring alerts',
    'Regular connectivity tests with providers']}, {'title':
    'Price Spikes or Anomalies', 'severity': 'medium', 'description':
    'Unusual price movements that may be data errors', 'immediate_actions':
    ['Identify affected currency pairs',
    'Compare data across multiple sources',
    'Flag suspicious movements for investigation',
    'Apply spike filters to affected feeds'], 'root_cause_investigation': [
    ...
    'Review historical data for similar patterns'], 'resolution_steps': [
    ...
    'Document findings for future reference'], 'prevention': [
    'Enhance data validation rules', 'Implement cross-source verification',
    'Improve anomaly detection algorithms']}, {'title':
    'Delayed Market Data', 'severity': 'medium', 'description':
    'Market data arriving with significant delay', 'immediate_actions': [
    ...
    'Contact provider to check for known issues'], 'resolution_steps': [
    'Optimize data processing pipeline',
    'Increase resource allocation if needed',
    'Implement better queuing mechanisms for data processing',
    'Update latency monitoring thresholds'], 'prevention': [
    'Regular performance testing of data pipeline',
    'Implement redundant data paths',
    'Add detailed latency monitoring by currency pair']}]}, 'execution': {
    'title': 'Trade Execution Incident Response', 'description':
    'Procedures for handling trade execution issues in forex trading',
    'incidents': [{'title': 'Order Rejection Pattern', 'severity': 'high',
    'description': 'Multiple orders being rejected by broker',
    'immediate_actions': ['Pause automated order submission',
    ...
    'root_cause_investigation': ['Analyze patterns in rejected orders',
    ...
    'prevention': ['Regular review of broker requirements and limits',
    'Pre-submission validation for order parameters',
    'Monitoring of rejection rates with alerting']}, {'title':
    'Severe Slippage Events', 'severity': 'high', 'description':
    'Orders executing with significantly higher slippage than expected',
    'immediate_actions': [
    ...
    'prevention': ['Implement pre-trade liquidity analysis',
    'Add volatility-based execution rules',
    'Include economic calendar awareness in trading system']}, {'title':
    'Order Stuck in Pending', 'severity': 'medium', 'description':
    'Orders remain in pending state without execution or rejection',
    'immediate_actions': ['Identify affected orders and their details',
    ...
    'resolution_steps': ['Cancel and replace stuck orders if needed',
    'Re-establish broker connectivity',
    'Verify order book consistency after resolution',
    'Implement order timeout and automatic cancellation'], 'prevention': [
    'Add order state monitoring with timeouts',
    'Implement automatic cancellation for orders pending too long',
    'Add redundant order status checking mechanisms']}]}, 'connectivity': {
    'title': 'Connectivity Incident Response', 'description':
    'Procedures for handling connectivity issues with brokers and data providers'
    , 'incidents': [{'title': 'Complete Broker Disconnection', 'severity':
    'critical', 'description': 'Total loss of connectivity with broker',
    'immediate_actions': [
    ...
    'Review recent changes to connection parameters'], 'resolution_steps':
    ...
    'prevention': ['Implement redundant connectivity paths',
    ...
    'title': 'Intermittent Connection Drops', 'severity': 'high',
    ...
    'Test alternative network paths if available'], 'resolution_steps': [
    ...
    'Update reconnection logic based on findings'], 'prevention': [
    'Implement connection quality monitoring',
    'Add circuit breakers triggered by connection instability',
    'Develop robust state recovery mechanisms',
    'Regular stress testing of reconnection capabilities']}]}, 'risk': {
    'title': 'Risk Management Incident Response', 'description':
    'Procedures for handling risk-related incidents in forex trading',
    'incidents': [{'title': 'Margin Call or Closeout', 'severity':
    'critical', 'description':
    'Account approaching or experiencing margin call situation',
    'immediate_actions': ['Pause all new position opening',
    ...
    'resolution_steps': ['Reduce positions in order of risk contribution',
    'Add funds to account if appropriate and available',
    'Adjust position sizing algorithms to prevent recurrence',
    'Document lessons learned and update risk limits'], 'prevention': [
    ...
    'Diversify position concentration risks']}, {'title':
    'Drawdown Limit Breach', 'severity': 'high', 'description':
    'Trading strategy exceeding maximum allowable drawdown',
    'immediate_actions': ['Pause trading for the affected strategy',
    'Document current drawdown level and circumstances',
    'Check for correlation with market conditions',
    'Verify drawdown calculation accuracy'], 'root_cause_investigation': [
    ...
    'resolution_steps': ['Adjust strategy parameters if issues identified',
    'Consider reducing position size upon restart',
    'Implement gradual recovery mode with lower risk',
    'Add additional risk filters before re-enabling'], 'prevention': [
    ...
    'Multiple drawdown measures (time-based windows)']}, {'title':
    'Unexpected Exposure Increase', 'severity': 'high', 'description':
    'Sudden increase in exposure to specific currency or risk factor',
    'immediate_actions': ['Identify source of increased exposure',
    ...
    'Add monitoring for similar patterns in the future'], 'prevention': [
    ...
    'Add currency exposure limits and alerts']}]}, 'system': {'title':
    'System Incident Response', 'description':
    'Procedures for handling system-related incidents in forex trading',
    'incidents': [{'title': 'Performance Degradation', 'severity': 'high',
    'description':
    'System showing significant latency or processing delays',
    'immediate_actions': [
    ...
    'Scale resources if possible and needed'], 'root_cause_investigation':
    ['Analyze system metrics to identify bottlenecks',
    'Check for recent changes in data volume or traffic',
    'Review recent deployments or configuration changes',
    'Examine database performance and query patterns'], 'resolution_steps':
    ['Optimize identified bottlenecks',
    'Increase resources for affected components',
    'Consider architectural changes if structural issue',
    'Implement performance testing to validate improvements'], 'prevention':
    ['Regular performance testing and benchmarking',
    'Resource utilization monitoring and alerts',
    'Capacity planning and scaling policies',
    'Performance regression testing for deployments']}, {'title':
    'Critical Service Failure', 'severity': 'critical', 'description':
    'Failure of a critical system component affecting trading',
    'immediate_actions': [
    ...
    'Resume operations with careful monitoring'], 'prevention': [
    'Implement robust service monitoring and health checks',
    'Regular disaster recovery testing',
    'Component redundancy for critical services',
    'Automated recovery procedures where possible']}, {'title':
    'Data Consistency Issue', 'severity': 'high', 'description':
    'Inconsistencies detected between system components',
    'immediate_actions': [
    ...
    'root_cause_investigation': ['Trace data flow between affected systems',
    'Review recent schema or data model changes',
    'Check for race conditions or timing issues',
    'Verify transaction integrity mechanisms'], 'resolution_steps': [
    ...
    'Document recovery process for future reference'], 'prevention': [
...
def get_runbook_for_incident(category: str, title: Optional[str]=None,
    severity: Optional[str]=None, runbooks: Optional[Dict[str, Any]]=None
    ...
    if not title and not severity:
        return {'title': category_runbooks.get('title',
            f'{category.capitalize()} Incidents'), 'description':
            category_runbooks.get('description', '')}
