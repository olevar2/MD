...
class OrderRequest:
    ...
    def __init__(self, instrument: str, order_type: OrderType, direction:
        OrderDirection, quantity: float, price: Optional[float]=None,
        stop_loss: Optional[float]=None, take_profit: Optional[float]=None,
        time_in_force: Optional[str]=None, good_till_date: Optional[
        datetime]=None, client_order_id: Optional[str]=None, **extra_params):
        """
        ...
        self.time_in_force = time_in_force or 'GTC'
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'instrument': self.instrument, 'order_type': self.
            order_type.value, 'direction': self.direction.value, 'quantity':
            self.quantity, 'price': self.price, 'stop_loss': self.stop_loss,
            'take_profit': self.take_profit, 'time_in_force': self.
            time_in_force, 'good_till_date': self.good_till_date.isoformat(
            ) if self.good_till_date else None, 'client_order_id': self.
            client_order_id, 'created_at': self.created_at.isoformat(),
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'OrderRequest':
        ...
        return cls(instrument=data.get('instrument', ''), order_type=
            order_type, direction=direction, quantity=data.get('quantity', 
            0), price=data.get('price'), stop_loss=data.get('stop_loss'),
            take_profit=data.get('take_profit'), time_in_force=data.get(
            'time_in_force', 'GTC'), good_till_date=good_till_date,
            client_order_id=data.get('client_order_id'), **extra_params)
...
class ExecutionReport:
    ...
    def __init__(self, broker_order_id: str, client_order_id: str,
        instrument: str, status: OrderStatus, filled_quantity: float=0.0,
        average_price: Optional[float]=None, rejection_reason: Optional[str
        ]=None, transaction_time: Optional[datetime]=None, **extra_data):
        """
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'broker_order_id': self.broker_order_id, 'client_order_id':
            self.client_order_id, 'instrument': self.instrument, 'status':
            self.status.value, 'filled_quantity': self.filled_quantity,
            'average_price': self.average_price, 'rejection_reason': self.
            rejection_reason, 'transaction_time': self.transaction_time.
            isoformat(), 'extra_data': self.extra_data}
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'ExecutionReport':
        ...
        if data.get('transaction_time'):
            if isinstance(data['transaction_time'], str):
                transaction_time = datetime.fromisoformat(data[
                    'transaction_time'])
            else:
                transaction_time = data['transaction_time']
        extra_data = data.get('extra_data', {})
        return cls(broker_order_id=data.get('broker_order_id', ''),
            client_order_id=data.get('client_order_id', ''), instrument=
            data.get('instrument', ''), status=status, filled_quantity=data
            .get('filled_quantity', 0.0), average_price=data.get(
            'average_price'), rejection_reason=data.get('rejection_reason'),
            transaction_time=transaction_time, **extra_data)
...
class PositionUpdate:
    ...
    def __init__(self, instrument: str, position_id: str, quantity: float,
        average_price: float, unrealized_pl: float, realized_pl: float=0.0,
        margin_used: Optional[float]=None, update_time: Optional[datetime]=
        None, **extra_data):
        """
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'instrument': self.instrument, 'position_id': self.
            position_id, 'quantity': self.quantity, 'average_price': self.
            average_price, 'unrealized_pl': self.unrealized_pl,
            'realized_pl': self.realized_pl, 'margin_used': self.
            margin_used, 'update_time': self.update_time.isoformat(),
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'PositionUpdate':
        ...
        extra_data = data.get('extra_data', {})
        return cls(instrument=data.get('instrument', ''), position_id=data.
            get('position_id', ''), quantity=data.get('quantity', 0.0),
            average_price=data.get('average_price', 0.0), unrealized_pl=
            data.get('unrealized_pl', 0.0), realized_pl=data.get(
            'realized_pl', 0.0), margin_used=data.get('margin_used'),
            update_time=update_time, **extra_data)
...
class AccountUpdate:
    ...
    def __init__(self, account_id: str, balance: float, equity: float,
        margin_used: float, margin_available: float, currency: str,
        update_time: Optional[datetime]=None, **extra_data):
        """
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'account_id': self.account_id, 'balance': self.balance,
            'equity': self.equity, 'margin_used': self.margin_used,
            'margin_available': self.margin_available, 'currency': self.
            currency, 'update_time': self.update_time.isoformat(),
            'extra_data': self.extra_data}
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'AccountUpdate':
        ...
        if data.get('update_time'):
            if isinstance(data['update_time'], str):
                ...
                update_time = data['update_time']
        extra_data = data.get('extra_data', {})
        return cls(account_id=data.get('account_id', ''), balance=data.get(
            'balance', 0.0), equity=data.get('equity', 0.0), margin_used=
            data.get('margin_used', 0.0), margin_available=data.get(
            'margin_available', 0.0), currency=data.get('currency', 'USD'),
            update_time=update_time, **extra_data)
