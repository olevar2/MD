...
This module provides database connectivity for the Trading Gateway Service.
"""
import logging
import asyncio
...
from trading_gateway_service.config import get_database_config
...
class Database:
    ...
    def __init__(self, config: Optional[DatabaseConfig]=None, logger:
    ...
    async def connect(self) ->None:
        ...
        if self.pool is not None:
            return
        try:
            self.pool = await asyncpg.create_pool(host=self.config.host,
                port=self.config.port, user=self.config.username, password=
                self.config.password, database=self.config.database,
                min_size=self.config.min_connections or 1, max_size=self.
                config.max_connections or 10)
            self.logger.info(
                f'Connected to database {self.config.database} at {self.config.host}:{self.config.port}'
                )
