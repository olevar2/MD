...
from portfolio_management_service.models.position import (
    Position, PositionCreate, PositionUpdate, PositionStatus, PositionPerformance
)
from portfolio_management_service.models.account import AccountDetails
...
async def get_position(position_id: str = Path(..., description="ID of the position to retrieve")):
...
async def update_position(
    ...
    position_id: str = Path(..., description="ID of the position to update")
):
    """Update a position."""
...
async def close_position(
    position_id: str = Path(..., description="ID of the position to close"),
...
@router.get("/accounts/{account_id}/historical-performance", response_model=Dict[str, Any])
async def get_historical_performance(
    ...
    try:
        return portfolio_service.get_historical_performance(account_id, period_days)
    except Exception as e:
        logger.error(f"Error retrieving historical performance: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to retrieve historical performance: {str(e)}")


@router.post("/accounts/{account_id}/update-prices", response_model=Dict[str, Any])
async def update_position_prices(
    price_updates: Dict[str, float] = Body(..., description="Map of symbols to current prices"),
    account_id: str = Path(..., description="ID of the account")
):
    """Update current prices for all open positions and recalculate unrealized PnL."""
    try:
        return portfolio_service.update_position_prices(account_id, price_updates)
    except Exception as e:
...
@router.post("/accounts/{account_id}/daily-snapshot", response_model=Dict[str, Any])
async def create_daily_snapshot(
    ...
    if not snapshot:
        raise HTTPException(status_code=404, detail=f"Failed to create snapshot for account {account_id}")
