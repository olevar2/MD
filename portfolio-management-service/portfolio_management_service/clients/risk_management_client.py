...
class RiskManagementClient:
    ...
    def __init__(self, base_url: str = None):
        ...
        self.base_url = base_url or os.environ.get('RISK_MANAGEMENT_URL', "http://risk-management-service:8007")
    ...
    async def close(self):
        ...
        operation_name="risk_management_api_request"
    )
    async def _make_request(self, method: str, endpoint: str, params: Optional[Dict] = None, data: Optional[Dict] = None) -> Dict:
        ...
        try:
            ...
            if 400 <= e.status < 500 and e.status != 429:
                ...
                # Convert to a non-retryable exception by wrapping
                raise ValueError(f"Client error {e.status}: {e.message}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error calling Risk Management Service: {str(e)}")
            raise
            
    async def check_position_risk(self, account_id: str, position_size: float, symbol: str, entry_price: float) -> Dict[str, Any]:
        """
    ...
    async def get_account_exposure(self, account_id: str) -> float:
        ...
        # Extract the account exposure from the portfolio risk result
    ...
    async def get_symbol_exposure(self, account_id: str, symbol: str) -> float:
        ...
        """
        # Check portfolio risk to get all exposures
        result = await self.check_portfolio_risk(account_id)
        
        # Extract symbol exposure from the result
        symbol_exposures = result.get("symbol_exposures", {})
        return symbol_exposures.get(symbol, 0.0)
    
    async def calculate_position_size(self, account_balance: float, risk_per_trade_pct: float, 
                                     stop_loss_pips: float, pip_value: float, leverage: float = 1.0) -> Dict[str, float]:
        """
        ...
        return await self._make_request("POST", "calculate/position-size", params=params)
    
    async def calculate_value_at_risk(self, portfolio_value: float, daily_returns: List[float],
                                     confidence_level: float = 0.95, time_horizon_days: int = 1) -> Dict[str, float]:
        """
        ...
        return await self._make_request("POST", "calculate/var", params=params, data=data)
    
    async def calculate_drawdown_risk(self, current_balance: float, historical_balances: List[float],
                                     max_drawdown_limit_pct: float = 20.0) -> Dict[str, Any]:
        """
        ...
        return await self._make_request("POST", "calculate/drawdown", params=params, data=data)
    
    async def calculate_max_trades(self, account_balance: float, risk_per_trade_pct: float, 
                                  portfolio_heat_limit_pct: float = 20.0) -> Dict[str, int]:
        """
        ...
        return await self._make_request("POST", "calculate/max-trades", params=params)
