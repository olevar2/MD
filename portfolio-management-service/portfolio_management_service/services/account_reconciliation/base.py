...
class ReconciliationBase:
    ...
    async def _publish_event(self, event_type: str, event_data: Dict[str, Any]) -> None:
        ...
        if self.event_publisher:
            try:
                ...
                logger.error(f"Error publishing event {event_type}: {str(e)}", exc_info=True)
    ...
    async def _create_reconciliation_report(
        ...
        """
        # Count discrepancies by severity and field type
        ...
        report = {
            ...
            "discrepancies": {
                "total_count": len(reconciliation_result["discrepancies"]),
                "by_severity": dict(severity_counts),
                "by_field_type": dict(field_counts),
                "total_monetary_difference": total_monetary_diff,
                "details": reconciliation_result["discrepancies"]
            },
    ...
    async def _update_internal_data(
        ...
        Args:
            ...
            value: New value
        """
        # In a real implementation, this would update the internal database
        logger.info(f"Updating internal data for account {account_id}, field {field} to {value}")
    ...
    async def _update_broker_data(
        ...
        logger.info(f"Updating broker data for account {account_id}, field {field} to {value}")
