...
class TaxReportingService:
    ...
    def __init__(self, trade_repository=None, account_repository=None,
        transaction_repository=None, tax_jurisdiction_repository=None,
    ...
    async def generate_tax_report(self, account_id: str, jurisdiction: str,
        report_type: TaxReportType, year: int, period: TaxPeriod=TaxPeriod.
        YEAR, period_number: Optional[int]=None, start_date: Optional[date]
        =None, end_date: Optional[date]=None, include_unrealized: bool=False
        ) ->TaxReport:
        """
        ...
        """
        start_date, end_date = self._determine_date_range(year, period,
            period_number, start_date, end_date)
        jurisdiction_rules = (await self.tax_jurisdiction_repository.
            get_jurisdiction(jurisdiction))
        transactions = (await self.transaction_repository.
            get_transactions_by_date_range(account_id, start_date, end_date))
        ...
        if report_type == TaxReportType.TRANSACTION_SUMMARY:
            ...
            report_data = self._generate_capital_gains_report(trades,
                jurisdiction_rules)
        ...
        report = TaxReport(account_id=account_id, jurisdiction=jurisdiction,
            report_type=report_type, start_date=start_date, end_date=
            end_date, generation_date=datetime.utcnow(), data=report_data)
        logger.info(
            f'Generated {report_type.value} tax report for account {account_id}, jurisdiction {jurisdiction}, period {start_date} to {end_date}'
            )
        return report

    async def export_tax_report(self, report: TaxReport, format: str='json',
        file_path: Optional[str]=None) ->Union[str, bytes, None]:
        """
        ...
        if format.lower() == 'json':
            export_data = json.dumps(report_data, indent=2, default=self.
                _json_serializer)
            ...
            export_data = self._convert_to_excel(report_data, report.
                report_type)
            content_type = (
        ...
        if file_path:
            mode = 'w' if format.lower() == 'csv' or format.lower(
                ) == 'json' else 'wb'
            encoding = 'utf-8' if format.lower() == 'csv' or format.lower(
                ) == 'json' else None
    ...
    def _convert_to_csv(self, report_data: Dict, report_type: TaxReportType
        ) ->str:
        ...
        if report_type == TaxReportType.TRANSACTION_SUMMARY:
            ...
            for category, data in report_data.get('categories', {}).items():
                writer.writerow([category, data.get('count', 0), data.get(
                    'total', 0), data.get('currency', 'USD')])
        elif report_type == TaxReportType.CAPITAL_GAINS:
            writer = csv.writer(output)
            writer.writerow(['Type', 'Gains', 'Losses', 'Net', 'Tax Rate',
                'Estimated Tax'])
            writer.writerow(['Short Term', report_data.get(
                'short_term_gains', 0), report_data.get('short_term_losses',
                0), report_data.get('net_short_term', 0), report_data.get(
                'short_term_tax_rate', 'N/A'), report_data.get(
                'estimated_short_term_tax', 0)])
            writer.writerow(['Long Term', report_data.get('long_term_gains',
                0), report_data.get('long_term_losses', 0), report_data.get
                ('net_long_term', 0), report_data.get('long_term_tax_rate',
                'N/A'), report_data.get('estimated_long_term_tax', 0)])
            writer.writerow(['Total', report_data.get('short_term_gains', 0
                ) + report_data.get('long_term_gains', 0), report_data.get(
                'short_term_losses', 0) + report_data.get(
                'long_term_losses', 0), report_data.get(
                'total_net_capital_gain', 0), 'N/A', report_data.get(
                'estimated_total_tax', 0)])
        elif report_type == TaxReportType.COMPLETE:
            writer = csv.writer(output)
            writer.writerow(['SUMMARY SECTION'])
            writer.writerow(['Total Taxable Amount', report_data.get(
                'total_taxable_amount', 0)])
            writer.writerow(['Estimated Total Tax', report_data.get(
                'estimated_total_tax', 0)])
            writer.writerow([])
            capital_gains = report_data.get('capital_gains', {})
            writer.writerow(['CAPITAL GAINS SECTION'])
            writer.writerow(['Net Short Term', capital_gains.get(
                'net_short_term', 0)])
            writer.writerow(['Net Long Term', capital_gains.get(
                'net_long_term', 0)])
            writer.writerow(['Total Net Capital Gain', capital_gains.get(
                'total_net_capital_gain', 0)])
            writer.writerow([])
            interest = report_data.get('interest_income', {})
            writer.writerow(['INTEREST INCOME SECTION'])
            writer.writerow(['Total Interest Income', interest.get(
    ...
    def _convert_to_excel(self, report_data: Dict, report_type: TaxReportType
        ) ->bytes:
        """Convert report data to Excel format"""
        if report_type == TaxReportType.TRANSACTION_SUMMARY:
            df_categories = pd.DataFrame([{'Category': category, 'Count':
                data.get('count', 0), 'Total': data.get('total', 0),
                'Currency': data.get('currency', 'USD')} for category, data in
                report_data.get('categories', {}).items()])
            with io.BytesIO() as output:
                with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                    df_categories.to_excel(writer, sheet_name='Summary',
                        index=False)
                return output.getvalue()
        elif report_type == TaxReportType.CAPITAL_GAINS:
            df = pd.DataFrame([{'Type': 'Short Term', 'Gains': report_data.
                get('short_term_gains', 0), 'Losses': report_data.get(
                'short_term_losses', 0), 'Net': report_data.get(
                'net_short_term', 0), 'Tax Rate': report_data.get(
                'short_term_tax_rate', 'N/A'), 'Estimated Tax': report_data
                .get('estimated_short_term_tax', 0)}, {'Type': 'Long Term',
                'Gains': report_data.get('long_term_gains', 0), 'Losses':
                report_data.get('long_term_losses', 0), 'Net': report_data.
                get('net_long_term', 0), 'Tax Rate': report_data.get(
                'long_term_tax_rate', 'N/A'), 'Estimated Tax': report_data.
                get('estimated_long_term_tax', 0)}, {'Type': 'Total',
                'Gains': report_data.get('short_term_gains', 0) +
                report_data.get('long_term_gains', 0), 'Losses': 
                report_data.get('short_term_losses', 0) + report_data.get(
                'long_term_losses', 0), 'Net': report_data.get(
                'total_net_capital_gain', 0), 'Tax Rate': 'N/A',
            ...
            with io.BytesIO() as output:
                with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                    pd.DataFrame({'Metric': ['Total Taxable Amount',
                        'Estimated Total Tax'], 'Value': [report_data.get(
                        'total_taxable_amount', 0), report_data.get(
                        'estimated_total_tax', 0)]}).to_excel(writer,
                        sheet_name='Summary', index=False)
                    capital_gains = report_data.get('capital_gains', {})
                    pd.DataFrame([{'Type': 'Short Term', 'Gains':
                        capital_gains.get('short_term_gains', 0), 'Losses':
                        capital_gains.get('short_term_losses', 0), 'Net':
                        capital_gains.get('net_short_term', 0)}, {'Type':
                        'Long Term', 'Gains': capital_gains.get(
                        'long_term_gains', 0), 'Losses': capital_gains.get(
                        'long_term_losses', 0), 'Net': capital_gains.get(
                        'net_long_term', 0)}]).to_excel(writer, sheet_name=
                        'Capital Gains', index=False)
                    interest = report_data.get('interest_income', {})
                    pd.DataFrame({'Total Interest Income': [interest.get(
                        'total_interest_income', 0)]}).to_excel(writer,
                        sheet_name='Interest Income', index=False)
                return output.getvalue()
        else:
            flattened_data = self._flatten_dict(report_data)
    ...
    def _flatten_dict(self, d: Dict, parent_key: str='', sep: str='_') ->Dict:
        ...
        for k, v in d.items():
            ...
            if isinstance(v, dict) and not (isinstance(v, dict) and any(
                isinstance(i, dict) for i in v.values())):
    ...
    async def calculate_holding_period_classifications(self, trades: List[
        Dict[str, Any]], jurisdiction_rules: Dict[str, Any]) ->Dict[str,
        List[Dict[str, Any]]]:
        """
        ...
        """
        long_term_threshold_days = jurisdiction_rules.get(
            'long_term_threshold_days', 365)
        classified_trades = {HoldingPeriod.SHORT_TERM.value: [],
            HoldingPeriod.LONG_TERM.value: [], HoldingPeriod.OTHER.value: []}
        for trade in trades:
    ...
    async def detect_wash_sales(self, account_id: str, trades: List[Dict[
        str, Any]], jurisdiction_rules: Dict[str, Any],
        detection_window_days: int=30) ->List[Dict[str, Any]]:
        """
        ...
        if not jurisdiction_rules.get('apply_wash_sale_rules', True):
            return []
        window_days = jurisdiction_rules.get('wash_sale_window_days',
            detection_window_days)
        sorted_trades = sorted(trades, key=lambda t: (t.get('instrument_id',
            ''), self._parse_date(t.get('exit_date'))))
        wash_sales = []
        instrument_trades = {}
            ...
            for i, trade in enumerate(instrument_specific_trades):
                ...
                for j, other_trade in enumerate(instrument_specific_trades):
                    ...
                    if wash_window_start <= entry_date <= wash_window_end:
                        wash_sales.append({'loss_trade_id': trade.get('id'),
                            'repurchase_trade_id': other_trade.get('id'),
                            'instrument_id': instrument, 'loss_amount':
                            trade.get('realized_pl'), 'loss_date':
                            exit_date.isoformat(), 'repurchase_date':
                            entry_date.isoformat(), 'days_between': (
                            entry_date - exit_date).days, 'disallowed_loss':
                            min(abs(trade.get('realized_pl', 0)),
                            other_trade.get('volume', 0))})
                        if jurisdiction_rules.get('single_wash_sale_match',
                            False):
                            break
        return wash_sales
    ...
    async def get_jurisdiction_rules(self, jurisdiction: str) ->Dict[str, Any]:
        ...
        """
        return await self.tax_jurisdiction_repository.get_jurisdiction_rules(
            jurisdiction)

    async def map_account_to_jurisdiction(self, account_id: str,
        jurisdiction: str, tax_identifier: Optional[str]=None,
        ...
        """
        mapping_info = {'jurisdiction': jurisdiction, 'mapped_at': datetime
            .utcnow()}
        if tax_identifier:
            mapping_info['tax_identifier'] = tax_identifier
        if additional_info:
            mapping_info['additional_info'] = additional_info
        await self.account_repository.update_tax_information(account_id,
            mapping_info)
        logger.info(
            f'Mapped account {account_id} to tax jurisdiction {jurisdiction}')

    def _determine_date_range(self, year: int, period: TaxPeriod,
        period_number: Optional[int]=None, start_date: Optional[date]=None,
        ...
        if period == TaxPeriod.MONTH:
            if not period_number or period_number < 1 or period_number > 12:
                raise ValueError(
                    'Valid month number (1-12) required for monthly reporting')
            last_day = calendar.monthrange(year, period_number)[1]
            return date(year, period_number, 1), date(year, period_number,
                last_day)
        if period == TaxPeriod.QUARTER:
            if not period_number or period_number < 1 or period_number > 4:
                raise ValueError(
                    'Valid quarter number (1-4) required for quarterly reporting'
                    )
    ...
    def _categorize_transactions(self, transactions: List[Dict[str, Any]],
        trades: List[Dict[str, Any]], jurisdiction_rules: Dict[str, Any]
        ...
        """
        categories = {'trading_gains': [], 'interest_income': [],
            'dividend_income': [], 'fees_and_commissions': [], 'other': []}
        for transaction in transactions:
            ...
            elif transaction_type in (TransactionType.COMMISSION,
                TransactionType.FEE):
    ...
    def _generate_transaction_summary(self, categorized_transactions: Dict[
        str, List[Dict[str, Any]]], jurisdiction_rules: Dict[str, Any]) ->Dict[
        str, Any]:
        """
        ...
        """
        summary = {'total_transactions': sum(len(cat) for cat in
            categorized_transactions.values()), 'categories': {}}
        for category, transactions in categorized_transactions.items():
            ...
            summary['categories'][category] = {'count': category_count,
                'total': category_sum, 'currency': transactions[0].get(
                'currency', 'USD'), 'transactions': transactions}
        summary['total_trading_gains'] = summary.get('categories', {}).get(
            'trading_gains', {}).get('total', 0)
        summary['total_interest_income'] = summary.get('categories', {}).get(
            'interest_income', {}).get('total', 0)
        summary['total_dividend_income'] = summary.get('categories', {}).get(
            'dividend_income', {}).get('total', 0)
        summary['total_fees_and_commissions'] = summary.get('categories', {}
            ).get('fees_and_commissions', {}).get('total', 0)
        return summary

    def _generate_profit_loss_report(self, categorized_transactions: Dict[
        str, List[Dict[str, Any]]], trades: List[Dict[str, Any]],
        jurisdiction_rules: Dict[str, Any]) ->Dict[str, Any]:
        """
        ...
        for instrument, instrument_trades in trades_by_instrument.items():
            ...
            pl_by_instrument[instrument] = {'total_pl': total_pl,
                'trade_count': trade_count, 'total_volume': total_volume,
                'average_pl_per_trade': total_pl / trade_count if 
        ...
        profit_loss = {'gross_trading_profit': total_trading_gains,
            'fees_and_commissions': total_fees, 'net_trading_profit': 
            total_trading_gains - total_fees, 'by_instrument':
            pl_by_instrument, 'winning_trades': sum(1 for t in trades if t.
            get('realized_pl', 0) > 0), 'losing_trades': sum(1 for t in
            trades if t.get('realized_pl', 0) <= 0), 'total_trades': len(
            trades)}
        return profit_loss

    def _generate_capital_gains_report(self, trades: List[Dict[str, Any]],
        jurisdiction_rules: Dict[str, Any]) ->Dict[str, Any]:
        """
        ...
        """
        short_term_gains = sum(t.get('realized_pl', 0) for t in trades if t
            .get('realized_pl', 0) > 0)
        short_term_losses = sum(t.get('realized_pl', 0) for t in trades if 
            t.get('realized_pl', 0) < 0)
        report = {'short_term_gains': short_term_gains, 'short_term_losses':
            short_term_losses, 'net_short_term': short_term_gains +
            short_term_losses, 'long_term_gains': 0, 'long_term_losses': 0,
            'net_long_term': 0, 'total_net_capital_gain': short_term_gains +
            short_term_losses}
        if 'short_term_rate' in jurisdiction_rules:
            report['short_term_tax_rate'] = jurisdiction_rules[
                'short_term_rate']
            report['estimated_short_term_tax'] = report['net_short_term'
                ] * jurisdiction_rules['short_term_rate']
            report['estimated_total_tax'] = report['estimated_short_term_tax']
        return report

    def _generate_interest_income_report(self, categorized_transactions:
        Dict[str, List[Dict[str, Any]]], jurisdiction_rules: Dict[str, Any]
        ) ->Dict[str, Any]:
        """
        ...
        report = {'total_interest_income': total_interest, 'transactions':
            interest_transactions}
        if 'interest_income_rate' in jurisdiction_rules:
            report['interest_income_tax_rate'] = jurisdiction_rules[
                'interest_income_rate']
            report['estimated_tax'] = total_interest * jurisdiction_rules[
                'interest_income_rate']
        return report

    def _generate_complete_tax_report(self, categorized_transactions: Dict[
        str, List[Dict[str, Any]]], trades: List[Dict[str, Any]],
        jurisdiction_rules: Dict[str, Any]) ->Dict[str, Any]:
        """
        ...
        profit_loss = self._generate_profit_loss_report(
            categorized_transactions, trades, jurisdiction_rules)
        capital_gains = self._generate_capital_gains_report(trades,
            jurisdiction_rules)
        interest_income = self._generate_interest_income_report(
            categorized_transactions, jurisdiction_rules)
        complete_report = {'transaction_summary': transaction_summary,
            'profit_loss': profit_loss, 'capital_gains': capital_gains,
            'interest_income': interest_income, 'total_taxable_amount': 
            capital_gains['total_net_capital_gain'] + interest_income[
    ...
    async def _calculate_unrealized_gains_losses(self, account_id: str,
        as_of_date: date, jurisdiction_rules: Dict[str, Any]) ->Dict[str, Any]:
        """
        ...
        unrealized_data = {'total_unrealized_pl': total_unrealized,
            'by_instrument': {}, 'positions': positions, 'note':
            'Unrealized gains/losses are not taxable in most jurisdictions until realized.'
            }
        for position in positions:
            ...
            unrealized_data['by_instrument'][instrument]['total_unrealized_pl'
                ] += unrealized_pl
            unrealized_data['by_instrument'][instrument]['position_count'] += 1
        if 'unrealized_gains_taxable' in jurisdiction_rules:
            unrealized_data['taxable'] = jurisdiction_rules[
                'unrealized_gains_taxable']
            if jurisdiction_rules['unrealized_gains_taxable']:
                unrealized_data['estimated_tax'] = unrealized_data[
                    'total_unrealized_pl'] * jurisdiction_rules.get(
                    'unrealized_gains_rate', 0)
