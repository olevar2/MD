...
def format_error_response(
    ...
    service: str = "portfolio-management-service"
) -> Dict[str, Any]:
    """
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def forex_platform_exception_handler(request: Request, exc: ForexTradingPlatformError):
        ...
        # Portfolio management specific errors
        if isinstance(exc, PortfolioNotFoundError) or isinstance(exc, PositionNotFoundError):
            status_code = status.HTTP_404_NOT_FOUND
        elif isinstance(exc, InsufficientBalanceError):
            status_code = status.HTTP_403_FORBIDDEN
        elif isinstance(exc, PortfolioOperationError):
            status_code = status.HTTP_400_BAD_REQUEST
        elif isinstance(exc, AccountReconciliationError) or isinstance(exc, TaxCalculationError):
