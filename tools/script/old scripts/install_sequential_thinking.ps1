# PowerShell script to install server-sequential-thinking for VS Code Insiders
...
# Add the server-sequential-thinking configuration
if ($configContent.mcpServers -eq $null) {
    $configContent.mcpServers = @{}
}

$configContent.mcpServers | Add-Member -MemberType NoteProperty -Name "server-sequential-thinking" -Value @{
    command = "cmd"
    args = @(
        "/c",
        "npx",
