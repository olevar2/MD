# PowerShell script to install @mem0ai/mem0-memory-mcp for VS Code Insiders
...
# Add the @mem0ai/mem0-memory-mcp configuration
if ($configContent.mcpServers -eq $null) {
    $configContent.mcpServers = @{}
}

$configContent.mcpServers | Add-Member -MemberType NoteProperty -Name "mem0-memory-mcp" -Value @{
    command = "cmd"
    args = @(
        "/c",
        "npx",
