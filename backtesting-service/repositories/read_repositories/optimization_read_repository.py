    ...
    async def get_all(self) -> List[OptimizationResult]:
        ...
        for filename in os.listdir(self.storage_path):
            if filename.endswith(".json"):
                try:
                    optimization_id = filename[:-5]  # Remove .json extension
                    optimization = await self.get_by_id(optimization_id)
                    if optimization:
                        optimizations.append(optimization)
                except Exception as e:
                    logger.error(f"Error loading optimization from {filename}: {e}")
        
        return optimizations
    
    async def get_by_criteria(self, criteria: Dict[str, Any]) -> List[OptimizationResult]:
        """
