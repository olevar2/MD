# PowerShell script to install @upstash/context7-mcp for VS Code Insiders
...
# Add the @upstash/context7-mcp configuration
if ($configContent.mcpServers -eq $null) {
    $configContent.mcpServers = @{}
}

$configContent.mcpServers | Add-Member -MemberType NoteProperty -Name "context7-mcp" -Value @{
    command = "cmd"
    args = @(
        "/c",
        "npx",
