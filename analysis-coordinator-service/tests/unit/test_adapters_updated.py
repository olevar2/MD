...
from analysis_coordinator_service.utils.resilience import CircuitBreaker
...
async def test_market_analysis_adapter_analyze_market():
    ...
    with patch.object(adapter, '_make_request', return_value={"result": "success"}):
        # Act
...
async def test_market_analysis_adapter_error_handling():
    ...
    with patch.object(adapter, '_make_request', side_effect=Exception("Market analysis request failed: Internal Server Error")):
        # Act & Assert
...
async def test_causal_analysis_adapter_error_handling():
    ...
    # Mock the _make_request method to raise an exception
    with patch.object(adapter, '_make_request', side_effect=Exception("Causal graph request failed: Internal Server Error")):
        # Act & Assert
        with pytest.raises(Exception) as excinfo:
...
async def test_backtesting_adapter_error_handling():
    ...
    # Mock the _make_request method to raise an exception
    with patch.object(adapter, '_make_request', side_effect=Exception("Backtest request failed: Internal Server Error")):
        # Act & Assert
        with pytest.raises(Exception) as excinfo:
