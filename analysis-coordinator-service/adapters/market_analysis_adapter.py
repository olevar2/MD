        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload) as response:
                if response.status != 200:
                    ...
                    logger.error(f"Market analysis request failed with status {response.status}: {error_text}")
                    raise Exception(f"Market analysis request failed: {error_text}")
                
                result = await response.json()
                logger.info(f"Market analysis request successful")
                return result
    ...
    async def get_patterns(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload) as response:
                if response.status != 200:
                    error_text = await response.text()
                    logger.error(f"Pattern recognition request failed with status {response.status}: {error_text}")
                    raise Exception(f"Pattern recognition request failed: {error_text}")
                
                result = await response.json()
                logger.info(f"Pattern recognition request successful")
                return result
    ...
    async def get_support_resistance(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload) as response:
                if response.status != 200:
                    error_text = await response.text()
                    logger.error(f"Support/resistance request failed with status {response.status}: {error_text}")
                    raise Exception(f"Support/resistance request failed: {error_text}")
                
                result = await response.json()
                logger.info(f"Support/resistance request successful")
                return result
    ...
    async def get_market_regime(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload) as response:
                if response.status != 200:
                    error_text = await response.text()
                    logger.error(f"Market regime request failed with status {response.status}: {error_text}")
                    raise Exception(f"Market regime request failed: {error_text}")
                
                result = await response.json()
                logger.info(f"Market regime request successful")
                return result
    ...
    async def get_correlations(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload) as response:
                if response.status != 200:
                    error_text = await response.text()
                    logger.error(f"Correlation analysis request failed with status {response.status}: {error_text}")
                    raise Exception(f"Correlation analysis request failed: {error_text}")
                
                result = await response.json()
                logger.info(f"Correlation analysis request successful")
                return result