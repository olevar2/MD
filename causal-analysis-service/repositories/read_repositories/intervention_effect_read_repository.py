...
class InterventionEffectReadRepository(ReadRepository):
    ...
    def __init__(self, storage_path: Optional[str]=None):
        ...
        self.storage_path = storage_path or os.environ.get(
            'INTERVENTION_EFFECT_STORAGE_PATH', './data/intervention_effects')
    ...
    async def get_by_id(self, id: str) ->Optional[InterventionEffect]:
        ...
        if os.path.exists(file_path):
            try:
                ...
                logger.debug(
                    f'Loaded intervention effect {id} from file storage')
                return effect
            except Exception as e:
                logger.error(
                    f'Error loading intervention effect {id} from file: {e}')
        ...
        for filename in os.listdir(self.storage_path):
            if filename.endswith('.json'):
                try:
                    ...
                    logger.error(
                        f'Error loading intervention effect from {filename}: {e}'
                        )
        return effects
