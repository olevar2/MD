...
from common_lib.ml.feature_interfaces import IFeatureProvider, IFeatureTransformer, IFeatureSelector, FeatureType, FeatureScope, SelectionMethod
...
class FeatureProviderAdapter(IFeatureProvider):
    ...
    def __init__(self, provider_instance=None):
        ...
        self.feature_cache = {}
        self.metadata_cache = {}
        self.default_features = ['close', 'open', 'high', 'low', 'volume',
            'sma_10', 'sma_20', 'sma_50', 'sma_200', 'ema_10', 'ema_20',
            'ema_50', 'ema_200', 'rsi_14', 'macd', 'macd_signal',
            'macd_histogram', 'bb_upper', 'bb_middle', 'bb_lower', 'atr_14',
    ...
    @with_exception_handling
    def get_feature_data(self, feature_names: List[str], symbol: str,
        timeframe: str, start_date: str=None, end_date: str=None, limit:
        int=None) ->pd.DataFrame:
        """
        ...
        if self.provider:
            try:
                return self.provider.get_feature_data(feature_names=
                    feature_names, symbol=symbol, timeframe=timeframe,
        ...
        if limit:
            ...
            start_date_dt = pd.to_datetime(start_date
                ) if start_date else end_date_dt - timedelta(days=100)
        date_range = pd.date_range(start=start_date_dt, end=end_date_dt,
            freq='D')
        np.random.seed(42)
        data = pd.DataFrame(index=date_range)
        if 'close' in feature_names or any(f for f in feature_names if f.
            startswith('sma_') or f.startswith('ema_')):
            close = 100 + np.cumsum(np.random.normal(0, 1, len(date_range)))
            data['close'] = close
        if 'open' in feature_names:
            data['open'] = data['close'].shift(1) * (1 + np.random.normal(0,
                0.01, len(date_range)))
            data['open'].iloc[0] = data['close'].iloc[0] * 0.99
        if 'high' in feature_names:
            data['high'] = data[['open', 'close']].max(axis=1) * (1 + np.
                abs(np.random.normal(0, 0.01, len(date_range))))
        if 'low' in feature_names:
            data['low'] = data[['open', 'close']].min(axis=1) * (1 - np.abs
                (np.random.normal(0, 0.01, len(date_range))))
        ...
        for feature in feature_names:
            ...
            if feature.startswith('sma_'):
                ...
                data[feature] = 25 + 10 * np.sin(np.linspace(0, 10, len(
                    date_range)))
                ...
                if all(col in data.columns for col in ['high', 'low', 'close']
                    ...
                    k = 100 * ((data['close'] - lowest_low) / (highest_high -
                        lowest_low))
                    if feature == 'stoch_k':
                        data[feature] = k
    ...
    @with_exception_handling
    def compute_features(self, feature_definitions: List[Dict[str, Any]],
        ...
        if self.provider:
            try:
                return self.provider.compute_features(feature_definitions=
                    feature_definitions, data=data)
            except Exception as e:
        ...
        for feature_def in feature_definitions:
            ...
            try:
                if not all(col in data.columns for col in source_columns):
                    logger.warning(
                        f'Missing source columns for feature {name}: {source_columns}'
                        )
                    continue
                if feature_type == FeatureType.RAW:
                    ...
                    for i, col in enumerate(source_columns):
                        col_name = name if len(source_columns
                            ) == 1 else f'{name}_{i}'
                        min_val = data[col].min()
                        max_val = data[col].max()
                        if max_val > min_val:
                            result[col_name] = (data[col] - min_val) / (max_val
                                 - min_val)
                        else:
                            result[col_name] = 0.5
                    ...
                    for i, col in enumerate(source_columns):
                        ...
                        result[col_name] = data[col].diff(window).apply(lambda
                            x: 1 if x > 0 else -1 if x < 0 else 0)
                elif feature_type == FeatureType.CROSSOVER:
                    if len(source_columns) >= 2:
                        method = params.get('method', 'binary')
                        if method == 'binary':
                            result[name] = np.where(data[source_columns[0]] >
                                data[source_columns[1]], 1, np.where(data[
                                source_columns[0]] < data[source_columns[1]
                                ], -1, 0))
                        elif method == 'distance':
                            result[name] = (data[source_columns[0]] - data[
                                source_columns[1]]) / data[source_columns[1]]
                    ...
                    for i, col in enumerate(source_columns):
                        ...
                        for category in categories:
                            result[f'{name}_{category}'] = (data[col] ==
                                category).astype(int)
                else:
                    for i, col in enumerate(source_columns):
                        col_name = name if len(source_columns
    ...
    def get_feature_metadata(self, feature_name: str) ->Dict[str, Any]:
        ...
        if self.provider:
            try:
                return self.provider.get_feature_metadata(feature_name=
                    feature_name)
            except Exception as e:
        ...
        metadata = {'name': feature_name, 'description':
            f'Feature {feature_name}', 'type': 'numeric', 'created_at':
            datetime.now().isoformat(), 'updated_at': datetime.now().
            isoformat(), 'version': '1.0.0', 'tags': []}
        if feature_name.startswith('sma_') or feature_name.startswith('ema_'):
            window = feature_name.split('_')[1]
            indicator_type = ('Simple Moving Average' if feature_name.
                startswith('sma_') else 'Exponential Moving Average')
            metadata.update({'description':
                f'{indicator_type} with window {window}', 'category':
                'trend', 'parameters': {'window': int(window)}, 'tags': [
                'moving_average', 'trend']})
        elif feature_name == 'rsi_14':
            metadata.update({'description':
                'Relative Strength Index with period 14', 'category':
                'oscillator', 'parameters': {'period': 14}, 'tags': [
                'oscillator', 'overbought', 'oversold'], 'min_value': 0,
                'max_value': 100})
        elif feature_name in ['macd', 'macd_signal', 'macd_histogram']:
            metadata.update({'description':
                f"MACD {feature_name.split('_')[1] if '_' in feature_name else 'line'}"
                , 'category': 'momentum', 'parameters': {'fast_period': 12,
                'slow_period': 26, 'signal_period': 9}, 'tags': ['momentum',
                'trend']})
        elif feature_name in ['bb_upper', 'bb_middle', 'bb_lower']:
            band_type = feature_name.split('_')[1]
            metadata.update({'description':
                f'Bollinger Band {band_type} line', 'category':
                'volatility', 'parameters': {'period': 20, 'std_dev': 2},
...
class FeatureTransformerAdapter(IFeatureTransformer):
    ...
    @with_exception_handling
    def transform(self, data: pd.DataFrame, feature_definitions: List[Dict[
        str, Any]]=None, fit: bool=False) ->pd.DataFrame:
        """
        Transform features.
        ...
        if self.transformer:
            try:
                return self.transformer.transform(data=data,
                    feature_definitions=feature_definitions, fit=fit)
            except Exception as e:
        ...
        if not feature_definitions:
            ...
            feature_definitions = [{'name': col, 'source_columns': [col],
                'feature_type': FeatureType.NORMALIZED, 'params': {}} for
                col in numeric_cols]
        for feature_def in feature_definitions:
            name = feature_def.get('name', 'unknown')
            feature_type = feature_def.get('feature_type', FeatureType.RAW)
                ...
                if feature_type == FeatureType.NORMALIZED:
                    for i, col in enumerate(source_columns):
                        ...
                        if fit or col not in self.scaler_params:
                            min_val = data[col].min()
                            max_val = data[col].max()
                            self.scaler_params[col] = {'min': min_val,
                                'max': max_val}
...
class FeatureSelectorAdapter(IFeatureSelector):
    ...
    def select_features(self, features: pd.DataFrame, target: pd.Series=
        None, method: str=None, n_features: int=None) ->pd.DataFrame:
        """
        Select features.
        ...
        if self.selector:
            try:
                return self.selector.select_features(features=features,
                    target=target, method=method, n_features=n_features)
            except Exception as e:
                logger.warning(f'Error selecting features: {str(e)}')
        if n_features is None:
            n_features = min(10, features.shape[1])
        if method is None:
            method = (SelectionMethod.CORRELATION if target is not None else
        ...
        if method == SelectionMethod.CORRELATION and target is not None:
            ...
            sorted_correlations = sorted(correlations.items(), key=lambda x:
                x[1], reverse=True)
            self.feature_importances = {f: c for f, c in sorted_correlations}
            self.selected_features = [f for f, c in sorted_correlations[:
                n_features]]
            ...
            self.selected_features = [f for f, v in sorted_variances[:
                n_features]]
        else:
            self.selected_features = list(features.columns[:n_features])
            self.feature_importances = {f: (1.0) for f in self.
                selected_features}
    ...
    def save_state(self, path: str) ->None:
        ...
        try:
            state = {'feature_importances': self.feature_importances,
