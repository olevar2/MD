...
from core.model_inference_cache import cache_model_inference
...
class PredictionType(Enum):
    ...
    PRICE_DIRECTION = auto()
    PRICE_TARGET = auto()
    VOLATILITY = auto()
    REVERSAL_PROBABILITY = auto()
    MOMENTUM = auto()
    RANGE_BOUND = auto()
...
class PredictionResult:
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        result = {'model_id': self.model_id, 'timestamp': self.timestamp.
            isoformat(), 'prediction_type': self.prediction_type.name,
            'horizon': self.horizon.name, 'horizon_seconds': self.horizon.
            value, 'value': self.value, 'probability': self.probability,
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'PredictionResult':
        ...
        return cls(model_id=data['model_id'], timestamp=datetime.
            fromisoformat(data['timestamp']), prediction_type=
            PredictionType[data['prediction_type']], horizon=
            PredictionHorizon[data['horizon']], value=data['value'],
            probability=data['probability'], confidence_interval=
            confidence_interval, feature_importance=feature_importance)
...
class ModelRegistry:
    ...
    def list_models(self, prediction_type: Optional[PredictionType]=None,
        ...
        if prediction_type:
            results = [m for m in results if m.prediction_type ==
                prediction_type]
        if instrument:
            results = [m for m in results if m.instrument is None or m.
    ...
    def _save_metadata(self, model_id: str, metadata: ModelMetadata) ->None:
        ...
        data = {'id': metadata.id, 'name': metadata.name, 'description':
            metadata.description, 'version': metadata.version, 'created_at':
            metadata.created_at.isoformat(), 'prediction_type': metadata.
            prediction_type.name, 'horizon': metadata.horizon.name,
            'features': metadata.features, 'metrics': metadata.metrics,
            'parameters': metadata.parameters, 'instrument': metadata.
            instrument, 'timeframe': metadata.timeframe}
        ...
        for metadata_path in metadata_files:
            ...
            try:
                with open(metadata_path, 'r') as f:
                    data = json.load(f)
                metadata = ModelMetadata(id=data['id'], name=data['name'],
                    description=data['description'], version=data['version'
                    ], created_at=datetime.fromisoformat(data['created_at']
                    ), prediction_type=PredictionType[data[
                    'prediction_type']], horizon=PredictionHorizon[data[
                    'horizon']], features=data['features'], metrics=data[
                    'metrics'], parameters=data['parameters'], instrument=
                    data.get('instrument'), timeframe=data.get('timeframe'))
...
class FeedbackCollector:
    ...
    def record_feedback(self, prediction_id: str, actual_value: Any,
        ...
        feedback = {'prediction_id': prediction_id, 'model_id': prediction.
            model_id, 'prediction': prediction.to_dict(), 'actual_value':
            actual_value, 'metrics': metrics, 'feedback_time': datetime.now
            ().isoformat()}
    ...
    def register_indicator_usage(self, model_id: str, indicator_names: List
        [str]) ->None:
        """
    ...
    def get_indicator_performance(self, indicator_name: str) ->Dict[str, Any]:
        ...
        return {'models_count': len(model_ids), 'models_with_metrics': len(
            model_metrics), 'avg_metrics': avg_metrics}
    ...
    def _prediction_path(self, prediction_id: str) ->str:
        ...
        return os.path.join(self.feedback_dir,
            f'prediction_{prediction_id}.json')
    ...
    def _save_prediction(self, prediction_id: str, prediction: PredictionResult
        ) ->None:
    ...
    def _save_feedback(self, prediction_id: str, feedback: Dict[str, Any]
        ) ->None:
...
class ModelConnector:
    ...
    def __init__(self, registry: Optional[ModelRegistry]=None, feedback:
        Optional[FeedbackCollector]=None):
        """
    ...
    @cache_model_inference(ttl=1800)
    @with_exception_handling
    def predict(self, model_id: str, symbol: str, timeframe: str, features:
        ...
        missing_features = [f for f in metadata.features if f not in
            features.columns]
        if missing_features:
        ...
        try:
            ...
            if hasattr(model, 'feature_importances_'):
                ...
                importance = model.coef_[0] if len(model.coef_.shape
                    ) > 1 else model.coef_
                feature_importance = dict(zip(metadata.features, importance))
            result = PredictionResult(model_id=model_id, timestamp=datetime
                .now(), prediction_type=metadata.prediction_type, horizon=
                metadata.horizon, value=value, probability=probability,
            ...
            logger.debug(
                f'Prediction with model {model_id} took {prediction_time:.3f}s'
                )

    def update_feature_data(self, indicator_name: str, data: pd.DataFrame
        ) ->None:
        """
    ...
    def evaluate_prediction(self, prediction_id: str, actual_value: Any
        ) ->Dict[str, float]:
        """
        ...
        if prediction.prediction_type == PredictionType.PRICE_DIRECTION:
            ...
            metrics['percentage_error'
                ] = error / actual_value if actual_value != 0 else float('inf')
