...
@patch.object(ReconciliationService, "reconcile_training_data")
async def test_reconcile_training_data(mock_reconcile, mock_reconciliation_result):
    """Test the reconcile_training_data endpoint."""
    # Configure the mock
    mock_reconcile.return_value = mock_reconciliation_result
    
    # Make the request
    response = client.post(
        "/api/v1/reconciliation/training-data",
        json={
            "model_id": "test-model",
            "version": "1.0",
...
@patch.object(ReconciliationService, "reconcile_inference_data")
async def test_reconcile_inference_data(mock_reconcile, mock_reconciliation_result):
    """Test the reconcile_inference_data endpoint."""
    # Configure the mock
    mock_reconcile.return_value = mock_reconciliation_result
    
    # Make the request
    response = client.post(
        "/api/v1/reconciliation/inference-data",
        json={
            "model_id": "test-model",
            "version": "1.0",
