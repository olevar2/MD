...
from ml_integration_service.adapters.feature_store_adapter import (
    FeatureProviderAdapter,
    FeatureTransformerAdapter,
    FeatureSelectorAdapter
)
...
from common_lib.indicators.indicator_interfaces import (
    IBaseIndicator, IAdvancedIndicator, IIndicatorAdapter
)
...
class TestIndicatorFeatures(unittest.TestCase):
    ...
    def _add_indicators(self):
        ...
        # Use adapter-based approach instead of direct imports
        # Create simple indicator implementations
        class SimpleMA:
            ...
            def calculate(self, data):
                result = data.copy()
                result[f'{self.name}_{self.period}'] = data['close'].rolling(window=self.period).mean()
                return result

        # Create adapters for indicators
        ...
        # Calculate indicators using adapters
