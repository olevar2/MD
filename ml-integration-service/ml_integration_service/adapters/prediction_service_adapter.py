...
class PredictionServiceAdapter(IMLPredictionService):
    ...
    async def get_prediction(
        ...
        try:
            if self.model_service:
                # Use the provided model service
                result = await self.model_service.get_prediction(
                    model_id=model_id,
                    inputs=inputs,
    ...
    async def get_batch_predictions(
        ...
        try:
            ...
            results = await get_batch_predictions(
                model_id=model_id,
                batch_inputs=batch_inputs,
                version_id=version_id,
                explanation_required=explanation_required,
    ...
    async def get_model_metadata(
        ...
        try:
            ...
            if self.model_service:
                # Use the provided model service
                metadata = await self.model_service.get_model_metadata(
                    model_id=model_id,
                    version_id=version_id
                )
            ...
            result = await get_model_metadata(
                model_id=model_id,
                version_id=version_id
            )
...
class SignalGeneratorAdapter(IMLSignalGenerator):
    ...
    async def generate_trading_signals(
        ...
        try:
            if self.signal_service:
                # Use the provided signal service
    ...
    async def get_model_performance(
        ...
        try:
            if self.signal_service:
                # Use the provided signal service
                return await self.signal_service.get_model_performance(
                    model_id=model_id,
                    symbol=symbol,
                    timeframe=timeframe,
