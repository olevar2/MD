...
# We'll use a relative import assuming the framework is accessible
...
T = TypeVar('T')  # Result type for inference
...
class ParallelInferenceProcessor:
    ...
    async def run_inference(self,
                     ...
                     models: List[ModelInferenceSpec],
        ...
        for model in models:
            ...
            # Create a wrapper function to pass both features and model spec
        ...
        for task_id, task_result in results.items():
            ...
            if task_result.success:
                ...
                logger.error(f"Error running inference for model {model_id}: {error_msg}")
        ...
        if errors and len(errors) == len(models):
            ...
            raise ModelPredictionError(
                message=f"Inference failed for all models:\n{error_details}"
            )
        
        return processed_results
    
    async def run_inference_for_instruments(self,
                                     instrument_features: Dict[str, pd.DataFrame],
                                     models: List[ModelInferenceSpec],
                                     inference_func: Callable[[pd.DataFrame, ModelInferenceSpec], Any],
        ...
        # Process each instrument's models
        results = {}
        errors = []
        
        for instrument, features in instrument_features.items():
            try:
                instrument_results = await self.run_inference(
                    features=features,
                    models=models,
                    inference_func=inference_func,
                    timeout=timeout
                ...
                logger.error(f"Error running inference for instrument {instrument}: {str(e)}")
        ...
        if errors and len(errors) == len(instrument_features):
            ...
            raise ModelPredictionError(
                message=f"Inference failed for all instruments:\n{error_details}"
            )
        
        return results
    
    async def run_batch_inference(self,
                           features_batch: List[pd.DataFrame],
                           model: ModelInferenceSpec,
                           inference_func: Callable[[List[pd.DataFrame], ModelInferenceSpec], List[Any]],
        ...
        try:
            # For batch inference, we don't parallelize internally
            # Instead, we run the batch inference function directly
            start_time = datetime.now()
            results = inference_func(features_batch, model)
            execution_time = (datetime.now() - start_time).total_seconds()
            
            logger.debug(f"Batch inference for model {model.model_id} completed in {execution_time:.2f}s")
            
            return results
        except Exception as e:
            error_msg = str(e)
            logger.error(f"Error running batch inference for model {model.model_id}: {error_msg}")
            raise ModelPredictionError(
                message=f"Batch inference failed for model {model.model_id}: {error_msg}",
                model_id=model.model_id
            )
    
    async def run_multi_model_batch_inference(self,
                                      features_batch: List[pd.DataFrame],
                                      models: List[ModelInferenceSpec],
                                      inference_func: Callable[[List[pd.DataFrame], ModelInferenceSpec], List[Any]],
        ...
        for model in models:
            ...
            # Create a wrapper function to pass both features batch and model spec
        ...
        for task_id, task_result in results.items():
            ...
            if task_result.success:
                ...
                error_msg = str(task_result.error) if task_result.error else "Unknown error"
                logger.error(f"Error running batch inference for model {model_id}: {error_msg}")
                errors.append((model_id, error_msg))
        ...
        if errors and len(errors) == len(models):
            error_details = "\n".join([f"{m}: {e}" for m, e in errors])
            raise ModelPredictionError(
                message=f"Batch inference failed for all models:\n{error_details}"
            )
