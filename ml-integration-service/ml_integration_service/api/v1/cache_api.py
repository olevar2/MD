...
from ml_integration_service.caching.model_inference_cache import get_cache_stats, clear_model_cache
from ml_integration_service.caching.feature_vector_cache import get_feature_cache_stats, clear_feature_cache
logger = logging.getLogger(__name__)
router = APIRouter(prefix='/cache', tags=['cache'], responses={(404): {
...
async def get_cache_statistics() ->Dict[str, Any]:
    ...
    try:
        ...
        return {'model_cache': model_stats, 'feature_cache': feature_stats,
            'total_entries': model_stats['total_entries'] + feature_stats[
            'total_entries'], 'total_active_entries': model_stats[
            'active_entries'] + feature_stats['active_entries'],
            'total_expired_entries': model_stats['expired_entries'] +
            feature_stats['expired_entries']}
...
@async_with_exception_handling
async def clear_cache(model_name: str=None, symbol: str=None, clear_all:
    ...
    try:
        if clear_all:
            ...
            return {'status': 'error', 'message':
                'No cache clearing parameters provided'}
    except Exception as e:
