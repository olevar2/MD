...
from ml_integration_service.di.container import get_reconciliation_service
from ml_integration_service.api.security import get_api_key
from ml_integration_service.api.rate_limiter import get_reconciliation_rate_limiter
...
class ReconciliationRequest(BaseModel):
    ...
    model_id: str = Field(..., description="ID of the model to reconcile data for")
    version: Optional[str] = Field(None, description="Version of the model")
    start_time: Optional[datetime] = Field(None, description="Start time for data reconciliation")
    end_time: Optional[datetime] = Field(None, description="End time for data reconciliation")
...
@router.post("/training-data", response_model=ReconciliationResponse, responses={
...
async def reconcile_training_data(
    ...
    req: Request = None,
    rate_limiter: Any = Depends(get_reconciliation_rate_limiter)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"Received training data reconciliation request for model {request.model_id}, "
            f"version {request.version}, strategy {request.strategy.name}"
        )

        result = await reconciliation_service.reconcile_training_data(
        ...
        logger.info(
            f"Completed training data reconciliation for model {request.model_id}, "
        ...
        logger.error(f"Validation error during training data reconciliation: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={
...
@router.post("/inference-data", response_model=ReconciliationResponse, responses={
    400: {"model": ErrorResponse, "description": "Bad Request"},
    401: {"model": ErrorResponse, "description": "Unauthorized"},
    429: {"model": ErrorResponse, "description": "Too Many Requests"},
    500: {"model": ErrorResponse, "description": "Internal Server Error"}
...
async def reconcile_inference_data(
    ...
    try:
        # Log the reconciliation request
        logger.info(
            f"Received inference data reconciliation request for model {request.model_id}, "
            f"version {request.version}, strategy {request.strategy.name}"
        )

        result = await reconciliation_service.reconcile_inference_data(
            model_id=request.model_id,
            version=request.version,
            start_time=request.start_time,
            end_time=request.end_time,
        ...
        # Log the reconciliation result
        logger.info(
            f"Completed inference data reconciliation for model {request.model_id}, "
            f"reconciliation ID: {result.reconciliation_id}, "
            f"status: {result.status.name}, "
            f"discrepancies: {result.discrepancy_count}, "
            f"resolutions: {result.resolution_count}"
        ...
        logger.error(f"Validation error during inference data reconciliation: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={
                "detail": f"Validation error: {str(e)}",
