...
class UserPreferenceManager:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.preferences_dir = self.config.get("preferences_dir", "./user_preferences")
        self.default_preferences = {
            "technical_knowledge": "intermediate",  # beginner, intermediate, advanced
            ...
            "language_style": "standard",  # simple, standard, technical
        ...
        logger.info(f"User preference manager initialized with preferences dir: {self.preferences_dir}")
    ...
    async def update_user_preferences(
        ...
        for key, value in preferences.items():
            if isinstance(value, dict) and key in current_preferences and isinstance(current_preferences[key], dict):
    ...
    async def detect_preferences_from_message(
        ...
        # Detect technical knowledge level
        if any(term in message_lower for term in ["beginner", "new to trading", "learning", "novice", "simple", "basics"]):
            detected_preferences["technical_knowledge"] = "beginner"
        elif any(term in message_lower for term in ["advanced", "experienced", "professional", "expert", "detailed", "technical"]):
            detected_preferences["technical_knowledge"] = "advanced"

        # Detect preferred indicators
        indicators = []
        indicator_terms = {
            "rsi": ["rsi", "relative strength", "relative strength index"],
            "macd": ["macd", "moving average convergence", "convergence divergence"],
            "bollinger": ["bollinger", "bollinger bands", "bands"],
            "ema": ["ema", "moving average", "exponential moving"],
            "sma": ["sma", "simple moving", "simple moving average"],
        ...
        timeframes = {
            "1m": ["1 minute", "1m", "one minute", "1 min"],
            "5m": ["5 minute", "5m", "five minute", "5 min"],
            "15m": ["15 minute", "15m", "fifteen minute", "15 min"],
            "30m": ["30 minute", "30m", "thirty minute", "30 min"],
            "1h": ["1 hour", "1h", "one hour", "hourly", "1 hr"],
            "4h": ["4 hour", "4h", "four hour", "4 hr"],
            "1d": ["daily", "1d", "one day", "day", "daily chart"],
            "1w": ["weekly", "1w", "one week", "week", "weekly chart"]
        }
        ...
        # Detect risk profile
        if any(term in message_lower for term in ["conservative", "cautious", "low risk", "safe", "careful"]):
            detected_preferences["risk_profile"] = "conservative"
        elif any(term in message_lower for term in ["moderate", "balanced", "medium risk"]):
            detected_preferences["risk_profile"] = "moderate"
        elif any(term in message_lower for term in ["aggressive", "high risk", "risky", "speculative"]):
            detected_preferences["risk_profile"] = "aggressive"

        # Detect language style preference
        if any(term in message_lower for term in ["simple", "explain simply", "easy to understand", "beginner friendly"]):
            detected_preferences["language_style"] = "simple"
        elif any(term in message_lower for term in ["standard", "normal", "regular"]):
            detected_preferences["language_style"] = "standard"
        elif any(term in message_lower for term in ["technical", "detailed", "advanced explanation", "in-depth"]):
        ...
        if "trend" in message_lower and "model" in message_lower:
            if "advanced" in message_lower or "latest" in message_lower:
                detected_preferences["preferred_trend_model"] = "trend_classifier_v3"
            elif "simple" in message_lower or "basic" in message_lower:
                detected_preferences["preferred_trend_model"] = "trend_classifier_v1"

        if "price" in message_lower and "model" in message_lower:
            if "advanced" in message_lower or "latest" in message_lower:
                detected_preferences["preferred_price_model"] = "price_movement_predictor_v2"

        # Detect explicit preference settings
        preference_keywords = {
            "prefer": ["prefer", "preference", "like", "want"],
            "use": ["use", "using", "utilize", "with"],
        ...
        if preference_actions and (
            "settings" in message_lower or
            "preferences" in message_lower or
            "profile" in message_lower
        ):
            # This is likely an explicit preference setting request
            # We'll extract more specific preferences
            ...
            if "notification" in message_lower or "alert" in message_lower:
                if "price" in message_lower and "alert" in message_lower:
                    enable = not ("disable" in message_lower or "turn off" in message_lower)
                    detected_preferences["notification_preferences"] = {"price_alerts": enable}
                ...
                if "trade" in message_lower and "signal" in message_lower:
                    ...
                    detected_preferences["notification_preferences"]["trade_signals"] = enable

                if "news" in message_lower:
                    enable = not ("disable" in message_lower or "turn off" in message_lower)
                    if "notification_preferences" not in detected_preferences:
                        detected_preferences["notification_preferences"] = {}
                    detected_preferences["notification_preferences"]["market_news"] = enable

        # Update user preferences if we detected any
