...
from ml_integration_service.models.reconciliation_models import (
...
class ReconciliationRepository:
    ...
    async def create_reconciliation_process(
        self,
        ...
        try:
            ...
            raise DataStorageError(f"Error creating reconciliation process: {str(e)}")
    
    async def update_reconciliation_process(
        self,
    ...
    ) -> Optional[ReconciliationProcess]:
        """
        ...
        try:
            ...
            # Update the reconciliation process
            stmt = (
                update(ReconciliationProcess)
                .where(ReconciliationProcess.id == reconciliation_id)
            ...
            raise DataStorageError(f"Error updating reconciliation process: {str(e)}")
    
    async def get_reconciliation_process(
        self,
        reconciliation_id: str,
        include_discrepancies: bool = False
    ) -> Optional[ReconciliationProcess]:
        """
        ...
        try:
            ...
            if include_discrepancies:
                stmt = (
                    select(ReconciliationProcess)
                    .options(selectinload(ReconciliationProcess.discrepancies))
                    .where(ReconciliationProcess.id == reconciliation_id)
                )
            else:
                stmt = (
            ...
            raise DataFetchError(f"Error getting reconciliation process: {str(e)}")
    ...
    async def add_discrepancy(
        ...
        resolved: bool = False,
        resolution_strategy: Optional[ReconciliationStrategy] = None,
        ...
        try:
            ...
            source_1_value_str = str(source_1_value) if source_1_value is not None else None
            source_2_value_str = str(source_2_value) if source_2_value is not None else None
            resolved_value_str = str(resolved_value) if resolved_value is not None else None
            ...
            discrepancy = Discrepancy(
                ...
                source_1_value=source_1_value_str,
                source_2_id=source_2_id,
                source_2_value=source_2_value_str,
    ...
    async def convert_to_result(
        ...
        stmt = (
            select(Discrepancy)
            .where(Discrepancy.reconciliation_id == reconciliation_process.id)
        )
        result = await self.session.execute(stmt)
        ...
        return ReconciliationResult(
            ...
            status=reconciliation_process.status,
            ...
            start_time=reconciliation_process.start_time,
            end_time=reconciliation_process.end_time,
            discrepancies=discrepancy_objects,
        )
