...
class ModelRobustnessTester:
    ...
    def __init__(
        self,
        model_client: Any,  # ModelRegistryClient instance
        market_simulator: Any  # MarketSimulator instance
    ):
    ...
    def test_extreme_conditions(
        ...
        for scenario in scenarios:
            # Generate synthetic market data for the scenario
            scenario_data = self.market_simulator.generate_scenario(
                scenario["conditions"],
                scenario.get("duration", "1D"),
                scenario.get("volatility", "high")
            )
            
            # Get model predictions
            predictions = self.model_client.get_predictions(model_id, scenario_data)
            
            # Calculate performance metrics
            metrics = self._calculate_metrics(predictions, scenario_data)
            
            # Determine if test passed
            passed = all(metric >= performance_threshold 
                        for metric in metrics.values())
            
            # Generate recommendations if needed
            recommendations = self._generate_recommendations(
                metrics, scenario["conditions"]
            ) if not passed else []
            
            results.append(StressTestResult(
                test_name=f"Extreme Scenario: {scenario['name']}",
                metrics=metrics,
    ...
    def test_recovery_time(
        ...
        for scenario in shock_scenarios:
            # Generate data with market shock
            shock_data = self.market_simulator.generate_shock_scenario(
                scenario["shock_type"],
                scenario.get("magnitude", "high"),
                scenario.get("duration", "1H")
            )
            
            # Get model predictions over time
            predictions = self.model_client.get_predictions(model_id, shock_data)
            
            # Calculate recovery metrics
            recovery_time, metrics = self._analyze_recovery(
                predictions, shock_data, scenario["shock_type"]
            )
            
            # Check if recovery time is acceptable
            passed = pd.Timedelta(recovery_time) <= pd.Timedelta(max_recovery_time)
            
            results.append(StressTestResult(
                test_name=f"Recovery from {scenario['shock_type']}",
                metrics={"recovery_time": recovery_time, **metrics},
                details={"scenario": scenario, "predictions": predictions},
                passed=passed,
                recommendations=self._get_recovery_recommendations(
                    scenario["shock_type"], recovery_time
                ) if not passed else []
            ))
    ...
    def _calculate_metrics(
        ...
        """Calculate performance metrics."""
        return {
            "accuracy": self._calculate_accuracy(predictions, actual_data),
    ...
    def _calculate_accuracy(
        ...
        return float(np.mean(
            np.abs(predictions["prediction"] - actual_data["actual"])
        ))
...
class SensitivityAnalyzer:
    ...
    def analyze_parameter_sensitivity(
        ...
        for param_name, (min_val, max_val) in param_ranges.items():
            ...
            # Test model with each parameter value
            ...
            sensitivity = {
                "gradient": np.gradient(outputs, param_values).mean(),
                "variance": np.var(outputs),
                "effect_size": (max(outputs) - min(outputs)) / np.std(outputs)
            }
    ...
    def analyze_input_sensitivity(
        ...
        samples = {
            name: np.random.uniform(min_val, max_val, n_montecarlo)
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for i in range(n_montecarlo):
                ...
                futures.append(
                    executor.submit(model_func, test_inputs)
                )
            
            outputs = [f.result() for f in futures]
        
        # Calculate sensitivity metrics for each input
        for input_name in input_ranges:
            ...
            results[input_name] = {
                ...
                "importance_score": abs(correlation * partial_rank)
            }
...
class LoadTester:
    ...
    def test_throughput(
        ...
        with ThreadPoolExecutor() as executor:
            while (pd.Timestamp.now() - start_time).total_seconds() < duration_seconds:
                # Calculate target QPS based on ramp-up
        ...
        # Determine if test passed
        passed = (
            metrics["p95_latency_ms"] <= self.max_latency_ms and
            metrics["error_rate"] <= 0.01
        )
        
        return StressTestResult(
            test_name=f"Throughput Test ({max_qps} QPS)",
            metrics=metrics,
            details={"errors": errors, "throughput_timeline": throughputs},
            passed=passed,
    ...
    def _make_prediction_request(
        ...
        return (end_time - start_time).total_seconds() * 1000  # Convert to ms
    ...
    def _get_performance_recommendations(
        ...
        if metrics["p95_latency_ms"] > self.max_latency_ms:
            recommendations.append(
                f"High latency detected (P95: {metrics['p95_latency_ms']:.2f}ms). "
                "Consider model optimization or hardware scaling."
            )
