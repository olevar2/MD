...
class MarketRegimeIdentifier:
    ...
    def __init__(self, 
        ...
        self.thresholds = thresholds or {
            "trend_strength": 25.0,  # ADX threshold for trend strength
            "volatility_high": 1.5,   # Multiplier for high volatility
            "range_bounds": 0.3,     # Max percentage of price movement for ranging
            "breakout_strength": 2.0  # Multiplier for breakout identification
        }
        ...
        self.regime_duration_gauge = Gauge(
            'market_regime_duration', 
            'Duration of current market regime in hours',
            ['instrument', 'timeframe', 'regime']
        )
        
        self.regime_transitions = Counter(
            'market_regime_transitions_total',
            'Number of transitions between market regimes',
            ['instrument', 'timeframe', 'from_regime', 'to_regime']
        )
        
        # Tool effectiveness metrics
        self.tool_effectiveness_gauge = Gauge(
            'tool_effectiveness_by_regime',
        ...
        self.win_rate_gauge = Gauge(
            'tool_win_rate',
            'Win rate percentage for trading tools',
            ['tool_name', 'instrument', 'timeframe']
        )
        
        self.signal_count_gauge = Gauge(
            'tool_signal_count',
            'Number of signals generated by trading tools',
            ['tool_name', 'instrument', 'timeframe', 'signal_type']
        )
        
        # Track the last identified regime and when it started
        self.current_regimes = {}  # {(instrument, timeframe): (regime, start_time)}
        
    def identify_regime(self, 
                       instrument: str, 
                       timeframe: str,
                       ohlcv_data: pd.DataFrame) -> MarketRegimeType:
        """
        ...
        if len(ohlcv_data) < max(self.lookback_periods.values()):
            logger.warning(f"Not enough data to identify regime for {instrument} {timeframe}")
    ...
    def _calculate_regime(self, df: pd.DataFrame) -> MarketRegimeType:
        ...
        volatility = self._calculate_atr_percentage(df, self.lookback_periods["volatility"])
        avg_volatility = volatility.rolling(window=self.lookback_periods["volatility"]).mean().iloc[-1]
        current_volatility = volatility.iloc[-1]
        
        # Calculate directional movement
        close = df['close'].values
        price_change = ((close[-1] / close[-self.lookback_periods["trend"]]) - 1) * 100
        
        # Check for breakout
        is_breakout = self._detect_breakout(df, self.lookback_periods["breakout"])
        ...
        elif current_volatility > avg_volatility * self.thresholds["volatility_high"]:
    ...
    def _calculate_adx(self, df: pd.DataFrame, period: int) -> float:
        ...
        # Calculate +DM, -DM, TR, +DI, -DI, DX, and ADX
        high = df['high'].values
        low = df['low'].values
        close = df['close'].values
        ...
        df_adx = pd.DataFrame({
            'TR': np.append(np.nan, tr),
            '+DM': np.append(np.nan, plus_dm),
            '-DM': np.append(np.nan, minus_dm)
        })
        ...
        # +DI and -DI
        df_adx['+DI' + str(period)] = 100 * df_adx['+DM' + str(period)] / df_adx['TR' + str(period)]
        df_adx['-DI' + str(period)] = 100 * df_adx['-DM' + str(period)] / df_adx['TR' + str(period)]
        
        # DX
        df_adx['DX'] = 100 * np.abs(
            (df_adx['+DI' + str(period)] - df_adx['-DI' + str(period)]) / 
            (df_adx['+DI' + str(period)] + df_adx['-DI' + str(period)])
        )
    ...
    def _calculate_atr_percentage(self, df: pd.DataFrame, period: int) -> pd.Series:
    ...
    def _detect_breakout(self, df: pd.DataFrame, period: int) -> bool:
        ...
        avg_range = (df['high'] - df['low']).rolling(window=period).mean().iloc[-2]
        
        # Breakout occurs when price moves beyond recent high/low with strong momentum
        breakout_threshold = avg_range * self.thresholds["breakout_strength"]
        
        is_upward_breakout = (current_close > recent_high) and (current_close - previous_close > breakout_threshold)
        is_downward_breakout = (current_close < recent_low) and (previous_close - current_close > breakout_threshold)
    ...
    def _detect_reversal(self, df: pd.DataFrame, period: int) -> bool:
        ...
        # Simple reversal detection based on moving average crossover
        short_ma = df['close'].rolling(window=5).mean()
        long_ma = df['close'].rolling(window=period).mean()
        
        # Check for crossover in last few candles
        previous_diff = short_ma.iloc[-2] - long_ma.iloc[-2]
        current_diff = short_ma.iloc[-1] - long_ma.iloc[-1]
        
        # Reversal occurs when short MA crosses long MA
        return (previous_diff * current_diff) < 0
    
    def _update_metrics(self, instrument: str, timeframe: str, regime: MarketRegimeType):
        """Update Prometheus metrics for the identified regime"""
        # Reset all regime values for this instrument/timeframe
        for r in MarketRegimeType:
            self.regime_gauge.labels(instrument=instrument, timeframe=timeframe, regime=r).set(0)
        
        # Set current regime to 1
        self.regime_gauge.labels(instrument=instrument, timeframe=timeframe, regime=regime).set(1)
        
        # Update regime duration and transitions
        ...
        if key in self.current_regimes:
            ...
            if last_regime != regime:
                # Regime changed, record transition
                self.regime_transitions.labels(
                ...
                # Reset duration and update current regime
                self.current_regimes[key] = (regime, now)
            else:
                # Same regime, update duration
                duration_hours = (now - start_time).total_seconds() / 3600.0
                self.regime_duration_gauge.labels(
                    instrument=instrument, 
                    timeframe=timeframe, 
                    regime=regime
                ).set(duration_hours)
        else:
            # First time seeing this instrument/timeframe
            self.current_regimes[key] = (regime, now)
    ...
    def export_metrics_for_tool_effectiveness(self, 
                                             tool_name: str,
                                             instrument: str,
                                             timeframe: str,
                                             success_rate: float,
        ...
        """
        # Record the effectiveness metric
        self.tool_effectiveness_gauge.labels(
            tool_name=tool_name,
            instrument=instrument,
            timeframe=timeframe,
            regime=regime
    ...
    def export_win_rate(self,
        ...
        """
        self.win_rate_gauge.labels(
            tool_name=tool_name,
            instrument=instrument,
            timeframe=timeframe
        ).set(win_rate)
        
    def export_signal_count(self,
                           tool_name: str,
                           instrument: str,
                           timeframe: str,
                           signal_type: str,
        ...
        """
        self.signal_count_gauge.labels(
            tool_name=tool_name,
            instrument=instrument,
            timeframe=timeframe,
    ...
    def fetch_effectiveness_data_from_api(self, api_url: str) -> List[Dict[str, Any]]:
        """
        ...
        try:
            response = requests.get(
                f"{api_url}/api/v1/tool-effectiveness/summary",
                params={
                    "last_n_days": 30  # Get data from last 30 days
                }
            )
            
            if response.status_code == 200:
                data = response.json()
                logger.info(f"Successfully fetched {len(data.get('records', []))} effectiveness records")
                return data.get("records", [])
            else:
                logger.error(f"Failed to fetch effectiveness data: {response.text}")
                return []
    ...
    def process_effectiveness_data(self, records: List[Dict[str, Any]]):
        ...
        # Group by tool_name, instrument, timeframe
        grouped_data = {}
        ...
        for record in records:
            ...
            accuracy = record.get("accuracy", 0) * 100  # Convert to percentage
            ...
            if regime not in grouped_data[key]["regimes"]:
                grouped_data[key]["regimes"][regime] = {
                    "total": 0,
                    "success": 0
                }
                
            grouped_data[key]["regimes"][regime]["total"] += 1
            grouped_data[key]["regimes"][regime]["success"] += 1 if accuracy >= 50 else 0
        ...
        for key, data in grouped_data.items():
            ...
            # Calculate win rate
            win_rate = (data["success_count"] / data["total_records"]) * 100 if data["total_records"] > 0 else 0
            
            # Export overall win rate
            self.export_win_rate(tool_name, instrument, timeframe, win_rate)
            
            # Export regime-specific metrics
            for regime, regime_data in data["regimes"].items():
                regime_win_rate = (regime_data["success"] / regime_data["total"]) * 100 if regime_data["total"] > 0 else 0
                ...
                self.export_metrics_for_tool_effectiveness(
                    tool_name, 
                    instrument, 
                    timeframe, 
                    regime_win_rate, 
            ...
            # Export signal counts (assuming we have this data)
            # self.export_signal_count(tool_name, instrument, timeframe, "buy", buy_count)
            # self.export_signal_count(tool_name, instrument, timeframe, "sell", sell_count)
...
def main():
    ...
    # Initialize the market regime identifier
    identifier = MarketRegimeIdentifier()
    
    # Start metrics server
    identifier.start_metrics_server(port=8080)
    
    # Periodically fetch and process effectiveness data
    analysis_engine_api_url = "http://localhost:8003"
    
    while True:
        try:
            # Fetch effectiveness data
            records = identifier.fetch_effectiveness_data_from_api(analysis_engine_api_url)
