...
# Counter metrics
MANIPULATION_DETECTION_TOTAL = Counter(
    'forex_manipulation_detection_total', 
    'Total number of market manipulation detection checks performed',
    ['currency_pair', 'timeframe']
)

MANIPULATION_ALERTS_TOTAL = Counter(
    'forex_manipulation_alerts_total',
    'Total number of manipulation alerts raised',
    ['currency_pair', 'pattern_type', 'severity']
)

MANIPULATION_FALSE_POSITIVES = Counter(
    'forex_manipulation_false_positives',
    'Number of false positive manipulation detections',
    ['currency_pair', 'pattern_type']
)

# Gauge metrics
MANIPULATION_CONFIDENCE_SCORE = Gauge(
    'forex_manipulation_confidence_score',
    'Confidence score of the latest manipulation detection',
    ['currency_pair', 'pattern_type']
)

MANIPULATION_IMPACT_ESTIMATE = Gauge(
    'forex_manipulation_impact_estimate',
    'Estimated market impact of detected manipulation',
    ['currency_pair']
)
...
MANIPULATION_PATTERN_COMPLEXITY = Summary(
    ...
    'Complexity score of detected manipulation patterns',
    ['pattern_type']
)

# Phase 7: Advanced Analytics Metrics
MANIPULATION_ML_MODEL_ACCURACY = Gauge(
    'forex_manipulation_ml_model_accuracy',
    'Accuracy score of machine learning model for manipulation detection',
    ['model_version', 'currency_pair']
)

MANIPULATION_ANOMALY_SCORE = Gauge(
    'forex_manipulation_anomaly_score',
    'Anomaly score for potential market manipulation',
    ['currency_pair', 'anomaly_type']
)
...
# API configurations
RISK_MGMT_API_URL = os.environ.get("RISK_MGMT_API_URL", "http://risk-management-service:8000")

def auth_handler(url, method, timeout, headers, data):
    """
    Authentication handler for Prometheus push gateway
    """
    return basic_auth_handler(url, method, timeout, headers, data, AUTH_USERNAME, AUTH_PASSWORD)
...
def fetch_and_export_metrics():
    ...
    """
    try:
        # Fetch metrics from Risk Management Service API
        response = requests.get(
            f"{RISK_MGMT_API_URL}/api/v1/manipulation/metrics",
        ...
        if response.status_code == 200:
            ...
            # Update detection totals
            for detection in metrics_data.get('detection_counts', []):
                MANIPULATION_DETECTION_TOTAL.labels(
                    currency_pair=detection.get('currency_pair', 'unknown'),
                    timeframe=detection.get('timeframe', 'unknown')
                ).inc(detection.get('count', 0))
            
            # Update alert totals
            for alert in metrics_data.get('alerts', []):
                MANIPULATION_ALERTS_TOTAL.labels(
                    currency_pair=alert.get('currency_pair', 'unknown'),
                    pattern_type=alert.get('pattern_type', 'unknown'),
                    severity=alert.get('severity', 'medium')
                ).inc(alert.get('count', 0))
            
            # Update false positives
            for fp in metrics_data.get('false_positives', []):
                MANIPULATION_FALSE_POSITIVES.labels(
                    currency_pair=fp.get('currency_pair', 'unknown'),
                    pattern_type=fp.get('pattern_type', 'unknown')
                ).inc(fp.get('count', 0))
            
            # Update confidence scores
            for confidence in metrics_data.get('confidence_scores', []):
                MANIPULATION_CONFIDENCE_SCORE.labels(
                    currency_pair=confidence.get('currency_pair', 'unknown'),
            ...
            for impact in metrics_data.get('impact_estimates', []):
                MANIPULATION_IMPACT_ESTIMATE.labels(
                    currency_pair=impact.get('currency_pair', 'unknown')
                ).set(impact.get('estimate', 0))
            
            # Update detection latencies
            for latency in metrics_data.get('detection_latencies', []):
                MANIPULATION_DETECTION_LATENCY.labels(
                    pattern_type=latency.get('pattern_type', 'unknown')
                ).observe(latency.get('duration_seconds', 0))
            
            # Update pattern complexities
            for complexity in metrics_data.get('pattern_complexities', []):
                MANIPULATION_PATTERN_COMPLEXITY.labels(
                    pattern_type=complexity.get('pattern_type', 'unknown')
                ).observe(complexity.get('complexity_score', 0))
            
            # Update advanced analytics metrics
            for ml_model in metrics_data.get('ml_models', []):
                MANIPULATION_ML_MODEL_ACCURACY.labels(
                    model_version=ml_model.get('version', 'unknown'),
                    currency_pair=ml_model.get('currency_pair', 'unknown')
                ).set(ml_model.get('accuracy', 0))
            
            for anomaly in metrics_data.get('anomalies', []):
                MANIPULATION_ANOMALY_SCORE.labels(
                    currency_pair=anomaly.get('currency_pair', 'unknown'),
                    anomaly_type=anomaly.get('type', 'unknown')
                ).set(anomaly.get('score', 0))
            
            for reliability in metrics_data.get('reliability_scores', []):
                MANIPULATION_DETECTION_RELIABILITY.labels(
                    detection_algorithm=reliability.get('algorithm', 'unknown')
                ).set(reliability.get('score', 0))
            
            for pattern in metrics_data.get('collaborative_patterns', []):
                MANIPULATION_COLLABORATIVE_PATTERN.labels(
                    pattern_signature=pattern.get('signature', 'unknown'),
                    actor_count=pattern.get('actor_count', 'unknown')
                ).inc(pattern.get('count', 0))
                
            logger.info(f"Successfully updated market manipulation metrics at {datetime.now().isoformat()}")
            
        else:
            logger.error(f"Failed to fetch market manipulation metrics: {response.status_code} - {response.text}")
...
def push_metrics_to_gateway():
    ...
    try:
        ...
        # Configure auth handler if credentials are provided
        ...
        logger.info(f"Successfully pushed metrics to gateway at {datetime.now().isoformat()}")
    except Exception as e:
...
def run_metrics_exporter():
    ...
    while True:
        try:
            ...
            # Sleep for a moment before retrying to avoid tight loop on persistent errors
            time.sleep(5)
...
MANIPULATION_TYPES_DETECTED = Counter(
    'forex_manipulation_types_detected',
    'Number of specific manipulation types detected',
    ['currency_pair', 'manipulation_type', 'severity']
)

API_REQUESTS = Counter(
    'forex_manipulation_api_requests_total',
...
DETECTION_SYSTEM_HEALTH = Gauge(
    'forex_manipulation_system_health',
...
DETECTION_PROCESSING_TIME = Summary(
    ...
    'Time spent processing manipulation detection requests',
    ['detection_type']
)
...
class MarketManipulationMetricsExporter:
    ...
    def __init__(self):
        ...
        self.manipulation_stats = {
            'stop_hunting': {'count': 0, 'avg_confidence': 0.0},
            'fake_breakout': {'count': 0, 'avg_confidence': 0.0},
            'volume_anomaly': {'count': 0, 'avg_confidence': 0.0}
        }
    ...
    def record_detection_check(self, currency_pair: str, timeframe: str) -> None:
        """
    ...
    def record_detection(self, 
        ...
        MANIPULATION_TYPES_DETECTED.labels(
            currency_pair=currency_pair,
            manipulation_type=manipulation_type,
            severity=severity
        ).inc()
        
        # Update confidence score gauge
        CONFIDENCE_SCORE.labels(manipulation_type=manipulation_type).set(confidence)
        
        # Record to confidence score distribution
        CONFIDENCE_SCORE_DISTRIBUTION.labels(manipulation_type=manipulation_type).observe(confidence)
        
        # Record processing time
        DETECTION_PROCESSING_TIME.labels(detection_type=manipulation_type).observe(processing_time)
        
        # Update local stats
        if manipulation_type in self.manipulation_stats:
            stats = self.manipulation_stats[manipulation_type]
            # Update rolling average of confidence score
            stats['avg_confidence'] = (stats['avg_confidence'] * stats['count'] + confidence) / (stats['count'] + 1)
            stats['count'] += 1
    ...
    def record_api_request(self, endpoint: str, status_code: int) -> None:
        ...
        """
        API_REQUESTS.labels(endpoint=endpoint, status_code=str(status_code)).inc()
    
    def record_detection_latency(self, detection_type: str, latency: float) -> None:
        """
    ...
    def update_system_health(self, component: str, is_healthy: bool) -> None:
        ...
        """
        DETECTION_SYSTEM_HEALTH.labels(component=component).set(1.0 if is_healthy else 0.0)
    ...
    def push_metrics(self) -> bool:
        ...
        try:
            ...
            push_to_gateway(
                PROMETHEUS_GATEWAY, 
                job='market_manipulation_detection',
                handler=auth_handler
            )
            
            self.last_push_time = now
            logger.info(f"Successfully pushed market manipulation metrics to gateway")
            return True
    ...
    def periodic_metrics_update(self) -> None:
        ...
        # Update confidence score gauges from accumulated stats
        for manipulation_type, stats in self.manipulation_stats.items():
            if stats['count'] > 0:
                CONFIDENCE_SCORE.labels(manipulation_type=manipulation_type).set(stats['avg_confidence'])
