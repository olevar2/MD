...
TRACE_ERROR_TOTAL = Counter(
    'trace_error_total',
    'Total number of errors in traced operations',
    ['operation', 'service', 'error_type']
)

CRITICAL_PATH_DURATION = Gauge(
    'critical_path_duration_seconds',
    'Duration of critical path operations',
    ['path_name']
)

TRACE_DEPENDENCY_LATENCY = Histogram(
    'trace_dependency_latency_seconds',
    'Latency of dependencies in traced operations',
...
class TraceCollector:
    ...
    def process_trace(self, trace_data: Dict[str, Any]) -> None:
        ...
        duration = trace_data.get('duration_ms', 0) / 1000  # Convert to seconds
        
        # Record duration
        TRACE_DURATION_SECONDS.labels(
            operation=operation,
            service=service
        ).observe(duration)
    ...
    def _process_dependencies(self, trace_data: Dict[str, Any]) -> None:
        ...
        for dep in trace_data.get('dependencies', []):
            TRACE_DEPENDENCY_LATENCY.labels(
                dependency=dep['name'],
                operation=trace_data.get('operation', 'unknown')
            ).observe(dep.get('duration_ms', 0) / 1000)
    ...
    def _update_critical_path_metrics(self, trace_data: Dict[str, Any]) -> None:
        ...
        for path_name, operations in self.critical_paths.items():
            if operation in operations:
                ...
                CRITICAL_PATH_DURATION.labels(
                    path_name=path_name
                ).set(duration)

    def start_span(self, operation: str, context: Optional[Dict[str, Any]] = None) -> Any:
