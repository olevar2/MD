...
TOOL_WIN_RATE = Gauge('tool_effectiveness_win_rate', 
                      ...
                      ['tool_id', 'timeframe', 'market_regime'])
...
REQUEST_FAILURES = Counter('http_request_failures_total', 
                          'Total count of HTTP request failures', 
                          ['endpoint'])
...
class ToolEffectivenessExporter:
    ...
    def __init__(self, base_url: str, update_interval: int):
        ...
        self.logger.info(f"Initialized Tool Effectiveness Exporter with base URL: {base_url}")
    ...
    def start(self):
        ...
        # Collect effectiveness metrics for all tools
        ...
        # Collect market regime reliability metrics
        self.collect_market_regime_metrics()
        
        # Log collection time
        end_time = time.time()
        self.logger.info(f"Metrics collection completed in {end_time - start_time:.2f} seconds")
    ...
    def collect_effectiveness_metrics(self):
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.error(f"Failed to get effectiveness metrics: {response.status_code} - {response.text}")
            ...
            for tool_data in metrics_data:
                ...
                # Update signal and outcome counts
                TOOL_SIGNAL_COUNT.labels(tool_id=tool_id).set(tool_data.get("signal_count", 0))
                
                success_count = int(tool_data.get("success_rate", 0) * tool_data.get("signal_count", 0) / 100)
                failure_count = tool_data.get("signal_count", 0) - success_count
                
                TOOL_OUTCOME_COUNT.labels(tool_id=tool_id, outcome="success").set(success_count)
                TOOL_OUTCOME_COUNT.labels(tool_id=tool_id, outcome="failure").set(failure_count)
                ...
                for metric in tool_data.get("metrics", []):
                    ...
                    # Set appropriate Prometheus metric based on name
                    if "win rate" in name:
                        TOOL_WIN_RATE.labels(
                            tool_id=tool_id, 
                            timeframe="all", 
                            market_regime="all"
                        ).set(value)
                        ...
                        # Extract regime from name (e.g., "Reliability in Trending Market")
                        regime = name.lower().replace("reliability in ", "").replace(" market", "")
                        TOOL_WIN_RATE.labels(
                            tool_id=tool_id,
                            timeframe="all",
            ...
            self.logger.info(f"Updated effectiveness metrics for {len(metrics_data)} tools")
            
        except Exception as e:
            self.logger.error(f"Error collecting effectiveness metrics: {str(e)}")
            REQUEST_FAILURES.labels(endpoint="metrics").inc()
    ...
    def collect_signal_quality_metrics(self):
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.error(f"Failed to get dashboard data: {response.status_code} - {response.text}")
            ...
            # For each tool, get quality analysis
            for tool_id in tool_ids:
                url = f"{self.base_url}/api/v1/signal-quality/quality-analysis?tool_id={tool_id}"
                request_time = REQUEST_LATENCY.labels(endpoint="quality-analysis")
                ...
                try:
                    ...
                    if response.status_code != 200:
                        self.logger.warning(f"Failed to get quality analysis for {tool_id}: {response.status_code}")
                        continue
                    
                    analysis = response.json()
                    
                    # Update overall quality score
                    SIGNAL_QUALITY_SCORE.labels(tool_id=tool_id).set(analysis.get("average_quality", 0))
                    
                    # Update quality by regime if available (requires tool to have signals in different regimes)
                    for bracket in analysis.get("quality_brackets", []):
                        # We're using the quality bracket as a proxy for different quality levels
                        # This isn't perfect but provides some insight into quality distribution
                        regime_label = f"q{int(bracket['quality_range'][0]*10)}_to_q{int(bracket['quality_range'][1]*10)}"
                        SIGNAL_QUALITY_BY_REGIME.labels(
                            tool_id=tool_id, 
                            market_regime=regime_label
                        ).set(bracket.get("success_rate", 0))
                
                except Exception as e:
                    self.logger.error(f"Error collecting quality analysis for {tool_id}: {str(e)}")
                    REQUEST_FAILURES.labels(endpoint="quality-analysis").inc()
            
            self.logger.info(f"Updated quality metrics for {len(tool_ids)} tools")
            
        except Exception as e:
            self.logger.error(f"Error collecting signal quality metrics: {str(e)}")
    ...
    def collect_trend_metrics(self):
        ...
        try:
            ...
            # For each tool, get quality trends
            for tool_id in tool_ids:
                url = f"{self.base_url}/api/v1/signal-quality/quality-trends?tool_id={tool_id}"
                ...
                try:
                    ...
                    if response.status_code != 200:
                        self.logger.warning(f"Failed to get quality trends for {tool_id}: {response.status_code}")
                        continue
                    
                    trends = response.json()
                    
                    # Update trend metrics
                    QUALITY_TREND.labels(tool_id=tool_id, timeframe="all").set(trends.get("quality_trend", 0))
                    SUCCESS_TREND.labels(tool_id=tool_id, timeframe="all").set(trends.get("success_trend", 0))
                
                except Exception as e:
                    self.logger.error(f"Error collecting quality trends for {tool_id}: {str(e)}")
    ...
    def collect_market_regime_metrics(self):
        ...
        try:
            ...
            url = f"{self.base_url}/api/v1/tool-effectiveness/regime-reliability"
            ...
            if response.status_code != 200:
                self.logger.error(f"Failed to get market regime metrics: {response.status_code} - {response.text}")
                REQUEST_FAILURES.labels(endpoint="regime-reliability").inc()
                return
            
            regime_data = response.json()
            
            # Update Prometheus metrics for each tool
            for tool_data in regime_data:
                tool_id = tool_data.get("tool_id", "unknown")
                
                # Process metrics for each market regime
                for regime, metrics in tool_data.get("regime_metrics", {}).items():
                    win_rate = metrics.get("win_rate", 0)
                    profit_factor = metrics.get("profit_factor", 1.0)
                    expected_payoff = metrics.get("expected_payoff", 0)
                    
                    # Set win rate by regime
                    TOOL_WIN_RATE.labels(
                        tool_id=tool_id,
                        timeframe="all",
                        market_regime=regime
                    ...
                    # Set signal quality by regime if available
            ...
            self.logger.info(f"Updated market regime metrics for {len(regime_data)} tools")
            
        except Exception as e:
            self.logger.error(f"Error collecting market regime metrics: {str(e)}")
            REQUEST_FAILURES.labels(endpoint="regime-reliability").inc()
...
if __name__ == "__main__":
    ...
    # Initialize exporter
    exporter = ToolEffectivenessExporter(
