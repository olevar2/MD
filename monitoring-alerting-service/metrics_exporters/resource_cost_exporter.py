...
import boto3  # For AWS Cost Explorer integration
from azure.mgmt.costmanagement import CostManagementClient  # For Azure cost management
...
class ResourceCostExporter:
    def __init__(
        ...
        self.storage_utilization = Gauge(
            'resource_storage_utilization',
            'Storage utilization percentage by service',
        ...
        self.hourly_cost = Gauge(
            'service_hourly_cost',
            'Hourly cost by service in USD',
            ['service', 'resource_type']
        )
        self.daily_cost = Gauge(
            'service_daily_cost',
            'Daily cost by service in USD',
            ['service', 'resource_type']
        )
        self.cost_efficiency = Gauge(
    ...
    def _init_cloud_clients(self):
        ...
        if self.cloud_provider == "aws":
            self.cost_client = boto3.client('ce')  # AWS Cost Explorer
            self.ec2_client = boto3.client('ec2')
            self.cloudwatch = boto3.client('cloudwatch')
        elif self.cloud_provider == "azure":
            # Azure clients initialization would go here
            pass
        ...
        for reservation in instances['Reservations']:
            for instance in reservation['Instances']:
                ...
                if cpu_stats['Datapoints']:
                    ...
                    ).set(cpu_stats['Datapoints'][0]['Average'])
    ...
    def _collect_aws_costs(self):
        ...
        cost_response = self.cost_client.get_cost_and_usage(
            TimePeriod={
