...
SCALING_EFFICIENCY = Gauge(
    'scaling_efficiency_score',
    'Resource scaling efficiency score (0-1)',
    ['service', 'resource_type']
)

COST_ANOMALY = Gauge(
    'service_cost_zscore',
    'Z-score of service cost vs historical average',
    ['service']
)
...
class ResourceCostMonitor:
    ...
    def collect_resource_metrics(self) -> None:
        ...
        try:
            ...
            for pod in pods:
                ...
                self._update_scaling_efficiency(service, cpu_usage, memory_usage)
    ...
    def collect_cost_metrics(self) -> None:
        ...
        try:
            ...
            for service, cost_data in current_costs.items():
                ...
                # Keep last 168 hours (1 week) of history
                self.historical_costs[service] = self.historical_costs[service][-168:]
        ...
        for service, metrics in self._get_service_metrics().items():
            ...
            if avg_cpu < 20 and avg_memory < 20:
                recommendations.append({
                    'service': service,
                    'type': 'downsizing',
                    'message': f'Consider downsizing {service} resources. Average utilization: CPU {avg_cpu}%, Memory {avg_memory}%',
                    'estimated_savings': self._calculate_downsizing_savings(service)
                })
        
        # Check for cost anomalies
        for service, costs in self.historical_costs.items():
            if len(costs) >= 24:  # At least 24 hours of history
                ...
                if current_cost > avg_cost * 1.5:
                    recommendations.append({
                        'service': service,
                        'type': 'cost_anomaly',
                        'message': f'Investigate high costs in {service}. Current cost is 50% above average.',
    ...
    def _update_scaling_efficiency(
        self, 
        service: str, 
        cpu_usage: float, 
        memory_usage: float
    ...
    def _calculate_cost_zscore(self, service: str, current_cost: float) -> float:
        ...
        std_dev = (
            sum((x - mean) ** 2 for x in historical) / len(historical)
        ) ** 0.5
    ...
    def _calculate_downsizing_savings(self, service: str) -> float:
        ...
        return current_costs[service]['hourly_cost'] * 0.3 * 24 * 30  # Monthly savings
