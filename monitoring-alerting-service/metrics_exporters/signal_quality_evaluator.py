...
class SignalQualityEvaluator:
    ...
    def evaluate_signal_quality(self, 
                               ...
                               market_regimes: pd.DataFrame) -> Dict[str, Any]:
        """
        ...
        # Combine all metrics
        results = {
            "overall_metrics": quality_metrics,
            "regime_metrics": regime_metrics
        }
        
        return results
    
    def _match_signals_to_outcomes(self, 
                                  signals: pd.DataFrame, 
        ...
        # Create a merged DataFrame with signals and their outcomes
        # This is a simplified implementation - in production, would need more sophisticated matching logic
        merged = pd.merge_asof(
            signals.sort_values('timestamp'), 
            outcomes.sort_values('entry_time'),
            left_on='timestamp', 
            right_on='entry_time',
            direction='forward', 
    ...
    def _add_regime_data(self, 
        ...
        # Ensure regimes DataFrame has datetime index
        if not isinstance(regimes.index, pd.DatetimeIndex):
            if 'timestamp' in regimes.columns:
                regimes = regimes.set_index('timestamp')
            else:
                raise ValueError("Market regimes DataFrame must have a timestamp column or datetime index")
        
        # Create a copy to avoid modifying the original
        result = data.copy()
        
        # For each row, determine the market regime at signal time
        result['market_regime'] = result['timestamp'].apply(
            lambda ts: self._get_regime_at_time(ts, regimes)
        )
        
        return result
    
    def _get_regime_at_time(self, timestamp: pd.Timestamp, regimes: pd.DataFrame) -> str:
        """Determine market regime at a specific timestamp"""
        # Find the closest regime classification before the timestamp
        try:
        ...
        # Default regime if no match found
        return 'unknown'
    ...
    def _calculate_quality_metrics(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        if len(data) == 0:
            self.logger.warning("No matched signal-outcome data available for quality metrics calculation")
            return {
                "accuracy_score": 0.0,
        ...
        # Calculate accuracy score (how often signal correctly predicted direction)
        if 'signal_direction' in data.columns and 'price_direction' in data.columns:
            correct_directions = (data['signal_direction'] == data['price_direction']).sum()
            metrics['accuracy_score'] = correct_directions / len(data)
        else:
            # Fallback calculation based on profitable trades
            profitable_trades = (data['pnl'] > 0).sum()
            metrics['accuracy_score'] = profitable_trades / len(data)
        
        # Calculate timeliness score (how early signals are compared to price movement)
        # Higher score means earlier signals (more valuable)
        if 'signal_to_movement_time' in data.columns:
            # Normalize time differences to a 0-1 scale where 1 is best (earliest signal)
            max_acceptable_delay = pd.Timedelta('1h')  # Configurable parameter
            timeliness_values = data['signal_to_movement_time'].apply(
                lambda x: max(0, 1 - (x / max_acceptable_delay))
            )
            metrics['timeliness_score'] = timeliness_values.mean()
        else:
            # Default timeliness if we can't calculate it
            metrics['timeliness_score'] = 0.5
        
        # Calculate consistency score (inverse of variance in signal performance)
        if 'pnl' in data.columns:
            # Normalize PnL to account for different position sizes
            if 'position_size' in data.columns and (data['position_size'] > 0).all():
            ...
            if mean_pnl > 0:
                # Convert coefficient of variation to a 0-1 score (1 is best/most consistent)
        ...
        # Calculate overall quality score (weighted average of individual metrics)
        metrics['overall_quality_score'] = (
            0.5 * metrics['accuracy_score'] +
            0.3 * metrics['timeliness_score'] +
    ...
    def _calculate_regime_specific_metrics(self, data: pd.DataFrame) -> Dict[str, Dict[str, float]]:
        ...
        # Get unique market regimes in the data
        ...
        for regime in regimes:
            ...
            regime_metrics[regime] = self._calculate_quality_metrics(regime_data)
