...
# Import metrics
from monitoring_alerting_service.metrics import (
    record_error,
    record_http_error,
...
def format_error_response(
    ...
    service: str = "monitoring-alerting-service"
) -> Dict[str, Any]:
    """
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def forex_platform_exception_handler(request: Request, exc: ForexTradingPlatformError):
        ...
        # Monitoring & Alerting specific errors
        if isinstance(exc, AlertNotFoundError):
            status_code = status.HTTP_404_NOT_FOUND
        elif isinstance(exc, NotificationError) or isinstance(exc, MetricsExporterError):
            status_code = status.HTTP_503_SERVICE_UNAVAILABLE
        elif isinstance(exc, AlertStorageError):
            status_code = status.HTTP_422_UNPROCESSABLE_ENTITY
        elif isinstance(exc, AlertRuleError) or isinstance(exc, ThresholdValidationError):
            status_code = status.HTTP_400_BAD_REQUEST
        ...
        try:
            ...
            logger.error(
                f"Failed to record error metrics: {str(metrics_exc)}",
