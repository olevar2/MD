#!/usr/bin/env python
"""
Deployment script for Monitoring Alerting Service.

This script deploys the Monitoring Alerting Service to the specified environment.
"""
...
logger = logging.getLogger("deploy_monitoring_alerting_service")

# Service configuration
SERVICE_NAME = "Monitoring Alerting Service"
SERVICE_PACKAGE = "monitoring_alerting_service"
SERVICE_MODULE = "monitoring_alerting_service.main"
SERVICE_PORT = 8009
...
def deploy_local(config: Dict[str, Any]):
    ...
    if "alertmanager_url" in config:
        env["ALERTMANAGER_URL"] = config["alertmanager_url"]
...
def deploy_docker(config: Dict[str, Any]):
    ...
    if "prometheus_url" in config:
        run_cmd.extend(["-e", f"PROMETHEUS_URL={config['prometheus_url']}"])
    if "alertmanager_url" in config:
        run_cmd.extend(["-e", f"ALERTMANAGER_URL={config['alertmanager_url']}"])
    if "grafana_url" in config:
        run_cmd.extend(["-e", f"GRAFANA_URL={config['grafana_url']}"])
...
def main():
    ...
    if args.alertmanager_url:
        config["alertmanager_url"] = args.alertmanager_url
    if args.grafana_url:
