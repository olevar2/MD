...
class AlertRule:
    ...
    def __init__(self, name: str, description: str, indicator_id: str,
        condition: str, threshold_value: float=None, timeframe: str='1h',
        enabled: bool=True, severity: str='info'):
        """
        ...
        self.id = (
            f"{indicator_id}_{condition}_{str(threshold_value).replace('.', '_')}_{timeframe}"
            )

    def check_condition(self, indicator_value: float, previous_value: float
        =None) ->bool:
        """
        ...
        if not self.enabled:
            return False
        if self.last_triggered and (datetime.now() - self.last_triggered
            ).total_seconds() < self.cooldown_minutes * 60:
            return False
        if self.condition == 'above' and self.threshold_value is not None:
            ...
            return abs(indicator_value - self.threshold_value) < 1e-05
        elif self.condition == 'crosses_above' and previous_value is not None and self.threshold_value is not None:
            return (indicator_value > self.threshold_value and 
                previous_value <= self.threshold_value)
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'id': self.id, 'name': self.name, 'description': self.
            description, 'indicator_id': self.indicator_id, 'condition':
            self.condition, 'threshold_value': self.threshold_value,
            'timeframe': self.timeframe, 'enabled': self.enabled,
            'severity': self.severity, 'cooldown_minutes': self.
            cooldown_minutes, 'last_triggered': self.last_triggered.
            isoformat() if self.last_triggered else None}

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) ->'AlertRule':
        """Create an alert rule from a dictionary"""
        rule = cls(name=data.get('name', ''), description=data.get(
            'description', ''), indicator_id=data.get('indicator_id', ''),
            condition=data.get('condition', ''), threshold_value=data.get(
            'threshold_value'), timeframe=data.get('timeframe', '1h'),
            enabled=data.get('enabled', True), severity=data.get('severity',
            'info'))
...
class AlertNotification:
    ...
    def __init__(self, rule: AlertRule, indicator_value: float, timestamp:
        datetime=None, symbol: str=None, message: str=None):
        """
        ...
        if not message:
            self.message = (
                f"Alert: {rule.name} - {rule.description} - Indicator {rule.indicator_id} {rule.condition} {rule.threshold_value if rule.threshold_value is not None else ''} Current value: {indicator_value}"
                )
        else:
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'id': self.id, 'rule': self.rule.to_dict() if self.rule else
            None, 'indicator_value': self.indicator_value, 'timestamp':
            self.timestamp.isoformat(), 'symbol': self.symbol, 'message':
            self.message, 'severity': self.rule.severity if self.rule else
            'info'}
...
class EmailNotificationChannel(NotificationChannel):
    ...
    def __init__(self, smtp_server: str, smtp_port: int, username: str,
        password: str, from_address: str, to_addresses: List[str], name:
        str='Email', enabled: bool=True):
        """
    ...
    async def send_notification(self, notification: AlertNotification) ->bool:
        ...
        try:
            ...
            msg['Subject'] = (
                f'Trading Alert: {notification.rule.name} [{notification.rule.severity.upper()}]'
                )
            body = f"""
            <html>
                <body>
    ...
    def to_dict(self) ->Dict[str, Any]:
        """Convert the channel to a dictionary for serialization"""
        data = super().to_dict()
        data.update({'smtp_server': self.smtp_server, 'smtp_port': self.
            smtp_port, 'username': self.username, 'from_address': self.
            from_address, 'to_addresses': self.to_addresses})
        return data
...
class AlertSystem:
    ...
    async def process_indicator_update(self, indicator_id: str, timeframe:
        str, value: float, symbol: str=None) ->List[AlertNotification]:
        """
        ...
        """
        previous_value = None
        if (indicator_id in self.previous_values and timeframe in self.
            previous_values[indicator_id]):
        ...
        for rule_id, rule in self.rules.items():
            if (rule.indicator_id == indicator_id and rule.timeframe ==
                timeframe):
    ...
    def get_active_rules(self) ->List[Dict[str, Any]]:
        ...
        return [rule.to_dict() for rule_id, rule in self.rules.items() if
            rule.enabled]
...
class SmartAlertFactory:
    ...
    def create_concordance_alert(name: str, indicators: List[str],
        ...
        """
        rule = AlertRule(name=name, description=
            f'Smart Alert based on concordance of {len(indicators)} indicators'
            , indicator_id=f"concordance_{'-'.join(indicators)}", condition
