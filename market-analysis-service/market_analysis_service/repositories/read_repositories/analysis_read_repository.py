...
from market_analysis_service.models.market_analysis_models import MarketAnalysisResponse, PatternRecognitionResponse, SupportResistanceResponse, MarketRegimeResponse, CorrelationAnalysisResponse, AnalysisType
...
class AnalysisReadRepository(ReadRepository):
    ...
    def __init__(self, data_dir: str='/data/market-analysis'):
        ...
        self.support_resistance_dir = os.path.join(self.data_dir,
            'support_resistance')
    ...
    async def get_by_id(self, id: str) ->Optional[Any]:
        ...
        result = await self._get_from_cache_or_file(id, self.analysis_cache,
            self.analysis_dir, MarketAnalysisResponse)
        if result:
            return result
        result = await self._get_from_cache_or_file(id, self.pattern_cache,
            self.pattern_dir, PatternRecognitionResponse)
        if result:
            return result
        result = await self._get_from_cache_or_file(id, self.
            support_resistance_cache, self.support_resistance_dir,
            SupportResistanceResponse)
        if result:
            return result
        result = await self._get_from_cache_or_file(id, self.
            market_regime_cache, self.market_regime_dir, MarketRegimeResponse)
        if result:
            return result
        result = await self._get_from_cache_or_file(id, self.
            correlation_cache, self.correlation_dir,
    ...
    @cached(cache_factory.get_cache(), 'get_all_analysis', ttl=3600)
    async def get_all(self) ->List[Any]:
        ...
        results = []
        results.extend(await self._get_all_from_directory(self.analysis_dir,
            MarketAnalysisResponse))
        results.extend(await self._get_all_from_directory(self.pattern_dir,
            PatternRecognitionResponse))
        results.extend(await self._get_all_from_directory(self.
            support_resistance_dir, SupportResistanceResponse))
        results.extend(await self._get_all_from_directory(self.
            market_regime_dir, MarketRegimeResponse))
        results.extend(await self._get_all_from_directory(self.
            correlation_dir, CorrelationAnalysisResponse))
        return results

    @cached(cache_factory.get_cache(), 'get_by_criteria_analysis', ttl=3600)
    ...
    async def _get_from_cache_or_file(self, id: str, cache: Dict[str, Any],
        directory: str, model_class: Any) ->Optional[Any]:
        """
    ...
    async def _get_all_from_directory(self, directory: str, model_class: Any
        ) ->List[Any]:
        """
        ...
        for filename in os.listdir(directory):
            if filename.endswith('.json'):
                try:
                    ...
                    result = await self._get_from_cache_or_file(result_id,
