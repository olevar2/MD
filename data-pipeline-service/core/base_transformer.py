...
class BaseMarketDataTransformer(ABC):
    ...
    def log_transformation_stats(self, original: pd.DataFrame, transformed:
        pd.DataFrame):
        """
        ...
        """
        self.logger.info(
            f'Transformed data: {len(original)} rows in, {len(transformed)} rows out'
            )
        added_columns = [col for col in transformed.columns if col not in
            original.columns]
        if added_columns:
            self.logger.info(f'Added columns: {added_columns}')
        removed_columns = [col for col in original.columns if col not in
            transformed.columns]
        if removed_columns:
    ...
    def execute(self, data: pd.DataFrame, **kwargs) ->pd.DataFrame:
        ...
        if not self.validate_data(data):
            self.logger.warning(
                'Data validation failed, returning original data')
            return data
