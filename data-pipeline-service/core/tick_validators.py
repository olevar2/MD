...
class QuoteSequenceValidator(ValidationStrategy):
    ...
    def __init__(self, bid_column: str='bid', ask_column: str='ask',
    ...
    def validate(self, data: pd.DataFrame) ->ValidationResult:
        ...
        if len(data) < 2:
            return ValidationResult(is_valid=True, message=
                'Not enough data points for sequence validation')
        ...
        for group_name, group_data in groups:
            ...
            threshold = 1e-06
            inconsistent = group_data[(bid_changes > threshold) & (
                ask_changes < -threshold) | (bid_changes < -threshold) & (
                ask_changes > threshold)].copy()
            if not inconsistent.empty:
                ...
                for i, row in sample.iterrows():
                    ...
                    inconsistencies.append({'timestamp': str(row[
                        'timestamp']), 'prev_timestamp': str(prev_row[
                        'timestamp']), 'prev_bid': float(prev_row[self.
                        bid_column]), 'prev_ask': float(prev_row[self.
                        ask_column]), 'current_bid': float(row[self.
                        bid_column]), 'current_ask': float(row[self.
                        ask_column]), 'bid_change': float(row[self.
                        bid_column] - prev_row[self.bid_column]),
                        'ask_change': float(row[self.ask_column] - prev_row
                        [self.ask_column])})
                issues.append({'instrument': str(group_name), 'count': len(
                    inconsistent), 'percentage': 100 * len(inconsistent) /
                    len(group_data), 'examples': inconsistencies})
        if issues:
            return ValidationResult(is_valid=True, message=
                'Quote sequence inconsistencies detected', details={
                'issues': issues}, severity=self.severity)
        return ValidationResult(is_valid=True, message=
            'Quote sequence validation passed')
...
class TickFrequencyValidator(ValidationStrategy):
    ...
    def __init__(self, min_expected_ticks_per_second: Optional[Dict[str,
        float]]=None, max_expected_ticks_per_second: Optional[Dict[str,
        float]]=None, analysis_window: pd.Timedelta=pd.Timedelta(minutes=1),
        min_analysis_windows: int=5, severity: ValidationSeverity=
    ...
    def validate(self, data: pd.DataFrame) ->ValidationResult:
        ...
        if not pd.api.types.is_datetime64_any_dtype(data['timestamp']):
            try:
                ...
                return ValidationResult(is_valid=False, message=
                    f'Failed to convert timestamp to datetime: {str(e)}',
        ...
        for instrument, group in data.groupby('instrument'):
            ...
            if time_span < self.analysis_window * self.min_analysis_windows:
                continue
            window_edges = pd.date_range(start=min_ts, end=max_ts + self.
                analysis_window, freq=self.analysis_window)
            window_counts = []
            for i in range(len(window_edges) - 1):
                window_start = window_edges[i]
                window_end = window_edges[i + 1]
                count = len(group[(group['timestamp'] >= window_start) & (
                    group['timestamp'] < window_end)])
                window_counts.append({'window_start': window_start,
                    'window_end': window_end, 'count': count,
                    'ticks_per_second': count / self.analysis_window.
                    total_seconds()})
            ...
            if min_ticks > 0:
                low_activity = [w for w in window_counts if w[
                    'ticks_per_second'] < min_ticks and w['count'] > 0]
            else:
                low_activity = []
            if max_ticks is not None:
                high_activity = [w for w in window_counts if w[
                    'ticks_per_second'] > max_ticks]
            else:
                high_activity = []
            if low_activity or high_activity:
                tps_values = [w['ticks_per_second'] for w in window_counts if
                    w['count'] > 0]
                issue_detail = {'instrument': instrument,
                    'windows_analyzed': len(window_counts),
                    'avg_ticks_per_second': float(np.mean(tps_values)) if
                    tps_values else 0, 'min_ticks_per_second': float(np.min
                    (tps_values)) if tps_values else 0,
                    'max_ticks_per_second': float(np.max(tps_values)) if
                    tps_values else 0}
                if low_activity:
                    issue_detail['low_activity_windows'] = len(low_activity)
                    issue_detail['low_activity_examples'] = [{
                        'window_start': str(w['window_start']),
                        'window_end': str(w['window_end']), 'ticks': w[
                        'count'], 'ticks_per_second': float(w[
                        'ticks_per_second'])} for w in sorted(low_activity,
                        key=lambda x: x['ticks_per_second'])[:5]]
                if high_activity:
                    ...
                    issue_detail['high_activity_examples'] = [{
                        ...
                        'window_end': str(w['window_end']), 'ticks': w[
                        'count'], 'ticks_per_second': float(w[
                        'ticks_per_second'])} for w in sorted(high_activity,
                        key=lambda x: -x['ticks_per_second'])[:5]]
                issues.append(issue_detail)
        if issues:
            return ValidationResult(is_valid=True, message=
                'Tick frequency anomalies detected', details={'issues':
                issues}, severity=self.severity)
        return ValidationResult(is_valid=True, message=
            'Tick frequency validation passed')
...
class TickVolumeConsistencyValidator(ValidationStrategy):
    ...
    def __init__(self, volume_column: str='volume', min_volume: Optional[
        float]=0, max_volume_spike_factor: Optional[float]=10.0,
        rolling_window: int=50, severity: ValidationSeverity=
        ValidationSeverity.WARNING):
        """
    ...
    def validate(self, data: pd.DataFrame) ->ValidationResult:
        ...
        if self.min_volume is not None:
            ...
            if not invalid_volume.empty:
                issues.append({'issue':
                    f'volume_less_than_{self.min_volume}', 'count': len(
                    invalid_volume), 'percentage': 100 * len(invalid_volume
                    ) / total_rows, 'examples': invalid_volume.head().
                    to_dict('records')})
        if self.max_volume_spike_factor is not None and len(data
            ) > self.rolling_window:
            ...
            for group_name, group_data in groups:
                ...
                rolling_avg = group_data[self.volume_column].rolling(window
                    =self.rolling_window, min_periods=1).mean()
                ...
                if not spikes.empty:
                    spike_issues.append({'instrument': str(group_name),
                        'count': len(spikes), 'percentage': 100 * len(
                        spikes) / len(group_data), 'examples': spikes.head(
                        ).to_dict('records')})
            if spike_issues:
                issues.append({'issue': 'volume_spikes_detected', 'details':
                    spike_issues})
        if issues:
            return ValidationResult(is_valid=True, message=
                'Tick volume consistency issues detected', details={
                'issues': issues}, severity=self.severity)
        return ValidationResult(is_valid=True, message=
            'Tick volume consistency validation passed')
...
class ValidateTickPriceReasonableness(BaseTickValidator):
    ...
    def __init__(self, min_bid_ask_spread: float=0.0, max_bid_ask_spread:
        float=0.1, max_price_deviation_factor: float=5.0, severity:
    ...
    def validate(self, data: pd.DataFrame) ->ValidationResult:
        ...
        if not inverted_ticks.empty:
            issues.append({'issue': 'bid_ask_inversion', 'count': len(
                inverted_ticks), 'percentage': 100 * len(inverted_ticks) /
                total_rows, 'examples': inverted_ticks.head().to_dict(
                'records')})
        if self.min_bid_ask_spread > 0:
            ...
            if not narrow_spread_ticks.empty:
                issues.append({'issue':
                    f'narrow_bid_ask_spread_{self.min_bid_ask_spread}',
                    'count': len(narrow_spread_ticks), 'percentage': 100 *
                    len(narrow_spread_ticks) / total_rows, 'examples':
                    narrow_spread_ticks.head().to_dict('records')})
        if self.max_bid_ask_spread > 0:
            wide_spread_ticks = data[data['spread'] > self.max_bid_ask_spread]
            if not wide_spread_ticks.empty:
                issues.append({'issue':
                    f'wide_bid_ask_spread_{self.max_bid_ask_spread}',
                    'count': len(wide_spread_ticks), 'percentage': 100 *
                    len(wide_spread_ticks) / total_rows, 'examples':
                    wide_spread_ticks.head().to_dict('records')})
        if self.max_price_deviation_factor > 0:
            ...
            lower_bound = (overall_mean - self.max_price_deviation_factor *
                overall_std)
            upper_bound = (overall_mean + self.max_price_deviation_factor *
                overall_std)
            unreasonable_price_ticks = data[(data['average_price'] <
                lower_bound) | (data['average_price'] > upper_bound)]
            if not unreasonable_price_ticks.empty:
                issues.append({'issue': 'unreasonable_price_deviation',
                    'count': len(unreasonable_price_ticks), 'percentage': 
                    100 * len(unreasonable_price_ticks) / total_rows,
                    'examples': unreasonable_price_ticks.head().to_dict(
                    'records')})
        if issues:
            return ValidationResult(is_valid=True, message=
                'Tick price reasonableness issues detected', details={
                'issues': issues}, severity=self.severity)
        return ValidationResult(is_valid=True, message=
            'Tick price reasonableness validation passed')
...
class ValidateTickDataCompleteness(BaseTickValidator):
    ...
    def __init__(self, required_fields: Optional[List[str]]=None, severity:
    ...
    def validate(self, data: pd.DataFrame) ->ValidationResult:
        ...
        for field in self.required_fields:
            if field not in data.columns:
                issues.append({'issue': f'missing_field_{field}', 'message':
                    f"Required field '{field}' is missing from the data.",
                    'severity': ValidationSeverity.ERROR, 'validator': self
                    .__class__.__name__})
        if issues:
            return ValidationResult(is_valid=False, message=
                'Missing required fields in tick data', details={'issues':
                issues}, severity=ValidationSeverity.ERROR)
        for field in self.required_fields:
            null_ticks = data[data[field].isnull()]
            if not null_ticks.empty:
                issues.append({'issue': f'null_value_in_field_{field}',
                    'count': len(null_ticks), 'percentage': 100 * len(
                    null_ticks) / total_rows, 'examples': null_ticks.head()
                    .to_dict('records')})
        if issues:
            return ValidationResult(is_valid=True, message=
                'Tick data completeness issues detected', details={'issues':
                issues}, severity=self.severity)
        return ValidationResult(is_valid=True, message=
            'Tick data completeness validation passed')
...
class ValidateTickTimestamps(BaseTickValidator):
    ...
    def __init__(self, max_gap_seconds: int=60, severity:
        ValidationSeverity=ValidationSeverity.ERROR):
        """
