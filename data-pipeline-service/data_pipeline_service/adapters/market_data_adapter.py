...
class MarketDataProviderAdapter(IMarketDataProvider):
    ...
    def __init__(self, market_data_service: Optional[MarketDataService]=None):
        ...
        self._market_data_service = market_data_service or MarketDataService(
            repository=MarketDataRepository(), cache=MarketDataCache())
        ...
        try:
            ...
            if not start_time:
                raise ValidationError('Start time cannot be empty', field=
                    'start_time')
            result = await self._market_data_service.get_historical_data(symbol
            ...
            if 'not found' in str(e).lower():
                ...
                raise ServiceError(
                    ...
                    service_name='market_data_service', operation=
                    'get_historical_data', cause=e)
            else:
                raise ServiceError(
                    f'Error fetching historical data for {symbol}: {str(e)}',
                    error_code=ErrorCode.SERVICE_UNAVAILABLE, service_name=
                    'market_data_service', operation='get_historical_data',
                    cause=e)
    ...
    async def get_latest_price(self, symbol: str) ->Dict[str, float]:
        ...
        try:
            if not symbol:
                raise ValidationError('Symbol cannot be empty', field='symbol')
            result = await self._market_data_service.get_latest_price(symbol
                =symbol)
            return result
        except ValidationError:
            raise
            ...
            if 'not found' in str(e).lower():
                raise DataError(f'No latest price found for {symbol}',
                    error_code=ErrorCode.DATA_MISSING_ERROR, data_source=
                    'market_data', data_type='latest_price', cause=e)
            else:
                raise ServiceError(
                    f'Error fetching latest price for {symbol}: {str(e)}',
                    error_code=ErrorCode.SERVICE_UNAVAILABLE, service_name=
                    'market_data_service', operation='get_latest_price',
                    cause=e)
    ...
    async def get_symbols(self) ->List[str]:
        ...
        try:
            ...
            raise ServiceError(f'Error fetching available symbols: {str(e)}',
                error_code=ErrorCode.SERVICE_UNAVAILABLE, service_name=
                'market_data_service', operation='get_symbols', cause=e)
...
class MarketDataCacheAdapter(IMarketDataCache):
    ...
    @async_with_exception_handling
    async def get_cached_data(self, symbol: str, timeframe: str, start_time:
        datetime, end_time: Optional[datetime]=None) ->Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            if not start_time:
                raise ValidationError('Start time cannot be empty', field=
                    'start_time')
            result = await self._market_data_cache.get(key=
                f"{symbol}_{timeframe}_{start_time.isoformat()}_{end_time.isoformat() if end_time else 'none'}"
                )
            return result
        except ValidationError:
            raise
            ...
            logger.warning(
                f'Error accessing cache for {symbol} {timeframe}: {str(e)}')
            return None

    @async_with_exception_handling
    async def cache_data(self, symbol: str, timeframe: str, data: pd.DataFrame
        ) ->bool:
        """
        Cache market data.
        ...
        try:
            ...
            key = (
                f'{symbol}_{timeframe}_{start_time.isoformat()}_{end_time.isoformat()}'
                )
            ...
            logger.error(
                f'Error caching data for {symbol} {timeframe}: {str(e)}')
            return False

    @async_with_exception_handling
    async def invalidate_cache(self, symbol: Optional[str]=None, timeframe:
        ...
        try:
            if symbol and timeframe:
                ...
                await self._market_data_cache.invalidate_pattern(pattern=
                    pattern)
            elif symbol:
                pattern = f'{symbol}_*'
                await self._market_data_cache.invalidate_pattern(pattern=
