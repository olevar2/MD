...
from data_pipeline_service.main import get_market_data_provider, get_market_data_cache
...
async def get_historical_data(
    ...
    market_data_provider: IMarketDataProvider = Depends(get_market_data_provider),
    ...
    try:
        ...
        cached_data = await market_data_cache.get_cached_data(
            symbol=symbol,
            timeframe=timeframe,
        ...
        # Cache miss, get data from provider
        logger.info(f"Cache miss for {symbol} {timeframe}, fetching from provider")
        data = await market_data_provider.get_historical_data(
            symbol=symbol,
            timeframe=timeframe,
            start_time=start_time,
            end_time=end_time
        ...
        await market_data_cache.cache_data(
            symbol=symbol,
            timeframe=timeframe,
            data=data
        )
...
async def get_latest_price(
    ...
    """
    try:
        # Get latest price from provider
