...
from data_pipeline_service.monitoring import (
...
async def get_query_performance() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting query performance stats: {str(e)}")


@router.get("/slow-queries", response_model=List[Dict[str, Any]])
async def get_slow_query_list(limit: int = Query(10, ge=1, le=100)) -> List[Dict[str, Any]]:
    """
...
async def set_slow_query_threshold(threshold: float = Query(..., gt=0)) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"status": "success", "message": f"Slow query threshold updated to {threshold}s"}
    except Exception as e:
        logger.error(f"Error updating slow query threshold: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error updating slow query threshold: {str(e)}")
