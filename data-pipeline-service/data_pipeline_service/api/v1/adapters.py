...
from data_pipeline_service.models.schemas import OHLCVData, PaginatedResponse, TickData
from data_pipeline_service.source_adapters.data_fetcher_manager import DataFetcherManager
from data_pipeline_service.source_adapters.dukascopy_adapter import DukascopyAdapter
...
# Initialize DataFetcherManager and adapters
data_fetcher_manager = DataFetcherManager()
dukascopy_adapter = DukascopyAdapter()
data_fetcher_manager.register_adapter("dukascopy", dukascopy_adapter, set_as_default=True)
...
class AdapterStatus(BaseModel):
    ...
    is_default_ohlcv: bool = Field(False, description="Whether this is the default OHLCV adapter")
    is_default_tick: bool = Field(False, description="Whether this is the default tick data adapter")
...
class InstrumentInfo(BaseModel):
    ...
    source_symbol: Optional[str] = Field(None, description="Symbol used by the data source")


@router.get(
    "",
    response_model=Dict[str, AdapterStatus],
    summary="Get adapter status",
    description="Get connection status for all registered data adapters.",
)
async def get_adapters_status():
    ...
    try:
        ...
        return {
            adapter_id: AdapterStatus(adapter_id=adapter_id, **status)
            for adapter_id, status in status_dict.items()
        }
...
@router.post(
    ...
    summary="Connect to all adapters",
    description="Connect to all registered data source adapters.",
)
...
@router.post(
    ...
    summary="Disconnect from all adapters",
    description="Disconnect from all registered data source adapters.",
)
...
@router.get(
    ...
    description="Get list of available instruments from a data source.",
)
async def get_instruments(
    ...
    """
    try:
        instruments = await data_fetcher_manager.get_available_instruments(adapter_id)
...
async def get_tick_data(
    symbol: str = Query(..., description=DESC_TRADING_SYMBOL),
    ...
    try:
        ...
        ticks = await data_fetcher_manager.get_tick_data(
            symbol, from_time, to_time, limit, adapter_id
        )
        return ticks
    except Exception as e:
...
@router.get(
    ...
    description="Get OHLCV data from a data source.",
)
async def get_ohlcv_data(
    ...
    try:
        ...
        candles = await data_fetcher_manager.get_ohlcv_data(
            symbol, timeframe, from_time, to_time, limit, adapter_id
        )
        return candles
    except Exception as e:
