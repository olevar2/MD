...
@router.get(
    ...
    description="Retrieve tick data for a specific instrument within a time range.",
)
async def get_tick_data(
    symbol: str = Query(..., description="Trading instrument symbol"),
    from_time: datetime = Query(
        ..., description="Start time for data query (ISO format)"
    ),
    to_time: datetime = Query(
        ..., description="End time for data query (ISO format)"
    ),
    limit: int = Query(
        10000, description="Maximum number of ticks to return"
    ),
    ...
    try:
        ...
        result = await service.get_tick_data(
            symbol=symbol,
            from_time=from_time,
            to_time=to_time,
            limit=limit,
...
@router.post(
    "",
    response_model=int,
    summary="Store tick data",
    description="Store a batch of tick data in the database.",
)
...
@router.get(
    "/window",
    response_model=PaginatedResponse,
    summary="Get tick data for a time window",
    description="Retrieve tick data for a specific instrument for a time window from now.",
)
async def get_tick_data_window(
    symbol: str = Query(..., description="Trading instrument symbol"),
    window: str = Query(
        "1h", description="Time window (e.g., '1h', '1d')"
    ),
    limit: int = Query(
        10000, description="Maximum number of ticks to return"
    ),
