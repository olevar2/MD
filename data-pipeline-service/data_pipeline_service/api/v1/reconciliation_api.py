...
class OHLCVReconciliationRequest(BaseModel):
    ...
    start_date: datetime = Field(..., description=
        'Start date for data reconciliation')
    end_date: datetime = Field(..., description=
        'End date for data reconciliation')
    timeframe: str = Field(..., description=
        "Timeframe for the data (e.g., '1h', '1d')")
...
@router.post('/ohlcv', response_model=ReconciliationResponse, responses={(
    400): {'model': ErrorResponse, 'description': 'Bad Request'}, (401): {
    'model': ErrorResponse, 'description': 'Unauthorized'}, (500): {'model':
    ErrorResponse, 'description': 'Internal Server Error'}})
@async_with_exception_handling
async def reconcile_ohlcv_data(request: OHLCVReconciliationRequest=Body(...
    ...
    try:
        logger.info(
            f'Received OHLCV data reconciliation request for symbol {request.symbol}, timeframe {request.timeframe}, strategy {request.strategy.name}'
            )
        result = await reconciliation_service.reconcile_ohlcv_data(symbol=
            request.symbol, start_date=request.start_date, end_date=request
            .end_date, timeframe=request.timeframe, strategy=request.
        ...
        logger.info(
            f'Completed OHLCV data reconciliation for symbol {request.symbol}, reconciliation ID: {result.reconciliation_id}, status: {result.status.name}, discrepancies: {result.discrepancy_count}, resolutions: {result.resolution_count}'
            )
        ...
        logger.error(
            f'Validation error during OHLCV data reconciliation: {str(e)}')
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail
...
@router.post('/tick-data', response_model=ReconciliationResponse, responses
    ={(400): {'model': ErrorResponse, 'description': 'Bad Request'}, (401):
    {'model': ErrorResponse, 'description': 'Unauthorized'}, (500): {
    'model': ErrorResponse, 'description': 'Internal Server Error'}})
@async_with_exception_handling
async def reconcile_tick_data(request: TickDataReconciliationRequest=Body(
    ...), api_key: str=Depends(get_api_key)) ->Dict[str, Any]:
    """
    Reconcile tick data.
    ...
    try:
        logger.info(
            f'Received tick data reconciliation request for symbol {request.symbol}, strategy {request.strategy.name}'
            )
        result = await reconciliation_service.reconcile_tick_data(symbol=
            request.symbol, start_date=request.start_date, end_date=request
            .end_date, strategy=request.strategy, tolerance=request.
            tolerance, auto_resolve=request.auto_resolve,
            notification_threshold=request.notification_threshold)
        logger.info(
            f'Completed tick data reconciliation for symbol {request.symbol}, reconciliation ID: {result.reconciliation_id}, status: {result.status.name}, discrepancies: {result.discrepancy_count}, resolutions: {result.resolution_count}'
            )
        return {'reconciliation_id': result.reconciliation_id, 'status':
            result.status.name, 'discrepancy_count': result.
            discrepancy_count, 'resolution_count': result.resolution_count,
        ...
        logger.error(
            f'Validation error during tick data reconciliation: {str(e)}')
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail
            ={'detail': f'Validation error: {str(e)}', 'error_type':
            'ValidationError'})
    except Exception as e:
