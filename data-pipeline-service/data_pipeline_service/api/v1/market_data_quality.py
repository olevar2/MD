...
from data_pipeline_service.quality.market_data_quality_framework import (
...
router = APIRouter(
    prefix="/api/v1/market-data-quality",
...
def get_quality_framework(
    ...
    if _quality_framework is None:
        _quality_framework = MarketDataQualityFramework(
...
class ValidateOHLCVRequest(BaseModel):
    ...
    instrument_type: str = Field("forex", description="Type of instrument (forex, crypto, stocks)")
    quality_level: DataQualityLevel = Field(DataQualityLevel.STANDARD, description="Quality level to apply")
    generate_report: bool = Field(False, description="Whether to generate a detailed report")
...
class ValidateAlternativeDataRequest(BaseModel):
    ...
    data_type: str = Field(..., description="Type of alternative data (news, economic, sentiment)")
    start_time: Optional[datetime] = Field(None, description="Start time for data retrieval")
    end_time: Optional[datetime] = Field(None, description="End time for data retrieval")
    generate_report: bool = Field(False, description="Whether to generate a detailed report")
...
class ValidationResponse(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Timestamp of validation")
...
@router.post("/validate/ohlcv", response_model=Union[ValidationResponse, DataQualityReport])
async def validate_ohlcv_data(
    request: ValidateOHLCVRequest,
    quality_framework: MarketDataQualityFramework = Depends(get_quality_framework),
    ...
    try:
        ...
        if not ohlcv_data:
            raise HTTPException(status_code=404, detail="No OHLCV data found for the specified parameters")
        ...
        # Validate data
        result = quality_framework.validate_ohlcv_data(
            data=df,
        ...
        if request.generate_report:
            ...
            return ValidationResponse(
                is_valid=result,
                message="OHLCV data validation passed" if result else "OHLCV data validation failed",
        ...
        raise HTTPException(status_code=500, detail=f"Error validating OHLCV data: {str(e)}")


@router.post("/validate/tick", response_model=Union[ValidationResponse, DataQualityReport])
async def validate_tick_data(
    request: ValidateTickRequest,
    quality_framework: MarketDataQualityFramework = Depends(get_quality_framework),
    market_data_service: MarketDataService = Depends(get_market_data_service)
):
    """
    ...
    try:
        ...
        # Get tick data
        tick_data = await market_data_service.get_tick_data(
            instrument=request.instrument,
            start_time=request.start_time,
            end_time=request.end_time
        )
        ...
        if not tick_data:
            raise HTTPException(status_code=404, detail="No tick data found for the specified parameters")
        ...
        # Validate data
        result = quality_framework.validate_tick_data(
            data=df,
            instrument_type=request.instrument_type,
            quality_level=request.quality_level,
            generate_report=request.generate_report
        ...
        if request.generate_report:
            ...
            return ValidationResponse(
                is_valid=result,
                message="Tick data validation passed" if result else "Tick data validation failed",
                timestamp=datetime.utcnow()
            )
        ...
        raise HTTPException(status_code=500, detail=f"Error validating tick data: {str(e)}")


@router.post("/validate/alternative", response_model=Union[ValidationResponse, DataQualityReport])
async def validate_alternative_data(
    request: ValidateAlternativeDataRequest,
    quality_framework: MarketDataQualityFramework = Depends(get_quality_framework),
    market_data_service: MarketDataService = Depends(get_market_data_service)
):
    """
    ...
    try:
        ...
        if not alternative_data:
            raise HTTPException(status_code=404, detail="No alternative data found for the specified parameters")
        ...
        # Validate data
        result = quality_framework.validate_alternative_data(
            data=df,
            data_type=request.data_type,
            generate_report=request.generate_report
        )
        ...
        if request.generate_report:
            ...
            return ValidationResponse(
                is_valid=result,
                message="Alternative data validation passed" if result else "Alternative data validation failed",
                timestamp=datetime.utcnow()
            )
        ...
        raise HTTPException(status_code=500, detail=f"Error validating alternative data: {str(e)}")


@router.get("/metrics", response_model=List[DataQualityMetrics])
async def get_data_quality_metrics(
    instrument: Optional[str] = Query(None, description="Instrument to get metrics for"),
    data_type: str = Query("ohlcv", description="Type of data (ohlcv, tick)"),
    ...
    """
    try:
        metrics = quality_framework.get_data_quality_metrics(
...
async def get_data_quality_sla(
    instrument_type: str = Path(..., description="Type of instrument (forex, crypto, stocks)"),
    data_type: str = Path(..., description="Type of data (ohlcv, tick)"),
    quality_framework: MarketDataQualityFramework = Depends(get_quality_framework)
):
    """
    ...
    """
    try:
        sla = quality_framework.get_data_quality_sla(
        ...
        raise HTTPException(status_code=500, detail=f"Error getting data quality SLA: {str(e)}")


@router.put("/sla/{key}", response_model=DataQualitySLA)
async def update_data_quality_sla(
    key: str = Path(..., description="SLA key (default, ohlcv, tick, forex, crypto, stocks, etc.)"),
    sla: SLAUpdateRequest = Body(..., description="SLA to update"),
    quality_framework: MarketDataQualityFramework = Depends(get_quality_framework)
):
    """
    ...
    """
    try:
        # Convert to DataQualitySLA
        data_quality_sla = DataQualitySLA(
            completeness=sla.completeness,
            timeliness=sla.timeliness,
            accuracy=sla.accuracy,
            consistency=sla.consistency,
            max_allowed_gaps_per_day=sla.max_allowed_gaps_per_day,
            max_allowed_spikes_per_day=sla.max_allowed_spikes_per_day,
            max_latency_seconds=sla.max_latency_seconds
        )
        
        # Update SLA
        quality_framework.set_data_quality_sla(
            sla=data_quality_sla,
            key=key
        )
        
        # Return updated SLA
        return quality_framework.get_data_quality_sla(
            instrument_type=key if key in ["forex", "crypto", "stocks"] else "forex",
            data_type=key if key in ["ohlcv", "tick"] else "ohlcv"
        )
    
    except Exception as e:
        logger.exception(f"Error updating data quality SLA: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error updating data quality SLA: {str(e)}")
