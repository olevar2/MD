...
def get_period_start(time: datetime, timeframe: str) -> datetime:
    ...
    if unit == 's':
        ...
        return (time - timedelta(days=days_since_monday)).replace(hour=0, minute=0, second=0, microsecond=0)
    else:
        raise ValueError(f'Unsupported timeframe unit: {unit}')
...
@router.get('/ohlcv', response_model=List[OHLCVData], summary='Retrieve historical OHLCV data', description='Get historical OHLCV data for a specified symbol and timeframe.', responses={200: {'description': 'OHLCV data retrieved successfully'}, 204: {'description': 'No data available for the requested parameters'}, 400: {'description': 'Invalid request parameters'}, 500: {'description': 'Internal server error'}})
async def get_historical_ohlcv(symbol: str=Query(..., description=DESC_SYMBOL), timeframe: Granularity=Query(..., description=DESC_TIMEFRAME), from_time: datetime=Query(None, description=DESC_START_TIME), to_time: datetime=Query(None, description=DESC_END_TIME), limit: Optional[int]=Query(1000, description=DESC_LIMIT), include_current: bool=Query(False, description=DESC_INCLUDE_CURRENT), sort_order: str=Query('asc', description=DESC_SORT_ORDER), fill_missing: bool=Query(False, description='Fill missing candles with interpolated data')):
    """
    ...
    try:
        ...
        logger.info(f'Retrieving OHLCV data for {symbol} {timeframe.value} from {from_time} to {to_time}')
        candles = await ohlcv_repository.get_ohlcv_data(symbol, timeframe.value, from_time, to_time, limit)
        if not candles:
            logger.warning(f'No OHLCV data found for {symbol} {timeframe.value}')
            return []
        if fill_missing and len(candles) > 0:
            candles = await ohlcv_service.fill_missing_candles(candles, timeframe.value, from_time, to_time)
        if not include_current and len(candles) > 0:
            current_period_start = get_period_start(datetime.now(timezone.utc), timeframe.value)
            candles = [candle for candle in candles if candle.timestamp.replace(tzinfo=timezone.utc) < current_period_start]
...
@router.get('/ohlcv-dataframe', summary='Retrieve OHLCV data as DataFrame', description='Get historical OHLCV data as pandas DataFrame, CSV, or NumPy array.', responses={200: {'description': 'OHLCV data retrieved successfully'}, 204: {'description': 'No data available for the requested parameters'}, 400: {'description': 'Invalid request parameters'}, 500: {'description': 'Internal server error'}})
async def get_ohlcv_dataframe(symbol: str=Query(..., description=DESC_SYMBOL), timeframe: Granularity=Query(..., description=DESC_TIMEFRAME), from_time: datetime=Query(None, description=DESC_START_TIME), to_time: datetime=Query(None, description=DESC_END_TIME), limit: Optional[int]=Query(1000, description=DESC_LIMIT), include_current: bool=Query(False, description=DESC_INCLUDE_CURRENT), sort_order: str=Query('asc', description=DESC_SORT_ORDER), fill_missing: bool=Query(False, description='Fill missing candles with interpolated data'), format: DataFormat=Query(DataFormat.JSON, description='Output data format')):
    """
    ...
    """
    try:
        candles = await get_historical_ohlcv(symbol, timeframe, from_time, to_time, limit, include_current, sort_order, fill_missing)
        if not candles:
            if format == DataFormat.JSON:
                return []
            raise HTTPException(status_code=204, detail='No data available for the requested parameters')
        if format in [DataFormat.DATAFRAME, DataFormat.CSV, DataFormat.NUMPY]:
            data = [{'timestamp': candle.timestamp, 'open': candle.open, 'high': candle.high, 'low': candle.low, 'close': candle.close, 'volume': candle.volume, 'symbol': candle.symbol, 'timeframe': candle.timeframe} for candle in candles]
            ...
            if format == DataFormat.DATAFRAME:
                ...
                return Response(content=csv_content, media_type='text/csv', headers={'Content-Disposition': f'attachment;filename={symbol}_{timeframe.value}.csv'})
...
def calculate_time_series_metrics(candles: List[OHLCVData], symbol: str, timeframe: str, from_time: datetime, to_time: datetime, source: str) -> TimeSeriesMetrics:
    """Helper function to calculate metrics from candle data."""
    if not candles:
        return TimeSeriesMetrics(symbol=symbol, timeframe=timeframe, start_time=from_time, end_time=to_time, count=0, earliest_timestamp=from_time, latest_timestamp=to_time, missing_points=calculate_expected_candles(from_time, to_time, timeframe), missing_percentage=100.0, average_gap=0, max_gap=0, source=source)
    ...
    return TimeSeriesMetrics(symbol=symbol, timeframe=timeframe, start_time=from_time, end_time=to_time, count=len(candles), earliest_timestamp=min(timestamps), latest_timestamp=max(timestamps), missing_points=missing_points, missing_percentage=missing_percentage, average_gap=avg_gap, max_gap=max_gap, source=source)

@router.get('/ohlcv/metrics', response_model=TimeSeriesMetrics, summary='Get OHLCV data metrics', description='Retrieve metadata and quality metrics about available OHLCV data.')
async def get_ohlcv_metrics(symbol: str=Query(..., description=DESC_SYMBOL), timeframe: Granularity=Query(..., description=DESC_TIMEFRAME), from_time: Optional[datetime]=Query(None, description='Start time for analysis'), to_time: Optional[datetime]=Query(None, description='End time for analysis')):
    """
    ...
    try:
        ...
        if from_time >= to_time:
            raise HTTPException(status_code=400, detail='from_time must be earlier than to_time')
        candles = await ohlcv_repository.get_ohlcv_data(symbol, timeframe.value, from_time, to_time, limit=None)
        source = await ohlcv_repository.get_data_source(symbol, timeframe.value) or 'unknown'
        metrics = calculate_time_series_metrics(candles=candles, symbol=symbol, timeframe=timeframe.value, from_time=from_time, to_time=to_time, source=source)
        if metrics.count == 0 and (not candles):
            raise HTTPException(status_code=204, detail='No data available for the requested parameters to calculate metrics')
        return metrics
...
@router.get('/ohlcv/available-ranges', response_model=Dict[str, Dict[str, Dict[str, Union[str, datetime]]]], summary='Get available data date ranges', description='Retrieve information about available data date ranges per symbol and timeframe.')
    ...
    try:
        ...
        for item in ranges:
            ...
            if symbol not in result:
                result[symbol] = {}
            result[symbol][timeframe] = {'earliest': start_date, 'latest': end_date, 'source': source}
        return result
    except Exception as e:
        logger.error(f'Error retrieving available data ranges: {str(e)}')
        raise HTTPException(status_code=500, detail=str(e))

@router.get('/ohlcv/available-symbols', response_model=Dict[str, List[str]], summary='Get available symbols by timeframe', description='Retrieve information about available symbols for each timeframe.')
...
def get_period_start(dt: datetime, timeframe: str) -> datetime:
    ...
    if timeframe.endswith('s'):
        ...
        return dt.replace(microsecond=0, second=0, minute=dt.minute - dt.minute % minutes)
    elif timeframe.endswith('h'):
        hours = int(timeframe[:-1])
        return dt.replace(microsecond=0, second=0, minute=0, hour=dt.hour - dt.hour % hours)
    elif timeframe.endswith('d'):
        return dt.replace(microsecond=0, second=0, minute=0, hour=0)
    elif timeframe.endswith('w'):
        days_from_monday = dt.weekday()
        return (dt - timedelta(days=days_from_monday)).replace(microsecond=0, second=0, minute=0, hour=0)
    elif timeframe.endswith('mn'):
        return dt.replace(microsecond=0, second=0, minute=0, hour=0, day=1)
    else:
        raise ValueError(f'Invalid timeframe format: {timeframe}')

def calculate_expected_candles(from_time: datetime, to_time: datetime, timeframe: str) -> int:
    """
...
@router.get('/ohlcv/filtered', response_model=List[OHLCVData], summary='Retrieve filtered OHLCV data', description='Get OHLCV data with advanced filtering options beyond basic time and symbol filters.', responses={200: {'description': 'Filtered OHLCV data retrieved successfully'}, 204: {'description': 'No data matches the filter criteria'}, 400: {'description': 'Invalid filter parameters'}, 500: {'description': 'Internal server error'}})
async def get_filtered_ohlcv(symbol: str=Query(..., description=DESC_SYMBOL), timeframe: Granularity=Query(..., description=DESC_TIMEFRAME), from_time: Optional[datetime]=Query(None, description=DESC_START_TIME), to_time: Optional[datetime]=Query(None, description=DESC_END_TIME), limit: Optional[int]=Query(1000, description=DESC_LIMIT), include_current: bool=Query(False, description=DESC_INCLUDE_CURRENT), sort_order: str=Query('asc', description=DESC_SORT_ORDER), filters: FilterParams=Depends()):
    """
    ...
    """
    try:
        candles = await get_historical_ohlcv(symbol, timeframe, from_time, to_time, limit, include_current, sort_order, False)
        if not candles:
            return []
...
@router.post('/ohlcv/batch', response_model=Dict[str, Dict[str, List[OHLCVData]]], summary='Batch retrieve OHLCV data', description='Get data for multiple symbols and timeframes in one request.', responses={200: {'description': 'Batch data retrieval successful'}, 400: {'description': 'Invalid request parameters'}, 500: {'description': 'Internal server error'}})
async def get_batch_ohlcv(request: Dict[str, List[str]]=Body(..., description='Dictionary mapping symbols to list of timeframes', example={'EURUSD': ['1m', '5m'], 'GBPUSD': ['1h']}), from_time: Optional[datetime]=Query(None, description=DESC_START_TIME), to_time: Optional[datetime]=Query(None, description=DESC_END_TIME), limit: Optional[int]=Query(1000, description='Maximum number of candles per symbol/timeframe'), include_current: bool=Query(False, description='Include current incomplete candle'), sort_order: str=Query('asc', description="Sorting order: 'asc' or 'desc'")):
    """
...
def filter_by_session(candles: List[OHLCVData], session: str) -> List[OHLCVData]:
    ...
    """
    session_hours = {'asian': (0, 9), 'london': (8, 16), 'new_york': (13, 21), 'overlap': (13, 16)}
    if session not in session_hours:
        raise ValueError(f'Invalid session: {session}')
    start_hour, end_hour = session_hours[session]
    return [candle for candle in candles if start_hour <= candle.timestamp.hour < end_hour]
...
async def get_historical_ohlcv_by_path(instrument: str=Path(..., description=DESC_INSTRUMENT_ID), start_time: datetime=Query(..., description='Start time for data retrieval (ISO format)'), end_time: datetime=Query(..., description='End time for data retrieval (ISO format)'), timeframe: TimeframeEnum=Query(..., description='Candle timeframe (e.g., 1m, 5m, 1h, 1d)'), include_incomplete: bool=Query(False, description='Include incomplete candles'), ohlcv_service: OHLCVService=Depends(get_ohlcv_service)):
    """
    ...
    try:
        ...
        data = await ohlcv_service.get_historical_ohlcv(instrument=instrument, start_time=start_time, end_time=end_time, timeframe=timeframe, include_incomplete=include_incomplete)
        return OHLCVResponse(instrument=instrument, timeframe=timeframe, start_time=start_time, end_time=end_time, candle_count=len(data), data=data)
...
async def get_aggregated_data(instrument: str=Path(..., description=DESC_INSTRUMENT_ID), start_time: datetime=Query(..., description='Start time for data retrieval (ISO format)'), end_time: datetime=Query(..., description='End time for data retrieval (ISO format)'), source_timeframe: TimeframeEnum=Query(..., description='Source timeframe (e.g., 1m, 5m)'), target_timeframe: TimeframeEnum=Query(..., description='Target timeframe (e.g., 1h, 1d)'), aggregation_method: AggregationMethodEnum=Query(AggregationMethodEnum.OHLCV, description='Aggregation method to use'), ohlcv_service: OHLCVService=Depends(get_ohlcv_service)):
    """
    ...
    try:
        ...
        raw_data = await ohlcv_service.get_historical_ohlcv(instrument=instrument, start_time=start_time, end_time=end_time, timeframe=source_timeframe)
        aggregator = TimeseriesAggregator()
        aggregated_data = aggregator.aggregate(data=raw_data, source_timeframe=source_timeframe, target_timeframe=target_timeframe, method=aggregation_method)
        return OHLCVResponse(instrument=instrument, timeframe=target_timeframe, start_time=start_time, end_time=end_time, candle_count=len(aggregated_data), data=aggregated_data)
    except ValidationError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
async def export_historical_ohlcv(instrument: str=Path(..., description=DESC_INSTRUMENT_ID), start_time: datetime=Query(..., description='Start time for data export (ISO format)'), end_time: datetime=Query(..., description='End time for data export (ISO format)'), timeframe: TimeframeEnum=Query(..., description='Candle timeframe (e.g., 1m, 5m, 1h, 1d)'), format: ExportFormatEnum=Query(ExportFormatEnum.CSV, description='Export format (csv, json, parquet)'), ohlcv_service: OHLCVService=Depends(get_ohlcv_service)):
    """
    ...
    try:
        ...
        validate_timeframe(timeframe)
        validate_date_range(start_time, end_time)
        data = await ohlcv_service.get_historical_ohlcv(instrument=instrument, start_time=start_time, end_time=end_time, timeframe=timeframe)
        if not data:
            return JSONResponse(status_code=404, content={'message': 'No data found for specified parameters'})
        if format == ExportFormatEnum.CSV:
            ...
            return StreamingResponse(io.StringIO(csv_content), media_type='text/csv', headers={'Content-Disposition': f'attachment; filename={filename}'})
        elif format == ExportFormatEnum.JSON:
            return {'metadata': {'instrument': instrument, 'timeframe': timeframe.value, 'start_time': start_time, 'end_time': end_time, 'count': len(data)}, 'data': format_ohlcv_for_json(data)}
            ...
            return StreamingResponse(io.BytesIO(parquet_bytes), media_type='application/octet-stream', headers={'Content-Disposition': f'attachment; filename={filename}'})
    except ValidationError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
async def get_ohlcv_batch_for_feature_calculation(request_data: OHLCVBatchRequest, ohlcv_service: OHLCVService=Depends(get_ohlcv_service), auth: ServiceAuth=Depends(verify_service_auth)):
    """
    ...
    try:
        results = await ohlcv_service.get_multi_instrument_ohlcv(instruments=request_data.instruments, start_time=request_data.start_time, end_time=request_data.end_time, timeframe=request_data.timeframe)
        return {'data': {instrument: format_ohlcv_for_json(data) for instrument, data in results.items()}}
    except Exception as e:
