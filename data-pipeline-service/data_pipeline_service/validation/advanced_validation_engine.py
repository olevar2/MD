...
class ValidationResult:
    ...
    def __init__(self, is_valid: bool, message: str, severity:
        ValidationSeverity=ValidationSeverity.ERROR, affected_rows:
        Optional[List[int]]=None, affected_columns: Optional[List[str]]=
        None, details: Optional[Dict[str, Any]]=None):
        """
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'is_valid': self.is_valid, 'message': self.message,
            'severity': self.severity.name, 'affected_rows_count': len(self
            .affected_rows), 'affected_columns': self.affected_columns,
            'details': self.details, 'timestamp': self.timestamp.isoformat()}

    def __str__(self) ->str:
        """String representation"""
        return (
            f"ValidationResult(valid={self.is_valid}, severity={self.severity.name}, message='{self.message}')"
            )
...
class DataValidator:
    ...
    def __init__(self, name: str, description: str, severity:
...
class NullValueValidator(DataValidator):
    ...
    def __init__(self, columns: Optional[List[str]]=None, max_null_fraction:
        float=0.0, severity: ValidationSeverity=ValidationSeverity.ERROR):
        """
        ...
        """
        super().__init__(name='null_value_validator', description=
            'Checks for null or missing values in data', severity=severity)
    ...
    def validate(self, data: pd.DataFrame) ->ValidationResult:
        ...
        if not check_columns:
            return ValidationResult(is_valid=False, message=
                'No valid columns to check', severity=ValidationSeverity.ERROR)
        problem_columns = []
        affected_rows = []
        column_null_counts = {}
        for column in check_columns:
            ...
            column_null_counts[column] = {'count': int(null_count),
        ...
        if problem_columns:
            return ValidationResult(is_valid=False, message=
                f'Found {len(problem_columns)} columns with excessive null values'
                , severity=self.severity, affected_rows=affected_rows,
                affected_columns=problem_columns, details={'null_counts':
                column_null_counts})
        else:
            return ValidationResult(is_valid=True, message=
                'No excessive null values found', severity=
                ValidationSeverity.INFO, details={'null_counts':
                column_null_counts})
...
class OutlierValidator(DataValidator):
    ...
    def __init__(self, columns: Optional[List[str]]=None, method: str=
        'zscore', threshold: float=3.0, severity: ValidationSeverity=
        ...
        Args:
            ...
            severity: Severity level for issues
        """
        super().__init__(name='outlier_validator', description=
            'Checks for outliers in numerical data', severity=severity)
        self.columns = columns
    ...
    def validate(self, data: pd.DataFrame) ->ValidationResult:
        ...
        if self.columns:
            check_columns = [col for col in self.columns if col in data.
                columns and pd.api.types.is_numeric_dtype(data[col])]
        else:
            check_columns = data.select_dtypes(include=['number']
                ).columns.tolist()
        if not check_columns:
            return ValidationResult(is_valid=False, message=
                'No valid numerical columns to check', severity=
        ...
        for column in check_columns:
            ...
            outlier_details[column] = {'count': int(outlier_count),
                'fraction': float(outlier_count / len(data)) if len(data) >
                0 else 0, 'method': self.method, 'threshold': self.threshold}
            if outlier_count > 0:
                problem_columns.append(column)
        ...
        if problem_columns:
            return ValidationResult(is_valid=False, message=
                f'Found outliers in {len(problem_columns)} columns',
                severity=self.severity, affected_rows=affected_rows,
                affected_columns=problem_columns, details={'outliers':
                outlier_details})
        else:
            return ValidationResult(is_valid=True, message=
                'No outliers found', severity=ValidationSeverity.INFO,
...
class OHLCVValidator(DataValidator):
    ...
    def __init__(self, open_col: str='open', high_col: str='high', low_col:
        str='low', close_col: str='close', volume_col: Optional[str]=
        'volume', timestamp_col: str='timestamp', check_gaps: bool=True,
        ...
        Args:
            ...
            severity: Severity level for issues
        """
        super().__init__(name='ohlcv_validator', description=
            'Validates OHLCV price data structure and integrity', severity=
            severity)
    ...
    def validate(self, data: pd.DataFrame) ->ValidationResult:
        ...
        required_cols = [self.open_col, self.high_col, self.low_col, self.
            close_col, self.timestamp_col]
        if self.volume_col:
        ...
        if missing_cols:
            return ValidationResult(is_valid=False, message=
                ...
                severity=self.severity, affected_columns=missing_cols)
        integrity_issues = []
        affected_rows = []
            ...
            try:
                ...
                if not gaps.empty:
                    ...
                    integrity_issues.append(
                        f'timestamp gaps ({len(gaps)} found)')
                    affected_rows.extend(gap_indices)
                    gap_details = {'gap_count': len(gaps), 'timeframe':
                        timeframe.total_seconds(), 'gap_positions': [int(i) for
                        i in gap_indices], 'gap_sizes': [g.total_seconds() for
                        g in gaps]}
        ...
        if integrity_issues:
            return ValidationResult(is_valid=False, message=
                f"OHLCV integrity issues found: {', '.join(integrity_issues)}",
                severity=self.severity, affected_rows=affected_rows,
                affected_columns=[self.open_col, self.high_col, self.
                low_col, self.close_col, self.timestamp_col], details={
                'issues': integrity_issues, 'gaps': gap_details})
        else:
            return ValidationResult(is_valid=True, message=
                'OHLCV data is valid', severity=ValidationSeverity.INFO,
...
class TickDataValidator(DataValidator):
    ...
    def __init__(self, bid_col: str='bid', ask_col: str='ask',
        timestamp_col: str='timestamp', max_spread_ratio: float=0.01,
        check_sequence: bool=True, severity: ValidationSeverity=
        ValidationSeverity.ERROR):
        """
        ...
        Args:
            ...
            severity: Severity level for issues
        """
        super().__init__(name='tick_data_validator', description=
            'Validates Forex tick data integrity', severity=severity)
    ...
    def validate(self, data: pd.DataFrame) ->ValidationResult:
        ...
        if self.max_spread_ratio > 0:
            ...
            if excessive_spread_mask.any():
                integrity_issues.append(
                    f'excessive spread (>{self.max_spread_ratio * 100}%)')
                affected_rows.extend(data[excessive_spread_mask].index.tolist()
                    )
        if self.check_sequence and len(data) > 1:
            if not pd.to_datetime(data[self.timestamp_col]
                ).is_monotonic_increasing:
                integrity_issues.append(
                    'timestamp sequence not monotonically increasing')
        ...
        if integrity_issues:
            return ValidationResult(is_valid=False, message=
                f"Tick data integrity issues found: {', '.join(integrity_issues)}"
                , severity=self.severity, affected_rows=affected_rows,
                affected_columns=[self.bid_col, self.ask_col, self.
                timestamp_col], details={'issues': integrity_issues})
        else:
            return ValidationResult(is_valid=True, message=
                'Tick data is valid', severity=ValidationSeverity.INFO)
...
class DataValidationEngine:
    ...
    def validate_data(self, data: pd.DataFrame) ->List[ValidationResult]:
        ...
        if not isinstance(data, pd.DataFrame):
            results.append(ValidationResult(is_valid=False, message=
                'Input data is not a pandas DataFrame', severity=
        ...
        for strategy in self.strategies:
            try:
                ...
                self.logger.warning(
                    f'Validation strategy {type(strategy).__name__} failed: {e}'
                    )
                results.append(ValidationResult(is_valid=False, message=
                    f'Strategy {type(strategy).__name__} execution error: {e}',
    ...
    def overall_validity(self, results: List[ValidationResult]) ->bool:
        ...
        Returns:
            ...
            False otherwise.
        """
        return not any(r.severity >= ValidationSeverity.ERROR for r in
            results if not r.is_valid)
