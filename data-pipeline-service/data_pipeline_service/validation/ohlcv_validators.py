...
import pandas as pd
import numpy as np
from common_lib.exceptions import DataValidationError # Updated import
...
def validate_instrument_format(instrument: str) -> None:
    ...
    """
    valid_formats = [
        # Currency pairs with underscore format (e.g., EUR_USD)
        r'^[A-Z]{3}_[A-Z]{3}$',
        # Stock tickers (e.g., AAPL, MSFT)
        r'^[A-Z]{1,5}$',
        # Indices (e.g., SPX500, NAS100)
        r'^[A-Z]{3,6}\d{2,3}$',
        # Cryptocurrency pairs (e.g., BTC_USD, ETH_EUR)
        r'^[A-Z]{3,4}_[A-Z]{3}$',
    ]
    ...
    raise ValidationError(f"Invalid instrument format: {instrument}. "
                          f"Expected format examples: EUR_USD, AAPL, SPX500, BTC_USD")
...
def validate_timeframe(timeframe: TimeframeEnum) -> None:
    ...
    if timeframe.value not in valid_timeframes:
        raise ValidationError(
            ...
            f"Supported timeframes: {', '.join(valid_timeframes)}"
        )
...
def validate_date_range(start_time: datetime, end_time: datetime, 
    ...
    # Check maximum time range if specified
...
def is_valid_timeframe_conversion(source_timeframe: TimeframeEnum, 
    ...
    """
    # Map timeframes to minutes for easy comparison
    timeframe_to_minutes = {
        TimeframeEnum.ONE_MINUTE: 1,
        TimeframeEnum.FIVE_MINUTES: 5,
        TimeframeEnum.FIFTEEN_MINUTES: 15,
        TimeframeEnum.THIRTY_MINUTES: 30,
        TimeframeEnum.ONE_HOUR: 60,
        TimeframeEnum.FOUR_HOURS: 240,
        TimeframeEnum.ONE_DAY: 1440,  # 24 * 60
        TimeframeEnum.ONE_WEEK: 10080  # 7 * 24 * 60
    }
    
    source_minutes = timeframe_to_minutes[source_timeframe]
    target_minutes = timeframe_to_minutes[target_timeframe]
    
    # Target timeframe must be larger and a multiple of source timeframe
    return (target_minutes > source_minutes and 
...
class CandlestickPatternValidator(ValidationStrategy):
    ...
    def validate(self, data: pd.DataFrame) -> ValidationResult:
        ...
        # Check candlestick integrity rules
        issues = []
        total_rows = len(data)
        ...
        if not high_open_invalid.empty:
            issues.append({
                "issue": "high_less_than_open",
                "count": len(high_open_invalid),
                "percentage": 100 * len(high_open_invalid) / total_rows
            })
            
        high_close_invalid = data[data["high"] < data["close"]]
        if not high_close_invalid.empty:
            issues.append({
                "issue": "high_less_than_close",
                "count": len(high_close_invalid),
                "percentage": 100 * len(high_close_invalid) / total_rows
            })
        ...
        if not low_open_invalid.empty:
            issues.append({
                "issue": "low_greater_than_open",
                "count": len(low_open_invalid),
                "percentage": 100 * len(low_open_invalid) / total_rows
            })
            
        low_close_invalid = data[data["low"] > data["close"]]
        if not low_close_invalid.empty:
            issues.append({
                "issue": "low_greater_than_close",
                "count": len(low_close_invalid),
                "percentage": 100 * len(low_close_invalid) / total_rows
            })
            
        if issues:
            return ValidationResult(
                is_valid=False,
                message="Candlestick pattern integrity issues detected",
                details={"issues": issues},
                severity=self.severity
            )
            
        return ValidationResult(is_valid=True, message="Candlestick pattern validation passed")
...
class VolumeChangeValidator(ValidationStrategy):
    ...
    def __init__(
        self, 
        max_relative_change: float = 10.0,
        window_size: int = 20,
        min_volume_for_check: float = 0.0,
    ...
    def validate(self, data: pd.DataFrame) -> ValidationResult:
        ...
        if len(data) <= self.window_size:
            return ValidationResult(is_valid=True, message="Not enough data points for volume change analysis")
            
        # Filter out low volume
        if self.min_volume_for_check > 0:
            data_filtered = data[data["volume"] >= self.min_volume_for_check].copy()
            
            # If all volumes are below threshold, skip the validation
            if len(data_filtered) == 0:
                return ValidationResult(is_valid=True, message="All volumes below minimum threshold for checking")
        else:
        ...
        for group_name, group_data in grouped:
            ...
            volume_ma = volume.rolling(window=self.window_size, min_periods=1).mean()
            
            # Calculate relative change compared to MA
            # Add small epsilon to avoid division by zero
            relative_change = (volume - volume_ma) / (volume_ma + 1e-10)
            
            # Find excessive changes
            excessive_change = group_data[abs(relative_change) > self.max_relative_change].copy()
            
            if not excessive_change.empty:
                # Take top 10 largest changes for reporting
                ...
                issues.append({
                    ...
                    "count": len(excessive_change),
                    "max_change": float(relative_change.abs().max()),
                    "examples": change_details
                })
        
        if issues:
            return ValidationResult(
                is_valid=True,  # Volume spikes don't necessarily make data invalid
                message="Suspicious volume changes detected",
                details={"issues": issues},
                severity=self.severity
            )
            
        return ValidationResult(is_valid=True, message="Volume change validation passed")
...
class GapDetectionValidator(ValidationStrategy):
    ...
    def __init__(
        ...
        allowed_gap_multiplier: float = 1.5, # Allow gaps up to 1.5x the timeframe
        severity: ValidationSeverity = ValidationSeverity.WARNING
    ):
        """
        ...
        timeframe_map = {
            TimeframeEnum.ONE_MINUTE: timedelta(minutes=1),
            TimeframeEnum.FIVE_MINUTES: timedelta(minutes=5),
            TimeframeEnum.FIFTEEN_MINUTES: timedelta(minutes=15),
            TimeframeEnum.THIRTY_MINUTES: timedelta(minutes=30),
            TimeframeEnum.ONE_HOUR: timedelta(hours=1),
            TimeframeEnum.FOUR_HOURS: timedelta(hours=4),
            TimeframeEnum.ONE_DAY: timedelta(days=1),
            TimeframeEnum.ONE_WEEK: timedelta(weeks=1)
        }
    ...
    def validate(self, data: pd.DataFrame) -> ValidationResult:
        ...
        if self.timestamp_column not in data.columns:
            return ValidationResult(
                is_valid=False,
                message=f"Missing timestamp column: {self.timestamp_column}",
                severity=ValidationSeverity.ERROR
            )
        ...
        if len(data) < 2:
            return ValidationResult(
                is_valid=True,
                message="Not enough data points for gap detection"
            )
        ...
        try:
            ...
            return ValidationResult(
                is_valid=False,
                message=f"Failed to process timestamp column: {str(e)}",
                severity=ValidationSeverity.ERROR
            )
        ...
        for group_name, group_data in groups:
            ...
            gaps = group_data.iloc[time_diffs[time_diffs > max_allowed_gap].index]

            if not gaps.empty:
                gap_details = []
                for i, row in gaps.head(10).iterrows(): # Report first 10 gaps
                    prev_idx = group_data.index.get_loc(i) - 1
                    if prev_idx >= 0:
                        prev_ts = group_data.iloc[prev_idx][self.timestamp_column]
                        gap_size = row[self.timestamp_column] - prev_ts
                        gap_details.append({
                            "timestamp": str(row[self.timestamp_column]),
                ...
                issues.append({
                    "instrument": str(group_name),
                    "count": len(gaps),
                    "percentage": 100 * len(gaps) / len(group_data),
                    "expected_interval_seconds": self.expected_interval.total_seconds(),
                    "max_allowed_gap_seconds": max_allowed_gap.total_seconds(),
                    "examples": gap_details
                })

        if issues:
            return ValidationResult(
                is_valid=True, # Gaps might be expected, treat as warning
                message="Time gaps detected in OHLCV data",
                details={"issues": issues},
                severity=self.severity
            )

        return ValidationResult(is_valid=True, message="No significant time gaps detected")
...
class VolumeChangeValidator(ValidationStrategy):
    ...
    def __init__(
        ...
        max_volume_spike_factor: Optional[float] = 10.0,
        min_volume_drop_factor: Optional[float] = 0.1,
    ...
    def validate(self, data: pd.DataFrame) -> ValidationResult:
        ...
        if not self.allow_zero_volume:
            ...
            if not zero_volume.empty:
                issues.append({
                    "issue": "zero_or_negative_volume",
                    "count": len(zero_volume),
                    "percentage": 100 * len(zero_volume) / total_rows,
                    "examples": zero_volume.head().to_dict('records')
                })

        # Check for spikes and drops (if factors are set)
        if (self.max_volume_spike_factor is not None or self.min_volume_drop_factor is not None) \
           and len(data) > self.rolling_window:
            ...
            for group_name, group_data in groups:
                 ...
                 rolling_avg = group_data[self.volume_column].rolling(window=self.rolling_window, min_periods=5).mean()
                 # Avoid division by zero or near-zero, handle NaNs
                 ...
                 if self.min_volume_drop_factor is not None:
                     drops = group_data[group_data[self.volume_column] < (rolling_avg * self.min_volume_drop_factor)]
                     if not drops.empty:
                         group_issues["drops"] = {
                             "count": len(drops),
        ...
        if issues:
            return ValidationResult(
                ...
                message="OHLCV volume change issues detected",
                details={"issues": issues},
                severity=self.severity
            )
    ...
    def validate(self, data: pd.DataFrame) -> ValidationResult:
        ...
        try:
            ...
            # Calculate expected volume based on High-Low range and a threshold
            # This is a heuristic and might need refinement based on asset characteristics
            volatility = df['high'] - df['low']
            # Avoid division by zero or issues with zero price
            median_price = (df['high'] + df['low']) / 2
            median_price[median_price == 0] = 1e-9 # Replace 0 with a small number

            # Simple heuristic: Volume should correlate somewhat with volatility relative to price
            # Define a threshold for 'unusual' volume relative to price movement
            # This threshold is arbitrary and needs tuning
            relative_volatility = volatility / median_price
            volume_volatility_ratio = df['volume'] / (relative_volatility + 1e-9) # Add epsilon

            # Identify outliers based on this ratio (e.g., using IQR or Z-score)
            ...
            outliers = df[(volume_volatility_ratio < lower_bound) | (volume_volatility_ratio > upper_bound)]

            if not outliers.empty:
                error_details = outliers[['open', 'high', 'low', 'close', 'volume']].to_dict('records')
                raise DataValidationError(
                    f"{len(outliers)} records found with potentially inconsistent volume relative to price range.",
                    validation_errors={"inconsistent_volume_records": error_details},
                    validator=self.__class__.__name__
                )
        except DataValidationError:
             raise # Re-raise specific validation errors
        except Exception as e: # Catch unexpected errors during the validation logic
            self.logger.exception(f"Unexpected error during volume consistency validation: {e}")
