...
class ValidationSeverity(str, Enum):
    ...
    INFO = "info"  # Just informational, doesn't affect validity
    WARNING = "warning"  # Potential issue but data still usable
    ERROR = "error"  # Critical issue, data not usable
    ...
    def validate(self, data: Any) -> Tuple[bool, List[ValidationResult]]:
        ...
        for validator in self.validators:
            try:
                ...
                # If any validator with ERROR severity fails, the overall result is invalid
                if not result.is_valid and result.severity == ValidationSeverity.ERROR:
                    is_valid = False
                    
            except DataValidationError as e: # Catch specific validation errors first
                logger.warning(f"Data validation failed using {validator.__class__.__name__}: {e}")
                results.append(e.to_dict()) # Append structured error info
                is_valid = False
            except (DataError, ForexTradingPlatformError) as e: # Catch other known platform errors
                 logger.warning(f"Platform error during validation using {validator.__class__.__name__}: {e}")
                 results.append({
                ...
                logger.exception(f"Unexpected error during validation using {validator.__class__.__name__}: {e}")
                # Record unexpected errors as well, but maybe flag them differently
                results.append({
                    "error_type": "UnexpectedValidationError",
                    "message": f"An unexpected error occurred in {validator.__class__.__name__}: {str(e)}",
                    "validator": validator.__class__.__name__,
...
class DataValidationEngine:
    ...
    def register_validator(self, data_type: str, validator: CompositeValidator) -> None:
        """
    ...
    def validate(self, data: Any, data_type: str) -> bool:
        ...
        if data_type not in self.validators:
            raise DataValidationError(f"No validator registered for data type: {data_type}")
        ...
        for result in results:
            ...
            if result.severity == ValidationSeverity.ERROR:
                logger.error(f"Validation error: {result.message}", extra=result.details)
            elif result.severity == ValidationSeverity.WARNING:
                logger.warning(f"Validation warning: {result.message}", extra=result.details)
            else:
                logger.info(f"Validation info: {result.message}", extra=result.details)
...
class SchemaValidator(ValidationStrategy):
    ...
    def __init__(self, required_columns: List[str], dtypes: Optional[Dict[str, Any]] = None):
        """
    ...
    def validate(self, data: pd.DataFrame) -> ValidationResult:
        ...
        for col, expected_dtype in self.dtypes.items():
            if col in data.columns:
                # Get actual dtype and check compatibility
                actual_dtype = data[col].dtype
                if not self._check_dtype_compatibility(actual_dtype, expected_dtype):
                    dtype_issues.append({
                        "column": col,
        ...
        if dtype_issues:
            return ValidationResult(
                is_valid=False,
                message="Data type issues detected",
    ...
    def _check_dtype_compatibility(self, actual, expected) -> bool:
        ...
        # Common numeric compatibility
        if str(expected) in ('float', 'float64') and str(actual) in ('int', 'int64'):
            return True
...
class NullValidator(ValidationStrategy):
    ...
    def __init__(
        ...
        max_null_percentage: Optional[Dict[str, float]] = None,
        severity: ValidationSeverity = ValidationSeverity.ERROR
    ):
        """
    ...
    def validate(self, data: pd.DataFrame) -> ValidationResult:
        ...
        # Check percentage thresholds for specified columns
        ...
        if null_issues:
            return ValidationResult(
                is_valid=False,
                message="Null value issues detected",
...
class OutlierValidator(ValidationStrategy):
        ...
        for col in self.columns_to_check:
            ...
            if len(outlier_indices) > 0:
                outlier_issues[col] = {
                    ...
                    # Only include first 10 outlier positions to avoid huge logs
                    "sample_indices": outlier_indices[:10]
                }
        
        if outlier_issues:
            return ValidationResult(
                is_valid=True,  # Outliers don't necessarily make data invalid
                message="Outliers detected in data",
    ...
    def _detect_outliers(self, series: pd.Series) -> pd.Series:
        ...
        if self.method == "zscore":
            ...
            # Avoid multiplication with zero
            if iqr == 0:
                return pd.Series(False, index=series.index)
...
class ForexPriceValidator(ValidationStrategy):
    ...
    def __init__(
        ...
        check_open_range: bool = True,
        check_high_low: bool = True,
        check_close_range: bool = True,
        severity: ValidationSeverity = ValidationSeverity.ERROR
    ):
        """
    ...
    def validate(self, data: pd.DataFrame) -> ValidationResult:
        ...
        # Determine what columns we need based on validation types
        if self.check_open_range and 'open' not in data.columns:
            return ValidationResult(
                is_valid=False,
                message="Missing 'open' column for open range validation",
                severity=ValidationSeverity.ERROR
            )
        ...
        if self.check_high_low:
            if 'high' not in data.columns or 'low' not in data.columns:
                return ValidationResult(
                    is_valid=False,
                    message="Missing 'high' or 'low' columns for high-low validation",
                    severity=ValidationSeverity.ERROR
                )
        ...
        if self.check_close_range and 'close' not in data.columns:
            return ValidationResult(
                is_valid=False,
                message="Missing 'close' column for close range validation",
                severity=ValidationSeverity.ERROR
            )
        
        issues = []
        
        # Group by instrument to validate each separately
        for instrument, group in data.groupby('instrument'):
            # Skip instruments not in our config
            ...
            # Check price ranges
            if self.check_open_range and min_price is not None and max_price is not None:
                invalid_open = group[(group['open'] < min_price) | (group['open'] > max_price)]
                if not invalid_open.empty:
                    issues.append({
                        "instrument": instrument,
                        "issue": "open_price_out_of_range",
            ...
            if self.check_close_range and min_price is not None and max_price is not None:
                invalid_close = group[(group['close'] < min_price) | (group['close'] > max_price)]
        ...
        if issues:
            return ValidationResult(
                is_valid=False,
                message="Forex price validation issues detected",
                details={"issues": issues},
                severity=self.severity
            )
        
        return ValidationResult(is_valid=True, message="Forex price validation passed")
...
class ForexSpreadValidator(ValidationStrategy):
    ...
    def validate(self, data: pd.DataFrame) -> ValidationResult:
        ...
        if self.bid_column not in data.columns:
            return ValidationResult(
                is_valid=False,
                message=f"Missing bid column '{self.bid_column}'",
                severity=ValidationSeverity.ERROR
            )
        ...
        if self.ask_column not in data.columns:
            return ValidationResult(
                is_valid=False,
                message=f"Missing ask column '{self.ask_column}'",
                severity=ValidationSeverity.ERROR
            )
        ...
        for instrument, group in data.groupby('instrument'):
            ...
            config = self.instrument_config[instrument]
            decimals = config.get('decimals', 5)
            max_spread_pips = config.get('max_spread_pips')
            ...
            spread_pips = (group[self.ask_column] - group[self.bid_column]) * pip_multiplier
            
            # Find instances of spread > max_spread_pips
            high_spread = group[spread_pips > max_spread_pips]
            
            if not high_spread.empty:
                issues.append({
                    "instrument": instrument,
                    "issue": "spread_exceeded_max",
                    "count": len(high_spread),
                    "max_allowed_pips": max_spread_pips,
                    "max_found_pips": float(spread_pips.max())
                })
                
            # Find instances of negative spread (which should never happen)
            ...
            if not negative_spread.empty:
                issues.append({
                    ...
                    "count": len(negative_spread)
                })
        
        if issues:
            return ValidationResult(
                is_valid=False,
                message="Forex spread validation issues detected",
                details={"issues": issues},
                severity=self.severity
            )
        
        return ValidationResult(is_valid=True, message="Forex spread validation passed")
...
class TimeSeriesContinuityValidator(ValidationStrategy):
    ...
    def validate(self, data: pd.DataFrame) -> ValidationResult:
        ...
        if not large_gaps.empty:
            # Get sample of gaps for reporting
            ...
            for i, row in sample_gaps.iterrows():
                ...
                gap_details.append({
                    "position": int(i),
                    "gap_size": str(gaps[i]),
                    "from_timestamp": str(prev_ts),
                    "to_timestamp": str(row[self.timestamp_column])
                })
                
            return ValidationResult(
                is_valid=True,  # Time series gaps don't necessarily make data invalid
                message=f"Time series continuity issues detected - {len(large_gaps)} gaps larger than {self.max_gap}",
                details={
                    "total_gaps": len(large_gaps),
                    "max_gap": str(gaps.max()),
                    "sample_gaps": gap_details
                },
                severity=self.severity
            )
            
        return ValidationResult(is_valid=True, message="Time series continuity validation passed")