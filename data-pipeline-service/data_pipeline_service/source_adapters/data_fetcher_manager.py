...
from common_lib.exceptions import DataFetchError, DataTransformationError, DataValidationError, ServiceError
...
from data_pipeline_service.source_adapters.base_adapter import DataSourceAdapter, OHLCVDataSourceAdapter, TickDataSourceAdapter
...
class DataFetcherManager:
    ...
    def register_adapter(self, adapter_id: str, adapter: DataSourceAdapter,
    ...
    @async_with_exception_handling
    async def get_ohlcv_data(self, symbol: str, timeframe: str, from_time:
        datetime, to_time: datetime, limit: Optional[int]=None, adapter_id:
        Optional[str]=None) ->List[OHLCVData]:
        """
        ...
        if adapter_id:
            ...
            if not adapter or not isinstance(adapter, OHLCVDataSourceAdapter):
                raise ValueError(
                    f'No valid OHLCV adapter found with ID: {adapter_id}')
        else:
        ...
        try:
            data_dicts = await adapter.get_ohlcv_data(symbol, timeframe,
                from_time, to_time, limit)
            return [OHLCVData(symbol=d['symbol'], timestamp=d['timestamp'],
                timeframe=d['timeframe'], open=d['open'], high=d['high'],
                low=d['low'], close=d['close'], volume=d['volume']) for d in
                data_dicts]
        except Exception as e:
            logger.error(f'Error fetching OHLCV data: {str(e)}')
            raise DataFetchError(f'Failed to fetch OHLCV data for {symbol}',
                source=adapter_id or 'default') from e

    @async_with_exception_handling
    async def get_tick_data(self, symbol: str, from_time: datetime, to_time:
        datetime, limit: Optional[int]=None, adapter_id: Optional[str]=None
        ) ->List[TickData]:
        """
        ...
        if adapter_id:
            ...
            if not adapter or not isinstance(adapter, TickDataSourceAdapter):
                raise ValueError(
                    f'No valid tick data adapter found with ID: {adapter_id}')
        else:
        ...
        try:
            if hasattr(adapter, 'get_tick_data_objects'):
                return await adapter.get_tick_data_objects(symbol,
                    from_time, to_time, limit)
            data_dicts = await adapter.get_tick_data(symbol, from_time,
                to_time, limit)
            return [TickData(symbol=d['symbol'], timestamp=d['timestamp'],
                bid=d['bid'], ask=d['ask'], bid_volume=d.get('bid_volume', 
                0.0), ask_volume=d.get('ask_volume', 0.0)) for d in data_dicts]
        except Exception as e:
            logger.error(f'Error fetching tick data: {str(e)}')
            raise DataFetchError(f'Failed to fetch tick data for {symbol}',
                source=adapter_id or 'default') from e

    async def get_available_instruments(self, adapter_id: Optional[str]=None
        ) ->List[Dict[str, Any]]:
        """
        ...
        if adapter_id:
            ...
            adapter = self.get_default_ohlcv_adapter(
                ) or self.get_default_tick_adapter()
            if not adapter:
    ...
    async def health_check(self) ->Dict[str, Dict[str, Any]]:
        ...
        for adapter_id, adapter in self._adapters.items():
            try:
                ...
                results[adapter_id] = {'status': 'connected' if
                    is_connected else 'disconnected', 'type': adapter.
                    __class__.__name__, 'is_default_ohlcv': adapter_id ==
                    self._default_ohlcv_adapter, 'is_default_tick': 
                    adapter_id == self._default_tick_adapter}
            except Exception as e:
                results[adapter_id] = {'status': 'error', 'message': str(e),
                    'type': adapter.__class__.__name__}
        return results

    async def subscribe_to_ticks(self, symbol: str, adapter_id: Optional[
        str]=None) ->bool:
        """
    ...
    async def unsubscribe_from_ticks(self, symbol: str, adapter_id:
    ...
    @async_with_exception_handling
    async def fetch_historical_ohlcv(self, symbol: str, start_date:
        datetime, end_date: datetime, timeframe: str) ->List[OHLCVData]:
        """
        ...
        if not adapters:
            raise ConfigurationError(
                f'No adapters configured or available for OHLCV data for symbol {symbol}'
                )
        tasks = []
        adapter_names = list(adapters.keys())
        for source_name, adapter in adapters.items():
            tasks.append(self._fetch_historical_data_async(adapter,
                source_name, symbol, start_date, end_date, timeframe))
        task_results = await asyncio.gather(*tasks, return_exceptions=True)
        for i, result in enumerate(task_results):
            source_name = adapter_names[i]
            if isinstance(result, Exception):
                if isinstance(result, (DataFetchError, DataValidationError,
                    ServiceError)):
                    self.logger.error(
                        f'Error fetching OHLCV data from {source_name} for {symbol}: {result}'
                        )
                    errors[source_name] = result
                else:
                    self.logger.exception(
                        f'Unexpected exception fetching OHLCV data from {source_name} for {symbol}: {result}'
                        )
                    errors[source_name] = DataFetchError(
                        f'Unexpected error from {source_name}: {result}',
                        source=source_name)
            elif result:
                results[source_name] = result
                self.logger.info(
                    f'Successfully fetched and validated {len(result)} OHLCV records for {symbol} from {source_name}'
                    )
            else:
                self.logger.warning(
                    f'No OHLCV data returned for {symbol} from {source_name} between {start_date} and {end_date}'
                    )
        if not results and errors:
            first_error = next(iter(errors.values()))
            raise DataFetchError(
                f'Failed to fetch data for {symbol} from all sources. First error: {first_error}'
                , details=errors) from first_error
        elif not results:
            raise DataFetchError(
                f'No data could be fetched for {symbol} from any configured source.'
                )
        try:
            processed_data = self._process_fetched_data(results, symbol,
                timeframe)
            return processed_data
        except DataTransformationError as e:
            self.logger.error(
                f'Error processing fetched data for {symbol}: {e}')
            raise
        except Exception as e:
            self.logger.exception(
                f'Unexpected error processing fetched data for {symbol}: {e}')
            raise DataTransformationError(
                f'Unexpected error processing data for {symbol}: {e}',
                transformation='DataFetcherManager Aggregation') from e

    @async_with_exception_handling
    async def fetch_tick_data(self, symbol: str, start_date: datetime,
        ...
        if not adapters:
            raise ConfigurationError(
                f'No adapters configured or available for tick data for symbol {symbol}'
                )
        tasks = []
        adapter_names = list(adapters.keys())
        for source_name, adapter in adapters.items():
            tasks.append(self._fetch_tick_data_async(adapter, source_name,
        ...
        for i, result in enumerate(task_results):
            ...
            if isinstance(result, Exception):
                if isinstance(result, (DataFetchError, DataValidationError,
                    ServiceError)):
                    self.logger.error(
                        f'Error fetching tick data from {source_name} for {symbol}: {result}'
                        )
                    errors[source_name] = result
                else:
                    self.logger.exception(
                        f'Unexpected exception fetching tick data from {source_name} for {symbol}: {result}'
                        )
                    errors[source_name] = DataFetchError(
                        f'Unexpected error from {source_name}: {result}',
                        source=source_name)
                ...
                results[source_name] = result
                self.logger.info(
                    f'Successfully fetched and validated {len(result)} tick records for {symbol} from {source_name}'
                    )
            else:
                self.logger.warning(
                    f'No tick data returned for {symbol} from {source_name} between {start_date} and {end_date}'
                    )
        if not results and errors:
            first_error = next(iter(errors.values()))
            raise DataFetchError(
                f'Failed to fetch tick data for {symbol} from all sources. First error: {first_error}'
                , details=errors) from first_error
        elif not results:
            raise DataFetchError(
                f'No tick data could be fetched for {symbol} from any configured source.'
                )
        try:
            processed_data = self._process_fetched_data(results, symbol, 'tick'
                )
            return processed_data
        except DataTransformationError as e:
            self.logger.error(
                f'Error processing fetched tick data for {symbol}: {e}')
            raise
        except Exception as e:
            self.logger.exception(
                f'Unexpected error processing fetched tick data for {symbol}: {e}'
                )
            raise DataTransformationError(
                f'Unexpected error processing tick data for {symbol}: {e}',
                transformation='DataFetcherManager Tick Aggregation') from e

    @async_with_exception_handling
    async def _fetch_historical_data_async(self, adapter: BaseAdapter,
        source_name: str, symbol: str, start_date: datetime, end_date:
        datetime, timeframe: str) ->List[OHLCVData]:
        """Helper coroutine to fetch and validate historical data from a single adapter."""
        try:
            raw_data = await adapter.get_historical_data(symbol, start_date,
                end_date, timeframe)
            if raw_data is None:
                return []
            validated_data = self.validation_engine.validate_ohlcv(raw_data,
            ...
            self.logger.exception(
                f'Unexpected error in adapter {source_name} for {symbol} OHLCV: {e}'
                )
            raise DataFetchError(f'Unexpected adapter error: {e}', source=
                source_name) from e

    async def _fetch_tick_data_async(self, adapter: BaseAdapter,
        source_name: str, symbol: str, start_date: datetime, end_date: datetime
        ) ->List[TickData]:
