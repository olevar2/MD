...
class FeatureStoreClient:
    ...
    def __init__(self, base_url: Optional[str]=None, api_key: Optional[str]
        ...
        """
        self.base_url = base_url or os.getenv('FEATURE_STORE_URL',
        ...
        self.logger.info(
            f'FeatureStoreClient initialized with base URL: {self.base_url}')
    ...
    @with_exception_handling
    def get_ohlcv_data(self, symbol: str, start_date: Union[str, datetime],
        end_date: Union[str, datetime], timeframe: str='1h') ->pd.DataFrame:
        """
        ...
        try:
            ...
            params = {'symbol': symbol, 'start_date': start_date_str,
                'end_date': end_date_str, 'timeframe': timeframe}
            self.logger.info(
                f'Requesting OHLCV data for {symbol} from {start_date_str} to {end_date_str} ({timeframe})'
                )
            ...
            if os.getenv('ENVIRONMENT', 'development') == 'development':
                ...
                return self._get_sample_ohlcv_data(symbol, start_date,
                    end_date, timeframe)
            else:
                raise

    @with_exception_handling
    def get_indicators(self, symbol: str, start_date: Union[str, datetime],
        end_date: Union[str, datetime], timeframe: str='1h', indicators:
        ...
        try:
            ...
            self.logger.info(
                f'Requesting indicators for {symbol} from {start_date_str} to {end_date_str} ({timeframe})'
                )
            if indicators:
            ...
            if os.getenv('ENVIRONMENT', 'development') == 'development':
                self.logger.warning(
                    'Returning sample indicator data for development')
                return self._get_sample_indicator_data(symbol, start_date,
                    end_date, timeframe, indicators)
            else:
                raise
    ...
    def get_available_indicators(self) ->List[Dict[str, Any]]:
        ...
        try:
            ...
            if 'data' in data and isinstance(data['data'], list):
                self.logger.info(
                    f"Retrieved information for {len(data['data'])} indicators"
                    )
                return data['data']
            else:
            ...
            self.logger.error(
                f'Error retrieving indicator information: {str(e)}')
            if os.getenv('ENVIRONMENT', 'development') == 'development':
                self.logger.warning(
                    'Returning sample indicator information for development')
                return self._get_sample_indicator_info()
            else:
                raise

    @with_exception_handling
    def compute_feature(self, feature_name: str, symbol: str, start_date:
        Union[str, datetime], end_date: Union[str, datetime], timeframe:
        str='1h', parameters: Optional[Dict[str, Any]]=None) ->pd.DataFrame:
        """
        ...
        try:
            ...
            body = {'feature_name': feature_name, 'symbol': symbol,
                'start_date': start_date_str, 'end_date': end_date_str,
                'timeframe': timeframe}
            if parameters:
                body['parameters'] = parameters
            self.logger.info(
                f"Requesting computation of feature '{feature_name}' for {symbol}"
                )
            ...
            if 'data' in data and data['data']:
                ...
                if 'timestamp' in df.columns:
                    df['timestamp'] = pd.to_datetime(df['timestamp'])
                self.logger.info(
                    f'Retrieved {len(df)} computed feature records')
                return df
            else:
                self.logger.warning(
                    f'No computed feature data returned for {feature_name}')
                return pd.DataFrame()
        except Exception as e:
    ...
    def _get_sample_ohlcv_data(self, symbol: str, start_date: Union[str,
        datetime], end_date: Union[str, datetime], timeframe: str
        ) ->pd.DataFrame:
        """
        ...
        df = pd.DataFrame({'timestamp': dates, 'open': opens, 'high': highs,
            'low': lows, 'close': closes, 'volume': volumes})
        return df

    def _get_sample_indicator_data(self, symbol: str, start_date: Union[str,
        datetime], end_date: Union[str, datetime], timeframe: str,
        ...
        """
        ohlcv_data = self._get_sample_ohlcv_data(symbol, start_date,
            end_date, timeframe)
        if ohlcv_data.empty:
            return pd.DataFrame()
        if not indicators:
        ...
        for indicator in indicators:
            if 'sma' in indicator:
                window = int(indicator.split('_')[1]
                    ) if '_' in indicator else 5
                df[indicator] = ohlcv_data['close'].rolling(window=min(
                    window, 3)).mean()
            elif 'ema' in indicator:
                window = int(indicator.split('_')[1]
                    ) if '_' in indicator else 5
                df[indicator] = ohlcv_data['close'].ewm(span=min(window, 3)
                    ).mean()
                ...
                if indicator == 'bb_upper':
                    df[indicator] = ohlcv_data['close'] + np.random.uniform(
                        0.001, 0.003, len(ohlcv_data))
    ...
    def _get_sample_indicator_info(self) ->List[Dict[str, Any]]:
        ...
        """
        return [{'id': 'sma_5', 'name': 'Simple Moving Average (5)',
            'description': '5-period Simple Moving Average', 'category':
            'trend', 'default_parameters': {'window': 5},
            'available_timeframes': ['1m', '5m', '15m', '1h', '4h', '1d']},
            {'id': 'sma_10', 'name': 'Simple Moving Average (10)',
            'description': '10-period Simple Moving Average', 'category':
            'trend', 'default_parameters': {'window': 10},
            'available_timeframes': ['1m', '5m', '15m', '1h', '4h', '1d']},
            {'id': 'sma_20', 'name': 'Simple Moving Average (20)',
            'description': '20-period Simple Moving Average', 'category':
            'trend', 'default_parameters': {'window': 20},
            'available_timeframes': ['1m', '5m', '15m', '1h', '4h', '1d']},
            {'id': 'ema_5', 'name': 'Exponential Moving Average (5)',
            'description': '5-period Exponential Moving Average',
            'category': 'trend', 'default_parameters': {'window': 5},
            'available_timeframes': ['1m', '5m', '15m', '1h', '4h', '1d']},
            {'id': 'ema_10', 'name': 'Exponential Moving Average (10)',
            'description': '10-period Exponential Moving Average',
            'category': 'trend', 'default_parameters': {'window': 10},
            'available_timeframes': ['1m', '5m', '15m', '1h', '4h', '1d']},
            {'id': 'ema_20', 'name': 'Exponential Moving Average (20)',
            'description': '20-period Exponential Moving Average',
            'category': 'trend', 'default_parameters': {'window': 20},
            'available_timeframes': ['1m', '5m', '15m', '1h', '4h', '1d']},
            {'id': 'rsi_14', 'name': 'Relative Strength Index (14)',
            'description': '14-period Relative Strength Index', 'category':
            'momentum', 'default_parameters': {'window': 14},
            'available_timeframes': ['1m', '5m', '15m', '1h', '4h', '1d']},
            {'id': 'macd', 'name': 'MACD Line', 'description':
            'Moving Average Convergence Divergence Line', 'category':
            'momentum', 'default_parameters': {'fast_window': 12,
            'slow_window': 26}, 'available_timeframes': ['5m', '15m', '1h',
            '4h', '1d']}, {'id': 'macd_signal', 'name': 'MACD Signal Line',
            'description':
            'Signal Line for Moving Average Convergence Divergence',
            'category': 'momentum', 'default_parameters': {'fast_window': 
            12, 'slow_window': 26, 'signal_window': 9},
            'available_timeframes': ['5m', '15m', '1h', '4h', '1d']}, {'id':
            'macd_hist', 'name': 'MACD Histogram', 'description':
            'Histogram showing difference between MACD Line and Signal Line',
            'category': 'momentum', 'default_parameters': {'fast_window': 
            12, 'slow_window': 26, 'signal_window': 9},
            'available_timeframes': ['5m', '15m', '1h', '4h', '1d']}, {'id':
            'bb_upper', 'name': 'Bollinger Bands (Upper)', 'description':
            'Upper band of Bollinger Bands', 'category': 'volatility',
            'default_parameters': {'window': 20, 'std_dev': 2},
            'available_timeframes': ['1m', '5m', '15m', '1h', '4h', '1d']},
            {'id': 'bb_middle', 'name': 'Bollinger Bands (Middle)',
            'description': 'Middle band of Bollinger Bands (SMA)',
            'category': 'volatility', 'default_parameters': {'window': 20},
            'available_timeframes': ['1m', '5m', '15m', '1h', '4h', '1d']},
            {'id': 'bb_lower', 'name': 'Bollinger Bands (Lower)',
            'description': 'Lower band of Bollinger Bands', 'category':
            'volatility', 'default_parameters': {'window': 20, 'std_dev': 2
            }, 'available_timeframes': ['1m', '5m', '15m', '1h', '4h', '1d'
            ]}, {'id': 'atr_14', 'name': 'Average True Range (14)',
            'description': '14-period Average True Range', 'category':
            'volatility', 'default_parameters': {'window': 14},
            'available_timeframes': ['1m', '5m', '15m', '1h', '4h', '1d']}]
