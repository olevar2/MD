...
COLOR_SCHEME = {
    ...
    'grid': '#e5e5e5',
    'text': '#333333',
    'highlight': '#bcbd22',  # Yellow
    'comparison': '#9467bd'  # Purple
}
...
class RewardVisualization:
    ...
    def plot_episode_rewards(
        ...
        plt.plot(rewards, alpha=0.5, color=COLOR_SCHEME['primary'], label='Episode Rewards')
        
        # Calculate moving average
        if len(rewards) >= window_size:
            moving_avg = [np.mean(rewards[max(0, i-window_size):i+1]) for i in range(len(rewards))]
            plt.plot(moving_avg, color=COLOR_SCHEME['secondary'], linewidth=2, label=f'{window_size}-Episode Moving Average')
    ...
    def plot_reward_distribution(
        ...
        plt.axvline(np.mean(rewards), color=COLOR_SCHEME['secondary'], linestyle='dashed', linewidth=2, 
                   label=f'Mean: {np.mean(rewards):.2f}')
        plt.axvline(np.median(rewards), color=COLOR_SCHEME['tertiary'], linestyle='dashed', linewidth=2,
    ...
    @staticmethod
    def create_interactive_rewards_plot(
        ...
        try:
            ...
            for label, rewards in rewards_data.items():
                fig.add_trace(go.Scatter(
                    ...
                    opacity=0.3,
                    visible=False  # Hide raw data by default
                ))
            
            # Add smoothed data for the first window size
            for label, rewards in rewards_data.items():
                # Calculate moving average with the first window size
            ...
            # Create buttons for different window sizes
            buttons = []
            ...
            buttons.append(dict(
                method="update",
                label="Raw Data",
                args=[{"visible": [True if i < len(rewards_data) else False 
                                  for i in range(len(rewards_data) * (len(window_sizes) + 1))]}]
            ))
            
            # Buttons for each window size
            ...
            # Update layout
            fig.update_layout(
                title="Interactive Reward Plot",
                xaxis_title="Episode",
                yaxis_title="Reward",
                updatemenus=[dict(
                    active=1,  # Default to first smoothing option
                    buttons=buttons
                )],
                height=600,
            ...
            RewardVisualization.plot_episode_rewards(
                next(iter(rewards_data.values())), 
                window_size=window_sizes[0],
                title="Reward Plot (Fallback Static Version)"
            )
...
class PolicyVisualization:
    ...
    def visualize_action_distribution(
        ...
        plt.figure(figsize=figsize)
        bars = plt.bar(action_labels, action_percentages, color=[COLOR_SCHEME['primary'], 
                                                              COLOR_SCHEME['secondary'],
                                                              COLOR_SCHEME['tertiary'],
                                                              COLOR_SCHEME['quaternary']][:len(action_labels)])
    ...
    @staticmethod
    def visualize_action_over_time(
        actions: List[int],
        action_labels: List[str],
        window_size: int = 100,
        ...
        Args:
            ...
            save_path: Optional path to save the figure
        """
        # Calculate action distribution over time
        ...
        for i in range(time_windows):
            ...
            percentages = [count / len(window_actions) * 100 for count in counts]
            action_distributions.append(percentages)
        
        # Create plot
        plt.figure(figsize=figsize)
        x = np.arange(time_windows) + 1  # Window numbers starting from 1
    ...
    def plot_policy_heatmap(
        ...
        if len(state_features) > max_states:
            # Subsample data
            indices = np.random.choice(len(state_features), max_states, replace=False)
        ...
        try:
            ...
            if len(state_features[0]) > 2:
                ...
                # If PCA doesn't capture enough variance, use t-SNE
            ...
            PolicyVisualization.visualize_action_distribution(actions, action_labels)
...
class MarketConditionVisualization:
    ...
    def plot_performance_by_regime(
        ...
        # Determine bar colors based on positive/negative returns
        for i, bar in enumerate(bars):
            if mean_returns[i] >= 0:
                bar.set_color(COLOR_SCHEME['tertiary'])  # Green for positive
            else:
                bar.set_color(COLOR_SCHEME['quaternary'])  # Red for negative
    ...
    @staticmethod
    def create_interactive_regime_performance(
        market_regimes: List[str],
        returns: List[float],
        ...
        try:
            ...
            # Calculate Sharpe ratios (assuming returns are already excess returns)
            sharpe_ratios = []
            for r in regimes:
                ret = regime_data[r]
            ...
            fig = make_subplots(
                rows=2, cols=2,
                subplot_titles=("Mean Return by Regime", "Return Distribution by Regime",
                              "Sharpe Ratio by Regime", "Sample Count by Regime"),
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    name="Sharpe Ratio",
                    marker_color=[
                        'green' if sr >= 0 else 'red' for sr in sharpe_ratios
                    ]
                ),
                row=2, col=1
            )
            ...
            fig.update_layout(
                height=800,
                showlegend=False,
                title_text="Agent Performance Across Market Regimes",
                template="plotly_white"
            )
            ...
            logger.error(f"Error creating interactive regime performance: {str(e)}")
            # Fall back to static plot
            MarketConditionVisualization.plot_performance_by_regime(market_regimes, returns)
...
class TrainingProgressDashboard:
    ...
    def __init__(
        ...
        compare_agents: Optional[List[Dict[str, Any]]] = None,
    ...
    def setup_widgets(self) -> None:
        ...
        try:
            ...
            # Set tab titles
            self.tabs.children = [reward_tab, performance_tab, policy_tab, comparison_tab]
            ...
            self.plot_dropdown = widgets.Dropdown(
                options=['Episode Rewards', 'Reward Distribution', 'Action Distribution',
                      'Performance by Regime', 'Interactive Rewards', 'Policy Heatmap'],
                value='Episode Rewards',
                description='Plot:',
                style={'description_width': 'initial'}
            )
            
            # Smoothing slider for rewards
            self.smoothing_slider = widgets.IntSlider(
                value=10,
                min=1,
                max=100,
                step=1,
            ...
            controls = widgets.HBox([self.plot_dropdown, self.smoothing_slider, self.update_button])
            ...
            # Store output widgets for later use
            self.outputs = {
                'rewards': reward_tab.children[1],
                'performance': performance_tab.children[0],
                'policy': policy_tab.children[0],
                'comparison': comparison_tab.children[0]
            }
            
        except Exception as e:
            logger.error(f"Error setting up dashboard widgets: {str(e)}")
            print(f"Error setting up interactive dashboard. Will fall back to static plots. Error: {str(e)}")
    ...
    def update_plots(self, _=None) -> None:
        ...
        try:
            ...
            if plot_type in ['Episode Rewards', 'Reward Distribution', 'Interactive Rewards']:
    ...
    def _plot_rewards(self, plot_type: str, smoothing: int) -> None:
        ...
        if plot_type == 'Episode Rewards':
            RewardVisualization.plot_episode_rewards(
                ...
                save_path=self.save_dir + "/rewards.png" if self.save_dir else None
            )
        elif plot_type == 'Reward Distribution':
            RewardVisualization.plot_reward_distribution(
                rewards,
                title="Reward Distribution",
                save_path=self.save_dir + "/reward_dist.png" if self.save_dir else None
            )
        elif plot_type == 'Interactive Rewards':
            # Prepare data for comparison
            rewards_data = {'Current Agent': rewards}
            
            # Add comparison agents if available
            for i, agent_data in enumerate(self.compare_agents):
                if 'rewards' in agent_data:
                    rewards_data[f"Agent {i+1}"] = agent_data['rewards']
                    
            # Create interactive plot
            RewardVisualization.create_interactive_rewards_plot(
                rewards_data,
    ...
    def _plot_performance(self) -> None:
        ...
        if 'market_regimes' not in self.training_data or 'returns' not in self.training_data:
        ...
        # Plot interactive performance visualization
    ...
    def _plot_policy(self, plot_type: str) -> None:
        ...
        """
        if 'actions' not in self.training_data or 'action_labels' not in self.training_data:
        ...
        if plot_type == 'Action Distribution':
            PolicyVisualization.visualize_action_distribution(
                actions,
                action_labels,
                title="Agent Action Distribution",
                save_path=self.save_dir + "/action_dist.png" if self.save_dir else None
            )
            
            # Also show action distribution over time
            PolicyVisualization.visualize_action_over_time(
                actions,
                action_labels,
                title="Action Distribution Over Time",
                save_path=self.save_dir + "/action_time.png" if self.save_dir else None
            )
            
        elif plot_type == 'Policy Heatmap':
            if 'state_features' not in self.training_data or 'feature_names' not in self.training_data:
            ...
            PolicyVisualization.plot_policy_heatmap(
                state_features,
                actions,
                feature_names,
                action_labels,
                title="Policy Heatmap",
                save_path=self.save_dir + "/policy_heatmap.png" if self.save_dir else None
            )
    ...
    def _plot_comparisons(self) -> None:
        ...
        if rewards_data:
            ...
            if self.save_dir:
                plt.savefig(f"{self.save_dir}/agent_comparison.png", dpi=300, bbox_inches='tight')
    ...
    def add_data(self, new_data: Dict[str, Any]) -> None:
        ...
        for key, value in new_data.items():
            if key in self.training_data:
                ...
                if isinstance(self.training_data[key], list) and isinstance(value, list):
        ...
        # Auto-update if enough new data
        if 'episode' in new_data and new_data['episode'] % self.update_interval == 0:
            self.update_plots()
...
class ModelExplainabilityTool:
    ...
    def visualize_feature_importance(
        ...
        try:
            if hasattr(model, 'predict'):
                ...
                baseline_actions = model.forward(torch.FloatTensor(samples)).argmax(dim=1).numpy()
            else:
        ...
        if method == 'perturbation':
            # Perturb each feature and measure the change in output
            for i, feature in enumerate(feature_names):
                perturbed = samples.copy()
                perturbed[:, i] = np.random.randn(n_samples)  # Randomize this feature
                
                try:
                    if hasattr(model, 'predict'):
                        perturbed_actions, _ = model.predict(perturbed, deterministic=True)
                    else:
                        perturbed_actions = model.forward(torch.FloatTensor(perturbed)).argmax(dim=1).numpy()
                        
                    # Calculate percentage of actions that changed
                    pct_changed = np.mean(perturbed_actions != baseline_actions) * 100
                    importance[feature] = pct_changed
                except Exception as e:
                    logger.error(f"Error computing importance for feature {feature}: {str(e)}")
        ...
        plt.figure(figsize=figsize)
        bars = plt.bar(importance.keys(), importance.values(), color=COLOR_SCHEME['primary'], alpha=0.7)
        ...
        plt.grid(True, linestyle='--', alpha=0.7, color=COLOR_SCHEME['grid'])
        plt.title("Feature Importance (% Actions Changed When Perturbed)", fontsize=14)
    ...
    @staticmethod
    def create_action_explanation_tool(
        model,
        feature_names: List[str],
        action_labels: List[str]
    ) -> None:
        ...
        try:
                ...
                with output:
                    try:
                        if hasattr(model, 'predict'):
                            ...
                            bars = plt.bar(action_labels, probs, color=COLOR_SCHEME['primary'])
                        ...
                        # Explain which features contributed most to this decision
                        print("\nFeature contributions:")
                        
                        # Simple contribution analysis: larger magnitude = more contribution
                        contributions = [(name, abs(value)) for name, value in zip(feature_names, features[0])]
                        contributions.sort(key=lambda x: x[1], reverse=True)
                        
                        for name, value in contributions[:5]:  # Show top 5 contributors
                            direction = "high" if value > 0 else "low"
                            print(f"- {name}: {value:.2f} ({'high' if value > 0 else 'low'})")
            ...
            # Create random button to set random values
            random_button = widgets.Button(
                description='Random Features',
                button_style='warning',
                tooltip='Set random feature values'
            )
            ...
            container = widgets.VBox([
                widgets.HTML("<h3>Explore Agent Behavior</h3>"),
                widgets.HTML("<p>Adjust the feature values and click 'Predict Action' to see what the agent would do.</p>"),
                sliders_box,
                buttons_box,
                output
            ])
...
class LearningVisualizationUI:
    ...
    def visualize_training_progress(
        ...
        episodes = [log.get('episode', i) for i, log in enumerate(training_logs)]
        rewards = [log.get('reward', 0.0) for log in training_logs]
        
        # Extract additional data if available
        returns = [log.get('return', 0.0) for log in training_logs if 'return' in log]
        market_regimes = [log.get('market_regime', 'unknown') for log in training_logs if 'market_regime' in log]
        actions = [log.get('action', 0) for log in training_logs if 'action' in log]
        ...
        if actions:
            ...
            self._plot_action_distribution(actions, [f"Action {a}" for a in action_labels], agent_name)
        ...
        if include_dashboard:
            try:
                ...
                dashboard_data = {
                    'rewards': rewards,
                    'returns': returns if returns else rewards,
                    'episodes': episodes,
                    'market_regimes': market_regimes if market_regimes else ['unknown'] * len(rewards),
                    'actions': actions if actions else [0] * len(rewards),
                    'action_labels': [f"Action {a}" for a in set(actions)] if actions else ["N/A"]
                }
                
                # Add state features if available
                if any('state_features' in log for log in training_logs):
                    state_features = [log.get('state_features', []) for log in training_logs if 'state_features' in log]
                    feature_names = [f"Feature {i}" for i in range(len(state_features[0]))] if state_features else []
                    dashboard_data['state_features'] = state_features
                    dashboard_data['feature_names'] = feature_names
                
                # Create and display dashboard
                dashboard = TrainingProgressDashboard(
    ...
    def _plot_learning_curve(self, episodes: List[int], rewards: List[float], agent_name: str) -> None:
        """Plot the learning curve."""
        RewardVisualization.plot_episode_rewards(
            rewards,
            window_size=min(20, max(1, len(rewards) // 20)),
            title=f"{agent_name} Learning Curve",
            save_path=f"{self.save_dir}/{agent_name.lower().replace(' ', '_')}_learning_curve.png" if self.save_dir else None
        )
    
    def _plot_regime_performance(self, regimes: List[str], returns: List[float], agent_name: str) -> None:
        """Plot performance by market regime."""
        MarketConditionVisualization.plot_performance_by_regime(
            regimes,
            returns,
            title=f"{agent_name} Performance by Market Regime",
            save_path=f"{self.save_dir}/{agent_name.lower().replace(' ', '_')}_regime_perf.png" if self.save_dir else None
        )
    
    def _plot_action_distribution(self, actions: List[int], action_labels: List[str], agent_name: str) -> None:
        """Plot action distribution."""
        PolicyVisualization.visualize_action_distribution(
            actions,
            action_labels,
            title=f"{agent_name} Action Distribution",
            save_path=f"{self.save_dir}/{agent_name.lower().replace(' ', '_')}_action_dist.png" if self.save_dir else None
        )
    ...
    def compare_agents(
        ...
        if include_interactive:
            try:
                ...
                for agent in agents_data:
                    ...
                    # Calculate moving average if data is long enough
                    if len(data) >= window_size:
                        smoothed = [np.mean(data[max(0, i-window_size):i+1]) for i in range(len(data))]
                    else:
                        smoothed = data
        ...
        if not include_interactive:
            ...
            if self.save_dir:
                plt.savefig(f"{self.save_dir}/agent_comparison_{metric}.png", dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    n_episodes = 1000
    rewards = np.random.normal(0, 1, n_episodes).cumsum() / 100  # Random walk
    ...
    regimes = np.random.choice(['Trending Bullish', 'Trending Bearish', 'Choppy', 'Ranging', 'Breakout'], n_episodes)
