...
class MLProfilingMonitor:
    ...
    def profile_model(
        ...
        for batch_size in batch_sizes:
            ...
            # Prepare input data for batch size
            ...
            # Start memory and CPU monitoring if requested
            ...
            for i in range(profile_runs):
                ...
                if profile_cpu:
                    ...
                    cpu_usage.append(cpu_end - cpu_start if cpu_end > cpu_start else cpu_end)

                if profile_memory:
                    mem_end = process.memory_info().rss / (1024 * 1024)  # MB
                    memory_usage.append(mem_end - mem_start if mem_end > mem_start else 0)

                # Log progress
                if (i + 1) % 10 == 0:
                    logger.info(f"Completed {i + 1}/{profile_runs} profile runs")
            ...
            batch_results = {
                ...
                "throughput": {
                    "samples_per_second": (batch_size * profile_runs) / (sum(latencies) / 1000),
                    "batches_per_second": profile_runs / (sum(latencies) / 1000)
                }
            }

            # Add CPU and memory statistics if profiled
            if profile_cpu:
                batch_results["cpu_usage_percent"] = {
                    "mean": np.mean(cpu_usage),
                    "max": np.max(cpu_usage)
                }

            if profile_memory:
                batch_results["memory_usage_mb"] = {
        ...
        summary = {
            ...
            "max_throughput": max(
                results["batch_sizes"][str(bs)]["throughput"]["samples_per_second"]
                for bs in batch_sizes
            ),
            "min_latency": min(
                results["batch_sizes"][str(bs)]["latency_ms"]["mean"]
                for bs in batch_sizes
            )
        }
        ...
        # Save results
        results_path = self.output_dir / f"{self.model_name}_profiling_results.json"
        ...
        logger.info(f"Profiling completed. Optimal batch size: {summary['optimal_batch_size']}")
        return results
        ...
        try:
            if self.framework == "tensorflow":
                if isinstance(input_data, np.ndarray):
                    # If input is a single sample, repeat it
                    if len(input_data.shape) == 1 or input_data.shape[0] == 1:
                        return np.repeat(input_data.reshape(1, -1), batch_size, axis=0)
                    # If input has multiple samples, take the first batch_size samples
                    elif input_data.shape[0] >= batch_size:
                        return input_data[:batch_size]
                    # If input has fewer samples than batch_size, repeat the samples
                    else:
                    ...
                    # Similar logic for TensorFlow tensors
                    ...
                    raise ValueError(f"Unsupported input type for TensorFlow: {type(input_data)}")
                ...
                if isinstance(input_data, np.ndarray):
                    ...
                    # Apply same logic as TensorFlow
                    if input_tensor.shape[0] == 1:
                        return input_tensor.repeat(batch_size, *[1 for _ in range(len(input_tensor.shape) - 1)])
                    elif input_tensor.shape[0] >= batch_size:
                        return input_tensor[:batch_size]
                    else:
                        repeats = batch_size // input_tensor.shape[0] + 1
                        repeated = input_tensor.repeat(repeats, *[1 for _ in range(len(input_tensor.shape) - 1)])
                        return repeated[:batch_size]
                elif isinstance(input_data, torch.Tensor):
                    # Apply same logic directly to PyTorch tensor
                    if input_data.shape[0] == 1:
                        return input_data.repeat(batch_size, *[1 for _ in range(len(input_data.shape) - 1)])
                    elif input_data.shape[0] >= batch_size:
                        return input_data[:batch_size]
                    else:
                        repeats = batch_size // input_data.shape[0] + 1
                        repeated = input_data.repeat(repeats, *[1 for _ in range(len(input_data.shape) - 1)])
                        return repeated[:batch_size]
                else:
                    raise ValueError(f"Unsupported input type for PyTorch: {type(input_data)}")
                ...
                if isinstance(input_data, np.ndarray):
                    ...
                    raise ValueError(f"Unsupported input type for ONNX: {type(input_data)}")
    ...
    def _export_trace(self, batch_size: int) -> Optional[Path]:
        ...
        try:
            if self.framework == "tensorflow":
                ...
                trace_path = self.output_dir / f"{self.model_name}_batch{batch_size}_trace"
                trace_path.mkdir(exist_ok=True, parents=True)

                # Create a simple input for tracing
                input_shape = [batch_size, 10]  # Assuming 10 features
                ...
                trace_path = self.output_dir / f"{self.model_name}_batch{batch_size}_trace.json"
                ...
                # ONNX Runtime doesn't have built-in profiling, so we'll skip
                return None
    ...
    def _find_optimal_batch_size(self, results: Dict[str, Any]) -> int:
        ...
        # Get batch sizes and their throughput
        batch_sizes = [int(bs) for bs in results["batch_sizes"].keys()]
        throughputs = [results["batch_sizes"][str(bs)]["throughput"]["samples_per_second"] for bs in batch_sizes]
        latencies = [results["batch_sizes"][str(bs)]["latency_ms"]["mean"] for bs in batch_sizes]
        ...
        # Find batch size with acceptable latency (below 95th percentile)
        latency_threshold = np.percentile(latencies, 95)
        acceptable_batch_sizes = [bs for bs, lat in zip(batch_sizes, latencies) if lat <= latency_threshold]

        if not acceptable_batch_sizes:
            # If no batch size has acceptable latency, return the one with lowest latency
            min_latency_idx = np.argmin(latencies)
            return batch_sizes[min_latency_idx]

        # Among acceptable batch sizes, find the one with highest throughput
        acceptable_throughputs = [throughputs[batch_sizes.index(bs)] for bs in acceptable_batch_sizes]
    ...
    def start_prometheus_exporter(self) -> None:
        ...
        logger.info(f"Starting Prometheus exporter on port {self.prometheus_port}")

        # Check if prometheus_client is available
        try:
            import prometheus_client
        except ImportError:
            logger.error("prometheus_client not available. Please install it to use this feature.")
            return
        ...
        self.prom_memory_gauge = prometheus_client.Gauge(
            'ml_memory_usage_megabytes',
            'Memory usage in megabytes',
            ['model_name', 'framework']
        )

        self.prom_cpu_gauge = prometheus_client.Gauge(
        ...
        logger.info(f"Prometheus exporter started on http://localhost:{self.prometheus_port}{self.prometheus_endpoint}")
        ...
        if record_metrics:
            ...
            try:
                ...
                if hasattr(self, "prom_inference_counter"):
                    ...
                    if success:
                        self.prom_inference_latency.labels(
                            model_name=self.model_name,
                            framework=self.framework,
                ...
                pass  # Prometheus client not available

        return result, latency_ms

    def generate_grafana_dashboard(
        self,
        dashboard_title: str = "ML Model Performance",
        ...
        dashboard = {
            ...
            "uid": f"ml-{self.model_name.lower().replace(' ', '-')}",
            "version": 1
        }
        ...
        panels.append({
            ...
            "targets": [
                {
                    "expr": f'rate(ml_inference_count_total{{model_name="{self.model_name}"}}[1m])',
                    "interval": "",
                    "legendFormat": "Inferences per second",
                    "refId": "A"
                }
            ],
            "thresholds": [],
        ...
        panels.append({
            ...
            "targets": [
                {
                    "expr": f'histogram_quantile(0.95, sum(rate(ml_inference_latency_milliseconds_bucket{{model_name="{self.model_name}"}}[1m])) by (le))',
                    "interval": "",
                    "legendFormat": "p95 Latency",
                    "refId": "A"
                },
                {
                    "expr": f'histogram_quantile(0.50, sum(rate(ml_inference_latency_milliseconds_bucket{{model_name="{self.model_name}"}}[1m])) by (le))',
                    "interval": "",
        ...
        panels.append({
            ...
            "targets": [
                {
                    "expr": f'rate(ml_inference_errors_total{{model_name="{self.model_name}"}}[1m])',
                    "interval": "",
        ...
        panels.append({
            ...
            "targets": [
                {
                    "expr": f'ml_memory_usage_megabytes{{model_name="{self.model_name}"}}',
                    "interval": "",
                    "legendFormat": "Memory (MB)",
                    "refId": "A"
                },
                {
                    "expr": f'ml_cpu_usage_percent{{model_name="{self.model_name}"}}',
                    "interval": "",
                    "legendFormat": "CPU (%)",
                    "refId": "B"
                }
    ...
    def configure_alerts(
        ...
        alerts = {
            "groups": [
                {
                    "name": f"ml_model_{self.model_name}_alerts",
                    "rules": [
                        {
                            "alert": "HighLatency",
                            "expr": f'histogram_quantile(0.95, sum(rate(ml_inference_latency_milliseconds_bucket{{model_name="{self.model_name}"}}[5m])) by (le)) > {latency_threshold_ms}',
                            "for": "1m",
                            "labels": {
                                "severity": "warning"
                            },
                            "annotations": {
                                ...
                                "description": f"95th percentile latency is above {latency_threshold_ms}ms for model {self.model_name}"
                            }
                        },
                        {
                            "alert": "HighErrorRate",
                            "expr": f'rate(ml_inference_errors_total{{model_name="{self.model_name}"}}[5m]) / rate(ml_inference_count_total{{model_name="{self.model_name}"}}[5m]) > {error_rate_threshold}',
                            "for": "1m",
                            "labels": {
                                "severity": "warning"
                            },
                            "annotations": {
                                ...
                                "description": f"Error rate is above {error_rate_threshold*100}% for model {self.model_name}"
                            }
                        },
                        {
                            "alert": "HighMemoryUsage",
                            "expr": f'ml_memory_usage_megabytes{{model_name="{self.model_name}"}} > {memory_threshold_mb}',
                            "for": "5m",
                            "labels": {
                                "severity": "warning"
                            },
                            "annotations": {
                                ...
                                "description": f"Memory usage is above {memory_threshold_mb}MB for model {self.model_name}"
                            }
                        },
                        {
                            "alert": "HighCPUUsage",
                            "expr": f'ml_cpu_usage_percent{{model_name="{self.model_name}"}} > {cpu_threshold_percent}',
                            "for": "5m",
                            "labels": {
                                "severity": "warning"
                            },
                            "annotations": {
                                ...
                                "description": f"CPU usage is above {cpu_threshold_percent}% for model {self.model_name}"
                            }
                        }
                    ]
                }
        ...
        # Save alert configuration if path provided
