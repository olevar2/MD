...
class CurriculumProgressionCallback(BaseCallback):
    ...
    def __init__(
        ...
        performance_metrics: List[str] = ["mean_reward", "success_rate"],
        progression_metrics: Dict[str, float] = None,
        verbose: int = 1
    ):
        """
        ...
        self.progression_metrics = progression_metrics or {
            "mean_reward": reward_threshold,
            "success_rate": 0.7
        }
        self.best_mean_reward = -np.inf
        self.current_curriculum_level = 0
        self.max_curriculum_level = 5  # Can be configured
    ...
    def _on_step(self) -> bool:
        ...
        # It's time to evaluate the agent
        ...
        # Compute success rate (optional, depends on your env)
        ...
        if "mean_reward" in self.progression_metrics and mean_reward < self.progression_metrics["mean_reward"]:
            ready_to_progress = False
            
        if "success_rate" in self.progression_metrics and success_rate < self.progression_metrics["success_rate"]:
            ready_to_progress = False
            
        # Update curriculum level if needed and if we haven't reached the max level
        if ready_to_progress and self.current_curriculum_level < self.max_curriculum_level:
            self.current_curriculum_level += 1
            
            # Update all environments with new curriculum level
            self._update_curriculum_level()
            
            # Log the progression
            self.progression_history.append({
                "timestep": self.num_timesteps,
                "old_level": self.current_curriculum_level - 1,
                "new_level": self.current_curriculum_level,
            ...
            if self.verbose > 0:
                logger.info(f"Curriculum level increased to {self.current_curriculum_level}")
    ...
    def _calculate_success_rate(self):
        ...
        # 1. Get this from the environment if it provides a success metric
        # 2. Define success criteria based on rewards or other metrics
    ...
    def _update_curriculum_level(self):
        ...
        # For VecEnv, we need to update all environments
        # This implementation depends on your environment's API
        
        # Example implementation
        try:
            # Try to set the curriculum level directly if supported
            ...
            # Alternative approach: reload environments with new settings
            # This is highly environment-specific and should be adapted
            pass
...
class TrainingMetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if len(self.model.ep_info_buffer) > 0 and len(self.model.ep_info_buffer[0]) > 0:
            ...
            # Try to get loss values if available (depends on algorithm)
            if hasattr(self.model, "logger") and hasattr(self.model.logger, "name_to_value"):
                loss_key = None
                explained_var_key = None
                
                # Find the right keys based on algorithm
                ...
                if loss_key:
                    self.metrics["losses"].append(self.model.logger.name_to_value[loss_key])
                else:
                    self.metrics["losses"].append(None)
                    
                if explained_var_key:
                    self.metrics["explained_variance"].append(self.model.logger.name_to_value[explained_var_key])
                else:
    ...
    def _save_metrics(self):
        ...
        metrics_df = pd.DataFrame({
            ...
            "reward": self.metrics["rewards"],
            "episode_length": self.metrics["episode_lengths"],
...
class DistributedCurriculumTrainer:
    ...
    def __init__(
        ...
        curriculum_levels: List[Dict[str, Any]] = None,
        ...
        self.curriculum_levels = curriculum_levels or self._default_curriculum_levels()
        self.log_dir = log_dir
        self.seed = seed
        ...
        return [
            {
                "level": 0,
                "name": "basic",
                "description": "Basic trading with minimal volatility",
            ...
            },
            {
                "level": 1,
                "name": "intermediate",
                "description": "Moderately challenging environment with some regime changes",
                "market_volatility": 0.2,
                "regime_transition_prob": 0.03,
                "news_impact_factor": 0.5,
            ...
            },
            {
                "level": 2,
                "name": "advanced",
                "description": "Challenging environment with frequent regime changes",
                "market_volatility": 0.3,
                "regime_transition_prob": 0.05,
                "news_impact_factor": 0.7,
            ...
            },
            {
                "level": 3,
                "name": "expert",
                "description": "Highly challenging environment with high volatility",
                "market_volatility": 0.4,
                "regime_transition_prob": 0.07,
                "news_impact_factor": 0.8,
            ...
            },
            {
                "level": 4,
                "name": "master",
                "description": "Extreme volatility with rapid regime changes and strong news impact",
                "market_volatility": 0.5,
                "regime_transition_prob": 0.1,
                "news_impact_factor": 1.0,
    ...
    def _make_env(self, level_config, rank, seed=0):
        ...
        """
        def _init():
            # Create broker simulator with the appropriate configuration
            from trading_gateway_service.simulation.forex_broker_simulator import ForexBrokerSimulator
            
            # Configure broker simulator according to curriculum level
            broker_config = self.base_env_config.get("broker_config", {})
            broker_config.update({
                "volatility_factor": level_config["market_volatility"],
                "news_impact_factor": level_config["news_impact_factor"],
            })
            
            broker_simulator = ForexBrokerSimulator(**broker_config)
            
            # Create simulation adapter
            adapter_config = self.base_env_config.get("adapter_config", {})
            adapter_config.update({
                "regime_transition_probability": level_config["regime_transition_prob"]
            })
            ...
            env_config = self.base_env_config.get("env_config", {})
            ...
            # Set curriculum level as an environment property for tracking
            env.curriculum_level = level_config["level"]
            env.curriculum_name = level_config["name"]
            
            # Add a method to update curriculum level
            def set_curriculum_level(new_level):
                if 0 <= new_level < len(self.curriculum_levels):
                    ...
                    broker_simulator.set_volatility_factor(level_config["market_volatility"])
                    broker_simulator.set_news_impact_factor(level_config["news_impact_factor"])
                    
                    # Update adapter parameters
                    simulation_adapter.regime_transition_probability = level_config["regime_transition_prob"]
    ...
    def _create_envs(self, level_idx=0):
        ...
        env_fns = [self._make_env(level_config, i, self.seed + i) for i in range(self.num_envs)]
        training_envs = SubprocVecEnv(env_fns)
        
        # Create a separate evaluation environment
        eval_env_fn = self._make_env(level_config, self.num_envs, self.seed + self.num_envs)
    ...
    def train(self, model_name, total_timesteps=1000000, eval_freq=10000):
        ...
        self.training_envs, self.eval_env = self._create_envs(self.current_curriculum_level)
        
        # Create the model
        self.current_model = self.model_factory.create_model(
            model_name,
            self.training_envs,
            verbose=1
        )
        ...
        progression_callback = CurriculumProgressionCallback(
            eval_env=self.eval_env,
            eval_freq=eval_freq,
            reward_threshold=self.curriculum_levels[self.current_curriculum_level]["reward_threshold"],
            progression_metrics={
                "mean_reward": self.curriculum_levels[self.current_curriculum_level]["reward_threshold"],
                "success_rate": self.curriculum_levels[self.current_curriculum_level]["success_threshold"]
            }
        )
        
        metrics_callback = TrainingMetricsCallback(
            log_dir=os.path.join(self.log_dir, "metrics"),
            save_freq=10000
        )
        
        # Combine callbacks
        callbacks = CallbackList([eval_callback, progression_callback, metrics_callback])
        
        # Start training
        start_time = time.time()
        logger.info(f"Starting distributed training with {self.num_envs} environments")
        logger.info(f"Initial curriculum level: {self.curriculum_levels[self.current_curriculum_level]['name']}")
        ...
        self._generate_training_report(
            ...
            progression_history=progression_callback.progression_history
        )
        
        return self.current_model
    
    def _generate_training_report(self, model_name, training_duration, progression_history):
