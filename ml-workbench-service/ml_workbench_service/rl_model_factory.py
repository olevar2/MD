...
class RLModelFactory:
    ...
    def __init__(self, default_tensorboard_log_dir: str='./logs/rl_models/'):
        ...
        self.algorithm_map = {RLAlgorithm.PPO: PPO, RLAlgorithm.A2C: A2C,
            RLAlgorithm.SAC: SAC, RLAlgorithm.TD3: TD3, RLAlgorithm.DQN: DQN}
        os.makedirs(self.tensorboard_log_dir, exist_ok=True)

    @with_exception_handling
    def create_model(self, algorithm: Union[str, RLAlgorithm], environment:
        EnhancedForexTradingEnv, params: Optional[Dict[str, Any]]=None,
        policy: str='MlpPolicy', tensorboard_log: Optional[str]=None) ->Any:
        """
        ...
        if isinstance(algorithm, str):
            try:
                ...
                raise ValueError(
                    f'Unsupported algorithm: {algorithm}. Supported algorithms: {[a.value for a in RLAlgorithm]}'
                    )
        ...
        if tensorboard_log is None and self.tensorboard_log_dir:
            tensorboard_log = os.path.join(self.tensorboard_log_dir,
                algorithm.value.lower())
        if algorithm in [RLAlgorithm.SAC, RLAlgorithm.TD3
            ] and 'action_noise' not in params:
            action_dim = environment.action_space.shape[0]
            params['action_noise'] = self._create_action_noise(algorithm,
                action_dim)
        logger.info(f'Creating {algorithm.value} model with {policy} policy')
        return algorithm_class(policy=policy, env=environment,
    ...
    def _get_default_params(self, algorithm: RLAlgorithm) ->Dict[str, Any]:
        ...
        if algorithm == RLAlgorithm.PPO:
            return {'learning_rate': 0.0003, 'n_steps': 2048, 'batch_size':
                64, 'n_epochs': 10, 'gamma': 0.99, 'gae_lambda': 0.95,
                'clip_range': 0.2, 'ent_coef': 0.01, 'verbose': 1}
        elif algorithm == RLAlgorithm.A2C:
            return {'learning_rate': 0.0007, 'n_steps': 5, 'gamma': 0.99,
                'ent_coef': 0.01, 'vf_coef': 0.5, 'verbose': 1}
        elif algorithm == RLAlgorithm.SAC:
            return {'learning_rate': 0.0003, 'buffer_size': 100000,
                'learning_starts': 1000, 'batch_size': 256, 'tau': 0.005,
                'gamma': 0.99, 'train_freq': 1, 'gradient_steps': 1,
                'verbose': 1}
        elif algorithm == RLAlgorithm.TD3:
            return {'learning_rate': 0.0003, 'buffer_size': 100000,
                'learning_starts': 1000, 'batch_size': 100, 'tau': 0.005,
                'gamma': 0.99, 'policy_delay': 2, 'verbose': 1}
        elif algorithm == RLAlgorithm.DQN:
            return {'learning_rate': 0.0001, 'buffer_size': 100000,
                'learning_starts': 1000, 'batch_size': 32, 'tau': 0.005,
                'gamma': 0.99, 'train_freq': 4, 'gradient_steps': 1,
                'target_update_interval': 1000, 'verbose': 1}
        else:
            return {}

    def _create_action_noise(self, algorithm: RLAlgorithm, action_dim: int
        ) ->Any:
        """
        ...
        if algorithm == RLAlgorithm.TD3:
            return NormalActionNoise(mean=np.zeros(action_dim), sigma=0.1 *
                np.ones(action_dim))
        elif algorithm == RLAlgorithm.SAC:
            return None
        else:
    ...
    @with_exception_handling
    def load_model(self, algorithm: Union[str, RLAlgorithm], model_path:
        str, environment: Optional[EnhancedForexTradingEnv]=None) ->Any:
        """
    ...
    def create_ensemble(self, models_config: List[Dict[str, Any]],
        environment: EnhancedForexTradingEnv) ->Dict[str, Any]:
        """
        ...
        for config in models_config:
            algorithm = config_manager.get('algorithm')
            params = config_manager.get('params', {})
            weight = config_manager.get('weight', 1.0)
            name = config_manager.get('name', f'{algorithm}_{len(ensemble)}')
            model = self.create_model(algorithm=algorithm, environment=
                environment, params=params)
            ensemble[name] = {'model': model, 'weight': weight}
