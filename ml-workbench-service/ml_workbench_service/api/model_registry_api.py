...
# Initialize services - these should be dependency-injected in a production app
# Using a default path for development - in production this would come from config
...
async def create_model(model_data: ModelRegistryCreateRequest):
    ...
    try:
        model_metadata = ModelMetadata(
            ...
            training_frequency=model_data.training_frequency,
...
async def list_models(
    ...
    try:
        ...
        return ModelListResponse(
            models=[ModelRegistryResponse(**m.to_dict()) for m in models],
            total=total,
            page=page,
            page_size=page_size
        )
        ...
        raise HTTPException(status_code=500, detail="An error occurred while listing models")


@router.get("/models/{model_id}", response_model=ModelRegistryResponse)
async def get_model(model_id: str = Path(..., description="ID of the model to retrieve")):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="An error occurred while retrieving the model")
...
async def update_model(
    ...
    model_data: ModelRegistryCreateRequest = Body(..., description="Updated model data")
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="An error occurred while updating the model")


@router.delete("/models/{model_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_model(model_id: str = Path(..., description="ID of the model to delete")):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="An error occurred while deleting the model")
...
async def create_model_version(
    ...
    version_data: ModelVersionCreateRequest = Body(..., description="Model version data"),
    model_file: Optional[UploadFile] = File(None, description="Model artifact file")
):
    """
    ...
    try:
        ...
        model_version = ModelVersion(
            ...
            framework_version=version_data.framework_version,
            hyperparameters=hyperparameters,
            metrics=metrics,
            training_dataset_id=version_data.training_dataset_id,
            validation_dataset_id=version_data.validation_dataset_id,
            test_dataset_id=version_data.test_dataset_id,
            feature_columns=version_data.feature_columns,
            target_column=version_data.target_column,
            preprocessing_steps=version_data.preprocessing_steps,
        ...
        # Register the version with the service
        ...
        raise HTTPException(status_code=500, detail=f"An error occurred while creating the model version: {str(e)}")
...
async def list_model_versions(
    ...
    page: int = Query(1, ge=1, description="Page number"),
    page_size: int = Query(10, ge=1, le=100, description="Page size"),
    status: Optional[ModelStatus] = Query(None, description="Filter by version status")
):
    """
    ...
    try:
        ...
        versions, total = model_registry_service.list_model_versions(
            model_id=model_id,
            page=page,
            page_size=page_size,
            filters=filters
        ...
        return ModelVersionListResponse(
            versions=[ModelVersionResponse(**v.to_dict()) for v in versions],
            total=total,
            page=page,
            page_size=page_size
        )
        ...
        raise HTTPException(status_code=500, detail="An error occurred while listing model versions")


@router.get("/models/{model_id}/versions/{version_id}", response_model=ModelVersionResponse)
async def get_model_version(
    model_id: str = Path(..., description="ID of the model"),
    version_id: str = Path(..., description="ID of the version to retrieve")
):
    """
    ...
    try:
        ...
        logger.error(f"Error retrieving version {version_id} for model {model_id}: {str(e)}")
        raise HTTPException(status_code=500, detail="An error occurred while retrieving the model version")


@router.post("/models/{model_id}/versions/{version_id}/stage", response_model=ModelVersionResponse)
async def transition_model_version_stage(
    model_id: str = Path(..., description="ID of the model"),
    version_id: str = Path(..., description="ID of the version"),
    stage_data: StageTransitionRequest = Body(..., description="Stage transition data")
):
    """
    Transition a model version to a new stage (staging, production, etc.).
    """
    try:
        updated_version = model_registry_service.transition_model_version_stage(
            model_id=model_id,
        ...
        logger.error(f"Error transitioning version {version_id} for model {model_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"An error occurred during stage transition: {str(e)}")


@router.get("/models/{model_id}/versions/{version_id}/download")
async def download_model_artifact(
    model_id: str = Path(..., description="ID of the model"),
    version_id: str = Path(..., description="ID of the version to download")
):
    """
    Download a model artifact.
    """
    try:
        model_path = model_registry_service.get_model_artifact_path(model_id, version_id)
        ...
        return FileResponse(
            path=model_path,
            filename=f"model_{model_id}_v{version_id}.bin",
        ...
        logger.error(f"Error downloading version {version_id} for model {model_id}: {str(e)}")
        raise HTTPException(status_code=500, detail="An error occurred while downloading the model artifact")


@router.put("/models/{model_id}/versions/{version_id}/metrics", response_model=ModelVersionResponse)
async def update_model_version_metrics(
    model_id: str = Path(..., description="ID of the model"),
    version_id: str = Path(..., description="ID of the version"),
    metrics: Dict[str, float] = Body(..., description="Updated metrics")
):
    """
    ...
    try:
        ...
        # Create metrics object from the provided metrics
        metrics_obj = ModelMetrics(**metrics)
        
        # Update the version with new metrics
        ...
        logger.error(f"Error updating metrics for version {version_id}: {str(e)}")
        raise HTTPException(status_code=500, detail="An error occurred while updating model metrics")


@router.delete("/models/{model_id}/versions/{version_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_model_version(
    ...
    try:
        ...
        logger.error(f"Error deleting version {version_id} for model {model_id}: {str(e)}")
        raise HTTPException(status_code=500, detail="An error occurred while deleting the model version")
...
async def search_models(
    ...
    """
    try:
        models, total = model_registry_service.search_models(
            query=query,
            page=page,
            page_size=page_size
        )
        ...
        raise HTTPException(status_code=500, detail="An error occurred while searching for models")
