...
from ml_workbench_service.models.model_models import (
    Model,
    ModelVersion,
    ModelStage,
    ModelFramework,
...
class ModelCreate(BaseModel):
    ...
    framework: ModelFramework = Field(..., description="Framework used for this model")
    tags: Optional[Dict[str, str]] = Field(None, description="Tags for the model")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Additional metadata for the model")
...
class ModelVersionCreate(BaseModel):
    ...
    experiment_run_id: Optional[str] = Field(None, description="ID of the experiment run that created this version")
    description: Optional[str] = Field(None, description="Description of the model version")
    source_uri: str = Field(..., description="URI where the model artifacts are stored")
    tags: Optional[Dict[str, str]] = Field(None, description="Tags for the model version")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Additional metadata for the model version")
    parameters: Optional[Dict[str, Any]] = Field(None, description="Parameters used to train this model version")
...
class ModelEvaluationCreate(BaseModel):
    ...
    timestamp: Optional[datetime] = Field(None, description="Time of evaluation")
...
@router.post(
    ...
    description="Create a new version for an existing ML model."
)
async def create_model_version(version: ModelVersionCreate):
    ...
    """
    try:
        created_version = model_service.create_version(
...
@router.get(
    ...
    description="Get a specific model version by ID."
)
...
@router.get(
    ...
    description="List all versions of an ML model."
)
...
@router.patch(
    ...
    response_model=ModelVersion,
    summary="Update model version stage",
    description="Update the stage of a model version."
)
async def update_model_version_stage(
    ...
    """
    try:
        updated_version = model_service.update_version_stage(
        ...
        logger.error(f"Error updating stage for model version {version_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
@router.post(
    ...
    description="Create an evaluation for a model version."
)
async def create_model_evaluation(
    ...
    """
    try:
        created_evaluation = model_service.create_evaluation(
            version_id=version_id,
        ...
        logger.error(f"Error creating evaluation for model version {version_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
@router.get(
    ...
    description="List all evaluations of a model version."
)
async def list_model_evaluations(
    version_id: str,
    dataset_id: Optional[str] = Query(None, description="Filter by dataset ID"),
    limit: int = Query(100, description="Maximum number of evaluations to return"),
    ...
    """
    try:
        evaluations = model_service.list_evaluations(
            version_id=version_id,
        ...
        logger.error(f"Error listing evaluations for model version {version_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
@router.post(
    ...
    description="Deploy a model version to a specific environment."
)
async def deploy_model_version(
    ...
    """
    try:
        # Queue deployment as a background task
        background_tasks.add_task(
            model_service.deploy_version,
            version_id=version_id,
...
@router.get(
    "/serving/{model_name}/versions/{version}",
    response_model=Dict[str, Any],
    summary="Get serving info",
    description="Get serving information for a deployed model version."
)
async def get_serving_info(
    model_name: str,
    version: str,
    environment: Optional[str] = Query(None, description="Environment to get serving info for")
):
    """
    ...
    """
    try:
        info = model_service.get_serving_info(
        ...
        if info is None:
            raise HTTPException(status_code=404, detail=f"Serving info for model {model_name} version {version} not found")
        ...
        logger.error(f"Error getting serving info for model {model_name} version {version}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
@router.post(
    ...
    description="Get a prediction from a deployed model."
)
async def get_prediction(
    model_name: str,
    data: Dict[str, Any],
    version: Optional[str] = Query(None, description="Model version to use"),
    environment: Optional[str] = Query(None, description="Environment to get prediction from")
):
    """
    ...
    """
    try:
        prediction = model_service.get_prediction(
            model_name=model_name,
            data=data,
            version=version,
            environment=environment
