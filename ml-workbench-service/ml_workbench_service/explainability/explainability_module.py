...
class SHAPExplainer(ModelExplainer):
    ...
    def __init__(
        ...
        # Create a model wrapper for TensorFlow/Keras models
        if isinstance(model, tf.keras.Model):
            # For neural network models
            self.wrapped_model = self._create_keras_wrapper(model, model_output)
            if background_data is not None:
                self.explainer = shap.DeepExplainer(self.wrapped_model, background_data)
            else:
                self.explainer = None  # Will be created on first explanation
        else:
    ...
    def _create_keras_wrapper(
        self,
        model: tf.keras.Model,
        output_idx: Union[str, int]
    ) -> Callable:
        """
        ...
        if isinstance(model.output, list) or (hasattr(model.output, '_keras_shape') and len(model.output.shape) > 0):
            ...
            # Create a function that returns only the specified output
    ...
    def explain(
        ...
        if isinstance(shap_values, list) and not isinstance(self.model, tf.keras.Model):
        ...
        **kwargs
        ...
        if plot_type == 'summary':
            if isinstance(shap_values, list) and len(shap_values) > 1:
                # Multi-class: choose a class to visualize
            ...
            if isinstance(shap_values, list) and len(shap_values) > 1:
                ...
                shap.waterfall_plot(
                    shap.Explanation(
                        ...
                        base_values=explanation['expected_value'][class_idx],
    ...
    def save(self, path: str) -> None:
        ...
        # Save the explainer if possible
        try:
            with open(os.path.join(path, "explainer.pkl"), "wb") as f:
                pickle.dump(self.explainer, f)
        except:
            logger.warning("Could not pickle the SHAP explainer. Will need to reinitialize on load.")
    ...
    def load(
        ...
        # Try to load the pickled explainer
        try:
            with open(os.path.join(path, "explainer.pkl"), "rb") as f:
                instance.explainer = pickle.load(f)
        except:
            logger.warning("Could not load the pickled SHAP explainer. "
                          "Will initialize on the first call to explain().")
...
class LIMEExplainer(ModelExplainer):
    ...
    def __init__(
        ...
        # Create a prediction wrapper for the model
        if isinstance(model, tf.keras.Model):
            self.predict_fn = model.predict
        else:
            self.predict_fn = model.predict if hasattr(model, 'predict') else model
    ...
    def explain(
        ...
        if self.explainer is None:
            if training_data is None:
                raise ValueError("Training data must be provided if explainer is not initialized")
    ...
    def save(self, path: str) -> None:
        ...
        try:
            ...
            logger.warning("Could not pickle the LIME explainer. Will need to reinitialize on load.")
    ...
    def load(
        ...
        try:
            ...
            logger.warning("Could not load the pickled LIME explainer. "
                         "Will initialize on the first call to explain().")
            if training_data is None:
                logger.warning("Training data should be provided to initialize the explainer")
...
class PermutationImportanceExplainer(ModelExplainer):
    ...
    def explain(
        ...
        # For supervised models, we need the targets
        if y is not None:
            ...
            # For unsupervised models or when y is not provided,
            # we implement our own permutation importance using prediction variance
            ...
            for i in range(X_values.shape[1]):
                ...
                for _ in range(n_repeats):
                    ...
                    # Calculate importance as the difference in variance
        ...
        feature_importance = {}
        for i, feature_name in enumerate(self.feature_names or [f"feature_{i}" for i in range(len(importances))]):
    ...
    def plot(
        ...
        **kwargs
        ...
        feature_names = explanation.get('feature_names', [f"feature_{i}" for i in range(len(importances))])
        ...
        if importances_std is not None:
            ax.barh(y_pos, importances[indices], xerr=importances_std[indices], 
        ...
        ax.invert_yaxis()  # Show most important feature at the top
...
class ExplainabilityModule:
    ...
    def explain_model(
        ...
        # Create explainers that aren't already added
        explanations = {}
        ...
        if 'lime' in methods:
            if 'lime' not in self.explainers:
                lime_explainer = LIMEExplainer(
                    ...
                    feature_names=feature_names,
                    class_names=class_names,
                    training_data=background_data or X
                )
    ...
    def save_explainers(self, path: str) -> None:
        ...
        logger.info(f"Saved ExplainabilityModule with {len(self.explainers)} explainers to {path}")
    ...
    def load(
        ...
        for explainer_name in metadata.get("explainers", []):
            # Load explainer metadata to determine its type
            ...
            # Load the appropriate explainer type
            if explainer_type == "shap":
                ...
                explainer = PermutationImportanceExplainer.load(explainer_path, model)
                module.add_explainer(explainer_name, explainer)
                
        logger.info(f"Loaded ExplainabilityModule with {len(module.explainers)} explainers from {path}")
        return module


# Helper function for generating explanations for MultitaskModel
def explain_multitask_model(
    ...
    Returns:
        Dictionary with explanations from all methods
    """
    # Create a wrapper function that returns predictions for the specific task
    def predict_wrapper(x):
        predictions = model.predict_single_task(x, task)
        return predictions
    
    # Create an explainability module
    ...
    # For classification tasks, set class names
    ...
    if task_config["activation"] == "softmax":
        if task == TaskType.PRICE_DIRECTION:
            ...
            class_names = ["trending_up", "trending_down", "volatile", "range_bound"]
            
    return explainability.explain_model(
