...
class FeatureImportanceAnalyzer:
    ...
    def analyze_feature_importance(
        ...
        """
        try:
            # Get model and version from registry
            model_version = self.model_registry.get_model_version(model_id, version_id)
            
            # Load the model
            model_path = self.model_registry.get_model_artifact_path(model_id, version_id)
            model_framework = model_version.framework
            
            # Load the model based on its framework
            model = self._load_model(model_path, model_framework)
            if model is None:
                raise ValueError(f"Failed to load model with framework {model_framework}")
                
            # Choose analysis method if auto
            if method == "auto":
                method = self._auto_select_method(model_framework, model)
                
            # Perform feature importance analysis
            if method == "built_in":
                feature_importances = self._get_built_in_importance(model, X_sample)
                ...
                if y_sample is None:
                    raise ValueError("y_sample is required for permutation importance")
                feature_importances = self._get_permutation_importance(
                    model, X_sample, y_sample, n_samples, random_state
                )
            elif method == "eli5" and ELI5_AVAILABLE:
                if y_sample is None:
                    raise ValueError("y_sample is required for ELI5 importance")
                feature_importances = self._get_eli5_importance(
                    model, X_sample, y_sample, n_samples, random_state
                )
            else:
                raise ValueError(f"Method {method} not supported or required package not available")
                
            # Create visualization
            visualization_data = self._create_visualization(feature_importances, method)
            
            # Create the response object
            result = ModelFeatureImportance(
                model_id=model_id,
    ...
    def _auto_select_method(self, framework: ModelFramework, model) -> str:
        ...
        if framework in [ModelFramework.SKLEARN, ModelFramework.XGBOOST, ModelFramework.LIGHTGBM]:
            # For tree-based models, use built-in importance if available
            if hasattr(model, "feature_importances_"):
                return "built_in"
        
        # If SHAP is available, prefer it for most models
        if SHAP_AVAILABLE:
            return "shap"
        ...
        return "built_in"  # Default, may not work for all models
        
    def _get_built_in_importance(self, model, X_sample: pd.DataFrame) -> List[FeatureImportance]:
        ...
        try:
            if hasattr(model, "feature_importances_"):
                ...
                raise AttributeError("Model does not have built-in feature importance")
                
            feature_names = X_sample.columns
            
            # Create sorted importances
            features = []
    ...
    def _get_shap_importance(self, model, X_sample: pd.DataFrame) -> List[FeatureImportance]:
    ...
    def _get_permutation_importance(
        self, model, X_sample: pd.DataFrame, y_sample: pd.Series, 
    ...
    def _get_eli5_importance(
        self, model, X_sample: pd.DataFrame, y_sample: pd.Series, 
        n_samples: int, random_state: int
    ) -> List[FeatureImportance]:
        """Get ELI5-based feature importance."""
        try:
            # Use ELI5's PermutationImportance
    ...
    def _create_visualization(self, features: List[FeatureImportance], method: str) -> Dict[str, Any]:
        ...
        try:
            ...
            feature_names = [f.feature_name for f in sorted_features[:15]]  # Top 15 features
            importance_scores = [f.importance_score for f in sorted_features[:15]]
    ...
    def _get_method_explanation(self, method: str) -> str:
        ...
        explanations = {
            'built_in': (
                "This analysis uses the model's built-in feature importance, which is typically "
                "based on how much each feature contributes to decreasing the loss function during training. "
                "For tree-based models, this represents the total reduction of the criterion brought by each feature."
            ),
            'shap': (
                "SHAP (SHapley Additive exPlanations) values show the impact of each feature on individual predictions. "
                "These values represent the contribution of each feature to the difference between the actual prediction "
                "and the mean prediction. Higher absolute values indicate stronger impact."
            ),
            'permutation': (
                "Permutation importance measures the increase in prediction error when a feature's values are randomly shuffled. "
                "This breaks the relationship between the feature and the true outcome, indicating how much the model depends "
                "on that feature for accuracy. Higher values indicate more important features."
            ),
            'eli5': (
                "This analysis uses ELI5's permutation importance, which measures how model performance decreases "
                "when a single feature is randomly shuffled. Features that cause larger performance drops when shuffled "
                "are considered more important."
            )
        }
        
        return explanations.get(method, "Feature importance analysis showing the relative impact of each feature on model predictions.")
