...
from ml_workbench_service.models.experiment_models import Dataset, Experiment, TrainingRun, Model, Prediction, ExperimentStatus, ModelType, ModelFramework, TrainingAlgorithm, ExperimentFilter, MetricValue
...
class ExperimentService:
    ...
    def __init__(self, experiment_repository: ExperimentRepository,
        feature_store_client: FeatureStoreClient, model_storage_path: str):
        """
    ...
    @async_with_exception_handling
    async def create_dataset_from_feature_store(self, name: str,
        description: Optional[str], symbols: List[str], timeframes: List[
        str], feature_ids: List[str], target_feature: Optional[str],
        start_date: datetime, end_date: datetime, train_ratio: float=0.7,
        validation_ratio: float=0.15, test_ratio: float=0.15, metadata:
        Optional[Dict[str, Any]]=None) ->Dataset:
        """
        ...
        try:
            ...
            dataset_path = os.path.join(self.model_storage_path, 'datasets',
                dataset_id)
            ...
            for symbol in symbols:
                for timeframe in timeframes:
                    logger.info(
                        f'Fetching feature data for {symbol}/{timeframe}')
                    try:
                        feature_data = (await self.feature_store.
                            get_feature_vectors(feature_ids=feature_ids,
            ...
            metadata['data_paths'] = {'combined': combined_data_path,
                'train': train_path, 'validation': val_path, 'test': test_path}
            dataset = Dataset(id=dataset_id, name=name, description=
                description, features=feature_ids, target=target_feature,
                size=total_samples, start_date=start_date, end_date=
                end_date, symbols=symbols, timeframes=timeframes,
                train_ratio=train_ratio, validation_ratio=validation_ratio,
                test_ratio=test_ratio, created_at=datetime.utcnow(),
                metadata=metadata)
            await self.repository.create_dataset(dataset)
            logger.info(
                f'Created dataset {dataset_id} with {total_samples} samples across {len(symbols)} symbols and {len(timeframes)} timeframes'
                )
            return dataset
        except Exception as e:
    ...
    async def list_datasets(self, name_filter: Optional[str]=None,
        symbol_filter: Optional[str]=None, timeframe_filter: Optional[str]=
        None, limit: int=100, offset: int=0) ->List[Dataset]:
        """
        ...
        """
        return await self.repository.list_datasets(limit=limit, offset=
            offset, name_filter=name_filter, symbol_filter=symbol_filter,
            timeframe_filter=timeframe_filter)

    async def load_dataset_data(self, dataset_id: str, subset: str='combined'
        ) ->pd.DataFrame:
        """
        ...
        if not dataset.metadata or 'data_paths' not in dataset.metadata:
            raise ValueError(
                f'Dataset {dataset_id} does not have valid file paths')
        data_paths = dataset.metadata['data_paths']
        if subset not in data_paths:
            raise ValueError(
                f"Invalid subset '{subset}'. Must be one of: {list(data_paths.keys())}"
                )
    ...
    async def create_experiment(self, name: str, description: Optional[str],
        dataset_id: str, model_type: ModelType, model_framework:
        ModelFramework, algorithm: TrainingAlgorithm, parameters: Optional[
        Dict[str, Any]]=None, tags: Optional[List[str]]=None, created_by:
        Optional[str]=None, notes: Optional[str]=None, metadata: Optional[
        Dict[str, Any]]=None) ->Experiment:
        """
        ...
        experiment_path = os.path.join(self.model_storage_path,
        ...
        experiment = Experiment(id=experiment_id, name=name, description=
            description, status=ExperimentStatus.CREATED, model_type=
            model_type, model_framework=model_framework, algorithm=
            algorithm, dataset_id=dataset_id, parameters=parameters,
            metrics={}, artifacts={}, tags=tags, created_at=datetime.utcnow
            (), created_by=created_by, notes=notes, metadata=metadata)
    ...
    async def list_experiments(self, filter_criteria: Optional[
        ExperimentFilter]=None, limit: int=100, offset: int=0) ->List[
        Experiment]:
        """
        ...
        """
        return await self.repository.list_experiments(filter_criteria=
            filter_criteria, limit=limit, offset=offset)

    async def update_experiment(self, experiment_id: str, **updates
        ) ->Experiment:
        """
    ...
    async def create_training_run(self, experiment_id: str, parameters:
        Optional[Dict[str, Any]]=None, notes: Optional[str]=None, metadata:
        Optional[Dict[str, Any]]=None) ->TrainingRun:
        """
        ...
        run_path = os.path.join(self.model_storage_path, 'experiments',
            experiment_id, f'run_{run_number}')
        ...
        training_run = TrainingRun(id=run_id, experiment_id=experiment_id,
            run_number=run_number, status=ExperimentStatus.CREATED,
            parameters=combined_params, metrics={}, artifacts={}, notes=
            notes, metadata=metadata)
        await self.repository.create_training_run(training_run)
        logger.info(
            f'Created training run {run_id} for experiment {experiment_id}')
        return training_run
    ...
    async def list_training_runs(self, experiment_id: str, limit: int=100,
        offset: int=0) ->List[TrainingRun]:
        """
        ...
        """
        return await self.repository.list_training_runs(experiment_id=
            experiment_id, limit=limit, offset=offset)

    async def update_training_run(self, run_id: str, **updates) ->TrainingRun:
        """
    ...
    async def add_metric_to_run(self, run_id: str, metric_name: str,
        metric_value: Union[float, Dict[str, float]], higher_is_better:
        bool=True) ->MetricValue:
        """
        ...
        """
        training_run = await self.get_training_run(run_id)
        metric = MetricValue(name=metric_name, value=metric_value,
            higher_is_better=higher_is_better, timestamp=datetime.utcnow())
        if training_run.metrics is None:
            training_run.metrics = {}
        training_run.metrics[metric_name] = metric
        await self.repository.update_training_run(training_run)
        logger.info(
            f'Added metric {metric_name}={metric_value} to run {run_id}')
        return metric

    async def create_model(self, name: str, version: str, description:
        Optional[str], experiment_id: str, training_run_id: Optional[str],
        model_artifacts: Dict[str, str], features: List[str], target:
        Optional[str], tags: Optional[List[str]]=None, metadata: Optional[
        ...
        model = Model(id=model_id, name=name, version=version, description=
            description, experiment_id=experiment_id, training_run_id=
            training_run_id, model_type=experiment.model_type,
            model_framework=experiment.model_framework, algorithm=
            experiment.algorithm, parameters=parameters, metrics=metrics,
            artifacts=model_artifacts, features=features, target=target,
            created_at=datetime.utcnow(), is_published=False, tags=tags,
            metadata=metadata)
    ...
    async def list_models(self, experiment_id: Optional[str]=None,
        is_published: Optional[bool]=None, name_filter: Optional[str]=None,
        limit: int=100, offset: int=0) ->List[Model]:
        """
        ...
        """
        return await self.repository.list_models(experiment_id=
            experiment_id, is_published=is_published, name_filter=
            name_filter, limit=limit, offset=offset)

    async def update_model(self, model_id: str, **updates) ->Model:
        """
        Update a model.
    ...
    async def record_prediction(self, model_id: str, input_data: Dict[str,
        Any], prediction_result: Any, confidence: Optional[float]=None,
        actual_value: Optional[Any]=None, additional_outputs: Optional[Dict
        [str, Any]]=None, metadata: Optional[Dict[str, Any]]=None
        ) ->Prediction:
        """
        ...
        if metadata is None:
            metadata = {}
        prediction = Prediction(model_id=model_id, input_data=input_data,
            prediction=prediction_result, confidence=confidence,
            actual_value=actual_value, additional_outputs=
            additional_outputs, metadata=metadata)
        await self.repository.create_prediction(prediction)
        return prediction

    async def list_predictions(self, model_id: str, start_time: Optional[
        datetime]=None, end_time: Optional[datetime]=None, limit: int=100,
        ...
        Returns:
            List of predictions
        """
        return await self.repository.list_predictions(model_id=model_id,
            start_time=start_time, end_time=end_time, limit=limit, offset=
            offset)
