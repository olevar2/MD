...
class ExperimentTracker:
    ...
    def __init__(self, experiment_name: Optional[str]=None):
        ...
        if mlflow_settings.s3_endpoint_url:
            os.environ['MLFLOW_S3_ENDPOINT_URL'
                ] = mlflow_settings.s3_endpoint_url
            if mlflow_settings.s3_ignore_tls:
                os.environ['MLFLOW_S3_IGNORE_TLS'] = 'true'
        self.client = MlflowClient()
        self.experiment_name = (experiment_name or mlflow_settings.
            default_experiment_name)
        self._create_experiment_if_not_exists()
        self.run_id = None
        self.active_run = None
        logging.info(
            f'ExperimentTracker initialized with experiment: {self.experiment_name}'
            )
    ...
    def _create_experiment_if_not_exists(self) ->None:
        ...
        if experiment is None:
            ...
            self.experiment_id = mlflow.create_experiment(name=self.
                experiment_name, artifact_location=artifact_location, tags=
                mlflow_settings.experiment_tags)
            logging.info(
                f'Created new experiment: {self.experiment_name}, ID: {self.experiment_id}'
                )
        else:
    ...
    def start_run(self, run_name: Optional[str]=None, tags: Optional[Dict[
        str, str]]=None) ->str:
        """
        ...
        default_tags = {'start_time': current_time, 'mlflow.source.type':
            'FOREX_TRADING_PLATFORM'}
        all_tags = {**default_tags, **tags or {}}
        for key, value in all_tags.items():
            mlflow.set_tag(key, value)
        logging.info(
            f"Started MLflow run: {run_name or 'unnamed'}, Run ID: {self.run_id}"
            )
        return self.run_id
    ...
    def end_run(self, status: str=RunStatus.FINISHED) ->None:
        ...
        if self.active_run:
            ...
            logging.info(
                f'Ended MLflow run: {self.run_id} with status: {status}')
            self.active_run = None
            self.run_id = None
        else:
    ...
    @with_exception_handling
    def log_model(self, model_object: Any, artifact_path: str,
        registered_model_name: Optional[str]=None, conda_env: Optional[
        Union[Dict[str, Any], str]]=None, signature: Optional[Any]=None,
        ...
        try:
            if 'sklearn' in str(type(model_object)):
                mlflow.sklearn.log_model(sk_model=model_object,
                    artifact_path=artifact_path, registered_model_name=
                    registered_model_name, conda_env=conda_env, signature=
                    signature, input_example=input_example)
            elif 'xgboost' in str(type(model_object)):
                mlflow.xgboost.log_model(xgb_model=model_object,
                    artifact_path=artifact_path, registered_model_name=
                    registered_model_name, conda_env=conda_env, signature=
                    signature, input_example=input_example)
            elif 'lightgbm' in str(type(model_object)):
                mlflow.lightgbm.log_model(lgb_model=model_object,
                    artifact_path=artifact_path, registered_model_name=
                    registered_model_name, conda_env=conda_env, signature=
                    signature, input_example=input_example)
            elif 'keras' in str(type(model_object)) or 'tensorflow' in str(type
                (model_object)):
                mlflow.tensorflow.log_model(model=model_object,
                    artifact_path=artifact_path, registered_model_name=
                    registered_model_name, conda_env=conda_env, signature=
                    signature, input_example=input_example)
            elif 'torch' in str(type(model_object)):
                mlflow.pytorch.log_model(pytorch_model=model_object,
                    artifact_path=artifact_path, registered_model_name=
                    registered_model_name, conda_env=conda_env, signature=
                    signature, input_example=input_example)
            else:
                mlflow.pyfunc.log_model(python_model=model_object,
                    artifact_path=artifact_path, registered_model_name=
                    registered_model_name, conda_env=conda_env, signature=
                    signature, input_example=input_example)
            ...
            if registered_model_name:
                logging.info(
                    f'Registered model with name: {registered_model_name}')
        except Exception as e:
            logging.error(f'Error logging model: {str(e)}')
            raise

    @with_exception_handling
    def compare_runs(self, run_ids: List[str], metrics: Optional[List[str]]
        =None) ->pd.DataFrame:
        """
        ...
        for run_id in run_ids:
            try:
                ...
                run_info = {'run_id': run_id, 'run_name': run_name,
                    'status': run_status, 'start_time': datetime.
                    fromtimestamp(run.info.start_time / 1000.0)}
                if run.info.end_time:
                    run_info['end_time'] = datetime.fromtimestamp(run.info.
                        end_time / 1000.0)
                    run_info['duration_seconds'] = (run.info.end_time - run
    ...
    def get_best_run(self, metric_name: str, max_results: int=10, ascending:
        bool=False) ->Tuple[str, float]:
        """
