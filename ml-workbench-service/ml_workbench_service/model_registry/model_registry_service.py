...
class ModelRegistryService:
    ...
    def __init__(self, registry_root_path: str):
        ...
        # Cache for metadata to avoid frequent disk reads
    ...
    def register_model(
        ...
        try:
            ...
            # Update model description if provided and different
        ...
        version_info = ModelVersion(
            ...
            creation_time=datetime.datetime.utcnow().isoformat(),
            description=version_desc or f"Version {version}",
            ...
            for name, artifact in artifacts.items():
                ...
                try:
                    if isinstance(artifact, pd.DataFrame):
                        artifact.to_csv(artifact_path.with_suffix(".csv"), index=False)
                        version_info.artifact_paths[name] = str(artifact_path.with_suffix(".csv"))
                    elif isinstance(artifact, dict) or isinstance(artifact, list):
                        with open(artifact_path.with_suffix(".json"), "w") as f:
                            json.dump(artifact, f, indent=2)
                        version_info.artifact_paths[name] = str(artifact_path.with_suffix(".json"))
                    else:
                        # Try joblib for other objects
                        joblib.dump(artifact, artifact_path.with_suffix(".joblib"))
                        version_info.artifact_paths[name] = str(artifact_path.with_suffix(".joblib"))
                except Exception as e:
        ...
        logger.info(f"Registered model '{model_name}' version {version} with ID {version_id}")
        return metadata
    ...
    def get_model_metadata(self, model_name: str) -> ModelMetadata:
        ...
        if not metadata_path.exists():
            raise ModelNotFoundException(f"Model '{model_name}' not found in registry")
        ...
        try:
            ...
            metadata = ModelMetadata(
                ...
                latest_version=metadata_dict["latest_version"],
                versions=[]
            )
            ...
            for version_dict in metadata_dict.get("versions", []):
                version_info = ModelVersion(
                    ...
                    creation_time=version_dict["creation_time"],
                    description=version_dict.get("description", ""),
                    metrics=ModelMetrics(**(version_dict.get("metrics", {}))),
                    parameters=version_dict.get("parameters", {}),
                    framework=version_dict.get("framework", "unknown"),
                    stage=ModelStage(version_dict.get("stage", "development")),
            ...
            raise ModelRegistryException(f"Failed to load metadata for model '{model_name}': {e}")
    ...
    def _save_metadata(self, metadata: ModelMetadata) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to save metadata for model '{metadata.name}': {e}")
    ...
    def load_model(
        ...
        if version is not None:
            ...
            # Find latest version in specified stage
            ...
            # Default to production, fallback to latest if none in production
            production_versions = [
                v for v in metadata.versions if v.stage == ModelStage.PRODUCTION
            ]
        ...
        if "model" not in version_info.artifact_paths:
            raise ModelRegistryException(f"Model artifact not found for '{model_name}' version {version_info.version}")
            
        model_path = version_info.artifact_paths["model"]
        try:
            model = joblib.load(model_path)
            logger.info(
                f"Loaded model '{model_name}' version {version_info.version} "
                f"(stage: {version_info.stage.value})"
            )
            return model
        except Exception as e:
    ...
    def load_artifact(
        ...
        try:
            ...
            raise ModelRegistryException(f"Failed to load artifact '{artifact_name}': {e}")
    
    def update_model_version_stage(
        self,
        model_name: str,
        version: int,
        stage: ModelStage
        ...
        # Check if we need to update other versions based on stage transitions
        if stage == ModelStage.PRODUCTION:
            # If moving to production, transition any existing production models to staging
            for v in metadata.versions:
                if v.version != version and v.stage == ModelStage.PRODUCTION:
                    v.stage = ModelStage.STAGING
                    logger.info(
                        f"Moved '{model_name}' version {v.version} from production to staging "
                        f"as version {version} is now in production"
                    )
        
        # Save metadata
        self._save_metadata(metadata)
        
        logger.info(f"Updated '{model_name}' version {version} stage to {stage.value}")
        return metadata
    ...
    def delete_model_version(
        ...
        # Check if this is the production model
    ...
    def search_models(
        ...
        model_type: Optional[str] = None,
        tag_filter: Optional[Dict[str, str]] = None,
        min_metric_filter: Optional[Dict[str, float]] = None,
        ...
        for metadata_file in metadata_files:
            try:
                ...
                # Find production version if available
                production_version = None
                ...
                if min_metric_filter and production_metrics:
                    if not all(
                        production_metrics.get(k, 0) >= v 
                        for k, v in min_metric_filter.items()
                    ):
                        continue
    ...
    def compare_versions(
        ...
        # Compare metrics
        metrics_comparison = {}
        all_metrics = set(vars(v1_info.metrics).keys()) | set(vars(v2_info.metrics).keys())
        ...
        # Compare parameters
        param_comparison = {}
        all_params = set(v1_info.parameters.keys()) | set(v2_info.parameters.keys())
        ...
        # Compare features
        feature_comparison = {
            "version1_only": [f for f in v1_info.feature_names if f not in v2_info.feature_names],
            "version2_only": [f for f in v2_info.feature_names if f not in v1_info.feature_names],
            "common": [f for f in v1_info.feature_names if f in v2_info.feature_names]
        }
        
        # Compare artifacts
        artifact_comparison = {
            "version1_only": [
                a for a in v1_info.artifact_paths.keys() 
                if a not in v2_info.artifact_paths
            ],
            "version2_only": [
                a for a in v2_info.artifact_paths.keys() 
                if a not in v1_info.artifact_paths
            ],
            "common": [
                a for a in v1_info.artifact_paths.keys() 
                if a in v2_info.artifact_paths
            ]
        }
        ...
        comparison = {
            ...
            "version1": {
                "version": version1,
                "creation_time": v1_info.creation_time,
                "stage": v1_info.stage.value,
                "framework": v1_info.framework
            },
            "version2": {
                "version": version2,
                "creation_time": v2_info.creation_time,
                "stage": v2_info.stage.value,
                "framework": v2_info.framework
            },
    ...
    def setup_ab_test(
        ...
        try:
            ...
            logger.info(
                f"Created A/B test '{test_name}' between '{model_name}' "
                f"versions {version_a} and {version_b} with {traffic_split:.0%} traffic to B"
            )
    ...
    def get_ab_test(self, test_id: str) -> Dict[str, Any]:
        ...
        if not test_path.exists():
            raise ModelRegistryException(f"A/B test with ID '{test_id}' not found")
