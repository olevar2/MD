...
class ModelVersion(BaseModel):
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'version_id': self.version_id, 'model_id': self.model_id,
            'version_number': self.version_number, 'created_at': self.
            created_at.isoformat(), 'updated_at': self.updated_at.isoformat
            (), 'created_by': self.created_by, 'description': self.
            description, 'status': self.status.value, 'framework': self.
            framework.value, 'framework_version': self.framework_version,
            'metrics': self.metrics.all_metrics, 'hyperparameters': self.
            hyperparameters.as_dict, 'training_dataset_id': self.
            training_dataset_id, 'validation_dataset_id': self.
            validation_dataset_id, 'test_dataset_id': self.test_dataset_id,
            'file_path': self.file_path, 'feature_columns': self.
            feature_columns, 'target_column': self.target_column,
            'preprocessing_steps': self.preprocessing_steps, 'tags': self.
            tags, 'notes': self.notes, 'experiment_id': self.experiment_id,
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'ModelVersion':
        ...
        if 'framework' in data and not isinstance(data['framework'],
            ModelFramework):
            data['framework'] = ModelFramework(data['framework'])
        if 'metrics' in data and isinstance(data['metrics'], dict):
            custom_metrics = {k: v for k, v in data['metrics'].items() if k
                 not in ModelMetrics.__fields__}
            standard_metrics = {k: v for k, v in data['metrics'].items() if
                k in ModelMetrics.__fields__}
            metrics_data = standard_metrics.copy()
            if custom_metrics:
                metrics_data['custom_metrics'] = custom_metrics
            data['metrics'] = ModelMetrics(**metrics_data)
        if 'hyperparameters' in data and isinstance(data['hyperparameters'],
            dict):
            if not isinstance(data['hyperparameters'], HyperParameters):
                if 'values' not in data['hyperparameters']:
                    data['hyperparameters'] = HyperParameters(values=data[
                        'hyperparameters'])
                else:
                    data['hyperparameters'] = HyperParameters(**data[
                        'hyperparameters'])
        return cls(**data)
...
class ModelMetadata(BaseModel):
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'model_id': self.model_id, 'name': self.name, 'description':
            self.description, 'created_at': self.created_at.isoformat(),
            'updated_at': self.updated_at.isoformat(), 'created_by': self.
            created_by, 'model_type': self.model_type.value, 'tags': self.
            tags, 'latest_version_id': self.latest_version_id,
            'latest_version_number': self.latest_version_number,
            'production_version_id': self.production_version_id,
            'staging_version_id': self.staging_version_id,
            'business_domain': self.business_domain, 'purpose': self.
            purpose, 'training_frequency': self.training_frequency,
            'monitoring_config': self.monitoring_config,
            'versioning_strategy': self.versioning_strategy, 'metadata':
            self.metadata}
    ...
    def from_dict(cls, data: Dict[str, Any]) ->'ModelMetadata':
        ...
        if 'updated_at' in data and isinstance(data['updated_at'], str):
            data['updated_at'] = datetime.fromisoformat(data['updated_at'])
        if 'model_type' in data and not isinstance(data['model_type'],
            ModelType):
...
class ModelRegistry:
    ...
    def __init__(self, storage_path: str=None):
        ...
        for directory in [self.metadata_dir, self.version_dir, self.
            artifacts_dir]:
            os.makedirs(directory, exist_ok=True)
        logger.info(
            f'Initialized ModelRegistry with storage path: {self.storage_path}'
            )

    def register_model(self, name: str, model_type: Union[ModelType, str],
        description: Optional[str]=None, created_by: str='system', tags:
        List[str]=None, business_domain: Optional[str]=None, purpose:
        Optional[str]=None, metadata: Dict[str, Any]=None) ->ModelMetadata:
        """
        ...
        model_metadata = ModelMetadata(name=name, description=description,
            created_by=created_by, model_type=model_type, tags=tags or [],
            business_domain=business_domain, purpose=purpose, metadata=
            metadata or {})
        self._save_model_metadata(model_metadata)
        logger.info(
            f'Registered new model: {name} (ID: {model_metadata.model_id})')
        return model_metadata

    def _save_model_metadata(self, metadata: ModelMetadata) ->None:
        """Save model metadata to disk"""
        metadata_path = os.path.join(self.metadata_dir,
            f'{metadata.model_id}.json')
    ...
    @with_exception_handling
    def create_model_version(self, model_id: str, framework: Union[
        ModelFramework, str], framework_version: str, created_by: str,
        model_file_path: Optional[str]=None, description: Optional[str]=
        None, hyperparameters: Optional[Union[Dict[str, Any],
        HyperParameters]]=None, metrics: Optional[Union[Dict[str, float],
        ModelMetrics]]=None, feature_columns: List[str]=None, target_column:
        Optional[str]=None, preprocessing_steps: List[Dict[str, Any]]=None,
        training_dataset_id: Optional[str]=None, validation_dataset_id:
        Optional[str]=None, test_dataset_id: Optional[str]=None, tags: List
        [str]=None, experiment_id: Optional[str]=None, notes: Optional[str]
        =None, metadata: Dict[str, Any]=None, status: ModelStatus=
        ModelStatus.DRAFT) ->ModelVersion:
        """
        ...
        if hyperparameters is not None and not isinstance(hyperparameters,
            HyperParameters):
        ...
        if metrics is not None and not isinstance(metrics, ModelMetrics):
            standard_metrics = {k: v for k, v in metrics.items() if k in
                ModelMetrics.__fields__}
        ...
        if model_file_path:
            ...
            artifact_path = os.path.join(model_artifacts_dir,
                f'version_{version_number}{file_ext}')
            try:
                shutil.copy2(model_file_path, artifact_path)
                logger.info(
                    f'Copied model artifact from {model_file_path} to {artifact_path}'
                    )
            except Exception as e:
                logger.error(f'Failed to copy model artifact: {str(e)}')
                artifact_path = None
        version = ModelVersion(model_id=model_id, version_number=
            version_number, created_by=created_by, description=description,
            status=status, framework=framework, framework_version=
            framework_version, hyperparameters=hyperparameters, metrics=
            metrics, feature_columns=feature_columns or [], target_column=
            target_column, preprocessing_steps=preprocessing_steps or [],
            training_dataset_id=training_dataset_id, validation_dataset_id=
            validation_dataset_id, test_dataset_id=test_dataset_id,
            file_path=artifact_path, tags=tags or [], experiment_id=
            experiment_id, notes=notes, metadata=metadata or {})
        ...
        self._save_model_metadata(model_metadata)
        logger.info(
            f'Created version {version_number} for model {model_metadata.name} (ID: {model_id})'
            )
        return version
    ...
    def get_model_metadata(self, model_id: str) ->Optional[ModelMetadata]:
        ...
        try:
            ...
            logger.error(
                f'Error loading model metadata for {model_id}: {str(e)}')
            return None
    ...
    @with_exception_handling
    def list_models(self, name_filter: Optional[str]=None, model_type:
        Optional[Union[ModelType, str]]=None, tags: Optional[List[str]]=
        None, business_domain: Optional[str]=None) ->List[ModelMetadata]:
        """
        ...
        for filename in os.listdir(self.metadata_dir):
            ...
            try:
                with open(os.path.join(self.metadata_dir, filename), 'r') as f:
                    ...
                    if name_filter and name_filter.lower(
                        ) not in model.name.lower():
                        continue
                    if model_type and model.model_type != model_type:
                        continue
                    if tags and not all(tag in model.tags for tag in tags):
                        continue
                    if (business_domain and model.business_domain !=
                        business_domain):
                        continue
                    models.append(model)
            except Exception as e:
    ...
    @with_exception_handling
    def list_versions(self, model_id: str, status_filter: Optional[Union[
        ModelStatus, str]]=None, min_version: Optional[int]=None,
        max_version: Optional[int]=None) ->List[ModelVersion]:
        """
            ...
            try:
                with open(os.path.join(self.version_dir, filename), 'r') as f:
                    ...
                    if status_filter and version.status != status_filter:
                        continue
                    if (min_version is not None and version.version_number <
                        min_version):
                        continue
                    if (max_version is not None and version.version_number >
                        max_version):
                        continue
    ...
    def set_version_status(self, version_id: str, status: Union[ModelStatus,
        str]) ->Optional[ModelVersion]:
        """
        ...
        if status == ModelStatus.PRODUCTION:
            ...
            logger.info(
                f'Set version {version.version_number} as production for model {model_metadata.name}'
                )
        elif status == ModelStatus.STAGING:
            model_metadata.staging_version_id = version_id
            logger.info(
                f'Set version {version.version_number} as staging for model {model_metadata.name}'
                )
        self._save_model_version(version)
        self._save_model_metadata(model_metadata)
        return version
    ...
    def delete_model(self, model_id: str) ->bool:
        ...
        try:
            ...
            logger.info(
                f'Deleted model {model_id} with {len(versions)} versions')
            return True
        except Exception as e:
            ...
            if model_metadata:
                ...
                if model_metadata.latest_version_id == version_id:
                    ...
                    if versions:
                        ...
                        model_metadata.latest_version_number = (latest.
                            version_number)
                    else:
                ...
                if updated:
                    ...
                    self._save_model_metadata(model_metadata)
            logger.info(
                f'Deleted version {version.version_number} of model {model_id}'
                )
            return True
        except Exception as e:
    ...
    def compare_versions(self, version_id_1: str, version_id_2: str) ->Dict[
        ...
        if version1.model_id != version2.model_id:
            logger.warning(
                f'Comparing versions from different models: {version1.model_id} vs {version2.model_id}'
                )
        metrics_diff = {}
        all_metrics = set(version1.metrics.all_metrics.keys()) | set(version2
        ...
        for metric in all_metrics:
            ...
            if v1_value is not None and v2_value is not None:
                ...
                rel_diff = abs_diff / v1_value if v1_value != 0 else float(
                    'inf')
                metrics_diff[metric] = {'version1': v1_value, 'version2':
                    v2_value, 'absolute_diff': abs_diff, 'relative_diff':
                    rel_diff}
        hyperparams_diff = {}
        all_params = set(version1.hyperparameters.as_dict.keys()) | set(
            version2.hyperparameters.as_dict.keys())
        for param in all_params:
            v1_value = version1.hyperparameters.values.get(param)
            v2_value = version2.hyperparameters.values.get(param)
            if v1_value != v2_value:
                hyperparams_diff[param] = {'version1': v1_value, 'version2':
                    v2_value}
        attributes_diff = {}
        if set(version1.feature_columns) != set(version2.feature_columns):
            attributes_diff['feature_columns'] = {'version1': version1.
                feature_columns, 'version2': version2.feature_columns,
                'added': list(set(version2.feature_columns) - set(version1.
                feature_columns)), 'removed': list(set(version1.
                feature_columns) - set(version2.feature_columns))}
        if version1.preprocessing_steps != version2.preprocessing_steps:
            attributes_diff['preprocessing_steps'] = {'version1': version1.
                preprocessing_steps, 'version2': version2.preprocessing_steps}
        return {'version1': {'version_id': version1.version_id,
            'version_number': version1.version_number, 'created_at':
            version1.created_at.isoformat(), 'status': version1.status.
            value}, 'version2': {'version_id': version2.version_id,
            'version_number': version2.version_number, 'created_at':
            version2.created_at.isoformat(), 'status': version2.status.
            value}, 'metrics_comparison': metrics_diff,
            'hyperparameters_comparison': hyperparams_diff,
            'attributes_comparison': attributes_diff, 'same_framework': 
            version1.framework == version2.framework,
            'same_training_dataset': version1.training_dataset_id ==
            version2.training_dataset_id}

    def get_best_version(self, model_id: str, metric: str, higher_is_better:
        bool=True, status_filter: Optional[Union[ModelStatus, str]]=None,
        min_version: Optional[int]=None) ->Optional[ModelVersion]:
        """
        ...
        """
        versions = self.list_versions(model_id=model_id, status_filter=
            status_filter, min_version=min_version)
        if not versions:
            return None
        best_version = None
        best_value = None
        for version in versions:
            ...
            if value is None:
                continue
            if (best_value is None or higher_is_better and value >
                best_value or not higher_is_better and value < best_value):
                best_value = value
    ...
    def update_version_metrics(self, version_id: str, metrics: Dict[str, float]
        ...
        standard_metrics = {k: v for k, v in metrics.items() if k in
            ModelMetrics.__fields__ and k != 'custom_metrics'}
        custom_metrics = {k: v for k, v in metrics.items() if k not in
            ModelMetrics.__fields__}
        ...
        self._save_model_version(version)
        logger.info(
            f'Updated metrics for version {version.version_number} of model {version.model_id}'
            )
        return version

    def add_version_tags(self, version_id: str, tags: List[str]) ->Optional[
        ModelVersion]:
        """
        ...
        self._save_model_version(version)
        logger.info(
            f'Added tags to version {version.version_number} of model {version.model_id}: {tags}'
            )
        return version

    def add_model_tags(self, model_id: str, tags: List[str]) ->Optional[
        ModelMetadata]:
        """
        Add tags to a model.
    ...
    def configure_ab_testing(self, model_id: str, version_ids: List[str],
        traffic_split: List[float]) ->bool:
        """
        ...
        for version_id in version_ids:
            ...
            if version.model_id != model_id:
                logger.error(
                    f'Version {version_id} does not belong to model {model_id}'
                    )
                return False
        ab_testing_config = {'enabled': True, 'configured_at': datetime.
            utcnow().isoformat(), 'versions': [{'version_id': vid,
            'traffic': split} for vid, split in zip(version_ids,
            traffic_split)]}
        ...
        self._save_model_metadata(model)
        logger.info(
            f'Configured A/B testing for model {model.name} with {len(version_ids)} versions'
            )
        return True
