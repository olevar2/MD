...
from copy import deepcopy # Needed for deep copying models

# Conditionally import libraries that might not be available
try:
    import torch
...
class InferenceBenchmark:
    ...
    def run_inference_benchmark(
        ...
        latencies = []
        memory_usages = []
        batch_sizes = [1, 4, 8, 16, 32]  # Test different batch sizes if applicable
        
        # Track process memory
        process = psutil.Process(os.getpid())
        
        # First run warmup episodes
        ...
        starting_memory = process.memory_info().rss / 1024 / 1024  # MB
        
        # Run benchmark episodes
        logger.info(f"Running {num_episodes} benchmark episodes with {steps_per_episode} steps each")
        ...
        for episode in range(num_episodes):
            ...
            # Log progress
            avg_step_latency = episode_stats['total_latency'] / max(1, episode_stats['inference_calls'])
            logger.info(f"Episode {episode+1}/{num_episodes}: "
                       f"Avg step latency: {avg_step_latency*1000:.2f}ms, "
        ...
        memory_change = memory_usages[-1] - starting_memory if memory_usages else 0
        ...
        self.latency_stats = {
            ...
            'throughput': total_inference_calls / max(0.001, total_latency)
        }
        
        self.memory_stats = {
            'start_mb': starting_memory,
            'end_mb': memory_usages[-1] if memory_usages else starting_memory,
            'peak_mb': peak_memory,
            'average_mb': avg_memory,
            'change_mb': memory_change
        }
        
        # Test batch performance if applicable
        batch_perf = {}
        if hasattr(self.model, 'predict_batch') or (hasattr(self.model, 'predict') and 
                                                     'batch_size' in self.model.predict.__code__.co_varnames):
    ...
    def _run_episode(self, steps_per_episode: int, warmup: bool = False) -> Dict[str, Any]:
    ...
    def _benchmark_batch_sizes(self, batch_sizes: List[int]) -> Dict[str, List[float]]:
        """Benchmark inference with different batch sizes."""
        batch_results = {"batch_size": [], "latency_ms": [], "throughput": []}
        
        # Generate a synthetic batch of observations for testing
        observation = self.environment.reset()
        if isinstance(observation, np.ndarray):
            sample_shape = observation.shape
        else:
            logger.warning("Cannot benchmark batch sizes with non-array observations")
        ...
        for batch_size in batch_sizes:
            ...
            if len(sample_shape) == 1:
                ...
                # Handle more complex observation shapes
                batch = np.stack([observation] * batch_size)
                
            # Measure batch inference time (average over multiple runs)
            num_runs = 50
            latencies = []
            ...
            for _ in range(num_runs):
                start_time = time.time()
                try:
                    # Try different methods based on the model
            ...
            if latencies:
                ...
                throughput = batch_size / avg_latency  # observations per second
                ...
                logger.info(f"Batch size {batch_size}: {avg_latency*1000:.2f}ms, "
    ...
    def generate_report(self, filename: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not self.latency_stats:
            logger.warning("No benchmark data available, run the benchmark first")
            return {}
            
        report = {
            "summary": {
                "average_inference_latency_ms": self.latency_stats.get('average_ms', 0),
                "p95_latency_ms": self.latency_stats.get('p95_ms', 0),
                "p99_latency_ms": self.latency_stats.get('p99_ms', 0),
                "inference_throughput": self.latency_stats.get('throughput', 0),
                "peak_memory_mb": self.memory_stats.get('peak_mb', 0),
                "memory_growth_mb": self.memory_stats.get('change_mb', 0)
            },
            "requirements_check": {
                "meets_latency_requirement": self.latency_stats.get('p95_ms', 100) < 50,  # Under 50ms requirement
                "meets_throughput_requirement": self.latency_stats.get('throughput', 0) > 20  # Arbitrarily chosen
            },
            "detail": {
                "latency": self.latency_stats,
                "memory": self.memory_stats
...
class TrainingBenchmark:
    ...
    def __init__(self, model_cls, model_kwargs, env_cls, env_kwargs, metrics_dir: Optional[str] = None):
        """
    ...
    def _create_env_and_model(self):
        ...
        try:
            ...
            logger.info(f"Created model {self.model_cls.__name__} and environment {self.env_cls.__name__}")
        except Exception as e:
    ...
    def run_training_benchmark(
        ...
        if not SB3_AVAILABLE:
            logger.warning("Stable Baselines 3 is not available. Cannot run training benchmark.")
            return {}
        
        # Benchmark callback to measure performance
            ...
            def _on_step(self):
                ...
                if self.num_timesteps % self.log_interval == 0:
                    ...
                    if elapsed > 0:
                        ...
                        if TORCH_AVAILABLE and torch.cuda.is_available():
                            try:
                                self.gpu_utilizations.append(torch.cuda.utilization())
                                self.gpu_memory_usages.append(torch.cuda.memory_allocated() / 1024 / 1024)  # MB
                            except:
                                self.gpu_utilizations.append(0)
                                self.gpu_memory_usages.append(0)
                                
                        # Progress report
                        logger.info(f"Step {self.num_timesteps}/{self.total_steps} "
                                   f"({self.num_timesteps/self.total_steps*100:.1f}%) | "
        ...
        # Start training with benchmark callback
        logger.info(f"Starting training benchmark for {total_timesteps} timesteps")
        start_time = time.time()
        ...
        try:
            ...
            # Run training with benchmark callback
            self.model.learn(total_timesteps=total_timesteps, callback=benchmark_callback)
            ...
            avg_cpu = np.mean(benchmark_callback.cpu_utilizations) if benchmark_callback.cpu_utilizations else 0
            avg_memory = np.mean(benchmark_callback.memory_usages) if benchmark_callback.memory_usages else 0
            peak_memory = max(benchmark_callback.memory_usages) if benchmark_callback.memory_usages else 0
            
            # GPU stats if available
            avg_gpu = np.mean(benchmark_callback.gpu_utilizations) if benchmark_callback.gpu_utilizations else 0
            avg_gpu_memory = np.mean(benchmark_callback.gpu_memory_usages) if benchmark_callback.gpu_memory_usages else 0
            
            self.training_stats = {
                'total_time_seconds': training_time,
                'total_timesteps': total_timesteps,
                'average_throughput': avg_throughput,
                'throughput_history': benchmark_callback.throughputs,
                'timestamps': [t - start_time for t in benchmark_callback.timestamps],
                ...
                'gpu_stats': {
                    'utilization_percent': avg_gpu,
                    'memory_mb': avg_gpu_memory
                } if TORCH_AVAILABLE and torch.cuda.is_available() else {},
                'step_times': np.diff(benchmark_callback.step_timestamps).tolist() if len(benchmark_callback.step_timestamps) > 1 else []
            }
            
            logger.info(f"Training benchmark completed in {training_time:.2f} seconds")
            logger.info(f"Average throughput: {avg_throughput:.2f} steps/sec")
            logger.info(f"CPU utilization: {avg_cpu:.1f}%, Memory usage: {avg_memory:.1f}MB")
            
            if TORCH_AVAILABLE and torch.cuda.is_available():
                logger.info(f"GPU utilization: {avg_gpu:.1f}%, GPU memory: {avg_gpu_memory:.1f}MB")
                
            # Save metrics if directory specified
            if self.metrics_dir:
                os.makedirs(self.metrics_dir, exist_ok=True)
                metrics_file = os.path.join(self.metrics_dir, f"training_benchmark_{int(time.time())}.json")
    ...
    def generate_training_report(self, filename: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if not self.training_stats:
            logger.warning("No training benchmark data available, run the benchmark first")
            return {}
            
        report = {
            "summary": {
                "total_training_time_seconds": self.training_stats.get('total_time_seconds', 0),
                "average_throughput": self.training_stats.get('average_throughput', 0),
                "cpu_utilization_percent": self.training_stats.get('cpu_utilization_percent', 0),
                "peak_memory_mb": self.training_stats.get('memory_usage_mb', {}).get('peak', 0)
            },
            "requirements_check": {
                "meets_throughput_requirement": self.training_stats.get('average_throughput', 0) > 100,  # Arbitrary threshold
                "meets_memory_requirement": self.training_stats.get('memory_usage_mb', {}).get('peak', 0) < 4000  # Under 4GB
            },
            "detail": self.training_stats,
            "timestamp": datetime.now().isoformat()
        }
...
class OptimizationTechniques:
    ...
    def optimize_environment(env, vectorized: bool = True, num_envs: int = 4):
        ...
        """
        if not SB3_AVAILABLE:
            logger.warning("Stable Baselines 3 is not available. Cannot optimize environment.")
            return env
        ...
        if vectorized:
            try:
                # Check if we can directly vectorize the environment
                if hasattr(env, "num_envs"):
                    logger.info(f"Environment is already vectorized with {env.num_envs} envs")
                    return env
                
                # Create a vectorized environment
                if num_envs > 1:
                    # Try to use SubprocVecEnv for true parallelism if environment supports it
                    try:
                        # Need to create a function that creates environment instances
                        def make_env():
                            return type(env)(**getattr(env, "init_kwargs", {}))
                        
                        vec_env = SubprocVecEnv([make_env for _ in range(num_envs)])
                        logger.info(f"Created SubprocVecEnv with {num_envs} parallel environments")
                        return vec_env
                    except Exception as e:
                    ...
                    # Just use DummyVecEnv for single environment
                    vec_env = DummyVecEnv([lambda: env])
                    return vec_env
            except Exception as e:
    ...
    @staticmethod
    def optimize_policy_network(policy, batch_norm: bool = True, dropout_rate: float = 0.0):
        """
        ...
        if not TORCH_AVAILABLE:
            logger.warning("PyTorch is not available. Cannot optimize policy network.")
            return policy
            
        # This is just a skeleton - actual implementation would depend on the policy architecture
        logger.info("Policy network optimization is model-specific. Returning original policy.")
        return policy
    ...
    def quantize_model(model: torch.nn.Module, 
        ...
        if not isinstance(model, torch.nn.Module):
             logger.warning(f"Quantization requires a torch.nn.Module. Got {type(model)}. Skipping.")
             return model
        ...
        try:
            ...
            quantized_model = deepcopy(model) # Work on a copy

            logger.info(f"Attempting {quantization_type} quantization with backend {backend}...")
            ...
            if quantization_type == 'dynamic':
                ...
                if eval_fn is None or calib_data is None:
                    logger.warning("Static/QAT quantization requires 'eval_fn' and 'calib_data'. Skipping.")
                    return model
                ...
                with torch.no_grad():
                    ...
                    # This part needs adaptation based on how calib_data is structured
                    # Assuming calib_data yields batches of inputs
                    for data_batch in calib_data: 
                        # Assuming data_batch is the input tensor or a tuple/dict containing it
                        if isinstance(data_batch, (list, tuple)):
                            inputs = data_batch[0].cpu() 
                        elif isinstance(data_batch, dict):
                             # Adapt based on expected key
                            inputs = data_batch.get('observations', data_batch.get('input')).cpu()
                        else:
                            inputs = data_batch.cpu()
                        quantized_model(inputs) # Forward pass for calibration
                ...
                # Evaluate accuracy impact (optional)
                logger.info("Evaluating accuracy of static quantized model...")
                accuracy = eval_fn(quantized_model, calib_data) # Use calib_data or a separate test set
                logger.info(f"Static quantized model accuracy: {accuracy}")

            else:
                logger.warning(f"Unsupported quantization type: {quantization_type}. Returning original model.")
                return model

            # Move back to original device if needed (though quantization often targets CPU)
    ...
    def prune_model(model: torch.nn.Module, 
        ...
        if not isinstance(model, torch.nn.Module):
             logger.warning(f"Pruning requires a torch.nn.Module. Got {type(model)}. Skipping.")
             return model

        try:
            logger.info(f"Applying {pruning_method} pruning with amount {amount}...")
            original_device = next(model.parameters()).device
            model.cpu().eval() # Pruning typically done on CPU

            parameters_to_prune = []
            for module in model.modules():
                if isinstance(module, module_types):
                    parameters_to_prune.append((module, 'weight'))
                    # Add bias pruning if desired: parameters_to_prune.append((module, 'bias'))

            if not parameters_to_prune:
                logger.warning(f"No modules of types {module_types} found for pruning.")
                return model
            ...
            # Add other methods like structured pruning if needed
            else:
                logger.warning(f"Unsupported pruning method: {pruning_method}. Skipping.")
                return model

            # Make pruning permanent by removing re-parameterization
            logger.info("Making pruning permanent...")
            for module, name in parameters_to_prune:
                if prune.is_pruned(module): # Check if pruning was applied
    ...
    @staticmethod
    def benchmark_optimized_model(original_model, 
                                  optimized_model, 
                                  environment, 
                                  eval_fn: Optional[Callable] = None, 
                                  eval_data: Optional[Any] = None,
        ...
        orig_perf = orig_benchmark.run_inference_benchmark(num_episodes, steps_per_episode)
        ...
        opt_perf = opt_benchmark.run_inference_benchmark(num_episodes, steps_per_episode)
        results["optimized"]["performance"] = opt_perf
        if eval_fn and eval_data:
             results["optimized"]["accuracy"] = eval_fn(optimized_model, eval_data)
        ...
        results["comparison"]["latency_reduction_percent"] = (1 - opt_latency / orig_latency) * 100 if orig_latency else 0
        results["comparison"]["throughput_increase_percent"] = (opt_throughput / orig_throughput - 1) * 100 if orig_throughput else 0
        results["comparison"]["peak_memory_reduction_percent"] = (1 - opt_peak_mem / orig_peak_mem) * 100 if orig_peak_mem else 0
        
        if "accuracy" in results["original"] and "accuracy" in results["optimized"]:
             orig_acc = results["original"]["accuracy"]
             opt_acc = results["optimized"]["accuracy"]
             # Handle different accuracy types (scalar vs dict)
             if isinstance(orig_acc, dict) and isinstance(opt_acc, dict):
                 # Assuming a primary metric like 'score' or 'accuracy' exists
                 primary_metric = next(iter(orig_acc)) 
                 results["comparison"]["accuracy_change"] = opt_acc.get(primary_metric, 0) - orig_acc.get(primary_metric, 0)
             elif isinstance(orig_acc, (int, float)) and isinstance(opt_acc, (int, float)):
                 results["comparison"]["accuracy_change"] = opt_acc - orig_acc
             else:
                 results["comparison"]["accuracy_change"] = "N/A (Incompatible formats)"


        logger.info(f"Comparison Summary:")
        logger.info(f"  Latency Reduction: {results['comparison']['latency_reduction_percent']:.2f}%")
        logger.info(f"  Throughput Increase: {results['comparison']['throughput_increase_percent']:.2f}%")
        logger.info(f"  Peak Memory Reduction: {results['comparison']['peak_memory_reduction_percent']:.2f}%")
        if "accuracy_change" in results["comparison"]:
             logger.info(f"  Accuracy Change: {results['comparison']['accuracy_change']}")
    ...
    def optimize_batch_inference(model, batch_size: int = 16):
        ...
        """
        # Create a batch inference wrapper if needed
        if not hasattr(model, 'predict_batch'):
            ...
            def predict_batch(self, observations, deterministic=False):
                if isinstance(observations, np.ndarray) and len(observations.shape) > 1:
                    batch_size = observations.shape[0]
                    results = [original_predict(observations[i:i+1], deterministic=deterministic) 
                              for i in range(batch_size)]
                    actions = np.vstack([r[0] for r in results])
                    states = [r[1] for r in results] if results[0][1] is not None else None
                    return actions, states
                else:
                    return original_predict(observations, deterministic=deterministic)
                    
            model.predict_batch = predict_batch.__get__(model)
            logger.info(f"Added batch prediction method optimized for batch size {batch_size}")
...
class PerformanceTester:
    ...
    def __init__(self, 
        ...
        # Create model and environment if needed
        if self.model is None and self.model_cls is not None:
            ...
            if self.environment is not None:
                self.model = self.model_cls(self.environment, **self.model_kwargs)
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        if self.model is not None and self.environment is not None:
            ...
            self.inference_benchmark = InferenceBenchmark(self.model, self.environment)
            inference_results = self.inference_benchmark.run_inference_benchmark()
        ...
        # Run training benchmark if possible
        if self.model_cls is not None and (self.env_cls is not None or self.environment is not None):
            logger.info("Running training benchmark...")
            self.training_benchmark = TrainingBenchmark(
                self.model_cls, 
                self.model_kwargs,
                self.env_cls or type(self.environment),
                self.env_kwargs,
        ...
        if self.output_dir:
            report_path = os.path.join(self.output_dir, "comprehensive_performance_report.json")
    ...
    def _run_optimization_demo(self) -> Dict[str, Any]:
        ...
        if self.model is None or self.environment is None:
            return {"status": "skipped", "reason": "Model or environment not available"}
            
        results = {}
        
        # 1. Vectorized environment optimization
        try:
            ...
            optimized_env = OptimizationTechniques.optimize_environment(original_env, num_envs=4)
            
            # Compare performance (simplified test)
            original_time = 0
            optimized_time = 0
            ...
            if hasattr(optimized_env, "reset"):
                ...
                for i in range(100):
                    if hasattr(optimized_env, "num_envs"):
                        action = np.zeros((optimized_env.num_envs,) + original_env.action_space.shape)
                    else:
                        action = np.zeros(original_env.action_space.shape)
            ...
            results["vectorized_env"] = {
                "original_time_ms": original_time * 1000,
                "optimized_time_ms": optimized_time * 1000,
                "speedup": speedup,
                "status": "success" if speedup > 1 else "no improvement"
            }
        ...
        try:
            ...
            optimized_model = OptimizationTechniques.optimize_batch_inference(self.model)
