...
from risk_management_service.models.risk_metrics import calculate_sharpe_ratio, calculate_sortino_ratio, calculate_max_drawdown, calculate_value_at_risk, calculate_expected_shortfall
...
class RLToolEffectiveness:
    ...
    def __init__(self, model_id: str, model_name: str, model_version: str,
        ...
        self.metric_halflife = {EffectivenessMetricType.REWARD: timedelta(
            days=30), EffectivenessMetricType.PNL: timedelta(days=30),
    ...
    def assess_action_quality(self, action: RLModelAction
        ) ->ActionQualityAssessment:
        """
        ...
        """
        if 'action_values' in action.metadata and len(action.metadata[
            'action_values']) > 0:
            optimal_action_value = max(action.metadata['action_values'].
                items(), key=lambda x: x[1])
            optimal_action = optimal_action_value[0]
            max_value = optimal_action_value[1]
            taken_value = action.metadata['action_values'].get(str(action.
                action_taken), list(action.metadata['action_values'].values
                ())[0])
            value_range = max(1e-06, max_value - min(action.metadata[
            ...
            if optimal_action != str(action.action_taken):
                explanation += (
                    f'Optimal would have been {optimal_action} with value {max_value:.4f} vs {taken_value:.4f}.'
                    )
            else:
                explanation += 'This was the optimal action.'
            assessment = ActionQualityAssessment(action=action,
                optimal_action=optimal_action, optimality_score=
                optimality_score, opportunity_cost=opportunity_cost,
                explanation=explanation, factors={'value_confidence': 
                taken_value / max(1e-06, max_value), 'exploration_ratio':
                action.metadata.get('exploration_ratio', 0.0),
                'action_entropy': action.metadata.get('action_entropy', 0.0)})
        else:
            assessment = ActionQualityAssessment(action=action,
                optimality_score=0.5, explanation=
                'Limited assessment: no action values available.')
    ...
    @with_exception_handling
    def update_effectiveness_metrics(self, episode_metrics: Dict[str, float
        ], market_regime: MarketRegimeType, timestamp: Optional[datetime]=None
        ) ->None:
        """
        ...
        for key, value in episode_metrics.items():
            ...
            if key not in self.effectiveness_metrics:
                self.effectiveness_metrics[key] = {'current': value,
                    'history': [], 'decay_factor': self._get_decay_factor(
                    metric_type)}
            else:
                current = self.effectiveness_metrics[key]['current']
                decay = self.effectiveness_metrics[key]['decay_factor']
                self.effectiveness_metrics[key]['current'] = current * (1 -
                    decay) + value * decay
            self.effectiveness_metrics[key]['history'].append({'timestamp':
                timestamp, 'value': value, 'decayed_value': self.
                effectiveness_metrics[key]['current']})
            ...
            if key not in self.regime_performance[regime_key]:
                self.regime_performance[regime_key][key] = {'values': [],
                    'mean': value, 'std': 0.0}
            ...
            self.regime_performance[regime_key][key]['std'] = np.std(values
                ) if len(values) > 1 else 0.0
        self.effectiveness_history.append({'timestamp': timestamp,
            'metrics': {k: v['current'] for k, v in self.
            effectiveness_metrics.items()}, 'market_regime': market_regime.
            name})
    ...
    def analyze_action_patterns(self) ->Dict[str, Any]:
        ...
        for assessment in self.action_assessments:
            row = {'timestamp': assessment.action.timestamp,
                'optimality_score': assessment.optimality_score,
                'opportunity_cost': assessment.opportunity_cost, 'reward':
                assessment.action.reward}
        ...
        if len(daily_optimality) > 10:
            try:
                slope, _, _, p_value, _ = stats.linregress(range(len(
                    daily_optimality)), daily_optimality)
                if p_value < 0.05:
        ...
        top_positive = {k: v for k, v in sorted(state_correlations.items(),
            key=lambda x: x[1], reverse=True)[:5]}
        top_negative = {k: v for k, v in sorted(state_correlations.items(),
            key=lambda x: x[1])[:5]}
        return {'mean_optimality_score': mean_optimality,
            'mean_opportunity_cost': mean_opportunity_cost, 'mean_reward':
            mean_reward, 'trend': trend, 'top_positive_correlations':
            top_positive, 'top_negative_correlations': top_negative,
            'sample_size': len(df), 'time_range': {'start': df['timestamp']
            .min(), 'end': df['timestamp'].max()}}
    ...
    def analyze_regime_performance(self) ->Dict[str, Any]:
        ...
        for regime, metrics in self.regime_performance.items():
            if not metrics:
                continue
            regime_data = {'sample_size': len(metrics.get('REWARD', {}).get
                ('values', []))}
            for key in ['REWARD', 'PNL', 'SHARPE_RATIO', 'MAX_DRAWDOWN',
                'WIN_RATE']:
                if key in metrics:
                    regime_data[key.lower()] = {'mean': metrics[key]['mean'
                        ], 'std': metrics[key]['std'], 'sample_size': len(
                        metrics[key]['values'])}
            results[regime] = regime_data
        return {'regime_analysis': results, 'best_regime': self.
            _identify_best_regime(), 'worst_regime': self.
            _identify_worst_regime()}
    ...
    def generate_effectiveness_report(self) ->Dict[str, Any]:
        ...
        current_metrics = {k: v['current'] for k, v in self.
            effectiveness_metrics.items()}
        insights = self._generate_insights(action_analysis, regime_analysis,
            decay_analysis, current_metrics)
        return {'model_id': self.model_id, 'model_name': self.model_name,
            'model_version': self.model_version, 'report_generated_at':
            datetime.now(), 'current_metrics': current_metrics,
            'action_analysis': action_analysis, 'regime_analysis':
            regime_analysis, 'effectiveness_decay': decay_analysis,
            'insights': insights, 'sample_sizes': {'actions_recorded': len(
            self.action_history), 'actions_assessed': len(self.
            action_assessments), 'metrics_history': len(self.
            effectiveness_history)}}

    def _generate_insights(self, action_analysis: Dict[str, Any],
        regime_analysis: Dict[str, Any], decay_analysis: Dict[str, float],
        ...
        if 'mean_optimality_score' in action_analysis:
            ...
            if score > 0.8:
                insights.append(
                    f'Model consistently makes near-optimal decisions with a score of {score:.2f}'
                    )
            elif score < 0.5:
                insights.append(
                    f'Model frequently makes suboptimal decisions with a score of {score:.2f}'
                    )
        if 'trend' in action_analysis:
            if action_analysis['trend'] == 'improving':
                insights.append(
                    'Model decision quality is improving over time, indicating successful learning'
                    )
            elif action_analysis['trend'] == 'degrading':
                insights.append(
                    'Model decision quality is degrading over time, suggesting model drift or changing market dynamics'
                    )
        if 'best_regime' in regime_analysis and regime_analysis['best_regime']:
            insights.append(
                f"Model performs best in {regime_analysis['best_regime']} market conditions"
                )
        if 'worst_regime' in regime_analysis and regime_analysis['worst_regime'
            ]:
            insights.append(
                f"Model struggles most in {regime_analysis['worst_regime']} market conditions"
                )
        ...
        for metric, decay_pct in decay_analysis.items():
            if decay_pct < -20:
                if not significant_decay:
                    insights.append(
                        'Model effectiveness shows significant decay in some metrics:'
                        )
                    significant_decay = True
                insights.append(
                    f'  - {metric} has declined by {abs(decay_pct):.1f}%')
        ...
        if 'WIN_RATE' in current_metrics:
            ...
            if win_rate > 0.6:
                insights.append(
                    f'High win rate of {win_rate:.1%}, indicating strong predictive ability'
                    )
            elif win_rate < 0.4:
                insights.append(
                    f'Low win rate of {win_rate:.1%}, suggesting refinement needed in trade entry/exit logic'
                    )
        return insights
    ...
    def plot_effectiveness_trends(self, metric_keys: List[str]=None) ->None:
        ...
        for item in self.effectiveness_history:
            row = {'timestamp': item['timestamp'], 'market_regime': item[
                'market_regime']}
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'model_id': self.model_id, 'model_name': self.model_name,
            'model_version': self.model_version, 'baseline_model_id': self.
            baseline_model_id, 'effectiveness_metrics': self.
            effectiveness_metrics, 'regime_performance': {(regime.name if
            isinstance(regime, MarketRegimeType) else regime): metrics for 
            regime, metrics in self.regime_performance.items()},
            'effectiveness_history': [{'timestamp': item['timestamp'].
            isoformat(), 'metrics': item['metrics'], 'market_regime': item[
            'market_regime']} for item in self.effectiveness_history],
    ...
    def load_from_file(cls, filepath: str) ->'RLToolEffectiveness':
        ...
        with open(filepath, 'r') as file:
            data = json.load(file)
        instance = cls(model_id=data['model_id'], model_name=data[
            'model_name'], model_version=data['model_version'],
        ...
        for item in data.get('effectiveness_history', []):
            instance.effectiveness_history.append({'timestamp': datetime.
                fromisoformat(item['timestamp']), 'metrics': item['metrics'
