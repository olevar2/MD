...
import logging
from common_lib.simulation.interfaces import IMarketRegimeSimulator, IBrokerSimulator, MarketRegimeType
...
class MarketRegimeSimulatorAdapter(IMarketRegimeSimulator):
    ...
    def get_all_regimes(self) ->Dict[str, MarketRegimeType]:
        ...
        if self.simulator:
            try:
                ...
                logger.warning(
                    f'Error getting all regimes from simulator: {str(e)}')
        return self._regime_cache

    @with_exception_handling
    def get_regime_probabilities(self, symbol: str) ->Dict[MarketRegimeType,
        float]:
        """
        ...
        if self.simulator:
            try:
                ...
                logger.warning(
                    f'Error getting regime probabilities from simulator: {str(e)}'
                    )
    ...
    @with_exception_handling
    def get_regime_history(self, symbol: str, lookback_periods: int=10) ->List[
        Dict[str, Any]]:
        """
        ...
        if self.simulator:
            try:
                return self.simulator.get_regime_history(symbol,
                    lookback_periods)
            except Exception as e:
                logger.warning(
                    f'Error getting regime history from simulator: {str(e)}')
        return []
...
class BrokerSimulatorAdapter(IBrokerSimulator):
    ...
    def __init__(self, simulator_instance=None):
        ...
        self._account = {'balance': 100000.0, 'equity': 100000.0,
            'margin_used': 0.0, 'margin_level': 100.0, 'free_margin': 100000.0}
    ...
    def get_current_price(self, symbol: str) ->Dict[str, float]:
        ...
        if symbol not in self._price_cache:
            if symbol == 'EUR/USD':
                self._price_cache[symbol] = {'bid': 1.1, 'ask': 1.1002}
            elif symbol == 'GBP/USD':
                self._price_cache[symbol] = {'bid': 1.3, 'ask': 1.3003}
            elif symbol == 'USD/JPY':
                self._price_cache[symbol] = {'bid': 110.0, 'ask': 110.03}
            else:
                self._price_cache[symbol] = {'bid': 1.0, 'ask': 1.0001}
    ...
    def get_account_summary(self) ->Dict[str, Any]:
        ...
        if self.simulator:
            try:
                ...
                logger.warning(
                    f'Error getting account summary from simulator: {str(e)}')
        return self._account

    @async_with_exception_handling
    async def submit_order(self, symbol: str, order_type: str, direction:
        ...
        if self.simulator:
            try:
                return await self.simulator.submit_order(symbol=symbol,
        ...
        execution_price = current_price['ask'
            ] if direction == 'buy' else current_price['bid']
        position_id = f'pos_{len(self._positions) + 1}'
        self._positions[position_id] = {'symbol': symbol, 'direction':
            direction, 'size': size, 'entry_price': execution_price,
            'current_price': execution_price, 'pnl': 0.0, 'open_time':
            datetime.now().isoformat()}
        return {'success': True, 'order_id': order_id, 'position_id':
            position_id, 'execution_price': execution_price, 'timestamp':
            datetime.now().isoformat()}
    ...
    def get_positions(self) ->Dict[str, Any]:
        ...
        if self.simulator:
            try:
                ...
                logger.warning(
                    f'Error getting positions from simulator: {str(e)}')
