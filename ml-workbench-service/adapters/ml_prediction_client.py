...
class MLPredictionClient:
    ...
    def __init__(self, api_base_url: str='http://localhost:8002', timeout:
        int=5):
        """
        ...
        logger.info(
            f'MLPredictionClient initialized with API at {self.api_base_url}')

    @with_exception_handling
    def get_prediction(self, model_name: str, inputs: Dict[str, Any],
        ...
        try:
            ...
            response = requests.post(endpoint, headers=headers, data=json.
                dumps(payload), timeout=self.timeout)
            request_time = (datetime.now() - start_time).total_seconds() * 1000
            response.raise_for_status()
            result = response.json()
            logger.debug(
                f'Prediction from {model_name} received in {request_time:.2f}ms'
                )
            return result
        except requests.exceptions.Timeout:
            logger.error(
                f'Request to ML prediction API timed out after {self.timeout}s'
                )
            raise Exception(
                f'ML prediction request timed out after {self.timeout}s')
