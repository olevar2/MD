...
from ml_workbench_service.optimization import ModelInferenceOptimizer, HardwareSpecificOptimizer
...
def demonstrate_gpu_optimization():
    ...
    if tf_model is not None:
        ...
        return
    optimizer = HardwareSpecificOptimizer(model_object=model, model_path=
        model_path, framework=framework, output_dir=
        './hardware_optimization_output', model_name='sample_model')
    ...
    if optimizer.available_hardware['gpu']['available']:
        ...
        gpu_results = optimizer.optimize_for_gpu(use_tensorrt=True,
            use_cuda_graphs=True, precision='fp16', sample_input=sample_input)
        logger.info(
            f"GPU optimization completed with {len(gpu_results['optimizations'])} optimizations"
            )
...
def demonstrate_cpu_optimization():
    ...
    cpu_results = optimizer.optimize_for_cpu(precision='fp32', use_mkl=True,
        use_onednn=True, num_threads=4, sample_input=sample_input)
    logger.info(
        f"CPU optimization completed with {len(cpu_results['optimizations'])} optimizations"
        )
