...
class TestFeatureStoreClient(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client = FeatureStoreClient(
            base_url="http://test-feature-store:8001/api/v1",
            use_cache=True
        )
        ...
        self.sample_ohlcv = pd.DataFrame({
            ...
            'open': np.random.rand(10) + 1.0,
            'high': np.random.rand(10) + 1.1,
            'low': np.random.rand(10) + 0.9,
            'close': np.random.rand(10) + 1.0,
            'volume': np.random.randint(1000, 10000, 10)
        })
        
        self.sample_indicators = pd.DataFrame({
            'timestamp': pd.date_range(start='2023-01-01', periods=10, freq='1h'),
            'sma_50': np.random.rand(10) + 1.0,
            'sma_200': np.random.rand(10) + 1.0,
    ...
    def test_init(self):
        ...
        client = FeatureStoreClient(
            base_url="http://custom-url:8001/api/v1",
            api_key="test-key",
            use_cache=True,
            cache_ttl=600
        )
    ...
    async def test_get_ohlcv_data(self, mock_get):
        ...
        mock_response.json.return_value = {
            'data': self.sample_ohlcv.to_dict(orient='records')
        }
    ...
    async def test_get_indicators(self, mock_get):
        ...
        mock_response.status = 200
        mock_response.json.return_value = {
            'data': self.sample_indicators.to_dict(orient='records')
        }
        mock_get.return_value.__aenter__.return_value = mock_response
        ...
        self.assertIn('params', kwargs)
        self.assertEqual(kwargs['params']['symbol'], "EUR/USD")
        self.assertEqual(kwargs['params']['indicators'], "sma_50,sma_200,rsi_14")
    ...
    async def test_compute_feature(self, mock_post):
        ...
        mock_response.json.return_value = {
            'data': [
                {'timestamp': '2023-01-01T00:00:00', 'volatility': 0.01},
                {'timestamp': '2023-01-01T01:00:00', 'volatility': 0.02}
            ]
        }
    ...
    async def test_get_indicator_metadata(self, mock_get):
        ...
        mock_response.json.return_value = {
            ...
            'description': 'Calculates the average of prices over a specified period.',
            'parameters': {
                'period': {'type': 'integer', 'default': 14, 'min': 1, 'max': 500}
            }
        }
        mock_get.return_value.__aenter__.return_value = mock_response
    ...
    async def test_error_handling(self, mock_get):
        ...
        with self.assertRaises(Exception):
            await self.client.get_ohlcv_data(
                symbol="INVALID/PAIR",
                start_date="2023-01-01",
                end_date="2023-01-10",
                timeframe="1h"
    ...
    async def test_caching(self, mock_get):
        ...
        result1 = await self.client.get_ohlcv_data(
            symbol="EUR/USD",
            start_date="2023-01-01",
            end_date="2023-01-10",
            timeframe="1h"
        ...
        result2 = await self.client.get_ohlcv_data(
            symbol="EUR/USD",
            start_date="2023-01-01",
            end_date="2023-01-10",
            timeframe="1h"
        ...
        # Check that the mock was called only once (second call used cache)
        mock_get.assert_called_once()
    
    def test_fallback_data(self):
        """Test fallback data generation."""
        # Test OHLCV fallback
        ohlcv = self.client._get_fallback_ohlcv_data(
            symbol="EUR/USD",
        ...
        indicators = self.client._get_fallback_indicator_data(
            symbol="EUR/USD",
            start_date=datetime.now() - timedelta(days=10),
            end_date=datetime.now(),
            timeframe="1h",
