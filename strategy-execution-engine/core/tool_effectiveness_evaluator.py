...
import json
import os
from common_lib.effectiveness.interfaces import SignalEvent, SignalOutcome, MarketRegimeEnum as MarketRegime, TimeFrameEnum as TimeFrame
...
class BacktestToolEffectivenessEvaluator:
    ...
    def __init__(self, backtest_id: str):
        ...
        self.output_dir = os.path.join('output', 'backtests', backtest_id,
            'effectiveness')
        os.makedirs(self.output_dir, exist_ok=True)

    @with_exception_handling
    def register_signal(self, tool_name: str, signal_type: str, direction:
        str, strength: float, timestamp: datetime, symbol: str, timeframe:
        Union[str, TimeFrame], price: float, market_regime: Union[str,
        MarketRegime]=MarketRegime.UNKNOWN, metadata: Dict[str, Any]=None,
        internal_id: Optional[str]=None) ->str:
        """
        ...
        if isinstance(timeframe, str):
            try:
                ...
                self.logger.warning(
                    f'Invalid timeframe: {timeframe}, using nearest match')
        ...
        if isinstance(market_regime, str):
            try:
                ...
                self.logger.warning(
                    f'Invalid market regime: {market_regime}, using UNKNOWN')
                market_regime = MarketRegime.UNKNOWN
        market_context = {'regime': market_regime, 'backtest_id': self.
            backtest_id}
        signal = SignalEvent(tool_name=tool_name, signal_type=signal_type,
            direction=direction, strength=strength, timestamp=timestamp,
            symbol=symbol, timeframe=timeframe, price_at_signal=price,
            metadata=metadata or {}, market_context=market_context)
        signal_id = self.tracker.register_signal(signal)
        self.pending_signals[signal_id] = {'signal': signal,
            'registered_at': datetime.now(), 'status': 'pending'}
        if internal_id:
            self.signals_map[internal_id] = signal_id
        self.logger.info(
            f'Registered signal from {tool_name}: {signal_id} (direction: {direction})'
            )
        return signal_id

    def register_outcome(self, signal_id: str, outcome: str, exit_price:
        Optional[float]=None, exit_timestamp: Optional[datetime]=None,
        max_favorable_price: Optional[float]=None, max_adverse_price:
        Optional[float]=None, profit_loss: Optional[float]=None, notes: str
        ='', internal_id: Optional[str]=None) ->bool:
        """
        ...
        if internal_id and signal_id is None:
            if internal_id in self.signals_map:
                ...
                self.logger.warning(
                    f'No mapped signal found for internal ID: {internal_id}')
                return False
        if outcome not in ['success', 'failure', 'undetermined']:
            self.logger.warning(
                f'Invalid outcome: {outcome}, must be success, failure, or undetermined'
                )
            outcome = 'undetermined'
        result = self.tracker.register_outcome(signal_id=signal_id, outcome
            =outcome, exit_price=exit_price, exit_timestamp=exit_timestamp,
            max_favorable_price=max_favorable_price, max_adverse_price=
            max_adverse_price, profit_loss=profit_loss, notes=notes)
    ...
    def bulk_evaluate_pending_signals(self, price_data: pd.DataFrame,
        evaluation_params: Dict[str, Any]=None) ->int:
        """
        ...
        success_threshold = params.get('success_threshold_pips', 10
            ) * params.get('pip_value', 0.0001)
        failure_threshold = params.get('failure_threshold_pips', 10
            ) * params.get('pip_value', 0.0001)
        ...
        if missing_cols:
            self.logger.error(
                f'Price data missing required columns: {missing_cols}')
            return 0
        ...
        for signal_id, signal_data in list(self.pending_signals.items()):
            ...
            if signal.direction == 'buy':
                ...
                if max_high >= entry_price + success_threshold:
                    outcome = 'success'
                    exit_price = entry_price + success_threshold
                elif max_low <= entry_price - failure_threshold:
                    outcome = 'failure'
                    exit_price = entry_price - failure_threshold
                else:
                ...
                self.register_outcome(signal_id=signal_id, outcome=outcome,
                    exit_price=exit_price, exit_timestamp=forward_data.iloc
                    [-1]['timestamp'], max_favorable_price=max_high,
                    max_adverse_price=max_low, profit_loss=exit_price -
                    entry_price if outcome != 'undetermined' else 0)
                evaluated_count += 1
                ...
                self.register_outcome(signal_id=signal_id, outcome=outcome,
                    exit_price=exit_price, exit_timestamp=forward_data.iloc
                    [-1]['timestamp'], max_favorable_price=max_low,
                    max_adverse_price=max_high, profit_loss=entry_price -
                    exit_price if outcome != 'undetermined' else 0)
                evaluated_count += 1
        return evaluated_count

    def calculate_tool_metrics(self, tool_name: Optional[str]=None, metrics:
        Optional[List[str]]=None, market_regime: Optional[MarketRegime]=
        None, timeframe: Optional[TimeFrame]=None, symbol: Optional[str]=
        ...
        """
        if tool_name:
            return self.tracker.calculate_metrics(tool_name=tool_name,
                metric_names=metrics, market_regime=market_regime,
                timeframe=timeframe, symbol=symbol, start_date=start_date,
                end_date=end_date)
        else:
            tool_names = self.tracker.get_tool_names()
            results = {}
            for name in tool_names:
                results[name] = self.tracker.calculate_metrics(tool_name=
                    name, metric_names=metrics, market_regime=market_regime,
                    timeframe=timeframe, symbol=symbol, start_date=
    ...
    def save_results(self, filename: Optional[str]=None) ->str:
        ...
        if filename is None:
            filename = (
                f"effectiveness_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                )
    ...
    def generate_effectiveness_report(self, output_format: str='json') ->Union[
        Dict[str, Any], str]:
        """
        ...
        """
        tool_names = self.tracker.get_tool_names()
        report_data = {'summary': {'backtest_id': self.backtest_id,
            'generated_at': datetime.now().isoformat(), 'total_signals':
            len(self.tracker.signals), 'total_outcomes': len(self.tracker.
            outcomes), 'tools_evaluated': len(tool_names)}, 'tools': {}}
        for tool_name in tool_names:
            tool_metrics = self.tracker.calculate_metrics(tool_name)
            report_data['tools'][tool_name] = tool_metrics
            regimes = {}
            for regime in MarketRegime:
                regime_metrics = self.tracker.calculate_metrics(tool_name=
                    tool_name, market_regime=regime)
        ...
        if output_format == 'markdown':
            md_report = [
                f'# Tool Effectiveness Report - Backtest {self.backtest_id}']
            md_report.append(
                f"\nGenerated: {report_data['summary']['generated_at']}")
            md_report.append(
                f"\nTotal Signals: {report_data['summary']['total_signals']}")
            md_report.append(
                f"Total Outcomes: {report_data['summary']['total_outcomes']}")
            md_report.append(
                f"Tools Evaluated: {report_data['summary']['tools_evaluated']}"
                )
            for tool_name, tool_data in report_data['tools'].items():
                md_report.append(f'\n## {tool_name}')
                md_report.append(
                    f"Sample Size: {tool_data.get('sample_size', 0)}")
                md_report.append('\n### Metrics')
                metrics_table = ['| Metric | Value | Sample Size |',
                    '|--------|-------|------------|']
                for metric_name, metric_data in tool_data.get('metrics', {}
                    ).items():
                ...
                if 'by_regime' in tool_data:
                    ...
                    for regime, regime_data in tool_data['by_regime'].items():
                        win_rate = regime_data.get('metrics', {}).get(
                            'win_rate', {}).get('value', 0)
                        if win_rate:
    ...
    def export_report(self, output_format: str='json', filename: Optional[
        str]=None) ->str:
        """
        ...
        if filename is None:
            filename = (
                f"effectiveness_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                )
        if output_format == 'markdown':
