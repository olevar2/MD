...
# Try to import Slack client for alerts
try:
...
class FeatureStoreAlerts:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        default_config = {
            ...
            "thresholds": {
                ...
                "cache_hit_rate": {
                    "warning": 0.5,   # 50% hit rate
                    "error": 0.3,     # 30% hit rate
                    "critical": 0.1   # 10% hit rate
                },
                "fallback_rate": {
                    "warning": 0.05,  # 5% fallback rate
                    "error": 0.10,    # 10% fallback rate
                    "critical": 0.20  # 20% fallback rate
                }
            },
            "alert_cooldown": {
                AlertLevel.INFO: 3600,      # 1 hour
                AlertLevel.WARNING: 1800,   # 30 minutes
                AlertLevel.ERROR: 600,      # 10 minutes
        ...
        if self.alert_config["slack_alerts"] and has_slack:
            try:
                self.slack_client = WebClient(token=self.alert_config["slack_config"]["token"])
                self.logger.info("Slack client initialized")
            except Exception as e:
                self.logger.warning(f"Failed to initialize Slack client: {str(e)}")
    ...
    def check_metrics(self) -> None:
        ...
        try:
            ...
            if total_api_calls > 0:
                ...
                self._check_threshold("error_rate", error_rate, "Error Rate", f"{error_rate:.2%}")
            
            # Check response time
            avg_response_time = metrics["performance"]["avg_response_time_ms"]
            self._check_threshold("response_time", avg_response_time, "Response Time", f"{avg_response_time:.2f}ms")
            
            # Check cache hit rate
            total_cache_requests = metrics["cache"]["hits"] + metrics["cache"]["misses"]
            if total_cache_requests > 0:
                cache_hit_rate = metrics["cache"]["hits"] / total_cache_requests
                self._check_threshold("cache_hit_rate", cache_hit_rate, "Cache Hit Rate", f"{cache_hit_rate:.2%}")
            
            # Check fallback rate
            if total_api_calls > 0:
                fallback_rate = metrics["fallbacks"]["total"] / total_api_calls
                self._check_threshold("fallback_rate", fallback_rate, "Fallback Rate", f"{fallback_rate:.2%}")
        
        except Exception as e:
            self.logger.error(f"Error checking metrics: {str(e)}")
    
    def _check_threshold(self, metric_name: str, value: float, display_name: str, formatted_value: str) -> None:
        """
        ...
        if metric_name in ["error_rate", "fallback_rate"]:
            ...
            if value >= thresholds.get("critical", float('inf')):
                ...
                self.trigger_alert(
                    ...
                    {"metric": metric_name, "value": value, "threshold": thresholds["error"]}
                )
    ...
    def trigger_alert(self, level: str, title: str, message: str, data: Optional[Dict[str, Any]] = None) -> None:
        """
        Trigger an alert.
        ...
        """
        if not self.alert_config["enabled"] or level not in self.alert_config["alert_levels"]:
            return
