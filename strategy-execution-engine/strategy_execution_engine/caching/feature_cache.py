...
# Try to import Redis client for distributed caching
try:
    import redis
    has_redis = True
except ImportError:
...
class FeatureCache:
    ...
    def __init__(
        ...
        self.logger.info(f"Feature cache initialized with max size: {max_size}, TTL: {default_ttl}s")
    ...
    def set(self, key: str, value: Any, ttl: Optional[int] = None) -> None:
        ...
        if self.redis_client:
            try:
                ...
                self.redis_client.setex(f"feature_cache:{key}", ttl, serialized_value)
            except Exception as e:
                self.logger.warning(f"Redis cache error: {str(e)}")
        ...
        with self._lock:
            ...
            if len(self.local_cache) >= self.max_size and key not in self.local_cache:
                self._evict_entries()
    ...
    def _evict_entries(self) -> None:
        ...
        with self._lock:
            ...
            expired_keys = [k for k, v in self.local_cache.items() if current_time > v.expiry]
            ...
            if len(self.local_cache) >= self.max_size:
                ...
                sorted_entries = sorted(self.local_cache.items(), key=lambda x: x[1].expiry)
    ...
    def _start_cleanup_thread(self) -> None:
        ...
        self.logger.debug(f"Started cache cleanup thread (interval: {self.cleanup_interval}s)")
