...
from strategy_execution_engine.error import AnalysisError, DataFetchError
...
@analysis_router.get("/performance/{backtest_id}", response_model=PerformanceAnalysisResponse)
...
async def compare_strategies(strategy_ids: List[str] = Query(..., description="List of strategy IDs to compare")):
    """
    ...
    """
    if not strategy_ids:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No strategy IDs provided")
