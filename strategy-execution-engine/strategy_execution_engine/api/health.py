...
async def detailed_health_check(service_container: ServiceContainer = Depends()):
    ...
    try:
        # Check analysis engine connection
        analysis_engine_status = await check_analysis_engine_connection(service_container)
        services_status["analysis_engine"] = analysis_engine_status

        # Check feature store connection
        feature_store_status = await check_feature_store_connection(service_container)
        services_status["feature_store"] = feature_store_status

        # Check trading gateway connection
        trading_gateway_status = await check_trading_gateway_connection(service_container)
...
async def check_analysis_engine_connection(service_container: ServiceContainer) -> Dict[str, Any]:
    """
    ...
    """
    try:
        # Get client from service container or create a new one
...
async def check_feature_store_connection(service_container: ServiceContainer) -> Dict[str, Any]:
    """
...
async def check_trading_gateway_connection(service_container: ServiceContainer) -> Dict[str, Any]:
    """
...
def check_strategy_loader(service_container: ServiceContainer) -> Dict[str, Any]:
    ...
    """
    try:
        # Get strategy loader from service container
        if service_container.has("strategy_loader"):
            ...
            return {
                "status": "degraded",
                "message": "Strategy loader not initialized"
            }
    except Exception as e:
...
def check_backtester(service_container: ServiceContainer) -> Dict[str, Any]:
    ...
    """
    try:
        # Get backtester from service container
        if service_container.has("backtester"):
            # Check if backtester data directory exists
            ...
            if os.path.exists(backtest_data_dir) and os.path.isdir(backtest_data_dir):
                return {
                    "status": "healthy",
                ...
                return {
                    "status": "degraded",
                    "message": f"Backtester data directory not found: {backtest_data_dir}"
                }
        else:
            return {
                "status": "degraded",
