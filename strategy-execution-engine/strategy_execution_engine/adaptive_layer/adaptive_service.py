...
class AdaptiveLayerService(IAdaptiveStrategyService):
    ...
    def __init__(
        ...
        # Map AdaptationLevel to AdaptationLevelEnum
        self.adaptation_level_map = {
            AdaptationLevel.NONE: AdaptationLevelEnum.NONE,
            AdaptationLevel.CONSERVATIVE: AdaptationLevelEnum.CONSERVATIVE,
            AdaptationLevel.MODERATE: AdaptationLevelEnum.MODERATE,
            AdaptationLevel.AGGRESSIVE: AdaptationLevelEnum.AGGRESSIVE,
            AdaptationLevel.EXPERIMENTAL: AdaptationLevelEnum.EXPERIMENTAL
        }
    ...
    async def get_adaptive_parameters(
        ...
        try:
            ...
            # Get market regime from context or use default
            market_regime = None
            ...
            # Get available tools from context or use default
            available_tools = context.get("available_tools", []) if context else []

            # Get signal weights for available tools
            signal_weights = {}
    ...
    async def get_adaptation_recommendations(
        ...
        try:
            ...
            self.logger.error(f"Error getting adaptation recommendations: {str(e)}")
    ...
    def set_adaptation_level(
        ...
        if isinstance(level, str):
            try:
                # Convert string to AdaptationLevelEnum
                enum_level = AdaptationLevelEnum(level.lower())

                # Map to internal AdaptationLevel
            ...
            # Map AdaptationLevelEnum to internal AdaptationLevel
            for internal_level, enum_value in self.adaptation_level_map.items():
                if enum_value == level:
                    self.adaptation_level = internal_level
                    break
            else:
                self.logger.warning(f"Invalid adaptation level enum, using MODERATE")
                self.adaptation_level = AdaptationLevel.MODERATE

        self.logger.info(f"Adaptation level set to: {self.adaptation_level.name}")
    ...
    def get_adaptation_level(self) -> str:
        ...
        """
        # Map internal AdaptationLevel to AdaptationLevelEnum
        enum_level = self.adaptation_level_map.get(self.adaptation_level, AdaptationLevelEnum.MODERATE)
    ...
    async def run_adaptation_cycle(
        ...
        try:
            ...
            # Run adaptations for each active tool
            results = {}
            for tool_id in active_tools:
                try:
                    ...
                    # Apply constraints to adjustments
                    constrained_adjustments = self._apply_parameter_constraints(
                        tool_id=tool_id,
                    ...
                    result = self._create_adaptation_result(
                        tool_id=tool_id,
                    ...
                    self.logger.error(f"Error adapting tool {tool_id}: {str(e)}")

            return {
                "market_regime": market_regime,
                "timeframe": timeframe,
                "symbol": symbol,
                "adaptations_made": len(results),
                "results": results
            }
        except Exception as e:
    ...
    async def _get_active_tools(self, lookback_hours: int) -> List[str]:
        ...
        try:
            ...
            # Use the adaptive layer service to get active tools
            active_tools = await self.adaptive_layer_service.get_adaptation_recommendations(
    ...
    async def _calculate_expected_impact(
        ...
        # Use the enhanced tracker to get tool effectiveness
        baseline_metrics = self.enhanced_tracker.get_tool_effectiveness(
            tool_id=tool_id,
            start_date=start_date,
            end_date=end_date
        )
