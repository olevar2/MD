...
from strategy_execution_engine.multi_asset.asset_strategy_factory import AssetStrategyFactory, AssetClass
from strategy_execution_engine.integration.analysis_integration_service import AnalysisIntegrationService
...
class MultiAssetStrategyExecutor:
    ...
    def __init__(
        ...
        analysis_service: AnalysisIntegrationService,
        adaptive_layer: AdaptiveLayerService,
    ...
    async def execute_strategy(
        ...
        strategy_key = f"{symbol}_{normalized_asset_class}_{strategy_name or 'default'}"
        if strategy_key not in self.active_strategies:
            strategy = self.strategy_factory.get_strategy_for_asset(
                symbol=symbol,
        ...
        # Use strategy's default timeframes if not specified
        if not timeframes:
            timeframes = strategy.get_default_timeframes()
            
        # Get integrated signals for each timeframe
        timeframe_signals = {}
        timeframe_analysis = {}
        
        for tf in timeframes:
            # Get integrated signals from analysis service
            analysis_results = await self.analysis_service.get_integrated_signals(
                symbol=symbol,
                timeframe=tf,
        ...
        market_regime = timeframe_analysis.get(timeframes[0], {}).get("market_regime", {}).get("regime", "unknown")
        await self._apply_adaptive_parameters(strategy, symbol, market_regime)
        
        # Execute strategy with the signals
        ...
        self.logger.info(f"Strategy {strategy_name or 'default'} for {symbol} ({asset_class}) executed: {strategy_result.get('decision', 'NEUTRAL')}")
        
        # Enhance result with asset-specific information
        enhanced_result = self._enhance_result_with_asset_info(strategy_result, symbol, normalized_asset_class)
        
        # Update adaptive layer with execution feedback
        self._update_adaptive_layer(enhanced_result, symbol, normalized_asset_class, market_regime)
        
        return enhanced_result
    
    async def _apply_adaptive_parameters(
        self,
        ...
        """
        try:
            # Get adaptive parameters for this symbol and regime
            adaptive_params = await self.adaptive_layer.get_adaptive_parameters(
                symbol=symbol,
            ...
            if adaptive_params:
                ...
                self.logger.debug(f"Applied adaptive parameters to {symbol}: {adaptive_params}")
        except Exception as e:
            self.logger.error(f"Error applying adaptive parameters: {str(e)}")
    
    def _enhance_result_with_asset_info(
        self,
        result: Dict[str, Any],
        symbol: str,
        asset_class: str
        ...
        # Add asset-specific risk parameters if order was generated
        if "order" in enhanced and enhanced["order"]:
            strategy = self.active_strategies.get(f"{symbol}_{asset_class}_default")
            if strategy:
                # Get position sizing based on asset class
                position_info = strategy.calculate_position_size(
                    direction=enhanced["order"].get("direction", "NEUTRAL"),
                    risk_percent=self.config.get("default_risk_percent", 1.0),
                    entry_price=enhanced["order"].get("entry_price")
                )
                
                # Update order with asset-specific position information
                enhanced["order"]["position_size"] = position_info.get("position_size")
                enhanced["order"]["risk_amount"] = position_info.get("risk_amount")
                enhanced["order"]["position_value"] = position_info.get("position_value")
                enhanced["order"]["asset_specific"] = position_info.get("asset_specific", {})
                
                # Add explanation
                enhanced["position_explanation"] = position_info.get("explanation", "")
        
        return enhanced
    
    def _update_adaptive_layer(
        self,
        result: Dict[str, Any],
        symbol: str,
        asset_class: str,
        ...
        try:
            ...
            feedback = {
                ...
                "executed": "order" in result and result["order"] is not None
            }
            
            # Send to adaptive layer for learning
            asyncio.create_task(self.adaptive_layer.record_execution_feedback(feedback))
        except Exception as e:
    ...
    async def execute_batch(
        ...
        for symbol, asset_class in symbols.items():
            tasks[symbol] = self.execute_strategy(symbol=symbol, asset_class=asset_class)
        ...
        for symbol, task in tasks.items():
            try:
                ...
                self.logger.error(f"Error executing strategy for {symbol}: {str(e)}")
    ...
    async def get_strategy_performance(
        ...
        """
        strategy_key = f"{symbol}_{asset_class.lower()}_{strategy_name or 'default'}"
        if strategy_key not in self.active_strategies:
