...
class FeedbackCollector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.feedback_storage = self.config.get("feedback_storage", "memory")
        self._feedback_data = []
        self._max_memory_items = self.config.get("max_memory_items", 1000)
        ...
        self.ml_client = self.config.get("ml_client", None)
        
        # Optional adaptive layer integration
        self.adaptive_layer_client = self.config.get("adaptive_layer_client", None)
        
        # Track aggregated metrics by conditions
    ...
    async def collect_execution_feedback(
        ...
        feedback = {
            ...
            "entry_time": position.open_time.isoformat() if position.open_time else None,
            "exit_time": position.close_time.isoformat() if position.close_time else None,
            "pnl": pnl,
            "metadata": position.metadata,
        }
        
        # Extract strategy and signal information from metadata
        if position.metadata:
            ...
            if "signal_confidence" in position.metadata:
                feedback["signal_confidence"] = position.metadata["signal_confidence"]
                
        # Calculate execution metrics
        metrics = await self._calculate_execution_metrics(position, original_position)
        ...
        # If ML client is provided, update the model
        if self.ml_client and hasattr(self.ml_client, "update_with_execution_feedback"):
            try:
                await self.ml_client.update_with_execution_feedback(feedback)
            except Exception as e:
                self.logger.warning(f"Failed to update ML client with feedback: {str(e)}")
        
        # If adaptive layer client is provided, update it
        if self.adaptive_layer_client and hasattr(self.adaptive_layer_client, "update_with_execution_feedback"):
            try:
                await self.adaptive_layer_client.update_with_execution_feedback(feedback)
            except Exception as e:
                self.logger.warning(f"Failed to update adaptive layer with feedback: {str(e)}")
        
        self.logger.debug(f"Collected execution feedback for {position.symbol}: ID={feedback_id}")
        return feedback_id
    ...
    async def get_execution_metrics(
        ...
        """
        if not group_by:
            # Return pre-aggregated metrics based on filter
            return self._get_filtered_aggregated_metrics(filter_params)
        
        # Need to compute custom aggregation
    ...
    async def _calculate_execution_metrics(
        ...
        if position.open_time and position.close_time:
            holding_time_seconds = (position.close_time - position.open_time).total_seconds()
            metrics["holding_time_seconds"] = holding_time_seconds
            
            # Convert to minutes and hours for convenience
            metrics["holding_time_minutes"] = holding_time_seconds / 60
            metrics["holding_time_hours"] = holding_time_seconds / 3600
        
        # Calculate slippage if we have original position
        if original_position and original_position.entry_price and position.entry_price:
            ...
            # Express slippage in pips (assuming 4 decimal places for most forex pairs)
            slippage_pips = slippage * 10000
            metrics["slippage_pips"] = slippage_pips
            
            # Express slippage as percentage of price
            slippage_percentage = (slippage / original_position.entry_price) * 100
            metrics["slippage_percentage"] = slippage_percentage
            
            # Classify slippage as positive or negative (price improvement or deterioration)
            if position.direction.value == "buy":
                # For buy orders, lower execution price is better
                is_positive = position.entry_price < original_position.entry_price
            else:
                # For sell orders, higher execution price is better
                is_positive = position.entry_price > original_position.entry_price
        ...
        # Calculate execution time if available in metadata
        if position.metadata and "execution_time_ms" in position.metadata:
            metrics["execution_time_ms"] = position.metadata["execution_time_ms"]
        
        # Check for partial fills if available in metadata
        if position.metadata and "partial_fills" in position.metadata:
            metrics["partial_fills"] = position.metadata["partial_fills"]
            metrics["partial_fill_count"] = len(position.metadata["partial_fills"])
    ...
    async def _store_feedback(self, feedback: Dict[str, Any]) -> None:
        ...
        if self.feedback_storage == "memory":
            ...
            # If we exceed the maximum, remove oldest entries
            if len(self._feedback_data) > self._max_memory_items:
                self._feedback_data = self._feedback_data[-self._max_memory_items:]
                
        elif self.feedback_storage == "file":
            # Store in a file (simple implementation)
            file_path = self.config.get("feedback_file_path", "execution_feedback.jsonl")
            try:
            ...
            # Store in a database (implementation would depend on database client)
            db_client = self.config.get("db_client", None)
            if db_client and hasattr(db_client, "store_execution_feedback"):
                try:
                    await db_client.store_execution_feedback(feedback)
                except Exception as e:
                    self.logger.error(f"Failed to write feedback to database: {str(e)}")
                    # Fall back to memory storage
                    self._feedback_data.append(feedback)
            else:
                # Fall back to memory storage if no DB client
                self._feedback_data.append(feedback)
    
    async def _get_filtered_feedback_data(
        self,
        filter_params: Optional[Dict[str, Any]] = None,
    ) -> List[Dict[str, Any]]:
        """
        ...
        for feedback in self._feedback_data:
            # Check if feedback matches all filter criteria
            matches = True
            ...
            for key, value in filter_params.items():
                ...
                if "." in key:
                    ...
                    # If we successfully navigated to the final value, compare it
    ...
    async def _update_aggregated_metrics(self, feedback: Dict[str, Any]) -> None:
        """
        ...
        """
        # Extract key dimensions for aggregation
        ...
        market_regime = feedback.get("market_conditions", {}).get("regime", "unknown")
        session = feedback.get("market_conditions", {}).get("session", "unknown")
        
        # Update instrument metrics
        self._update_dimension_metrics(self._aggregated_metrics["instrument"], instrument, feedback)
        
        # Update strategy metrics
        self._update_dimension_metrics(self._aggregated_metrics["strategy"], strategy, feedback)
        
        # Update signal type metrics
        self._update_dimension_metrics(self._aggregated_metrics["signal_type"], signal_type, feedback)
        
        # Update market regime metrics
        self._update_dimension_metrics(self._aggregated_metrics["market_regime"], market_regime, feedback)
        
        # Update session metrics
        self._update_dimension_metrics(self._aggregated_metrics["session"], session, feedback)
    
    def _update_dimension_metrics(
        self,
        ...
        # Update holding time
        holding_time = feedback.get("execution_metrics", {}).get("holding_time_minutes")
        if holding_time is not None:
            # Update running average
            prev_total = metrics["avg_holding_time_minutes"] * (metrics["count"] - 1)
            metrics["avg_holding_time_minutes"] = (prev_total + holding_time) / metrics["count"]
        ...
        if slippage is not None:
            ...
            metrics["avg_slippage_pips"] = (prev_total + slippage) / metrics["count"]
        
        # Update price improvement count
        price_improvement = feedback.get("execution_metrics", {}).get("price_improvement")
        if price_improvement:
            metrics["price_improvement_count"] += 1
    
    def _get_filtered_aggregated_metrics(
        self,
        filter_params: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        """
        ...
        if dimension and dimension in self._aggregated_metrics:
            ...
            # Extract value filter if present
        ...
        # If no specific dimension, return all
        return self._aggregated_metrics
    ...
    def _aggregate_feedback_data(
        ...
        for group_key, group_data in groups.items():
            ...
            if group_metrics["count"] > 0:
                group_metrics["avg_pnl"] = group_metrics["total_pnl"] / group_metrics["count"]
            
            if holding_time_count > 0:
                group_metrics["avg_holding_time_minutes"] = total_holding_time / holding_time_count
            
            if slippage_count > 0:
                group_metrics["avg_slippage_pips"] = total_slippage / slippage_count
                
            # Add derived metrics
            if group_metrics["count"] > 0:
                group_metrics["win_rate"] = group_metrics["win_count"] / group_metrics["count"]
                group_metrics["price_improvement_rate"] = group_metrics["price_improvement_count"] / group_metrics["count"]
            
            # Create nested structure based on group_by fields
            keys = group_key.split(":")
            current_level = result
            
            for i, (field, key) in enumerate(zip(group_by, keys)):
                if i == len(group_by) - 1:
                    # Last level, store metrics
                    if field not in current_level:
                        current_level[field] = {}
                    current_level[field][key] = group_metrics
                else:
                    # Intermediate level
                    field_name = field.split(".")[-1]  # Use last part of nested field
    ...
    async def store_execution_quality_metrics(
        self,
        instrument: str,
        strategy_id: str,
        ...
        # Log the storage of execution quality metrics
        self.logger.info(
            f"Stored execution quality metrics for {instrument} (strategy: {strategy_id}): "
            f"ID={feedback_id}, metrics={list(metrics.keys())}"
        )
