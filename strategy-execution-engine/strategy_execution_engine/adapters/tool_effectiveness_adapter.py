...
import asyncio
import json
from common_lib.effectiveness.interfaces import MarketRegimeEnum as MarketRegime, TimeFrameEnum as TimeFrame, SignalEvent, SignalOutcome, IToolEffectivenessTracker
...
class ToolEffectivenessTrackerAdapter(IToolEffectivenessTracker):
    ...
    def record_signal(self, signal: SignalEvent) ->str:
        ...
        if self.tracker:
            try:
                ...
                self.logger.warning(
                    f'Error recording signal with tracker: {str(e)}')
    ...
    def record_outcome(self, outcome: SignalOutcome) ->bool:
        ...
        if self.tracker:
            try:
                ...
                self.logger.warning(
                    f'Error recording outcome with tracker: {str(e)}')
    ...
    def get_tool_effectiveness(self, tool_id: str, market_regime: Optional[
        ...
        if self.tracker:
            try:
                return self.tracker.get_tool_effectiveness(tool_id,
                    market_regime, timeframe, symbol, start_date, end_date)
            except Exception as e:
                self.logger.warning(
                    f'Error getting tool effectiveness from tracker: {str(e)}')
        cache_key = f'{tool_id}_{market_regime}_{timeframe}_{symbol}'
        if cache_key in self.effectiveness_cache:
            cache_time = self.effectiveness_cache_expiry.get(cache_key,
                datetime.min)
            if datetime.now() < cache_time:
                return self.effectiveness_cache[cache_key]
        try:
            loop = asyncio.get_event_loop()
            effectiveness = loop.run_until_complete(self.
                _get_tool_effectiveness_from_api(tool_id, market_regime,
                timeframe, symbol, start_date, end_date))
            self.effectiveness_cache[cache_key] = effectiveness
            self.effectiveness_cache_expiry[cache_key] = datetime.now(
                ) + timedelta(minutes=self.cache_ttl)
            return effectiveness
        except Exception as e:
            self.logger.error(
                f'Error getting tool effectiveness from API: {str(e)}')
    ...
    def get_best_tools(self, market_regime: str, timeframe: Optional[str]=
        ...
        if self.tracker:
            try:
                return self.tracker.get_best_tools(market_regime, timeframe,
                    symbol, min_signals, top_n)
            except Exception as e:
                self.logger.warning(
                    f'Error getting best tools from tracker: {str(e)}')
        try:
            loop = asyncio.get_event_loop()
            best_tools = loop.run_until_complete(self.
                _get_best_tools_from_api(market_regime, timeframe, symbol,
                min_signals, top_n))
            return best_tools
        except Exception as e:
    ...
    def get_tool_history(self, tool_id: str, limit: Optional[int]=None,
        ...
        if self.tracker:
            try:
                return self.tracker.get_tool_history(tool_id, limit,
                    start_date, end_date)
            except Exception as e:
                self.logger.warning(
                    f'Error getting tool history from tracker: {str(e)}')
        try:
            loop = asyncio.get_event_loop()
            history = loop.run_until_complete(self.
                _get_tool_history_from_api(tool_id, limit, start_date,
                end_date))
            return history
        except Exception as e:
    ...
    def get_performance_summary(self, timeframe: Optional[str]=None, symbol:
        ...
        if self.tracker:
            try:
                return self.tracker.get_performance_summary(timeframe,
                    symbol, start_date, end_date)
            except Exception as e:
                self.logger.warning(
                    f'Error getting performance summary from tracker: {str(e)}'
                    )
        try:
            loop = asyncio.get_event_loop()
            summary = loop.run_until_complete(self.
                _get_performance_summary_from_api(timeframe, symbol,
                start_date, end_date))
            return summary
        except Exception as e:
            self.logger.error(
                f'Error getting performance summary from API: {str(e)}')
            return {'total_tools': 0, 'total_signals': 0,
                'regime_performance': {}, 'tool_rankings': {}}

    @async_with_exception_handling
    async def _send_signal_to_api(self, signal_id: str, signal: SignalEvent
        ) ->bool:
        """Send a signal to the API."""
        try:
            payload = {'signal_id': signal_id, 'tool_id': signal.tool_id,
                'symbol': signal.symbol, 'timestamp': signal.timestamp.
                isoformat(), 'direction': signal.direction, 'timeframe':
                signal.timeframe, 'confidence': signal.confidence,
                'market_regime': signal.market_regime, 'metadata': signal.
                metadata or {}}
    ...
    async def _send_outcome_to_api(self, outcome: SignalOutcome) ->bool:
        ...
        try:
            payload = {'signal_id': outcome.signal_id, 'result': outcome.
                result, 'pips': outcome.pips, 'profit_loss': outcome.
                profit_loss, 'exit_timestamp': outcome.exit_timestamp.
                isoformat(), 'metadata': outcome.metadata or {}}
    ...
    @async_with_exception_handling
    async def _get_tool_effectiveness_from_api(self, tool_id: str,
    ...
    @async_with_exception_handling
    async def _get_best_tools_from_api(self, market_regime: str, timeframe:
    ...
    @async_with_exception_handling
    async def _get_tool_history_from_api(self, tool_id: str, limit:
        Optional[int]=None, start_date: Optional[datetime]=None, end_date:
        ...
        try:
            ...
            if end_date:
                params['end_date'] = end_date.isoformat()
            response = await self.client.get('/tool-effectiveness/history',
                params=params)
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    @async_with_exception_handling
    async def _get_performance_summary_from_api(self, timeframe: Optional[
        str]=None, symbol: Optional[str]=None, start_date: Optional[
        ...
        try:
            ...
            if end_date:
                params['end_date'] = end_date.isoformat()
            response = await self.client.get('/tool-effectiveness/summary',
                params=params)
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    def _calculate_effectiveness_locally(self, tool_id: str, market_regime:
        ...
        signals = [signal for signal_id, signal in self.signals_cache.items
            () if signal.tool_id == tool_id and (market_regime is None or 
            signal.market_regime == market_regime) and (timeframe is None or
            signal.timeframe == timeframe) and (symbol is None or signal.
            symbol == symbol) and (start_date is None or signal.timestamp >=
            start_date) and (end_date is None or signal.timestamp <= end_date)]
        signal_ids = [signal_id for signal_id, signal in self.signals_cache
            .items() if signal in signals]
        outcomes = [outcome for signal_id, outcome in self.outcomes_cache.
            items() if signal_id in signal_ids]
        if not outcomes:
            return {'tool_id': tool_id, 'signal_count': 0, 'win_rate': 0.0,
                'average_pips': 0.0, 'average_profit_loss': 0.0,
                'regime_performance': {}}
        ...
        average_profit_loss = sum(outcome.profit_loss for outcome in outcomes
            ) / len(outcomes)
        regime_performance = {}
        if market_regime:
            regime_signals = [signal for signal in signals if signal.
                market_regime == market_regime]
            regime_signal_ids = [signal_id for signal_id, signal in self.
                signals_cache.items() if signal in regime_signals]
            regime_outcomes = [outcome for signal_id, outcome in self.
                outcomes_cache.items() if signal_id in regime_signal_ids]
            if regime_outcomes:
                regime_win_count = sum(1 for outcome in regime_outcomes if 
                    outcome.result == 'win')
                regime_win_rate = regime_win_count / len(regime_outcomes)
                regime_performance[market_regime] = regime_win_rate
        return {'tool_id': tool_id, 'signal_count': len(outcomes),
            'win_rate': win_rate, 'average_pips': average_pips,
