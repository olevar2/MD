...
class StatisticalValidatorAdapter:
    ...
    def validate_parameter_adjustment(
        ...
        # Fallback implementation if no service is available
        try:
            # Extract metric values
            before_values = [p.get(metric_name, 0) for p in before_performance if metric_name in p]
            after_values = [p.get(metric_name, 0) for p in after_performance if metric_name in p]
            ...
            # Perform t-test
            t_stat, p_value = stats.ttest_ind(after_values, before_values, equal_var=False)
            ...
            return {
                ...
                "pct_change": (after_mean - before_mean) / before_mean if before_mean != 0 else float('inf'),
                "p_value": p_value,
                "t_statistic": t_stat,
                "confidence_level": confidence_level,
                "sample_sizes": {
    ...
    def validate_strategy_performance(
        ...
        try:
            ...
            if baseline_metrics:
                ...
                baseline_profit_factor = baseline_metrics.get("profit_factor", 1.0)
                
                # Calculate p-value for win rate using binomial test
                wins = int(win_rate * trade_count)
                p_value_win_rate = stats.binom_test(wins, trade_count, baseline_win_rate)
                
                # Determine if the strategy is valid
                is_win_rate_valid = (win_rate > baseline_win_rate) and (p_value_win_rate < (1 - confidence_level))
                ...
                return {
                    ...
                    "win_rate": {
                        ...
                        "p_value": p_value_win_rate,
                        "is_valid": is_win_rate_valid
                    },
                    "profit_factor": {
                        "value": profit_factor,
                        "baseline": baseline_profit_factor,
                        "is_valid": is_profit_factor_valid
                    },
                    "trade_count": trade_count,
                    "confidence_level": confidence_level
                }
            else:
                # Without baseline, use standard thresholds
                ...
                return {
                    ...
                    "win_rate": {
                        ...
                        "threshold": 0.5,
                        "p_value": p_value_win_rate,
                        "is_valid": is_win_rate_valid and (p_value_win_rate < (1 - confidence_level))
                    },
                    "profit_factor": {
                        "value": profit_factor,
                        "threshold": 1.0,
    ...
    def validate_signal_quality(
        ...
        if self.service:
            try:
                return self.service.validate_signal_quality(
                    signals=signals,
                    outcomes=outcomes,
        ...
        try:
            ...
            if len(matched_pairs) < min_signals:
                return {
                    "is_valid": False,
                    "reason": f"Insufficient matched signals ({len(matched_pairs)} < {min_signals})",
                    "metrics": {}
                }
            
            # Calculate metrics
            correct_count = sum(1 for s, o in matched_pairs if o.get("is_correct", False))
            accuracy = correct_count / len(matched_pairs)
            
            # Calculate precision and recall for each direction
            buy_signals = [(s, o) for s, o in matched_pairs if s.get("direction") == "buy"]
            sell_signals = [(s, o) for s, o in matched_pairs if s.get("direction") == "sell"]
            
            buy_correct = sum(1 for s, o in buy_signals if o.get("is_correct", False))
            sell_correct = sum(1 for s, o in sell_signals if o.get("is_correct", False))
            ...
            # Determine if the signals are valid
            ...
            return {
                ...
                "metrics": {
                    ...
                    "signal_count": len(matched_pairs),
