...
class FibonacciTradingStrategy(AdvancedTAStrategy):
    ...
    def _init_strategy_config(self) -> None:
        ...
        self.adaptive_params = {
            ...
            "zone_threshold_pips": 10,  # Size of zones around levels
            "swing_lookback": 20,       # Bars to look back for swing points
            "min_swing_pips": 50,       # Minimum size for valid swing
            "confirm_with_candle_pattern": True,  # Require candle pattern confirmation
            "min_volume_increase": 1.5,  # Min volume increase for confirmation
            "wait_for_pullback": True,   # Wait for pullback to level
            "multi_tf_confirmation": True,  # Require confirmation from multiple timeframes
            "atr_multiple_sl": 1.5,     # ATR multiple for stop loss
            "target_fib_level": 1.618,  # Target profit at this extension level
            "use_adaptive_targets": True,  # Adjust targets based on market regime
        }

        # Configuration specific to Fibonacci strategies
        self.config.update({
            "trend_strength_threshold": 0.6,  # Min trend strength to consider
            "use_historical_success_rates": True,  # Use success rates to weight levels
            "preferred_trade_direction": "trend_following",  # or "counter_trend"
            "fibonacci_counting_method": "swing_high_low",  # or "zigzag"
        })

        # Historical success rates for different levels (will be updated through learning)
        self.level_success_rates = {
            "retracement": {
                0.236: 0.65, 0.382: 0.72, 0.5: 0.68, 0.618: 0.78, 0.786: 0.62
            },
            "extension": {
                1.272: 0.67, 1.414: 0.65, 1.618: 0.75, 2.0: 0.63, 2.618: 0.58
            }
        }
        ...
        self.regime_parameters = {
            MarketRegime.TRENDING.value: {
                "preferred_levels": {"retracement": [0.382, 0.5, 0.618], "extension": [1.618, 2.0]},
                "min_swing_pips": 60,
                "target_fib_level": 1.618
            },
            MarketRegime.RANGING.value: {
                "preferred_levels": {"retracement": [0.618, 0.786], "extension": [1.272, 1.414]},
                "min_swing_pips": 30,
                "target_fib_level": 1.272
            },
            MarketRegime.VOLATILE.value: {
                "preferred_levels": {"retracement": [0.5, 0.618], "extension": [1.414, 1.618]},
                "min_swing_pips": 80,
                "target_fib_level": 1.414,
                "zone_threshold_pips": 15
            },
            MarketRegime.BREAKOUT.value: {
                "preferred_levels": {"retracement": [0.236, 0.382], "extension": [1.618, 2.0, 2.618]},
                "min_swing_pips": 40,
                "target_fib_level": 2.0
            }
        }

        # Initialize learning metrics
        self.level_metrics = {level: {"hits": 0, "misses": 0} for level in self.adaptive_params["retracement_levels"]}
        self.level_metrics.update({level: {"hits": 0, "misses": 0} for level in self.adaptive_params["extension_levels"]})
    ...
    def _adapt_parameters_to_regime(self, regime: MarketRegime) -> None:
        ...
        if not params:
            return
        # Filter params that correspond to adaptive settings
        adaptive_updates = {k: v for k, v in params.items() if k in self.adaptive_params}
        self.apply_adaptive_parameters(adaptive_updates)
        self.logger.info(f"Adapted Fibonacci parameters to regime {regime}: {adaptive_updates}")
    ...
    def _perform_strategy_analysis(
        ...
        try:
            analysis_result = {}
            market_regime = additional_data.get("market_regime", MarketRegime.UNKNOWN)

            # Get parameters specific to current market regime
            regime_params = self.regime_parameters.get(
                market_regime.value,
                self.regime_parameters.get(MarketRegime.TRENDING.value)
            )

            # Apply regime-specific parameters if adaptive targets enabled
            if self.adaptive_params["use_adaptive_targets"]:
                preferred_levels = regime_params.get("preferred_levels", {})
                zone_threshold_pips = regime_params.get(
                    "zone_threshold_pips",
                    self.adaptive_params["zone_threshold_pips"]
                )
                target_fib_level = regime_params.get(
                    "target_fib_level",
                    self.adaptive_params["target_fib_level"]
                )
            else:
                preferred_levels = {
                    "retracement": self.adaptive_params["retracement_levels"],
                    "extension": self.adaptive_params["extension_levels"]
                }
                zone_threshold_pips = self.adaptive_params["zone_threshold_pips"]
            ...
            for timeframe, df in price_data.items():
                ...
                swings = self._identify_swing_points(
                    df,
                    lookback=self.adaptive_params["swing_lookback"],
                    min_swing_pips=regime_params.get(
                        "min_swing_pips",
                        self.adaptive_params["min_swing_pips"]
                    )
                )

                # Calculate Fibonacci levels
                fib_levels = self._calculate_fibonacci_levels(df, swings, preferred_levels)

                # Identify potential zones
                fib_zones = self._identify_fibonacci_zones(
                    df,
                    fib_levels,
                    zone_threshold_pips=zone_threshold_pips
                )

                # Check for price interactions with Fibonacci zones
                ...
                if self.adaptive_params["confirm_with_candle_pattern"]:
                    confirmations = self._check_confirmations(df, fib_zones, zone_interactions)
            ...
            analysis_result["fibonacci_confluence"] = self._calculate_fibonacci_confluence(timeframe_analysis)

            # Generate potential trades based on analysis
            potential_trades = self._generate_potential_trades(
                symbol,
                price_data[self.primary_timeframe],
            ...
            self.logger.error(f"Error in Fibonacci strategy analysis: {str(e)}", exc_info=True)
            return {"error": str(e)}
    ...
    def _identify_swing_points(
        ...
        for i in range(lookback, len(df) - lookback):
            ...
            if df['high'].iloc[i] == max(df['high'].iloc[i-lookback:i+lookback+1]):
                ...
                swing_size_pips = (df['high'].iloc[i] - min(left_min, right_min)) * pip_multiplier
        ...
        for i in range(lookback, len(df) - lookback):
            ...
            if df['low'].iloc[i] == min(df['low'].iloc[i-lookback:i+lookback+1]):
                ...
                swing_size_pips = (max(left_max, right_max) - df['low'].iloc[i]) * pip_multiplier
    ...
    def _calculate_fibonacci_levels(
        ...
        preferred_levels: Dict[str, List[float]]
    ) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        # Get the most recent swing points that define a complete move
        ...
        for level in preferred_levels.get("retracement", self.adaptive_params["retracement_levels"]):
            if end_point["price"] > start_point["price"]:  # Uptrend
            ...
            retrace_levels.append({
                ...
                "price": price_level,
                "type": "retracement",
                "direction": "uptrend" if end_point["price"] > start_point["price"] else "downtrend",
                "success_rate": self.level_success_rates["retracement"].get(level, 0.5)
            })

        # Calculate extension levels
        for level in preferred_levels.get("extension", self.adaptive_params["extension_levels"]):
            if end_point["price"] > start_point["price"]:  # Uptrend - extensions continue up
                price_level = end_point["price"] + (price_range * level)
            else:  # Downtrend - extensions continue down
                price_level = end_point["price"] - (price_range * level)
            ...
            extend_levels.append({
                ...
                "type": "extension",
                "direction": "uptrend" if end_point["price"] > start_point["price"] else "downtrend",
                "success_rate": self.level_success_rates["extension"].get(level, 0.5)
            })

        return {"retracement": retrace_levels, "extension": extend_levels}

    def _get_recent_swing_pair(
        self,
        swings: Dict[str, List[Dict[str, Any]]],
        trend: str
        ...
        if trend == "uptrend":
            # In uptrend, look for a low followed by a high
            if not lows:
                return None

            latest_low = max(lows, key=lambda x: x["bar_index"])

            # Find the latest high after this low
            highs_after_low = [h for h in highs if h["bar_index"] > latest_low["bar_index"]]
            ...
            # In downtrend, look for a high followed by a low
            if not highs:
                return None

            latest_high = max(highs, key=lambda x: x["bar_index"])

            # Find the latest low after this high
            lows_after_high = [l for l in lows if l["bar_index"] > latest_high["bar_index"]]
            ...
            # In sideways market, find the most recent significant swing pair
    ...
    def _identify_fibonacci_zones(
        self,
        df: pd.DataFrame,
        fib_levels: Dict[str, List[Dict[str, Any]]],
        zone_threshold_pips: int = 10
    ) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        # Create zones for retracement levels
        for level in fib_levels["retracement"]:
            retracement_zones.append({
                "level": level["level"],
                "price": level["price"],
                "upper": level["price"] + zone_threshold,
                "lower": level["price"] - zone_threshold,
                "type": "retracement",
                "direction": level["direction"],
                "success_rate": level["success_rate"]
            })

        # Create zones for extension levels
        for level in fib_levels["extension"]:
            extension_zones.append({
                "level": level["level"],
                "price": level["price"],
                "upper": level["price"] + zone_threshold,
                "lower": level["price"] - zone_threshold,
    ...
    def _check_zone_interactions(
        self,
        df: pd.DataFrame,
        fib_zones: Dict[str, List[Dict[str, Any]]]
    ) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        # Check for interactions with retracement zones
        for zone in fib_zones["retracement"]:
            for i, row in recent_data.iterrows():
                if row['low'] <= zone["upper"] and row['high'] >= zone["lower"]:
                    # Price has interacted with this zone
                    retracement_interactions.append({
                        "level": zone["level"],
                        "price": zone["price"],
                        "candle_index": i,
                        "interaction_type": "bounce" if (
                            (zone["direction"] == "uptrend" and row['close'] > row['open']) or
                            (zone["direction"] == "downtrend" and row['close'] < row['open'])
                        ) else "rejection",
                        "strength": abs(row['close'] - row['open']) / abs(row['high'] - row['low']),
                        "zone": zone
                    })
                    break  # Count only the first interaction with this zone

        # Check for interactions with extension zones
        for zone in fib_zones["extension"]:
            for i, row in recent_data.iterrows():
                if row['low'] <= zone["upper"] and row['high'] >= zone["lower"]:
                    # Price has interacted with this zone
                    extension_interactions.append({
                        "level": zone["level"],
                        "price": zone["price"],
                        "candle_index": i,
                        "interaction_type": "target_reached",
                        "strength": abs(row['close'] - row['open']) / abs(row['high'] - row['low']),
                        "zone": zone
                    })
                    break  # Count only the first interaction with this zone

        return {"retracement": retracement_interactions, "extension": extension_interactions}

    def _check_confirmations(
        self,
        df: pd.DataFrame,
        fib_zones: Dict[str, List[Dict[str, Any]]],
        zone_interactions: Dict[str, List[Dict[str, Any]]]
    ) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        for interaction in zone_interactions["retracement"]:
            ...
            if 'volume' in df.columns and len(interaction_df) > 1:
                ...
                volume_increase = current_volume >= (avg_volume * self.adaptive_params["min_volume_increase"])

            # Check for candle pattern confirmation
            pattern_confirmation = False
            if zone["direction"] == "uptrend" and interaction["interaction_type"] == "bounce":
                # Look for bullish reversal patterns at uptrend retracement levels
                candle = interaction_df.iloc[-1]
                prev_candle = interaction_df.iloc[-2] if len(interaction_df) > 1 else None

                # Simple bullish engulfing or hammer check
                if prev_candle is not None:
                    ...
                    hammer = (
                        ...
                        (candle['high'] - candle['close']) < 0.3 * (candle['close'] - candle['low']) and
                        (candle['close'] - candle['open']) > 0.6 * (candle['high'] - candle['low'])
                    )
                    pattern_confirmation = bullish_engulfing or hammer

            elif zone["direction"] == "downtrend" and interaction["interaction_type"] == "bounce":
                # Look for bearish reversal patterns at downtrend retracement levels
                candle = interaction_df.iloc[-1]
                prev_candle = interaction_df.iloc[-2] if len(interaction_df) > 1 else None

                # Simple bearish engulfing or shooting star check
                if prev_candle is not None:
                    bearish_engulfing = (
                        candle['open'] > prev_candle['close'] and
                        candle['close'] < prev_candle['open'] and
                    ...
                    shooting_star = (
                        candle['close'] < candle['open'] and
                        (candle['close'] - candle['low']) < 0.3 * (candle['high'] - candle['close']) and
                        (candle['open'] - candle['close']) > 0.6 * (candle['high'] - candle['low'])
                    )
                    pattern_confirmation = bearish_engulfing or shooting_star

            # Add confirmation if we found one
            if volume_increase or pattern_confirmation:
                retracement_confirmations.append({
                    "level": zone["level"],
                    "price": zone["price"],
                    "volume_confirmation": volume_increase,
                    "pattern_confirmation": pattern_confirmation,
                    "strength": (0.7 if volume_increase else 0) + (0.8 if pattern_confirmation else 0),
                    "zone": zone,
    ...
    def _calculate_fibonacci_confluence(
        ...
        # Find confluence in retracement levels
        confluence["retracement_confluence"] = self._find_level_confluence(all_retrace_levels)

        # Find confluence in extension levels
        confluence["extension_confluence"] = self._find_level_confluence(all_extend_levels)

        return confluence

    def _find_level_confluence(
        self,
        ...
        for i in range(1, len(sorted_levels)):
            ...
            if (current_level["lower"] <= prev_level["upper"] and
                current_level["upper"] >= prev_level["lower"]):
                current_group.append(current_level)
            else:
                # If we had more than one level in the group, it's a confluence
                if len(current_group) > 1:
                    avg_price = sum(l["price"] for l in current_group) / len(current_group)
                    avg_success = sum(l["success_rate"] for l in current_group) / len(current_group)
                    ...
                    confluence_groups.append({
                        "price": avg_price,
                        "levels": current_group,
                        "timeframes": timeframes,
                        "num_timeframes": len(timeframes),
                        "strength": len(current_group) * avg_success,
    ...
    def _generate_potential_trades(
        self,
        symbol: str,
        df: pd.DataFrame,
        fibonacci_confluence: Dict[str, List[Dict[str, Any]]],
        target_fib_level: float = 1.618,
        additional_data: Optional[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        # Process retracement confluence points
        for conf in fibonacci_confluence.get("retracement_confluence", []):
            # Skip if not enough timeframes in confluence
            if conf["num_timeframes"] < 2 and self.adaptive_params["multi_tf_confirmation"]:
                continue

            # Check if price is near the confluence point
            ...
            if conf["direction"] == "uptrend" and price_near:
                # Bullish setup at uptrend retracement
                stop_loss = conf["price"] - (atr * self.adaptive_params["atr_multiple_sl"])

                # Look for target in extension levels
                targets = []
                for ext_conf in fibonacci_confluence.get("extension_confluence", []):
                    if ext_conf["direction"] == "uptrend" and ext_conf["price"] > current_price:
                        targets.append(ext_conf["price"])

                if not targets and fibonacci_confluence.get("extension_confluence"):
                    # Use the first extension level as target
                    for ext in fibonacci_confluence.get("extension_confluence", []):
                        if ext["direction"] == "uptrend" and ext["price"] > current_price:
                            targets.append(ext["price"])
                            break

                if not targets:
                    # If no extension confluences, use a default target
                ...
                potential_trades.append({
                    ...
                    "stop_loss": stop_loss,
                    "targets": targets,
                    "risk_reward": (targets[0] - current_price) / (current_price - stop_loss) if targets else 0,
                    "confidence": conf["strength"],
                    "timeframes": conf["timeframes"],
                    "reason": f"Bullish setup at {conf['levels'][0]['level']} retracement with {conf['num_timeframes']} timeframe confluence"
                })

            elif conf["direction"] == "downtrend" and price_near:
                # Bearish setup at downtrend retracement
                stop_loss = conf["price"] + (atr * self.adaptive_params["atr_multiple_sl"])

                # Look for target in extension levels
                targets = []
                for ext_conf in fibonacci_confluence.get("extension_confluence", []):
                    if ext_conf["direction"] == "downtrend" and ext_conf["price"] < current_price:
                        targets.append(ext_conf["price"])

                if not targets and fibonacci_confluence.get("extension_confluence"):
                    # Use the first extension level as target
                    for ext in fibonacci_confluence.get("extension_confluence", []):
                        if ext["direction"] == "downtrend" and ext["price"] < current_price:
                            targets.append(ext["price"])
                            break
                ...
                potential_trades.append({
                    ...
                    "stop_loss": stop_loss,
                    "targets": targets,
                    "risk_reward": (current_price - targets[0]) / (stop_loss - current_price) if targets else 0,
                    "confidence": conf["strength"],
                    "timeframes": conf["timeframes"],
                    "reason": f"Bearish setup at {conf['levels'][0]['level']} retracement with {conf['num_timeframes']} timeframe confluence"
                })
    ...
    def _calculate_combined_score(self, analysis_result: Dict[str, Any]) -> float:
        ...
        # Adjust confidence based on number of timeframes
    ...
    def _generate_signals(
        ...
        # Take the best trade based on confidence
        ...
        signal = {
            ...
            "risk_reward": best_trade["risk_reward"],
            "timeframes": best_trade["timeframes"],
