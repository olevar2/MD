...
from common_lib.signal_flow.interface import (
    ISignalFlowManager,
    ISignalMonitor,
    ISignalValidator,
    ISignalAggregator,
...
class StrategySignalManager:
    ...
    def __init__(
        ...
        signal_validator: ISignalValidator,
        signal_aggregator: ISignalAggregator,
        signal_executor: ISignalExecutor,
    ...
    async def process_new_signal(self, signal: SignalFlow) -> bool:
        ...
        try:
            ...
            if signal.signal_id in self.active_signals:
                self.logger.warning(f"Duplicate signal received: {signal.signal_id}")
                return False
            ...
            # 3. Store signal and update state
            async with self.signal_lock:
                self.active_signals[signal.signal_id] = signal
                self.signal_states[signal.signal_id] = SignalFlowState.VALIDATED
                
            # 4. Get other active signals for the same symbol/timeframe
            related_signals = await self._get_related_signals(signal)
            
            # 5. Aggregate signals if needed
            if related_signals:
                aggregation_result = await self.aggregator.aggregate_signals(
                    signals=[signal] + related_signals,
                    symbol=signal.symbol,
                    timeframe=signal.timeframe
                )
                await self._handle_aggregation_result(signal, aggregation_result)
            
            # 6. Execute signal
            execution_success = await self.executor.execute_signal(signal)
            
            if execution_success:
                await self._update_signal_state(signal.signal_id, SignalFlowState.EXECUTING)
                self.logger.info(f"Successfully executed signal {signal.signal_id}")
            else:
                await self._update_signal_state(signal.signal_id, SignalFlowState.REJECTED)
                self.logger.error(f"Failed to execute signal {signal.signal_id}")
            
            return execution_success
            
        except Exception as e:
            self.logger.error(f"Error processing signal {signal.signal_id}: {str(e)}", exc_info=True)
            await self._update_signal_state(signal.signal_id, SignalFlowState.REJECTED)
            return False
            ...
            async with self.signal_lock:
                ...
                if current_state == SignalFlowState.EXECUTING:
                    ...
                    if await self.executor.cancel_signal(signal_id):
                        await self._update_signal_state(signal_id, SignalFlowState.REJECTED)
                        return True
                else:
                    # Just mark as rejected if not yet executing
                    await self._update_signal_state(signal_id, SignalFlowState.REJECTED)
                    return True
    ...
    async def _handle_invalid_signal(
        self,
        signal: SignalFlow,
        ...
        self.logger.warning(
            f"Invalid signal {signal.signal_id}: "
            f"Failed checks: {[k for k, v in validation_result.validation_checks.items() if not v]}"
        )
    ...
    async def _handle_aggregation_result(
        self,
        signal: SignalFlow,
        aggregation_result: SignalAggregationResult
    ) -> None:
        """Handle signal aggregation results"""
        await self.monitor.update_signal_metrics(
            signal.signal_id,
            {
                ...
                "contributing_signals": aggregation_result.contributing_signals,
    ...
    async def _get_related_signals(self, signal: SignalFlow) -> List[SignalFlow]:
        ...
        return [
            ...
            if (s.symbol == signal.symbol and
                ...
                s.signal_id != signal.signal_id and
                self.signal_states[s.signal_id] in [
                    SignalFlowState.VALIDATED,
                    SignalFlowState.AGGREGATED
                ])
        ]
    ...
    async def _update_signal_state(
        ...
        async with self.signal_lock:
            ...
            await self.flow_manager.update_signal_state(signal_id, state, metadata or {})
