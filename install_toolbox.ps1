# PowerShell script to install @smithery/toolbox for VS Code Insiders
...
# Add the @smithery/toolbox configuration
if ($configContent.mcpServers -eq $null) {
    $configContent.mcpServers = @{}
}

$configContent.mcpServers | Add-Member -MemberType NoteProperty -Name "toolbox" -Value @{
    command = "cmd"
    args = @(
        "/c",
        "npx",
