...
from analysis_engine.services.tool_effectiveness import MarketRegime, PredictionResult, ToolEffectivenessTracker
...
class ToolEffectivenessRepository:
    ...
    def __init__(self, storage_type: str='memory', redis_url: Optional[str]
        =None, mongo_url: Optional[str]=None, cleanup_interval: int=86400):
        """
        ...
        self.logger = logging.getLogger(
            f'{__name__}.ToolEffectivenessRepository')
        self.memory_storage: Dict[str, Any] = {'predictions': {},
    ...
    async def initialize(self):
        ...
        try:
            if self.storage_type == 'redis' and self.redis_url:
                ...
                self.mongo = motor.motor_asyncio.AsyncIOMotorClient(self.
                    mongo_url)
    ...
    @with_analysis_resilience('get_predictions')
    @async_with_exception_handling
    async def get_predictions(self, tool_id: str, market_regime: Optional[
        MarketRegime]=None, start_time: Optional[datetime]=None, end_time:
        Optional[datetime]=None, limit: Optional[int]=None) ->List[
        PredictionResult]:
        """
        ...
        try:
            if self.storage_type == 'memory':
                return await self._get_predictions_memory(tool_id,
                    market_regime, start_time, end_time, limit)
            elif self.storage_type == 'redis':
                return await self._get_predictions_redis(tool_id,
                    market_regime, start_time, end_time, limit)
            elif self.storage_type == 'mongo':
                return await self._get_predictions_mongo(tool_id,
                    market_regime, start_time, end_time, limit)
            return []
        except Exception as e:
    ...
    async def _get_predictions_memory(self, tool_id: str, market_regime:
        Optional[MarketRegime]=None, start_time: Optional[datetime]=None,
        end_time: Optional[datetime]=None, limit: Optional[int]=None) ->List[
        PredictionResult]:
        ...
        for pred_dict in predictions:
            pred = PredictionResult(tool_id=pred_dict['tool_id'], timestamp
                =datetime.fromisoformat(pred_dict['timestamp']),
                market_regime=MarketRegime(pred_dict['market_regime']),
                prediction=pred_dict['prediction'], actual_outcome=
                pred_dict.get('actual_outcome'), confidence=pred_dict.get(
                'confidence', 0.0), impact=pred_dict.get('impact', 0.0),
    ...
    async def _get_predictions_redis(self, tool_id: str, market_regime:
        Optional[MarketRegime]=None, start_time: Optional[datetime]=None,
        end_time: Optional[datetime]=None, limit: Optional[int]=None) ->List[
        PredictionResult]:
    ...
    async def _get_predictions_mongo(self, tool_id: str, market_regime:
        Optional[MarketRegime]=None, start_time: Optional[datetime]=None,
        end_time: Optional[datetime]=None, limit: Optional[int]=None) ->List[
        PredictionResult]:
        ...
        async for doc in cursor:
            pred = PredictionResult(tool_id=doc['tool_id'], timestamp=doc[
                'timestamp'], market_regime=MarketRegime(doc[
                'market_regime']), prediction=doc['prediction'],
                actual_outcome=doc.get('actual_outcome'), confidence=doc.
                get('confidence', 0.0), impact=doc.get('impact', 0.0),
                metadata=doc.get('metadata'))
            predictions.append(pred)
        return predictions

    @async_with_exception_handling
    async def store_effectiveness(self, tool_id: str, market_regime:
        MarketRegime, effectiveness: float):
        """
        ...
        try:
            if self.storage_type == 'memory':
                await self._store_effectiveness_memory(tool_id,
                    market_regime, effectiveness)
            elif self.storage_type == 'redis':
                await self._store_effectiveness_redis(tool_id,
                    market_regime, effectiveness)
            elif self.storage_type == 'mongo':
                await self._store_effectiveness_mongo(tool_id,
                    market_regime, effectiveness)
        except Exception as e:
            self.logger.error(f'Error storing effectiveness: {str(e)}')

    async def _store_effectiveness_memory(self, tool_id: str, market_regime:
        MarketRegime, effectiveness: float):
        ...
        self.memory_storage['effectiveness'][tool_id][market_regime.value] = {
            'score': effectiveness, 'updated_at': datetime.now().isoformat()}

    async def _store_effectiveness_redis(self, tool_id: str, market_regime:
        MarketRegime, effectiveness: float):
        """Store effectiveness in Redis"""
        if not self.redis:
            raise RuntimeError('Redis connection not initialized')
        key = f'effectiveness:{tool_id}:{market_regime.value}'
        value = json.dumps({'score': effectiveness, 'updated_at': datetime.
            now().isoformat()})
        await self.redis.set(key, value)

    async def _store_effectiveness_mongo(self, tool_id: str, market_regime:
        MarketRegime, effectiveness: float):
        """Store effectiveness in MongoDB"""
        if not self.mongo:
            raise RuntimeError('MongoDB connection not initialized')
        collection = self.mongo.analysis_engine.effectiveness
        await collection.update_one({'tool_id': tool_id, 'market_regime':
            market_regime.value}, {'$set': {'score': effectiveness,
            'updated_at': datetime.now()}}, upsert=True)

    @with_resilience('get_effective_tools')
    @async_with_exception_handling
    async def get_effective_tools(self, market_regime: MarketRegime,
        min_score: float=0.5, max_age: Optional[timedelta]=None) ->Dict[str,
        float]:
        """
        ...
        try:
            if self.storage_type == 'memory':
                return await self._get_effective_tools_memory(market_regime,
                    min_score, max_age)
            elif self.storage_type == 'redis':
                return await self._get_effective_tools_redis(market_regime,
                    min_score, max_age)
            elif self.storage_type == 'mongo':
                return await self._get_effective_tools_mongo(market_regime,
                    min_score, max_age)
            return {}
        except Exception as e:
    ...
    async def _get_effective_tools_memory(self, market_regime: MarketRegime,
        min_score: float=0.5, max_age: Optional[timedelta]=None) ->Dict[str,
        float]:
    ...
    async def _get_effective_tools_redis(self, market_regime: MarketRegime,
        min_score: float=0.5, max_age: Optional[timedelta]=None) ->Dict[str,
        float]:
    ...
    async def _get_effective_tools_mongo(self, market_regime: MarketRegime,
        min_score: float=0.5, max_age: Optional[timedelta]=None) ->Dict[str,
        float]:
        ...
        if not self.mongo:
            raise RuntimeError('MongoDB connection not initialized')
        collection = self.mongo.analysis_engine.effectiveness
        query = {'market_regime': market_regime.value, 'score': {'$gte':
            min_score}}
        if max_age:
