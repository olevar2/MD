...
class ValidationReport:
    ...
    def to_dict(self) ->Dict:
        ...
        return {'summary': {'total_endpoints': self.total_endpoints,
            'standardized_endpoints': self.standardized_endpoints,
            'non_standardized_endpoints': self.non_standardized_endpoints,
            'standardization_percentage': self.standardized_endpoints /
            self.total_endpoints * 100 if self.total_endpoints > 0 else 0},
            'endpoints': [{'path': endpoint.path, 'methods': endpoint.
            methods, 'tags': endpoint.tags, 'summary': endpoint.summary,
            'description': endpoint.description, 'module_path': endpoint.
            module_path, 'function_name': endpoint.function_name,
            'is_standardized': endpoint.is_standardized,
            'standardization_issues': endpoint.standardization_issues} for
            endpoint in sorted(self.endpoints, key=lambda e: e.path)]}
    ...
    def print_summary(self) ->None:
        ...
        if self.total_endpoints > 0:
            percentage = (self.standardized_endpoints / self.
                total_endpoints * 100)
...
def is_standardized_path(path: str) ->Tuple[bool, List[str]]:
    ...
    if len(parts) < 4:
        issues.append(
            'Path does not follow the pattern /api/v1/{service}/{domain}/{resource}'
            )
    elif parts[0] != 'api':
    ...
    for part in parts:
        if part and not re.match('^[a-z0-9-]+$', part) and not part.startswith(
            'v'):
            if part.startswith('{') and part.endswith('}'):
                param = part[1:-1]
                if not re.match('^[a-z0-9-]+$', param):
...
def collect_endpoints(app: FastAPI) ->List[EndpointInfo]:
    ...
    for route in app.routes:
        if isinstance(route, APIRoute):
            ...
            endpoint = EndpointInfo(path=path, methods=methods, tags=tags,
                summary=summary, description=description, module_path=
                module_path, function_name=function_name, is_standardized=
                is_standardized, standardization_issues=issues)
...
def validate_api_endpoints() ->ValidationReport:
    ...
    market_regime_endpoints = ['/api/v1/analysis/market-regimes/detect',
    ...
    health_endpoints = ['/api/v1/analysis/health-checks',
        '/api/v1/analysis/health-checks/liveness',
        '/api/v1/analysis/health-checks/readiness']
    legacy_endpoints = ['/api/v1/adaptive-layer/parameters/generate',
        ...
        '/market-regime/detect/', '/market-regime/history/',
        ...
        '/market-regime/underperforming-tools/', '/health', '/health/live',
        '/health/ready']
    for path in (adaptive_layer_endpoints + market_regime_endpoints +
        health_endpoints):
        is_standardized, issues = is_standardized_path(path)
        endpoint = EndpointInfo(path=path, methods=['GET'] if 'history' in
            path or 'insights' in path or 'performance' in path else [
            'POST'], tags=['Adaptive Layer'] if 'adaptations' in path else 
            ['Market Regime'] if 'market-regimes' in path else ['Health'],
            is_standardized=is_standardized, standardization_issues=issues)
        report.add_endpoint(endpoint)
    for path in legacy_endpoints:
        is_standardized, issues = is_standardized_path(path)
        endpoint = EndpointInfo(path=path, methods=['GET'] if 'history' in
            path or 'insights' in path or 'performance' in path else [
            'POST'], tags=['Legacy'], is_standardized=is_standardized,
...
def main():
    ...
    parser.add_argument('--output', '-o', help='Output file path', default=
        'api_standardization_report.json')
