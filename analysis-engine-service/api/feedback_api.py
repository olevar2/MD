...
router = APIRouter(prefix='/api/v1/feedback', tags=['feedback'], responses=
...
class FeedbackSubmissionModel(BaseModel):
    ...
    category: FeedbackCategory = Field(..., description=
...
class ParameterPerformanceModel(BaseModel):
    ...
    strategy_id: str = Field(..., description='Strategy identifier')
    statistical_significance: bool = Field(..., description=
    ...
    effectiveness_score: float = Field(..., description=
...
class StrategyVersionModel(BaseModel):
    ...
    metrics: Dict[str, Any] = Field(default_factory=dict, description=
        'Performance metrics')
    metadata: Dict[str, Any] = Field(default_factory=dict, description=
        'Additional metadata')
...
class FeedbackLearningModel(BaseModel):
    ...
    influential_parameters: List[Dict[str, Any]] = Field(..., description=
        'Most influential parameters')
    success_factors: List[Dict[str, Any]] = Field(..., description=
        'Success factors')
...
@async_with_exception_handling
async def submit_feedback(submission: FeedbackSubmissionModel,
    feedback_collector: TradingFeedbackCollector=Depends(
    get_trading_feedback_collector)) ->Dict[str, Any]:
    """
...
@async_with_exception_handling
async def record_strategy_outcome(outcome: StrategyOutcomeModel,
    feedback_connector: FeedbackLoopConnector=Depends(
    get_feedback_loop_connector)) ->Dict[str, Any]:
    """
    Record the outcome of a strategy execution
    """
    try:
        feedback_data = {'feedback_id': '', 'strategy_id': outcome.
            strategy_id, 'instrument': outcome.instrument, 'timeframe':
            outcome.timeframe, 'source': FeedbackSource.STRATEGY_EXECUTION.
            value, 'category': FeedbackCategory.PERFORMANCE_METRICS.value,
            'outcome_metrics': outcome.outcome_metrics, 'metadata': {
            'market_regime': outcome.market_regime, 'adaptation_id':
            outcome.adaptation_id, 'version_id': outcome.version_id, **
            outcome.metadata}, 'timestamp': datetime.utcnow().isoformat()}
        feedback_id = await feedback_connector.process_execution_feedback(
            feedback_data)
...
@async_with_exception_handling
async def get_feedback_statistics(strategy_id: Optional[str]=None, model_id:
    Optional[str]=None, instrument: Optional[str]=None, start_time:
    Optional[datetime]=None, end_time: Optional[datetime]=None,
    feedback_collector: TradingFeedbackCollector=Depends(
    get_trading_feedback_collector)) ->Dict[str, Any]:
    """
    ...
    """
    try:
        stats = await feedback_collector.get_statistics(strategy_id=
        ...
        raise HTTPException(status_code=status.
            HTTP_500_INTERNAL_SERVER_ERROR, detail=
            f'Failed to retrieve feedback statistics: {str(e)}')


@router.get('/items', response_model=PaginatedResponse)
@async_with_exception_handling
async def get_feedback_items(source: Optional[FeedbackSource]=None,
    category: Optional[FeedbackCategory]=None, strategy_id: Optional[str]=
    None, model_id: Optional[str]=None, instrument: Optional[str]=None,
    status: Optional[FeedbackStatus]=None, start_time: Optional[datetime]=
    None, end_time: Optional[datetime]=None, pagination: PaginationParams=
    Depends(), feedback_collector: TradingFeedbackCollector=Depends(
    get_trading_feedback_collector)) ->Dict[str, Any]:
    """
    Get paginated list of feedback items matching filters
    """
    try:
        items = await feedback_collector.get_feedback_by_filter(source=
            source, category=category, strategy_id=strategy_id, model_id=
            model_id, instrument=instrument, start_time=start_time,
            end_time=end_time, limit=pagination.page_size)
        total_count = await feedback_collector.get_feedback_count(source=
            source, category=category, strategy_id=strategy_id, model_id=
            model_id, instrument=instrument, start_time=start_time,
            end_time=end_time)
        return {'items': items, 'total': total_count, 'page': pagination.
            page, 'page_size': pagination.page_size}
    except Exception as e:
        logger.error(f'Error retrieving feedback items: {str(e)}')
        raise HTTPException(status_code=status.
            HTTP_500_INTERNAL_SERVER_ERROR, detail=
            f'Failed to retrieve feedback items: {str(e)}')


@router.get('/parameters/{strategy_id}')
@async_with_exception_handling
async def get_parameter_performance(strategy_id: str, min_samples: int=
    Query(10, description=
    'Minimum number of samples required for statistics'),
    parameter_tracking: ParameterTrackingService=Depends(
    get_parameter_tracking)) ->Dict[str, Any]:
    """
    ...
    try:
        performance_data = await parameter_tracking.get_parameter_performance(
            strategy_id=strategy_id, min_samples=min_samples)
        parameters = []
        for param in performance_data.get('parameters', []):
            if param.get('sample_size', 0) >= min_samples:
                parameters.append(param)
        return {'strategy_id': strategy_id, 'min_samples': min_samples,
            'parameters': parameters}
    except Exception as e:
        logger.error(f'Error retrieving parameter performance: {str(e)}')
        raise HTTPException(status_code=status.
            HTTP_500_INTERNAL_SERVER_ERROR, detail=
            f'Failed to retrieve parameter performance: {str(e)}')


@router.get('/strategy/versions/{strategy_id}')
@async_with_exception_handling
async def get_strategy_version_history(strategy_id: str, mutation_service:
    StrategyMutationService=Depends(get_strategy_mutation)) ->Dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(status_code=status.
            HTTP_500_INTERNAL_SERVER_ERROR, detail=
            f'Failed to retrieve strategy version history: {str(e)}')


@router.post('/strategy/mutate/{strategy_id}')
@async_with_exception_handling
async def trigger_strategy_mutation(strategy_id: str, force: bool=Query(
    False, description="Force mutation even if conditions aren't optimal"),
    specific_parameters: Optional[List[str]]=Body(None, description=
    'Specific parameters to mutate'), market_regime: Optional[str]=Query(
    None, description='Target market regime'), mutation_service:
    StrategyMutationService=Depends(get_strategy_mutation)) ->Dict[str, Any]:
    """
    Trigger a mutation of a strategy
    """
    try:
        result = await mutation_service.mutate_strategy(strategy_id=
            strategy_id, force=force, specific_parameters=
            specific_parameters, market_regime=market_regime)
        return result
    except Exception as e:
        logger.error(f'Error mutating strategy: {str(e)}')
        raise HTTPException(status_code=status.
            HTTP_500_INTERNAL_SERVER_ERROR, detail=
            f'Failed to mutate strategy: {str(e)}')


@router.get('/strategy/mutation-effectiveness/{strategy_id}')
@async_with_exception_handling
async def get_strategy_mutation_effectiveness(strategy_id: str,
    ...
    """
    try:
        effectiveness = await mutation_service.get_mutation_effectiveness(
            strategy_id)
        ...
        raise HTTPException(status_code=status.
            HTTP_500_INTERNAL_SERVER_ERROR, detail=
            f'Failed to retrieve mutation effectiveness: {str(e)}')


@router.post('/strategy/evaluate/{strategy_id}')
@async_with_exception_handling
async def evaluate_strategy_versions(strategy_id: str, market_regime:
    Optional[str]=Query(None, description='Market regime to evaluate for'),
    mutation_service: StrategyMutationService=Depends(get_strategy_mutation)
    ) ->Dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(status_code=status.
            HTTP_500_INTERNAL_SERVER_ERROR, detail=
            f'Failed to evaluate strategy versions: {str(e)}')


@router.get('/system/status')
@async_with_exception_handling
async def get_feedback_system_status(feedback_loop: FeedbackLoop=Depends(
    get_feedback_loop), feedback_connector: FeedbackLoopConnector=Depends(
    get_feedback_loop_connector), feedback_collector:
    TradingFeedbackCollector=Depends(get_trading_feedback_collector)
    ...
    try:
        ...
        is_healthy = loop_health.get('is_healthy', False
            ) and connector_health.get('is_running', False)
        status = 'healthy' if is_healthy else 'degraded'
        system_status = {'status': status, 'components': {'feedback_loop':
            loop_health, 'feedback_connector': connector_health,
            'feedback_collector': {'total_collected': collector_stats.get(
            'total_collected', 0), 'is_healthy': collector_stats.get(
            'total_collected', 0) > 0}}, 'statistics': {'total_feedback':
            collector_stats.get('total_collected', 0),
            'feedback_count_by_source': collector_stats.get('by_source', {}
            ), 'feedback_count_by_category': collector_stats.get(
            'by_category', {})}, 'last_updated': datetime.utcnow().isoformat()}
...
@async_with_exception_handling
async def get_learning_insights(strategy_id: str, time_window_days: int=
    Query(30, description='Analysis window in days'), feedback_loop:
    FeedbackLoop=Depends(get_feedback_loop)) ->FeedbackLearningModel:
    """
    ...
    try:
        ...
        influential_params = feedback_loop.get_influential_parameters(
            strategy_id)
        success_factors = [insight for insight in insights if insight.get(
            'type') == 'success_factor']
        risk_factors = [insight for insight in insights if insight.get(
            'type') == 'risk_factor']
        return {'strategy_id': strategy_id, 'insights': insights,
            'total_feedback_count': stats.get('total', 0),
            'influential_parameters': influential_params, 'success_factors':
            success_factors, 'risk_factors': risk_factors, 'timestamp':
...
@async_with_exception_handling
async def reset_feedback_statistics(confirm: bool=Query(False, description=
    'Confirmation required'), feedback_collector: TradingFeedbackCollector=
    Depends(get_trading_feedback_collector)) ->Dict[str, str]:
    """
    Reset feedback statistics counters
    """
    if not confirm:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail
            ='Confirmation required to reset statistics')
    try:
