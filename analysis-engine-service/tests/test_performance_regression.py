...
class PerformanceRegressionTests(unittest.TestCase):
    ...
    def set_up_class(cls):
        ...
        cls.test_data = generate_test_data(symbols=['EURUSD', 'GBPUSD',
            'USDJPY', 'AUDUSD'], timeframes=['H1', 'H4', 'D1'], days=30)
    ...
    def _load_baseline(cls) ->Dict[str, Any]:
        ...
        """
        baseline_path = os.path.join(os.path.dirname(os.path.abspath(
    ...
    def _compare_with_baseline(self, component: str, operation: str,
        current: Dict[str, Any], threshold: float=0.1) ->Tuple[bool, Dict[
        str, Any]]:
        """
        ...
        """
        if (not self.baseline or component not in self.baseline or 
            operation not in self.baseline[component]):
        ...
        for metric, value in current.items():
            if metric in baseline:
                ...
                if isinstance(value, (int, float)) and isinstance(
                    ...
                    comparison[metric] = {'current': value, 'baseline':
                        baseline_value, 'diff': value - baseline_value,
                        'diff_pct': diff_pct}
                    if metric.endswith('_time') and diff_pct > threshold:
                        regression_detected = True
                        logger.warning(
                            f'Performance regression detected in {component}.{operation}.{metric}: {diff_pct:.2%}'
                            )
            else:
                comparison[metric] = {'current': value, 'baseline': None,
                    'diff': None, 'diff_pct': None}
        return regression_detected, comparison

    def _update_baseline(self, component: str, operation: str, measurements:
        Dict[str, Any]):
        """
    ...
    def _plot_comparison(self, component: str, operation: str, comparison:
        ...
        """
        time_metrics = {k: v for k, v in comparison.items() if k.endswith(
            '_time') and v['baseline'] is not None}
        if not time_metrics:
            return
        ...
        plt.tight_layout()
        plt.savefig(
            f'performance_results/{component}_{operation}_comparison.png')
        plt.close()
    ...
    def test_optimized_confluence_detector(self):
        ...
        result = detector.detect_confluence_optimized(symbol=symbol,
            price_data=price_data, signal_type=signal_type,
            signal_direction=signal_direction, related_pairs=related_pairs)
        ...
        measurements = {'cold_time': cold_time, 'warm_time': warm_time,
            'memory_usage': memory_usage, 'confirmation_count': result[
            'confirmation_count'], 'contradiction_count': result[
            'contradiction_count'], 'confluence_score': result[
            'confluence_score']}
        regression_detected, comparison = self._compare_with_baseline(component
            ='OptimizedConfluenceDetector', operation=
            'detect_confluence_optimized', current=measurements)
        self._plot_comparison(component='OptimizedConfluenceDetector',
            operation='detect_confluence_optimized', comparison=comparison)
        if os.environ.get('UPDATE_BASELINE') == '1':
            self._update_baseline(component='OptimizedConfluenceDetector',
                operation='detect_confluence_optimized', measurements=
                measurements)
    ...
    def test_ml_confluence_detector(self):
        ...
        measurements = {'cold_time': cold_time, 'warm_time': warm_time,
            ...
            'confirmation_count'], 'contradiction_count': result[
            'contradiction_count'], 'confluence_score': result[
            'confluence_score'], 'pattern_score': result['pattern_score'],
            'prediction_score': result['prediction_score']}
        regression_detected, comparison = self._compare_with_baseline(component
            ='MLConfluenceDetector', operation='detect_confluence_ml',
            current=measurements)
        self._plot_comparison(component='MLConfluenceDetector', operation=
            'detect_confluence_ml', comparison=comparison)
        if os.environ.get('UPDATE_BASELINE') == '1':
            self._update_baseline(component='MLConfluenceDetector',
                operation='detect_confluence_ml', measurements=measurements)
        self.assertFalse(regression_detected, 'Performance regression detected'
            )
