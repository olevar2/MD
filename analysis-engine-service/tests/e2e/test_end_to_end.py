...
class EndToEndTests(unittest.TestCase):
    ...
    async def _test_confluence_detection(self, use_ml=False):
        ...
        # Call API
        if use_ml:
            response = await self.client.detect_confluence_ml(
                symbol=symbol,
                timeframe=timeframe,
            ...
            response = await self.client.detect_confluence(
                symbol=symbol,
                timeframe=timeframe,
                signal_type=signal_type,
                signal_direction=signal_direction
    ...
    async def _test_divergence_analysis(self, use_ml=False):
        ...
        # Call API
        if use_ml:
            response = await self.client.analyze_divergence_ml(
                symbol=symbol,
                timeframe=timeframe
            )
        else:
            response = await self.client.analyze_divergence(
                symbol=symbol,
                timeframe=timeframe
            )
    ...
    async def _test_pattern_recognition(self, use_ml=False):
        ...
        # Call API
        if use_ml:
            response = await self.client.recognize_patterns_ml(
                symbol=symbol,
                timeframe=timeframe
            )
        else:
            response = await self.client.recognize_patterns(
                symbol=symbol,
                timeframe=timeframe
            )
    ...
    async def _test_currency_strength(self):
        ...
        # Call API
        response = await self.client.get_currency_strength(
            timeframe=timeframe,
            method=method
        )
    ...
    async def _test_related_pairs(self):
        ...
        # Call API
        response = await self.client.get_related_pairs(
            symbol=symbol,
    ...
    async def _test_ml_models(self):
        ...
        # If models exist, test getting model info
        if response:
    ...
    async def _run_all_tests(self):
        ...
        return {
            ...
            "ml_confluence": ml_confluence_result,
            "ml_divergence": ml_divergence_result,
            "ml_pattern": ml_pattern_result,
            "currency_strength": currency_strength_result,
            "related_pairs": related_pairs_result,
            "ml_models": ml_models_result
        }
    ...
    def test_end_to_end(self):
        ...
        # Compare optimized vs ML results
        self._compare_results(
            optimized=results["confluence"],
            ml=results["ml_confluence"],
            name="confluence"
        )
        
        self._compare_results(
            optimized=results["divergence"],
            ml=results["ml_divergence"],
            name="divergence"
        )
        
        self._compare_results(
            optimized=results["pattern"],
            ml=results["ml_pattern"],
            name="pattern"
        )
    ...
    def _compare_results(self, optimized, ml, name):
        ...
        if name == "confluence":
            ...
            # For pattern recognition, compare the top pattern score
