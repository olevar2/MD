...
class ServiceContainer:
    ...
    async def initialize(self) ->None:
        ...
        try:
            ...
            for name, service in self._services.items():
                if hasattr(service, 'initialize') and callable(service.
                    initialize):
            ...
            for name, analyzer in self._analyzers.items():
                if hasattr(analyzer, 'initialize') and callable(analyzer.
                    initialize):
            ...
            analysis_service = self.get_service('analysis_service'
                ) if 'analysis_service' in self._services else AnalysisService(
                )
            indicator_service = self.get_service('indicator_service'
                ) if 'indicator_service' in self._services else IndicatorService(
                )
            pattern_service = self.get_service('pattern_service'
                ) if 'pattern_service' in self._services else PatternService()
            ...
            adapter_factory.register_adapter(IAnalysisProvider,
                analysis_adapter)
            adapter_factory.register_adapter(IIndicatorProvider,
                indicator_adapter)
            adapter_factory.register_adapter(IPatternRecognizer,
                pattern_adapter)
            self._logger.info('Adapters registered with adapter factory')
            self._initialized = True
            self._logger.info('Service container initialized')
        except Exception as e:
            self._logger.error(f'Error initializing container: {e}',
                exc_info=True)
            raise ServiceContainerError(
    ...
    def health_check(self) ->HealthCheck:
        ...
        if self._health_check is None:
            ...
            self._health_check = HealthCheck(service_name=
                'analysis-engine-service', version=settings.app_version)
    ...
    def _setup_health_checks(self) ->None:
        ...
        for name, service in self._services.items():
            self._health_check.add_component_check(name, lambda
                service_name=name, service_instance=service:
                check_component_health(service_instance, service_name))
        for name, analyzer in self._analyzers.items():
            self._health_check.add_component_check(name, lambda
                analyzer_name=name, analyzer_instance=analyzer:
                check_component_health(analyzer_instance, analyzer_name))
    ...
    async def cleanup(self) ->None:
        ...
        try:
            ...
            for name, service in self._services.items():
                if name != 'adapter_factory' and hasattr(service, 'cleanup'
                    ) and callable(service.cleanup):
            ...
            self._logger.error(f'Error cleaning up container: {e}',
                exc_info=True)
            raise ServiceContainerError(
                f'Failed to clean up container: {str(e)}')
