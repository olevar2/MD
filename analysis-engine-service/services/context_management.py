...
class ContextManager:
    ...
    def __init__(self, config: Dict[str, Any]=None):
        ...
        self.max_history_turns = self.config_manager.get('max_history_turns', 10)
        self.max_context_age = self.config_manager.get('max_context_age', 3600)
        self.context_store = {}

    @with_resilience('get_context')
    ...
    def _initialize_user_context(self, user_id: str) ->None:
        ...
        Args:
            user_id: User ID
        """
        self.context_store[user_id] = {'user_id': user_id, 'created_at':
            datetime.now().isoformat(), 'last_activity': datetime.now().
            isoformat(), 'conversation_history': deque(maxlen=self.
            max_history_turns), 'current_state': {'intent': None,
            'sub_intent': None, 'entities': [], 'topic': None},
            'entity_memory': {'currency_pair': None, 'timeframe': None,
            'indicator': None, 'price': None, 'amount': None},
            'conversation_flow': None, 'user_preferences': {}}
    ...
    def _update_conversation_history(self, context: Dict[str, Any], message:
        str, response: Dict[str, Any]) ->None:
        """
    ...
    def _update_current_state(self, context: Dict[str, Any], intent: Dict[
        str, Any], entities: List[Dict[str, Any]], response: Dict[str, Any]
        ) ->None:
        """
        ...
        if intent:
            ...
            current_state['sub_intent'] = intent.get('sub_intent', {}).get(
                'intent')
        ...
        if current_state['intent'] == 'trading':
            ...
            for entity in entities:
                if entity['label'] == 'INDICATOR':
                    current_state['topic'
                        ] = f"information_indicator_{entity['value']}"
                    break
                elif entity['label'] == 'TERM':
                    current_state['topic'
                        ] = f"information_term_{entity['value']}"
                    break
            else:
                current_state['topic'] = 'information_general'

    def _update_entity_memory(self, context: Dict[str, Any], entities: List
        [Dict[str, Any]]) ->None:
        """
    ...
    def _update_conversation_flow(self, context: Dict[str, Any], intent:
        Dict[str, Any]) ->None:
        """
        ...
        if previous_intent == 'analysis' and current_state['intent'
            ] == 'trading':
            context['conversation_flow'] = 'analysis_to_trading'
        elif previous_intent == 'information' and current_state['intent'
            ] == 'chart':
            context['conversation_flow'] = 'information_to_chart'
        elif previous_intent == 'chart' and current_state['intent'
            ] == 'analysis':
    ...
    def get_conversation_summary(self, user_id: str) ->Dict[str, Any]:
        ...
        """
        context = self.get_context(user_id)
        summary = {'turn_count': len(context['conversation_history']),
            'current_topic': context['current_state']['topic'],
            'current_intent': context['current_state']['intent'],
            'current_currency_pair': context['entity_memory'][
            'currency_pair'], 'current_timeframe': context['entity_memory']
            ['timeframe'], 'conversation_flow': context['conversation_flow'
