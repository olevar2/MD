...
router = APIRouter(prefix='/v1/analysis/health-checks', tags=['Health'])
...
@router.get('', response_model=ServiceHealth, summary=
    ...
    'Get detailed health status of the service including all components and dependencies.'
    )
@async_with_exception_handling
...
@router.get('/liveness', summary='Liveness probe', description=
    'Kubernetes liveness probe endpoint to determine if the service is alive.')
@async_with_exception_handling
...
@router.get('/readiness', summary='Readiness probe', description=
    'Kubernetes readiness probe endpoint to determine if the service is ready to receive traffic.'
    )
@async_with_exception_handling
    ...
    try:
        ...
        if health_status.status == HealthStatus.UNHEALTHY:
            ...
            return {'status': 'not ready', 'reason': 'Service is unhealthy',
                'details': {'components': [{'name': component.name,
                'status': component.status, 'message': component.message} for
                component in health_status.components if component.status ==
                HealthStatus.UNHEALTHY], 'dependencies': [{'name':
                dependency.name, 'status': dependency.status, 'message':
                dependency.message} for dependency in health_status.
                dependencies if dependency.status == HealthStatus.UNHEALTHY]}}
        return {'status': 'ready'}
    except Exception as e:
...
async def legacy_health_check(health_check: HealthCheck=Depends(
    get_health_check)) ->ServiceHealth:
    """
    ...
    """
    logger.info(
        'Legacy health check endpoint called - consider migrating to /v1/analysis/health-checks'
        )
    return await health_check.check_health()
...
async def legacy_liveness_probe() ->Dict[str, str]:
    ...
    """
    logger.info(
        'Legacy liveness probe endpoint called - consider migrating to /v1/analysis/health-checks/liveness'
        )
    return {'status': 'alive'}


@legacy_router.get('/health/ready')
async def legacy_readiness_probe(response: Response, health_check:
    HealthCheck=Depends(get_health_check)) ->Dict[str, Any]:
    """
    ...
    """
    logger.info(
        'Legacy readiness probe endpoint called - consider migrating to /v1/analysis/health-checks/readiness'
        )
    health_status = await health_check.check_health()
    if health_status.status == HealthStatus.UNHEALTHY:
        response.status_code = status.HTTP_503_SERVICE_UNAVAILABLE
        return {'status': 'not ready', 'reason': 'Service is unhealthy',
            'details': {'components': [{'name': component.name, 'status':
            component.status, 'message': component.message} for component in
            health_status.components if component.status == HealthStatus.
            UNHEALTHY], 'dependencies': [{'name': dependency.name, 'status':
            dependency.status, 'message': dependency.message} for
            dependency in health_status.dependencies if dependency.status ==
...
def setup_health_routes(app, service_container: ServiceContainer) ->None:
    ...
    app.dependency_overrides[get_health_check
        ] = lambda : service_container.health_check
