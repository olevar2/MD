...
class MarketRegimeDetector:
    ...
    async def get_price_data(self, instrument: str, timeframe: str,
        ...
        Returns:
            List[Any]: Price data
        """
        return self.price_repository.get_prices(instrument=instrument,
    ...
    def extract_features(self, prices: List[Any]) ->Dict[str, float]:
        ...
        sma50 = np.mean(close_prices[-50:]) if len(close_prices
            ) >= 50 else sma20
        avg_range = np.mean([(p.high - p.low) for p in prices])
        avg_price = np.mean(close_prices)
        volatility_ratio = avg_range / avg_price * 100
        price_change = (close_prices[-1] / close_prices[0] - 1) * 100
        ma_diff = (sma20 / sma50 - 1) * 100
        return {'atr': atr, 'adx': adx, 'rsi': rsi, 'sma20': sma20, 'sma50':
            sma50, 'volatility_ratio': volatility_ratio, 'price_change':
            price_change, 'ma_diff': ma_diff, 'close': close_prices[-1]}
