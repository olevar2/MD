...
class ToolEffectivenessDashboardGenerator:
    ...
    def generate_overview_data(self) ->Dict[str, Any]:
        ...
        if not tools:
            ...
            return {'tools_count': 0, 'signals_count': 0, 'outcomes_count':
        ...
        for tool in tools:
            ...
            for signal in tool_signals:
                signal_outcomes = self.repository.get_outcomes_for_signal(
                    signal.signal_id)
                outcomes_count += len(signal_outcomes)
                success_count += sum(1 for o in signal_outcomes if o.success)
        overall_success_rate = (success_count / outcomes_count * 100 if 
            outcomes_count > 0 else 0)
        top_tools = []
        for tool in tools:
            win_rate, signal_count, outcome_count = (self.repository.
                get_tool_win_rate(tool.tool_id))
            if outcome_count >= 10:
                top_tools.append({'tool_id': tool.tool_id, 'name': tool.
                    name, 'win_rate': win_rate, 'signal_count':
                    signal_count, 'outcome_count': outcome_count})
        top_tools = sorted(top_tools, key=lambda x: x['win_rate'], reverse=True
            )[:5]
        return {'tools_count': len(tools), 'signals_count': signals_count,
            'outcomes_count': outcomes_count, 'overall_success_rate':
            overall_success_rate, 'top_performing_tools': top_tools,
            'timestamp': datetime.now().isoformat()}

    def generate_tool_comparison_data(self, tool_ids: Optional[List[str]]=
        None, timeframe: Optional[str]=None, instrument: Optional[str]=None,
        ...
        if not tools:
            ...
            return {'tools': [], 'metrics': [], 'timestamp': datetime.now()
                .isoformat()}
        comparison_data = []
        for tool in tools:
            win_rate, signal_count, outcome_count = (self.repository.
                get_tool_win_rate(tool_id=tool.tool_id, timeframe=timeframe,
                instrument=instrument, from_date=from_date, to_date=to_date))
            if outcome_count < 5:
                continue
            latest_metrics = self.repository.get_latest_tool_metrics(tool.
                tool_id)
            tool_data = {'tool_id': tool.tool_id, 'name': tool.name,
                'description': tool.description, 'win_rate': win_rate,
                'profit_factor': latest_metrics.get('profit_factor'),
                'expected_payoff': latest_metrics.get('expected_payoff'),
                'signal_count': signal_count, 'outcome_count': outcome_count}
            comparison_data.append(tool_data)
        comparison_data = sorted(comparison_data, key=lambda x: x[
            'win_rate'], reverse=True)
        return {'tools': comparison_data, 'metrics': ['win_rate',
            'profit_factor', 'expected_payoff'], 'timestamp': datetime.now(
            ).isoformat()}

    def generate_regime_analysis_data(self, tool_id: str, timeframe:
        ...
        if not tool:
            ...
            return {'tool_id': tool_id, 'regimes': {}, 'timestamp':
                datetime.now().isoformat()}
        performance_matrix = (self.market_regime_service.
            get_regime_performance_matrix(tool_id=tool_id, timeframe=
            timeframe, instrument=instrument, from_date=from_date, to_date=
            to_date))
        optimal_conditions = (self.market_regime_service.
            find_optimal_market_conditions(tool_id=tool_id, timeframe=
            timeframe, instrument=instrument, from_date=from_date, to_date=
            to_date))
        regime_data = []
        for regime, metrics in performance_matrix.get('regimes', {}).items():
            regime_data.append({'regime': regime, 'win_rate': metrics.get(
                'win_rate', 0), 'profit_factor': metrics.get(
                'profit_factor'), 'expected_payoff': metrics.get(
                'expected_payoff', 0), 'sample_size': metrics.get(
                'sample_size', 0)})
        regime_data = sorted(regime_data, key=lambda x: x['win_rate'],
            reverse=True)
        return {'tool_id': tool_id, 'tool_name': tool.name,
            'tool_description': tool.description, 'regime_data':
            regime_data, 'is_regime_specific': optimal_conditions.get(
            'is_regime_specific', False), 'best_regime': optimal_conditions
            .get('best_regime', {}), 'worst_regime': optimal_conditions.get
            ('worst_regime', {}), 'regime_variance': optimal_conditions.get
            ('regime_variance', 0), 'timestamp': datetime.now().isoformat()}

    def generate_performance_trends(self, tool_id: str, period_days: int=90,
        ...
        if not tool:
            self.logger.warning(f'Tool {tool_id} not found')
            return {'tool_id': tool_id, 'trends': [], 'timestamp': datetime
                .now().isoformat()}
        end_date = datetime.now()
        start_date = end_date - timedelta(days=period_days)
        metrics = self.repository.get_effectiveness_metrics(tool_id=tool_id,
        ...
        while current_date <= end_date:
            interval_end = min(current_date + timedelta(days=interval_days),
                end_date)
            interval_signals = self.repository.get_signals(tool_id=tool_id,
                from_date=current_date, to_date=interval_end)
            ...
            win_rate = success_count / len(outcomes
                ) * 100 if outcomes else None
            trend_data.append({'interval_start': current_date.isoformat(),
                'interval_end': interval_end.isoformat(), 'win_rate':
                win_rate, 'signal_count': len(interval_signals),
                'outcome_count': len(outcomes)})
            current_date = interval_end + timedelta(days=1)
        return {'tool_id': tool_id, 'tool_name': tool.name, 'period_days':
            period_days, 'interval_days': interval_days, 'trends':
            trend_data, 'timestamp': datetime.now().isoformat()}

    def generate_dashboard_data(self, timeframe: Optional[str]=None,
        ...
        comparison_data = self.generate_tool_comparison_data(timeframe=
            timeframe, instrument=instrument, from_date=from_date, to_date=
            to_date)
        tools = self.repository.get_tools()
        regime_analyses = []
        performance_trends = []
        for tool in tools:
            _, signal_count, outcome_count = self.repository.get_tool_win_rate(
                tool.tool_id)
            if outcome_count >= 10:
                regime_analysis = self.generate_regime_analysis_data(tool_id
                    =tool.tool_id, timeframe=timeframe, instrument=
        ...
        complementarity = (self.market_regime_service.
            compute_tool_complementarity(tool_ids=tool_ids, timeframe=
            timeframe, instrument=instrument, from_date=from_date, to_date=
            to_date))
        timeframes = set()
        instruments = set()
        ...
        dashboard_data = {'generated_at': datetime.now().isoformat(),
            'overview': overview_data, 'tool_comparison': comparison_data,
            'regime_analyses': regime_analyses, 'performance_trends':
            performance_trends, 'tool_complementarity': complementarity,
            'filters': {'timeframes': list(timeframes), 'instruments': list
            (instruments), 'regimes': list(regimes)}}
