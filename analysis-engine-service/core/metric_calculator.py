...
class MetricBatchCalculator:
    ...
    @with_exception_handling
    def recalculate_all_metrics(self, days_back: int=90, tool_ids: Optional
        ...
        if not tools:
            return {'status': 'completed', 'message':
                'No tools found for processing', 'duration_seconds': time.
                time() - start_time, 'tools_processed': 0}
        results = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers
            ) as executor:
            future_to_tool = {executor.submit(self.
                _recalculate_tool_metrics, tool.tool_id, days_back): tool.
                tool_id for tool in tools}
            for future in concurrent.futures.as_completed(future_to_tool):
                ...
                try:
                    ...
                    results.append({'tool_id': tool_id, 'status': 'error',
                        'error': str(e)})
        return {'status': 'completed', 'duration_seconds': time.time() -
            start_time, 'tools_processed': len(tools), 'results': results}

    @with_exception_handling
    def _recalculate_tool_metrics(self, tool_id: str, days_back: int) ->Dict[
        str, Any]:
        ...
        if not tool:
            return {'tool_id': tool_id, 'status': 'error', 'error':
                'Tool not found'}
        signals = self.repository.get_signals(tool_id=tool_id, from_date=
            from_date, limit=10000)
        if not signals:
            return {'tool_id': tool_id, 'status': 'completed', 'message':
                'No signals found for processing', 'signals_processed': 0,
        ...
        return {'tool_id': tool_id, 'tool_name': tool.name, 'status':
            'completed', 'signals_processed': processed_count,
            'duration_seconds': time.time() - start_time}

    @with_exception_handling
    def generate_periodic_reports(self, report_type: str='monthly') ->Dict[
        str, Any]:
        ...
        if not tools:
            return {'status': 'completed', 'message':
                'No tools found for processing', 'duration_seconds': time.
                time() - start_time, 'reports_generated': 0}
        results = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers
            ) as executor:
            future_to_tool = {executor.submit(self._generate_tool_report,
                tool.tool_id, report_type): tool.tool_id for tool in tools}
            for future in concurrent.futures.as_completed(future_to_tool):
                tool_id = future_to_tool[future]
                try:
                    result = future.result()
                    ...
                    logger.error(
                        f'Error generating report for tool {tool_id}: {e}')
                    results.append({'tool_id': tool_id, 'status': 'error',
                        'error': str(e)})
        return {'status': 'completed', 'duration_seconds': time.time() -
            start_time, 'reports_generated': len(results), 'results': results}

    def _generate_tool_report(self, tool_id: str, report_type: str) ->Dict[
        str, Any]:
        ...
        report_data = {'report_id': str(tool_id) + '_' + report_type + '_' +
            now.strftime('%Y%m%d'), 'tool_id': tool_id, 'tool_name': tool.
            name, 'report_type': report_type, 'period': period,
            'generated_at': now, 'period_start': from_date, 'period_end':
            now, 'summary': {'total_signals': total_signals,
            'total_outcomes': total_outcomes, 'win_rate': win_rate,
            'total_pnl': total_pnl}, 'metrics': metrics_dict}
        ...
        return {'tool_id': tool_id, 'tool_name': tool.name, 'report_id':
            report_data['report_id'], 'report_type': report_type, 'status':
            'completed', 'duration_seconds': time.time() - start_time}
