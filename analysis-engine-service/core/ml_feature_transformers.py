...
class FeatureTransformerRegistry:
    ...
    def _register_default_transformers(self):
        ...
        self.register('standard_scaler', TransformerType.SCALER, lambda
            params: StandardScaler(**params))
        self.register('minmax_scaler', TransformerType.SCALER, lambda
            params: MinMaxScaler(**params))
        self.register('robust_scaler', TransformerType.SCALER, lambda
            params: RobustScaler(**params))
        self.register('select_k_best', TransformerType.SELECTOR, lambda
            params: SelectKBest(score_func=params.pop('score_func',
            f_regression), **params))
        self.register('pca', TransformerType.DECOMPOSER, lambda params: PCA
            (**params))
        self.register('lag_features', TransformerType.TIME_SERIES, lambda
            params: LagFeatureTransformer(**params))
        self.register('rolling_features', TransformerType.TIME_SERIES, lambda
            params: RollingFeatureTransformer(**params))
        self.register('diff_features', TransformerType.TIME_SERIES, lambda
            params: DiffFeatureTransformer(**params))

    def register(self, name: str, transformer_type: TransformerType,
        ...
        """
        self._transformers[name] = {'type': transformer_type, 'factory':
            factory_func}
        logger.debug(
            f'Registered transformer: {name} ({transformer_type.value})')

    @with_resilience('get_transformer')
    ...
    def list_transformers(self, transformer_type: TransformerType=None) ->List[
        str]:
        """
        ...
        return [name for name, info in self._transformers.items() if info[
    ...
    def build_pipeline(self, configs: List[TransformerConfig]) ->Pipeline:
        ...
        for i, config in enumerate(configs):
            try:
                ...
                logger.error(
                    f'Error creating transformer {config.name}: {str(e)}')
...
class LagFeatureTransformer:
    ...
    def transform(self, X):
        ...
        if X_df is None:
            logging.error(
                'Failed to ensure DataFrame in LagFeatureTransformer.transform'
                )
            return None
...
class RollingFeatureTransformer:
    ...
    def __init__(self, windows: List[int]=None, functions: List[str]=None,
        min_periods: int=1):
        """
    ...
    def transform(self, X):
        ...
        if X_df is None:
            logging.error(
                'Failed to ensure DataFrame in RollingFeatureTransformer.transform'
                )
            return None
        for window in self.windows:
            for func in self.functions:
                for col in X_df.columns:
                    rolling = X_df[col].rolling(window=window, min_periods=
                        self.min_periods)
                    if func == 'mean':
                        ...
                        X_df[f'{col}_rolling_{window}_{func}'
                            ] = rolling.median()
                    elif func == 'sum':
...
class DiffFeatureTransformer:
    ...
    def transform(self, X):
        ...
        if X_df is None:
            logging.error(
                'Failed to ensure DataFrame in DiffFeatureTransformer.transform'
                )
            return None
        for period in self.periods:
            for col in X_df.columns:
                X_df[f'{col}_diff_{period}'] = X_df[col].diff(period)
                if not (X_df[col] == 0).any():
...
class FeatureSelector:
    ...
    @with_analysis_resilience('calculate_feature_importance')
    def calculate_feature_importance(self, X: pd.DataFrame, y: pd.Series,
        ...
        feature_names = X.columns if hasattr(X, 'columns') else [f'feature_{i}'
             for i in range(X.shape[1])]
        self.feature_importance = {name: score for name, score in zip(
            feature_names, scores)}
        return dict(sorted(self.feature_importance.items(), key=lambda x: x
            [1], reverse=True))
    ...
    def select_k_best(self, X: pd.DataFrame, y: pd.Series, k: int=10
        ...
        self.selected_features = [name for name, selected in zip(
            feature_names, mask) if selected]
        X_df = ensure_dataframe(X, copy=False)
        if X_df is not None:
            return X_df[self.selected_features]
        else:
            logging.warning(
                'Input to select_k_best was not a DataFrame, returning NumPy array subset.'
                )
            return X_new

    def select_by_threshold(self, X: pd.DataFrame, y: pd.Series, threshold:
        float=0.01, method: str='mutual_info') ->pd.DataFrame:
        """
        ...
        self.selected_features = [name for name, score in importance.items(
            ) if score >= threshold]
        X_df = ensure_dataframe(X, copy=False)
        if X_df is not None:
            missing_cols = [col for col in self.selected_features if col not in
                X_df.columns]
            if missing_cols:
                logging.error(
                    f'Missing selected features in DataFrame: {missing_cols}')
                available_features = [col for col in self.selected_features if
                    col in X_df.columns]
                return X_df[available_features]
            return X_df[self.selected_features]
        elif isinstance(X, np.ndarray):
            logging.warning(
                'Input to select_by_threshold was a NumPy array. Index-based selection requires feature names mapping.'
                )
            return X
        else:
            logging.error(
                f'Unsupported input type {type(X)} for select_by_threshold.')
            return X

    def pca_transform(self, X: pd.DataFrame, n_components: int=None,
        variance: float=0.95) ->pd.DataFrame:
        """
...
class TimeSeriesFeatureGenerator:
    ...
    def add_lag_features(self, lags: List[int]=None
        ) ->'TimeSeriesFeatureGenerator':
        """Add lag feature transformer"""
        self.transformers.append(LagFeatureTransformer(lags=lags))
        return self

    def add_rolling_features(self, windows: List[int]=None, functions: List
        [str]=None) ->'TimeSeriesFeatureGenerator':
        """Add rolling window feature transformer"""
        self.transformers.append(RollingFeatureTransformer(windows=windows,
            functions=functions))
        return self

    def add_diff_features(self, periods: List[int]=None
        ) ->'TimeSeriesFeatureGenerator':
...
class TechnicalIndicatorMLFeatureExtractor:
    ...
    def __init__(self, registry=None):
        ...
        self.base_indicators = ['RSI', 'MACD', 'BollingerBands', 'ATR',
            'OBV', 'CCI', 'StochasticOscillator', 'ADX', 'EMA', 'SMA',
            'WilliamsR']

    @with_exception_handling
    def extract_features(self, data: pd.DataFrame, indicator_names: List[
        str]=None, include_price_features: bool=True, **params) ->pd.DataFrame:
        """
        ...
        if include_price_features:
            if 'close' in data.columns:
                ...
                features['price_log_return_1d'] = np.log(data['close'] /
                    data['close'].shift(1))
                if 'high' in data.columns and 'low' in data.columns:
                    features['daily_volatility'] = (data['high'] - data['low']
                        ) / data['close']
                    features['hl_to_close'] = (data['high'] - data['low']) / (
                        data['close'] - data['low'])
            if 'volume' in data.columns:
                features['volume_change'] = data['volume'].pct_change()
                features['volume_ma_ratio'] = data['volume'] / data['volume'
                    ].rolling(20).mean()
        ...
        for name in indicators_to_use:
            try:
                ...
                result = self.registry.calculate_indicator(name, data, **
                    indicator_params)
    ...
    def extract_and_transform(self, data: pd.DataFrame, indicator_names:
        List[str]=None, include_ts_features: bool=True, **params
        ) ->pd.DataFrame:
        """
