...
from analysis_engine.services.tool_effectiveness import SignalEvent, SignalOutcome, MarketRegime, TimeFrame, WinRateMetric, ProfitFactorMetric
...
class EnhancedToolEffectivenessTracker:
    ...
    def register_signal(self, signal_event: SignalEvent) ->str:
        ...
        try:
            ...
            self.logger.info(
                f"Registered signal {signal_id} from tool '{signal_event.tool_name}' for {signal_event.symbol}"
                )
            return signal_id
    ...
    def register_outcome(self, signal_outcome: SignalOutcome) ->None:
        ...
        try:
            ...
            if signal_id is None:
                self.logger.warning(
                    'Signal outcome has no associated signal ID')
                return
            self.logger.info(
                f'Registered outcome for signal {signal_id}: {signal_outcome.outcome} with profit/loss {signal_outcome.profit_loss}'
                )
        except Exception as e:
    ...
    @with_exception_handling
    def calculate_metrics(self, outcomes: List[SignalOutcome], tool_name:
        Optional[str]=None, market_regime: Optional[MarketRegime]=None,
        ...
        try:
            if tool_name:
                outcomes = [o for o in outcomes if o.signal_event.tool_name ==
                    tool_name]
            if not outcomes:
                self.logger.warning(
                    'No outcomes match criteria for metrics calculation')
                return {'error': 'No outcomes match the specified criteria',
                    'sample_size': 0, 'metrics': {}}
            win_rate = self.win_rate_metric.calculate(outcomes,
                market_regime, timeframe, symbol, start_date, end_date)
            profit_factor = self.profit_factor_metric.calculate(outcomes,
                market_regime, timeframe, symbol, start_date, end_date)
            expected_payoff = self.expected_payoff_metric.calculate(outcomes,
                market_regime, timeframe, symbol, start_date, end_date)
            reliability_by_regime = self.reliability_metric.calculate(outcomes,
                timeframe, symbol, start_date, end_date)
            signal_quality = (self.signal_quality_evaluator.
                evaluate_all_metrics(outcomes, market_regime, timeframe))
            metrics_report = {'sample_size': len(outcomes),
                'filtered_sample_size': win_rate['sample_size'],
                'time_range': {'start_date': start_date.isoformat() if
                start_date else None, 'end_date': end_date.isoformat() if
                end_date else None}, 'filters': {'tool_name': tool_name,
                'market_regime': market_regime.value if market_regime else
                None, 'timeframe': timeframe.value if timeframe else None,
                'symbol': symbol}, 'core_metrics': {'win_rate': win_rate,
                'profit_factor': profit_factor, 'expected_payoff':
                expected_payoff, 'reliability_by_regime':
                reliability_by_regime}, 'signal_quality_metrics':
                signal_quality, 'composite_score': self.
                _calculate_composite_score(win_rate, profit_factor,
                expected_payoff, reliability_by_regime, signal_quality)}
            self.logger.info(
                f"Calculated metrics for {metrics_report['filtered_sample_size']} outcomes with win rate: {win_rate['value']:.2f}, profit factor: {profit_factor['value']}"
                )
            return metrics_report
        except Exception as e:
            self.logger.error(f'Error calculating metrics: {str(e)}')
            return {'error': f'Failed to calculate metrics: {str(e)}',
                'sample_size': len(outcomes) if outcomes else 0, 'metrics': {}}

    @with_analysis_resilience('analyze_tool_by_market_regime')
    @with_exception_handling
    def analyze_tool_by_market_regime(self, outcomes: List[SignalOutcome],
        ...
        """
        try:
            tool_outcomes = [o for o in outcomes if o.signal_event.
                tool_name == tool_name]
            if not tool_outcomes:
                self.logger.warning(f"No outcomes found for tool '{tool_name}'"
                    )
                return {'error':
                    f"No outcomes found for tool '{tool_name}'",
                    'sample_size': 0, 'regimes': {}}
            regimes = set()
            ...
            for regime in regimes:
                regime_outcomes = [o for o in tool_outcomes if o.
                    signal_event.market_context.get('regime', MarketRegime.
                    UNKNOWN.value) == regime]
                regime_metrics[regime] = {'sample_size': len(
                    regime_outcomes), 'win_rate': self.win_rate_metric.
                    calculate(regime_outcomes), 'profit_factor': self.
                    profit_factor_metric.calculate(regime_outcomes),
                    'expected_payoff': self.expected_payoff_metric.
                    calculate(regime_outcomes)}
            overall_metrics = {'win_rate': self.win_rate_metric.calculate(
                tool_outcomes), 'profit_factor': self.profit_factor_metric.
                calculate(tool_outcomes), 'expected_payoff': self.
                expected_payoff_metric.calculate(tool_outcomes)}
            if len(regime_metrics) > 1:
                best_regime = max(regime_metrics.items(), key=lambda x: x[1
                    ]['win_rate']['value'] if x[1]['win_rate']['value'] is not
                    None else -1)[0]
                worst_regime = min(regime_metrics.items(), key=lambda x: x[
                    1]['win_rate']['value'] if x[1]['win_rate']['value'] is not
                    None else float('inf'))[0]
            else:
                best_regime = list(regime_metrics.keys())[0
                    ] if regime_metrics else None
                worst_regime = None
            return {'tool_name': tool_name, 'sample_size': len(
                tool_outcomes), 'overall_metrics': overall_metrics,
                'regime_metrics': regime_metrics, 'best_regime':
                best_regime, 'worst_regime': worst_regime}
        except Exception as e:
            self.logger.error(
                f'Error analyzing tool by market regime: {str(e)}')
            return {'error':
                f'Failed to analyze tool by market regime: {str(e)}',
                'sample_size': len(outcomes) if outcomes else 0, 'regimes': {}}

    @with_resilience('get_effectiveness_trend')
    @with_exception_handling
    def get_effectiveness_trend(self, outcomes: List[SignalOutcome],
        tool_name: Optional[str]=None, window_size: int=20, step_size: int=5
        ) ->Dict[str, Any]:
        """
        ...
        try:
            ...
            if len(outcomes) < window_size:
                self.logger.warning(
                    f'Not enough outcomes for trend analysis (need at least {window_size})'
                    )
                return {'error':
                    f'Not enough outcomes for trend analysis (need at least {window_size})'
                    , 'sample_size': len(outcomes), 'trends': {}}
            ...
            for i in range(0, len(sorted_outcomes) - window_size + 1, step_size
                ...
                trend_data.append({'end_date': end_date.isoformat(),
                    'window_size': len(window), 'metrics': {'win_rate':
                    win_rate['value'], 'profit_factor': profit_factor[
                    'value'], 'expected_payoff': expected_payoff['value']}})
            if len(trend_data) > 1:
                first_win_rate = trend_data[0]['metrics']['win_rate']
                last_win_rate = trend_data[-1]['metrics']['win_rate']
                win_rate_trend = ('improving' if last_win_rate >
                    first_win_rate else 'declining')
            ...
            return {'tool_name': tool_name, 'sample_size': len(outcomes),
                'window_size': window_size, 'step_size': step_size,
                'trend_points': len(trend_data), 'trend_direction': {
                'win_rate': win_rate_trend, 'profit_factor': pf_trend},
                'trend_data': trend_data}
        except Exception as e:
            self.logger.error(
                f'Error calculating effectiveness trend: {str(e)}')
            return {'error':
                f'Failed to calculate effectiveness trend: {str(e)}',
                'sample_size': len(outcomes) if outcomes else 0, 'trends': {}}

    @with_exception_handling
    def _calculate_composite_score(self, win_rate: Dict[str, Any],
        profit_factor: Dict[str, Any], expected_payoff: Dict[str, Any],
        reliability: Dict[str, Any], signal_quality: Dict[str, Any]
        ) ->Optional[float]:
        """
        ...
        try:
            ...
            if profit_factor['value'] is not None and profit_factor['value'
                ] != float('inf'):
            ...
            if expected_payoff['value'] is not None:
                ep_normalized = 1 / (1 + np.exp(-expected_payoff['value'] *
                    0.1))
