...
def calculate_basic_statistics(values: List[float]) ->Dict[str, float]:
    ...
    """
    if not values:
        return {'count': 0, 'min': 0.0, 'max': 0.0, 'mean': 0.0, 'median': 
            0.0, 'std_dev': 0.0, 'variance': 0.0}
    try:
        values_array = np.array(values)
        return {'count': len(values), 'min': float(np.min(values_array)),
            'max': float(np.max(values_array)), 'mean': float(np.mean(
            values_array)), 'median': float(np.median(values_array)),
            'std_dev': float(np.std(values_array)), 'variance': float(np.
            var(values_array))}
    except:
        count = len(values)
        min_val = min(values)
        ...
        if count % 2 == 0:
            median = (sorted_values[count // 2 - 1] + sorted_values[count // 2]
                ) / 2
        else:
            median = sorted_values[count // 2]
        variance = sum((x - mean) ** 2 for x in values) / count
        std_dev = math.sqrt(variance)
        return {'count': count, 'min': min_val, 'max': max_val, 'mean':
            mean, 'median': median, 'std_dev': std_dev, 'variance': variance}


@with_exception_handling
def calculate_percentiles(values: List[float], percentiles: List[int]=None
    ) ->Dict[str, float]:
    """
...
@with_exception_handling
def calculate_moving_average(values: List[float], window_size: int=5) ->List[
    float]:
    """
...
@with_exception_handling
def calculate_correlation(values1: List[float], values2: List[float]) ->Tuple[
    float, str]:
    """
    ...
    try:
        ...
        numerator = sum((x - mean1) * (y - mean2) for x, y in zip(values1,
            values2))
...
@with_exception_handling
def detect_outliers(values: List[float], method: str='zscore', threshold:
    float=3.0) ->List[int]:
    """
    ...
    try:
        ...
        if method == 'zscore':
            ...
            outliers = np.where((values_array < lower_bound) | (
