...
class MoneyFlowIndex(AdvancedAnalysisBase):
    ...
    def __init__(self, period: int=14, overbought_level: float=80.0,
        oversold_level: float=20.0, output_prefix: str='MFI', **kwargs):
        """
        ...
        """
        parameters = {'period': period, 'overbought_level':
            overbought_level, 'oversold_level': oversold_level,
    ...
    def calculate(self, df: pd.DataFrame) ->pd.DataFrame:
        ...
        result_df['typical_price'] = (result_df['high'] + result_df['low'] +
            result_df['close']) / 3.0
        result_df['money_flow'] = result_df['typical_price'] * result_df[
            'volume']
        result_df['direction'] = result_df['typical_price'].diff().fillna(0.0)
        result_df['positive_flow'] = np.where(result_df['direction'] >= 0,
            result_df['money_flow'], 0.0)
        result_df['negative_flow'] = np.where(result_df['direction'] < 0,
            result_df['money_flow'], 0.0)
        result_df['pos_flow_sum'] = result_df['positive_flow'].rolling(window
            =period).sum()
        result_df['neg_flow_sum'] = result_df['negative_flow'].rolling(window
            =period).sum()
        result_df['money_ratio'] = np.where(result_df['neg_flow_sum'] != 0,
            result_df['pos_flow_sum'] / result_df['neg_flow_sum'], 100.0)
        result_df[f'{prefix}'] = 100.0 - 100.0 / (1.0 + result_df[
            'money_ratio'])
        result_df[f'{prefix}_overbought'] = result_df[f'{prefix}'
            ] >= overbought
        result_df[f'{prefix}_oversold'] = result_df[f'{prefix}'] <= oversold
        cols_to_drop = ['typical_price', 'money_flow', 'direction',
            'positive_flow', 'negative_flow', 'pos_flow_sum',
            'neg_flow_sum', 'money_ratio']
    ...
    def initialize_incremental(self) ->Dict[str, Any]:
        ...
        """
        state = {'period': self.parameters['period'], 'last_typical_price':
            None, 'positive_flows': [], 'negative_flows': [],
    ...
    def update_incremental(self, state: Dict[str, Any], new_data: Dict[str,
        ...
        if not all(field in new_data for field in required_fields):
            logger.warning(
                f'Required fields {required_fields} missing in new_data')
            return state
        typical_price = (new_data['high'] + new_data['low'] + new_data['close']
            ) / 3.0
