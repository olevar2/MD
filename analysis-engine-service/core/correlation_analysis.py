...
@async_with_exception_handling
async def analyze_currency_correlations(data: Dict[str, Any], window_sizes:
    Optional[List[int]]=Query(None, description=
    ...
    try:
        ...
        if not data or not isinstance(data, dict):
            raise HTTPException(status_code=400, detail=
                'Request must contain currency pair data')
        if len(data) < 2:
            raise HTTPException(status_code=400, detail=
        ...
        raise HTTPException(status_code=500, detail=
            f'Failed to analyze currency correlations: {str(e)}')


@router.post('/lead-lag', response_model=Dict[str, Any])
@async_with_exception_handling
async def analyze_lead_lag_relationships(data: Dict[str, Any], max_lag:
    Optional[int]=Query(10, description=
    ...
    try:
        ...
        return {'status': 'success', 'results': lead_lag_results,
            'timestamp': datetime.now().isoformat()}
    except HTTPException:
        raise
    except Exception as e:
        ...
        raise HTTPException(status_code=500, detail=
            f'Failed to analyze lead-lag relationships: {str(e)}')


@router.post('/breakdown-detection', response_model=Dict[str, Any])
@async_with_exception_handling
async def detect_correlation_breakdowns(data: Dict[str, Any], short_window:
    Optional[int]=Query(5, description='Short-term correlation window'),
    long_window: Optional[int]=Query(60, description=
    'Long-term correlation window for comparison'), change_threshold:
    Optional[float]=Query(0.3, description=
    ...
    try:
        ...
        return {'status': 'success', 'results': breakdown_results,
            'timestamp': datetime.now().isoformat()}
    except HTTPException:
        raise
    except Exception as e:
        ...
        raise HTTPException(status_code=500, detail=
            f'Failed to detect correlation breakdowns: {str(e)}')


@router.post('/cointegration', response_model=Dict[str, Any])
@async_with_exception_handling
async def test_pair_cointegration(data: Dict[str, Any], significance:
    Optional[float]=Query(0.05, description=
    'P-value threshold for cointegration significance'), current_user: User
    =Depends(get_current_user)):
    """
    ...
    try:
        ...
        return {'status': 'success', 'results': cointegration_results,
            'timestamp': datetime.now().isoformat()}
    except HTTPException:
        raise
    except Exception as e:
        ...
        raise HTTPException(status_code=500, detail=
            f'Failed to test cointegration: {str(e)}')
