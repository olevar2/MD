...
class EffectivenessLogger:
    ...
    def __init__(self, analyzer_name: str, storage_path: str=None):
        ...
        self.storage_path = storage_path or os.environ.get(
            'EFFECTIVENESS_LOG_PATH', './effectiveness_logs')
    ...
    def _ensure_storage_path(self):
        ...
        try:
            ...
            logger.info(
                f'Effectiveness logs will be stored in: {self.storage_path}')
        except Exception as e:
            logger.error(f'Failed to create effectiveness log directory: {e}')

    def log(self, prediction: Dict[str, Any], actual: Dict[str, Any],
        accuracy: float, timeframe: Optional[str]=None, instrument:
        Optional[str]=None, result_id: Optional[str]=None, parameters:
        ...
        log_entry = {'id': log_id, 'timestamp': timestamp, 'analyzer_name':
            self.analyzer_name, 'prediction': prediction, 'actual': actual,
            'accuracy': accuracy, 'timeframe': timeframe, 'instrument':
            instrument, 'result_id': result_id, 'parameters': parameters}
    ...
    def _update_summary(self, log_entry: Dict[str, Any]) ->None:
        ...
        """
        try:
            summary_file = os.path.join(self.storage_path,
                f'{self.analyzer_name}_summary.json')
            try:
                with open(summary_file, 'r') as f:
                    summary = json.load(f)
            except (FileNotFoundError, json.JSONDecodeError):
                summary = {'analyzer_name': self.analyzer_name,
                    'total_predictions': 0, 'average_accuracy': 0.0,
                    'by_timeframe': {}, 'by_instrument': {}, 'last_updated':
                    None}
            current_total = summary['total_predictions']
            current_avg = summary['average_accuracy']
            summary['total_predictions'] += 1
            summary['average_accuracy'] = (current_total * current_avg +
                log_entry['accuracy']) / summary['total_predictions']
            timeframe = log_entry.get('timeframe')
            if timeframe:
                if timeframe not in summary['by_timeframe']:
                    summary['by_timeframe'][timeframe] = {'count': 0,
                        'average_accuracy': 0.0}
                tf_stats = summary['by_timeframe'][timeframe]
                tf_stats['count'] += 1
                tf_stats['average_accuracy'] = ((tf_stats['count'] - 1) *
                    tf_stats['average_accuracy'] + log_entry['accuracy']
                    ) / tf_stats['count']
            instrument = log_entry.get('instrument')
            if instrument:
                if instrument not in summary['by_instrument']:
                    summary['by_instrument'][instrument] = {'count': 0,
                        'average_accuracy': 0.0}
                inst_stats = summary['by_instrument'][instrument]
                inst_stats['count'] += 1
                inst_stats['average_accuracy'] = ((inst_stats['count'] - 1) *
                    inst_stats['average_accuracy'] + log_entry['accuracy']
                    ) / inst_stats['count']
    ...
    def get_logs(self, limit: int=100, offset: int=0) ->List[Dict[str, Any]]:
        ...
        if offset < len(self.logs):
            return self.logs[-offset - limit:-offset
                ] if offset > 0 else self.logs[-limit:]
        return self._load_logs_from_storage(limit, offset)

    @with_exception_handling
    def _load_logs_from_storage(self, limit: int, offset: int) ->List[Dict[
        str, Any]]:
        """
        ...
        try:
            ...
            if not os.path.exists(analyzer_dir):
                return []
            log_files = [f for f in os.listdir(analyzer_dir) if f.endswith(
                '.json')]
            log_files.sort(key=lambda f: os.path.getmtime(os.path.join(
                analyzer_dir, f)), reverse=True)
    ...
    def get_summary(self) ->Dict[str, Any]:
        ...
        try:
            ...
            return {'analyzer_name': self.analyzer_name,
                'total_predictions': 0, 'average_accuracy': 0.0,
                'by_timeframe': {}, 'by_instrument': {}, 'last_updated': None}
        except Exception as e:
            logger.error(f'Failed to load effectiveness summary: {e}')
            return {'analyzer_name': self.analyzer_name, 'error': str(e)}
