...
router = APIRouter(prefix='/feedback', tags=['feedback'], responses={(404):
...
@async_with_exception_handling
async def get_feedback_statistics(strategy_id: Optional[str]=Query(None,
    description='Filter by strategy ID'), model_id: Optional[str]=Query(
    None, description='Filter by model ID'), instrument: Optional[str]=
...
@async_with_exception_handling
async def trigger_model_retraining(model_id: str, feedback_service:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=
            f'Error triggering model retraining: {str(e)}')


@router.put('/rules')
@async_with_exception_handling
async def update_feedback_rules(rule_updates: Dict[str, Any]=Body(...,
    description='Rule updates to apply'), feedback_service:
    FeedbackIntegrationService=Depends(get_feedback_service)):
    """
...
async def get_parameter_performance(strategy_id: str, min_samples: int=
    ...
    try:
        ...
        if 'parameter_performance' not in stats:
            return {'message': 'No parameter performance data available',
                'parameters': {}}
        filtered_parameters = {}
