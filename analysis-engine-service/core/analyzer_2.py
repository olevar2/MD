...
try:
    ...
    logger.error(
        f'Failed to initialize EventPublisher in MarketRegimeAnalyzer: {e}',
        exc_info=True)
...
class MarketRegimeAnalyzer:
    ...
    async def detect_regime(self, instrument: str, timeframe: str,
        ...
        prices = await self.detector.get_price_data(instrument=instrument,
            timeframe=timeframe, from_date=from_date, to_date=to_date)
        if not prices or len(prices) < 20:
            return MarketRegimeResult(instrument=instrument, timeframe=
                timeframe, regime='unknown', direction='neutral',
                volatility='unknown', strength=0, detected_at=datetime.utcnow()
                )
        features = self.detector.extract_features(prices)
        return self.classifier.classify_regime(instrument=instrument,
            timeframe=timeframe, features=features)

    @async_with_exception_handling
    async def detect_regime_change(self, instrument: str, timeframe: str,
        days_back: int=60) ->RegimeChangeResult:
        """
        ...
        current_regime = await self.detect_regime(instrument=instrument,
            timeframe=timeframe, from_date=mid_point, to_date=now)
        previous_regime = await self.detect_regime(instrument=instrument,
            timeframe=timeframe, from_date=start_date, to_date=mid_point)
        change_result = self.classifier.classify_regime_change(current_regime
            =current_regime, previous_regime=previous_regime)
        result = RegimeChangeResult(instrument=instrument, timeframe=
            timeframe, period_days=days_back, regime_changed=change_result[
            'regime_changed'], direction_changed=change_result[
            'direction_changed'], volatility_changed=change_result[
            'volatility_changed'], change_significance=change_result[
            'change_significance'], current_regime=current_regime,
            previous_regime=previous_regime, analyzed_at=now)
        if result.regime_changed and event_publisher:
            try:
                payload = MarketRegimeChangePayload(symbol=instrument,
                    timeframe=timeframe, previous_regime=previous_regime.
                    regime.value, current_regime=current_regime.regime.
                    value, confidence_score=current_regime.strength,
                    detection_timestamp=current_regime.detected_at)
                event = MarketRegimeChangeEvent(payload=payload)
                event_publisher.publish(topic=settings.KAFKA_REGIME_TOPIC,
                    event=event)
            except Exception as e:
                logger.error(f'Failed to publish MarketRegimeChangeEvent: {e}',
                    exc_info=True)
        return result
