...
class ToolEffectivenessScheduler:
    ...
    async def start(self):
        ...
        self.scheduled_tasks = {'hourly': {'interval': 60 * 60, 'next_run':
            self._next_hour(), 'func': self.calculate_hourly_metrics},
            'daily': {'interval': 24 * 60 * 60, 'next_run': self._next_time
            (0, 30), 'func': self.calculate_daily_metrics}, 'weekly': {
            'interval': 7 * 24 * 60 * 60, 'next_run': self._next_monday(1, 
            0), 'func': self.calculate_weekly_metrics}, 'monthly': {
            'interval': 30 * 24 * 60 * 60, 'next_run': self._next_month_day
            (1, 2, 0), 'func': self.calculate_monthly_metrics}}
    ...
    @with_analysis_resilience('calculate_hourly_metrics')
    @async_with_exception_handling
    async def calculate_hourly_metrics(self):
        ...
        try:
            db = SessionLocal()
            await self._calculate_metrics(db=db, lookback_hours=1,
                save_to_db=True, metric_types=['win_rate', 'profit_factor',
                'expected_payoff'])
            db.close()
            self.logger.info(
                'Hourly effectiveness metrics calculation completed')
            return True
        except Exception as e:
            self.logger.error(f'Error calculating hourly metrics: {str(e)}')
            return False

    @with_analysis_resilience('calculate_daily_metrics')
    @async_with_exception_handling
    async def calculate_daily_metrics(self):
        ...
        try:
            db = SessionLocal()
            await self._calculate_metrics(db=db, lookback_hours=24,
                save_to_db=True, metric_types=['win_rate', 'profit_factor',
                'expected_payoff', 'reliability_by_regime'])
            db.close()
            self.logger.info(
                'Daily effectiveness metrics calculation completed')
            return True
        except Exception as e:
            self.logger.error(f'Error calculating daily metrics: {str(e)}')
            return False

    @with_analysis_resilience('calculate_weekly_metrics')
    @async_with_exception_handling
    async def calculate_weekly_metrics(self):
        ...
        try:
            db = SessionLocal()
            await self._calculate_metrics(db=db, lookback_hours=168,
                save_to_db=True, metric_types=['win_rate', 'profit_factor',
                'expected_payoff', 'reliability_by_regime'],
                include_regime_specific=True)
            db.close()
            self.logger.info(
                'Weekly effectiveness metrics calculation completed')
            return True
        except Exception as e:
            self.logger.error(f'Error calculating weekly metrics: {str(e)}')
            return False

    @with_analysis_resilience('calculate_monthly_metrics')
    @async_with_exception_handling
    async def calculate_monthly_metrics(self):
        ...
        try:
            db = SessionLocal()
            await self._calculate_metrics(db=db, lookback_hours=720,
                save_to_db=True, metric_types=['win_rate', 'profit_factor',
                'expected_payoff', 'reliability_by_regime'],
                include_regime_specific=True, include_signal_quality=True)
            db.close()
            self.logger.info(
                'Monthly effectiveness metrics calculation completed')
            return True
        except Exception as e:
            self.logger.error(f'Error calculating monthly metrics: {str(e)}')
            return False

    async def _calculate_metrics(self, db: Session, lookback_hours: int,
        save_to_db: bool=True, metric_types: Optional[List[str]]=None,
        include_regime_specific: bool=False, include_signal_quality: bool=False
        ) ->Dict[str, Any]:
        """
        ...
        tool_ids = repository.get_tool_ids(start_date=start_date, end_date=
            end_date)
        results = {}
        for tool_id in tool_ids:
            outcomes = repository.get_outcomes(tool_id=tool_id, start_date=
            ...
            metrics = tracker.calculate_metrics(outcomes=domain_outcomes,
                tool_name=tool_id, start_date=start_date, end_date=end_date)
            results[tool_id] = metrics
            if save_to_db:
                self._save_metrics_to_db(repository, tool_id, metrics,
                    lookback_hours)
            if include_regime_specific:
                for regime in MarketRegime:
                    regime_metrics = tracker.calculate_metrics(outcomes=
                        domain_outcomes, tool_name=tool_id, market_regime=
                        regime, start_date=start_date, end_date=end_date)
                    if save_to_db and regime_metrics.get('filtered_sample_size'
                        , 0) > 0:
                        self._save_regime_metrics_to_db(repository, tool_id,
                            regime, regime_metrics, lookback_hours)
        return results

    @with_exception_handling
    def _save_metrics_to_db(self, repository: ToolEffectivenessRepository,
        tool_id: str, metrics: Dict[str, Any], lookback_hours: int) ->None:
        ...
        try:
            ...
            for metric_name, metric_data in core_metrics.items():
                ...
                if metric_data.get('value') is not None:
                    repository.create_metric({'tool_id': tool_id,
                        'metric_type': metric_name, 'value': metric_data[
                        'value'], 'sample_size': metrics.get(
                        'filtered_sample_size', 0), 'lookback_hours':
                        lookback_hours, 'created_at': datetime.utcnow()})
            composite_score = metrics.get('composite_score')
            if composite_score is not None:
                repository.create_metric({'tool_id': tool_id, 'metric_type':
                    'composite_score', 'value': composite_score,
                    'sample_size': metrics.get('filtered_sample_size', 0),
                    'lookback_hours': lookback_hours, 'created_at':
                    datetime.utcnow()})
            ...
            for metric_name, quality_data in quality_metrics.items():
                if quality_data.get('value') is not None:
                    repository.create_metric({'tool_id': tool_id,
                        'metric_type': f'quality_{metric_name}', 'value':
                        quality_data['value'], 'sample_size': quality_data.
                        get('sample_size', 0), 'lookback_hours':
                        lookback_hours, 'created_at': datetime.utcnow()})
        except Exception as e:
            self.logger.error(f'Error saving metrics to database: {str(e)}')

    @with_exception_handling
    def _save_regime_metrics_to_db(self, repository:
        ToolEffectivenessRepository, tool_id: str, regime: MarketRegime,
        metrics: Dict[str, Any], lookback_hours: int) ->None:
            ...
            for metric_name, metric_data in core_metrics.items():
                ...
                if metric_data.get('value') is not None:
                    repository.create_metric({'tool_id': tool_id,
                        ...
                        'value': metric_data['value'], 'sample_size':
                        metrics.get('filtered_sample_size', 0),
                        'market_regime': regime.value, 'lookback_hours':
                        lookback_hours, 'created_at': datetime.utcnow()})
        except Exception as e:
            self.logger.error(
                f'Error saving regime metrics to database: {str(e)}')
