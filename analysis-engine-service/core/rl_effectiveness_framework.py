"""
Rl effectiveness framework module.
...
class ToolEffectivenessMetric(str, Enum):
    """
    ToolEffectivenessMetric class that inherits from str, Enum.
...
class RLEffectivenessFramework:
    ...
    def __init__(self, baseline_strategies: Dict[str, Any]=None,
        significance_level: float=0.05, bootstrap_samples: int=1000):
        """
    ...
    def add_model_results(self, model_name: str, trades: pd.DataFrame,
        actions: Optional[pd.DataFrame]=None, market_data: Optional[pd.
        DataFrame]=None, model_confidence: Optional[pd.DataFrame]=None):
        """
        ...
        """
        self.models_results[model_name] = {'trades': trades, 'actions':
            actions, 'market_data': market_data, 'model_confidence':
            model_confidence, 'metrics': self._calculate_metrics(trades)}
    ...
    def _calculate_metrics(self, trades: pd.DataFrame) ->Dict[str, float]:
        ...
        metrics[ToolEffectivenessMetric.WIN_RATE] = len(winning_trades) / len(
            trades) if len(trades) > 0 else 0
        ...
        metrics[ToolEffectivenessMetric.PROFIT_FACTOR
            ] = total_profit / total_loss if total_loss > 0 else float('inf')
        metrics[ToolEffectivenessMetric.AVERAGE_TRADE] = trades['profit_loss'
            ].mean()
        metrics[ToolEffectivenessMetric.AVERAGE_WINNER] = winning_trades[
            'profit_loss'].mean() if not winning_trades.empty else 0
        metrics[ToolEffectivenessMetric.AVERAGE_LOSER] = losing_trades[
            'profit_loss'].mean() if not losing_trades.empty else 0
        metrics[ToolEffectivenessMetric.EXPECTANCY] = metrics[
            ToolEffectivenessMetric.WIN_RATE] * metrics[ToolEffectivenessMetric
            .AVERAGE_WINNER] + (1 - metrics[ToolEffectivenessMetric.WIN_RATE]
            ) * metrics[ToolEffectivenessMetric.AVERAGE_LOSER]
        if 'cumulative_returns' not in trades.columns:
            trades_sorted = trades.sort_values('entry_time')
            trades_sorted['cumulative_returns'] = (1 + trades_sorted[
                'profit_loss']).cumprod()
            if ('entry_time' in trades.columns and 'exit_time' in trades.
                columns):
                ...
                if pd.notna(start_date) and pd.notna(end_date):
                    ...
                    metrics[ToolEffectivenessMetric.PROFIT_PER_DAY] = (
                        trades_sorted['cumulative_returns'].iloc[-1] - 1
                        ) / trading_days if trading_days > 0 else 0
        cumulative_returns = trades_sorted['cumulative_returns']
        running_max = cumulative_returns.cummax()
        drawdown = cumulative_returns / running_max - 1
        metrics[ToolEffectivenessMetric.MAX_DRAWDOWN] = abs(drawdown.min()
            ) if len(drawdown) > 0 else 0
        if len(trades) > 1:
            returns = trades['profit_loss']
            metrics[ToolEffectivenessMetric.SHARPE_RATIO] = returns.mean(
                ) / returns.std() * np.sqrt(252) if returns.std() > 0 else 0
            downside_returns = returns[returns < 0]
            metrics[ToolEffectivenessMetric.SORTINO_RATIO] = returns.mean(
                ) / downside_returns.std() * np.sqrt(252
                ) if not downside_returns.empty and downside_returns.std(
                ) > 0 else 0
            metrics[ToolEffectivenessMetric.CALMAR_RATIO] = metrics[
                ToolEffectivenessMetric.PROFIT_PER_DAY] * 252 / metrics[
                ToolEffectivenessMetric.MAX_DRAWDOWN] if metrics[
                ToolEffectivenessMetric.MAX_DRAWDOWN] > 0 else 0
        return metrics

    def compare_models(self, metrics: List[ToolEffectivenessMetric]=None
        ) ->pd.DataFrame:
        """
        ...
        if not self.models_results:
            return pd.DataFrame()
        if metrics is None:
            metrics = [ToolEffectivenessMetric.WIN_RATE,
        ...
        for model_name, results in self.models_results.items():
            comparison[model_name] = {metric.value: results['metrics'].get(
                metric, float('nan')) for metric in metrics}
        return pd.DataFrame.from_dict(comparison, orient='index')

    def compare_regimes(self, models: List[str]=None, regimes: List[
        MarketRegime]=None) ->pd.DataFrame:
        """
        ...
        for model in models:
            ...
            for regime in regimes:
                ...
                data.append([regime_data.get(ToolEffectivenessMetric.
                    WIN_RATE, float('nan')), regime_data.get(
                    ToolEffectivenessMetric.PROFIT_FACTOR, float('nan')),
                    regime_data.get(ToolEffectivenessMetric.EXPECTANCY,
                    float('nan')), regime_data.get('trade_count', 0)])
                index.append((model, regime))
        columns = ['Win Rate', 'Profit Factor', 'Expectancy', 'Trade Count']
        return pd.DataFrame(data, index=pd.MultiIndex.from_tuples(index,
            names=['Model', 'Regime']), columns=columns)

    def statistical_comparison(self, baseline_model: str, test_model: str,
        metric: ToolEffectivenessMetric=ToolEffectivenessMetric.PROFIT_FACTOR
        ) ->Dict[str, Any]:
        """
        ...
        """
        if (baseline_model not in self.models_results or test_model not in
            self.models_results):
        ...
        if metric == ToolEffectivenessMetric.PROFIT_FACTOR:
            ...
            baseline_values = baseline_trades['profit_loss']
            test_values = test_trades['profit_loss']
        t_stat, p_value = stats.ttest_ind(baseline_values, test_values,
            equal_var=False)
        bootstrap_means = []
        for _ in range(self.bootstrap_samples):
            baseline_sample = baseline_values.sample(len(baseline_values),
                replace=True).mean()
            test_sample = test_values.sample(len(test_values), replace=True
                ).mean()
            bootstrap_means.append(test_sample - baseline_sample)
        bootstrap_means = np.array(bootstrap_means)
        confidence_interval = np.percentile(bootstrap_means, 2.5
            ), np.percentile(bootstrap_means, 97.5)
        return {'baseline_mean': baseline_values.mean(), 'test_mean':
            test_values.mean(), 'difference': test_values.mean() -
            baseline_values.mean(), 't_statistic': t_stat, 'p_value':
            p_value, 'significant': p_value < self.significance_level,
            'confidence_interval': confidence_interval, 'sample_sizes': (
            len(baseline_values), len(test_values))}

    def plot_regime_performance(self, model: str=None, metric:
        ToolEffectivenessMetric=ToolEffectivenessMetric.WIN_RATE, figsize:
        Tuple[int, int]=(10, 6)):
        """
        ...
        if model is not None and model in self.regime_results:
            ...
            plt.title(
                f"{metric.value.replace('_', ' ').title()} by Market Regime\nModel: {model}"
                )
        else:
            data = []
            for model_name, regime_data in self.regime_results.items():
                for regime, metrics in regime_data.items():
                    if metric in metrics:
                        data.append({'Model': model_name, 'Regime': regime,
    ...
    def plot_confidence_calibration(self, model: str):
        ...
        """
        if model not in self.models_results or self.models_results[model][
            'actions'] is None:
            print(f'No action data available for model {model}')
            return
        actions = self.models_results[model]['actions']
        if ('confidence' not in actions.columns or 'correct' not in actions
            .columns):
    ...
    @with_analysis_resilience('analyze_model_decay')
    ...
    def generate_effectiveness_report(self, model: str) ->Dict[str, Any]:
        ...
        if results['actions'] is not None:
            action_quality = self.action_quality_analyzer.analyze_actions(
                results['actions'])
        return {'model_name': model, 'trade_count': len(trades),
            'overall_metrics': overall_metrics, 'regime_metrics':
            regime_metrics, 'decay_analysis': decay_analysis,
            'action_quality': action_quality, 'trading_period': {'start':
            trades['entry_time'].min(), 'end': trades['exit_time'].max(),
            'days': (trades['exit_time'].max() - trades['entry_time'].min()
            ).days}}
...
class MarketRegimeEffectivenessAnalyzer:
    ...
    @with_analysis_resilience('analyze_regimes')
    def analyze_regimes(self, trades: pd.DataFrame) ->Dict[str, Dict[str,
        float]]:
        """
        ...
        for regime, regime_trades in trades.groupby('market_regime'):
            ...
            win_rate = len(winning_trades) / len(regime_trades) if len(
                regime_trades) > 0 else 0
            total_profit = winning_trades['profit_loss'].sum(
                ) if not winning_trades.empty else 0
            total_loss = abs(regime_trades[regime_trades['profit_loss'] <= 
                0]['profit_loss'].sum())
            ...
            avg_loss = regime_trades[regime_trades['profit_loss'] <= 0][
                'profit_loss'].mean() if len(regime_trades[regime_trades[
                'profit_loss'] <= 0]) > 0 else 0
            expectancy = win_rate * avg_win + (1 - win_rate) * avg_loss
            regime_metrics[regime] = {ToolEffectivenessMetric.WIN_RATE:
                win_rate, ToolEffectivenessMetric.PROFIT_FACTOR:
                profit_factor, ToolEffectivenessMetric.AVERAGE_TRADE:
                avg_trade, ToolEffectivenessMetric.EXPECTANCY: expectancy,
                'trade_count': len(regime_trades)}
        return regime_metrics

    def compare_regime_effectiveness(self, trades_by_model: Dict[str, pd.
        ...
        for model_name, trades in trades_by_model.items():
            ...
            for regime, metrics in regime_metrics.items():
                ...
                row.update({metric.value: value for metric, value in
                    metrics.items()})
...
class ActionQualityAnalyzer:
    ...
    @with_analysis_resilience('analyze_actions')
    def analyze_actions(self, actions: pd.DataFrame) ->Dict[str, Any]:
        ...
        if not all(col in actions.columns for col in required_columns):
            logger.warning(
                f'Actions dataframe missing required columns: {required_columns}'
                )
            return results
        ...
        if 'confidence' in actions.columns:
            ...
            results['confidence_distribution'] = {'min': actions[
                'confidence'].min(), 'q1': actions['confidence'].quantile(
                0.25), 'median': actions['confidence'].median(), 'q3':
                actions['confidence'].quantile(0.75), 'max': actions[
                'confidence'].max()}
        if 'correct' in actions.columns:
            correct_actions = actions[actions['correct'] == True]
            results['accuracy'] = len(correct_actions) / len(actions) if len(
                actions) > 0 else 0
            if 'confidence' in actions.columns:
                results['calibration'] = self._calculate_calibration_metrics(
                    actions)
        return results

    def _calculate_calibration_metrics(self, actions: pd.DataFrame) ->Dict[
        str, Any]:
        """
        ...
        for bin_name, group in actions.groupby('confidence_bin'):
            ...
            calibration_data.append({'bin': bin_name, 'bin_center':
                bin_name.mid, 'accuracy': accuracy, 'avg_confidence':
                avg_confidence, 'count': len(group), 'calibration_error':
                abs(accuracy - avg_confidence)})
        calibration_df = pd.DataFrame(calibration_data)
        return {'calibration_data': calibration_data,
            'mean_calibration_error': calibration_df['calibration_error'].
            mean() if not calibration_df.empty else 0}

    def plot_calibration_curve(self, actions: pd.DataFrame, figsize: Tuple[
        int, int]=(10, 6)):
        """
        ...
        if ('confidence' not in actions.columns or 'correct' not in actions
            .columns):
            print(
                "Actions dataframe must have 'confidence' and 'correct' columns"
                )
            return
        actions['confidence_bin'] = pd.cut(actions['confidence'], bins=10)
        bin_data = actions.groupby('confidence_bin').agg({'correct': 'mean',
            'confidence': 'mean', 'action': 'count'}).reset_index()
        bin_data = bin_data.rename(columns={'correct': 'accuracy', 'action':
            'count'})
        plt.figure(figsize=figsize)
        plt.plot([0, 1], [0, 1], 'k--', label='Perfect calibration')
        plt.scatter(bin_data['confidence'], bin_data['accuracy'], s=
            bin_data['count'] / bin_data['count'].max() * 100, alpha=0.7)
        for _, row in bin_data.iterrows():
            plt.annotate(f"{row['count']}", (row['confidence'], row[
                'accuracy']), textcoords='offset points', xytext=(0, 5), ha
                ='center')
...
class EffectivenessDecayAnalyzer:
    ...
    @with_analysis_resilience('analyze_decay')
    def analyze_decay(self, trades: pd.DataFrame, window_size: int=20
        ) ->pd.DataFrame:
        """
        ...
        rolling_metrics['win_rate'] = trades_sorted['profit_loss'].apply(lambda
            x: 1 if x > 0 else 0).rolling(window_size, min_periods=5).mean()
        rolling_metrics['avg_trade'] = trades_sorted['profit_loss'].rolling(
            window_size, min_periods=5).mean()
        rolling_metrics['sharpe_ratio'] = trades_sorted['profit_loss'].rolling(
            window_size, min_periods=10).apply(lambda x: x.mean() / x.std() *
    ...
    """
        ...
        rolling_metrics['profit_factor'] = trades_sorted['profit_loss'
            ].rolling(window_size, min_periods=10).apply(rolling_profit_factor)
        if pd.api.types.is_datetime64_any_dtype(trades_sorted['entry_time']):
            ...
            date_index = pd.date_range(start=trades_sorted['entry_time'].
                min().date(), end=trades_sorted['entry_time'].max().date())
            daily_metrics = pd.DataFrame(index=date_index)
            daily_metrics['trade_count'] = trade_dates['trade_count']
            daily_metrics['trade_count'] = daily_metrics['trade_count'].fillna(
                0)
            for metric in ['win_rate', 'avg_trade', 'sharpe_ratio',
                'profit_factor']:
                daily_values = rolling_metrics.groupby(rolling_metrics[
                    'entry_time'].dt.date)[metric].last()
                daily_values.index = pd.to_datetime(daily_values.index)
                daily_metrics[metric] = daily_values
                daily_metrics[metric] = daily_metrics[metric].fillna(method
                    ='ffill')
            daily_metrics['win_rate_trend'] = daily_metrics['win_rate'
                ].rolling(10, min_periods=5).apply(lambda x: stats.
                linregress(range(len(x)), x)[0])
            daily_metrics['avg_trade_trend'] = daily_metrics['avg_trade'
                ].rolling(10, min_periods=5).apply(lambda x: stats.
                linregress(range(len(x)), x)[0])
            daily_metrics['decay_warning'] = (daily_metrics[
                'win_rate_trend'] < 0) & (daily_metrics['avg_trade_trend'] < 0)
            daily_metrics['decay_warning_count'] = daily_metrics[
                'decay_warning'].rolling(7).sum()
            daily_metrics['needs_retraining'] = daily_metrics[
                'decay_warning_count'] >= 5
