...
class LoadTester:
    ...
    def __init__(
        ...
        self.symbols = ["EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "USDCAD", "EURGBP", "EURJPY", "GBPJPY"]
        self.signal_types = ["trend", "reversal", "breakout"]
        self.signal_directions = ["bullish", "bearish"]
        self.timeframes = ["M15", "M30", "H1", "H4", "D1"]
        
        logger.info(f"Initialized load tester with {users} users, {duration}s duration, {spawn_rate} spawn rate")
    ...
    def run(self):
        ...
        with ThreadPoolExecutor(max_workers=self.users) as executor:
            ...
            # Spawn users at the specified rate
            ...
            with tqdm(total=self.duration, desc="Load Test Progress") as pbar:
                while time.time() < end_time:
                    ...
                    if current_time >= next_spawn_time and spawned_users < self.users:
                        futures.append(executor.submit(self._user_task))
                        spawned_users += 1
    ...
    def _user_task(self):
        ...
        while time.time() < time.time() + self.duration:
            ...
            endpoint = random.choice(["confluence", "divergence", "currency_strength", "related_pairs"])
            ...
            # Random sleep between requests (0.5-2.0 seconds)
    ...
    def _test_confluence(self):
        ...
        # Record result
        self.results["confluence"].append({
    ...
    def _test_divergence(self):
        ...
        # Record result
        self.results["divergence"].append({
            "response_time": response_time,
            "status_code": response.status_code,
            "timestamp": time.time()
        })
    ...
    def _test_currency_strength(self):
        ...
        # Record result
        self.results["currency_strength"].append({
            "response_time": response_time,
            "status_code": response.status_code,
            "timestamp": time.time()
        })
    ...
    def _test_related_pairs(self):
        ...
        # Record result
        self.results["related_pairs"].append({
            "response_time": response_time,
            "status_code": response.status_code,
            "timestamp": time.time()
        })
    ...
    def _calculate_results(self):
        ...
        for endpoint, results in self.results.items():
            ...
            plt.axvline(stats[endpoint]["mean"], color='r', linestyle='dashed', linewidth=1, label=f"Mean: {stats[endpoint]['mean']:.3f}s")
            plt.axvline(stats[endpoint]["p95"], color='g', linestyle='dashed', linewidth=1, label=f"95th: {stats[endpoint]['p95']:.3f}s")
            plt.axvline(stats[endpoint]["p99"], color='b', linestyle='dashed', linewidth=1, label=f"99th: {stats[endpoint]['p99']:.3f}s")
            plt.title(f"Response Time Distribution - {endpoint.capitalize()}")
            plt.xlabel("Response Time (seconds)")
            plt.ylabel("Count")
            plt.legend()
            plt.grid(True, alpha=0.3)
            plt.savefig(f"performance_results/{endpoint}_response_time_distribution.png")
            ...
            plt.grid(True, alpha=0.3)
            plt.savefig(f"performance_results/{endpoint}_response_time_over_time.png")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Load test the Analysis Engine API")
    ...
    parser.add_argument("--spawn-rate", type=float, default=1, help="User spawn rate per second")
