...
from common_lib.multi_asset.interfaces import AssetClassEnum as AssetClass, IAssetRegistry
from common_lib.multi_asset.interfaces import MarketTypeEnum as MarketType
...
class AssetConfig(BaseModel):
    ...
    min_price_precision: int = Field(..., description=
        'Minimum price precision (decimal places)')
    pip_value: float = Field(..., description='Value of 1 pip in price units')
    pip_location: int = Field(..., description=
        'Decimal position of pip (e.g., -4 for 4 decimal places)')
    min_quantity: float = Field(..., description='Minimum tradable quantity')
    quantity_precision: int = Field(..., description=
        'Quantity precision (decimal places)')
    margin_rate: Optional[float] = Field(None, description=
        'Margin rate if applicable')
    trading_hours: Optional[Dict[str, Any]] = Field(None, description=
        'Trading hours configuration')
    lot_size: Optional[float] = Field(None, description=
        'Standard lot size if applicable')
    point_value: Optional[float] = Field(None, description=
        'Value of 1 point in account currency')
    tick_size: Optional[float] = Field(None, description=
        'Minimum price movement')
    trading_fee: Optional[float] = Field(None, description=
        'Trading fee percentage')
    swap_rates: Optional[Dict[str, float]] = Field(None, description=
        'Overnight swap rates for long/short positions')
...
class AssetDefinition(BaseModel):
    ...
    base_currency: Optional[str] = Field(None, description=
        'Base currency for forex/crypto pairs')
    quote_currency: Optional[str] = Field(None, description=
        'Quote currency for forex/crypto pairs')
    config: AssetConfig
    enabled: bool = True
...
class AssetRegistry(IAssetRegistry):
    ...
    Implements the IAssetRegistry interface from common-lib.
    """
    ...
    def load_config(self, config_path: str) ->bool:
        ...
        try:
            ...
            self.logger.info(
                f'Loaded {len(self.assets)} assets and {len(self.correlations)} correlations'
                )
            return True
    ...
    def register_asset(self, asset: AssetDefinition) ->None:
        ...
        self.logger.info(
            f'Registered asset: {asset.symbol} ({asset.display_name})')
    ...
    def list_assets(self, asset_class: Optional[AssetClass]=None,
        market_type: Optional[MarketType]=None, currency: Optional[str]=None
        ...
        if currency:
            result = [a for a in result if a.base_currency == currency or a
    ...
    def get_correlated_assets(self, symbol: str, threshold: float=0.7) ->List[
        ...
        for key, correlations in self.correlations.items():
            ...
            if abs(most_recent.correlation) >= threshold:
                other_symbol = symbols[0] if symbols[1] == symbol else symbols[
                    1]
                asset = self.get_asset(other_symbol)
                if asset:
                    result.append({'symbol': other_symbol, 'display_name':
                        asset.display_name, 'correlation': most_recent.
                        correlation, 'as_of_date': most_recent.as_of_date,
                        'asset_class': asset.asset_class})
        result.sort(key=lambda x: abs(x['correlation']), reverse=True)
        return result

    @with_resilience('create_asset_group')
    def create_asset_group(self, group_name: str, symbols: List[str]) ->bool:
        ...
        if not all(symbol in self.assets for symbol in symbols):
            self.logger.error(
                f'Cannot create group {group_name}: not all symbols are registered'
                )
            return False
        self._asset_groups[group_name] = set(symbols)
        return True
    ...
    def get_asset_group(self, group_name: str) ->List[AssetDefinition]:
        ...
        if group_name not in self._asset_groups:
            return []
        return [self.assets[symbol] for symbol in self._asset_groups[
            group_name] if symbol in self.assets]
    ...
    def get_trading_parameters(self, symbol: str) ->Dict[str, Any]:
        ...
        if not asset:
            return {}
        return {'min_quantity': asset.config.min_quantity,
            'quantity_precision': asset.config.quantity_precision,
            'price_precision': asset.config.min_price_precision,
            'pip_value': asset.config.pip_value, 'lot_size': asset.config.
            lot_size, 'margin_rate': asset.config.margin_rate,
