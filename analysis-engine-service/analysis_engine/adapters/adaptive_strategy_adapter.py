...
class AdaptiveStrategyServiceAdapter(IAdaptiveStrategyService):
    ...
    async def get_adaptive_parameters(
        ...
        if self.service:
            try:
                return await self.service.get_adaptive_parameters(
                    symbol=symbol,
                    timeframe=timeframe,
    ...
    async def record_strategy_performance(
        ...
        # Log performance metrics if no service is available
        logger.info(
            f"Strategy performance recorded (adapter): {strategy_id}, "
    ...
    async def run_adaptation_cycle(
        ...
        return {
            ...
            "symbol": symbol,
            "adaptations_made": 0,
            "status": "not_implemented_in_adapter"
        }
    ...
    async def get_adaptation_recommendations(
        ...
        if self.service:
            try:
                return await self.service.get_adaptation_recommendations(
                    symbol=symbol,
                    timeframe=timeframe,
