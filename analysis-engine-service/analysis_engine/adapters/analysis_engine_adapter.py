...
class AnalysisEngineAdapter(IAnalysisEngine):
    ...
    async def run_analysis(
        ...
        try:
            if parameters:
                analyzer = await self.analysis_service.get_analyzer(analyzer_name, parameters)
            else:
                analyzer = await self.analysis_service.get_analyzer(analyzer_name)
            
            result = await self.analysis_service.run_analysis(analyzer_name, market_data)
            return result
    ...
    async def get_confluence_analysis(
        ...
        """
        try:
            confluence_result = await self.analysis_service.run_confluence_analysis(
    ...
    async def analyze_pattern_effectiveness(
        ...
        """
        try:
            effectiveness = await self.tool_effectiveness_service.analyze_effectiveness(
                tool_id=pattern_name,
                instrument=symbol,
                timeframe=timeframe,
                from_date=from_date,
                to_date=to_date
            ...
            logger.error(f"Error analyzing pattern effectiveness for {pattern_name}: {str(e)}")
            return {
                "error": str(e),
