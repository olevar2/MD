...
class IndicatorProviderAdapter(IIndicatorProvider):
    ...
    async def calculate_indicator(
        ...
        try:
            ...
            if not indicator_name:
                raise ValidationError("Indicator name cannot be empty", field="indicator_name")
            ...
            if "not found" in str(e).lower() or "unknown indicator" in str(e).lower():
                raise DataError(
                    f"Unknown indicator: {indicator_name}",
                    error_code=ErrorCode.DATA_MISSING_ERROR,
                    data_source="indicator_service",
                    data_type="indicator",
                    cause=e
                )
            elif "invalid data" in str(e).lower():
                raise DataError(
                    f"Invalid data for indicator {indicator_name}",
                ...
                raise ServiceError(
                    ...
                    service_name="indicator_service",
    ...
    async def calculate_multiple_indicators(
        ...
        try:
            ...
            # Call the service method for each indicator
            result = {}
            for indicator_info in indicators:
                indicator_name = indicator_info.get("name")
                if not indicator_name:
                    raise ValidationError("Indicator name cannot be empty", field="name")
                
                params = indicator_info.get("parameters", {})
                
                indicator_result = await self._indicator_service.calculate_indicator(
                    indicator_name=indicator_name,
                    data=data,
                    parameters=params
                )
            ...
            if "not found" in str(e).lower() or "unknown indicator" in str(e).lower():
                raise DataError(
                    f"Unknown indicator in the list",
                    error_code=ErrorCode.DATA_MISSING_ERROR,
                    data_source="indicator_service",
                    data_type="indicator",
                    cause=e
                ...
                raise ServiceError(
                    ...
                    error_code=ErrorCode.SERVICE_UNAVAILABLE,
                    service_name="indicator_service",
                    operation="calculate_multiple_indicators",
                    cause=e
                )
    ...
    async def get_indicator_info(self, indicator_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Call the service method
            result = await self._indicator_service.get_indicator_info(indicator_name=indicator_name)
    ...
    async def get_all_indicators_info(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Get detailed info for each indicator
            result = []
            for indicator_name in indicators:
                indicator_info = await self._indicator_service.get_indicator_info(indicator_name=indicator_name)
            ...
            raise ServiceError(
                f"Error getting all indicators info: {str(e)}",
                error_code=ErrorCode.SERVICE_UNAVAILABLE,
                service_name="indicator_service",
                operation="get_all_indicators_info",
                cause=e
            )
