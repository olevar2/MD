...
class IncrementalUpdater(Generic[T, U]):
    ...
    def __init__(
        self,
        calculate_func: Callable[[T], U],
        update_func: Callable[[U, T], U],
        max_incremental_updates: int = 10
    ):
        """
    ...
    def update(self, data: T) -> U:
        ...
        # Check if we need to recalculate from scratch
...
class DataFrameUpdater:
    ...
    def __init__(
        self,
        calculate_func: Callable[[pd.DataFrame], Any],
        max_incremental_updates: int = 10,
    ...
    def update(self, data: pd.DataFrame) -> Any:
        ...
        # Update last data
        self.last_data = pd.concat([self.last_data, new_rows]).drop_duplicates(subset=[self.timestamp_column])
        self.update_count += 1
        
        logger.debug(f"Incremental update {self.update_count} with {len(new_rows)} new rows")
    ...
    def _get_new_rows(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.timestamp_column not in data.columns:
            logger.warning(f"Timestamp column {self.timestamp_column} not found in data")
            return pd.DataFrame()
    ...
    def _update_incrementally(self, new_rows: pd.DataFrame) -> Any:
        ...
        Returns:
            Updated result
        """
        # Default implementation: recalculate from scratch
        # Subclasses should override this method with more efficient implementations
...
class TechnicalIndicatorUpdater(DataFrameUpdater):
    ...
    def _update_incrementally(self, new_rows: pd.DataFrame) -> pd.Series:
        ...
        last_values = self.last_data.tail(self.window)[self.price_column].values
        ...
        temp_df = pd.DataFrame({
            self.price_column: combined_values
        })
        
        # Calculate the indicator for the combined values
        indicator = self.calculate_func(temp_df)
        
        # Get the new indicator values
...
class MovingAverageUpdater(TechnicalIndicatorUpdater):
    ...
    def _update_incrementally(self, new_rows: pd.DataFrame) -> pd.Series:
        ...
        # Calculate new moving average values
        new_ma_values = []
        ...
        for i, new_value in enumerate(new_values):
            ...
            # Update last_ma for the next iteration
            last_ma = new_ma
        
        # Create a new Series for the new values
        new_ma = pd.Series(
            new_ma_values,
            index=new_rows.index
        )
...
class RSIUpdater(TechnicalIndicatorUpdater):
    ...
    def _update_incrementally(self, new_rows: pd.DataFrame) -> pd.Series:
        ...
        for new_price in new_values:
            ...
            self.last_avg_gain = (self.last_avg_gain * (self.window - 1) + gain) / self.window
            self.last_avg_loss = (self.last_avg_loss * (self.window - 1) + loss) / self.window
            
            # Calculate RS and RSI
            rs = self.last_avg_gain / self.last_avg_loss if self.last_avg_loss != 0 else float('inf')
            rsi = 100 - (100 / (1 + rs))
