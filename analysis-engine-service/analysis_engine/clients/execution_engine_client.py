...
from core_foundations.exceptions.client_exceptions import ExecutionEngineConnectionError, ExecutionEngineTimeoutError, ExecutionEngineAuthError, StrategyDeploymentError, StrategyParameterUpdateError
...
class ExecutionEngineClient:
    ...
    def __init__(self, config_manager: Optional[ConfigurationManager]=None):
        ...
        self.base_url = self.config.get('base_url',
            'http://strategy-execution-engine:8080')
        self.auth_token = self.config_manager.get('auth_token')
        self.timeout = self.config_manager.get('timeout', 30.0)
        self.circuit_breaker = CircuitBreaker(failure_threshold=self.config
            .get('circuit_breaker_failure_threshold', 5), recovery_timeout=
            self.config_manager.get('circuit_breaker_recovery_timeout', 60), name=
        ...
        self.retryable_exceptions = [ExecutionEngineConnectionError,
            ExecutionEngineTimeoutError]
        logger.info(
            f'ExecutionEngineClient initialized with base URL: {self.base_url}'
            )
    ...
    def _load_config(self) ->Dict[str, Any]:
        ...
        default_config = {'base_url':
            'http://strategy-execution-engine:8080', 'timeout': 30.0,
            'circuit_breaker_failure_threshold': 5,
            'circuit_breaker_recovery_timeout': 60, 'max_retries': 3,
            'backoff_factor': 1.5, 'max_backoff': 30}
        if self.config_manager:
            try:
                execution_config = self.config_manager.get_config(
                    'execution_engine_client')
                if execution_config:
                    return {**default_config, **execution_config}
            except Exception as e:
                logger.warning(
                    f'Failed to load execution engine client config: {e}')
    ...
    async def _make_request(self, method: str, endpoint: str, data:
        ...
        if self.circuit_breaker.state == CircuitBreakerState.OPEN:
            logger.warning(
                'Circuit breaker is open. Refusing to make request to Strategy Execution Engine.'
                )
        ...
        try:
            ...
            if response.status_code == 401 or response.status_code == 403:
                raise ExecutionEngineAuthError(
            ...
            raise ExecutionEngineConnectionError(
                f'Failed to connect to Strategy Execution Engine: {e}')
        except httpx.TimeoutException as e:
            self.circuit_breaker.record_failure()
            logger.error(
                f'Timeout connecting to Strategy Execution Engine: {e}')
            raise ExecutionEngineTimeoutError(
                f'Request to Strategy Execution Engine timed out: {e}')
        except Exception as e:
            if not isinstance(e, (ExecutionEngineAuthError,
                StrategyDeploymentError, StrategyParameterUpdateError)):
                self.circuit_breaker.record_failure()
            logger.error(
                f'Error making request to Strategy Execution Engine: {e}')
            raise

    @async_retry(retry_config_attr='retry_config')
    @async_with_exception_handling
    async def set_strategy_parameter(self, strategy_id: str, parameter_name:
        str, new_value: Any) ->bool:
        """
        ...
        """
        logger.info(
            f"Updating parameter '{parameter_name}' for strategy {strategy_id} to {new_value}"
            )
        update_data = {'parameter_name': parameter_name, 'value': new_value,
            'updated_at': datetime.utcnow().isoformat(), 'update_id': str(
            uuid.uuid4())}
        try:
            response = await self._make_request('PUT',
                f'/api/v1/strategies/{strategy_id}/parameters', update_data)
            success = response.get('success', False)
            if success:
                logger.info(
                    f"Successfully updated parameter '{parameter_name}' for strategy {strategy_id}"
                    )
            else:
                logger.warning(
                    f"Failed to update parameter '{parameter_name}' for strategy {strategy_id}: {response.get('message', 'Unknown error')}"
                    )
            return success
        except Exception as e:
            logger.error(
                f'Error updating parameter for strategy {strategy_id}: {e}')
            if isinstance(e, StrategyParameterUpdateError):
                raise
            raise StrategyParameterUpdateError(
                f'Failed to update parameter: {e}')

    @async_retry(retry_config_attr='retry_config')
    @async_with_exception_handling
    async def deploy_strategy(self, strategy_id: str, config: Dict[str, Any]
        ) ->Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Deploying strategy {strategy_id} with config version {config_manager.get('version', 'unknown')}"
            )
        deployment_data = {'strategy_id': strategy_id, 'config': config,
            'deployment_id': str(uuid.uuid4()), 'deployed_at': datetime.
            utcnow().isoformat()}
        try:
            response = await self._make_request('POST',
                '/api/v1/strategies/deploy', deployment_data)
            if response.get('success', False):
                ...
                raise StrategyDeploymentError(
                    f'Failed to deploy strategy: {error_msg}')
        except Exception as e:
    ...
    @with_resilience('get_strategy_status')
    @async_with_exception_handling
    async def get_strategy_status(self, strategy_id: str) ->Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f'Error getting status for strategy {strategy_id}: {e}')
            raise

    @async_with_exception_handling
    async def list_strategies(self, status: Optional[str]=None, asset_class:
    ...
    async def pause_strategy(self, strategy_id: str) ->bool:
        ...
        try:
            response = await self._make_request('PUT',
                f'/api/v1/strategies/{strategy_id}/pause', {'paused_at':
    ...
    async def resume_strategy(self, strategy_id: str) ->bool:
        ...
        try:
            response = await self._make_request('PUT',
                f'/api/v1/strategies/{strategy_id}/resume', {'resumed_at':
                datetime.utcnow().isoformat()})
            return response.get('success', False)
        except Exception as e:
