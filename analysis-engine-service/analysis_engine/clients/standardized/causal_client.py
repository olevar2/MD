...
class CausalClient:
    ...
    def __init__(self, base_url: Optional[str] = None, timeout: int = 30):
        ...
        logger.info(f"Initialized Causal Analysis client with base URL: {self.base_url}")
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.request(
                        ...
                        if response.status >= 500:
                            ...
                            logger.error(f"Server error from Causal Analysis API: {error_text}")
                            raise ServiceUnavailableError(f"Causal Analysis API server error: {response.status}")
                        
                        if response.status >= 400:
                            error_text = await response.text()
                            logger.error(f"Client error from Causal Analysis API: {error_text}")
                            raise Exception(f"Causal Analysis API client error: {response.status} - {error_text}")
                        
                        return await response.json()
            except aiohttp.ClientError as e:
                logger.error(f"Connection error to Causal Analysis API: {str(e)}")
                raise ServiceUnavailableError(f"Failed to connect to Causal Analysis API: {str(e)}")
            except asyncio.TimeoutError:
                logger.error(f"Timeout connecting to Causal Analysis API")
                raise ServiceTimeoutError(f"Timeout connecting to Causal Analysis API")
    ...
    async def discover_structure(
        ...
        return await self._make_request("POST", "/discover-structure", data=request_data)
    ...
    async def estimate_effect(
        ...
        return await self._make_request("POST", "/estimate-effect", data=request_data)
    ...
    async def analyze_counterfactuals(
        ...
        return await self._make_request("POST", "/counterfactual-analysis", data=request_data)
    ...
    async def analyze_currency_pair_relationships(
        ...
        logger.info(f"Analyzing relationships between {len(price_data)} currency pairs")
        return await self._make_request("POST", "/currency-pair-relationships", data=request_data)
    ...
    async def enhance_trading_signals(
        ...
        return await self._make_request("POST", "/enhance-trading-signals", data=request_data)
    ...
    async def validate_relationship(
        ...
        return await self._make_request("POST", "/validate-relationship", data=request_data)
