...
class BacktestingClient:
    ...
    def __init__(self, base_url: Optional[str] = None, timeout: int = 60):
        ...
        self.circuit_breaker = circuit_breaker(
            failure_threshold=5,
            recovery_timeout=30,
            name="backtesting_client"
        )

        logger.info(f"Initialized Backtesting client with base URL: {self.base_url}")
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.request(
                        ...
                        if response.status >= 500:
                            ...
                            logger.error(f"Server error from Backtesting API: {error_text}")
                            raise ServiceUnavailableError(f"Backtesting API server error: {response.status}")

                        if response.status >= 400:
                            error_text = await response.text()
                            logger.error(f"Client error from Backtesting API: {error_text}")
                            raise Exception(f"Backtesting API client error: {response.status} - {error_text}")

                        return await response.json()
            except aiohttp.ClientError as e:
                logger.error(f"Connection error to Backtesting API: {str(e)}")
                raise ServiceUnavailableError(f"Failed to connect to Backtesting API: {str(e)}")
            except asyncio.TimeoutError:
                logger.error(f"Timeout connecting to Backtesting API")
                raise ServiceTimeoutError(f"Timeout connecting to Backtesting API")
    ...
    async def run_backtest(
        ...
        slippage_settings: Optional[Dict[str, Any]] = None,
        data_source: str = "historical",
        data_parameters: Optional[Dict[str, Any]] = None
    ) -> Dict:
        """
        ...
        request_data = {
            ...
            "commission_settings": commission_settings or {},
            "slippage_model": slippage_model,
            "slippage_settings": slippage_settings or {},
            "data_source": data_source,
            "data_parameters": data_parameters or {}
        }

        logger.info(f"Running backtest for strategy {strategy_config.get('strategy_id')}")
    ...
    async def run_walk_forward_optimization(
        ...
        request_data = {
            ...
            "anchor_size_days": anchor_size_days,
            "optimization_metric": optimization_metric,
            "parallel_jobs": parallel_jobs
        }

        logger.info(f"Running walk-forward optimization for strategy {strategy_config.get('strategy_id')}")
        return await self._make_request("POST", "/walk-forward", data=request_data)
    ...
    async def run_monte_carlo_simulation(
        ...
        logger.info(f"Running Monte Carlo simulation for backtest {backtest_id}")
        return await self._make_request("POST", "/monte-carlo", data=request_data)

    async def run_stress_test(
        self,
        backtest_id: str,
        stress_scenarios: List[Dict[str, Any]],
        apply_to_parameters: Optional[List[str]] = None
    ) -> Dict:
        """
        ...
        request_data = {
            ...
            "apply_to_parameters": apply_to_parameters
        }

        logger.info(f"Running stress test for backtest {backtest_id}")
        return await self._make_request("POST", "/stress-test", data=request_data)
