...
class MonitoringClient:
    ...
    def __init__(self, base_url: Optional[str] = None, timeout: int = 30):
        ...
        logger.info(f"Initialized Monitoring client with base URL: {self.base_url}")
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.request(
                        ...
                        if response.status >= 500:
                            ...
                            logger.error(f"Server error from Monitoring API: {error_text}")
                            raise ServiceUnavailableError(f"Monitoring API server error: {response.status}")
                        
                        if response.status >= 400:
                            error_text = await response.text()
                            logger.error(f"Client error from Monitoring API: {error_text}")
                            raise Exception(f"Monitoring API client error: {response.status} - {error_text}")
                        
                        return await response.json()
            except aiohttp.ClientError as e:
                logger.error(f"Connection error to Monitoring API: {str(e)}")
                raise ServiceUnavailableError(f"Failed to connect to Monitoring API: {str(e)}")
            except asyncio.TimeoutError:
                logger.error(f"Timeout connecting to Monitoring API")
                raise ServiceTimeoutError(f"Timeout connecting to Monitoring API")
    ...
    async def get_async_performance_metrics(
        ...
        logger.info(f"Getting async performance metrics for {operation if operation else 'all operations'}")
        return await self._make_request("GET", "/async-performance", params=params)
