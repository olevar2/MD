...
class ManipulationDetectionClient:
    ...
    def __init__(self, base_url: Optional[str] = None, timeout: int = 30):
        ...
        self.circuit_breaker = circuit_breaker(
            failure_threshold=5,
            recovery_timeout=30,
            name="manipulation_detection_client"
        )
        
        logger.info(f"Initialized Manipulation Detection client with base URL: {self.base_url}")
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.request(
                        ...
                        if response.status >= 500:
                            ...
                            logger.error(f"Server error from Manipulation Detection API: {error_text}")
                            raise ServiceUnavailableError(f"Manipulation Detection API server error: {response.status}")
                        
                        if response.status >= 400:
                            error_text = await response.text()
                            logger.error(f"Client error from Manipulation Detection API: {error_text}")
                            raise Exception(f"Manipulation Detection API client error: {response.status} - {error_text}")
                        
                        return await response.json()
            except aiohttp.ClientError as e:
                logger.error(f"Connection error to Manipulation Detection API: {str(e)}")
                raise ServiceUnavailableError(f"Failed to connect to Manipulation Detection API: {str(e)}")
            except asyncio.TimeoutError:
                logger.error(f"Timeout connecting to Manipulation Detection API")
                raise ServiceTimeoutError(f"Timeout connecting to Manipulation Detection API")
    ...
    async def detect_manipulation_patterns(
        ...
        include_protection: Optional[bool] = True
    ) -> Dict:
        """
        ...
        logger.info(f"Detecting manipulation patterns for {metadata.get('symbol')}")
        return await self._make_request("POST", "/detect", data=data, params=params)
    ...
    async def detect_stop_hunting(
        ...
        recovery_threshold: Optional[float] = 0.5
    ) -> Dict:
        """
        ...
        logger.info(f"Detecting stop hunting patterns for {metadata.get('symbol')}")
        return await self._make_request("POST", "/stop-hunting", data=data, params=params)
    ...
    async def detect_fake_breakouts(
        ...
        logger.info(f"Detecting fake breakout patterns for {metadata.get('symbol')}")
        return await self._make_request("POST", "/fake-breakouts", data=data, params=params)
    ...
    async def detect_volume_anomalies(
        ...
        return await self._make_request("POST", "/volume-anomalies", data=data, params=params)
