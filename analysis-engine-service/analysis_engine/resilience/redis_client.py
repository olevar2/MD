...
from analysis_engine.resilience import (
    retry_with_policy, timeout_handler, create_circuit_breaker
)
...
class ResilientRedisClient:
    ...
    def __init__(self, settings: Settings = None):
        ...
        self.redis_socket_connect_timeout = self.settings.redis.socket_connect_timeout
        ...
        self.circuit_breaker = create_circuit_breaker(
            ...
            resource_name="redis",
            config=get_circuit_breaker_config("redis")
        )
    ...
    @retry_with_policy(
        ...
        backoff_factor=2.0,
        jitter=True,
        exceptions=[RedisError, ConnectionError, TimeoutError],
        service_name="analysis_engine",
        operation_name="redis_operation"
    )
    @timeout_handler(timeout_seconds=3.0)
    def execute_redis_operation(self, operation_func: Callable[[Redis], T]) -> T:
        """
        ...
        """
        return self.circuit_breaker.execute(self._execute_redis_operation, operation_func)
    
    def _execute_redis_operation(self, operation_func: Callable[[Redis], T]) -> T:
        """
