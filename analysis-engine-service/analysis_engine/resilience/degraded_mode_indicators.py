...
class DegradedIndicator:
    ...
    def _validate_data(self, data: pd.DataFrame, required_columns: List[str]) -> bool:
        ...
        if missing:
            logger.warning(f"Missing required columns for {self.name}: {missing}")
            return False
...
class DegradedMovingAverage(DegradedIndicator):
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Calculate MA using pandas rolling function for optimal performance
        window = self.params["window"]
        price_col = self.params["price_column"]
        
        try:
            result[f"degraded_ma_{window}"] = result[price_col].rolling(
                window=window, min_periods=1
            ).mean()
            
            # Update cache for future use
            AnalysisEngineDegradedMode.update_cache(
                "indicators", 
                f"ma_{price_col}_{window}", 
                result[f"degraded_ma_{window}"].iloc[-1] if not result.empty else None
            )
...
class DegradedExponentialMovingAverage(DegradedIndicator):
    ...
    def __init__(self, window: int = 14, price_column: str = "close", smoothing: float = 2.0):
        """
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            # Calculate EMA using pandas ewm function for optimal performance
            # The span parameter approximates the window length
            result[f"degraded_ema_{window}"] = result[price_col].ewm(
                span=window, 
                adjust=False
            ).mean()
            ...
            AnalysisEngineDegradedMode.update_cache(
                "indicators", 
                f"ema_{price_col}_{window}", 
                result[f"degraded_ema_{window}"].iloc[-1] if not result.empty else None
            )
        except Exception as e:
...
class DegradedRelativeStrengthIndex(DegradedIndicator):
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self._validate_data(data, [self.params["price_column"]]) or len(data) < self.params["window"]:
            return result
        ...
        try:
            ...
            # For all other values use the smoothed formula:
            # avg_gain = (previous_avg_gain * (window-1) + current_gain) / window
            if len(gain) > window:
            ...
            rsi = rsi.fillna(50)  # Middle value as default
            ...
            AnalysisEngineDegradedMode.update_cache(
                "indicators", 
                f"rsi_{price_col}_{window}", 
                result[f"degraded_rsi_{window}"].iloc[-1] if not result.empty else None
            )
        except Exception as e:
...
class DegradedBollingerBands(DegradedIndicator):
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            middle_band = result[price_col].rolling(window=window, min_periods=1).mean()
            
            # Calculate standard deviation
            std_dev = result[price_col].rolling(window=window, min_periods=1).std()
            ...
            if not result.empty:
                ...
                AnalysisEngineDegradedMode.update_cache(
                    "indicators", 
                    f"bollinger_{price_col}_{window}_{num_std}", 
                    last_values
                )
        except Exception as e:
            logger.error(f"Error calculating degraded Bollinger Bands: {str(e)}")
...
# Register all degraded mode indicators
def register_all_degraded_indicators():
    ...
    # Register indicators as fallbacks for feature-store service
    manager.register_fallback_strategy("feature-store", "get_moving_average", DegradedMovingAverage)
    manager.register_fallback_strategy("feature-store", "get_exponential_moving_average", DegradedExponentialMovingAverage)
    manager.register_fallback_strategy("feature-store", "get_relative_strength_index", DegradedRelativeStrengthIndex)
    manager.register_fallback_strategy("feature-store", "get_bollinger_bands", DegradedBollingerBands)
    manager.register_fallback_strategy("feature-store", "get_macd", DegradedMACD)
