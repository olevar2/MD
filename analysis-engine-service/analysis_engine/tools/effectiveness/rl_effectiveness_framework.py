...
class RLModelEffectivenessAnalyzer:
    ...
    def calculate_regime_specific_metrics(
    ...
    ) -> Dict[MarketRegime, RegimeSpecificPerformance]:
        """
        ...
        """
        logger.info(f"Calculating regime-specific metrics for agent {agent_name}")
        ...
        for regime, actions in regime_groups.items():
            if len(actions) < self.min_sample_size:
                logger.warning(f"Insufficient data for {regime.value} regime (n={len(actions)})")
                continue
                
            # Filter market data for this regime
            regime_indices = [i for i, r in enumerate(regime_labels) if r == regime]
            regime_market_data = market_data.iloc[regime_indices]
            
            # Calculate regime-specific metrics
            metrics = self._calculate_agent_metrics(agent_name, actions, regime_market_data)
            ...
            # Create result
            results[regime] = RegimeSpecificPerformance(
                regime=regime,
                metrics=metrics,
            ...
            # Store data for future baseline calculations
            self.regime_data[regime].append({
    ...
    def calculate_comparative_effectiveness(
        self,
        agent_metrics: Dict[str, RLAgentMetrics],
        baseline_strategy_metrics: Optional[RLAgentMetrics] = None
        ...
        # If baseline is provided, compare each agent to it
        if baseline_strategy_metrics:
            for agent_name, metrics in agent_metrics.items():
                comparison = self._compare_metrics(metrics, baseline_strategy_metrics)
                results[f"{agent_name}_vs_baseline"] = comparison
        
        # Compare agents to each other
        agent_names = list(agent_metrics.keys())
        for i in range(len(agent_names)):
            for j in range(i+1, len(agent_names)):
                name_i, name_j = agent_names[i], agent_names[j]
                comparison = self._compare_metrics(agent_metrics[name_i], agent_metrics[name_j])
                results[f"{name_i}_vs_{name_j}"] = comparison
                
        return results

    def track_effectiveness_decay(
        self,
        agent_name: str,
        lookback_periods: List[int] = [1, 7, 30, 90]
    ) -> Dict[str, List[float]]:
        """
        ...
        if agent_name not in self.historical_data:
            logger.warning(f"No historical data available for agent {agent_name}")
            return {}
        ...
        # Calculate trend for each lookback period
        for period in lookback_periods:
            cutoff_date = datetime.now() - timedelta(days=period)
            period_data = [entry for entry in agent_history 
            ...
            avg_reward = sum(entry["metrics"].avg_reward for entry in period_data) / len(period_data)
            avg_sharpe = sum(entry["metrics"].sharpe_ratio for entry in period_data) / len(period_data)
            avg_win_rate = sum(entry["metrics"].win_rate for entry in period_data) / len(period_data)
            
            results["avg_reward"].append(avg_reward)
            results["sharpe_ratio"].append(avg_sharpe)
            results["win_rate"].append(avg_win_rate)
        
        # Detect significant decay
        if len(results["sharpe_ratio"]) > 1 and all(x is not None for x in results["sharpe_ratio"]):
            # Check if there's a consistent downward trend
            if all(results["sharpe_ratio"][i] > results["sharpe_ratio"][i+1] 
                   for i in range(len(results["sharpe_ratio"])-1)):
                results["decay_detected"] = True
                logger.warning(f"Effectiveness decay detected for agent {agent_name}")
            
        return results

    def analyze_action_quality(
        self,
        agent_name: str,
        agent_actions: List[Dict[str, Any]],
        market_data: pd.DataFrame,
        ...
        confidences = [action.get("confidence", 0.5) for action in agent_actions]
        outcomes = [action.get("outcome", 0) for action in agent_actions]
        
        # Separate high and low confidence actions
        ...
        # Success rates
        results["overall_success_rate"] = np.mean([o > 0 for o in outcomes]) if outcomes else 0
        results["high_conf_success_rate"] = np.mean([o > 0 for o in high_conf_outcomes]) if high_conf_outcomes else 0
        results["low_conf_success_rate"] = np.mean([o > 0 for o in low_conf_outcomes]) if low_conf_outcomes else 0
        
        # Average returns
        results["overall_avg_return"] = np.mean(outcomes) if outcomes else 0
        results["high_conf_avg_return"] = np.mean(high_conf_outcomes) if high_conf_outcomes else 0
        results["low_conf_avg_return"] = np.mean(low_conf_outcomes) if low_conf_outcomes else 0
        
        # Confidence calibration (how well confidence predicts outcomes)
        if len(confidences) > self.min_sample_size and len(set(confidences)) > 1:
            # Bin confidence scores
            bins = np.linspace(0, 1, 11)  # 0.0, 0.1, 0.2, ..., 1.0
            binned_conf = np.digitize(confidences, bins) - 1
            binned_conf = [min(b, 9) for b in binned_conf]  # Ensure no out of bounds
            
            # Calculate success rate for each bin
            bin_success_rates = []
            for bin_idx in range(10):
                bin_outcomes = [outcomes[i] for i, b in enumerate(binned_conf) if b == bin_idx]
                if bin_outcomes:
            ...
            results["brier_score"] = np.mean([(confidences[i] - binary_outcomes[i])**2 
    ...
    def generate_effectiveness_report(
        ...
        if cutoff_date:
            filtered_data = [entry for entry in self.historical_data[agent_name] 
                            if entry["timestamp"] >= cutoff_date]
        else:
            filtered_data = self.historical_data[agent_name]
            
        if not filtered_data:
            return {"error": f"No data available for {agent_name} in the specified time period"}
        ...
        report = {
            ...
            "overall_metrics": self._calculate_aggregate_metrics(filtered_data),
            "regime_performance": self._get_regime_performance(agent_name, filtered_data),
            "effectiveness_trend": self.track_effectiveness_decay(agent_name),
        }
        
        # Add visualization data if requested
        if include_visualizations:
            report["visualization_data"] = self._prepare_visualization_data(filtered_data)
        ...
        try:
            ...
            if file_path.endswith('.json'):
                with open(file_path, 'w') as f:
                    json.dump(report, f, indent=4, default=self._json_serializer)
    ...
    def update_historical_data(
        ...
        self.historical_data[agent_name].append({
            "timestamp": datetime.now(),
            "metrics": metrics
        })
        ...
        if len(self.historical_data[agent_name]) > 1000:
            ...
            self.historical_data[agent_name] = self.historical_data[agent_name][-1000:]
    ...
    def _calculate_agent_metrics(
        ...
        avg_position_size = np.mean([abs(ps) for ps in position_sizes]) if position_sizes else 0
        
        return RLAgentMetrics(
            name=agent_name,
    ...
    def _calculate_statistical_significance(
        self,
        metrics: RLAgentMetrics,
        baseline_metrics: Optional[RLAgentMetrics]
    ) -> Optional[float]:
        """Calculate statistical significance versus baseline."""
        if not baseline_metrics:
            return None
            
        # Use t-test for significance testing
        # In a real implementation, we would need actual distributions of returns/rewards
        # This is a simplified placeholder
        return 0.03  # Simulated p-value
    ...
    def _get_baseline_metrics(
        ...
        # Otherwise, retrieve from historical data
    ...
    def _compare_metrics(
        self,
        metrics_a: RLAgentMetrics,
        metrics_b: RLAgentMetrics
    ) -> Dict[str, float]:
        """Compare two sets of metrics."""
        return {
            "avg_reward_diff": metrics_a.avg_reward - metrics_b.avg_reward,
            "sharpe_ratio_diff": metrics_a.sharpe_ratio - metrics_b.sharpe_ratio,
            "sortino_ratio_diff": metrics_a.sortino_ratio - metrics_b.sortino_ratio,
            "win_rate_diff": metrics_a.win_rate - metrics_b.win_rate,
            "max_drawdown_diff": metrics_a.max_drawdown - metrics_b.max_drawdown,
        }
        
    def _calculate_aggregate_metrics(self, data: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        result = {
            ...
            "avg_sortino_ratio": np.mean([m.sortino_ratio for m in metrics_list]),
    ...
    def _get_regime_performance(
        ...
        # This would require regime information in the data
        # Simplified implementation
        return {
            "trending": {"avg_reward": 0.5, "win_rate": 0.6},
            "ranging": {"avg_reward": 0.3, "win_rate": 0.5},
            "volatile": {"avg_reward": 0.2, "win_rate": 0.4},
        }
        
    def _prepare_visualization_data(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Prepare data for visualizations."""
        # Simplified implementation
        return {
            "timestamps": [entry["timestamp"].isoformat() for entry in data],
            "rewards": [entry["metrics"].avg_reward for entry in data],
            "sharpe_ratios": [entry["metrics"].sharpe_ratio for entry in data],
            "win_rates": [entry["metrics"].win_rate for entry in data],
        }
        
    def _convert_report_to_html(self, report: Dict[str, Any]) -> str:
        """Convert report to HTML format."""
        # This would require an HTML template and rendering logic
        # Simplified implementation
        html = f"""
        <html>
        <head>
...
class RLToolEffectivenessIntegration(BaseEffectivenessMetric):
    ...
    def __init__(self, config: Dict[str, Any]):
        super().__init__(name="rl_model_effectiveness", description="RL model effectiveness metrics")
        self.analyzer = RLModelEffectivenessAnalyzer(
    ...
    def calculate_effectiveness(
        ...
        # Convert decisions and outcomes to agent actions format
        agent_actions = self._convert_to_agent_actions(decisions, outcomes)
        
        # Extract regime labels from context
        regime_labels = context.get("regime_labels", [])
        if not regime_labels:
            # Default to unknown regime if not provided
        ...
        agent_metrics = RLAgentMetrics(
            ...
            avg_trade_duration=np.mean([a.get("duration", 0) for a in agent_actions]) if agent_actions else 0,
            avg_position_size=np.mean([abs(a.get("position_size", 0)) for a in agent_actions]) if agent_actions else 0,
            action_distribution=self._calculate_action_distribution(agent_actions),
        ...
        # Create standardized effectiveness metrics
        return {
            "overall_effectiveness": self._calculate_overall_effectiveness(agent_metrics, action_quality),
            "regime_effectiveness": self._calculate_regime_effectiveness(regime_metrics),
            "confidence_calibration": action_quality.get("brier_score", 1.0),
            "risk_adjusted_return": agent_metrics.sharpe_ratio,
            "win_rate": agent_metrics.win_rate,
            # Add more standard metrics as needed
        }
        
    def _convert_to_agent_actions(
        self,
        decisions: List[Dict[str, Any]],
        outcomes: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        for i, decision in enumerate(decisions):
            ...
            if i < len(outcomes):
                ...
                action.update({
                    ...
                    "return": outcome.get("return", 0),
                    "duration": outcome.get("duration", 0),
                    "outcome": outcome.get("outcome", 0)
                })
    ...
    def _calculate_action_distribution(self, actions: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        return {
            action_type: action_types.count(action_type) / len(action_types)
    ...
    def _calculate_overall_effectiveness(
        ...
        components = [
            ...
            (1 - metrics.max_drawdown) * 0.2,  # 20% weight on drawdown (inverse)
            action_quality.get("high_conf_success_rate", 0) * 0.15,  # 15% weight on high confidence success
            (1 - action_quality.get("brier_score", 1.0)) * 0.15  # 15% weight on confidence calibration
        ]
        
        # Normalize to 0-1 range
        effectiveness = sum(components)
        return min(max(effectiveness, 0), 1)  # Clamp to [0, 1]
        
    def _calculate_regime_effectiveness(
        self,
        regime_metrics: Dict[MarketRegime, RegimeSpecificPerformance]
    ) -> Dict[str, float]:
        """Calculate effectiveness scores by regime."""
        result = {}
        
        for regime, performance in regime_metrics.items():
            # Calculate regime-specific score (simplified)
...
class ReinforcementLearningAnalyticsService:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.tool_effectiveness_integration = RLToolEffectivenessIntegration(config)
    ...
    def calculate_decision_attribution(
        ...
        # Get top drivers (top 5 features)
        ...
        # Analyze feature attribution for each decision type
        decision_types = set(d.get("action", "unknown") for d in decisions)
        
        for decision_type in decision_types:
            # Get indices of this decision type
            indices = [
                i for i, d in enumerate(decisions)
                if d.get("action", "unknown") == decision_type
            ]
            
            # Calculate average importance for this decision type
            type_importances = {}
            for feature, values in feature_importances.items():
                if indices:
                    # Get importance values for this decision type
                    type_values = [values[i] for i in indices if i < len(values)]
                    type_importances[feature] = np.mean(type_values) if type_values else 0
                    
            # Sort and get top features for this type
            sorted_type_features = sorted(
                type_importances.items(),
                key=lambda x: x[1],
                reverse=True
            )[:5]  # Top 5 features
            
            results["feature_attribution"][decision_type] = [
                {"feature": feature, "importance": importance}
                for feature, importance in sorted_type_features
            ]
    ...
    def compare_agent_performance(
        self,
        agent_metrics: Dict[str, RLAgentMetrics],
        baseline_metrics: Optional[RLAgentMetrics] = None,
        regime_data: Optional[Dict[str, List[Dict[str, Any]]]] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        # Basic comparison
        comparison = self.effectiveness_analyzer.calculate_comparative_effectiveness(
            agent_metrics,
            baseline_metrics
        )
        ...
        results = {
            ...
            "key_metrics_comparison": self._compare_key_metrics(agent_metrics, baseline_metrics),
            "statistical_significance": {},
        }
        
        # Add regime-specific comparison if data is available
        if regime_data:
            regime_comparison = {}
            for regime, regime_metrics in regime_data.items():
                regime_comparison[regime] = self._compare_regime_performance(
                    regime,
                    {agent: metrics.get(regime, {}) for agent, metrics in regime_data.items()}
                )
        ...
        if len(agent_names) > 1:
            ...
            for i in range(len(agent_names)):
                for j in range(i+1, len(agent_names)):
                    name_i, name_j = agent_names[i], agent_names[j]
                    # Simulated p-value
                    results["statistical_significance"][f"{name_i}_vs_{name_j}"] = 0.03
    ...
    def generate_analytics_dashboard_data(
        ...
        """
        # Generate effectiveness report as base
        ...
        # Enhance with additional dashboard data
        dashboard_data = {
            "agent_name": agent_name,
            "time_period": time_period,
            "overview_metrics": report.get("overall_metrics", {}),
        ...
        # Add metrics history if provided
        if metrics_history:
            dashboard_data["metrics_history"] = self._prepare_metrics_history(metrics_history)
            
        # Add predictions for key metrics (simplified implementation)
        dashboard_data["predictions"] = {
            "next_7_days": {
                "avg_reward": report.get("overall_metrics", {}).get("avg_reward", 0) * 1.05,
                "win_rate": min(report.get("overall_metrics", {}).get("avg_win_rate", 0) * 1.02, 1.0),
            }
        }
    ...
    def save_analytics_report(
        ...
        """
        try:
            # Apply report template if available
            ...
            if format == "json":
                ...
                html_content = self._create_html_report(report_data, report_type)
                with open(output_path, 'w') as f:
                    f.write(html_content)
            elif format == "pdf":
    ...
    def _compare_key_metrics(
        ...
        key_metrics = [
            "avg_reward", "sharpe_ratio", "sortino_ratio", 
        ...
        for metric in key_metrics:
            ...
            # Get metric values for each agent
    ...
    def _load_report_templates(self) -> Dict[str, Any]:
        ...
        # This would load actual templates from files or a template repository
        # Simplified implementation with hardcoded templates
        return {
            "effectiveness": {
                "sections": [
                    "overview",
    ...
    def _apply_report_template(
        ...
        # This would filter and structure the report according to the template
        # Simplified implementation that returns original data
        return report_data
    ...
    def _create_html_report(
        ...
        # This would use a proper HTML templating engine
        # Simplified implementation
        html = f"""
        <!DOCTYPE html>
        <html>
