...
class TimeframeFeedbackCollector:
    ...
    async def collect_feedback(self, feedback: TradeFeedback) ->str:
        ...
        try:
            ...
            if not instrument:
                logger.warning(
                    f"Missing instrument in feedback {feedback.id}, using 'unknown'"
                    )
                instrument = 'unknown'
            if not timeframe:
                logger.warning(
                    f"Missing timeframe in feedback {feedback.id}, using 'unknown'"
                    )
                timeframe = 'unknown'
            ...
            if len(feedback_list) > self.max_items_per_timeframe:
                ...
                self.recent_feedback[instrument][timeframe] = feedback_list[
                    -self.max_items_per_timeframe:]
            if self.event_publisher:
                await self.event_publisher.publish(
                    'feedback.timeframe.collected', {'feedback_id':
                    feedback.id, 'instrument': instrument, 'timeframe':
                    timeframe, 'timestamp': feedback.timestamp})
            logger.debug(f'Collected {timeframe} feedback for {instrument}')
            return feedback.id
        except Exception as e:
            logger.error(f'Error collecting timeframe feedback: {str(e)}',
                exc_info=True)
            raise TimeframeFeedbackError(
                f'Failed to collect timeframe feedback: {str(e)}')

    @with_database_resilience('get_feedback_by_timeframe')
    @async_with_exception_handling
    async def get_feedback_by_timeframe(self, instrument: str, timeframe:
        Optional[str]=None, start_time: Optional[datetime]=None, end_time:
        Optional[datetime]=None, limit: int=100) ->Dict[str, Any]:
        """
        ...
        try:
            ...
            result = {'instrument': instrument, 'status': 'success',
                'timeframes': {}, 'query_time': datetime.utcnow().isoformat()}
            timeframes_to_check = []
            if timeframe:
                if timeframe in self.recent_feedback[instrument]:
                    timeframes_to_check = [timeframe]
                else:
                    return {'instrument': instrument, 'status':
                        'timeframe_not_found', 'available_timeframes': list
                        (self.recent_feedback[instrument].keys())}
            else:
                timeframes_to_check = list(self.recent_feedback[instrument]
                    .keys())
            for tf in timeframes_to_check:
                ...
                if start_time or end_time:
                    ...
                    for fb in self.recent_feedback[instrument][tf]:
                        try:
                            ...
                            continue
                filtered_feedback = sorted(filtered_feedback, key=lambda fb:
                    fb.timestamp if hasattr(fb, 'timestamp') else '',
                    reverse=True)
                filtered_feedback = filtered_feedback[:limit]
                feedback_data = []
                for fb in filtered_feedback:
                    item = {'id': fb.id, 'timestamp': fb.timestamp,
                        'source': fb.source.value if hasattr(fb, 'source') else
                        'unknown', 'category': fb.category.value if hasattr
                        (fb, 'category') else 'unknown'}
                ...
                error_magnitudes = [fb.error_magnitude for fb in
                    filtered_feedback if hasattr(fb, 'error_magnitude') and
                    fb.error_magnitude is not None]
                stats = {'count': len(filtered_feedback), 'has_error_data':
                    len(error_magnitudes) > 0}
                if error_magnitudes:
                    stats['avg_error'] = sum(error_magnitudes) / len(
                        error_magnitudes)
                    ...
                    if len(error_magnitudes) > 1:
                        ...
                        variance = sum((x - mean) ** 2 for x in
                            error_magnitudes) / len(error_magnitudes)
                        stats['std_dev'] = variance ** 0.5
                result['timeframes'][tf] = {'feedback': feedback_data,
                    'statistics': stats}
            return result
        except Exception as e:
            logger.error(f'Error getting feedback by timeframe: {str(e)}',
                exc_info=True)
            raise TimeframeFeedbackError(
                f'Failed to get feedback by timeframe: {str(e)}')
