...
class StatisticalValidator:
    ...
    def __init__(self, 
        ...
        self.correlation_thresholds = correlation_thresholds or {
            ...
            "strong": 0.7,    # 0.4 - 0.7
            "very_strong": 1.0  # 0.7 - 1.0
        }
        self.logger = logger
    
    def validate_prediction_improvements(self, 
        ...
        """
        if len(baseline_errors) < self.min_samples or len(new_errors) < self.min_samples:
            return {
                "is_significant": False,
                "reason": f"Insufficient samples (need at least {self.min_samples})",
                "p_value": None,
        ...
        improvement_percentage = (mean_improvement / baseline_mean) * 100 if baseline_mean != 0 else 0
        
        # Perform statistical test (paired t-test if same length, otherwise independent t-test)
        if len(baseline_errors) == len(new_errors):
            t_stat, p_value = stats.ttest_rel(baseline_errors, new_errors)
        else:
            t_stat, p_value = stats.ttest_ind(baseline_errors, new_errors, equal_var=False)
    ...
    def analyze_timeframe_correlations(self, 
                                     timeframe_data: Dict[str, List[float]]) -> Dict[str, Any]:
        """
        ...
        if not timeframe_data or len(timeframe_data) < 2:
            return {
                "error": "Need at least two timeframes for correlation analysis"
            }
            
        # Check if each timeframe has sufficient data
        for timeframe, errors in timeframe_data.items():
            if len(errors) < self.min_samples:
                return {
                    "error": f"Insufficient samples for timeframe {timeframe} (has {len(errors)}, need {self.min_samples})"
                }
        ...
        pairwise_correlations.sort(key=lambda x: x["abs_correlation"], reverse=True)
        
        # Identify strongest correlations
        strongest_pairs = pairwise_correlations[:3] if len(pairwise_correlations) >= 3 else pairwise_correlations
        
        # Get overall statistics
        significant_correlations = [pc for pc in pairwise_correlations if pc["is_significant"]]
        positive_correlations = [pc for pc in pairwise_correlations if pc["correlation"] > 0]
        negative_correlations = [pc for pc in pairwise_correlations if pc["correlation"] < 0]
        ...
        return {
            ...
            "summary": {
                ...
                "positive_pairs": len(positive_correlations),
                "negative_pairs": len(negative_correlations),
                "avg_correlation_strength": np.mean([pc["abs_correlation"] for pc in pairwise_correlations])
            }
        }
    
    def validate_parameter_adjustment(self, 
                                     before_performance: List[Dict[str, Any]],
                                     after_performance: List[Dict[str, Any]],
        ...
        Returns:
            Dictionary with validation results
        """
        # Extract the specified metric from performance data
        try:
            before_values = [item[metric_name] for item in before_performance if metric_name in item]
            after_values = [item[metric_name] for item in after_performance if metric_name in item]
        except (KeyError, TypeError) as e:
            return {
                "error": f"Error extracting metric '{metric_name}': {str(e)}",
                "is_valid": False
            }
            
        if len(before_values) < self.min_samples or len(after_values) < self.min_samples:
            return {
                "is_valid": False,
                "reason": f"Insufficient samples (need at least {self.min_samples})",
        ...
        # This could be customized based on the metric
        higher_is_better = metric_name in ["profit_factor", "win_rate", "profit", "return", "sharpe_ratio"]
        
        # Calculate improvement
        if higher_is_better:
            improvement = after_mean - before_mean
            improvement_pct = (improvement / before_mean) * 100 if before_mean != 0 else 0
        ...
        effect_size = abs(before_mean - after_mean) / pooled_std if pooled_std != 0 else 0
        ...
        return {
            "is_valid": p_value < self.significance_level and is_improvement,
    ...
    def _classify_correlation_strength(self, corr_value: float) -> CorrelationStrength:
        """
