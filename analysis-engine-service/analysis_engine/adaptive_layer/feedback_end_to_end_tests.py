...
class MockToolEffectivenessRepository(ToolEffectivenessRepository):
    ...
    def get_parameter_variation_outcomes(
        self,
        strategy_id: str,
        parameter_name: str,
    ...
    def get_strategy_outcomes(
        ...
        if market_regime:
            return [o for o in outcomes if o.get('market_regime') == market_regime]
        return outcomes
    ...
    def get_outcomes_before_date(
        ...
        try:
            ...
            return [
                o for o in outcomes
                if start_date <= datetime.fromisoformat(o.get('timestamp', '')) < target_date
            ]
...
class MockAdaptationEngine:
    ...
    async def adapt_strategy(
        ...
        # Generate mock parameter adjustments
        if strategy_id in self.strategies:
            ...
            for param, config in strategy.get("parameters", {}).items():
                ...
                min_val = config.get("min", current - 1)
                max_val = config.get("max", current + 1)
                
                # Adjust parameter with some randomness
                new_value = current + (max_val - min_val) * (np.random.random() - 0.5) * 0.1
                new_value = max(min_val, min(max_val, new_value))
                
                adaptation["parameters_adjusted"][param] = {
                    "old_value": current,
                    "new_value": new_value,
                ...
                self.strategies[strategy_id]["parameters"][param]["current"] = new_value
...
def adaptation_engine():
    ...
    engine.add_strategy("test_strategy", {
        "stop_loss": {"current": 20, "min": 10, "max": 50},
        "take_profit": {"current": 40, "min": 20, "max": 100},
        "entry_threshold": {"current": 0.7, "min": 0.5, "max": 0.9},
    })
...
def statistical_analyzer(validator, parameter_tracking, repository):
    ...
    return ParameterStatisticalAnalyzer(validator, parameter_tracking, repository)
...
def generate_test_data(repo: MockToolEffectivenessRepository) -> None:
    ...
    # Generate parameter outcomes with different values
    param_values = [15, 20, 25, 30]
    regimes = ["trend", "range", "volatile"]
    
    for regime in regimes:
        # Generate outcomes for each parameter value in each regime
        for value in param_values:
            # Generate outcomes with different profit patterns per regime
            if regime == "trend":
                # Trend regime favors higher stop loss values
                base_profit = value * 0.5
                variance = 10
            elif regime == "range":
                # Range regime favors middle stop loss values
                base_profit = 20 - abs(value - 22.5)
                variance = 8
            else:  # volatile
                # Volatile regime favors lower stop loss values
                base_profit = 30 - value * 0.8
                variance = 15
            
            outcomes = []
            for i in range(40):  # Generate 40 outcomes per value/regime
    ...
    # Post-adaptation period (with improvement)
    ...
    # Add the adaptation outcomes to strategy outcomes as well
...
def generate_trade_feedback(
    ...
    market_regime: Optional[str] = None,
    source: FeedbackSource = FeedbackSource.STRATEGY_EXECUTION,
    category: FeedbackCategory = FeedbackCategory.SUCCESS,
) -> TradeFeedback:
...
class TestFeedbackLoopEndToEnd:
    ...
    @pytest.mark.asyncio
    async def test_feedback_collection_and_routing(self, feedback_router, event_bus):
        ...
        published_events = [e for e in event_bus.published_events if e["event_type"] == "feedback.routed"]
        assert len(published_events) == 1
        assert published_events[0]["data"]["feedback_id"] == feedback.feedback_id
    
    @pytest.mark.asyncio
    async def test_parameter_tracking_and_effectiveness(self, parameter_tracking):
        ...
        history = parameter_tracking.get_parameter_history(strategy_id, parameter_name)
        ...
        success = await parameter_tracking.record_parameter_performance(
            ...
            strategy_id=strategy_id,
            parameter_name=parameter_name,
            performance_metrics={"win_rate": 0.65, "profit_factor": 1.5, "profit": 100}
        )
        
        assert success is True
        
        # Calculate effectiveness
        effectiveness = await parameter_tracking.calculate_parameter_effectiveness(
            strategy_id=strategy_id,
            parameter_name=parameter_name
        )
    ...
    async def test_statistical_validation(self, validator, repository):
        ...
        result = validator.validate_parameter_effectiveness(
            strategy_id=strategy_id,
            parameter_name=parameter_name
        )
        ...
        # Check for regime effectiveness differences
    ...
    async def test_feedback_loop_integration(self, feedback_loop, repository):
        ...
        feedback_loop.record_strategy_outcome(
            strategy_id=strategy_id,
            instrument="EURUSD",
            timeframe="H1",
            adaptation_id=adaptation_id,
            outcome_metrics={"profit": 50, "win": True},
            market_regime="trend"
        )
    ...
    @pytest.mark.asyncio
    async def test_adaptation_engine_integration(
        self, adaptation_engine, feedback_loop, validator, event_bus
    ):
        """Test integration with adaptation engine."""
        strategy_id = "test_strategy"
        
        # Trigger adaptation based on market conditions
        adaptation = await adaptation_engine.adapt_strategy(
            strategy_id=strategy_id,
            market_conditions={"regime": "trend", "volatility": "high"}
        )
        
        assert "adaptation_id" in adaptation
        assert "parameters_adjusted" in adaptation
        
        # Record outcome for this adaptation
        feedback_loop.record_strategy_outcome(
            strategy_id=strategy_id,
            instrument="EURUSD",
            timeframe="H1",
        ...
        # Validate adaptation significance (will likely return insufficient data
        # since we only have one outcome, but the integration path is tested)
        result = validator.validate_adaptation_significance(
            adaptation_id=adaptation["adaptation_id"]
        )
    ...
    @pytest.mark.asyncio
    async def test_model_feedback_integration(self, model_feedback_integrator, event_bus):
        """Test integration with model feedback."""
        model_id = "test_model"
        
        # Generate model feedback
        feedback = generate_trade_feedback(
            strategy_id="test_strategy",
        ...
        # Process feedback
        result = await model_feedback_integrator.collect_model_feedback(feedback)
        
        assert result is True
        assert model_id in model_feedback_integrator.feedback_by_model
        assert len(model_feedback_integrator.feedback_by_model[model_id]["recent_feedback"]) == 1
        assert len(model_feedback_integrator.feedback_by_model[model_id]["error_trends"]) == 1
    ...
    async def test_end_to_end_feedback_flow(
        ...
        # 1. Register feedback handler that updates parameters
        async def parameter_update_handler(feedback):
            if feedback.outcome_metrics.get("profit", 0) > 0:
                # Successful trade, record parameter performance
                param_id = str(uuid.uuid4())  # Normally this would come from a previous change
                ...
                await parameter_tracking.record_parameter_performance(
                    ...
                    performance_metrics={
                        "win_rate": 1.0,
                        "profit_factor": 2.0,
                        "profit": feedback.outcome_metrics.get("profit", 0)
                    }
                )
            return True
        ...
        feedback_router.add_route(
            source=FeedbackSource.STRATEGY_EXECUTION,
            category=FeedbackCategory.SUCCESS,
            handler=parameter_update_handler
        )
        
        # 2. Set up event handling for feedback
        async def feedback_event_handler(data):
            # This would normally trigger adaptation decision
            ...
            if strategy_id:
                ...
                feedback_loop.record_strategy_outcome(
                    ...
                    adaptation_id=data.get("adaptation_id", "test_adaptation"),
        ...
        # 5. Process published events
        await event_bus.process_published_events()
        
        # 6. Verify that feedback was recorded in the feedback loop
        performance = feedback_loop.get_performance_by_regime(strategy_id)
        assert "trend" in performance
        assert performance["trend"]["count"] >= 1
        
        # 7. Test adaptation based on feedback
        adaptation = await adaptation_engine.adapt_strategy(
            strategy_id=strategy_id,
        ...
        # 8. Record parameter change from adaptation
        for param_name, param_data in adaptation["parameters_adjusted"].items():
            param_id = await parameter_tracking.record_parameter_change(
                ...
                change_reason="Adaptation",
                source_component="AdaptationEngine",
                confidence_level=param_data["confidence"]
            )
