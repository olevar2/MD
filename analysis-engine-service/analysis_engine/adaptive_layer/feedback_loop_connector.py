...
class FeedbackLoopConnector:
    ...
    def __init__(
        ...
        strategy_execution_api_url: str,
        ...
        logger.info("FeedbackLoopConnector initialized with strategy execution API at %s", 
    ...
    async def start(self):
        ...
        self.http_client = httpx.AsyncClient(
            timeout=self.config.get("http_timeout", 10.0),
            limits=httpx.Limits(
        ...
        if self.config.get("enable_monitoring", True):
            self._monitoring_task = asyncio.create_task(self._monitor_loop_health())
    ...
    async def send_adaptation_to_strategy_execution(self, adaptation: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            endpoint = f"{self.strategy_execution_api_url}/api/v1/strategies/{strategy_id}/adapt"
            ...
            if self.event_publisher:
                await self.event_publisher.publish(
                    "feedback.adaptation.sent",
                    {
                        "strategy_id": strategy_id,
                        "adaptation_id": adaptation.get("adaptation_id"),
                        "timestamp": self.last_adaptation_time.isoformat(),
                        "success": True
                    }
                )
            
            logger.debug("Successfully sent adaptation to strategy %s", strategy_id)
            return True
            
        except Exception as e:
            self.connection_errors += 1
            logger.error("Failed to send adaptation to strategy %s: %s", strategy_id, str(e))
            
            # Publish error event if configured
            if self.event_publisher:
                await self.event_publisher.publish(
                    "feedback.adaptation.error",
                    {
                        "strategy_id": strategy_id,
                        "adaptation_id": adaptation.get("adaptation_id"),
                        "error": str(e),
    ...
    async def process_execution_feedback(self, feedback_data: Dict[str, Any]) -> str:
        """
        ...
        """
        try:
            # Create a TradeFeedback object from the data
            # In a real implementation, we would have validation and conversion logic
            feedback = TradeFeedback(
                id=feedback_data.get("feedback_id") or "",  # Will be generated if empty
                strategy_id=feedback_data.get("strategy_id"),
                model_id=feedback_data.get("model_id"),
                instrument=feedback_data.get("instrument"),
                timeframe=feedback_data.get("timeframe"),
                source=feedback_data.get("source"),
                category=feedback_data.get("category"),
                status=FeedbackStatus.NEW,
                outcome_metrics=feedback_data.get("metrics") or {},
                metadata=feedback_data.get("metadata") or {},
                timestamp=feedback_data.get("timestamp") or datetime.utcnow().isoformat()
            )
            
            # Collect the feedback
            feedback_id = await self.trading_feedback_collector.collect_feedback(feedback)
    ...
    async def get_loop_health(self) -> Dict[str, Any]:
        ...
        if self.last_feedback_time:
            time_since_feedback = (now - self.last_feedback_time).total_seconds()
            
        time_since_adaptation = None
        if self.last_adaptation_time:
            time_since_adaptation = (now - self.last_adaptation_time).total_seconds()
        ...
        return {
            ...
            "last_feedback_time": self.last_feedback_time.isoformat() if self.last_feedback_time else None,
            "last_adaptation_time": self.last_adaptation_time.isoformat() if self.last_adaptation_time else None,
            "seconds_since_last_feedback": time_since_feedback,
    ...
    async def _monitor_loop_health(self):
        ...
        check_interval = self.config.get("monitoring_interval_seconds", 300)  # 5 minutes default
        alert_threshold = self.config.get("feedback_alert_threshold_seconds", 3600)  # 1 hour default
        ...
        while self._is_running:
            try:
                ...
                if (self.last_feedback_time and 
                    (now - self.last_feedback_time).total_seconds() > alert_threshold):
                    ...
                    if self.event_publisher:
                        await self.event_publisher.publish(
                            "feedback.loop.alert",
                            {
                                "alert_type": "no_recent_feedback",
                                "seconds_since_last": (now - self.last_feedback_time).total_seconds(),
                                "threshold": alert_threshold,
                                "timestamp": now.isoformat()
                            }
                        )
                
                # Publish health metrics if configured
                if self.event_publisher and self.config.get("publish_health_metrics", True):
                    await self.event_publisher.publish(
                        "feedback.loop.health",
                        {
                            ...
                            "adaptation_count": self.adaptation_count,
                            "connection_errors": self.connection_errors,
                            "is_healthy": self._is_running and self.connection_errors < 
                                         self.config.get("max_connection_errors", 10),
                            "timestamp": now.isoformat()
                        }
                    )
                
            except Exception as e:
                logger.error("Error in feedback loop health monitor: %s", str(e))
