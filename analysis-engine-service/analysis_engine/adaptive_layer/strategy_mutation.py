...
class StrategyMutationEngine:
    ...
    """
    def __init__(self,
                 parameter_feedback_service: Any,
    ...
    async def mutate_strategy(self,
                              strategy_id: str,
                              mutation_type: str = 'directed',
                              mutation_goal: Optional[str] = None,
        ...
        Returns:
            New version ID.
        """
        # fetch current strategy and performance
        current = await self.strategy_repository.get_strategy(strategy_id)
        performance = await self.strategy_repository.get_strategy_performance(strategy_id)

        # generate new params
        if mutation_type == 'random':
            new_params = self._random_mutation(current['parameters'])
        else:
            # directed or evolutionary
            recs = await self.parameter_feedback_service.generate_parameter_recommendations(strategy_id)
        ...
        logger.info("Strategy %s mutated to version %s", strategy_id, new_version)

        # publish event
        await self.event_bus.publish('strategy.mutated', variant)
        return version_id

    async def evaluate_strategy_fitness(self,
                                        strategy_id: str,
                                        version: int) -> float:
        """
        Evaluates the fitness of a specific strategy version based on performance metrics.
        """
        perf = await self.strategy_repository.get_strategy_performance(strategy_id, version)
        # composite fitness (e.g., weighted Sharpe and profit)
    ...
    async def select_best_strategy_variant(self,
                                           strategy_id: str) -> Dict[str, Any]:
        """
        Selects the best performing variant of a strategy.
        """
        variants = await self.strategy_repository.get_strategy_variants(strategy_id)
        best = None
        best_score = float('-inf')
        for v in variants:
            score = await self.evaluate_strategy_fitness(strategy_id, v['version'])
            if score > best_score:
                best_score = score
                best = v
        if best:
            await self.strategy_repository.set_active_variant(strategy_id, best['version'])
        return best or {}

    async def get_mutation_history(self,
                                   strategy_id: str,
                                   limit: int = 10) -> List[Dict[str, Any]]:
        """
        Returns mutation history for a strategy.
        """
        return await self.strategy_repository.get_mutation_history(strategy_id, limit)

    async def get_fitness_evolution(self,
    ...
    def _directed_mutation(self,
                            params: Dict[str, Any],
                            recs: Dict[str, str]) -> Dict[str, Any]:
        new = params.copy()
