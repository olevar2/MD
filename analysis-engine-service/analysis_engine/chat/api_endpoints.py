...
import uuid
from .chat_backend_service import ChatBackendService
...
@router.post('/message')
@async_with_exception_handling
async def send_message(message_data: Dict[str, Any]=Body(...), user_id: str
    =Depends(get_user_id)):
    """
...
@async_with_exception_handling
async def execute_action(action_data: Dict[str, Any]=Body(...), user_id:
    str=Depends(get_user_id)):
    """
...
@router.get('/history')
@async_with_exception_handling
async def get_history(limit: int=Query(50, ge=1, le=100), before: Optional[
    str]=Query(None), user_id: str=Depends(get_user_id)):
    """
    Get chat history.
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=
            f'Error getting chat history: {str(e)}')
...
async def clear_history(user_id: str=Depends(get_user_id)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=
            f'Error clearing chat history: {str(e)}')
