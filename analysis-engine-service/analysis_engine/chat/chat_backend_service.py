...
class ChatBackendService:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        try:
            # Initialize ML model connector using adapter
            from analysis_engine.adapters import (
            ...
            # Initialize ML client for backward compatibility
    ...
    async def process_message(
        ...
        # Add user_id to context for preference management
        ...
        if self.user_preference_manager:
            try:
                ...
                detected_preferences = await self.user_preference_manager.detect_preferences_from_message(user_id, message)
                if detected_preferences:
                    logger.info(f"Detected preferences for user {user_id}: {detected_preferences}")
        ...
        if self.nlp_processor:
            try:
                ...
                if hasattr(self.nlp_processor, 'process_message'):
                    ...
                    nlp_result = self.nlp_processor.process_message(user_id, message, user_context)
                    ...
                    logger.info(f"Enhanced NLP results - Intent: {intent}, Entities: {len(entities)}")
                else:
                    ...
                    if hasattr(self.nlp_processor, 'get_primary_intent'):
                        intent = self.nlp_processor.get_primary_intent(message, entities)
                    else:
                        intent = self._determine_intent(message, entities)

                    logger.info(f"Basic NLP results - Intent: {intent}, Entities: {len(entities)}, Sentiment: {sentiment['compound']}")

                    # Extract custom entities if available
                    if hasattr(self.nlp_processor, 'extract_custom_entities'):
                        custom_entities = self.nlp_processor.extract_custom_entities(message)
                        if custom_entities:
                            entities.extend(custom_entities)
                            logger.info(f"Extracted {len(custom_entities)} custom entities")
            except Exception as e:
                logger.error(f"Error processing message with NLP: {str(e)}")
                # Simple intent detection if NLP fails
        ...
        if intent_info:
            # Use enhanced intent info
            response = await self._generate_response_with_intent_info(user_id, message, intent_info, entities, user_context)
        else:
        ...
        # Update context if context management is available
        if self.nlp_processor and hasattr(self.nlp_processor, 'update_context'):
            try:
                self.nlp_processor.update_context(
                    user_id,
                    message,
    ...
    async def _generate_response_with_intent_info(
        self,
        user_id: str,
        message: str,
        ...
        primary_confidence = intent_info.get("primary", {}).get("confidence", 0.0)

        # Extract sub-intent
        sub_intent = intent_info.get("sub_intent", {}).get("intent")
        sub_confidence = intent_info.get("sub_intent", {}).get("confidence", 0.0)

        # Extract secondary intent
        secondary_intent = intent_info.get("secondary", {}).get("intent")
        secondary_confidence = intent_info.get("secondary", {}).get("confidence", 0.0)

        logger.info(f"Generating response for primary intent: {primary_intent} ({primary_confidence:.2f}), " +
                   f"sub-intent: {sub_intent} ({sub_confidence:.2f}), " +
                   f"secondary intent: {secondary_intent} ({secondary_confidence:.2f})")

        # Generate response based on primary intent and sub-intent
        if primary_intent == "trading":
            # Use sub-intent for more specific handling
            if sub_intent == "buy":
                response = await self._handle_buy_intent(message, entities, context)
            elif sub_intent == "sell":
                response = await self._handle_sell_intent(message, entities, context)
            elif sub_intent == "modify":
                response = await self._handle_modify_intent(message, entities, context)
            elif sub_intent == "close":
                response = await self._handle_close_intent(message, entities, context)
            else:
                # Fall back to general trading intent
                response = await self._handle_trading_intent(message, entities, context)
        elif primary_intent == "analysis":
            # Use sub-intent for more specific handling
            if sub_intent == "technical":
                response = await self._handle_technical_analysis_intent(message, entities, context)
            elif sub_intent == "fundamental":
                response = await self._handle_fundamental_analysis_intent(message, entities, context)
            elif sub_intent == "sentiment":
                response = await self._handle_sentiment_analysis_intent(message, entities, context)
            else:
                # Fall back to general analysis intent
            ...
            response = await self._handle_account_intent(message, entities, context)
        elif primary_intent == "settings":
            response = await self._handle_settings_intent(message, entities, context)

        # If confidence is low, consider secondary intent
        if primary_confidence < 0.4 and secondary_confidence > 0.3:
            # Add a note about alternative interpretation
    ...
    async def _handle_analysis_intent(
        ...
        # Default analysis text (fallback)
        ...
        if user_id and self.user_preference_manager:
            try:
                ...
                user_preferences = await self.user_preference_manager.get_user_preferences(user_id)

                # Detect and update preferences from the message
                detected_preferences = await self.user_preference_manager.detect_preferences_from_message(user_id, message)
                if detected_preferences:
                    logger.info(f"Updated user preferences from message: {detected_preferences}")
            except Exception as e:
                logger.error(f"Error getting user preferences: {str(e)}")

        # Get analysis from ML model connector if available
        if self.ml_model_connector:
            try:
                # Get market analysis
                analysis_result = await self.ml_model_connector.get_market_analysis(
                    symbol=symbol,
                    timeframe=timeframe,
                ...
                if analysis_result:
                    ...
                    # Combine explanations into a coherent text
                    if explanations:
                        combined_text = ""
                        ...
                        # Use fallback text with predictions
                        predictions = analysis_result.get("predictions", {})
                        if predictions:
                            trend = predictions.get("trend", {}).get("direction", "neutral")
                            confidence = analysis_result.get("confidence", 0.5)
                            confidence_text = "high" if confidence > 0.7 else "moderate" if confidence > 0.5 else "low"

                            analysis_text = f"Based on my analysis of recent market data for {symbol} on the {timeframe} timeframe, the trend appears to be {trend} with {confidence_text} confidence. "
                            ...
                            if "support_resistance" in predictions:
                                ...
                                if support:
                                    support_str = ", ".join([str(s) for s in support[:2]])
                                    analysis_text += f"Key support levels are at {support_str}. "

                                if resistance:
                                    resistance_str = ", ".join([str(r) for r in resistance[:2]])
                                    analysis_text += f"Key resistance levels are at {resistance_str}. "
            except Exception as e:
                logger.error(f"Error getting analysis from ML model connector: {str(e)}")

        # Fallback to ML client for backward compatibility
        elif self.ml_client:
            try:
    ...
    async def _handle_information_intent(
        ...
        if "rsi" in message_lower:
            return {
                "text": "The Relative Strength Index (RSI) is a momentum oscillator that measures the speed and change of price movements. It ranges from 0 to 100 and is typically used to identify overbought or oversold conditions. An RSI above 70 is considered overbought, while an RSI below 30 is considered oversold. Traders also look for divergences between RSI and price to identify potential reversals."
            }
    ...
    async def _handle_account_intent(
        self,
        message: str,
        entities: List[Dict[str, Any]],
        context: Optional[Dict[str, Any]]
        ...
        return {
            "text": "I can provide information about your trading account, including balance, equity, margin, and performance. What specific information would you like to know?"
        }

    async def _handle_buy_intent(
        self,
        message: str,
        entities: List[Dict[str, Any]],
        context: Optional[Dict[str, Any]]
        ...
        return {
            "text": f"I can help you buy {symbol}{' with ' + str(amount) + ' lots' if amount else ''}. Would you like me to execute this trade for you?",
            "tradingAction": trading_action
        }

    async def _handle_sell_intent(
        self,
        message: str,
        entities: List[Dict[str, Any]],
        context: Optional[Dict[str, Any]]
        ...
        return {
            "text": f"I can help you sell {symbol}{' with ' + str(amount) + ' lots' if amount else ''}. Would you like me to execute this trade for you?",
            "tradingAction": trading_action
        }
    ...
    async def _handle_modify_intent(
        ...
        has_take_profit = "profit" in message.lower() or "tp" in message.lower() or "target" in message.lower()
        ...
        for entity in entities:
            if entity["label"] == "PRICE":
                if entity.get("level_type") == "support" or (has_stop_loss and not has_take_profit):
                    stop_loss = entity.get("value")
                elif entity.get("level_type") == "resistance" or (has_take_profit and not has_stop_loss):
        ...
        if not stop_loss and not take_profit:
            response_text += " What would you like to modify? You can change the stop loss, take profit, or position size."
        else:
    ...
    async def _handle_close_intent(
        self,
        message: str,
        entities: List[Dict[str, Any]],
        context: Optional[Dict[str, Any]]
        ...
        return {
            "text": f"I can help you close your {symbol} position. Would you like me to execute this action?",
            "tradingAction": trading_action
        }

    async def _handle_technical_analysis_intent(
        self,
        message: str,
        entities: List[Dict[str, Any]],
        context: Optional[Dict[str, Any]]
        ...
        if user_id and self.user_preference_manager:
            try:
                ...
                # Update preferred indicators if specified in the message
                if indicators:
        ...
        if self.ml_model_connector:
            try:
                # Get market analysis with focus on technical indicators
                analysis_result = await self.ml_model_connector.get_market_analysis(
                    symbol=symbol,
                    timeframe=timeframe,
                    user_preferences=user_preferences
                ...
                if analysis_result:
                    ...
                    # Focus on specific indicators if requested
                    if indicators and "feature_importance" in analysis_result:
                        # Create a focused technical analysis response
                        technical_text = f"Based on technical analysis of {symbol} on the {timeframe} timeframe, focusing on {', '.join(indicators)}: "

                        # Add trend explanation with focus on requested indicators
                        if "trend" in explanations:
                            technical_text += explanations["trend"] + " "

                        # Add specific indicator details
                        indicator_details = self._get_indicator_details(indicators, analysis_result)
                        if indicator_details:
                            technical_text += indicator_details

                        response_text = technical_text
                    else:
                        # Use general technical analysis from ML model
                        combined_text = ""
                        ...
                        if combined_text:
                            response_text = f"Based on technical analysis of {symbol} on the {timeframe} timeframe: {combined_text}"
            except Exception as e:
                logger.error(f"Error getting technical analysis from ML model connector: {str(e)}")
    ...
    def _get_indicator_details(
        ...
        # Get indicator values from analysis result
        if "inputs" in analysis_result and "indicators" in analysis_result["inputs"]:
            indicator_values = analysis_result["inputs"]["indicators"]

            # Map common indicator names to their keys in the data
            indicator_map = {
                "rsi": "rsi",
                ...
                "ema": "ema_50",
                "moving average": "ema_50",
                "exponential moving average": "ema_50",
                "sma": "sma_50",
            ...
            # Add details for each requested indicator
            for indicator in indicators:
                ...
                if key and key in indicator_values:
                    ...
                    if key == "rsi":
                        condition = "overbought" if value > 70 else "oversold" if value < 30 else "neutral territory"
                        details += f"The RSI is at {value:.1f}, indicating {condition}. "
                    elif key == "macd":
                        signal = indicator_values.get("macd_signal", 0)
                        histogram = indicator_values.get("macd_histogram", 0)
                        if histogram > 0:
                            trend = "bullish" if histogram > signal else "potentially bullish"
                        else:
                            trend = "bearish" if histogram < signal else "potentially bearish"
                        details += f"The MACD is at {value:.4f} with signal line at {signal:.4f}, suggesting a {trend} momentum. "
                    elif key.startswith("ema") or key.startswith("sma"):
                        period = key.split("_")[1]
                        details += f"The {period}-period {key.split('_')[0].upper()} is at {value:.4f}. "
                    elif key == "atr":
                        details += f"The ATR is at {value:.4f}, indicating {'high' if value > 0.005 else 'moderate' if value > 0.002 else 'low'} volatility. "
                    elif key == "bollinger_bands":
                        details += f"Price is currently near the {'upper' if value.get('position') > 0.7 else 'lower' if value.get('position') < 0.3 else 'middle'} Bollinger Band. "
                    elif key == "stochastic":
                        details += f"The Stochastic oscillator is at {value:.1f}, indicating {'overbought' if value > 80 else 'oversold' if value < 20 else 'neutral'} conditions. "

        return details

    async def _handle_fundamental_analysis_intent(
        self,
        message: str,
        entities: List[Dict[str, Any]],
        context: Optional[Dict[str, Any]]
        ...
        # Create response text
        response_text = f"Based on fundamental analysis of {symbol}, the outlook is cautiously optimistic. "

        if symbol == "EURUSD":
            response_text += "Recent economic data from the Eurozone shows improving manufacturing PMI, while US inflation data came in slightly higher than expected. The ECB has maintained its hawkish stance, while the Fed has signaled potential rate cuts later this year. This divergence in monetary policy could support the Euro in the medium term."
        elif symbol == "GBPUSD":
            response_text += "The UK economy has shown resilience with better-than-expected GDP growth and declining inflation. The Bank of England is maintaining higher rates for longer than initially anticipated, which provides support for the Pound. However, ongoing trade negotiations post-Brexit continue to create uncertainty."
        elif symbol == "USDJPY":
            response_text += "The Bank of Japan has begun to shift away from its ultra-loose monetary policy, which has provided support for the Yen. Meanwhile, US Treasury yields have stabilized, reducing the interest rate differential that had been driving USDJPY higher. Geopolitical tensions in Asia could trigger safe-haven flows into the Yen."
        else:
            response_text += "Recent economic indicators and central bank policies suggest changing dynamics in the currency pair. Interest rate differentials, inflation data, and economic growth projections are key factors to monitor for future price movements."

        return {
            "text": response_text
        }

    async def _handle_sentiment_analysis_intent(
        self,
        message: str,
        entities: List[Dict[str, Any]],
        context: Optional[Dict[str, Any]]
        ...
        # Create response text
        response_text = f"Based on sentiment analysis for {symbol}, market sentiment is currently neutral with a slight bullish bias. "
        response_text += "Retail positioning data shows 55% of traders are long, while institutional positioning indicates balanced exposure. "
        response_text += "Recent news sentiment has been positive following better-than-expected economic data. "
        response_text += "Social media analysis shows increasing mentions with a positive sentiment score of 65/100. "
        response_text += "Options market data suggests traders are hedging against downside risk, with the put/call ratio at 1.2."
    ...
    async def _handle_settings_intent(
        ...
        return {
            "text": "I can help you configure your platform settings, including preferences, notifications, and display options. What specific settings would you like to adjust?"
        }
