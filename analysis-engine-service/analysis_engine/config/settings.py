...
class AnalysisEngineSettings(AppSettings):
    ...
    def validate_cors_origins(cls, v: List[str]) -> List[str]:
        ...
        # If wildcard is present, only allow it as the only item
        if "*" in v and len(v) > 1:
    ...
    # --- Database Settings ---
    # Override the required fields from AppSettings to make them optional
    ...
    def DATABASE_URL(self) -> str:
        ...
        # Check if we have all required components to build the URL
    ...
    # --- Redis Settings ---
    # Override Redis settings from AppSettings with additional validation
    ...
    KAFKA_ANALYSIS_TOPIC: str = Field(
        default="analysis_events",
        env="KAFKA_ANALYSIS_TOPIC",
        description="Kafka topic for analysis-related events"
    )
    KAFKA_REGIME_TOPIC: str = Field(
        default="market_regime_events",
        env="KAFKA_REGIME_TOPIC",
        description="Kafka topic for market regime change events"
    )
    KAFKA_SIGNAL_TOPIC: str = Field(
        default="trading_signals",
        env="KAFKA_SIGNAL_TOPIC",
        description="Kafka topic for generated trading signals"
    )
    ...
    ML_INTEGRATION_SERVICE_URL: str = Field(
        default="http://ml-integration-service:8000/api/v1",
        env="ML_INTEGRATION_SERVICE_URL",
        description="URL for the ML Integration Service"
    )
    FEATURE_STORE_SERVICE_URL: str = Field(
        default="http://feature-store-service:8000/api/v1",
        env="FEATURE_STORE_SERVICE_URL",
        description="URL for the Feature Store Service"
    )
    ...
    SERVICE_TIMEOUT_SECONDS: int = Field(
        ...
        ge=1,
        le=300,
        env="SERVICE_TIMEOUT_SECONDS",
        description="Default timeout for external service requests in seconds"
    )

    # External service retry settings
    SERVICE_MAX_RETRIES: int = Field(
        default=3,
        ge=0,
        le=10,
        env="SERVICE_MAX_RETRIES",
        description="Maximum number of retries for external service requests"
    )
    SERVICE_RETRY_BACKOFF: float = Field(
        default=0.5,
        ge=0.1,
        le=60.0,
        env="SERVICE_RETRY_BACKOFF",
        description="Backoff factor for retries in seconds"
    )
    ...
    ENABLE_ML_INTEGRATION: bool = Field(
        default=True,
        env="ENABLE_ML_INTEGRATION",
        description="Enable machine learning integration"
    )
    ANALYSIS_CACHE_TTL: int = Field(
        default=3600,
        ge=0,
        le=86400,
        env="ANALYSIS_CACHE_TTL",
        description="Time-to-live for analysis cache in seconds (0 to disable caching)"
    )
    ...
    RATE_LIMIT_REQUESTS: int = Field(
        ...
        description="Standard rate limit requests per minute"
    )
    ...
    RATE_LIMIT_PREMIUM: int = Field(
        ...
        ge=1,
        le=50000,
        env="RATE_LIMIT_PREMIUM",
        description="Premium rate limit requests per minute"
    )
    ...
    TRACING_SAMPLE_RATE: float = Field(
        ...
        description="Sampling rate for distributed tracing (0.0-1.0)"
    )
    ...
    WS_CLOSE_TIMEOUT: int = Field(
        ...
        ge=1,
        le=60,
        env="WS_CLOSE_TIMEOUT",
        description="Timeout for WebSocket close handshake in seconds"
    )

    # --- Feature Flags ---
    FEATURE_MULTI_TIMEFRAME_ANALYSIS: bool = Field(
        default=True,
        env="FEATURE_MULTI_TIMEFRAME_ANALYSIS",
        description="Enable multi-timeframe analysis"
    )
    FEATURE_MARKET_REGIME_DETECTION: bool = Field(
        default=True,
        env="FEATURE_MARKET_REGIME_DETECTION",
    ...
    FEATURE_CORRELATION_ANALYSIS: bool = Field(
        default=True,
        env="FEATURE_CORRELATION_ANALYSIS",
