...
class EnhancedDeprecationMonitor:
    ...
    def __new__(cls):
        ...
        with cls._lock:
            if cls._instance is None:
                cls._instance = super(EnhancedDeprecationMonitor, cls).__new__(cls)
    ...
    def __init__(self):
        ...
        report_dir = self._config.get("monitoring", {}).get("report_directory", "logs/deprecation")
        ...
        # Set up report paths
        self._report_path = os.path.join(self._report_dir, "deprecation_report.json")
        self._history_path = os.path.join(self._report_dir, "deprecation_history.json")
    ...
    def _initialize_module_stats(self) -> None:
        ...
        for module in self._config.get("modules", []):
            ...
            if name and name not in self._module_stats:
                self._module_stats[name] = ModuleUsageStats(
                    name=name,
                    ...
                    impact_level=module.get("impact_level", "medium")
                )
        ...
        if os.path.exists(self._report_path):
            try:
                with open(self._report_path, 'r') as f:
                    ...
                    # Load module stats
                    for module_name, module_data in report_data.get("modules", {}).items():
                        self._module_stats[module_name] = ModuleUsageStats(
                            name=module_name,
                            total_usages=module_data.get("total_usages", 0),
                            unique_locations=module_data.get("unique_locations", 0),
                            ...
                            migration_guide=module_data.get("migration_guide", ""),
                            migration_tool=module_data.get("migration_tool", ""),
                            description=module_data.get("description", ""),
                            impact_level=module_data.get("impact_level", "medium")
                        )
                    
                    # Load usage data
                    for module_data in report_data.get("modules", {}).values():
                        for usage in module_data.get("usages", []):
                            key = f"{usage.get('module_name')}:{usage.get('caller_file')}:{usage.get('caller_line')}:{usage.get('caller_function')}"
                            self._usages[key] = DeprecationUsage(
                                module_name=usage.get("module_name", ""),
                                caller_file=usage.get("caller_file", ""),
                                caller_line=usage.get("caller_line", 0),
                                ...
                                call_stack=usage.get("call_stack", []),
        ...
        if os.path.exists(self._history_path):
            try:
                with open(self._history_path, 'r') as f:
                    ...
                    # Update usage trends
                    for module_name, module_data in history_data.get("modules", {}).items():
                        if module_name in self._module_stats:
                            self._module_stats[module_name].usage_trend = module_data.get("usage_trend", {})
                
                logger.debug(f"Loaded deprecation history from {self._history_path}")
            except Exception as e:
                logger.error(f"Failed to load deprecation history: {e}")
    
    def record_usage(self, module_name: str, context: Dict[str, Any] = None) -> None:
        """
        ...
        call_stack = []
        current_frame = frame
        for _ in range(5):  # Capture up to 5 frames
            if current_frame:
                frame_info = {
                ...
                call_stack.append(f"{frame_info['file']}:{frame_info['line']} in {frame_info['function']}")
        ...
        with self._lock:
            ...
            if module_name not in self._module_stats:
                ...
                module_config = next((m for m in self._config.get("modules", []) if m.get("name") == module_name), {})
                self._module_stats[module_name] = ModuleUsageStats(
                    name=module_name,
                    replacement=module_config.get("replacement", ""),
                    migration_guide=module_config.get("migration_guide", ""),
                    migration_tool=module_config.get("migration_tool", ""),
                    description=module_config.get("description", ""),
                    impact_level=module_config.get("impact_level", "medium")
                )
    ...
    def _update_module_stats(self, module_name: str) -> None:
        ...
        """
        # Get all usages for this module
        module_usages = [u for u in self._usages.values() if u.module_name == module_name]
        
        # Update basic stats
        self._module_stats[module_name].total_usages = sum(u.count for u in module_usages)
        self._module_stats[module_name].unique_locations = len(module_usages)
        
        # Update first/last seen
        if module_usages:
            timestamps = [u.timestamp for u in module_usages]
            first_seen = datetime.datetime.fromtimestamp(min(timestamps)).isoformat()
            last_seen = datetime.datetime.fromtimestamp(max(timestamps)).isoformat()
        ...
        # Update top callers
        top_callers = sorted(module_usages, key=lambda u: u.count, reverse=True)[:10]
        self._module_stats[module_name].top_callers = [
            {
                "file": u.caller_file,
                "line": u.caller_line,
                "function": u.caller_function,
                "count": u.count,
    ...
    def get_usage_report(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            deprecation_date = datetime.datetime.strptime(
                self._config.get("deprecation_date", "2023-12-31"),
                "%Y-%m-%d"
            ).date()
            ...
            report = {
                ...
                "deprecation_date": self._config.get("deprecation_date", "2023-12-31"),
            ...
            for module_name, module_stats in self._module_stats.items():
                ...
                report["modules"][module_name] = {
                    "total_usages": module_stats.total_usages,
                    "unique_locations": module_stats.unique_locations,
                    "first_seen": module_stats.first_seen,
                    "last_seen": module_stats.last_seen,
                    "usage_trend": module_stats.usage_trend,
                    "top_callers": module_stats.top_callers,
                    "replacement": module_stats.replacement,
                    "migration_guide": module_stats.migration_guide,
                    "migration_tool": module_stats.migration_tool,
                    "description": module_stats.description,
                    "impact_level": module_stats.impact_level,
                    "usages": [u.to_dict() for u in module_usages]
                }
    ...
    def generate_charts(self, output_dir: str = None) -> Dict[str, str]:
        ...
        # Generate usage by module chart
        charts["usage_by_module"] = self._generate_usage_by_module_chart(output_dir)
        
        # Generate trend chart
        charts["usage_trend"] = self._generate_trend_chart(output_dir)
        
        # Generate migration progress chart
        charts["migration_progress"] = self._generate_migration_progress_chart(output_dir)
    ...
    def _generate_migration_progress_chart(self, output_dir: str) -> str:
        ...
        # Get initial usage counts (estimate from trend data)
        initial_counts = {}
        for module_name, module_stats in self._module_stats.items():
            trend = module_stats.usage_trend
            if trend:
                # Use the maximum daily count as an estimate of initial usage
                initial_counts[module_name] = max(trend.values()) * 10  # Rough estimate
            else:
        ...
        for module_name, module_stats in self._module_stats.items():
            ...
            if initial > 0:
                # Higher percentage means more progress (fewer usages)
                            ...
                            for i, line in enumerate(lines):
                                for pattern in patterns:
                                    if pattern.search(line):
                                        ...
                                        module_results.append((rel_path, i + 1, line.strip()))
                                        break
    ...
    def get_migration_status(self) -> Dict[str, Any]:
        ...
        # Calculate overall progress
        if total_initial > 0:
            overall_progress = max(0, min(100, 100 - (total_current / total_initial * 100)))
        else:
