...
class RegimeTransitionPredictor:
    ...
    def __init__(
        self,
        regime_detector: Optional[MarketRegimeAnalyzer] = None,
        transition_history_size: int = 100,
        ...
        # Inter-market correlation data
        self.correlated_markets = {}

        self.logger.info(f"RegimeTransitionPredictor initialized with {len(MarketRegime)} regimes")
        ...
        for from_regime in MarketRegime:
            ...
            for to_regime in MarketRegime:
                if from_regime != to_regime:
                    transition_matrix[from_regime.value][to_regime.value] = 1.0 / (len(MarketRegime) - 1)
    ...
    async def predict_regime_transition(
        ...
        timeframe: str = "1h",
        use_inter_market_correlations: bool = True,
        ...
        early_warnings = self._calculate_early_warning_indicators(price_data, current_regime)

        # Get transition probabilities from current regime
        if current_regime.value in self.transition_probabilities:
            transition_probs = self.transition_probabilities[current_regime.value]
        else:
            transition_probs = {r.value: 1.0 / (len(MarketRegime) - 1) for r in MarketRegime if r != current_regime}

        # Incorporate inter-market correlations if requested
        inter_market_signals = {}
        if use_inter_market_correlations and self.correlation_service:
            # Update correlated markets if needed
            if symbol not in self.correlated_markets or not self.correlated_markets[symbol]:
                await self.update_correlated_markets(symbol)

            # Analyze correlated markets if data is provided
            if correlated_markets_data and self.correlated_markets.get(symbol):
                ...
                # Incorporate signals into early warnings
                if inter_market_signals:
                    early_warnings.update(inter_market_signals)

        # Adjust probabilities based on early warning indicators
        adjusted_probs = self._adjust_probabilities(transition_probs, early_warnings)
        ...
        # Map to probability level
        probability_level = self._map_probability_to_level(transition_probability)
        ...
        prediction = {
            ...
            "correlated_markets_count": len(self.correlated_markets.get(symbol, {})),
            "timestamp": datetime.now().isoformat()
        }

        # Update transition history
        self._update_transition_history(symbol, current_regime.value, prediction)

        return prediction

    def _calculate_early_warning_indicators(
        self,
        price_data: pd.DataFrame,
        ...
        # 2. Trend strength change indicator
        ...
        if high_col and low_col:
            ...
            # Compare recent ADX to historical
            recent_adx = adx.iloc[-5:].mean()
            historical_adx = adx.iloc[-20:-5].mean()

            if not np.isnan(recent_adx) and not np.isnan(historical_adx) and historical_adx > 0:
                adx_change = (recent_adx / historical_adx) - 1

                # Normalize to 0-1 range
                trend_strength_change = min(1.0, max(0.0, (adx_change + 0.5) / 1.5))
                indicators["trend_strength_change"] = trend_strength_change

        # 3. Price pattern indicator
        # Look for specific price patterns that often precede regime changes
        ...
        if not np.isnan(ma_short.iloc[-1]) and not np.isnan(ma_medium.iloc[-1]):
            ...
            if (ma_short.iloc[-2] < ma_medium.iloc[-2] and ma_short.iloc[-1] >= ma_medium.iloc[-1]) or \
               (ma_short.iloc[-2] > ma_medium.iloc[-2] and ma_short.iloc[-1] <= ma_medium.iloc[-1]):
                ma_crossover = 0.7

        if not np.isnan(ma_medium.iloc[-1]) and not np.isnan(ma_long.iloc[-1]):
            # Medium-term crossover
            if (ma_medium.iloc[-2] < ma_long.iloc[-2] and ma_medium.iloc[-1] >= ma_long.iloc[-1]) or \
               (ma_medium.iloc[-2] > ma_long.iloc[-2] and ma_medium.iloc[-1] <= ma_long.iloc[-1]):
                ma_crossover = 0.9

        indicators["ma_crossover"] = ma_crossover

        # 4. Volume change indicator (if volume data is available)
        ...
        if current_regime == MarketRegime.TRENDING:
            ...
            price_higher_high = close_prices.iloc[-1] > close_prices.iloc[-10:-2].max()
            ...
            # For ranging regimes, look for range expansion
            recent_range = (high_prices.iloc[-5:].max() - low_prices.iloc[-5:].min()) / close_prices.iloc[-5:].mean()
            historical_range = (high_prices.iloc[-20:-5].max() - low_prices.iloc[-20:-5].min()) / close_prices.iloc[-20:-5].mean()

            if historical_range > 0:
                range_expansion = (recent_range / historical_range) - 1

                # Normalize to 0-1 range
                range_expansion_indicator = min(1.0, max(0.0, (range_expansion + 0.5) / 1.5))
                indicators["range_expansion"] = range_expansion_indicator

        # Calculate overall early warning score
        if indicators:
            indicators["overall_warning"] = sum(indicators.values()) / len(indicators)
    ...
    def _adjust_probabilities(
        ...
        if overall_warning >= self.early_warning_threshold:
            # High warning level - adjust probabilities more aggressively

            # Specific adjustments based on indicators
            if "volatility_change" in early_warnings and early_warnings["volatility_change"] > 0.7:
                # High volatility change often precedes transition to volatile regime
                if MarketRegime.VOLATILE.value in adjusted_probs:
                    adjusted_probs[MarketRegime.VOLATILE.value] *= 1.5

            if "trend_strength_change" in early_warnings and early_warnings["trend_strength_change"] > 0.7:
                # Increasing trend strength often precedes transition to trending regime
                if MarketRegime.TRENDING.value in adjusted_probs:
                    adjusted_probs[MarketRegime.TRENDING.value] *= 1.5

            if "ma_crossover" in early_warnings and early_warnings["ma_crossover"] > 0.7:
                # MA crossovers often precede trend changes
                if MarketRegime.TRENDING.value in adjusted_probs:
                    adjusted_probs[MarketRegime.TRENDING.value] *= 1.3

            if "range_expansion" in early_warnings and early_warnings["range_expansion"] > 0.7:
            ...
            # Incorporate inter-market signals if available
            if "correlated_markets_dominant_regime" in early_warnings:
                dominant_regime = early_warnings.get("correlated_markets_dominant_regime_type")
                confidence = early_warnings.get("correlated_markets_dominant_regime")

                if dominant_regime and dominant_regime in adjusted_probs:
                    # Boost probability of the dominant regime among correlated markets
                    adjusted_probs[dominant_regime] *= (1.0 + confidence * 0.5)

            if "inter_market_transition_signal" in early_warnings:
                # General boost to transition probabilities based on inter-market signals
                signal_strength = early_warnings["inter_market_transition_signal"]

                # Apply stronger boost to regimes that are different from current
            ...
            if "leading_markets_signal" in early_warnings:
                # Boost from markets that historically lead the primary market
                leading_signal = early_warnings["leading_markets_signal"]

                # This is a stronger signal, so apply a larger boost
                for regime in adjusted_probs:
                    if regime != current_regime.value:
    ...
    def _map_probability_to_level(self, probability: float) -> TransitionProbability:
        """
    ...
    def _update_transition_history(
        ...
        # Add to history
        self.transition_history[symbol].append({
        ...
        if len(self.transition_history[symbol]) > self.transition_history_size:
            self.transition_history[symbol] = self.transition_history[symbol][-self.transition_history_size:]
    ...
    def update_transition_probabilities(
        ...
        if from_regime not in self.transition_probabilities:
            self.transition_probabilities[from_regime] = {
                r.value: 1.0 / (len(MarketRegime) - 1) for r in MarketRegime if r.value != from_regime
            }

        # Learning rate
        alpha = 0.1

        # Update probability of the specific transition
        current_prob = self.transition_probabilities[from_regime].get(to_regime, 0.0)

        if occurred:
            # Increase probability if transition occurred
            new_prob = current_prob + alpha * (1.0 - current_prob)
        else:
            # Decrease probability if transition did not occur
        ...
        if total_prob > 0:
            self.transition_probabilities[from_regime] = {
                k: v / total_prob for k, v in self.transition_probabilities[from_regime].items()
            }
    ...
    def get_most_likely_transitions(self, min_probability: float = 0.3) -> List[Dict[str, Any]]:
        """
            ...
            for to_regime, probability in transitions.items():
                if probability >= min_probability:
                    likely_transitions.append({
                        ...
                        "probability_level": self._map_probability_to_level(probability).value
                    })

        # Sort by probability (descending)
        return sorted(likely_transitions, key=lambda x: x["probability"], reverse=True)

    async def _analyze_correlated_markets(
        self,
        symbol: str,
        current_regime: MarketRegime,
        correlated_markets_data: Dict[str, pd.DataFrame]
    ) -> Dict[str, float]:
        """
        ...
        for correlated_symbol, correlation in self.correlated_markets[symbol].items():
            # Skip if no data available
            if correlated_symbol not in correlated_markets_data or correlated_markets_data[correlated_symbol].empty:
                continue

            # Detect regime for correlated market
            try:
                corr_regime_result = self.regime_detector.detect_regime(correlated_markets_data[correlated_symbol])
                corr_regime = corr_regime_result.get("regime", MarketRegime.UNKNOWN)

                # Count regimes
                regime_counts[corr_regime.value] += 1

                # Check if different from current regime (potential leading indicator)
                if corr_regime != current_regime:
                    regime_transitions_detected += 1

                    # Check if this market has historically led the primary market
                    # This would require historical analysis of regime transitions
                    # For now, we'll use a simplified approach based on correlation
                    if abs(correlation) > 0.8:
                        leading_markets_count += 1

            except Exception as e:
                self.logger.warning(f"Error detecting regime for {correlated_symbol}: {e}")

        # Calculate percentage of correlated markets in each regime
        total_markets = sum(regime_counts.values())
        if total_markets > 0:
            regime_percentages = {regime: count / total_markets for regime, count in regime_counts.items()}

            # Find dominant regime among correlated markets
            dominant_regime = max(regime_percentages.items(), key=lambda x: x[1])

            # If dominant regime is different from current and has significant representation
            if dominant_regime[0] != current_regime.value and dominant_regime[1] >= 0.5:
                signals["correlated_markets_dominant_regime"] = dominant_regime[1]
                signals["correlated_markets_dominant_regime_type"] = dominant_regime[0]
            ...
            # Add leading markets signal if any
    ...
    async def update_correlated_markets(self, symbol: str, min_correlation: float = 0.7) -> Dict[str, float]:
        """
        ...
        if not self.correlation_service:
            self.logger.warning("No correlation service available for inter-market analysis")
            return {}

        try:
            # Get correlated markets from correlation service
            correlated_markets = await self.correlation_service.get_highest_correlations(
                symbol=symbol,
                min_threshold=min_correlation
            )

            # Store in instance variable
            self.correlated_markets[symbol] = {
                market["symbol"]: market["correlation"]
                for market in correlated_markets
            }

            self.logger.info(f"Updated {len(self.correlated_markets[symbol])} correlated markets for {symbol}")
