...
from analysis_engine.core.exceptions_bridge import AnalysisError, AnalyzerNotFoundError, InsufficientDataError, InvalidAnalysisParametersError, AnalysisTimeoutError, MarketRegimeError, generate_correlation_id, async_with_exception_handling, with_exception_handling
...
class AnalysisService:
    ...
    def __init__(self, tool_effectiveness_repository: Optional[
        ToolEffectivenessRepository]=None):
        ...
        self.analyzers = {}
        self.analyzer_classes = {'confluence': ConfluenceAnalyzer}
    ...
    async def _discover_analyzers(self):
        ...
        try:
            ...
            for _, name, is_pkg in pkgutil.walk_packages(analysis_package.
                __path__, analysis_package.__name__ + '.'):
                if is_pkg:
                    continue
                try:
                    ...
                    for attr_name, attr_value in module.__dict__.items():
                        if inspect.isclass(attr_value) and issubclass(
                            ...
                            ) and attr_value != BaseAnalyzer:
                            analyzer_name = getattr(attr_value, 'name',
                                attr_name.lower())
    ...
    async def get_analyzer(self, name: str, parameters: Optional[Dict[str,
        Any]]=None) ->BaseAnalyzer:
        ...
        if name in self.analyzer_classes:
            try:
                ...
                analyzer = analyzer_class(tool_effectiveness_repository=
                    self.tool_effectiveness_repository, parameters=parameters)
                self.analyzers[name] = analyzer
                return analyzer
            except Exception as e:
                logger.error(f'Error initializing analyzer {name}: {str(e)}',
                    exc_info=True)
                raise AnalysisError(message=
                    f"Failed to initialize analyzer '{name}'", error_code=
                    'ANALYZER_INITIALIZATION_ERROR', details={
                    'analyzer_name': name, 'parameters': parameters,
                    'error': str(e)}, correlation_id=correlation_id)
        available_analyzers = list(self.analyzers.keys()) + [name for name in
            self.analyzer_classes.keys() if name not in self.analyzers]
        logger.warning(
            f"Analyzer '{name}' not found. Available analyzers: {available_analyzers}"
            , extra={'correlation_id': correlation_id})
        raise AnalyzerNotFoundError(analyzer_name=name, available_analyzers
            =available_analyzers, correlation_id=correlation_id)

    @async_with_exception_handling
    async def run_analysis(self, analyzer_name: str, data: Union[MarketData,
        Dict[str, MarketData]]) ->AnalysisResult:
        ...
        if not analyzer_name:
            raise InvalidAnalysisParametersError(message=
                'Analyzer name cannot be empty', error_code=
                'EMPTY_ANALYZER_NAME', details={'analyzer_name':
                analyzer_name}, correlation_id=correlation_id)
        if data is None:
            raise InvalidAnalysisParametersError(message=
                'Data cannot be null', error_code='NULL_DATA', details={
                'analyzer_name': analyzer_name}, correlation_id=correlation_id)
        try:
            ...
            if isinstance(data, MarketData):
                if not hasattr(data, 'is_valid') or not callable(data.is_valid
                    ...
                    logger.warning(
                        ...
                        , extra={'correlation_id': correlation_id,
                        'analyzer_name': analyzer_name, 'data_type': type(
                        data).__name__})
                    ...
                    raise InsufficientDataError(message=
                        f'Insufficient data for {analyzer_name} analysis',
                        symbol=symbol, timeframe=timeframe,
                        available_points=available_points, required_points=
                        getattr(analyzer, 'min_data_points', 'unknown'),
                        correlation_id=correlation_id)
            elif isinstance(data, dict):
                if not data:
                    raise InvalidAnalysisParametersError(message=
                        'Empty timeframe dictionary provided for analysis',
                        error_code='EMPTY_TIMEFRAME_DICT', details={
                        'analyzer_name': analyzer_name}, correlation_id=
                        correlation_id)
                invalid_timeframes = []
                for timeframe, market_data in data.items():
                    if not hasattr(market_data, 'is_valid') or not callable(
                        market_data.is_valid):
                        logger.warning(
                            f'MarketData object for timeframe {timeframe} does not have is_valid method'
                            , extra={'correlation_id': correlation_id,
                            'analyzer_name': analyzer_name, 'timeframe':
                            timeframe})
                        continue
                    if not market_data.is_valid():
                        symbol = getattr(market_data, 'symbol', 'unknown')
                        available_points = len(market_data.close) if hasattr(
                            market_data, 'close') else 0
                        invalid_timeframes.append({'timeframe': timeframe,
                            'symbol': symbol, 'available_points':
                            available_points})
                if invalid_timeframes:
                    raise InsufficientDataError(message=
                        f'Insufficient data for {analyzer_name} analysis in {len(invalid_timeframes)} timeframes'
                        , timeframes=invalid_timeframes, correlation_id=
                        correlation_id)
            start_time = time.time()
            try:
                result = await analyzer.execute(data)
                execution_time = time.time() - start_time
                logger.debug(f'Analysis completed: {analyzer_name}', extra=
                    {'correlation_id': correlation_id, 'analyzer_name':
                    analyzer_name, 'execution_time': execution_time,
                    'is_valid': result.is_valid if hasattr(result,
                    'is_valid') else True})
                if result and hasattr(result, 'metadata'
                    ) and 'correlation_id' not in result.metadata:
                    result.metadata['correlation_id'] = correlation_id
                return result
            except asyncio.TimeoutError:
                symbol = getattr(data, 'symbol', 'unknown') if isinstance(data,
                    MarketData) else 'multiple'
                timeframe = getattr(data, 'timeframe', 'unknown'
                    ) if isinstance(data, MarketData) else 'multiple'
                raise AnalysisTimeoutError(message=
                    f'Analysis timed out for {analyzer_name}',
                    analyzer_name=analyzer_name, symbol=symbol, timeframe=
                    timeframe, timeout_seconds=getattr(settings,
                    'ANALYSIS_TIMEOUT', 30), correlation_id=correlation_id)
        except (AnalyzerNotFoundError, InsufficientDataError,
            InvalidAnalysisParametersError, AnalysisTimeoutError):
            raise
        except Exception as e:
            symbol = 'unknown'
            timeframe = 'unknown'
            ...
            logger.error(
                ...
                analyzer_name, 'symbol': symbol, 'timeframe': timeframe},
                exc_info=True)
            raise AnalysisError(message=
                f'Error during analysis with {analyzer_name}', error_code=
                'ANALYSIS_EXECUTION_ERROR', details={'analyzer_name':
                analyzer_name, 'error': str(e), 'data_type': type(data).
                __name__, 'symbol': symbol, 'timeframe': timeframe,
                'traceback': traceback.format_exc()}, correlation_id=
                correlation_id)

    async def run_multi_timeframe_analysis(self, market_data: Dict[str,
        MarketData], parameters: Dict[str, Any]=None) ->AnalysisResult:
        ...
        try:
            ...
            if not market_data or len(market_data) == 0:
                raise InsufficientDataError(message=
                    'No timeframes provided for multi-timeframe analysis',
                    correlation_id=correlation_id)
            for timeframe, data in market_data.items():
                if not data.is_valid():
                    raise InsufficientDataError(message=
                        f'Insufficient data for timeframe {timeframe}',
                        timeframe=timeframe, symbol=getattr(data, 'symbol',
                        None), available_points=len(data.close) if hasattr(
                        data, 'close') else 0, correlation_id=correlation_id)
            start_time = time.time()
            result = await analyzer.execute(market_data)
            execution_time = time.time() - start_time
            logger.debug('Multi-timeframe analysis completed', extra={
                'correlation_id': correlation_id, 'timeframes': list(
                market_data.keys()), 'execution_time': execution_time,
                'is_valid': result.is_valid if hasattr(result, 'is_valid') else
                True})
            return result
        except (AnalyzerNotFoundError, InsufficientDataError):
            raise
        except Exception as e:
            logger.error(f'Error during multi-timeframe analysis: {str(e)}',
                extra={'correlation_id': correlation_id}, exc_info=True)
            raise AnalysisError(message=
                'Error during multi-timeframe analysis', error_code=
                'MULTI_TIMEFRAME_ANALYSIS_ERROR', details={'error': str(e),
                'timeframes': list(market_data.keys()) if market_data else
                []}, correlation_id=correlation_id)

    async def run_confluence_analysis(self, market_data: Any, parameters:
        Optional[Dict[str, Any]]=None) ->AnalysisResult:
        ...
        try:
            ...
            if isinstance(market_data, MarketData
                ) and not market_data.is_valid():
                raise InsufficientDataError(message=
                    'Insufficient data for confluence analysis', symbol=
                    getattr(market_data, 'symbol', None), timeframe=getattr
                    (market_data, 'timeframe', None), available_points=len(
                    market_data.close) if hasattr(market_data, 'close') else
                    0, correlation_id=correlation_id)
            start_time = time.time()
            result = await analyzer.execute(market_data)
            execution_time = time.time() - start_time
            logger.debug('Confluence analysis completed', extra={
                'correlation_id': correlation_id, 'execution_time':
                execution_time, 'is_valid': result.is_valid if hasattr(
                result, 'is_valid') else True, 'symbol': getattr(
                market_data, 'symbol', 'unknown') if isinstance(market_data,
                MarketData) else 'multiple'})
            return result
        except (AnalyzerNotFoundError, InsufficientDataError):
            raise
        except Exception as e:
            logger.error(f'Error during confluence analysis: {str(e)}',
                extra={'correlation_id': correlation_id}, exc_info=True)
            symbol = getattr(market_data, 'symbol', None) if isinstance(
                market_data, MarketData) else None
            timeframe = getattr(market_data, 'timeframe', None) if isinstance(
                market_data, MarketData) else None
            raise AnalysisError(message='Error during confluence analysis',
                error_code='CONFLUENCE_ANALYSIS_ERROR', details={'error':
                str(e), 'symbol': symbol, 'timeframe': timeframe,
                'data_type': type(market_data).__name__}, correlation_id=
                correlation_id)
    ...
    async def list_available_analyzers(self) ->List[Dict[str, Any]]:
        ...
        for name, analyzer in self.analyzers.items():
            available.append({'name': name, 'type': analyzer.__class__.
                __name__, 'active': True, 'parameters': analyzer.parameters})
        for name, analyzer_class in self.analyzer_classes.items():
            if name not in self.analyzers:
                available.append({'name': name, 'type': analyzer_class.
                    __name__, 'active': False, 'parameters': {}})
        return available

    @with_analysis_resilience('get_analyzer_details')
    async def get_analyzer_details(self, analyzer_name: str) ->Optional[Dict
        [str, Any]]:
        """
        ...
        if analyzer_name in self.analyzers:
            ...
            return {'name': analyzer_name, 'type': analyzer.__class__.
                __name__, 'active': True, 'parameters': analyzer.parameters,
                'performance': performance_metrics}
        if analyzer_name in self.analyzer_classes:
            analyzer_class = self.analyzer_classes[analyzer_name]
            return {'name': analyzer_name, 'type': analyzer_class.__name__,
                'active': False, 'parameters': {}, 'description': 
                analyzer_class.__doc__ or 'No description available'}
        return None

    @with_analysis_resilience('get_analyzer_performance')
    async def get_analyzer_performance(self, analyzer_name: str) ->Optional[
        Dict[str, Any]]:
        """
    ...
    @with_analysis_resilience('get_analyzer_effectiveness')
    async def get_analyzer_effectiveness(self, analyzer_name: str,
        ...
        if timeframe and 'by_timeframe' in summary:
            if timeframe in summary['by_timeframe']:
                return {'analyzer_name': analyzer_name, 'timeframe':
                    timeframe, **summary['by_timeframe'][timeframe]}
            else:
                return {'analyzer_name': analyzer_name, 'timeframe':
                    timeframe, 'error': f'No data for timeframe {timeframe}'}
        if instrument and 'by_instrument' in summary:
            if instrument in summary['by_instrument']:
                return {'analyzer_name': analyzer_name, 'instrument':
                    instrument, **summary['by_instrument'][instrument]}
            else:
                return {'analyzer_name': analyzer_name, 'instrument':
                    instrument, 'error': f'No data for instrument {instrument}'
                    }
        return summary

    async def record_effectiveness(self, analyzer_name: str,
        analysis_result: Dict[str, Any], actual_outcome: Dict[str, Any],
        timeframe: Optional[str]=None, instrument: Optional[str]=None
        ) ->Optional[str]:
        """
        ...
        if not analyzer:
            logger.warning(
                f'Cannot record effectiveness: analyzer {analyzer_name} not found'
                )
            return None
        if isinstance(analysis_result, dict):
            result = AnalysisResult(analyzer_name=analyzer_name,
                result_data=analysis_result.get('result_data', {}),
                is_valid=analysis_result.get('is_valid', True), metadata=
                analysis_result.get('metadata', {}))
        else:
            result = analysis_result
        log_id = analyzer.log_effectiveness(analysis_result=result,
            actual_outcome=actual_outcome, timeframe=timeframe, instrument=
            instrument)
        return log_id
