...
class AdaptiveLayerIntegrationService:
    ...
    def update_strategy_parameters(
        ...
        """
        try:
            self.logger.info(f"Updating strategy parameters for {strategy_id} on {symbol} {timeframe}")
            ...
            try:
                ...
                self.logger.warning(f"Invalid adaptation strategy: {adaptation_strategy}. Using MODERATE instead.")
            
            adaptive_layer = AdaptiveLayer(
                tool_effectiveness_repository=self.repository,
                market_regime_service=self.market_regime_service,
                adaptation_strategy=adaptation_mode
            )
            ...
            # Apply parameters to strategy execution engine if URL provided
            if self.strategy_execution_api_url:
                self.logger.info(f"Applying parameters to strategy execution engine for {strategy_id}")
                update_result = self._apply_parameters_to_strategy(
                ...
                return {
                    ...
                    "parameters": adaptive_params,
                    "applied": True,
                    "application_result": update_result,
                    "timestamp": datetime.now().isoformat()
                }
            else:
                self.logger.info(f"Strategy execution API URL not provided. Parameters generated but not applied.")
                return {
                    "strategy_id": strategy_id,
                    "parameters": adaptive_params,
                    "applied": False,
                    "application_result": "Strategy execution API URL not provided",
                    "timestamp": datetime.now().isoformat()
                }
        except Exception as e:
            self.logger.error(f"Error updating strategy parameters: {str(e)}", exc_info=True)
            return {
                "strategy_id": strategy_id,
                "error": str(e),
                "applied": False,
    ...
    def _apply_parameters_to_strategy(
        ...
        try:
            ...
            # Make API request to strategy execution engine
            endpoint = f"{self.strategy_execution_api_url}/strategies/{strategy_id}/parameters"
            ...
            if response.status_code == 200:
                ...
                self.logger.error(f"Error applying parameters to strategy: {response.text}")
                return {
                    "success": False,
                    "error": f"API error: {response.status_code} - {response.text}"
                }
        except Exception as e:
            self.logger.error(f"Error applying parameters to strategy: {str(e)}", exc_info=True)
            return {
                "success": False,
                "error": str(e)
            }
...
class AdaptiveStrategyOptimizer:
    ...
    def generate_strategy_recommendations(
        ...
        """
        try:
            self.logger.info(f"Generating recommendations for strategy {strategy_id} on {symbol} {timeframe}")
            ...
            self.logger.info(f"Detected market regime: {current_regime} with confidence {regime_confidence:.2f}")
            
            # Get effectiveness metrics for all available tools
            tool_metrics = self._get_tool_effectiveness(
            ...
            # Sort tools by effectiveness in current regime
            tools_by_effectiveness = sorted(
                tool_metrics, 
                key=lambda x: (x.get("win_rate", 0) * x.get("profit_factor", 0)), 
                reverse=True
            )
            
            # Find tools that should be added (high effectiveness, not currently used)
            tools_to_add = [
                t for t in tools_by_effectiveness[:5]  # Top 5 effective tools
                if t["tool_id"] not in current_tools and 
                t.get("sample_size", 0) >= 20 and  # Sufficient sample size
                t.get("win_rate", 0) >= 55 and  # Minimum win rate
                t.get("profit_factor", 0) >= 1.2  # Minimum profit factor
            ]
            
            # Find tools that should be removed (low effectiveness, currently used)
            tools_to_remove = [
                t for t in tools_by_effectiveness
                if t["tool_id"] in current_tools and (
                    t.get("sample_size", 0) >= 20 and  # Sufficient sample size
                    (t.get("win_rate", 0) < 45 or  # Low win rate
                     t.get("profit_factor", 0) < 1.0)  # Loss-making
                )
            ]
            
            # Find parameter optimizations for current tools
            ...
            self.logger.error(f"Error generating strategy recommendations: {str(e)}", exc_info=True)
            return {
                "strategy_id": strategy_id,
                "error": str(e),
                "timestamp": datetime.now().isoformat()
    ...
    def analyze_strategy_effectiveness_trend(
        self,
        strategy_id: str,
        symbol: str,
        timeframe: str,
        ...
        """
        try:
            self.logger.info(f"Analyzing effectiveness trend for strategy {strategy_id} on {symbol} {timeframe}")
            ...
            for i in range(look_back_periods):
                ...
                periods.append({
                    ...
                    "end_date": period_end,
                    "period": f"Period {look_back_periods - i}"
                })
            
            # Collect effectiveness metrics for each period
            period_metrics = []
            ...
            self.logger.error(f"Error analyzing strategy effectiveness trend: {str(e)}", exc_info=True)
            return {
                "strategy_id": strategy_id,
                "error": str(e),
                "timestamp": datetime.now().isoformat()
    ...
    def _get_tool_effectiveness(
        ...
        for tool_id in tools:
            try:
                # Get effectiveness metrics for this tool in the specified regime
                ...
                # Add tool ID to metrics
                metrics["tool_id"] = tool_id
                
                result.append(metrics)
                
            except Exception as e:
                self.logger.error(f"Error getting effectiveness for tool {tool_id}: {str(e)}")
    ...
    def _generate_parameter_optimizations(
        ...
        for tool_id in tools:
            try:
                # Get optimal parameters for this tool in the specified regime
                optimal_params = self.repository.get_optimal_parameters(
                    tool_id=tool_id,
                    symbol=symbol,
                    timeframe=timeframe,
                    regime=regime
                ...
                self.logger.error(f"Error getting optimal parameters for tool {tool_id}: {str(e)}")
                optimizations[tool_id] = {"error": str(e)}
        
        return optimizations
    
    def _analyze_effectiveness_trends(self, period_metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        """
        # Calculate trend directions (improving, stable, declining)
        win_rate_trend = self._calculate_metric_trend([
            p["metrics"].get("win_rate", 0) for p in period_metrics
        ])
        
        profit_factor_trend = self._calculate_metric_trend([
            p["metrics"].get("profit_factor", 0) for p in period_metrics
        ])
        
        # Calculate stability (standard deviation)
        import numpy as np
        win_rate_stability = np.std([
            p["metrics"].get("win_rate", 0) for p in period_metrics
        ]) if len(period_metrics) > 1 else 0
        
        profit_factor_stability = np.std([
            p["metrics"].get("profit_factor", 0) for p in period_metrics
        ]) if len(period_metrics) > 1 else 0
    ...
    def _calculate_metric_trend(self, values: List[float]) -> str:
        ...
        # Check if values are consistently increasing or decreasing
        ...
        if avg_change > 0.02:  # 2% improvement threshold
            return "improving"
        elif avg_change < -0.02:  # 2% decline threshold
            return "declining"
        else:
            return "stable"
