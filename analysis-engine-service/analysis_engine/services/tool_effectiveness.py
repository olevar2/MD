...
class PredictionResult:
    ...
    def is_correct(self) -> bool:
        ...
        return (
            self.actual_outcome is not None and
            self.prediction == self.actual_outcome
        )
...
class ToolEffectivenessTracker:
    ...
    def record_prediction(self, result: PredictionResult):
        ...
        """
        try:
            # Initialize tool history if needed
            ...
            # Update regime performance if outcome is known
        ...
        try:
            ...
            if market_regime:
                predictions = [
                    ...
                    if p.market_regime == market_regime
                ]
            if lookback:
            ...
            regime_performance = {
                regime.value: self.regime_performance[regime].get(tool_id, 0.0)
                for regime in MarketRegime
                if tool_id in self.regime_performance[regime]
            }
            ...
            self.logger.error(
                f"Error getting tool effectiveness: {str(e)}",
                exc_info=True
            )
            return {
                "tool_id": tool_id,
    ...
    def get_best_tools(
        ...
        """
        try:
            # Get all tools with sufficient predictions
            tool_metrics = []
            
            for tool_id in self.predictions:
                metrics = self.get_tool_effectiveness(
                    tool_id,
            ...
            sorted_metrics = sorted(
                tool_metrics,
                key=lambda x: (
                    x["regime_performance"].get(market_regime.value, 0.0),
                    x["accuracy"]
                ),
                reverse=True
            )
            ...
            self.logger.error(
                f"Error getting best tools: {str(e)}",
                exc_info=True
            )
            return []
    ...
    def get_tool_history(
        ...
        try:
            ...
            history = [
                pred.to_dict()
                for pred in reversed(self.predictions[tool_id])
            ]
            ...
            self.logger.error(
                f"Error getting tool history: {str(e)}",
                exc_info=True
            )
            return []
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        try:
            summary = {
                "total_tools": len(self.predictions),
            ...
            for regime in MarketRegime:
                best_tools = self.get_best_tools(
                    regime,
                    min_predictions=10,
                    top_n=5
                )
                
                summary["regime_performance"][regime.value] = {
                    "top_tools": [
                        {
                            "tool_id": tool["tool_id"],
                            "accuracy": tool["accuracy"],
                            "impact": tool["average_impact"]
                        }
                        for tool in best_tools
                    ]
                }
            
            # Calculate overall tool rankings
            all_tools = []
            for tool_id in self.predictions:
            ...
            sorted_tools = sorted(
                all_tools,
                key=lambda x: (x["accuracy"], x["average_impact"]),
                reverse=True
            )
            
            summary["tool_rankings"] = {
                "by_accuracy": [
                    {
                        "tool_id": tool["tool_id"],
                        "accuracy": tool["accuracy"]
