...
class ToolEffectivenessService:
    ...
    def generate_performance_report(
        ...
        metrics_over_time = self.repository.get_aggregated_metrics(
            tool_id=tool_id,
        ...
        report_data = {
            ...
            "recommendations": self._generate_recommendations(win_rate, total_pnl, metrics_over_time)
        }
    ...
    def compare_tools(
        ...
        comparison_data = self.repository.get_comparative_metrics(
            tool_ids=tool_ids,
        ...
        for tool_id, metrics in comparison_data.items():
            ...
            result["tools"][tool_id] = {
                ...
                "trend": self._calculate_trend([m.metric_value for m in metrics]) if metrics else "neutral"
            }
        ...
        sorted_tools = sorted(
            ...
            reverse=(metric_type != "drawdown")  # For drawdown, lower is better
        )
    ...
    def detect_market_regime(
        ...
        # This is a placeholder - would need market data service integration
        # In a real implementation, this would analyze price action, volatility, etc.
        
        return {
            "instrument": instrument,
            "timeframe": timeframe,
            "regime": "trending",  # Options: trending, ranging, volatile
            "confidence": 0.85,
            "volatility": "medium",
    ...
    def _generate_recommendations(
        ...
        win_rate: float, 
        total_pnl: float,
        metrics_over_time: Dict[str, Dict[str, float]]
    ) -> List[Dict[str, Any]]:
        ...
        if win_rate < 0.4:
            recommendations.append({
                "type": "warning",
                "metric": "win_rate",
                "message": "Win rate is below 40%, consider reviewing entry criteria"
            })
        elif win_rate > 0.65:
            recommendations.append({
                "type": "positive",
                "metric": "win_rate",
                "message": "Excellent win rate above 65%, maintain current approach"
            })
        ...
        if total_pnl < 0:
            recommendations.append({
                "type": "warning",
                "metric": "pnl",
                "message": "Negative total PnL, review position sizing and stop loss strategy"
            })
        
        # Trend-based recommendations
        if metrics_over_time and len(metrics_over_time) >= 2:
            periods = sorted(metrics_over_time.keys())
            if "win_rate" in metrics_over_time[periods[-1]] and "win_rate" in metrics_over_time[periods[0]]:
                ...
                if latest_win_rate < first_win_rate * 0.8:
                    recommendations.append({
                        "type": "warning",
                        "metric": "win_rate_trend",
                        "message": "Win rate has decreased significantly, consider adapting to changing market conditions"
                    })
