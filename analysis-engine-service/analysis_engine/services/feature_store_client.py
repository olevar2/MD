...
from analysis_engine.resilience import (
    ...
    retry_with_policy, timeout_handler, bulkhead
)
...
class FeatureStoreClient:
    ...
    def __init__(self, settings: Settings = None):
        ...
        self.indicator_cb = create_circuit_breaker(
            service_name="analysis_engine",
            resource_name="feature_store_indicator",
            config=get_circuit_breaker_config("feature_store")
        )

        self.market_data_cb = create_circuit_breaker(
            service_name="analysis_engine",
            resource_name="feature_store_market_data",
            config=get_circuit_breaker_config("feature_store")
        )

        self.recalculate_cb = create_circuit_breaker(
            service_name="analysis_engine",
            resource_name="feature_store_recalculate",
            config=get_circuit_breaker_config("feature_store")
        )

        self.metadata_cb = create_circuit_breaker(
            service_name="analysis_engine",
            resource_name="feature_store_metadata",
            config=get_circuit_breaker_config("feature_store")
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "data" in data and data["data"]:
                    ...
                    self.logger.warning(f"No indicator data returned for {indicator_type} on {symbol}")
                    return pd.DataFrame()
            else:
                error_msg = f"Error fetching {indicator_type} indicator: {response.text}"
                self.logger.error(error_msg)
                raise FeatureStoreError(error_msg)

        except requests.RequestException as e:
            error_msg = f"Request error fetching {indicator_type} indicator: {str(e)}"
            self.logger.error(error_msg)
            raise FeatureStoreError(error_msg)

        except Exception as e:
            error_msg = f"Unexpected error fetching {indicator_type} indicator: {str(e)}"
            self.logger.error(error_msg)
            raise FeatureStoreError(error_msg)
    ...
    def get_market_data(
        ...
        try:
            ...
            # If lookback is specified, calculate start_date
            if lookback and not start_date:
                if not end_date:
                    end_date = datetime.now()

                # Calculate approximate start date based on lookback and timeframe
                if isinstance(end_date, str):
                    end_dt = pd.to_datetime(end_date)
                else:
                    end_dt = end_date

                # Simple lookback calculation (can be improved with business days, etc.)
                if timeframe.endswith('m'):
                    minutes = int(timeframe[:-1])
                    start_date = (end_dt - timedelta(minutes=minutes * lookback)).isoformat()
                elif timeframe.endswith('h'):
                    hours = int(timeframe[:-1])
                    start_date = (end_dt - timedelta(hours=hours * lookback)).isoformat()
                elif timeframe.endswith('d'):
                    days = int(timeframe[:-1])
                    start_date = (end_dt - timedelta(days=days * lookback)).isoformat()
                else:
                    # Default to daily if timeframe format is unknown
    ...
    @with_resilience(
        service_name="analysis_engine",
        operation_name="recalculate_indicator",
        service_type="feature_store",
        exceptions=[requests.RequestException, requests.Timeout, requests.ConnectionError]
    )
    def recalculate_indicator(
        ...
        try:
            ...
            if response.status_code in (200, 202):
                ...
                error_msg = f"Error requesting indicator recalculation: {response.text}"
                self.logger.error(error_msg)
                raise FeatureStoreError(error_msg)
            ...
            error_msg = f"Unexpected error for indicator recalculation: {str(e)}"
            self.logger.error(error_msg)
            raise FeatureStoreError(error_msg)

    @with_resilience(
        service_name="analysis_engine",
        operation_name="get_available_indicators",
        service_type="feature_store",
        exceptions=[requests.RequestException, requests.Timeout, requests.ConnectionError]
    )
    def get_available_indicators(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_msg = f"Error fetching available indicators: {response.text}"
                self.logger.error(error_msg)
                raise FeatureStoreError(error_msg)

        except requests.RequestException as e:
            error_msg = f"Request error fetching available indicators: {str(e)}"
            self.logger.error(error_msg)
            raise FeatureStoreError(error_msg)

        except Exception as e:
            error_msg = f"Unexpected error fetching available indicators: {str(e)}"
            self.logger.error(error_msg)
            raise FeatureStoreError(error_msg)
