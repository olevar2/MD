...
def get_causal_inference_service(request: Request) ->CausalInferenceService:
    ...
    try:
        service = request.app.state.service_container.resolve(
            CausalInferenceService)
        if not service:
            raise HTTPException(status_code=503, detail=
                'CausalInferenceService not available')
        return service
    except Exception as e:
        logger.error(f'Error resolving CausalInferenceService: {e}',
            exc_info=True)
        raise HTTPException(status_code=500, detail=
            'Could not resolve CausalInferenceService')
...
async def get_analysis_provider(request: Request) ->IAnalysisProvider:
    ...
    try:
        adapter_factory = request.app.state.service_container.get_service(
            'adapter_factory')
