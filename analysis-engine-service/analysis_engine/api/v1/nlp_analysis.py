...
async def analyze_economic_report(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to analyze economic report: {str(e)}"
        )
...
async def get_combined_insights(
    ...
    try:
        ...
        if not news_data and not economic_reports:
            raise HTTPException(
                status_code=400,
                detail="Request must contain at least 'news_data' or 'economic_reports'"
            )
            
        results = await nlp_integration.generate_nlp_insights(news_data, economic_reports)
        
        # Filter by currency pairs if specified
        if currency_pairs and isinstance(currency_pairs, list) and "aggregate_insights" in results:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to generate combined insights: {str(e)}"
        )
