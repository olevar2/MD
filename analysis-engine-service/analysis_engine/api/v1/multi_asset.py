...
async def list_assets(
    ...
    multi_asset_service: MultiAssetService = Depends(get_multi_asset_service),
    ...
    try:
        ...
        if asset_class:
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid asset class: {asset_class}. Must be one of: {', '.join([a.value for a in AssetClass])}"
                )
        
        # Get assets filtered by class if specified
        if asset_class_enum:
            symbols = multi_asset_service.list_assets_by_class(asset_class_enum)
        else:
            # Get all assets from all supported classes
            symbols = []
        ...
        for symbol in symbols:
            ...
            if asset_info:
                assets.append({
                    "symbol": symbol,
                    "display_name": asset_info.get("display_name", symbol),
                    "asset_class": asset_info.get("asset_class"),
                    "market_type": asset_info.get("market_type"),
                    "base_currency": asset_info.get("base_currency"),
                    "quote_currency": asset_info.get("quote_currency")
                })
...
async def get_correlated_assets(
    symbol: str = Path(..., description="Asset symbol"),
    threshold: float = Query(0.6, description="Minimum correlation threshold", ge=0.0, le=1.0),
    multi_asset_service: MultiAssetService = Depends(get_multi_asset_service),
    current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        correlated_assets = multi_asset_service.asset_registry.get_correlated_assets(symbol, threshold)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get correlated assets: {str(e)}"
        )
...
async def list_asset_groups(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to list asset groups: {str(e)}"
        )
...
async def get_asset_group(
    ...
    try:
        ...
        if not symbols:
            raise HTTPException(
                status_code=404,
                detail=f"Group not found or empty: {group_name}"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get asset group: {str(e)}"
        )
...
async def get_analysis_parameters(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get analysis parameters: {str(e)}"
        )
