...
def get_correlation_service(multi_asset_service: MultiAssetService = Depends()) -> CorrelationTrackingService:
    """Dependency for getting the correlation tracking service"""
    return CorrelationTrackingService(asset_registry=multi_asset_service.asset_registry)


@router.get("/matrix", summary="Get correlation matrix")
async def get_correlation_matrix(
    symbols: List[str] = Query(..., description="List of symbols to include in the correlation matrix"),
    correlation_service: CorrelationTrackingService = Depends(get_correlation_service),
...
async def get_symbol_correlations(
    symbol: str = Path(..., description="Symbol to find correlations for"),
    threshold: float = Query(0.7, description="Minimum correlation threshold", ge=0.0, le=1.0),
    limit: int = Query(10, description="Maximum number of results to return", ge=1, le=50),
    correlation_service: CorrelationTrackingService = Depends(get_correlation_service),
    multi_asset_service: MultiAssetService = Depends(),
    current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        correlations = await correlation_service.get_highest_correlations(symbol, threshold, limit)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get symbol correlations: {str(e)}"
        )
...
async def get_cross_asset_correlations(
    symbol: str = Path(..., description="Symbol to find correlations for"),
    asset_classes: Optional[List[str]] = Query(None, description="Asset classes to include"),
    correlation_service: CorrelationTrackingService = Depends(get_correlation_service),
    multi_asset_service: MultiAssetService = Depends(),
    current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        # Convert string asset classes to enums if provided
        asset_class_enums = None
        if asset_classes:
            try:
                asset_class_enums = [AssetClass(ac.lower()) for ac in asset_classes]
            except ValueError as e:
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid asset class: {str(e)}"
                )
        
        results = await correlation_service.get_cross_asset_correlations(
            symbol, asset_class_enums
        )
        
        # Convert enum keys to strings for JSON response
        string_results = {ac.value if isinstance(ac, AssetClass) else ac: vals 
                         for ac, vals in results.items()}
        
        return {
            "symbol": symbol,
            "asset_class": asset_info.get("asset_class"),
            "cross_asset_correlations": string_results
        }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get cross-asset correlations: {str(e)}"
        )


@router.get("/changes/{symbol1}/{symbol2}", summary="Get correlation changes")
async def get_correlation_changes(
    symbol1: str = Path(..., description="First symbol"),
    symbol2: str = Path(..., description="Second symbol"),
    correlation_service: CorrelationTrackingService = Depends(get_correlation_service),
    multi_asset_service: MultiAssetService = Depends(),
    current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        if not multi_asset_service.get_asset_info(symbol1):
            raise HTTPException(
                status_code=404,
                detail=f"Asset not found: {symbol1}"
            )
            
        if not multi_asset_service.get_asset_info(symbol2):
            raise HTTPException(
                status_code=404,
                detail=f"Asset not found: {symbol2}"
            )
        
        changes = await correlation_service.get_correlation_changes(symbol1, symbol2)
        
        # Check for regime change
        regime_change = await correlation_service.find_correlation_regime_change(symbol1, symbol2)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get correlation changes: {str(e)}"
        )


@router.get("/visualization", summary="Get correlation visualization data")
async def get_visualization_data(
    symbols: List[str] = Query(..., description="List of symbols to include"),
    correlation_service: CorrelationTrackingService = Depends(get_correlation_service),
    current_user: User = Depends(get_current_user)
):
    """
...
async def update_correlations(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to update correlations: {str(e)}"
        )
