...
def find_client_modules() ->List[str]:
    ...
    for file_path in client_dir.glob('*.py'):
        if file_path.name.startswith('__'
            ) or file_path.name == 'client_factory.py':
            continue
    ...
    """
        ...
        for router in routers:
            for route in router.routes:
                if route.methods and ('HEAD' in route.methods or 'OPTIONS' in
                    route.methods):
                    continue
                http_method = 'GET'
                if route.methods:
                ...
                has_response_model = hasattr(route, 'response_model'
                    ) and route.response_model is not None
                has_summary = hasattr(route, 'summary'
                    ) and route.summary is not None
                has_description = hasattr(route, 'description'
                    ) and route.description is not None
                ...
                endpoints.append(EndpointInfo(path=path, http_method=
                    http_method, function_name=function_name, module_path=
                    module_name, has_response_model=has_response_model,
                    has_summary=has_summary, has_description=
                    has_description, has_correlation_id=has_correlation_id,
    ...
    try:
        ...
        for name, obj in inspect.getmembers(module):
            if inspect.isclass(obj
                ) and 'Client' in name and name != 'BaseClient':
                client_class = obj
                break
        if not client_class:
            return None
        ...
        if ('retry_with_backoff' in source_code or 'circuit_breaker' in
            source_code):
            has_resilience = True
        ...
        if 'timeout' in source_code:
            has_timeout = True
        for name, method in inspect.getmembers(client_class, predicate=
            inspect.isfunction):
            ...
            for param_name, param in signature.parameters.items():
                ...
                if param.annotation == inspect.Parameter.empty:
                    ...
                    break
            if method.__annotations__.get('return', inspect.Parameter.empty
                ) == inspect.Parameter.empty:
                has_type_hints = False
                break
        return ClientInfo(name=client_class.__name__, module_path=
            module_name, has_resilience=has_resilience, has_error_handling=
            has_error_handling, has_logging=has_logging, has_timeout=
            has_timeout, has_type_hints=has_type_hints)
    except Exception as e:
...
def validate_standardization() ->ValidationResult:
    ...
    for endpoint in all_endpoints:
        is_compliant = True
        endpoint_issues = []
        if not re.match('^/v1/analysis/[a-z-]+/.*$', endpoint.path):
            is_compliant = False
            endpoint_issues.append(
                f'URL structure does not follow pattern: {endpoint.path}')
        if (not endpoint.has_response_model and endpoint.http_method !=
            'DELETE'):
            is_compliant = False
        ...
        if is_compliant:
            ...
            issues.append(
                f'Endpoint {endpoint.http_method} {endpoint.path} ({endpoint.function_name}) has issues:'
                )
    ...
    for client in all_clients:
        ...
        if is_compliant:
            ...
            for issue in client_issues:
                issues.append(f'  - {issue}')
    return ValidationResult(endpoints=all_endpoints, clients=all_clients,
        compliant_endpoints=compliant_endpoints, non_compliant_endpoints=
        non_compliant_endpoints, compliant_clients=compliant_clients,
        non_compliant_clients=non_compliant_clients, issues=issues)


def generate_report(result: ValidationResult, output_file: Optional[str]=None
    ) ->None:
    ...
    report.append(
        f'- Non-Compliant Endpoints: {result.non_compliant_endpoints}')
    report.append(
        f'- Compliance Rate: {result.compliant_endpoints / len(result.endpoints) * 100:.2f}%'
        )
    report.append('')
    ...
    report.append(
        f'- Compliance Rate: {result.compliant_clients / len(result.clients) * 100:.2f}%'
        )
    report.append('')
    if result.issues:
    ...
    report.append('')
    report.append(
        '| Method | Path | Function | Response Model | Summary | Description | Correlation ID | Error Handling |'
        )
    report.append(
        '|--------|------|----------|---------------|---------|-------------|---------------|----------------|'
        )
    for endpoint in sorted(result.endpoints, key=lambda e: e.path):
        report.append(
            f"| {endpoint.http_method} | {endpoint.path} | {endpoint.function_name} | {'✅' if endpoint.has_response_model else '❌'} | {'✅' if endpoint.has_summary else '❌'} | {'✅' if endpoint.has_description else '❌'} | {'✅' if endpoint.has_correlation_id else '❌'} | {'✅' if endpoint.has_error_handling else '❌'} |"
            )
    report.append('')
    report.append('## Clients')
    report.append('')
    report.append(
        '| Client | Resilience | Error Handling | Logging | Timeout | Type Hints |'
        )
    report.append(
        '|--------|------------|---------------|---------|---------|------------|'
        )
    for client in sorted(result.clients, key=lambda c: c.name):
        report.append(
            f"| {client.name} | {'✅' if client.has_resilience else '❌'} | {'✅' if client.has_error_handling else '❌'} | {'✅' if client.has_logging else '❌'} | {'✅' if client.has_timeout else '❌'} | {'✅' if client.has_type_hints else '❌'} |"
            )
    report_text = '\n'.join(report)
    if output_file:
