        ...
        schema_extra = {
            "example": {
                "ohlcv": [
                    ...
                    {"timestamp": "2025-04-01T13:00:00", "open": 1.2355, "high": 1.2370, "low": 1.2350, "close": 1.2365, "volume": 1100}
                ],
                "metadata": {
                    "symbol": "EUR/USD",
                    "timeframe": "1h"
...
router = APIRouter(
    prefix="/v1/analysis/manipulation-detection",
    tags=["Manipulation Detection"]
)
...
@router.post(
    ...
    summary="Detect manipulation patterns",
    description="Analyze market data for potential manipulation patterns."
)
async def detect_manipulation_patterns(
    request: ManipulationDetectionRequest,
    request_obj: Request,
    ...
    try:
        ...
        if len(request.ohlcv) < 100:
            raise InsufficientDataError(
                message="OHLCV data must contain at least 100 data points",
        ...
        # Convert request to the format expected by the detector
        data = {
            "ohlcv": [item.dict() for item in request.ohlcv],
        ...
        if not result.is_valid:
            raise ManipulationDetectionError(
        ...
        # Wrap other exceptions in ManipulationDetectionError
        logger.error(
            f"Error detecting manipulation patterns: {str(e)}",
            extra={"correlation_id": correlation_id},
            exc_info=True
        )

        raise ManipulationDetectionError(
            message="Failed to detect manipulation patterns",
            correlation_id=correlation_id
        )
...
@router.post(
    "/stop-hunting",
    response_model=AnalysisResponse,
    summary="Detect stop hunting",
    description="Specifically analyze for stop hunting patterns."
)
async def detect_stop_hunting(
    ...
    try:
        ...
        # Wrap other exceptions in ManipulationDetectionError
        logger.error(
            f"Error detecting stop hunting patterns: {str(e)}",
            extra={"correlation_id": correlation_id},
            exc_info=True
        )

        raise ManipulationDetectionError(
            message="Failed to detect stop hunting patterns",
            correlation_id=correlation_id
        )
...
@router.post(
    "/fake-breakouts",
    response_model=AnalysisResponse,
    summary="Detect fake breakouts",
    description="Specifically analyze for fake breakout patterns."
)
async def detect_fake_breakouts(
    ...
    try:
        ...
        # Wrap other exceptions in ManipulationDetectionError
        logger.error(
            f"Error detecting fake breakout patterns: {str(e)}",
            extra={"correlation_id": correlation_id},
            exc_info=True
        )

        raise ManipulationDetectionError(
            message="Failed to detect fake breakout patterns",
            correlation_id=correlation_id
        )
...
@router.post(
    "/volume-anomalies",
    response_model=AnalysisResponse,
    summary="Detect volume anomalies",
    description="Specifically analyze for volume anomalies."
)
async def detect_volume_anomalies(
    ...
    try:
        ...
        # Wrap other exceptions in ManipulationDetectionError
        logger.error(
            f"Error detecting volume anomalies: {str(e)}",
            extra={"correlation_id": correlation_id},
            exc_info=True
        )

        raise ManipulationDetectionError(
            message="Failed to detect volume anomalies",
            correlation_id=correlation_id
        )
...
legacy_router = APIRouter(
    prefix="/api/v1/manipulation",
    tags=["Manipulation Detection (Legacy)"]
)
...
async def legacy_detect_manipulation_patterns(
    data: Dict[str, Any],
    sensitivity: Optional[float] = Query(1.0, description="Detection sensitivity multiplier (0.5-2.0)"),
    include_protection: Optional[bool] = Query(True, description="Include protection recommendations"),
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/manipulation-detection/detect")
    ...
    request = ManipulationDetectionRequest(ohlcv=ohlcv_models, metadata=metadata_model)
    return await detect_manipulation_patterns(request, request_obj, sensitivity, include_protection, current_user)

@legacy_router.post("/stop-hunting", response_model=AnalysisResponse)
async def legacy_detect_stop_hunting(
    data: Dict[str, Any],
    lookback: Optional[int] = Query(30, description="Lookback period for stop hunting detection"),
    recovery_threshold: Optional[float] = Query(0.5, description="Recovery percentage threshold"),
    request_obj: Request = None,
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/manipulation-detection/stop-hunting")
    ...
    request = ManipulationDetectionRequest(ohlcv=ohlcv_models, metadata=metadata_model)
    return await detect_stop_hunting(request, request_obj, lookback, recovery_threshold, current_user)

@legacy_router.post("/fake-breakouts", response_model=AnalysisResponse)
async def legacy_detect_fake_breakouts(
    data: Dict[str, Any],
    threshold: Optional[float] = Query(0.7, description="Fake breakout detection threshold"),
    request_obj: Request = None,
    current_user: User = Depends(get_current_user)
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/manipulation-detection/fake-breakouts")
    ...
    request = ManipulationDetectionRequest(ohlcv=ohlcv_models, metadata=metadata_model)
    return await detect_fake_breakouts(request, request_obj, threshold, current_user)

@legacy_router.post("/volume-anomalies", response_model=AnalysisResponse)
async def legacy_detect_volume_anomalies(
    data: Dict[str, Any],
    z_threshold: Optional[float] = Query(2.0, description="Z-score threshold for volume anomaly detection"),
    request_obj: Request = None,
    current_user: User = Depends(get_current_user)
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/manipulation-detection/volume-anomalies")
    ...
    request = ManipulationDetectionRequest(ohlcv=ohlcv_models, metadata=metadata_model)
    return await detect_volume_anomalies(request, request_obj, z_threshold, current_user)
