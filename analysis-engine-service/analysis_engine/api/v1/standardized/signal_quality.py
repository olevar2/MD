...
class QualityTrendRequest(BaseModel):
    ...
    window_size: int = Field(20, description="Size of moving window for trend analysis")
...
router = APIRouter(
    prefix="/v1/analysis/signal-quality",
    tags=["Signal Quality"]
)
...
@router.post(
    ...
    summary="Evaluate signal quality",
    description="Evaluate the quality of a specific trading signal and store the results."
)
async def evaluate_signal_quality(
    signal_id: str = Path(..., description="ID of the signal to evaluate"),
    request: QualityEvaluationRequest = None,
    request_obj: Request = None,
    repository: ToolEffectivenessRepository = Depends(get_repository),
    evaluator: SignalQualityEvaluator = Depends(get_quality_evaluator)
):
    """
    ...
    # Increment signal evaluation operations counter
    analysis_engine_metrics.signal_generation_operations_total.labels(
        service="analysis-engine-service",
        signal_type="quality_evaluation",
        instrument=request.market_context.get("instrument", "unknown"),
        timeframe=request.market_context.get("timeframe", "unknown")
    ).inc()
    ...
    try:
        # If request is None, create an empty one
        if request is None:
        ...
        quality_metrics = evaluator.evaluate_signal_quality(
            signal=signal_event,
        ...
        logger.info(
            f"Evaluated quality for signal {signal_id}",
            extra={
                "correlation_id": correlation_id,
                "signal_id": signal_id,
        ...
        analysis_engine_metrics.signal_quality_score.labels(
            service="analysis-engine-service",
            signal_type="quality_evaluation",
            instrument=request.market_context.get("instrument", "unknown"),
            timeframe=request.market_context.get("timeframe", "unknown")
        ).set(quality_metrics['overall_quality'])
        ...
        analysis_engine_metrics.analysis_errors_total.labels(
            service="analysis-engine-service",
            analysis_type="signal_quality_evaluation",
        ...
        # Wrap other exceptions in SignalQualityError
        logger.error(
        ...
        raise SignalQualityError(
            message=f"Failed to evaluate signal quality for {signal_id}",
            correlation_id=correlation_id
        )
...
@router.post(
    ...
    summary="Analyze signal quality",
    description="Analyze the relationship between signal quality and outcomes."
)
async def analyze_signal_quality(
    ...
    try:
        ...
        if not signal_outcomes:
            raise InsufficientDataError(
                message="No outcomes found for the signals",
                correlation_id=correlation_id
            )
        ...
        # Wrap other exceptions in SignalQualityError
        logger.error(
            f"Error analyzing signal quality: {str(e)}",
            extra={"correlation_id": correlation_id},
            exc_info=True
        )

        raise SignalQualityError(
            message="Failed to analyze signal quality",
            correlation_id=correlation_id
        )
...
@router.post(
    ...
    description="Analyze trends in signal quality over time for a specific tool."
)
async def analyze_quality_trends(
    ...
    try:
        ...
        if not signals or len(signals) < request.window_size:
            raise InsufficientDataError(
                message=f"Insufficient signals found. Need at least {request.window_size}, found {len(signals) if signals else 0}",
                correlation_id=correlation_id
            )
        ...
        trend_result = analyzer.analyze_quality_trends(signal_outcomes, request.window_size)

        if "error" in trend_result:
            raise AnalysisError(
                message=trend_result["error"],
                correlation_id=correlation_id
            )

        # Log successful analysis
        logger.info(
            f"Analyzed quality trends for tool {request.tool_id}",
            extra={
                "correlation_id": correlation_id,
                "tool_id": request.tool_id,
        ...
        raise SignalQualityError(
            message=f"Failed to analyze quality trends for tool {request.tool_id}",
            correlation_id=correlation_id
        )
...
legacy_router = APIRouter(
    prefix="/api/v1/signal-quality",
    tags=["Signal Quality (Legacy)"]
)
...
@legacy_router.post("/signals/{signal_id}/quality", response_model=SignalQualityResponse)
async def legacy_evaluate_signal_quality(
    signal_id: str,
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/signal-quality/signals/{signal_id}/evaluate")
    return await evaluate_signal_quality(signal_id, request, request_obj, repository, evaluator)

@legacy_router.get("/quality-analysis", response_model=QualityAnalysisResponse)
async def legacy_analyze_signal_quality(
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/signal-quality/analyze")
    request = QualityAnalysisRequest(
        tool_id=tool_id,
        timeframe=timeframe,
    ...
    return await analyze_signal_quality(request, request_obj, repository, analyzer)

@legacy_router.get("/quality-trends", response_model=QualityTrendResponse)
async def legacy_analyze_quality_trends(
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/signal-quality/trends")
    request = QualityTrendRequest(
        tool_id=tool_id,
        window_size=window_size,
        days=days
    )
    return await analyze_quality_trends(request, request_obj, repository, analyzer)
