...
from analysis_engine.services.tool_effectiveness import (
    ...
    ReliabilityByMarketRegimeMetric
)
...
class SignalRequest(BaseModel):
    ...
    tool_id: str = Field(..., description="Identifier for the trading tool that generated the signal")
    signal_type: str = Field(..., description="Type of signal (buy, sell, etc.)")
    ...
    confidence: float = Field(default=1.0, ge=0.0, le=1.0, description="Signal confidence level (0.0-1.0)")
    timeframe: str = Field(..., description="Timeframe of the analysis (e.g., '1H', '4H', '1D')")
    market_regime: MarketRegimeEnum = Field(default=MarketRegimeEnum.UNKNOWN, description="Market regime at signal time")
    additional_data: Optional[Dict] = Field(default=None, description="Any additional signal metadata")
...
class OutcomeRequest(BaseModel):
    ...
    signal_id: str = Field(..., description="ID of the signal that this outcome is associated with")
    success: bool = Field(..., description="Whether the signal led to a successful trade")
    realized_profit: float = Field(default=0.0, description="Profit/loss realized from the trade")
    timestamp: datetime = Field(..., description="When the outcome was recorded")
    additional_data: Optional[Dict] = Field(default=None, description="Any additional outcome metadata")
...
class ReportRequest(BaseModel):
    ...
    tool_id: Optional[str] = Field(None, description="Filter by specific tool ID")
    timeframe: Optional[str] = Field(None, description="Filter by specific timeframe")
    instrument: Optional[str] = Field(None, description="Filter by specific instrument")
    market_regime: Optional[MarketRegimeEnum] = Field(None, description="Filter by market regime")
    from_date: Optional[datetime] = Field(None, description="Start date for metrics")
    to_date: Optional[datetime] = Field(None, description="End date for metrics")
...
# --- Singleton instance of the effectiveness tracker ---
tracker = ToolEffectivenessTracker()

# --- API Router Setup ---

router = APIRouter(
    prefix="/v1/analysis/effectiveness",
    tags=["Tool Effectiveness"]
)
...
@router.post(
    ...
    status_code=201,
    summary="Register a signal",
    description="Register a new signal from a trading tool for effectiveness tracking."
)
async def register_signal(
    ...
    try:
        # Create Signal object for in-memory tracking
        signal = Signal(
        ...
        if not tool:
            ...
            repository.create_tool({
                "tool_id": request.tool_id,
                "name": request.tool_id,  # Use tool_id as name by default
            })
        ...
        logger.info(
            f"Registered signal {signal_id} for tool {request.tool_id}",
            extra={
        ...
        # Wrap exceptions in EffectivenessAnalysisError
        logger.error(
            f"Error registering signal: {str(e)}",
            extra={"correlation_id": correlation_id},
            exc_info=True
        )

        raise EffectivenessAnalysisError(
            message=f"Failed to register signal: {str(e)}",
            correlation_id=correlation_id
        )
...
@router.post(
    ...
    status_code=201,
    summary="Register an outcome",
    description="Register the outcome of a previously registered signal."
)
async def register_outcome(
    ...
    try:
        # Create outcome object for in-memory tracking
        ...
        if not signal:
            raise InsufficientDataError(
                message=f"Signal not found: {request.signal_id}",
                correlation_id=correlation_id
            )
        ...
        # Use a lookback period of 30 days by default
        start_date = now - timedelta(days=30)

        # Get signals and outcomes for this tool
        signals = repository.get_signals(
            tool_id=tool_id,
            from_date=start_date,
            to_date=now,
            limit=1000  # Reasonable limit for metric calculation
        )
        ...
        if signals and outcomes:
            # Convert DB models to service models for calculation
            service_signals = [
                Signal(
                    id=str(s.signal_id),
                    tool_id=s.tool_id,
            ...
            # Calculate metrics
            win_rate = WinRateMetric().calculate(service_signals, service_outcomes)
            profit_factor = ProfitFactorMetric().calculate(service_signals, service_outcomes)
            expected_payoff = ExpectedPayoffMetric().calculate(service_signals, service_outcomes)

            # Store metrics in database
            repository.save_effectiveness_metric({
                "tool_id": tool_id,
        ...
        # Log successful registration
        logger.info(
            f"Registered outcome for signal {request.signal_id}",
            extra={
                "correlation_id": correlation_id,
                "signal_id": request.signal_id,
                "success": request.success,
        ...
        raise InsufficientDataError(
            message=f"Signal not found: {str(e)}",
            correlation_id=correlation_id
        )

    except Exception as e:
        # Wrap other exceptions in EffectivenessAnalysisError
        logger.error(
            f"Error registering outcome: {str(e)}",
            extra={"correlation_id": correlation_id},
            exc_info=True
        )

        raise EffectivenessAnalysisError(
            message=f"Failed to register outcome: {str(e)}",
            correlation_id=correlation_id
        )

@router.get(
    "/metrics",
    response_model=List[ToolEffectiveness],
    summary="Get effectiveness metrics",
    description="Retrieve effectiveness metrics for trading tools with optional filtering."
)
async def get_effectiveness_metrics(
    ...
    instrument: Optional[str] = Query(None, description="Filter by specific instrument"),
    market_regime: Optional[MarketRegimeEnum] = Query(None, description="Filter by market regime"),
    from_date: Optional[datetime] = Query(None, description="Start date for metrics calculation"),
    to_date: Optional[datetime] = Query(None, description="End date for metrics calculation"),
    db: Session = Depends(get_db_session)
):
    """
    ...
    try:
        ...
        # Get all tools that match filters
        if tool_id:
        ...
        for tool in tools:
            ...
            metrics_data = repository.get_effectiveness_metrics(
            ...
            # Get signals for date range calculation
            signals = repository.get_signals(
                tool_id=tool.tool_id,
                timeframe=timeframe,
                instrument=instrument,
            ...
            # Calculate first and last signal dates
            ...
            if 'win_rate' in metrics_dict:
                formatted_metrics.append(EffectivenessMetric(
                    name="Win Rate",
                    value=metrics_dict['win_rate'].value,
            ...
            if 'profit_factor' in metrics_dict:
                formatted_metrics.append(EffectivenessMetric(
                    name="Profit Factor",
                    value=metrics_dict['profit_factor'].value,
            ...
            if 'expected_payoff' in metrics_dict:
                formatted_metrics.append(EffectivenessMetric(
                    ...
                    value=metrics_dict['expected_payoff'].value,
                    description="Average profit/loss per signal"
                ))

            # Add reliability by regime metrics if available
            for metric in metrics_data:
                if metric.metric_type.startswith('reliability_'):
                    regime = metric.metric_type.split('_')[1]
                    formatted_metrics.append(EffectivenessMetric(
                        name=f"Reliability in {regime.capitalize()} Market",
                        value=metric.value,
                        description=f"Success rate in {regime} market conditions"
                    ))
        ...
        logger.info(
            f"Retrieved effectiveness metrics for {len(result)} tools",
            extra={
                "correlation_id": correlation_id,
                "tool_id": tool_id,
                "timeframe": timeframe,
        ...
        # Wrap exceptions in EffectivenessAnalysisError
        logger.error(
            f"Error retrieving effectiveness metrics: {str(e)}",
            extra={"correlation_id": correlation_id},
            exc_info=True
        )

        raise EffectivenessAnalysisError(
            message=f"Failed to retrieve effectiveness metrics: {str(e)}",
            correlation_id=correlation_id
        )
...
@router.get(
    ...
    description="Get aggregated data suitable for dashboard visualization."
)
async def get_dashboard_data(
    ...
    try:
        ...
        tool_performance = []
        for tool in tools:
            # Get win rate from stored metrics
            ...
            # Get outcome count and success rate
            outcome_count = 0
            success_count = 0
            ...
            success_rate = (success_count / outcome_count) * 100 if outcome_count > 0 else 0
        ...
        top_tools = sorted(tool_performance, key=lambda x: x["success_rate"], reverse=True)[:5]

        # Calculate overall metrics
        total_signals = len(all_signals)
        total_outcomes = sum(t["outcomes_count"] for t in tool_performance)
        overall_success_rate = sum(t["success_rate"] * t["outcomes_count"] for t in tool_performance) / total_outcomes if total_outcomes > 0 else 0
...
@router.post(
    ...
    status_code=201,
    summary="Save effectiveness report",
    description="Save a new effectiveness report."
)
async def save_effectiveness_report(
    ...
    try:
        ...
        metrics_result = await get_effectiveness_metrics(
        ...
        # Create report data
        report_data = {
            "metrics": [metric.dict() for metric in metrics_result],
            "generated_at": datetime.utcnow().isoformat(),
            "summary": {
                "tool_count": len(metrics_result),
                "total_signals": sum(m.signal_count for m in metrics_result),
                "avg_success_rate": sum(m.success_rate for m in metrics_result) / len(metrics_result) if metrics_result else 0
            }
        }
        ...
        filters = {
            ...
            "timeframe": request.timeframe,
            "instrument": request.instrument,
            "market_regime": request.market_regime.value if request.market_regime else None,
        ...
        # Log successful save
        logger.info(
            f"Saved effectiveness report: {request.name}",
            extra={
                "correlation_id": correlation_id,
                "report_id": report.id,
                "tool_id": request.tool_id,
        ...
        # Wrap exceptions in EffectivenessAnalysisError
        logger.error(
            f"Error saving effectiveness report: {str(e)}",
            extra={"correlation_id": correlation_id},
            exc_info=True
        )

        raise EffectivenessAnalysisError(
            message=f"Failed to save effectiveness report: {str(e)}",
            correlation_id=correlation_id
        )
...
@router.get(
    ...
    summary="Get effectiveness reports",
    description="Get all saved effectiveness reports."
)
async def get_effectiveness_reports(
    request_obj: Request,
    skip: int = Query(0, description="Skip items for pagination"),
    limit: int = Query(100, description="Limit items for pagination"),
    db: Session = Depends(get_db_session)
):
    """
    ...
    try:
        ...
        # Log successful retrieval
        logger.info(
            f"Retrieved {len(reports)} effectiveness reports",
            extra={
                "correlation_id": correlation_id,
                "skip": skip,
                "limit": limit
        ...
        # Wrap exceptions in EffectivenessAnalysisError
        logger.error(
            f"Error retrieving effectiveness reports: {str(e)}",
            extra={"correlation_id": correlation_id},
            exc_info=True
        )

        raise EffectivenessAnalysisError(
            message=f"Failed to retrieve effectiveness reports: {str(e)}",
            correlation_id=correlation_id
        )
...
@router.get(
    ...
    summary="Get effectiveness report",
    description="Get a specific effectiveness report by ID."
)
async def get_effectiveness_report(
    ...
    try:
        ...
        # Log successful retrieval
        logger.info(
            f"Retrieved effectiveness report {report_id}",
            extra={
                "correlation_id": correlation_id,
                "report_id": report_id,
        ...
        raise EffectivenessAnalysisError(
            message=f"Failed to retrieve effectiveness report: {str(e)}",
            correlation_id=correlation_id
        )
...
@router.delete(
    "/tools/{tool_id}/data",
    status_code=200,
    summary="Clear tool data",
    description="Clear all data for a specific tool (for testing or resetting purposes)."
)
async def clear_tool_data(
    tool_id: str = Path(..., description="ID of the tool to clear data for"),
    request_obj: Request = None,
    db: Session = Depends(get_db_session)
):
    """
    ...
    try:
        ...
        # Get IDs of remaining signals
        remaining_signal_ids = {s.id for s in tracker.signals}

        # Remove outcomes for removed signals
        original_outcome_count = len(tracker.outcomes)
        tracker.outcomes = [o for o in tracker.outcomes if o.signal_id in remaining_signal_ids]
        ...
        # Clear from database
        repository = ToolEffectivenessRepository(db)
        db_signals_removed, db_outcomes_removed = repository.delete_tool_data(tool_id)

        # Log successful deletion
        logger.info(
            f"Cleared data for tool {tool_id}",
            extra={
                "correlation_id": correlation_id,
                "tool_id": tool_id,
        ...
        return {
            ...
            "signals_removed": signals_removed + db_signals_removed,
            "outcomes_removed": outcomes_removed + db_outcomes_removed,
            "message": f"Removed all data for tool {tool_id}"
        }

    except Exception as e:
        # Wrap exceptions in EffectivenessAnalysisError
        logger.error(
            f"Error clearing tool data for {tool_id}: {str(e)}",
            extra={"correlation_id": correlation_id},
            exc_info=True
        )

        raise EffectivenessAnalysisError(
            message=f"Failed to clear tool data: {str(e)}",
            correlation_id=correlation_id
        )
...
legacy_router = APIRouter(
    prefix="/api/v1/tool-effectiveness",
    tags=["Tool Effectiveness (Legacy)"]
)
...
async def legacy_register_signal(
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/effectiveness/signals")
...
async def legacy_register_outcome(
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/effectiveness/outcomes")
...
async def legacy_get_effectiveness_metrics(
    tool_id: Optional[str] = Query(None, description="Filter by specific tool ID"),
    timeframe: Optional[str] = Query(None, description="Filter by specific timeframe"),
    instrument: Optional[str] = Query(None, description="Filter by specific instrument"),
    market_regime: Optional[MarketRegimeEnum] = Query(None, description="Filter by market regime"),
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/effectiveness/metrics")
    return await get_effectiveness_metrics(
        request_obj, tool_id, timeframe, instrument, market_regime, from_date, to_date, db
    )
...
async def legacy_get_dashboard_data(
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/effectiveness/dashboard-data")
...
async def legacy_save_effectiveness_report(
    name: str = Query(..., description="Name of the report"),
    description: Optional[str] = Query(None, description="Description of the report"),
    tool_id: Optional[str] = Query(None, description="Filter by specific tool ID"),
    timeframe: Optional[str] = Query(None, description="Filter by specific timeframe"),
    instrument: Optional[str] = Query(None, description="Filter by specific instrument"),
    market_regime: Optional[MarketRegimeEnum] = Query(None, description="Filter by market regime"),
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/effectiveness/reports")
    request = ReportRequest(
        name=name,
...
async def legacy_get_effectiveness_reports(
    skip: int = Query(0, description="Skip items for pagination"),
    limit: int = Query(100, description="Limit items for pagination"),
    request_obj: Request = None,
    db: Session = Depends(get_db_session)
...
async def legacy_get_effectiveness_report(
    report_id: int,
    request_obj: Request = None,
    db: Session = Depends(get_db_session)
):
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/effectiveness/reports/{report_id}")
    report = await get_effectiveness_report(report_id, request_obj, db)
    return report.dict()

@legacy_router.delete("/tool/{tool_id}/data/", status_code=200)
async def legacy_clear_tool_data(
    tool_id: str,
    request_obj: Request = None,
    db: Session = Depends(get_db_session)
):
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/effectiveness/tools/{tool_id}/data")
