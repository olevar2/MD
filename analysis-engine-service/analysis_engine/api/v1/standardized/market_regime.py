...
class RegimeHistoryRequest(BaseModel):
    ...
    symbol: str = Field(..., description="Trading symbol (e.g., 'EURUSD')")
    timeframe: str = Field(..., description="Timeframe for analysis (e.g., '1h', '4h', 'D')")
    limit: Optional[int] = Field(10, description="Maximum number of history entries to return")
...
class RegimeAnalysisRequest(BaseModel):
    ...
    timeframe: Optional[str] = Field(None, description="Timeframe for analysis (e.g., '1h', '4h', 'D')")
    instrument: Optional[str] = Field(None, description="Trading instrument (e.g., 'EUR_USD')")
...
class OptimalConditionsRequest(BaseModel):
    ...
    tool_id: str = Field(..., description="Identifier for the trading tool")
    min_sample_size: int = Field(10, description="Minimum sample size for reliable analysis")
    timeframe: Optional[str] = Field(None, description="Timeframe for analysis (e.g., '1h', '4h', 'D')")
    instrument: Optional[str] = Field(None, description="Trading instrument (e.g., 'EUR_USD')")
    from_date: Optional[datetime] = Field(None, description="Start date for analysis")
    to_date: Optional[datetime] = Field(None, description="End date for analysis")
...
class ToolComplementarityRequest(BaseModel):
    ...
    tool_ids: List[str] = Field(..., description="List of tool identifiers to analyze")
    timeframe: Optional[str] = Field(None, description="Timeframe for analysis (e.g., '1h', '4h', 'D')")
    instrument: Optional[str] = Field(None, description="Trading instrument (e.g., 'EUR_USD')")
    from_date: Optional[datetime] = Field(None, description="Start date for analysis")
    to_date: Optional[datetime] = Field(None, description="End date for analysis")
...
class RecommendationRequest(BaseModel):
    ...
    timeframe: Optional[str] = Field(None, description="Timeframe for analysis (e.g., '1h', '4h', 'D')")
    min_sample_size: int = Field(10, description="Minimum sample size for reliable analysis")
    min_win_rate: float = Field(50.0, description="Minimum win rate for recommended tools")
...
class TrendAnalysisRequest(BaseModel):
    ...
    look_back_periods: int = Field(6, description="Number of periods to look back")

class UnderperformingToolsRequest(BaseModel):
    """Request model for getting underperforming tools"""
    win_rate_threshold: float = Field(50.0, description="Win rate threshold for underperforming tools")
    min_sample_size: int = Field(20, description="Minimum sample size for reliable analysis")
    timeframe: Optional[str] = Field(None, description="Timeframe for analysis (e.g., '1h', '4h', 'D')")
    instrument: Optional[str] = Field(None, description="Trading instrument (e.g., 'EUR_USD')")
    from_date: Optional[datetime] = Field(None, description="Start date for analysis")
    to_date: Optional[datetime] = Field(None, description="End date for analysis")
...
def get_market_regime_analysis_service(db: Session = Depends(get_db_session_dependency)):
...
router = APIRouter(
    prefix="/v1/analysis/market-regimes",
    tags=["Market Regime Analysis"]
)
...
async def detect_market_regime(
    ...
    request: Request,
    service: MarketRegimeService = Depends(get_market_regime_service_dependency)
):
    """
    ...
    try:
        ...
        analysis_engine_metrics.market_regime.labels(
            service="analysis-engine-service",
            instrument=request_data.symbol,
            timeframe=request_data.timeframe,
            regime_type=regime_type
        ).set(1.0)  # Set to 1 to indicate current regime

        # Record confidence
        confidence = regime_result.get("confidence", 0.0)
        analysis_engine_metrics.market_regime_confidence.labels(
            service="analysis-engine-service",
            instrument=request_data.symbol,
            timeframe=request_data.timeframe,
            regime_type=regime_type
        ...
        # Record error metric
        analysis_engine_metrics.analysis_errors_total.labels(
            service="analysis-engine-service",
            analysis_type="market_regime_detection",
...
@router.post(
    ...
    description="Get historical regime data for a specific symbol and timeframe."
)
...
@router.post(
    ...
    response_model=Dict,
    summary="Analyze tool regime performance",
    description="Get the performance metrics of a tool across different market regimes."
)
async def analyze_tool_regime_performance(
    request_data: RegimeAnalysisRequest,
    request: Request,
    service: MarketRegimeAnalysisService = Depends(get_market_regime_analysis_service)
):
    """
...
@router.post(
    "/tools/optimal-conditions",
    response_model=Dict,
    summary="Find optimal market conditions",
    description="Find the optimal market conditions for a specific tool."
)
...
@router.post(
    "/tools/complementarity",
    response_model=Dict,
    summary="Analyze tool complementarity",
    description="Analyze how well different tools complement each other across market regimes."
)
...
@router.post(
    ...
    description="Generate a comprehensive performance report for all tools across market regimes."
)
...
@router.post(
    ...
    response_model=Dict,
    summary="Recommend tools for regime",
    description="Recommend the best trading tools for the current market regime."
)
...
@router.post(
    "/tools/effectiveness-trends",
    response_model=Dict,
    summary="Analyze effectiveness trends",
    description="Analyze how the effectiveness of a tool has changed over time across market regimes."
)
...
@router.post(
    ...
    response_model=Dict,
    summary="Get underperforming tools",
    description="Identify underperforming trading tools that may need optimization or retirement."
)
...
async def legacy_detect_market_regime(
    request_data: DetectRegimeRequest,
    request: Request,
    service: MarketRegimeService = Depends(get_market_regime_service_dependency)
):
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/market-regimes/detect")
    return await detect_market_regime(request_data, request, service)

@legacy_router.post("/history/", response_model=List[Dict])
async def legacy_get_regime_history(
    request_data: RegimeHistoryRequest,
    request: Request,
    service: MarketRegimeService = Depends(get_market_regime_service_dependency)
):
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/market-regimes/history")
    return await get_regime_history(request_data, request, service)

@legacy_router.post("/regime-analysis/", response_model=Dict)
async def legacy_analyze_tool_regime_performance(
    request_data: RegimeAnalysisRequest,
    request: Request,
    service: MarketRegimeAnalysisService = Depends(get_market_regime_analysis_service)
):
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/market-regimes/tools/regime-analysis")
