...
class RuleUpdate(BaseModel):
    ...
    priority: Optional[int] = Field(None, description="Priority of the rule (lower numbers = higher priority)")
    conditions: Optional[Dict[str, Any]] = Field(None, description="Conditions for rule matching")
    actions: Optional[Dict[str, Any]] = Field(None, description="Actions to take when rule matches")

class RuleUpdateRequest(BaseModel):
    """Request model for updating feedback rules"""
    updates: List[RuleUpdate] = Field(..., description="List of rule updates to apply")
...
@router.get(
    ...
    description="Get feedback statistics with optional filtering."
)
async def get_feedback_statistics(
    ...
    try:
        ...
        logger.info(
            "Retrieved feedback statistics",
            extra={
                "correlation_id": correlation_id,
                "strategy_id": strategy_id,
                "model_id": model_id,
                "instrument": instrument,
                "total_count": stats.get("total_feedback_count", 0)
            }
        )
        ...
        raise FeedbackError(
            message=f"Error retrieving feedback statistics: {str(e)}",
            correlation_id=correlation_id
        )
...
@router.post(
    ...
    description="Trigger retraining of a specific model based on collected feedback."
)
async def trigger_model_retraining(
    ...
    try:
        ...
        # Log successful retraining trigger
        logger.info(
            f"Triggered retraining for model {model_id}",
            extra={
                "correlation_id": correlation_id,
                "model_id": model_id,
        ...
        response = RetrainingResponse(
            ...
            message=result.get("message", "Model retraining initiated successfully")
        )
        ...
        # Raise standardized error
        raise FeedbackError(
            message=f"Error triggering model retraining: {str(e)}",
            correlation_id=correlation_id
        )
...
@router.put(
    ...
    summary="Update feedback rules",
    description="Update feedback categorization rules."
)
async def update_feedback_rules(
    ...
    try:
        ...
        logger.info(
            f"Updated feedback rules: {', '.join(rule_updates.keys())}",
            extra={
                "correlation_id": correlation_id,
                "rule_count": len(rule_updates),
                "rule_ids": list(rule_updates.keys())
            }
        )
        ...
        response = RuleUpdateResponse(
            ...
            status="success",
            message=f"Successfully updated {len(rule_updates)} rules"
        )
        ...
        # Log error
        logger.error(
            f"Error updating feedback rules: {str(e)}",
            extra={"correlation_id": correlation_id},
            exc_info=True
        )
        ...
        # Raise standardized error
        raise FeedbackError(
            message=f"Error updating feedback rules: {str(e)}",
            correlation_id=correlation_id
        )

@router.get(
    "/strategies/{strategy_id}/parameters",
    response_model=ParameterPerformanceResponse,
    summary="Get parameter performance",
...
async def get_parameter_performance(
    ...
    try:
        ...
        if "parameter_performance" not in stats:
            ...
            return ParameterPerformanceResponse(
                strategy_id=strategy_id,
                min_samples=min_samples,
                parameters={}
            )
        ...
        logger.info(
            ...
            extra={
                ...
                "strategy_id": strategy_id,
                "min_samples": min_samples,
                "parameter_count": len(filtered_parameters)
            }
        )
        ...
        # Raise standardized error
        raise FeedbackError(
            message=f"Error retrieving parameter performance: {str(e)}",
            correlation_id=correlation_id
        )
...
@router.post(
    ...
    description="Submit feedback for a signal, model, or strategy."
)
async def submit_feedback(
    feedback_data: Dict[str, Any] = Body(..., description="Feedback data to submit"),
    ...
    try:
        ...
        # Add correlation ID to feedback data
        ...
        logger.info(
            f"Submitted feedback for {feedback_data['target_id']}",
            extra={
                "correlation_id": correlation_id,
...
async def legacy_get_feedback_statistics(
    strategy_id: Optional[str] = Query(None, description="Filter by strategy ID"),
    model_id: Optional[str] = Query(None, description="Filter by model ID"),
    instrument: Optional[str] = Query(None, description="Filter by instrument"),
    start_time: Optional[datetime] = Query(None, description="Start time for filtering (ISO format)"),
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/feedback/statistics")
    return await get_feedback_statistics(
        request_obj, strategy_id, model_id, instrument, start_time, end_time, feedback_service
    )

@legacy_router.post("/model/retrain/{model_id}")
async def legacy_trigger_model_retraining(
    model_id: str,
    request_obj: Request = None,
    feedback_service: FeedbackIntegrationService = Depends(get_feedback_service)
):
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/feedback/models/{model_id}/retrain")
    result = await trigger_model_retraining(model_id, request_obj, feedback_service)
    return result.dict()

@legacy_router.put("/rules")
async def legacy_update_feedback_rules(
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/feedback/rules")
...
async def legacy_get_parameter_performance(
    strategy_id: str,
    min_samples: int = Query(10, description="Minimum number of samples required for parameter statistics"),
    request_obj: Request = None,
    feedback_service: FeedbackIntegrationService = Depends(get_feedback_service)
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/feedback/strategies/{strategy_id}/parameters")
    result = await get_parameter_performance(strategy_id, request_obj, min_samples, feedback_service)
    return result.dict()

@legacy_router.post("/submit")
async def legacy_submit_feedback(
    feedback_data: Dict[str, Any] = Body(..., description="Feedback data to submit"),
    request_obj: Request = None,
    feedback_service: FeedbackIntegrationService = Depends(get_feedback_service)
):
    ...
    """
    logger.info("Legacy endpoint called - consider migrating to /api/v1/analysis/feedback/submit")
