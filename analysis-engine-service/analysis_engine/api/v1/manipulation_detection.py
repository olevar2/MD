...
@async_with_exception_handling
async def detect_manipulation_patterns(data: Dict[str, Any], sensitivity:
    ...
    try:
        ...
        if not result.is_valid:
            raise HTTPException(status_code=422, detail=result.result_data.
                get('error', 'Analysis failed'))
        if (not include_protection and 'protection_recommendations' in
            result.result_data):
            result.result_data['protection_recommendations'] = []
        return {'status': 'success', 'symbol': data.get('metadata', {}).get
            ('symbol', 'unknown'), 'timeframe': data.get('metadata', {}).
            get('timeframe', 'unknown'), 'results': result.result_data,
        ...
        raise HTTPException(status_code=500, detail=
            f'Failed to detect manipulation patterns: {str(e)}')


@router.post('/stop-hunting', response_model=Dict[str, Any])
@async_with_exception_handling
async def detect_stop_hunting(data: Dict[str, Any], lookback: Optional[int]
    =Query(30, description='Lookback period for stop hunting detection'),
    recovery_threshold: Optional[float]=Query(0.5, description=
    'Recovery percentage threshold'), current_user: User=Depends(
    get_current_user)):
    """
    ...
    try:
        ...
        return {'status': 'success', 'symbol': data.get('metadata', {}).get
            ('symbol', 'unknown'), 'timeframe': data.get('metadata', {}).
            get('timeframe', 'unknown'), 'results': filtered_results,
            'timestamp': datetime.now().isoformat()}
    except HTTPException:
        raise
    except Exception as e:
        ...
        raise HTTPException(status_code=500, detail=
            f'Failed to detect stop hunting patterns: {str(e)}')


@router.post('/fake-breakouts', response_model=Dict[str, Any])
@async_with_exception_handling
async def detect_fake_breakouts(data: Dict[str, Any], threshold: Optional[
    float]=Query(0.7, description='Fake breakout detection threshold'),
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=
            f'Failed to detect fake breakout patterns: {str(e)}')


@router.post('/volume-anomalies', response_model=Dict[str, Any])
@async_with_exception_handling
async def detect_volume_anomalies(data: Dict[str, Any], z_threshold:
    Optional[float]=Query(2.0, description=
    'Z-score threshold for volume anomaly detection'), current_user: User=
    ...
    try:
        ...
        if not ohlcv_sample or len(ohlcv_sample
            ) == 0 or 'volume' not in ohlcv_sample[0]:
            raise HTTPException(status_code=400, detail=
                'Volume data is required for volume anomaly detection')
        ...
        raise HTTPException(status_code=500, detail=
            f'Failed to detect volume anomalies: {str(e)}')
