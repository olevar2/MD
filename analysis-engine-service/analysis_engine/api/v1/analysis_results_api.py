...
# Create a startup event handler to initialize the service
@router.on_startup
...
async def list_analyzers(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to list analyzers: {str(e)}"
        )

@router.get("/{analyzer_name}", summary="Get analyzer details")
async def get_analyzer_details(
    analyzer_name: str = Path(..., description="Name of the analyzer"),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get analyzer details: {str(e)}"
        )
...
async def run_analysis(
    ...
    try:
        ...
        # Convert the input data to MarketData objects
        market_data = data["market_data"]

        # Check if we're working with multi-timeframe data
        if isinstance(market_data, dict) and all(isinstance(v, dict) for v in market_data.values()):
            # Multi-timeframe data
            converted_data = {
                tf: MarketData.from_dict(md)
                for tf, md in market_data.items()
            }
        else:
        ...
        # Run the analysis
        result = await analysis_service.run_analysis(analyzer_name, converted_data)
...
@router.get("/{analyzer_name}/performance", summary="Get analyzer performance metrics")
async def get_performance_metrics(
    ...
    try:
        ...
        if not metrics:
            raise HTTPException(
                status_code=404,
                detail=f"Performance metrics for analyzer {analyzer_name} not found"
            )
        ...
        logger.error(f"Error getting performance metrics for {analyzer_name}: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get performance metrics: {str(e)}"
        )

@router.get("/{analyzer_name}/effectiveness", summary="Get analyzer effectiveness metrics")
async def get_effectiveness_metrics(
    ...
    try:
        ...
        if not metrics:
            raise HTTPException(
                status_code=404,
                detail=f"Effectiveness metrics for analyzer {analyzer_name} not found"
            )
        ...
        logger.error(f"Error getting effectiveness metrics for {analyzer_name}: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get effectiveness metrics: {str(e)}"
        )

@router.post("/{analyzer_name}/effectiveness/record", summary="Record effectiveness")
async def record_effectiveness(
    ...
    try:
        ...
        log_id = await analysis_service.record_effectiveness(
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to record effectiveness: {str(e)}"
        )

# Specialized endpoints for specific analyzers
...
async def run_multi_timeframe_analysis(
    ...
    try:
        ...
        market_data = {
            tf: MarketData.from_dict(md)
            for tf, md in data["market_data"].items()
        }
        ...
        # Run the analysis
        result = await analysis_service.run_multi_timeframe_analysis(market_data, parameters)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to run multi-timeframe analysis: {str(e)}"
        )
...
async def run_confluence_analysis(
    ...
    try:
        ...
        # Run the analysis
        result = await analysis_service.run_confluence_analysis(data, parameters)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to run confluence analysis: {str(e)}"
        )
