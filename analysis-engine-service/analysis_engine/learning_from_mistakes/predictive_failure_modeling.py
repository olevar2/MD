...
class PredictiveFailureModel:
    ...
    def __init__(self, error_pattern_system: ErrorPatternRecognitionSystem,
        model_dir: str=None, config: Dict[str, Any]=None):
        """
        ...
        self.feature_importance_threshold = self.config.get(
    ...
    def prepare_training_data(self, strategy_id: str, instrument: str,
        timeframe: str, successful_trades: List[Dict[str, Any]],
        failed_trades: List[Dict[str, Any]]) ->Tuple[pd.DataFrame, np.ndarray]:
        """
        ...
        logger.info(
            ...
            , strategy_id, instrument, timeframe, X.shape[1], X.shape[0], 
            100 * y.mean())
        return X, y
    ...
    def train_model(self, strategy_id: str, instrument: str, timeframe: str,
        X: pd.DataFrame, y: np.ndarray) ->Dict[str, Any]:
        """
        ...
        model = GradientBoostingClassifier(n_estimators=100, learning_rate=
            0.1, max_depth=5, min_samples_split=15, random_state=42)
        ...
        self.feature_importances[key] = dict(sorted(feature_importances.
            items(), key=lambda item: item[1], reverse=True))
        model_filename = (
            f"failure_model_{strategy_id}_{instrument}_{timeframe}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
            )
        model_path = os.path.join(self.model_dir, model_filename)
        model_data = {'model': model, 'scaler': scaler, 'feature_columns':
            X.columns.tolist(), 'feature_importances': self.
            feature_importances[key], 'training_date': datetime.now(),
            'threshold': threshold}
        try:
        ...
        performance = {'precision': precision, 'recall': recall, 'f1_score':
            f1, 'auc': auc, 'threshold': threshold, 'training_samples': len
            (X), 'failure_rate': y.mean(), 'training_date': datetime.now(),
    ...
    def predict_failure_risk(self, strategy_id: str, instrument: str,
        timeframe: str, market_conditions: Dict[str, Any]) ->Dict[str, Any]:
        """
        ...
        if key not in self.models:
            raise ValueError(
                f'No trained model found for {strategy_id} on {instrument} ({timeframe})'
                )
        ...
        top_contributors = sorted(feature_contributions.items(), key=lambda
            x: abs(x[1]), reverse=True)[:5]
        for feature, contribution in top_contributors:
            contributing_factors.append({'feature': feature, 'importance':
                self.feature_importances[key][feature], 'value': input_df[
                feature].values[0], 'contribution': contribution,
                'direction': 'increases risk' if contribution > 0 else
                'decreases risk'})
        if is_high_risk:
            primary_factor = contributing_factors[0]['feature'
                ] if contributing_factors else 'unknown factors'
            insight = f'High failure risk due primarily to {primary_factor}'
        else:
            insight = 'Low failure risk for current conditions'
        result = {'strategy_id': strategy_id, 'instrument': instrument,
            'timeframe': timeframe, 'failure_probability':
            failure_probability, 'is_high_risk': is_high_risk,
            'prediction_threshold': self.prediction_threshold,
            'contributing_factors': contributing_factors, 'insight':
            insight, 'timestamp': datetime.now()}
        logger.info('Predicted failure risk for %s on %s (%s): %.1f%% (%s)',
            strategy_id, instrument, timeframe, 100 * failure_probability, 
            'HIGH RISK' if is_high_risk else 'low risk')
        return result

    @with_database_resilience('load_model')
    @with_exception_handling
    def load_model(self, strategy_id: str, instrument: str, timeframe: str,
        ...
        try:
            if model_path is None:
                pattern = (
                    f'failure_model_{strategy_id}_{instrument}_{timeframe}_*.pkl'
                    )
                model_files = [f for f in os.listdir(self.model_dir) if f.
                    startswith(
                    f'failure_model_{strategy_id}_{instrument}_{timeframe}_')]
                if not model_files:
                    logger.warning(f'No model files found matching {pattern}')
                    return False
                model_files.sort(key=lambda f: os.path.getmtime(os.path.
                    join(self.model_dir, f)), reverse=True)
            ...
            self.model_performance[key] = {'threshold': threshold,
                'model_path': model_path, 'loading_date': datetime.now()}
            logger.info(
                'Loaded failure prediction model for %s on %s (%s) from %s',
                strategy_id, instrument, timeframe, model_path)
            return True
        except Exception as e:
            logger.error('Failed to load model for %s on %s (%s): %s',
                strategy_id, instrument, timeframe, str(e))
            return False

    @with_resilience('get_model_performance')
    def get_model_performance(self, strategy_id: str, instrument: str,
    ...
    @with_resilience('get_feature_importances')
    def get_feature_importances(self, strategy_id: str, instrument: str,
