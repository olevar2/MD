...
@with_exception_handling
def plot_causal_graph(graph_data: Dict[str, List[str]], layout: str='dot',
    ...
    try:
        ...
        print(
            'Warning: graphviz library not found. Install it (`pip install graphviz`) and ensure Graphviz backend is installed.'
            )
        print('Skipping causal graph plotting.')
        return None
    dot = graphviz.Digraph(comment='Causal Graph', graph_attr={'rankdir': 'LR'}
        )
    ...
    if filename:
        try:
            dot.render(filename, view=False, format=filename.split('.')[-1] if
                '.' in filename else 'png')
            print(f'Causal graph saved to {filename}')
            return None
        except Exception as e:
            raise CausalVisualizationError(
                f'Failed to save graph to {filename}: {e}')
    else:
        return dot


def plot_estimated_effects(effects: Dict[str, float], errors: Optional[Dict
    [str, float]]=None, title: str='Estimated Causal Effects') ->go.Figure:
    """
    ...
    error_values = [errors.get(name, 0) for name in effect_names
        ] if errors else None
    fig = go.Figure(data=[go.Bar(x=effect_names, y=effect_values, error_y=
        dict(type='data', array=error_values) if error_values else None,
        name='Estimated Effect')])
    fig.update_layout(title=title, xaxis_title='Treatment / Variable',
        yaxis_title='Estimated Effect Size', template='plotly_white')
    return fig


def plot_counterfactuals(observed_data: pd.Series, counterfactual_data: pd.
    Series, intervention_point: Optional[Any]=None, title: str=
    'Counterfactual Analysis') ->go.Figure:
    """
    ...
    """
    if not isinstance(observed_data.index, pd.DatetimeIndex) or not isinstance(
        counterfactual_data.index, pd.DatetimeIndex):
        print(
            'Warning: Input data indices are not DatetimeIndex. Plotting may proceed but time-axis formatting might be incorrect.'
            )
    ...
    if not observed_data.index.equals(counterfactual_data.index):
        print(
            'Warning: Indices of observed and counterfactual data do not match. Attempting alignment.'
            )
        common_index = observed_data.index.intersection(counterfactual_data
            .index)
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=observed_data.index, y=observed_data, mode=
        'lines', name='Observed Outcome'))
    fig.add_trace(go.Scatter(x=counterfactual_data.index, y=
        counterfactual_data, mode='lines', name=
        'Counterfactual Outcome (No Intervention)', line=dict(dash='dash')))
    if (intervention_point is not None and intervention_point in
        observed_data.index):
        fig.add_vline(x=intervention_point, line_width=2, line_dash='dot',
            line_color='red', annotation_text='Intervention',
            annotation_position='top left')
    fig.update_layout(title=title, xaxis_title='Time', yaxis_title=
        'Outcome Variable', legend_title='Scenario', template='plotly_white')
    return fig


if __name__ == '__main__':
    print('--- Causal Graph Example ---')
    causal_structure = {'Interest Rate': ['Exchange Rate', 'Inflation'],
        'Oil Price': ['Inflation'], 'Geopolitics': ['Oil Price',
    ...
    try:
        ...
        print(
            'Error: Graphviz executable not found in your PATH. Please install Graphviz.'
            )
    print('\n--- Estimated Effects Example ---')
    estimated_effects = {'Intervention A': 0.5, 'Intervention B': -0.2,
        'Control Feature X': 0.1}
    effect_errors = {'Intervention A': 0.1, 'Intervention B': 0.05,
        'Control Feature X': 0.02}
    try:
        fig_effects = plot_estimated_effects(estimated_effects, errors=
            effect_errors, title='Impact of Interventions on Price Volatility')
    ...
    counterfactual.loc[intervention_date:] = counterfactual.loc[
        intervention_date:] - np.linspace(0, 5, len(counterfactual.loc[
        intervention_date:]))
    try:
        fig_cf = plot_counterfactuals(observed, counterfactual,
            intervention_point=intervention_date, title=
            'Counterfactual: Impact of Policy Change')
