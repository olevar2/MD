...
# Use common interface instead of direct import
...
class CausalStrategyEnhancer(ICausalStrategyEnhancer):
    ...
    # ICausalStrategyEnhancer interface implementation
    ...
    async def enhance_strategy(
        ...
        try:
            logger.info(f"Enhancing strategy {strategy_id} with {enhancement_type}")

            if enhancement_type == "causal":
                return await self.apply_causal_enhancement(
                    strategy_id=strategy_id,
                    causal_factors=parameters.get("causal_factors", []),
                    enhancement_parameters=parameters.get("enhancement_parameters", {})
                )
            else:
                logger.warning(f"Unsupported enhancement type: {enhancement_type}")
                return {
                    "success": False,
    ...
    async def compare_enhancements(
        ...
        logger.info(f"Comparing enhancements for strategy {strategy_id}: {enhancement_ids}")
        return {
            "strategy_id": strategy_id,
    ...
    async def identify_causal_factors(
        ...
        try:
            # Get historical data for causal analysis
            symbols = ["EUR/USD"]  # Default symbol if not specified
            timeframe = self.default_timeframe

            # Extract dates from data_period
            start_date = datetime.fromisoformat(data_period.get("start_date",
                                                              (datetime.now() - self.default_window).isoformat()))
            end_date = datetime.fromisoformat(data_period.get("end_date", datetime.now().isoformat()))
            ...
            if historical_data.empty:
                logger.error(f"Failed to retrieve historical data for strategy {strategy_id}")
                return {
                    "strategy_id": strategy_id,
            ...
            causal_graph = self.causal_inference.discover_causal_structure(
                prepared_data,
                method="granger",
                cache_key=f"{strategy_id}_{','.join(sorted(symbols))}_{timeframe}"
            )

            # Extract causal factors from graph
            causal_factors = []
            for node, data in causal_graph.nodes(data=True):
                if node != "strategy_performance" and causal_graph.has_edge(node, "strategy_performance"):
                    edge_data = causal_graph.get_edge_data(node, "strategy_performance")
    ...
    async def generate_causal_graph(
        ...
        try:
            # Similar implementation to identify_causal_factors but returns graph structure
            symbols = ["EUR/USD"]  # Default symbol if not specified
            timeframe = self.default_timeframe
            ...
            nodes = []
            for node, data in causal_graph.nodes(data=True):
                node_type = "outcome" if node == "strategy_performance" else "factor"
    ...
    async def apply_causal_enhancement(
        ...
        try:
            # In a real implementation, this would modify the strategy based on causal factors
            enhancements = []

            # Process each causal factor
            for factor in causal_factors:
                ...
                if "volatility" in factor_name.lower():
                    enhancements.append({
                        "type": "filter",
                        "description": "Added market volatility filter",
                        "details": "Filter trades during high volatility periods"
                    })

                if "volume" in factor_name.lower():
                    enhancements.append({
                        "type": "filter",
                        "description": "Added volume threshold filter",
                        "details": "Only trade when volume is above threshold"
                    })

                if "trend" in factor_name.lower():
                    enhancements.append({
                        "type": "parameter",
                        "description": "Adjusted trend sensitivity",
                        "details": "Modified trend detection parameters"
                    })

            # Add default enhancement if none were added
            if not enhancements:
                enhancements.append({
                    "type": "parameter",
                    "description": "Adjusted stop loss based on volatility",
                    "details": "Dynamic stop loss calculation using ATR"
                })

            return {
                "strategy_id": strategy_id,
                "enhancement_id": f"causal_{datetime.now().strftime('%Y%m%d%H%M%S')}",
                "causal_factors": causal_factors,
