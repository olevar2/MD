...
class CausalRelationshipAnalyzer:
    ...
    def detect_granger_causality(
        ...
        for cause in potential_causes:
            ...
            pair_data = pd.concat([data[cause], data[target_var]], axis=1).dropna()
            try:
                granger_test = grangercausalitytests(pair_data, maxlag=self.max_lag, verbose=False)
                
                # Extract the best lag based on minimum p-value
                best_lag = min(granger_test.items(), 
                             key=lambda x: x[1][0]['ssr_chi2test'][1])[0]
                
                results[cause] = {
                    'best_lag': best_lag,
                    'p_value': granger_test[best_lag][0]['ssr_chi2test'][1],
                    'f_stat': granger_test[best_lag][0]['ssr_chi2test'][0],
                    'is_significant': granger_test[best_lag][0]['ssr_chi2test'][1] < self.significance_level
                }
            except Exception as e:
                logger.error(f"Error testing Granger causality for {cause} -> {target_var}: {str(e)}")
                
        return results
    
    def discover_structural_relationships(
        self,
        data: pd.DataFrame,
        standardize: bool = True
    ) -> Tuple[StructureModel, Dict[Tuple[str, str], float]]:
        """
        ...
        try:
            ...
            logger.error(f"Error discovering structural relationships: {str(e)}")
    ...
    def validate_relationship(
        ...
        if method in ['granger', 'combined']:
            granger_result = self.detect_granger_causality(data, effect, [cause])
        ...
        if method in ['correlation', 'combined']:
            ...
            for lag in range(1, self.max_lag + 1):
                correlation = stats.pearsonr(
                    data[cause].iloc[:-lag],
                    data[effect].iloc[lag:]
                )
                if abs(correlation[0]) > abs(max_correlation):
                    max_correlation = correlation[0]
                    best_lag = lag
            
            results['details']['correlation'] = {
                'coefficient': max_correlation,
                'best_lag': best_lag,
                'is_significant': abs(max_correlation) > 0.3  # Adjustable threshold
            }
            
            if results['details']['correlation']['is_significant']:
                results['methods_passed'].append('correlation')
        
        # Calculate confidence score and overall validity
        n_methods = 2 if method == 'combined' else 1
        results['confidence_score'] = len(results['methods_passed']) / n_methods
