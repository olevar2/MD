...
class CausalPredictor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> 'CausalPredictor':
        ...
        self.causal_graph, _ = self.graph_generator.generate_validated_graph(
            data,
            focus_variables=list(X.columns) + [self.target_variable],
            min_confidence=self.causal_threshold
        )
        
        # Create lagged features based on causal relationships
        X_lagged = self._create_lagged_features(X)
        y_aligned = y.iloc[X_lagged.index]
        
        # Fit base model with causal features
...
class CausalEnsemblePredictor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> 'CausalEnsemblePredictor':
        ...
        # Fit each model and get their predictions
        for model in self.models:
            model.fit(X, y)
            pred = model.predict(X)
            predictions.append(pred)
            
            # Calculate confidence score based on causal relationships
            if hasattr(model, 'causal_graph'):
                confidence = np.mean([
                    data.get('weight', 0)
                    for _, _, data in model.causal_graph.edges(data=True)
                ])
        ...
        # Calculate model weights based on performance and causal confidence
        errors = np.array([
            np.mean((pred - y) ** 2)
        ...
        # Combine weights
        self.model_weights = 0.7 * performance_weights + 0.3 * confidence_weights
    ...
    def get_model_insights(self) -> Dict[str, Any]:
        ...
        insights = {
            'model_weights': dict(enumerate(self.model_weights)),
            'causal_relationships': {},
            'feature_importance': {}
        }
        
        # Aggregate causal relationships from all models
            ...
            if hasattr(model, 'get_causal_feature_importance'):
                ...
                for feature, importance in imp.items():
                    ...
                    insights['feature_importance'][feature].append({
                        'model_id': i,
                        'importance': importance * self.model_weights[i]
                    })
