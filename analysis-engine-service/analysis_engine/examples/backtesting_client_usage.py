...
async def run_backtest_example():
    ...
    strategy_config = {'strategy_id': 'moving_average_crossover',
        'parameters': {'fast_period': 10, 'slow_period': 30},
        'risk_settings': {'max_drawdown_pct': 20, 'max_risk_per_trade_pct':
        2}, 'position_sizing': {'method': 'fixed_risk', 'risk_pct': 1}}
    start_date = datetime.utcnow() - timedelta(days=365)
    end_date = datetime.utcnow()
    instruments = ['EUR_USD', 'GBP_USD']
    initial_capital = 10000.0
    try:
        result = await client.run_backtest(strategy_config=strategy_config,
            start_date=start_date, end_date=end_date, instruments=
            instruments, initial_capital=initial_capital, commission_model=
            'fixed', commission_settings={'fixed_commission': 5.0},
            slippage_model='fixed', slippage_settings={'fixed_pips': 1},
            data_source='historical', data_parameters={'timeframe': 'H1'})
    ...
    try:
        ...
        if trades:
            ...
            if profitable_trades:
                avg_profit = sum(t.get('profit', 0) for t in profitable_trades
...
async def run_walk_forward_optimization_example():
    ...
    strategy_config = {'strategy_id': 'moving_average_crossover',
        'parameters': {'fast_period': 10, 'slow_period': 30}}
    parameter_ranges = {'fast_period': [5, 10, 15, 20], 'slow_period': [20,
        30, 40, 50]}
    start_date = datetime.utcnow() - timedelta(days=365)
    end_date = datetime.utcnow()
    instruments = ['EUR_USD']
    try:
        result = await client.run_walk_forward_optimization(strategy_config
            =strategy_config, parameter_ranges=parameter_ranges, start_date
            =start_date, end_date=end_date, instruments=instruments,
            initial_capital=10000.0, window_size_days=90, anchor_size_days=
            30, optimization_metric='sharpe_ratio', parallel_jobs=2)
        logger.info(
            f"Walk-forward optimization completed with ID: {result.get('optimization_id')}"
            )
        ...
        logger.info(
            f"In-sample return: {in_sample_metrics.get('total_return_pct', 0):.2f}%"
            )
        logger.info(
            f"Out-of-sample return: {out_of_sample_metrics.get('total_return_pct', 0):.2f}%"
            )
