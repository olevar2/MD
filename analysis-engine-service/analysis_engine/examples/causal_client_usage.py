...
async def discover_structure_example():
    ...
    data = [
        ...
        {"timestamp": "2023-01-01T02:00:00Z", "EUR_USD": 1.0760, "GBP_USD": 1.2515, "USD_JPY": 130.40},
        {"timestamp": "2023-01-01T03:00:00Z", "EUR_USD": 1.0765, "GBP_USD": 1.2520, "USD_JPY": 130.35},
        {"timestamp": "2023-01-01T04:00:00Z", "EUR_USD": 1.0770, "GBP_USD": 1.2525, "USD_JPY": 130.30}
    ]
    
    try:
        # Discover causal structure
        result = await client.discover_structure(
            data=data,
            method="granger",
            cache_key="example_discovery",
            force_refresh=True
        )
...
async def estimate_effect_example():
    ...
    data = [
        ...
        {"timestamp": "2023-01-01T02:00:00Z", "volatility": 0.10, "spread": 2.0, "volume": 800, "return": 0.005},
        {"timestamp": "2023-01-01T03:00:00Z", "volatility": 0.18, "spread": 3.5, "volume": 1500, "return": 0.03},
        {"timestamp": "2023-01-01T04:00:00Z", "volatility": 0.14, "spread": 2.8, "volume": 1100, "return": 0.015}
    ]
    
    try:
        # Estimate causal effect
        result = await client.estimate_effect(
            data=data,
...
async def analyze_counterfactuals_example():
    ...
    try:
        ...
        result = await client.analyze_counterfactuals(
            data=data,
            target="return",
...
async def analyze_currency_pair_relationships_example():
    ...
    price_data = {
        "EUR_USD": {
            "ohlc": [
                ...
                {"timestamp": "2023-01-01T02:00:00Z", "open": 1.0760, "high": 1.0770, "low": 1.0755, "close": 1.0765},
                {"timestamp": "2023-01-01T03:00:00Z", "open": 1.0765, "high": 1.0775, "low": 1.0760, "close": 1.0770},
                {"timestamp": "2023-01-01T04:00:00Z", "open": 1.0770, "high": 1.0780, "low": 1.0765, "close": 1.0775}
            ]
        },
        "GBP_USD": {
            "ohlc": [
                ...
                {"timestamp": "2023-01-01T02:00:00Z", "open": 1.2510, "high": 1.2520, "low": 1.2505, "close": 1.2515},
                {"timestamp": "2023-01-01T03:00:00Z", "open": 1.2515, "high": 1.2525, "low": 1.2510, "close": 1.2520},
                {"timestamp": "2023-01-01T04:00:00Z", "open": 1.2520, "high": 1.2530, "low": 1.2515, "close": 1.2525}
            ]
        }
    }
    ...
    try:
        ...
        result = await client.analyze_currency_pair_relationships(
