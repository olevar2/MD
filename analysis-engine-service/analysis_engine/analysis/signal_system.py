...
try:
    ...
    logger.error(f"Failed to initialize EventPublisher in SignalSystem: {e}", exc_info=True)
...
class Signal:
    ...
    price: float
    metadata: Dict = None  # Additional signal-specific information
    ...
    def __str__(self) -> str:
        return f"{self.indicator_name} {self.signal_type.name} ({self.strength:.2f}) @ {self.timestamp}"
...
class AggregatedSignal:
    ...
    def __str__(self) -> str:
        return (f"{self.signal_type.name} ({self.strength:.2f}) @ {self.timestamp} "
                f"[Confidence: {self.confidence:.2f}, Contributors: {self.num_contributors}]")
...
class SignalConflict:
    ...
    def conflict_severity(self) -> float:
        ...
        buy_strength = sum(s.strength for s in self.conflicting_signals if s.is_buy)
        sell_strength = sum(s.strength for s in self.conflicting_signals if s.is_sell)
        
        # Higher when strong signals conflict
        return abs(buy_strength - sell_strength) / (buy_strength + sell_strength)
    
    def __str__(self) -> str:
        return (f"Conflict @ {self.timestamp} "
                f"[Severity: {self.conflict_severity:.2f}, "
                f"Signals: {len(self.conflicting_signals)}]")


class ConflictResolutionStrategy(Enum):
    """Strategies for resolving signal conflicts."""
    MAJORITY_VOTE = "majority_vote"  # Use the most common signal type
    WEIGHTED_MAJORITY = "weighted_majority"  # Weight by signal strength
    STRONGEST_SIGNAL = "strongest_signal"  # Use the single strongest signal
    TIME_PRIORITY = "time_priority"  # Favor the most recent signal
    MOST_RELIABLE = "most_reliable"  # Favor the most historically accurate indicator
    CUSTOM = "custom"  # Use a custom resolution function
...
class SignalConcordance:
    ...
    def __init__(self, indicators: List[str]):
        ...
        self.agreement_matrix = pd.DataFrame(
            np.identity(len(indicators)),  # Start with identity matrix
            index=indicators,
            columns=indicators
        )
        self.samples_count = pd.DataFrame(
            np.zeros((len(indicators), len(indicators))),
            index=indicators,
            columns=indicators
        )
    ...
    def update(self, signals_by_indicator: Dict[str, List[Signal]]) -> None:
        ...
        """
        # Group signals by timestamp to find contemporaneous signals
        signals_by_time = {}
        ...
        # Update agreement matrix for each timestamp
        for timestamp, indicators_signals in signals_by_time.items():
            for ind1 in indicators_signals:
                for ind2 in indicators_signals:
                    ...
                    # Update agreement based on signal types
                    agreement = self._calculate_signal_agreement(signal1, signal2)
                    
                    # Exponential moving average for agreement
                    alpha = 0.1  # Weight for new observation
                    old_agreement = self.agreement_matrix.loc[ind1, ind2]
                    new_agreement = (1 - alpha) * old_agreement + alpha * agreement
                    self.agreement_matrix.loc[ind1, ind2] = new_agreement
    
    def _calculate_signal_agreement(self, signal1: Signal, signal2: Signal) -> float:
        """Calculate agreement between two signals from -1.0 to 1.0."""
        # Perfect agreement if same signal type
        if signal1.signal_type == signal2.signal_type:
            return 1.0
            
        # Perfect disagreement if opposite signals
        if ((signal1.is_buy and signal2.is_sell) or
            (signal1.is_sell and signal2.is_buy)):
            return -1.0
            
        # Partial disagreement if one is neutral
    ...
    def get_most_concordant_indicators(self, n: int = 3) -> List[Tuple[str, str, float]]:
        ...
        for i, ind1 in enumerate(self.indicators):
            for j, ind2 in enumerate(self.indicators):
                if i < j:  # Only include each pair once
                    ...
                    if samples > 5:  # Only include pairs with sufficient samples
    ...
    def get_most_discordant_indicators(self, n: int = 3) -> List[Tuple[str, str, float]]:
...
class SignalSystem:
    ...
    def __init__(self):
        ...
        self.signals: Dict[str, List[Signal]] = {}  # Indicator name -> signals
        self.aggregated_signals: List[AggregatedSignal] = []
        self.conflicts: List[SignalConflict] = []
        self.concordance = None
        self.indicator_reliability: Dict[str, float] = {}  # Historical reliability scores
        self._conflict_resolution_strategy = ConflictResolutionStrategy.WEIGHTED_MAJORITY
    ...
    def add_signal(self, signal: Signal) -> None:
        ...
        if event_publisher:
            try:
                # Assuming signal.metadata contains 'symbol' and 'timeframe'
                # If not, this needs adjustment based on how symbol/timeframe are passed
                symbol = signal.metadata.get('symbol', 'UNKNOWN')
                timeframe = signal.metadata.get('timeframe', 'UNKNOWN')

                payload = SignalGeneratedPayload(
                    signal_id=f"{signal.indicator_name}_{signal.timestamp.isoformat()}", # Example ID
                    symbol=symbol,
                    timeframe=timeframe,
                ...
                event_publisher.publish(topic=settings.KAFKA_SIGNAL_TOPIC, event=event)
            except Exception as pub_exc:
                logger.error(f"Failed to publish SignalGeneratedEvent for {signal}: {pub_exc}", exc_info=True)
        # --- End Publish Event ---
    ...
    def set_indicator_reliability(self, indicator_name: str, reliability: float) -> None:
        """
    ...
    def set_conflict_resolution_strategy(self, 
        ...
        if strategy == ConflictResolutionStrategy.CUSTOM:
            if custom_func is None:
                raise ValueError("Custom resolution function must be provided with CUSTOM strategy")
    ...
    def _detect_conflicts(self, 
        ...
        if buy_signals and sell_signals:
            # We have both buy and sell signals
    ...
    def _resolve_conflict(self, conflict: SignalConflict) -> Optional[Signal]:
        ...
        # Get the strongest signal as default result
        ...
        if strategy == ConflictResolutionStrategy.STRONGEST_SIGNAL:
            ...
            if buy_count > sell_count:
                # Create a new signal representing majority vote
                result = Signal(
                    timestamp=timestamp,
                    indicator_name="MAJORITY_VOTE",
                    signal_type=SignalType.BUY,
                    strength=sum(s.strength for s in signals if s.is_buy) / buy_count,
                    price=signals[0].price,
                    metadata={"contributing_signals": [s.indicator_name for s in signals if s.is_buy]}
                )
            elif sell_count > buy_count:
                result = Signal(
                    timestamp=timestamp,
                    ...
                    strength=sum(s.strength for s in signals if s.is_sell) / sell_count,
                    price=signals[0].price,
                    metadata={"contributing_signals": [s.indicator_name for s in signals if s.is_sell]}
                )
            else:
            ...
            # Weight by signal strength and indicator reliability
            buy_weight = 0.0
            sell_weight = 0.0
            
            for s in signals:
                reliability = self.indicator_reliability.get(s.indicator_name, 0.5)
            ...
            if buy_weight > sell_weight:
                result = Signal(
                    timestamp=timestamp,
                    indicator_name="WEIGHTED_MAJORITY",
                    signal_type=SignalType.BUY,
                ...
                # Equal weights - use strongest signal
                result = strongest
            ...
            # Signal from most reliable indicator wins
            if self.indicator_reliability:
                result = max(signals, key=lambda s: self.indicator_reliability.get(s.indicator_name, 0.0))
            else:
                # No reliability data - use strongest signal
                result = strongest
    ...
    def _aggregate_signals(self, 
                          timestamp: datetime, 
                          signals: List[Signal]) -> AggregatedSignal:
        """
        ...
        if conflict:
            ...
            if resolved:
                ...
                # Unable to resolve - use neutral
                signal_type = SignalType.NEUTRAL
                strength = 0.0
        else:
            # No conflict - use simple aggregation
            buy_signals = [s for s in signals if s.is_buy]
            sell_signals = [s for s in signals if s.is_sell]
            ...
            for i in range(len(signals)):
                for j in range(i+1, len(signals)):
                    ...
                    if s1.signal_type == s2.signal_type:
                        total_agreement += 1.0
                    elif ((s1.is_buy and s2.is_sell) or
                          (s1.is_sell and s2.is_buy)):
                        total_agreement += 0.0  # Complete disagreement
                    else:
                        total_agreement += 0.5  # Partial agreement (neutral)
                    pairs += 1
            
            confidence = total_agreement / pairs if pairs > 0 else 0.5
        else:
            # Single signal - use its strength as confidence
    ...
    def get_indicators_by_reliability(self) -> List[Tuple[str, float]]:
        ...
        return sorted(
            self.indicator_reliability.items(),
            key=lambda x: x[1],
            reverse=True
        )
    ...
    def get_signals_for_period(self, 
                              start_time: datetime,
                              end_time: datetime) -> List[AggregatedSignal]:
        """
        ...
        """
        return [s for s in self.aggregated_signals 
    ...
    def clear_signals(self, older_than: Optional[datetime] = None) -> None:
        ...
        if older_than is None:
            ...
            for indicator, signals in self.signals.items():
                self.signals[indicator] = [s for s in signals if s.timestamp >= older_than]
            
            self.aggregated_signals = [s for s in self.aggregated_signals 
                                     if s.timestamp >= older_than]
            
            self.conflicts = [c for c in self.conflicts if c.timestamp >= older_than]
            logger.info(f"Cleared signals older than {older_than}")


# Create a global signal system instance
