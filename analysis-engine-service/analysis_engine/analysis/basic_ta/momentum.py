...
class Momentum(BaseIndicator):
    ...
    def __init__(self, period: int=14, price_column: str='close', normalize:
        bool=False, **kwargs):
        """
    ...
    def calculate(self, df: pd.DataFrame) ->pd.DataFrame:
        ...
        result_df = df.copy()
        if self.normalize:
            momentum = result_df[self.price_column].pct_change(periods=self
                .period) * 100
        else:
    ...
    def initialize_incremental(self) ->Dict[str, Any]:
        ...
        return {'prices': [], 'last_momentum': None}
...
class RateOfChange(BaseIndicator):
    ...
    def __init__(self, period: int=14, price_column: str='close',
        smoothing_period: Optional[int]=None, smoothing_type: str='sma', **
        kwargs):
        """
        ...
        if smoothing_period:
            self.smoothed_column = (
                f'{self.output_column}_{smoothing_type}{smoothing_period}')
    ...
    def calculate(self, df: pd.DataFrame) ->pd.DataFrame:
        ...
        validate_dataframe(df, required_columns=[self.price_column])
        result_df = df.copy()
        roc = result_df[self.price_column].pct_change(periods=self.period
            ) * 100
        result_df[self.output_column] = roc
        if self.smoothing_period:
            if self.smoothing_type == 'ema':
                smooth_roc = roc.ewm(span=self.smoothing_period, adjust=False
                    ).mean()
            else:
    ...
    def initialize_incremental(self) ->Dict[str, Any]:
        """Initialize state for incremental calculation."""
        state = {'prices': [], 'roc_values': [], 'last_roc': None}
        if self.smoothing_period:
    ...
    def update_incremental(self, state: Dict[str, Any], new_data: Dict[str,
        ...
        if len(state['prices']) > self.period:
            ...
            if past_price != 0:
                ...
                if self.smoothing_period and len(state['roc_values']
                    ) > self.smoothing_period:
                    state['roc_values'] = state['roc_values'][-self.
                        smoothing_period:]
                if self.smoothing_period and len(state['roc_values']
                    ) == self.smoothing_period:
                    if self.smoothing_type == 'ema':
                        ...
                        if state['last_smoothed_roc'] is not None:
                            state['last_smoothed_roc'] = alpha * roc + (1 -
                                alpha) * state['last_smoothed_roc']
                        else:
                            state['last_smoothed_roc'] = np.mean(state[
                                'roc_values'])
                    else:
                        state['last_smoothed_roc'] = np.mean(state[
                            'roc_values'])
