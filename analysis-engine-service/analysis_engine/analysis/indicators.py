...
class IndicatorResult:
    ...
    def to_dict(self) ->Dict[str, Any]:
        ...
        return {'name': self.name, 'values': self.values.tolist() if
            isinstance(self.values, np.ndarray) else self.values.to_list(),
            'parameters': self.parameters, 'metadata': self.metadata or {}}
...
class IndicatorClient:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.warning(
                'Common-lib adapter not available, falling back to talib')
    ...
    @with_exception_handling
    def calculate_rsi(self, data: Union[np.ndarray, pd.Series], period: int=14
        ) ->pd.Series:
        """
    ...
    def calculate_macd(self, data: Union[np.ndarray, pd.Series],
        ...
        try:
            ...
            macd, signal, hist = talib.MACD(data, fastperiod=fast_period,
                slowperiod=slow_period, signalperiod=signal_period)
            return {'macd': pd.Series(macd), 'signal': pd.Series(signal),
    ...
    def calculate_bollinger_bands(self, data: Union[np.ndarray, pd.Series],
        ...
        try:
            if self.use_common_lib:
                result = self.common_lib_adapter.calculate_bollinger_bands(data
                    , period, num_std)
                return {'upper': result['upper'], 'middle': result['middle'
                    ], 'lower': result['lower']}
            upper, middle, lower = talib.BBANDS(data, timeperiod=period,
                nbdevup=num_std, nbdevdn=num_std)
            return {'upper': pd.Series(upper), 'middle': pd.Series(middle),
    ...
    @with_exception_handling
    def calculate_atr(self, high: Union[np.ndarray, pd.Series], low: Union[
        np.ndarray, pd.Series], close: Union[np.ndarray, pd.Series], period:
        int=14) ->pd.Series:
        """
        ...
        try:
            if self.use_common_lib:
                if isinstance(high, np.ndarray) and isinstance(low, np.ndarray
                    ...
                    data = pd.DataFrame({'high': high, 'low': low, 'close':
                        close})
                else:
                    data = pd.DataFrame({'high': high, 'low': low, 'close':
                        close})
    ...
    @with_exception_handling
    def calculate_stochastic(self, high: Union[np.ndarray, pd.Series], low:
        Union[np.ndarray, pd.Series], close: Union[np.ndarray, pd.Series],
        k_period: int=14, d_period: int=3, slowing: int=3) ->Dict[str, pd.
        Series]:
        """
        ...
        try:
            if self.use_common_lib:
                if isinstance(high, np.ndarray) and isinstance(low, np.ndarray
                    ...
                    data = pd.DataFrame({'high': high, 'low': low, 'close':
                        close})
                k, d = self.common_lib_adapter.calculate_stochastic(data,
                    k_period, d_period, slowing)
                return {'k': k, 'd': d}
            k, d = talib.STOCH(high, low, close, fastk_period=k_period,
                slowk_period=slowing, slowk_matype=0, slowd_period=d_period,
                slowd_matype=0)
    ...
    @with_analysis_resilience('calculate_adx')
    @with_exception_handling
    def calculate_adx(self, high: Union[np.ndarray, pd.Series], low: Union[
        np.ndarray, pd.Series], close: Union[np.ndarray, pd.Series], period:
        int=14) ->Dict[str, pd.Series]:
        """
        ...
        try:
            ...
            return {'adx': pd.Series(adx), 'plus_di': pd.Series(plus_di),
    ...
    @with_analysis_resilience('calculate_obv')
    @with_exception_handling
    def calculate_obv(self, close: Union[np.ndarray, pd.Series], volume:
        Union[np.ndarray, pd.Series]) ->pd.Series:
        """
    ...
    @with_analysis_resilience('calculate_mfi')
    @with_exception_handling
    def calculate_mfi(self, high: Union[np.ndarray, pd.Series], low: Union[
        np.ndarray, pd.Series], close: Union[np.ndarray, pd.Series], volume:
        Union[np.ndarray, pd.Series], period: int=14) ->pd.Series:
        """
        ...
        """
        try:
            return pd.Series(talib.MFI(high, low, close, volume, timeperiod
                =period))
        except Exception as e:
            self.logger.error(f'Error calculating MFI: {str(e)}')
            return pd.Series(np.nan, index=range(len(close)))

    @with_analysis_resilience('calculate_aroon')
    @with_exception_handling
    def calculate_aroon(self, high: Union[np.ndarray, pd.Series], low:
        Union[np.ndarray, pd.Series], period: int=25) ->Dict[str, pd.Series]:
        """
        ...
        try:
            ...
            return {'aroon_up': pd.Series(aroon_up), 'aroon_down': pd.
                Series(aroon_down)}
        except Exception as e:
            self.logger.error(f'Error calculating Aroon: {str(e)}')
            empty = pd.Series(np.nan, index=range(len(high)))
            return {'aroon_up': empty, 'aroon_down': empty}

    @with_analysis_resilience('calculate_ichimoku')
    @with_exception_handling
    def calculate_ichimoku(self, high: Union[np.ndarray, pd.Series], low:
        Union[np.ndarray, pd.Series], close: Union[np.ndarray, pd.Series],
        ...
        try:
            ...
            return {'tenkan_sen': tenkan_sen, 'kijun_sen': kijun_sen,
                'senkou_span_a': senkou_span_a, 'senkou_span_b':
                senkou_span_b, 'chikou_span': chikou_span}
        except Exception as e:
            self.logger.error(f'Error calculating Ichimoku: {str(e)}')
            empty = pd.Series(np.nan, index=range(len(close)))
            return {'tenkan_sen': empty, 'kijun_sen': empty,
                'senkou_span_a': empty, 'senkou_span_b': empty,
                'chikou_span': empty}
