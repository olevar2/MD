...
class OrderFlowBar:
    ...
    point_of_control: float = 0.0  # Price level with highest volume
...
class OrderFlowIndicator(AdvancedAnalysisBase):
    ...
    def __init__(self, name: str = "OrderFlowIndicator", 
                 tick_size: float = 0.0001, price_precision: int = 5):
        """
    ...
    def _infer_trade_direction(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Initialize buy and sell volume columns if they don't exist
        ...
        # Use tick rule to infer trade direction
        # If price went up, consider volume as buying pressure
        # If price went down, consider volume as selling pressure
        # This is a simplified approach, actual tick data would be more accurate
        
        result['price_change'] = result['close'].diff()
        
        # Assign volume to buy or sell based on price change
        buy_mask = result['price_change'] > 0
        sell_mask = result['price_change'] < 0
        unchanged_mask = result['price_change'] == 0
        
        # For unchanged prices, split volume equally
        result.loc[buy_mask, 'buy_volume'] = result.loc[buy_mask, 'volume']
        result.loc[sell_mask, 'sell_volume'] = result.loc[sell_mask, 'volume']
        result.loc[unchanged_mask, 'buy_volume'] = result.loc[unchanged_mask, 'volume'] / 2
        result.loc[unchanged_mask, 'sell_volume'] = result.loc[unchanged_mask, 'volume'] / 2
    ...
    def _create_order_flow_bars(self, data: pd.DataFrame) -> List[OrderFlowBar]:
        ...
        """
        # Ensure data has buy/sell volume information
        if 'buy_volume' not in data.columns or 'sell_volume' not in data.columns:
            data = self._infer_trade_direction(data)
        
        bars = []
        
        # Create an order flow bar for each row
        for idx, row in data.iterrows():
            time = row.name if isinstance(data.index, pd.DatetimeIndex) else idx
            bar = OrderFlowBar(
                time=time,
                open=row['open'],
                high=row['high'],
...
class CumulativeDeltaIndicator(OrderFlowIndicator):
    ...
    def __init__(self, tick_size: float = 0.0001, price_precision: int = 5,
        ...
        """
        super().__init__(name="CumulativeDeltaIndicator", 
                        tick_size=tick_size, 
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Calculate delta (buy volume - sell volume) for each bar
        result['delta'] = result['buy_volume'] - result['sell_volume']
        
        # Calculate cumulative delta
        if self.reset_session and isinstance(result.index, pd.DatetimeIndex):
            # Reset at the start of each session (e.g., trading day)
            result['session_date'] = result.index.date
            result['cumulative_delta'] = result.groupby('session_date')['delta'].cumsum()
            result.drop('session_date', axis=1, inplace=True)
        else:
            # No reset, continuous cumulative delta
            result['cumulative_delta'] = result['delta'].cumsum()
        
        # Calculate high and low values of cumulative delta for each bar
        # These can be used to identify divergences with price
        result['cd_high'] = result.groupby(result.index.date)['cumulative_delta'].cummax() \
            if isinstance(result.index, pd.DatetimeIndex) else result['cumulative_delta'].cummax()
        
        result['cd_low'] = result.groupby(result.index.date)['cumulative_delta'].cummin() \
            if isinstance(result.index, pd.DatetimeIndex) else result['cumulative_delta'].cummin()
        
        # Calculate delta divergence (difference between price and cumulative delta direction)
        # A positive divergence occurs when price makes a lower low but CD makes a higher low
        # A negative divergence occurs when price makes a higher high but CD makes a lower high
        result['price_direction'] = np.sign(result['close'].diff())
        result['cd_direction'] = np.sign(result['cumulative_delta'].diff())
        
        # Divergence occurs when price and CD move in opposite directions
        result['delta_divergence'] = result['price_direction'] * result['cd_direction']
        # Negative values indicate divergence
        
        return result
    
    def visualize(self, data: pd.DataFrame, window: Optional[int] = None) -> plt.Figure:
        """
        ...
        """
        # Use subset of data if window is specified
        if window is not None and window > 0:
            plot_data = data.iloc[-window:]
        else:
            plot_data = data
        
        # Create figure with two subplots (price and CD)
        ...
        # Plot cumulative delta in bottom subplot
        ax2.plot(plot_data.index, plot_data['cumulative_delta'], label='Cumulative Delta', color='blue')
        ...
        if not divergence_points.empty:
            ...
            ax2.scatter(divergence_points.index, divergence_points['cumulative_delta'], 
                       color='purple', marker='^')
...
class VolumeProfilerRealTime(OrderFlowIndicator):
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Create price levels for the entire dataset
        price_levels = self._create_price_levels(result)
        
        # Calculate delta and total volume
        result['delta'] = result['buy_volume'] - result['sell_volume']
        ...
        if self.dynamic_profile:
            ...
            # Fixed period approach (e.g., daily, weekly)
            self._calculate_fixed_profiles(result)
        
        # Add profile information to the result DataFrame
    ...
    def _create_price_levels(self, data: pd.DataFrame) -> Dict[float, OrderFlowLevel]:
        """
        ...
        for idx, row in data.iterrows():
            # Create a range of prices from low to high for this bar
            bar_min = self._round_price(row['low'])
            bar_max = self._round_price(row['high'])
            
            # Simple volume distribution based on price range
            # This is a simplified approach; actual volume distribution would require tick data
            ...
            while current_price <= bar_max:
                if current_price in price_levels:
                    price_levels[current_price].update(
                        buy_vol=int(buy_per_level),
                        sell_vol=int(sell_per_level)
                    )
                current_price = self._round_price(current_price + self.tick_size)
    ...
    def _calculate_rolling_profiles(self, data: pd.DataFrame):
        ...
        # Create a profile for each bar (starting from num_profile_bars)
        for i in range(self.num_profile_bars - 1, len(data)):
            # Get window of data
            window_data = data.iloc[max(0, i - self.num_profile_bars + 1):i + 1]
            
            # Create price levels for this window
            price_levels = self._create_price_levels(window_data)
            
            # Find point of control (price level with highest volume)
            poc_price = max(price_levels, key=lambda p: price_levels[p].total_volume, default=0)
    ...
    def _calculate_fixed_profiles(self, data: pd.DataFrame):
        ...
        if isinstance(data.index, pd.DatetimeIndex):
            ...
            for date, group_data in grouped:
                # Convert date to datetime for consistent indexing
                start_time = group_data.index.min()
                end_time = group_data.index.max()
                
                # Create price levels for this group
                ...
                # Store profile
                self.profiles[(start_time, end_time)] = {
                    'price_levels': price_levels,
                    'point_of_control': poc_price,
                    'value_area_high': value_area[1],
                    'value_area_low': value_area[0],
    ...
    def _calculate_value_area(self, price_levels: Dict[float, OrderFlowLevel]) -> Tuple[float, float]:
        """
        ...
        # Find point of control (price level with highest volume)
        poc_price = max(price_levels.keys(), key=lambda p: price_levels[p].total_volume)
        ...
        # Expand until we reach the target volume
        while current_volume < target_volume:
            ...
            # Expand in the direction with higher volume
    ...
    def visualize_profile(self, profile_id: Union[int, Tuple[datetime, datetime]],
                         show_delta: bool = True) -> plt.Figure:
        """
        ...
        # Extract volume and delta data
        volumes = [price_levels[p].total_volume for p in sorted_prices]
        deltas = [price_levels[p].delta for p in sorted_prices]
        
        # Create horizontal bars
        bars = ax.barh(sorted_prices, volumes, align='center', height=self.tick_size*0.8)
        
        # Color bars based on delta if requested
        if show_delta:
        ...
        # Add value area highlight
        ax.axhspan(va_low - self.tick_size/2, va_high + self.tick_size/2, 
        ...
        if isinstance(profile_id, tuple):
            title = f"Volume Profile: {profile_id[0].date()} to {profile_id[1].date()}"
        else:
            title = f"Volume Profile: Bar {profile_id}"
            
        ax.set_title(f"{title}\nPOC: {poc:.{self.price_precision}f}, "
                    f"VAH: {va_high:.{self.price_precision}f}, "
                    f"VAL: {va_low:.{self.price_precision}f}")
