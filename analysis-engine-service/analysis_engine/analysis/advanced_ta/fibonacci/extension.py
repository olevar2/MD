...
class FibonacciExtension(FibonacciBase):
    ...
    def __init__(self, ratios: Optional[Dict[float, str]]=None, key_levels:
        ...
        """
        default_ratios = {(0.0): '0%', (0.618): '61.8%', (1.0): '100%', (
            1.272): '127.2%', (1.414): '141.4%', (1.618): '161.8%', (2.0):
            '200%', (2.618): '261.8%', (3.618): '361.8%', (4.236): '423.6%'}
        default_key_levels = [0.0, 1.0, 1.618, 2.618]
        super().__init__(fibonacci_type=FibonacciType.EXTENSION, ratios=
            ratios or default_ratios, key_levels=key_levels or
            default_key_levels)
        ...
        try:
            if not self._validate_points(points):
                raise FibonacciAnalysisError(
                    'Invalid points for Fibonacci extension')
            if len(points) < 3:
                raise FibonacciAnalysisError(
                    'Fibonacci extension requires at least 3 points')
            point1 = points[0]
            point2 = points[1]
            point3 = points[2]
            initial_direction = (FibonacciDirection.UPTREND if point2.price >
                point1.price else FibonacciDirection.DOWNTREND)
            ...
            result = {'type': self.fibonacci_type.value, 'direction':
                initial_direction.value, 'point1': {'index': point1.index,
                'price': point1.price, 'timestamp': point1.timestamp},
                'point2': {'index': point2.index, 'price': point2.price,
                'timestamp': point2.timestamp}, 'point3': {'index': point3.
                index, 'price': point3.price, 'timestamp': point3.timestamp
            ...
            logger.error(f'Error calculating Fibonacci extension: {str(e)}',
                exc_info=True)
            raise FibonacciAnalysisError(
                f'Failed to calculate Fibonacci extension: {str(e)}')
