...
class FibonacciRetracement(FibonacciBase):
    ...
    def __init__(
        ...
        """
        super().__init__(
            fibonacci_type=FibonacciType.RETRACEMENT,
            ratios=ratios,
    ...
    def calculate(
        ...
        try:
            ...
            if not self._validate_points(points):
                raise FibonacciAnalysisError("Invalid points for Fibonacci retracement")
            ...
            for ratio in self.sorted_ratios:
                if direction == FibonacciDirection.UPTREND:
                    # For uptrend, retracements go down from high to low
                    value = end_price - (ratio * price_range)
                else:
                    # For downtrend, retracements go up from low to high
                    value = end_price + (ratio * price_range)
            ...
            logger.error(f"Error calculating Fibonacci retracement: {str(e)}", exc_info=True)
            raise FibonacciAnalysisError(f"Failed to calculate Fibonacci retracement: {str(e)}")
