...
class FixedPositionSizing(PositionSizingModel):
    ...
    def calculate_position_size(self, 
        ...
        # Default to the risk percentage if not explicitly specified
        percentage = self.settings.get('percentage', risk_per_trade)
        value_to_risk = capital * percentage
        
        # Calculate units based on available capital and entry price
        if entry_price > 0:
...
class RiskBasedPositionSizing(PositionSizingModel):
    ...
    def calculate_position_size(self, 
        ...
        if stop_price is None:
            logger.warning("Stop price is required for risk-based position sizing")
            return 0
...
class FixedSlippageModel(SlippageModel):
    ...
    def calculate_slippage(self, 
        ...
        # Get slippage amount in pips or percentage
        ...
        if slippage_unit == 'pips':
            # For forex, typically 0.0001 for most pairs, 0.01 for JPY pairs
        ...
        # Apply direction (buy orders pay more, sell orders receive less)
        if direction == 'long':
...
class VolumeBasedSlippageModel(SlippageModel):
    ...
    def calculate_slippage(self, 
        ...
        # Calculate volatility if market data is provided
        volatility_factor = 1.0
        if market_data is not None and len(market_data) > 1:
            # Use price standard deviation as volatility measure
        ...
        slippage_percentage = base_slippage * (1 + (quantity * volume_factor)) * volatility_factor
...
class BacktestEngine:
    ...
    def __init__(self, config: BacktestConfiguration):
        ...
        self.equity_curve = []  # Equity history
    ...
    def _create_position_sizing_model(self) -> PositionSizingModel:
        ...
        if model_name == "fixed":
            ...
            logger.warning(f"Unknown position sizing model: {model_name}, using fixed")
    ...
    def _create_commission_model(self) -> CommissionModel:
        ...
        if model_name == "fixed":
            ...
            logger.warning(f"Unknown commission model: {model_name}, using fixed")
    ...
    def calculate_metrics(self) -> Dict[str, float]:
        ...
        # Extract key values
        returns = [trade.pnl_percentage for trade in self.trades if trade.pnl_percentage is not None]
        profits = [trade.pnl for trade in self.trades if trade.pnl is not None and trade.pnl > 0]
        losses = [trade.pnl for trade in self.trades if trade.pnl is not None and trade.pnl < 0]
        ...
        if losses:
            ...
            metrics['max_loss'] = np.min(losses)  # Most negative
        
        if metrics['winning_trades'] > 0 and metrics['losing_trades'] > 0:
            metrics['win_rate'] = metrics['winning_trades'] / metrics['total_trades']
            metrics['profit_factor'] = sum(profits) / abs(sum(losses)) if sum(losses) != 0 else float('inf')
            metrics['expectancy'] = (metrics['win_rate'] * metrics.get('mean_profit', 0) - 
                                  (1 - metrics['win_rate']) * abs(metrics.get('mean_loss', 0)))
        
        # Calculate advanced metrics if equity curve exists
        if self.equity_curve:
            ...
            if 'equity' in equity.columns:
                ...
                # Annualized return
                total_days = (equity.iloc[-1]['timestamp'] - equity.iloc[0]['timestamp']).days
                if total_days > 0:
                    total_return = (equity.iloc[-1]['equity'] / equity.iloc[0]['equity']) - 1
                    metrics['annualized_return'] = ((1 + total_return) ** (365 / total_days)) - 1
                ...
                annualized_std = equity['returns'].std() * np.sqrt(252)  # Assuming 252 trading days
                if annualized_std > 0:
                    metrics['sharpe_ratio'] = metrics.get('annualized_return', 0) / annualized_std
                ...
                if len(downside_returns) > 0:
                    ...
                    if downside_std > 0:
                        metrics['sortino_ratio'] = metrics.get('annualized_return', 0) / downside_std
    ...
    def _calculate_position_size(self, capital: float, risk_per_trade: float, 
        ...
        return self.position_sizing_model.calculate_position_size(
            capital, risk_per_trade, entry_price, stop_price, instrument_data
        )
    
    def _calculate_slippage(self, price: float, quantity: float, direction: str,
                         instrument: str, order_type: str, 
                         market_data: Optional[pd.DataFrame] = None) -> float:
        """Calculate slippage using the configured model."""
        return self.slippage_model.calculate_slippage(
            price, quantity, direction, instrument, order_type, market_data
        )
    ...
    def _calculate_commission(self, price: float, quantity: float, 
        ...
        return self.commission_model.calculate_commission(
            price, quantity, instrument, direction
        )
...
def calculate_drawdowns(equity_curve: pd.Series) -> pd.DataFrame:
    ...
    for name, group in grouped:
        if (group < 0).any():  # Only include actual drawdown periods
            start_date = group.index[0]
            end_date = group.index[-1]
            max_drawdown = group.min()
            recovery_date = None
            
            # Look for recovery (next peak after this drawdown)
            if end_date != equity_curve.index[-1]:  # Not the last period
    ...
    if results:
        ...
        return pd.DataFrame(columns=['start_date', 'end_date', 'max_drawdown', 
                                   'recovery_date', 'duration_days', 'recovered'])
...
def calculate_trade_statistics(trades: List[TradeData]) -> Dict[str, Any]:
    ...
    for trade in trades:
        if trade.pnl is not None:
            trade_data.append({
                ...
                'duration': (trade.exit_time - trade.entry_time).total_seconds() / (60 * 60 * 24)  # days
            })
    ...
    stats = {
        ...
        'average_duration_days': df['duration'].mean(),
        'pnl_by_direction': {
            'long': df[df['direction'] == 'long']['pnl'].sum(),
            'short': df[df['direction'] == 'short']['pnl'].sum()
        }
    }
    ...
    if len(df) > 1:
        ...
        stats['sortino_ratio'] = returns.mean() / downside_returns.std() * np.sqrt(252) if not downside_returns.empty and downside_returns.std() > 0 else 0
