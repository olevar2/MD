...
from analysis_engine.analysis.backtesting.core import BacktestResult, BacktestConfiguration, BacktestEngine, DataSplit
    ...
    def generate_windows(self) ->List[WalkForwardWindow]:
        ...
        while True:
            ...
            if test_end > self.end_date:
                break
            window = WalkForwardWindow(train_start=current_train_start,
...
class WalkForwardOptimizer:
    ...
    def __init__(self, config: WalkForwardOptimizationConfig,
        strategy_factory: Callable[[BacktestConfiguration], BacktestEngine],
        data_provider: Callable[[List[str], datetime, datetime, str], Dict[
        str, pd.DataFrame]]):
        """
    ...
    def run_optimization(self) ->WalkForwardResult:
        ...
        """
        logger.info(
            f'Starting walk-forward optimization for {self.config.strategy_name}'
            )
        logger.info(f'Generated {len(self.windows)} windows')
        for i, window in enumerate(self.windows):
            logger.info(
                f'Processing window {i + 1}/{len(self.windows)}: {window.train_start.date()} - {window.test_end.date()}'
                )
        ...
        self.results = WalkForwardResult(strategy_name=self.config.
            strategy_name, start_date=self.config.start_date, end_date=self
            .config.end_date, windows=self.windows, overall_performance=
            overall_performance, equity_curve=equity_curve, best_parameters
            =self._get_most_robust_parameters(), parameter_robustness=
            parameter_robustness)
        return self.results

    @with_exception_handling
    def _optimize_window(self, window: WalkForwardWindow) ->Tuple[Dict[str,
        Any], Dict[str, Any]]:
        """
        ...
        """
        logger.info(
            f'Optimizing window: {window.train_start.date()} - {window.train_end.date()}'
            )
        train_data = self.data_provider(self.config.instruments, window.
            train_start, window.train_end, self.config.data_timeframe)
        if not train_data or not all(len(df) >= self.config.min_window_bars for
            df in train_data.values()):
            logger.warning('Not enough data for training window')
            return {}, {'error': 'Insufficient data'}
        param_combinations = self._generate_parameter_combinations()
        results = []
        if self.config.n_jobs > 1:
            with concurrent.futures.ProcessPoolExecutor(max_workers=self.
                config.n_jobs) as executor:
                futures = [executor.submit(self._evaluate_parameters,
                    params, train_data, window) for params in
                    param_combinations]
            ...
            for params in param_combinations:
                try:
                    result = self._evaluate_parameters(params, train_data,
                        window)
                    results.append(result)
                except Exception as e:
        ...
        if self.config.optimization_direction == 'maximize':
            sorted_results = sorted(results, key=lambda x: x['metrics'].get
                (optimization_metric, float('-inf')), reverse=True)
        else:
            sorted_results = sorted(results, key=lambda x: x['metrics'].get
                (optimization_metric, float('inf')))
        best_result = sorted_results[0]
        best_params = best_result['parameters']
        optimization_details = {'evaluated_parameters': len(results),
            'best_parameters': best_params, 'best_metric': {
            optimization_metric: best_result['metrics'].get(
            optimization_metric)}, 'parameter_statistics': self.
            _calculate_parameter_statistics(results), 'sorted_results':
            sorted_results[:10]}
        return best_params, optimization_details

    def _test_window(self, window: WalkForwardWindow, parameters: Dict[str,
        Any]) ->Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f'Testing window: {window.test_start.date()} - {window.test_end.date()}'
            )
        test_data = self.data_provider(self.config.instruments, window.
            test_start, window.test_end, self.config.data_timeframe)
        if not test_data or not all(len(df) >= self.config.min_window_bars for
            df in test_data.values()):
            logger.warning('Not enough data for test window')
            return {'error': 'Insufficient data'}
        backtest_config = BacktestConfiguration(strategy_name=self.config.
            strategy_name, strategy_parameters=parameters, start_date=
            window.test_start, end_date=window.test_end, initial_capital=
            self.config.initial_capital, instruments=self.config.
            instruments, data_timeframe=self.config.data_timeframe,
        ...
        test_result = {'metrics': result.dict().get('metrics', {}),
            'final_portfolio_value': result.final_portfolio_value,
            'total_return': result.total_return, 'max_drawdown': result.
            max_drawdown, 'sharpe_ratio': result.sharpe_ratio,
            'trades_total': result.trades_total, 'win_rate': result.win_rate}
        return test_result
    ...
    Args:
        ...
        current_params: Description of current_params
    ...
    @with_exception_handling
    def _evaluate_parameters(self, parameters: Dict[str, Any], data: Dict[
        str, pd.DataFrame], window: WalkForwardWindow) ->Dict[str, Any]:
        """
        ...
        """
        try:
            backtest_config = BacktestConfiguration(strategy_name=self.
                config.strategy_name, strategy_parameters=parameters,
                start_date=window.train_start, end_date=window.train_end,
                initial_capital=self.config.initial_capital, instruments=
                self.config.instruments, data_timeframe=self.config.
                data_timeframe, slippage_model=self.config.slippage_model,
                ...
                position_sizing_settings=self.config.
                position_sizing_settings, risk_management_settings=self.
                config.risk_management_settings, data_settings=self.config.
                data_settings)
            strategy = self.strategy_factory(backtest_config)
            result = strategy.run_backtest(data)
            evaluation = {'parameters': parameters, 'metrics': result.dict(
                ).get('metrics', {}), 'final_portfolio_value': result.
                final_portfolio_value, 'total_return': result.total_return,
                'max_drawdown': result.max_drawdown, 'sharpe_ratio': result
                .sharpe_ratio, 'trades_total': result.trades_total,
                'win_rate': result.win_rate}
            return evaluation
        except Exception as e:
            logger.error(f'Error evaluating parameters: {e}')
            return {'parameters': parameters, 'metrics': {}, 'error': str(e)}

    def _calculate_parameter_statistics(self, results: List[Dict[str, Any]]
        ...
        for param_key in parameter_keys:
            unique_values = set(result['parameters'].get(param_key) for
                result in results)
            value_stats = {}
            for value in unique_values:
                filtered_results = [r for r in results if r['parameters'].
                    get(param_key) == value]
                metric_values = [r['metrics'].get(optimization_metric, 0) for
                    r in filtered_results]
                if metric_values:
                    value_stats[str(value)] = {'count': len(metric_values),
                        'mean': float(np.mean(metric_values)), 'median':
                        float(np.median(metric_values)), 'std': float(np.
                        std(metric_values)), 'min': float(np.min(
                        metric_values)), 'max': float(np.max(metric_values))}
    ...
    def _calculate_overall_performance(self) ->Dict[str, float]:
        ...
        """
        test_results = [window.backtest_result for window in self.windows if
            window.backtest_result and 'error' not in window.backtest_result]
        if not test_results:
            return {}
        overall = {}
        ...
        overall['compounded_return'] = np.prod([(1 + result.get(
            'total_return', 0)) for result in test_results]) - 1
        profitable_windows = sum(1 for result in test_results if result.get
            ('total_return', 0) > 0)
        overall['profitable_windows'] = profitable_windows
        overall['profitable_window_percentage'] = profitable_windows / len(
            test_results) if test_results else 0
        if ('avg_total_return' in overall and 'std_total_return' in overall and
            overall['std_total_return'] > 0):
            overall['consistency_score'] = overall['avg_total_return'
                ] / overall['std_total_return']
        else:
    ...
    def _analyze_parameter_robustness(self) ->Dict[str, float]:
        ...
        """
        if not self.windows or not all(window.optimization_result for
            window in self.windows):
            return {}
        best_params_list = [window.optimization_result.get(
            'best_parameters', {}) for window in self.windows if window.
            optimization_result and 'best_parameters' in window.
            optimization_result and window.optimization_result[
            'best_parameters']]
        ...
        for key in all_param_keys:
            values = [params.get(key) for params in best_params_list if key in
                params]
            if not values:
                continue
    ...
    def _get_most_robust_parameters(self) ->Dict[str, Any]:
        ...
        for param, robustness in parameter_robustness.items():
            param_values = [params.get(param) for params in
                best_params_list if param in params]
            if not param_values:
                continue
