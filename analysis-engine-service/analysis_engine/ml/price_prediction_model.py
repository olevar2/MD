...
class PricePredictionModel:
    ...
    def __init__(
        ...
        # Create scalers for data normalization
        self.feature_scaler = MinMaxScaler()
        self.target_scaler = MinMaxScaler()
        
        logger.info(f"Price prediction model initialized with input_window={input_window}, output_window={output_window}")
    ...
    def _create_model(self) -> tf.keras.Model:
        ...
        """
        # Input layer
        input_layer = layers.Input(shape=(self.input_window, len(self.feature_columns)))
        
        # Encoder
        encoder = layers.Conv1D(filters=64, kernel_size=3, padding='same', activation='relu')(input_layer)
        encoder = layers.MaxPooling1D(pool_size=2)(encoder)
        encoder = layers.Conv1D(filters=128, kernel_size=3, padding='same', activation='relu')(encoder)
        encoder = layers.MaxPooling1D(pool_size=2)(encoder)
        
        # LSTM layers
        x = layers.Bidirectional(layers.LSTM(128, return_sequences=True))(encoder)
    ...
    def preprocess_data(
        ...
        X = []
        for i in range(len(feature_data) - self.input_window - self.output_window + 1):
            X.append(feature_data[i:i+self.input_window])
        
        if not is_training:
            # For prediction, just return the last input window
        ...
        for i in range(len(target_data) - self.input_window - self.output_window + 1):
            y.append(target_data[i+self.input_window:i+self.input_window+self.output_window, 0])
    ...
    def train(
        ...
        Returns:
            Training history
        """
        logger.info(f"Training price prediction model on {len(training_data)} symbols")
        ...
        logger.info(f"Validation loss (MSE): {evaluation[0]:.4f}, MAE: {evaluation[1]:.4f}")
    ...
    def predict(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Inverse transform to get actual prices
        predictions_actual = self.target_scaler.inverse_transform(predictions_reshaped)
        ...
        for i in range(self.output_window):
            if i == 0:
                change = (predictions_actual[0, i] - last_price) / last_price * 100
            else:
                change = (predictions_actual[0, i] - predictions_actual[0, i-1]) / predictions_actual[0, i-1] * 100
        ...
        logger.debug(f"Price prediction completed in {prediction_time:.4f} seconds")
        
        return {
            "predictions": predictions_actual[0].tolist(),
