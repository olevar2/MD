...
class EnhancedToolEffectivenessTrackerAdapter(IEnhancedToolEffectivenessTracker
    ...
    def __init__(self, tracker_instance: Optional[
        EnhancedToolEffectivenessTracker]=None):
        """
    ...
    def get_tool_effectiveness(self, tool_id: str, market_regime: Optional[
        ...
        return self.tracker.get_tool_effectiveness(tool_id=tool_id,
            market_regime=market_regime, timeframe=timeframe, symbol=symbol,
    ...
    def get_best_tools(self, market_regime: str, timeframe: Optional[str]=
        ...
        return self.tracker.get_best_tools(market_regime=market_regime,
            timeframe=timeframe, symbol=symbol, min_signals=min_signals,
            top_n=top_n)

    @with_resilience('get_tool_performance_history')
    def get_tool_performance_history(self, tool_id: str, market_regime:
        ...
        return self.tracker.get_performance_history(tool_id=tool_id,
            market_regime=market_regime, timeframe=timeframe, symbol=symbol,
            start_date=start_date, end_date=end_date)

    @with_resilience('get_regime_transition_performance')
    def get_regime_transition_performance(self, tool_id: str, from_regime:
        ...
        return self.tracker.get_regime_transition_performance(tool_id=
            tool_id, from_regime=from_regime, to_regime=to_regime,
            timeframe=timeframe, symbol=symbol)

    @with_resilience('get_tool_correlation_matrix')
    def get_tool_correlation_matrix(self, tool_ids: List[str],
        ...
        return self.tracker.get_correlation_matrix(tool_ids=tool_ids,
            market_regime=market_regime, timeframe=timeframe, symbol=symbol)

    @with_resilience('get_tool_effectiveness_confidence')
    def get_tool_effectiveness_confidence(self, tool_id: str, market_regime:
        Optional[str]=None, timeframe: Optional[str]=None, symbol: Optional
        [str]=None) ->Dict[str, Any]:
        """Get confidence metrics for tool effectiveness."""
        return self.tracker.get_effectiveness_confidence(tool_id=tool_id,
            market_regime=market_regime, timeframe=timeframe, symbol=symbol)

    @with_resilience('get_optimal_tool_combination')
    def get_optimal_tool_combination(self, market_regime: str, timeframe:
        ...
        return self.tracker.get_optimal_combination(market_regime=
            market_regime, timeframe=timeframe, symbol=symbol, max_tools=
            max_tools)
...
class AdaptiveLayerServiceAdapter(IAdaptiveLayerService):
    ...
    def __init__(self, adaptive_layer_instance: Optional[AdaptiveLayer]=
        None, repository: Optional[ToolEffectivenessRepository]=None):
        """
        Initialize the adapter.
        ...
        self.adaptive_layer = adaptive_layer_instance or AdaptiveLayer(self
            .repository)
    ...
    async def get_adaptive_parameters(self, symbol: str, timeframe: str,
        ...
        return self.adaptive_layer.generate_adaptive_parameters(symbol=
            symbol, timeframe=timeframe_enum or timeframe, price_data=
            price_data, available_tools=available_tools, recent_signals=
            recent_signals)
    ...
    async def record_strategy_performance(self, strategy_id: str, symbol:
        ...
        try:
            await self.repository.save_strategy_performance(strategy_id=
                strategy_id, symbol=symbol, timeframe=timeframe,
                performance_metrics=performance_metrics, parameters_used=
                parameters_used)
            return True
    ...
    async def get_adaptation_recommendations(self, symbol: str, timeframe:
        ...
        if isinstance(timeframe, str):
            ...
            timeframe_enum = timeframe
        market_regime = (self.adaptive_layer.market_regime_service.
            detect_regime(price_data=current_market_data.get('price_data'),
            symbol=symbol, timeframe=timeframe_enum or timeframe))
        tool_metrics = {}
        available_tools = current_market_data.get('available_tools', [])
        for tool_id in available_tools:
            metrics = await self.repository.get_tool_metrics(tool_id=
                tool_id, market_regime=market_regime, timeframe=timeframe)
            tool_metrics[tool_id] = metrics
        recommendations = {'symbol': symbol, 'timeframe': timeframe,
            'strategy_id': strategy_id, 'market_regime': market_regime,
            'confidence': 0.8, 'recommended_tools': [],
            'parameter_adjustments': {}, 'risk_recommendation': 'neutral',
            'timestamp': datetime.now().isoformat()}
        sorted_tools = sorted(tool_metrics.items(), key=lambda x: x[1].get(
            'effectiveness_score', 0.0), reverse=True)
        recommendations['recommended_tools'] = [{'tool_id': tool_id,
            'effectiveness_score': metrics.get('effectiveness_score', 0.0),
            'weight': max(0.1, min(1.0, metrics.get('effectiveness_score', 
            0.5) * 2))} for tool_id, metrics in sorted_tools[:5]]
