...
from core_foundations.exceptions.client_exceptions import MLClientConnectionError, MLClientTimeoutError, MLClientAuthError, MLJobSubmissionError
...
class MLPipelineClient:
    ...
    def __init__(self, config_manager: Optional[ConfigurationManager]=None):
        ...
        self.timeout = self.config_manager.get('timeout', 30.0)
        self.auth_token = self.config_manager.get('auth_token')
        ...
        recovery_timeout = self.config.get('circuit_breaker_recovery_timeout',
            60)
        self.circuit_breaker = CircuitBreaker(fail_max=failure_threshold,
            reset_timeout=recovery_timeout)
        self.max_attempts = self.config_manager.get('max_retries', 3)
        self.base_delay = self.config_manager.get('base_delay', 1.0)
        self.backoff_factor = self.config_manager.get('backoff_factor', 1.5)
        self.max_delay = self.config_manager.get('max_backoff', 30)
        self.retryable_exceptions = [MLClientConnectionError,
            MLClientTimeoutError]
        logger.info(
            f'MLPipelineClient initialized. Base URL: {self.base_url}, Timeout: {self.timeout}, Max Retries: {self.max_attempts}'
            )
    ...
    def _load_config(self) ->Dict[str, Any]:
        ...
        """
        default_config = {'base_url': 'http://ml-integration-service:8000',
            'timeout': 30.0, 'circuit_breaker_failure_threshold': 5,
            'circuit_breaker_recovery_timeout': 60, 'max_retries': 3,
            'base_delay': 1.0, 'backoff_factor': 1.5, 'max_backoff': 30,
            'auth_token': None}
        if self.config_manager:
            try:
                ml_config = self.config_manager.get_section(
                    'ml_pipeline_client')
                default_config.update(ml_config)
                logger.info(
                    'Loaded configuration for MLPipelineClient from ConfigurationManager.'
                    )
            except Exception as e:
                logger.warning(
                    f'Could not load MLPipelineClient config from manager: {e}. Using defaults.'
                    )
    ...
    async def _make_request(self, method: str, endpoint: str, data:
        ...
        if self.circuit_breaker.current_state == CircuitBreakerState.OPEN:
            logger.warning(
                'Circuit breaker is open. Refusing to make request to ML service.'
                )
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                logger.debug(
                    f'Making {method.upper()} request to {url} with data: {data}'
                    )
                if method.upper() == 'GET':
                ...
                if response.status_code == 401 or response.status_code == 403:
                    logger.error(
                        f'Authentication failed for {method.upper()} {url}: {response.status_code}'
                        )
                    raise MLClientAuthError(
                        f'Authentication failed: {response.status_code}')
                if response.status_code >= 400:
                    self.circuit_breaker.record_failure()
                    logger.error(
                        f'ML service request failed for {method.upper()} {url}: {response.status_code}, {response.text}'
                        )
                    raise MLJobSubmissionError(
                        f'ML service request failed: {response.status_code}, {response.text}'
                        )
            ...
            raise MLClientConnectionError(
                f'Failed to connect to ML service: {e}') from e
        except httpx.TimeoutException as e:
            self.circuit_breaker.record_failure()
            logger.error(f'Timeout connecting to ML service ({url}): {e}')
            raise MLClientTimeoutError(f'Request to ML service timed out: {e}'
                ) from e
        except Exception as e:
            self.circuit_breaker.record_failure()
            logger.error(
                f'Unexpected error making request to ML service ({url}): {e}',
                exc_info=True)
            raise MLClientError(f'An unexpected error occurred: {e}') from e

    @retry_with_policy(exceptions=lambda self: self.retryable_exceptions,
        max_attempts=lambda self: self.max_attempts, base_delay=lambda self:
        self.base_delay, max_delay=lambda self: self.max_delay,
        backoff_factor=lambda self: self.backoff_factor, service_name=
        'ml-pipeline-client', operation_name='start_retraining_job')
    @async_with_exception_handling
    async def start_retraining_job(self, model_id: str, params: Optional[
        ...
        """
        endpoint = f'/jobs/retrain/{model_id}'
        payload = params if params is not None else {}
        logger.info(
            f'Starting retraining job for model_id: {model_id} with params: {payload}'
            )
        try:
            ...
            if not job_id:
                logger.error(
                    f"ML service response missing 'job_id'. Response: {response_data}"
                    )
                raise MLJobSubmissionError(
                    'ML service response did not include a job ID.')
            logger.info(
                f'Successfully submitted retraining job for model_id: {model_id}. Job ID: {job_id}'
                )
            return job_id
        except (MLClientConnectionError, MLClientTimeoutError,
            MLJobSubmissionError, MLClientAuthError) as e:
            logger.error(
                f'Failed to start retraining job for model_id {model_id}: {e}')
            raise
        except Exception as e:
            logger.error(
                f'An unexpected error occurred while starting retraining job for model_id {model_id}: {e}'
                , exc_info=True)
            raise MLClientError(
                f'An unexpected error occurred during job submission: {e}'
                ) from e
    ...
    async def list_models(self) ->List[Dict[str, Any]]:
        ...
        try:
            ...
            if not isinstance(models_data, list):
                logger.error(
                    f'Expected list of models, but got: {type(models_data)}')
                raise MLClientError(
                    'Invalid response format when listing models.')
    ...
    async def get_model_details(self, model_id: str) ->Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f'Successfully fetched details for model_id: {model_id}')
            return model_data
        except Exception as e:
    ...
    async def deploy_model(self, model_id: str, environment: str=
        'production', config: Optional[Dict[str, Any]]=None) ->Dict[str, Any]:
        """
        ...
        payload = {'environment': environment, **config or {}}
        logger.info(
            f'Deploying model_id: {model_id} to environment: {environment}')
        try:
            response_data = await self._make_request('POST', endpoint, data
                =payload)
            logger.info(
                f'Successfully initiated deployment for model_id: {model_id} to {environment}'
                )
            return response_data
        except Exception as e:
            logger.error(
                f'Failed to deploy model_id {model_id} to environment {environment}: {e}'
                )
            raise
    ...
    async def get_prediction(self, model_id: str, features: Dict[str, Any],
        ...
        try:
            ...
            logger.debug(
                f'Prediction latency: {latency_ms:.2f}ms for model_id: {model_id}'
                )
            response_data['metadata'] = {'request_time': start_time.
                isoformat(), 'response_time': end_time.isoformat(),
                'latency_ms': latency_ms, 'model_id': model_id, 'version': 
                version or 'latest'}
            logger.info(
                f'Successfully obtained prediction from model_id: {model_id}')
            return response_data
        except Exception as e:
            logger.error(
                f'Failed to get prediction from model_id {model_id}: {e}')
            raise
