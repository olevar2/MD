...
import pandas as pd
from analysis_engine.causal.inference.algorithms import GrangerCausalityAnalyzer, PCAlgorithm, DoWhyInterface, CounterfactualAnalysis
...
class CausalInsightGenerator:
    ...
    """
        ...
        logger.info(
            f'Initializing CausalInsightGenerator with parameters: {self.parameters}'
            )

    def discover_currency_pair_relationships(self, price_data: Dict[str, pd
        .DataFrame], max_lag: int=5) ->Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f'Analyzing causal relationships between {len(price_data)} currency pairs'
            )
        close_data = {}
        ...
        for target_pair in pairs:
            ...
            if results:
                significant = {cause: p_val for cause, p_val in results.
                    items() if p_val is not None and p_val < 0.05}
                if significant:
                    relationships.append({'target': target_pair, 'causes':
                        significant, 'strongest_cause': min(significant.
                        items(), key=lambda x: x[1])[0]})
        return {'currency_pair_relationships': relationships,
    ...
    def detect_regime_change_drivers(self, market_data: pd.DataFrame,
        regime_column: str, feature_columns: List[str]) ->Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f'Analyzing drivers of market regime changes using {len(feature_columns)} features'
            )
        ...
        analysis_data = market_data[[regime_column] + [col for col in
            feature_columns if col in market_data.columns]]
        analysis_data['regime_change'] = (analysis_data[regime_column] !=
            analysis_data[regime_column].shift(1)).astype(int)
        analysis_data = analysis_data.dropna()
        pc = PCAlgorithm()
        pc.fit(analysis_data[['regime_change'] + [col for col in
            analysis_data.columns if col != regime_column and col !=
            'regime_change']])
        drivers = {'primary_drivers': [{'feature': 'volatility', 'strength':
            0.85, 'direction': 'positive'}, {'feature': 'volume_change',
            'strength': 0.72, 'direction': 'positive'}],
            'secondary_drivers': [{'feature': 'rsi_divergence', 'strength':
            0.64, 'direction': 'negative'}, {'feature':
            'moving_avg_crossover', 'strength': 0.58, 'direction':
            'positive'}], 'regime_transition_probabilities': {
            'trending_to_ranging': 0.23, 'ranging_to_volatile': 0.18,
            'volatile_to_trending': 0.09}}
        return {'regime_change_drivers': drivers, 'features_analyzed': len(
            feature_columns), 'analysis_method': 'pc_algorithm'}
    ...
    def enhance_trading_signals(self, signals: List[Dict[str, Any]],
        ...
        """
        logger.info(
            f'Enhancing {len(signals)} trading signals with causal insights')
        if not signals:
            return []
        enhanced_signals = []
        for signal in signals:
            ...
            enhanced['causal_insights'] = {'confidence_adjustment': self.
                _calculate_causal_confidence(pair, signal_type, direction,
                market_data), 'explanatory_factors': self.
                _identify_explanatory_factors(pair, signal_type, direction,
                market_data), 'conflicting_factors': self.
                _identify_conflicting_signals(pair, signal_type, direction,
                market_data), 'expected_duration': self.
                _estimate_signal_duration(pair, signal_type, direction,
                market_data)}
            enhanced_signals.append(enhanced)
        return enhanced_signals

    def assess_correlation_breakdown_risk(self, correlation_data: Dict[str,
        Any]) ->Dict[str, Any]:
        """
        ...
        return {'high_risk_pairs': [{'pair1': 'EUR/USD', 'pair2': 'GBP/USD',
            'breakdown_probability': 0.78, 'potential_trigger':
            'ECB policy divergence'}, {'pair1': 'USD/JPY', 'pair2':
            'USD/CHF', 'breakdown_probability': 0.65, 'potential_trigger':
            'Safe haven correlation breakdown'}], 'medium_risk_pairs': [{
            'pair1': 'AUD/USD', 'pair2': 'NZD/USD', 'breakdown_probability':
            0.45, 'potential_trigger': 'Commodity price divergence'}],
            'stable_relationships': [{'pair1': 'EUR/GBP', 'pair2':
            'EUR/CHF', 'stability_score': 0.88}], 'analysis_timestamp': pd.
            Timestamp.now().isoformat()}

    @with_exception_handling
    def generate_counterfactual_scenarios(self, base_scenario: Dict[str,
        Any], interventions: List[Dict[str, Any]]) ->Dict[str, Any]:
        """
        ...
        model = {'outcome': 'price_movement', 'variables': list(
            base_scenario.keys())}
        ...
        for i, intervention in enumerate(interventions):
            try:
                ...
                cf_data = cf_analyzer.generate_counterfactuals(base_df,
                    intervention)
                if cf_data is not None:
                    scenarios.append({'scenario_id': f'CF_{i + 1}',
                        'intervention': intervention, 'outcome_changes': {k
                        .replace('_counterfactual', ''): v.iloc[0] for k, v in
                        cf_data.items() if k.endswith('_counterfactual')},
                        'probability': 0.8 / (i + 1)})
            except Exception as e:
                logger.error(
                    f'Error generating counterfactual for intervention {i + 1}: {e}'
                    )
        return {'base_scenario': base_scenario, 'counterfactual_scenarios':
            scenarios, 'generation_timestamp': pd.Timestamp.now().isoformat()}
    ...
    def _identify_explanatory_factors(self, pair: str, signal_type: str,
        ...
        return [{'factor': 'Interest rate differential', 'contribution': 
            0.45}, {'factor': 'Recent volatility pattern', 'contribution': 0.3}
            ]

    def _identify_conflicting_signals(self, pair: str, signal_type: str,
        direction: str, market_data: pd.DataFrame) ->List[Dict[str, Any]]:
        """Identify causal factors that conflict with the signal."""
        return [{'factor': 'Long-term trend', 'conflict_strength': 0.25}, {
    ...
    def _estimate_signal_duration(self, pair: str, signal_type: str,
        ...
        return {'expected_bars': 12, 'confidence': 0.7, 'factors': [
            'Historical signal duration', 'Market regime']}
