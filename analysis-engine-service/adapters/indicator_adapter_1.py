...
class IndicatorAdapter:
    ...
    def __init__(self, multi_asset_service: Optional[
        MultiAssetServiceAdapter]=None, feature_store_client: Optional[
    ...
    @with_resilience('get_indicator')
    def get_indicator(self, indicator_type: str, symbol: str, data: pd.
        DataFrame=None, **kwargs) ->pd.DataFrame:
        """
        ...
        if not asset_info:
            self.logger.warning(
                f'Asset info not found for {symbol}, using default parameters')
            asset_class = AssetClass.FOREX
        else:
            asset_class = asset_info.get('asset_class')
        adjusted_params = self._adjust_parameters_for_asset(asset_class,
            indicator_type, kwargs)
        if data is None:
            lookback = self._get_required_lookback(indicator_type,
                adjusted_params)
            data = self.feature_store_client.get_market_data(symbol,
                lookback=lookback)
        indicator_cols = self._get_indicator_columns(indicator_type,
            adjusted_params)
        if all(col in data.columns for col in indicator_cols):
            self.logger.debug(
                f'Indicator {indicator_type} columns already in DataFrame')
            return data
        indicator_data = self.feature_store_client.get_indicator(indicator_type
            =indicator_type, symbol=symbol, params=adjusted_params)
        if indicator_data is not None and len(indicator_data) > 0:
            if isinstance(data.index, pd.DatetimeIndex) and isinstance(
                indicator_data.index, pd.DatetimeIndex):
                result = data.join(indicator_data, how='left')
            else:
                merge_cols = [col for col in ['timestamp', 'date', 'time'] if
                    col in data.columns and col in indicator_data.columns]
                if merge_cols:
                    result = pd.merge(data, indicator_data, on=merge_cols,
                        how='left')
                else:
                    self.logger.warning(
                        f'Could not determine how to merge data frames for {indicator_type}'
                        )
                    if len(data) == len(indicator_data):
                        for col in indicator_cols:
                            if (col in indicator_data.columns and col not in
                                data.columns):
            ...
            self.logger.warning(
                f'Could not fetch {indicator_type} from feature store')
            return data

    def moving_average(self, data: pd.DataFrame, symbol: str, column: str=
        'close', period: int=20, ma_type: str='sma') ->pd.DataFrame:
        """
        ...
        params = {'window': adjusted_period, 'price_column': column,
            'ma_type': ma_type}
        return self.get_indicator(ma_type, symbol, data, **params)

    def relative_strength_index(self, data: pd.DataFrame, symbol: str,
        column: str='close', period: int=14) ->pd.DataFrame:
        """
    ...
    def bollinger_bands(self, data: pd.DataFrame, symbol: str, column: str=
        'close', period: int=20, std_dev: float=2.0) ->pd.DataFrame:
        """
    ...
    def average_true_range(self, data: pd.DataFrame, symbol: str, period:
        int=14) ->pd.DataFrame:
        """
        ...
        if atr_col in result.columns:
            if asset_class == AssetClass.FOREX:
                ...
                if pip_col not in result.columns:
                    pip_factor = asset_info.get('trading_parameters', {}).get(
                        'pip_value', 0.0001)
                    if pip_factor > 0:
    ...
    def macd(self, data: pd.DataFrame, symbol: str, column: str='close',
        ...
        adjusted_signal = self._adjust_macd_signal_period(asset_class,
            signal_period)
        params = {'price_column': column, 'fast_period': adjusted_fast,
            'slow_period': adjusted_slow, 'signal_period': adjusted_signal}
        return self.get_indicator('macd', symbol, data, **params)

    def _adjust_parameters_for_asset(self, asset_class: AssetClass,
        indicator_type: str, params: Dict[str, Any]) ->Dict[str, Any]:
        ...
        if indicator_type in ['sma', 'ema', 'wma']:
            ...
            if 'signal_period' in adjusted_params:
                adjusted_params['signal_period'
                    ] = self._adjust_macd_signal_period(asset_class,
                    adjusted_params['signal_period'])
            ...
            if 'std_dev' in adjusted_params:
                adjusted_params['std_dev'] = adjusted_params['std_dev'
                    ] * self._get_std_dev_multiplier(asset_class)
        return adjusted_params

    def _get_indicator_columns(self, indicator_type: str, params: Dict[str,
        Any]) ->List[str]:
        ...
        if indicator_type == 'sma':
            ...
            return [f'bb_middle_{window}', f'bb_upper_{window}',
                f'bb_lower_{window}']
        elif indicator_type == 'atr':
    ...
    def _get_required_lookback(self, indicator_type: str, params: Dict[str,
        Any]) ->int:
        """
    ...
    def _adjust_ma_period_for_asset(self, asset_class: AssetClass, period: int
        ) ->int:
    ...
    def _adjust_rsi_period_for_asset(self, asset_class: AssetClass, period: int
        ) ->int:
        """Adjust RSI period based on asset class"""
        if asset_class == AssetClass.FOREX:
            return period
        elif asset_class == AssetClass.CRYPTO:
            return max(10, int(period * 0.8))
        elif asset_class == AssetClass.STOCKS:
            return period
        else:
            return period

    def _adjust_atr_period_for_asset(self, asset_class: AssetClass, period: int
        ) ->int:
    ...
    def _adjust_macd_fast_period(self, asset_class: AssetClass, period: int
        ) ->int:
        """Adjust MACD fast period based on asset class"""
        if asset_class == AssetClass.CRYPTO:
            return max(8, int(period * 0.9))
        else:
            return period

    def _adjust_macd_slow_period(self, asset_class: AssetClass, period: int
        ) ->int:
        """Adjust MACD slow period based on asset class"""
        if asset_class == AssetClass.CRYPTO:
            return int(period * 0.9)
        else:
            return period

    def _adjust_macd_signal_period(self, asset_class: AssetClass, period: int
        ) ->int:
