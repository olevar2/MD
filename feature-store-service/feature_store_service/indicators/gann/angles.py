...
class GannAngles(BaseGannIndicator):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Validate pivot_type
        valid_pivot_types = ['swing_low', 'swing_high', 'recent_low', 'recent_high', 'major_low', 'major_high']
        ...
        if angle_types is None:
            ...
            # Check if all provided angle types are valid
            invalid_angles = [a for a in angle_types if a not in all_angles]
            if invalid_angles:
                print(f"Warning: Invalid angle types {invalid_angles} will be ignored")

            # Filter out invalid angle types
            self.angle_types = [a for a in angle_types if a in all_angles]

            if not self.angle_types:
                print("Warning: No valid angle types provided, using default '1x1' angle")
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if pivot_idx is None or pivot_price is None:
                ...
                print(f"Warning: Could not find a valid pivot point for {self.pivot_type}")
                return result
            ...
            for angle_type in self.angle_types:
                try:
                    ...
                    print(f"Warning: Error calculating {angle_type} angle: {str(e)}")
            ...
            # Return the original data with empty angle columns
            for angle_type in self.angle_types:
                col_name_up = f"gann_angle_up_{angle_type}"
                col_name_down = f"gann_angle_down_{angle_type}"
                result[col_name_up] = None
