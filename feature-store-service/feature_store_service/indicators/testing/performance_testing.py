...
class PerformanceTestResult:
    ...
    def __init__(self, name: str, standard_time: float, enhanced_time: float,
                standard_memory: float = None, enhanced_memory: float = None):
        """
        ...
        self.memory_reduction = (
            (standard_memory / enhanced_memory if enhanced_memory > 0 else float('inf'))
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'enhanced_time': self.enhanced_time,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'PerformanceTestResult':
        ...
        return cls(
            ...
            enhanced_time=data['enhanced_time'],
            standard_memory=data.get('standard_memory'),
            enhanced_memory=data.get('enhanced_memory')
        )
...
class IndicatorPerformanceBenchmark:
    ...
    def benchmark_indicator(self, standard_indicator: BaseIndicator, 
                           enhanced_indicator: PerformanceEnhancedIndicator,
                           test_data: pd.DataFrame, name: str = None) -> PerformanceTestResult:
        """
    ...
    def benchmark_multiple_data_sizes(self, standard_indicator_factory: Callable[[], BaseIndicator],
                                     enhanced_indicator_factory: Callable[[], PerformanceEnhancedIndicator],
                                     data_generator: Callable[[int], pd.DataFrame],
                                     sizes: List[int], name_prefix: str = "") -> List[PerformanceTestResult]:
        """
        ...
        for size in sizes:
            # Generate indicators and test data
    ...
    def plot_results(self, metric: str = 'speedup', save_path: Optional[str] = None):
        ...
        # Separate results by name prefix (for grouped tests)
        result_groups = {}
        for result in self.results:
            # Extract prefix and size from name
...
class PerformanceTests(unittest.TestCase):
    ...
    def test_gpu_acceleration(self):
        ...
        np.testing.assert_allclose(test_array, cpu_array, rtol=1e-5)
        ...
        # Verify results
        np.testing.assert_allclose(ma_gpu[20-1:], ma_cpu[20-1:], rtol=1e-5)
    ...
    def test_memory_optimization(self):
        ...
        large_df = pd.DataFrame({
            ...
            'category_col': np.random.choice(['A', 'B', 'C', 'D'], size=100000),
            'sparse_col': np.random.choice([0, 1], p=[0.9, 0.1], size=100000)
        })
        
        # Measure original memory usage
        original_size = large_df.memory_usage(deep=True).sum() / (1024 * 1024)  # MB
        ...
        # Verify reduction
        self.assertLess(optimized_size, original_size * 0.8,  # At least 20% reduction
    ...
    def generate_benchmark_report(self, output_dir: str = "benchmark_results"):
        ...
        # Test SMA with different data sizes
        sizes = [1000, 10000, 100000]
        benchmark.benchmark_multiple_data_sizes(
            lambda: SMA(window=20),
            lambda: EnhancedSMA(window=20, use_gpu=is_gpu_available()),
            generate_data,
            sizes,
            name_prefix="SMA_"
        )
        ...
        benchmark.plot_results(metric='memory_reduction', 
                             save_path=os.path.join(output_dir, "memory_chart.png"))
