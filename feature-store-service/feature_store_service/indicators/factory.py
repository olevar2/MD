...
# Corrected import path for RSI, MACD, Stochastic and newly implemented oscillators
...
# Dictionary mapping indicator names to their respective classes
INDICATOR_REGISTRY = {
    # Moving Averages
    "sma": SMA,
    "ema": EMA,
    ...
    # Momentum Indicators (now from oscillators.py)
    "rsi": RSI,
    "macd": MACD,
    ...
    # Oscillators (newly implemented)
    "cci": CCI,
    "williams_r": Williams_R,
    "roc": ROC
}

def create_indicator(indicator_name: str, parameters: Dict[str, Any]) -> BaseIndicator:
    """
    ...
    # Check if the indicator exists in our registry
    ...
    try:
        ...
        logger.debug(f"Created {indicator_name} indicator with parameters: {parameters}")
        return indicator
    except Exception as e:
        logger.error(f"Error creating {indicator_name}: {str(e)}")
        raise ValueError(f"Failed to create indicator '{indicator_name}': {str(e)}")
...
def get_available_indicators() -> Dict[str, Dict[str, Any]]:
    ...
    for name, indicator_class in INDICATOR_REGISTRY.items():
        ...
        if not metadata:
            ...
            metadata = {
                "name": name,
                "description": getattr(indicator_class, "__doc__", "No description available"),
