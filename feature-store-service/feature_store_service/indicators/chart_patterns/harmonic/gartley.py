"""
Gartley Pattern Module.

This module provides implementation of the Gartley harmonic pattern recognition.
"""
...
class GartleyPattern(BasePatternDetector):
    """
    Gartley Pattern Detector.

    The Gartley pattern is a harmonic pattern that uses Fibonacci retracement levels
    ...
    - CD: 1.272-1.618 extension of BC
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        self.name = "gartley_pattern"
        self.lookback_period = lookback_period
        self.tolerance = max(0.01, min(0.10, tolerance))

        # Fibonacci ratios for Gartley pattern
        self.ab_ratio = 0.618  # AB should be 0.618 retracement of XA
        self.bc_ratio_min = 0.382  # BC should be 0.382-0.886 retracement of AB
        self.bc_ratio_max = 0.886
        self.cd_ratio_min = 1.272  # CD should be 1.272-1.618 extension of BC
        self.cd_ratio_max = 1.618
        self.xd_ratio = 0.786  # D should be 0.786 retracement of XA

    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        """
        Calculate Gartley pattern recognition for the given data.

        Args:
            data: DataFrame with OHLCV data

        Returns:
            DataFrame with Gartley pattern values
        """
        ...
        result['pattern_gartley_bullish'] = 0
        result['pattern_gartley_bearish'] = 0

        # Find potential swing points (highs and lows)
        swing_highs, swing_lows = self._find_swing_points(data, window=5)

        # Look for bullish Gartley patterns (forms at lows)
        bullish_patterns = self._find_bullish_gartley(data, swing_highs, swing_lows)

        # Look for bearish Gartley patterns (forms at highs)
        bearish_patterns = self._find_bearish_gartley(data, swing_highs, swing_lows)

        # Mark pattern locations in the result DataFrame
        for pattern in bullish_patterns:
            d_point = pattern['points']['D']['idx']
            if 0 <= d_point < len(result):
                result.loc[result.index[d_point], 'pattern_gartley_bullish'] = 1

        for pattern in bearish_patterns:
            d_point = pattern['points']['D']['idx']
            if 0 <= d_point < len(result):
                result.loc[result.index[d_point], 'pattern_gartley_bearish'] = 1
    ...
    def _find_bullish_gartley(self, data: pd.DataFrame, swing_highs: List[int], swing_lows: List[int]) -> List[Dict[str, Any]]:
        """
        Find bullish Gartley patterns in the data.
        ...
        Returns:
            List of bullish Gartley patterns
        """
        patterns = []
        window = 5  # Window size for swing point detection

        # Need at least 4 swing points to form a Gartley pattern
        ...
        for x_idx in swing_lows:
            ...
            for a_idx in [h for h in swing_highs if h > x_idx]:
                ...
                for b_idx in [l for l in swing_lows if l > a_idx]:
                    ...
                    # Check if AB is approximately 0.618 retracement of XA
    ...
    def _find_bearish_gartley(self, data: pd.DataFrame, swing_highs: List[int], swing_lows: List[int]) -> List[Dict[str, Any]]:
        """
        Find bearish Gartley patterns in the data.
        ...
        Returns:
            List of bearish Gartley patterns
        """
        patterns = []
        window = 5  # Window size for swing point detection
                ...
                for b_idx in [h for h in swing_highs if h > a_idx]:
                    ...
                    for c_idx in [l for l in swing_lows if l > b_idx]:
                        ...
                        for d_idx in [h for h in swing_highs if h > c_idx]:
                            ...
                            # All conditions met, we have a bearish Gartley pattern
                            pattern = {
                                'start_idx': x_idx,
                                'end_idx': d_idx,
                                'pattern_type': 'gartley',
    ...
    def _calculate_pattern_strength(self, ab_ratio: float, ab_target: float,
        ...
        # XD ratio is most important for Gartley pattern
