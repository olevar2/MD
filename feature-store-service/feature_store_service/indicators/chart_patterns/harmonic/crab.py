...
This module provides implementation of the Crab harmonic pattern recognition.
"""
...
from feature_store_service.indicators.chart_patterns.base import BasePatternDetector, PatternType


class CrabPattern(BasePatternDetector):
    """
    Crab Pattern Detector.
    
    The Crab pattern is a harmonic pattern that uses Fibonacci retracement levels
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        self.name = "crab_pattern"
        self.lookback_period = lookback_period
        self.tolerance = max(0.01, min(0.10, tolerance))
        
        # Fibonacci ratios for Crab pattern
        self.ab_ratio_min = 0.382  # AB should be 0.382-0.618 retracement of XA
        self.ab_ratio_max = 0.618
        self.bc_ratio_min = 0.382  # BC should be 0.382-0.886 retracement of AB
        self.bc_ratio_max = 0.886
        self.cd_ratio_min = 2.618  # CD should be 2.618-3.618 extension of BC
        self.cd_ratio_max = 3.618
        self.xd_ratio = 1.618  # D should be 1.618 extension of XA
        
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        """
        Calculate Crab pattern recognition for the given data.
        
        Args:
            data: DataFrame with OHLCV data
            
        Returns:
            DataFrame with Crab pattern values
        """
        ...
        result['pattern_crab_bullish'] = 0
        result['pattern_crab_bearish'] = 0
        
        # Find potential swing points (highs and lows)
        swing_highs, swing_lows = self._find_swing_points(data, window=5)
        
        # Look for bullish Crab patterns (forms at lows)
        bullish_patterns = self._find_bullish_crab(data, swing_highs, swing_lows)
        
        # Look for bearish Crab patterns (forms at highs)
        bearish_patterns = self._find_bearish_crab(data, swing_highs, swing_lows)
        
        # Mark pattern locations in the result DataFrame
        for pattern in bullish_patterns:
            d_point = pattern['points']['D']['idx']
            if 0 <= d_point < len(result):
                result.loc[result.index[d_point], 'pattern_crab_bullish'] = 1
        
        for pattern in bearish_patterns:
            d_point = pattern['points']['D']['idx']
            if 0 <= d_point < len(result):
                result.loc[result.index[d_point], 'pattern_crab_bearish'] = 1
    ...
    def _find_bullish_crab(self, data: pd.DataFrame, swing_highs: List[int], swing_lows: List[int]) -> List[Dict[str, Any]]:
        """
        Find bullish Crab patterns in the data.
        ...
        Returns:
            List of bullish Crab patterns
        """
        patterns = []
        window = 5  # Window size for swing point detection
        
        # Need at least 4 swing points to form a Crab pattern
        ...
        for x_idx in swing_lows:
            ...
            for a_idx in [h for h in swing_highs if h > x_idx]:
                ...
                for b_idx in [l for l in swing_lows if l > a_idx]:
                    ...
                    # Check if AB is within 0.382-0.618 retracement of XA
                    ab_ratio = ab_diff / xa_diff
                    if not (self._is_within_tolerance(ab_ratio, self.ab_ratio_min, self.ab_ratio_max)):
                        continue
                        ...
                        for d_idx in [l for l in swing_lows if l > c_idx]:
                            ...
                            pattern = {
                                ...
                                'strength': self._calculate_pattern_strength(
                                    ab_ratio, (self.ab_ratio_min + self.ab_ratio_max) / 2,
    ...
    def _find_bearish_crab(self, data: pd.DataFrame, swing_highs: List[int], swing_lows: List[int]) -> List[Dict[str, Any]]:
        """
        Find bearish Crab patterns in the data.
                ...
                for b_idx in [h for h in swing_highs if h > a_idx]:
                    ...
                    for c_idx in [l for l in swing_lows if l > b_idx]:
                        ...
                        for d_idx in [h for h in swing_highs if h > c_idx]:
                            ...
                            # All conditions met, we have a bearish Crab pattern
                            pattern = {
                                'start_idx': x_idx,
                                'end_idx': d_idx,
                                'pattern_type': 'crab',
    ...
    def _calculate_pattern_strength(self, ab_ratio: float, ab_target: float,
        ...
        # XD ratio is most important for Crab pattern
