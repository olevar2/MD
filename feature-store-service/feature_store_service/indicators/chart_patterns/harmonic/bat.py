...
This module provides implementation of the Bat harmonic pattern recognition.
"""
...
class BatPattern(BasePatternDetector):
    """
    Bat Pattern Detector.
    
    The Bat pattern is a harmonic pattern that uses Fibonacci retracement levels
    ...
    Bullish Bat:
    - XA: Initial leg
    - AB: 0.382-0.5 retracement of XA
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        self.name = "bat_pattern"
        self.lookback_period = lookback_period
        self.tolerance = max(0.01, min(0.10, tolerance))
        
        # Fibonacci ratios for Bat pattern
        self.ab_ratio_min = 0.382  # AB should be 0.382-0.5 retracement of XA
        self.ab_ratio_max = 0.5
        ...
        self.xd_ratio = 0.886  # D should be 0.886 retracement of XA
        
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        """
        Calculate Bat pattern recognition for the given data.
        
        Args:
            data: DataFrame with OHLCV data
            
        Returns:
            DataFrame with Bat pattern values
        """
        ...
        result['pattern_bat_bullish'] = 0
        result['pattern_bat_bearish'] = 0
        
        # Find potential swing points (highs and lows)
        swing_highs, swing_lows = self._find_swing_points(data, window=5)
        
        # Look for bullish Bat patterns (forms at lows)
        bullish_patterns = self._find_bullish_bat(data, swing_highs, swing_lows)
        
        # Look for bearish Bat patterns (forms at highs)
        bearish_patterns = self._find_bearish_bat(data, swing_highs, swing_lows)
        
        # Mark pattern locations in the result DataFrame
        for pattern in bullish_patterns:
            d_point = pattern['points']['D']['idx']
            if 0 <= d_point < len(result):
                result.loc[result.index[d_point], 'pattern_bat_bullish'] = 1
        
        for pattern in bearish_patterns:
            d_point = pattern['points']['D']['idx']
            if 0 <= d_point < len(result):
                result.loc[result.index[d_point], 'pattern_bat_bearish'] = 1
    ...
    def _find_bullish_bat(self, data: pd.DataFrame, swing_highs: List[int], swing_lows: List[int]) -> List[Dict[str, Any]]:
        """
        Find bullish Bat patterns in the data.
        ...
        Returns:
            List of bullish Bat patterns
        """
        patterns = []
        window = 5  # Window size for swing point detection
        
        # Need at least 4 swing points to form a Bat pattern
        ...
        for x_idx in swing_lows:
            ...
            for a_idx in [h for h in swing_highs if h > x_idx]:
                ...
                for b_idx in [l for l in swing_lows if l > a_idx]:
                    ...
                    # Check if AB is within 0.382-0.5 retracement of XA
                    ...
                    for c_idx in [h for h in swing_highs if h > b_idx]:
                        ...
                        for d_idx in [l for l in swing_lows if l > c_idx]:
                            ...
                            # Check if D is approximately 0.886 retracement of XA
                            xd_ratio = xd_diff / xa_diff
                            if not self._is_within_tolerance(xd_ratio, self.xd_ratio):
                                continue
                            
                            # All conditions met, we have a bullish Bat pattern
                            pattern = {
                                'start_idx': x_idx,
                                'end_idx': d_idx,
                                'pattern_type': 'bat',
    ...
    def _find_bearish_bat(self, data: pd.DataFrame, swing_highs: List[int], swing_lows: List[int]) -> List[Dict[str, Any]]:
        """
        Find bearish Bat patterns in the data.
                ...
                for b_idx in [h for h in swing_highs if h > a_idx]:
                    ...
                    for c_idx in [l for l in swing_lows if l > b_idx]:
                        ...
                        for d_idx in [h for h in swing_highs if h > c_idx]:
                            ...
                            # All conditions met, we have a bearish Bat pattern
                            pattern = {
                                'start_idx': x_idx,
                                'end_idx': d_idx,
                                'pattern_type': 'bat',
    ...
    def _calculate_pattern_strength(self, ab_ratio: float, ab_target: float,
        ...
        # XD ratio is most important for Bat pattern
