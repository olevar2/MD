...
def plot_chart_with_patterns(
    ...
    """
    # Convert data to mplfinance format
...
def plot_harmonic_pattern(
    data: pd.DataFrame,
    pattern: Dict[str, Any],
    title: str = "Harmonic Pattern",
    ...
    # Create a slice of data that includes the pattern
    ...
    # Adjust indices to match the slice
    x_idx -= start_idx
    a_idx -= start_idx
    b_idx -= start_idx
    c_idx -= start_idx
...
def plot_candlestick_pattern(
    data: pd.DataFrame,
    pattern: Dict[str, Any],
    lookback: int = 5,
    lookahead: int = 5,
    title: str = "Candlestick Pattern",
    figsize: Tuple[int, int] = (10, 6),
    save_path: Optional[str] = None
) -> plt.Figure:
    """
    ...
    # Adjust index to match the slice
    ...
    if pattern_type == 'doji':
        ...
        # Highlight Hammer/Hanging Man candle
        ...
        # Highlight Engulfing candles (two candles)
        color = 'green' if direction == 'bullish' else 'red'
...
def _add_patterns_to_chart(ax: plt.Axes, data: pd.DataFrame, patterns: Dict[str, List[Dict[str, Any]]]) -> None:
    """
    ...
    for pattern_type, pattern_list in patterns.items():
        for pattern in pattern_list:
            ...
            if 'start_idx' in pattern and 'end_idx' in pattern:
                ...
                # Draw rectangle around the pattern
                ...
                ax.annotate(
                    f"{pattern_type.capitalize()}{direction_str}",
                    (start_date, start_price),
                    xytext=(0, -15),
...
def _add_pattern_legend(ax: plt.Axes, patterns: Dict[str, List[Dict[str, Any]]]) -> None:
    """
...
def _add_fibonacci_ratios(
    ...
    ax.annotate(
        f"AB/XA: {ab_ratio:.3f}",
        ((a_date + b_date) / 2, (a_price + b_price) / 2),
        xytext=(10, 10),
        textcoords='offset points',
    ...
    ax.annotate(
        f"BC/AB: {bc_ratio:.3f}",
        ((b_date + c_date) / 2, (b_price + c_price) / 2),
        xytext=(10, -20),
        textcoords='offset points',
        bbox=dict(boxstyle="round,pad=0.3", fc="yellow", alpha=0.5)
    )
    ...
    ax.annotate(
        f"CD/BC: {cd_ratio:.3f}",
        ((c_date + d_date) / 2, (c_price + d_price) / 2),
        xytext=(10, 10),
        textcoords='offset points',
        bbox=dict(boxstyle="round,pad=0.3", fc="yellow", alpha=0.5)
    )
    ...
    ax.annotate(
        f"XD/XA: {xd_ratio:.3f}",
        ((x_date + d_date) / 2, (x_price + d_price) / 2),
        xytext=(-40, -20),
        textcoords='offset points',
        bbox=dict(boxstyle="round,pad=0.3", fc="yellow", alpha=0.5)
    )
...
def _highlight_candle(ax: plt.Axes, data: pd.DataFrame, idx: int, color: str, label: str) -> None:
    """
    ...
    width = 0.8  # Width of the highlight
    rect = patches.Rectangle(
        (date - width/2, low_price),
        width,
    ...
    # Add arrow pointing to the candle
    ax.annotate(
        label,
        (date, high_price),
        xytext=(0, 20),
