    ...
    category = "price"
      def __init__(
        ...
        **kwargs
        ...
        result[self.name_close] = (result['open'] + result['high'] + 
                                   result['low'] + result['close']) / 4
        
        # Initialize Heikin-Ashi open with the first available regular open and close
        result[self.name_open] = result['open'].copy()
        
        # Calculate subsequent Heikin-Ashi open values
        for i in range(1, len(result)):
            result.loc[result.index[i], self.name_open] = (
                (result.loc[result.index[i-1], self.name_open] + 
                 result.loc[result.index[i-1], self.name_close]) / 2
            )
        
        # Calculate Heikin-Ashi high and low
        result[self.name_high] = result[['high', self.name_open, self.name_close]].max(axis=1)
        result[self.name_low] = result[['low', self.name_open, self.name_close]].min(axis=1)
        
        # Enhance visual representation of trends through trend identification
        result[self.name_trend] = 0  # 0 = neutral
        
        # Bullish candles (close > open)
        result.loc[result[self.name_close] > result[self.name_open], self.name_trend] = 1
        
        # Bearish candles (close < open)
        result.loc[result[self.name_close] < result[self.name_open], self.name_trend] = -1
        
        # Integrate trend strength analysis
        # Calculate the body size as a percentage of the full range
        body_size = abs(result[self.name_close] - result[self.name_open])
        candle_range = result[self.name_high] - result[self.name_low]
        result[self.name_strength] = body_size / candle_range * 100
        
        # Identify strong trends based on threshold
        consecutive_trend = result[self.name_trend].rolling(window=4).sum()
        result.loc[consecutive_trend >= self.trend_threshold, self.name_trend] = 2  # Strong bullish
        result.loc[consecutive_trend <= -self.trend_threshold, self.name_trend] = -2  # Strong bearish
