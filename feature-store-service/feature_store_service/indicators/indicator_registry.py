...
class IndicatorRegistry:
    ...
    def register_indicator(self, indicator_class: Type[BaseIndicator]) ->None:
        ...
        if indicator_id in self._indicators:
            logger.warning(
                f"Indicator '{indicator_id}' already registered, overwriting")
        self._indicators[indicator_id] = indicator_class
        logger.debug(f"Registered indicator '{indicator_id}'")

    def register_indicators(self, indicator_classes: List[Type[BaseIndicator]]
        ) ->None:
        """
    ...
    def get_indicators_by_category(self, category: str) ->Dict[str, Type[
        BaseIndicator]]:
        """
        ...
        """
        return {indicator_id: indicator_class for indicator_id,
            indicator_class in self._indicators.items() if indicator_class.
            category == category}
    ...
    @with_exception_handling
    def create_indicator(self, indicator_id: str, **kwargs) ->Optional[
        BaseIndicator]:
        """
        ...
        if indicator_class:
            try:
                ...
                logger.error(
                    f"Error creating indicator '{indicator_id}': {str(e)}")
        return None

    @with_exception_handling
    def validate_parameters(self, indicator_id: str, params: Dict[str, Any]
        ) ->Dict[str, Any]:
        """
