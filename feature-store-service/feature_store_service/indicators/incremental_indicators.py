...
class SMAState(IndicatorState):
    ...
    def update(self, new_data: Dict[str, float]) -> Optional[float]:
        ...
        # If we have more values than our window, remove the oldest
...
class EMAState(IndicatorState):
    ...
    def __init__(self, window_size: int, price_type: str = 'close', smoothing: float = 2.0):
        super().__init__(
            name=f"EMA_{window_size}",
            parameters={"window_size": window_size, "price_type": price_type, "smoothing": smoothing}
        )
        self.window_size = window_size
        self.price_type = price_type
        ...
        self.count = 0
        self.values = []  # Store values for SMA calculation during initialization
    ...
    def update(self, new_data: Dict[str, float]) -> Optional[float]:
        ...
        # Initialize EMA with SMA value for the first window_size points
        ...
        # Apply EMA formula: current = price * multiplier + previous * (1 - multiplier)
        self.last_value = price * self.multiplier + self.last_value * (1 - self.multiplier)
        return self.last_value
    ...
    def reset(self) -> None:
        ...
        if not self.is_ready and self.count == self.window_size:
            self.is_ready = True
            self.last_value = price  # Simplified - in reality would use SMA
            return self.last_value
...
class RSIState(IndicatorState):
    ...
    def update(self, new_data: Dict[str, float]) -> Optional[float]:
        ...
        # Need at least one previous price to calculate change
        ...
        # First RSI value uses simple averages
...
class MACDState(IndicatorState):
    ...
    def update(self, new_data: Dict[str, float]) -> Dict[str, Optional[float]]:
        ...
        # Both EMAs must be ready to calculate MACD
        if fast_value is not None and slow_value is not None:
            macd_line = fast_value - slow_value
            result["macd_line"] = macd_line
            
            # Update signal line EMA with the MACD value
        ...
        # Store components in last_value for reference
...
class BollingerBandsState(IndicatorState):
    ...
    def update(self, new_data: Dict[str, float]) -> Optional[Dict[str, float]]:
        ...
        # Only calculate Bollinger Bands if we have enough data points
        if len(self.prices) == self.window_size:
            self.is_ready = True
            ...
            variance = (self.sum_sq - (self.sum ** 2) / self.window_size) / self.window_size
            std_dev = max(0.0001, np.sqrt(variance))  # Avoid division by zero
...
class ATRState(IndicatorState):
    ...
    def update(self, new_data: Dict[str, float]) -> Optional[float]:
        # Need open, high, low, close for ATR calculation
        required_fields = ['high', 'low', 'close']
        for field in required_fields:
            if field not in new_data:
                logging.warning(f"Required field {field} not found in data: {new_data}")
                return self.last_value
        ...
        if self.prev_close is not None:
            tr = max(
                ...
                abs(high - self.prev_close),  # Current high to previous close
                abs(low - self.prev_close)    # Current low to previous close
            )
        else:
            tr = high - low  # First bar, just use high-low range
        ...
        if self.count >= self.window_size:
            if self.atr is None:
                ...
                self.atr = (self.atr * (self.window_size - 1) + tr) / self.window_size
...
class IncrementalIndicatorManager:
    ...
    def add_indicator(self, indicator: IndicatorState) -> None:
        ...
        self.logger.info(f"Added indicator: {indicator.name} with parameters {indicator.parameters}")
    ...
    def update_all(self, new_data: Dict[str, float]) -> Dict[str, Any]:
        ...
        for name, indicator in self.indicators.items():
            try:
                ...
                if value is not None:  # Only include ready indicators
                    results[name] = value
