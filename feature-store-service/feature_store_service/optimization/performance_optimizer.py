...
class PerformanceProfile:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'points_per_second': self.data_points / self.execution_time if self.execution_time > 0 else 0,
...
class PerformanceOptimizer:
    ...
    def analyze_performance(
        ...
        with self._lock:
            ...
            points_per_second = [
                p.data_points / p.execution_time if p.execution_time > 0 else 0
                for p in profiles
            ]
            ...
            return {
                ...
                'throughput': {
                    'mean': np.mean(points_per_second),
                    'median': np.median(points_per_second),
                    'std': np.std(points_per_second)
                },
                ...
                'bottlenecks': self._detect_bottlenecks(profiles)
            }
        ...
        for param, impact in param_impact.items():
            if param in optimized:
                if impact['correlation'] > 0.5:
                    # Parameter increases correlate with worse performance
                    optimized[param] = max(
                        impact['optimal_range'][0],
                        optimized[param] * 0.8
                    )
                elif impact['correlation'] < -0.5:
                    # Parameter increases correlate with better performance
                    optimized[param] = min(
                        impact['optimal_range'][1],
                        optimized[param] * 1.2
                    )
    ...
    def _analyze_parameter_impact(
        ...
        for param in param_names:
            ...
            optimal_values = [x[0] for x in sorted_data[:max(3, len(sorted_data)//3)]]
    ...
    def _detect_bottlenecks(
        ...
        if std_time > mean_time * 0.5:
            bottlenecks.append({
                ...
                'severity': 'high',
                'description': 'High variability in execution times'
            })
        ...
        if memory_trend > 0:
            bottlenecks.append({
                ...
                'metric': 'memory',
                'severity': 'medium',
                'description': 'Increasing memory usage trend'
            })
    ...
    def end_calculation(
        ...
        with self._lock:
            ...
            profile = PerformanceProfile(
                ...
                execution_time=end_time - info['start_time'],
                cpu_time=end_time - info['start_time'],  # Approximation
                memory_used=end_memory - info['start_memory'],
