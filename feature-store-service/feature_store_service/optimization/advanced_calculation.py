...
class IncrementalCalculator:
    ...
    def finalize_calculation(self, calc_id: str) -> Any:
        ...
        # Finalize calculation if supported
        if (finalize_func := self.functions[func_name].get('finalize')) is not None:
...
class SmartCache:
    ...
    def _generate_key(self, func_name: str, args: tuple, kwargs: dict) -> str:
        ...
        for k, v in sorted(kwargs.items()):
            if isinstance(v, (pd.DataFrame, pd.Series)):
                v_hash = hashlib.md5(pd.util.hash_pandas_object(v).values).hexdigest()
    ...
    def get(self, func_name: str, args: tuple, kwargs: dict) -> Tuple[bool, Any]:
        ...
        if key in self.cache:
            ...
            if time.time() - entry['timestamp'] <= self.ttl_seconds:
                # Cache hit
                self.hits += 1
                # Update timestamp to extend TTL (LRU-like behavior)
    ...
    def set(self, func_name: str, args: tuple, kwargs: dict, value: Any):
        ...
        try:
            ...
            # If value can't be pickled, don't cache it
            logger.warning(f"Could not cache value for {func_name} (not serializable)")
            return
        
        # Check if value is too large
        if size_bytes > self.max_size_bytes * 0.25:
            logger.warning(f"Value for {func_name} is too large to cache ({size_bytes / 1024 / 1024:.2f} MB)")
            return
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            'usage_percent': (self.size_bytes / self.max_size_bytes) * 100 if self.max_size_bytes > 0 else 0,
...
class PreAggregator:
    ...
    def register_aggregation(self, name: str, source_timeframe: str, 
        ...
        logger.info(f"Registered aggregation: {name} ({source_timeframe} → {', '.join(target_timeframes)})")
    ...
    def aggregate(self, name: str, data: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        # Perform aggregation for each target timeframe
    ...
    def get_aggregated_data(self, name: str, timeframe: str) -> Optional[pd.DataFrame]:
        """
...
class LazyCalculator:
    ...
    def register_computation(self, name: str, compute_func: Callable, 
        ...
        logger.debug(f"Registered computation: {name} (depends on: {dependencies})")
...
# Initialize module-level instances for global access
...
def register_incremental_ma(name: str = "moving_average"):
    ...
    def update_ma(state: dict, new_value: float) -> float:
        ...
        # Remove old value if window size exceeded
    ...
    incremental_calculator.register_incremental_function(
        name, init_ma, update_ma
    )
...
def register_incremental_rsi(name: str = "rsi"):
    ...
    def update_rsi(state: dict, new_value: float) -> float:
        ...
        # We need at least window+1 values to calculate RSI
        if len(values) <= window:
            return 50.0  # Default value during initialization
        
        # Remove oldest values if exceeding window
        ...
        if not state['is_initialized'] and len(gains) == window:
            ...
            state['avg_gain'] = (state['avg_gain'] * (window - 1) + gains[-1]) / window
            state['avg_loss'] = (state['avg_loss'] * (window - 1) + losses[-1]) / window
        else:
            return 50.0
...
def register_common_aggregations():
    ...
    # Standard OHLCV aggregation functions
    ohlcv_agg_funcs = {
        'open': 'first',
        'high': 'max',
        'low': 'min',
    ...
    # Register for different timeframe combinations
    pre_aggregator.register_aggregation(
        name="ohlcv_1m",
        source_timeframe="1m",
    ...
    pre_aggregator.register_aggregation(
        name="ohlcv_5m",
        source_timeframe="5m",
        target_timeframes=["15m", "30m", "1h", "4h", "1d"],
        aggregation_funcs=ohlcv_agg_funcs
    )
