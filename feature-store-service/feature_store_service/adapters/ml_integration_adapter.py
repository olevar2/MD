...
from common_lib.ml.feature_interfaces import (
    IMLFeatureConsumer, FeatureType, FeatureScope, SelectionMethod
)
...
class MLFeatureConsumerAdapter(IMLFeatureConsumer):
    ...
    def get_required_features(self, model_id: str = None) -> List[Dict[str, Any]]:
        ...
        if self.consumer:
            try:
                # Try to use the wrapped consumer if available
        ...
        # Fallback to default features if no consumer available
        default_features = [
            {
            ...
            },
            {
                ...
                "params": {"window": 5},
                "scope": FeatureScope.VOLUME,
                "is_sequential": False,
                "lookback_periods": 1
            }
        ]

        # Cache the features if model_id is provided
        if model_id:
    ...
    def prepare_model_inputs(
        ...
        if self.consumer:
            try:
                # Try to use the wrapped consumer if available
                return self.consumer.prepare_model_inputs(
                    features=features,
                    model_id=model_id,
        ...
        # Fallback to simple preparation if no consumer available
        X = features.copy()
        y = None
    ...
    def get_feature_importance_feedback(
        ...
        if self.consumer:
            try:
                # Try to use the wrapped consumer if available
                return self.consumer.get_feature_importance_feedback(
                    model_id=model_id,
                    features=features
                )
            except Exception as e:
                logger.warning(f"Error getting feature importance feedback: {str(e)}")

        # Check if we have cached importance for this model
        ...
        # Fallback to random importance if no consumer available
