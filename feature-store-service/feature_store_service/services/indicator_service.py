...
class IndicatorService:
    ...
    def __init__(self):
        ...
        self._calculation_cache = {}  # Simple in-memory cache
    ...
    def calculate_indicator(
        ...
        **indicator_params
        ...
        try:
            ...
            indicator = self._registry.create_indicator(indicator_name, **indicator_params)
            
            # Calculate the indicator
            self.logger.info(f"Calculating {indicator_name} with parameters {indicator_params}")
            result = indicator.calculate(data)
            
            return result
            
        except KeyError:
            self.logger.error(f"Indicator '{indicator_name}' not found in registry")
            raise
        except ValueError as e:
            self.logger.error(f"Error calculating {indicator_name}: {str(e)}")
            raise
        except Exception as e:
            self.logger.error(f"Unexpected error calculating {indicator_name}: {str(e)}")
            raise
    ...
    def calculate_multiple_indicators(
        ...
        for indicator_config in indicators:
            ...
            try:
                # Calculate each indicator, building on the results of previous ones
                result = self.calculate_indicator(result, name, **params)
            except Exception as e:
                self.logger.error(f"Error calculating {name}: {str(e)}")
                # Continue with the next indicator instead of failing completely
                continue
    ...
    def transform_time_series(self, request: TimeSeriesTransformRequest) -> TimeSeriesTransformResponse:
        ...
        try:
            # Get the data for the requested instruments
            features = []
            
            for instrument in request.instruments:
                for timeframe in request.timeframes:
                    # Process the transformation using the computation engine
                    transformed_data = self._computation_engine.transform_time_series(
                    ...
                    feature_id = f"{request.transformation_type}_{instrument}_{timeframe}"
                    self._repository.store_feature_data(
                    ...
                    feature_metadata = FeatureMetadataResponse(
                        feature_id=feature_id,
                        name=f"{request.transformation_type} of {instrument}",
                        instrument=instrument,
                        timeframe=timeframe,
                        value_type="float",
                        description=f"{request.transformation_type} transformation of {instrument} with parameters {request.parameters}",
                        parameters=request.parameters
                    )
                    
                    features.append(feature_metadata)
            
            return TimeSeriesTransformResponse(
                status="success",
                message=f"Successfully transformed time series data for {len(features)} features",
                features=features
            )
    ...
    def get_feature_vectors(self, request: FeatureVectorRequest) -> FeatureResponse:
        ...
        """
        self.logger.info(f"Retrieving feature vectors for {len(request.feature_ids)} features")
        ...
        try:
            ...
            # Retrieve the feature vectors from the repository
            feature_data = self._repository.query_features(query)
            
            # Format the feature vectors for the response
            feature_vectors = []
            ...
            return FeatureResponse(
                status="success",
                message=f"Successfully retrieved {len(feature_vectors)} feature vectors",
    ...
    def get_feature_metadata(self, feature_ids: List[str]) -> List[FeatureMetadataResponse]:
        ...
        try:
            ...
            for feature_id in feature_ids:
                feature_metadata = self._repository.get_feature_metadata(feature_id)
                if feature_metadata:
                    metadata_response = FeatureMetadataResponse(
                        feature_id=feature_id,
                        name=feature_metadata.get('name', feature_id),
                        instrument=feature_metadata.get('instrument', ''),
                        timeframe=feature_metadata.get('timeframe', ''),
                        value_type=feature_metadata.get('value_type', 'float'),
                        description=feature_metadata.get('description', ''),
                        parameters=feature_metadata.get('parameters', {})
                    )
    ...
    def calculate_time_series_transform(
        self,
        request: TimeSeriesTransformRequest
    ) -> TimeSeriesTransformResponse:
        """
        ...
        """
        try:
            self.logger.info(f"Processing time series transform request for {request.transformation_type}")
            
            # Use the computation engine to perform the transformation
            result_features = self._computation_engine.transform_time_series(
                instruments=request.instruments,
                timeframes=request.timeframes,
                transform_type=request.transformation_type,
                parameters=request.parameters,
                start_time=request.start_time,
                end_time=request.end_time
            )
            ...
            return TimeSeriesTransformResponse(
                status="success",
                message=f"Successfully calculated {request.transformation_type} transform",
                features=feature_metadata_list
            )
            
        except ValueError as e:
            self.logger.error(f"Invalid parameters for transformation: {str(e)}")
            return TimeSeriesTransformResponse(
                status="error",
            ...
            self.logger.error(f"Error calculating time series transform: {str(e)}")
            return TimeSeriesTransformResponse(
                status="error",
    ...
    def list_available_features(self) -> List[FeatureMetadataResponse]:
        ...
        """
        try:
            # Get all feature metadata from the repository
