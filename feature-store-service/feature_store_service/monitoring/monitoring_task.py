...
class MonitoringTask:
    ...
    def __init__(
        ...
        self.report_dir = report_dir or os.path.join(os.getcwd(), 'monitoring_reports')
    ...
    async def _collect_and_report(self) -> None:
        ...
        # Get monitoring data
        monitoring_report = self.indicator_service.monitoring.get_monitoring_report(
            time_window=timedelta(minutes=self.monitoring_interval)
        )
        ...
        dependency_info = self.indicator_service.dependency_tracker.export_dependencies()
    ...
    def _check_alerts(self, report: Dict[str, Any]) -> None:
        ...
        # Check error rates
        for indicator, stats in report['monitoring']['errors'].get('calculation_errors', {}).items():
            if stats['count'] > 10:  # More than 10 errors in monitoring interval
                logger.warning(f"High error rate for indicator {indicator}: {stats['count']} errors")
                
        # Check performance
        for indicator, stats in report['monitoring']['performance'].items():
            if stats['success_rate'] < 0.95:  # Less than 95% success rate
                logger.warning(f"Low success rate for indicator {indicator}: {stats['success_rate']:.2%}")
            
            if stats['avg_duration'] > 1.0:  # Average duration > 1 second
                logger.warning(f"High calculation time for indicator {indicator}: {stats['avg_duration']:.2f}s")
