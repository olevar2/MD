...
# Get scheduler instance (will be initialized in main.py)
...
async def create_schedule(request: ScheduleRequest):
    ...
    try:
        # Process the indicators and params
        indicator_specs = []
        ...
        # Create the job
        job_info = await scheduler.add_job(
            job_id=job_id,
            name=request.name,
...
async def update_schedule(job_id: str, request: ScheduleRequest):
    ...
    try:
        ...
        # Update the job
        job_info = await scheduler.update_job(
            job_id=job_id,
            name=request.name,
            description=request.description,
            symbols=request.symbols,
...
async def run_schedule_now(job_id: str, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Trigger the job to run in the background
        background_tasks.add_task(scheduler.run_job_now, job_id)
        
        return {
            "message": f"Job {job_id} triggered to run",
...
def _get_interval_seconds(interval: ScheduleInterval) -> int:
