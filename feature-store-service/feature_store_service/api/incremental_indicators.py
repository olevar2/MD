...
router = APIRouter(prefix='/api/v1/incremental-indicators', tags=[
    'incremental-indicators'], responses={(404): {'description': 'Not found'}})
...
class IndicatorParams(BaseModel):
    ...
    indicator_type: str = Field(..., description=
        "Type of indicator (e.g., 'SMA', 'EMA', 'MACD')")
    params: Optional[Dict[str, Any]] = Field(None, description=
        'Parameters for the indicator')
    lookback_days: int = Field(60, description=
        'Number of days of history to use for initialization')
...
class IndicatorInfo(BaseModel):
    ...
    params: Dict[str, Any] = Field(..., description=
        'Parameters for the indicator')
    is_initialized: bool = Field(..., description=
        'Whether the indicator is initialized')
    last_updated: Optional[datetime] = Field(None, description=
        'Timestamp of the last update')


@router.post('/initialize', response_model=Dict[str, bool], summary=
    'Initialize an incremental indicator')
@async_with_exception_handling
async def initialize_indicator(params: IndicatorParams, indicator_service:
    IncrementalIndicatorService=Depends(get_indicator_manager)):
    """
    ...
    """
    try:
        indicator = await indicator_service.get_or_initialize_indicator(symbol
            =params.symbol, timeframe=params.timeframe, indicator_type=
            params.indicator_type, params=params.params, lookback_days=
            params.lookback_days)
        if indicator and indicator.is_initialized:
            return {'success': True, 'message':
                f'Initialized {params.indicator_type} for {params.symbol} {params.timeframe}'
                }
        else:
            raise HTTPException(status_code=500, detail=
                f'Failed to initialize {params.indicator_type} for {params.symbol} {params.timeframe}'
                )
    except Exception as e:
        logger.error(f'Error initializing indicator: {str(e)}')
        raise HTTPException(status_code=500, detail=
            f'Error initializing indicator: {str(e)}')


@router.post('/update', response_model=IndicatorResponse, summary=
    'Update an incremental indicator')
@async_with_exception_handling
async def update_indicator(data_point: DataPoint, indicator_type: str=Query
    (..., description='Type of indicator to update'), params: Optional[Dict
    [str, Any]]=None, indicator_service: IncrementalIndicatorService=
    Depends(get_indicator_manager)):
    """
    ...
    """
    try:
        data_dict = {'timestamp': data_point.timestamp, 'open': data_point.
            open, 'high': data_point.high, 'low': data_point.low, 'close':
            data_point.close, 'volume': data_point.volume}
        result = await indicator_service.update_indicator(symbol=data_point
            .symbol, timeframe=data_point.timeframe, indicator_type=
            indicator_type, new_data_point=data_dict, params=params)
        if result:
            return IndicatorResponse(symbol=data_point.symbol, timeframe=
                data_point.timeframe, indicator_type=indicator_type, values
                =result, timestamp=data_point.timestamp)
        else:
            raise HTTPException(status_code=500, detail=
                f'Failed to update {indicator_type} for {data_point.symbol} {data_point.timeframe}'
                )
    except Exception as e:
        logger.error(f'Error updating indicator: {str(e)}')
        raise HTTPException(status_code=500, detail=
            f'Error updating indicator: {str(e)}')


@router.post('/update-all', response_model=Dict[str, Dict[str, float]],
    summary='Update all indicators for a symbol')
@async_with_exception_handling
async def update_all_indicators(data_point: DataPoint, indicator_service:
    IncrementalIndicatorService=Depends(get_indicator_manager)):
    """
    ...
    try:
        ...
        result = await indicator_service.update_all_indicators_for_symbol(
            symbol=data_point.symbol, timeframe=data_point.timeframe,
            new_data_point=data_dict)
        return result
    except Exception as e:
        logger.error(f'Error updating indicators: {str(e)}')
        raise HTTPException(status_code=500, detail=
            f'Error updating indicators: {str(e)}')


@router.get('/active', response_model=Dict[str, IndicatorInfo], summary=
    'Get all active indicators')
@async_with_exception_handling
async def get_active_indicators(symbol: Optional[str]=Query(None,
    description='Filter by symbol'), timeframe: Optional[str]=Query(None,
    description='Filter by timeframe'), indicator_type: Optional[str]=Query
    (None, description='Filter by indicator type'), indicator_service:
    IncrementalIndicatorService=Depends(get_indicator_manager)):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=
            f'Error getting active indicators: {str(e)}')


@router.post('/load-states', response_model=Dict[str, int], summary=
    'Load saved indicator states')
@async_with_exception_handling
async def load_saved_states(indicator_service: IncrementalIndicatorService=
    Depends(get_indicator_manager)):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=
            f'Error loading indicator states: {str(e)}')
