...
from feature_store_service.services.reconciliation_service import ReconciliationService
...
class ReconciliationRequest(BaseModel):
    ...
    features: List[str] = Field(..., description="List of features to reconcile")
    start_time: datetime = Field(..., description="Start time for data reconciliation")
    end_time: datetime = Field(..., description="End time for data reconciliation")
...
class FeatureVersionReconciliationRequest(BaseModel):
    ...
    feature_name: str = Field(..., description="Name of the feature to reconcile")
...
@router.post("/feature-data", response_model=ReconciliationResponse, responses={
...
async def reconcile_feature_data(
    ...
    try:
        ...
        logger.info(
            f"Received feature data reconciliation request for symbol {request.symbol}, "
            f"features {request.features}, strategy {request.strategy.name}"
        )

        result = await reconciliation_service.reconcile_feature_data(
        ...
        logger.info(
            f"Completed feature data reconciliation for symbol {request.symbol}, "
        ...
        logger.error(f"Validation error during feature data reconciliation: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={
...
@router.post("/feature-version", response_model=ReconciliationResponse, responses={
    400: {"model": ErrorResponse, "description": "Bad Request"},
    401: {"model": ErrorResponse, "description": "Unauthorized"},
    500: {"model": ErrorResponse, "description": "Internal Server Error"}
})
async def reconcile_feature_version(
    request: FeatureVersionReconciliationRequest = Body(...),
    api_key: str = Depends(get_api_key)
) -> Dict[str, Any]:
    """
    ...
    try:
        # Log the reconciliation request
        logger.info(
            f"Received feature version reconciliation request for feature {request.feature_name}, "
        ...
        # Log the reconciliation result
        logger.info(
            f"Completed feature version reconciliation for feature {request.feature_name}, "
            f"reconciliation ID: {result.reconciliation_id}, "
            f"status: {result.status.name}, "
            f"discrepancies: {result.discrepancy_count}, "
            f"resolutions: {result.resolution_count}"
        ...
        logger.error(f"Validation error during feature version reconciliation: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={
                "detail": f"Validation error: {str(e)}",
