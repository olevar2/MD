...
class IndicatorLogger:
    ...
    def _setup_handlers(self):
        ...
        # File handlers
        handlers = {
            'validation': (logging.INFO, 'validation.log'),
            'error': (logging.ERROR, 'error.log'),
            'performance': (logging.INFO, 'performance.log')
        }
    ...
    def log_validation(
        ...
        details = details or {}
        self.logger.info(
            f"Validation: {indicator_name} - {validation_type}",
            extra={
    ...
    def log_error(
        ...
        details = details or {}
        self.logger.error(
            f"Error: {indicator_name} - {message}",
            extra={
                'indicator': indicator_name,
...
class IndicatorReport:
    ...
    def generate_validation_report(
        ...
        return {
            ...
            'by_type': validations_by_type,
            'by_indicator': validations_by_indicator,
            'time_range': {
    ...
    def generate_error_report(
        ...
        for entry in entries:
            ...
            # Count by indicator
            errors_by_indicator[indicator] = errors_by_indicator.get(indicator, 0) + 1
            
        return {
            'total_errors': total_errors,
            'by_type': errors_by_type,
            'by_indicator': errors_by_indicator,
            'time_range': {
                'start': min(e['timestamp'] for e in entries),
                'end': max(e['timestamp'] for e in entries)
            }
    ...
    def generate_performance_report(
        ...
        for entry in entries:
            ...
            if indicator not in performance_by_indicator:
                performance_by_indicator[indicator] = {
                    'count': 0,
                    'total_time': 0.0,
                    'total_points': 0,
                    'min_time': float('inf'),
            ...
            stats['min_points_per_sec'] = min(stats['min_points_per_sec'], points_per_sec)
            stats['max_points_per_sec'] = max(stats['max_points_per_sec'], points_per_sec)
            
        # Calculate averages
        for stats in performance_by_indicator.values():
            stats['avg_time'] = stats['total_time'] / stats['count']
            stats['avg_points_per_sec'] = stats['total_points'] / stats['total_time']
            
        return {
            'total_executions': len(entries),
            'by_indicator': performance_by_indicator,
            'time_range': {
                'start': min(e['timestamp'] for e in entries),
                'end': max(e['timestamp'] for e in entries)
            }
    ...
    def generate_summary_report(
        ...
        """
        validation_report = self.generate_validation_report(start_time, end_time)
        error_report = self.generate_error_report(start_time, end_time)
        performance_report = self.generate_performance_report(start_time, end_time)
