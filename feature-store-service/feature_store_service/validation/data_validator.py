...
class DataValidationService:
    ...
    def _validate_ohlcv(self, data: pd.DataFrame) -> ValidationResult:
        ...
        for col, expected_types in type_checks.items():
            ...
            if str(data[col].dtype) not in expected_types:
                type_issues.append(f"{col} (expected {expected_types}, got {data[col].dtype})")
        ...
        if price_issues:
            return ValidationResult(
                is_valid=False,
                message="Price relationship validation failed",
        ...
        return ValidationResult(
            is_valid=True,
            message="OHLCV validation passed",
            severity=ValidationSeverity.INFO
        )

    def _validate_indicator_input(self, data: pd.DataFrame, params: Optional[Dict[str, Any]] = None) -> ValidationResult:
        """
            ...
            for param, value in params.items():
                if isinstance(value, (int, float)):
                    ...
                    try:
                        ...
                        param_issues.append(f"{param} has invalid time period format")

            if param_issues:
                return ValidationResult(
                    is_valid=False,
                    message="Invalid indicator parameters",
                    details={"param_issues": param_issues},
                    severity=ValidationSeverity.ERROR
                )

        return ValidationResult(
            is_valid=True,
            message="Indicator input validation passed",
            severity=ValidationSeverity.INFO
        )
    ...
    def _validate_time_series(self, data: pd.DataFrame) -> ValidationResult:
        ...
        # Get the most common time difference (assumed to be the expected interval)
        expected_interval = time_diffs.mode()[0]
        
        # Find gaps larger than 2x the expected interval
        gaps = time_diffs[time_diffs > 2 * expected_interval]
        
        if not gaps.empty:
            return ValidationResult(
                is_valid=False,
                message="Found gaps in time series",
                details={
                    "gap_count": len(gaps),
        ...
        return ValidationResult(
            is_valid=True,
            message="Time series validation passed",
            severity=ValidationSeverity.INFO
        )

    def _validate_calculation(self, result: pd.DataFrame, expected_properties: Optional[Dict[str, Any]] = None) -> ValidationResult:
        """
        ...
        if nan_cols:
            validation_issues.append({
                "issue": "nan_values",
                "details": f"Found NaN values in columns: {nan_cols}"
            })
        ...
        if inf_cols:
            validation_issues.append({
                ...
                "details": f"Found infinity values in columns: {inf_cols}"
            })

        # Validate expected properties if provided
        if expected_properties:
            for prop, expected in expected_properties.items():
                if prop == "columns" and not all(col in result.columns for col in expected):
                    ...
                    validation_issues.append({
                        "issue": "wrong_length",
                        "details": f"Expected length {expected}, got {len(result)}"
                    })
    ...
    def _handle_outliers(self, data: pd.DataFrame, columns: List[str], threshold: float = 3.0) -> pd.DataFrame:
    ...
    def _handle_calculation_error(self, error: Exception, indicator_name: str) -> None:
        """Log calculation errors and provide recovery suggestions."""
        error_msg = f"Calculation error in {indicator_name}: {str(error)}"
        # Here you would typically log the error and potentially trigger alerts
        raise ValueError(error_msg)

    def validate(
        self,
        validation_type: Union[str, IndicatorValidationType],
        data: pd.DataFrame,
        **kwargs
    ) -> ValidationResult:
        """
