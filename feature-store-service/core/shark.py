"""
Shark Pattern Detector Module.

This module provides the SharkPatternDetector class for detecting Shark harmonic patterns.
"""
...
class SharkPatternDetector(BasePatternDetector):
    """
    Shark Pattern Detector.
    
    This class detects Shark harmonic patterns in price data.
    """
    ...
    def detect(self, data: pd.DataFrame, pivot_indices: pd.Index) -> pd.DataFrame:
        ...
        for i in range(len(pivot_indices) - 4):
            ...
            if matches_template:
                ...
                # For Shark pattern, target is often a 0.5 retracement of the CD leg
                target = d_val + (0.5 * cd) * (-1 if is_bullish else 1)
