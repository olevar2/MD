...
from ..indicators.incremental_indicators import IncrementalIndicatorManager, SMAState, EMAState, RSIState, MACDState, BollingerBandsState, ATRState
...
class RealTimeFeatureProcessor:
    ...
    def _configure_indicators(self, indicator_configs: List[Dict[str, Any]]
        ) ->None:
        """
        ...
        for config in indicator_configs:
            indicator_type = config_manager.get('type', '').lower()
            if indicator_type == 'sma':
                self.add_sma_indicator(window_size=config.get('window_size',
                    20), price_type=config_manager.get('price_type', 'close'))
            elif indicator_type == 'ema':
                self.add_ema_indicator(window_size=config.get('window_size',
                    20), price_type=config_manager.get('price_type', 'close'),
                    smoothing=config_manager.get('smoothing', 2.0))
            elif indicator_type == 'rsi':
                self.add_rsi_indicator(window_size=config.get('window_size',
                    14), price_type=config_manager.get('price_type', 'close'))
            elif indicator_type == 'macd':
                self.add_macd_indicator(fast_period=config.get(
                    'fast_period', 12), slow_period=config.get(
                    'slow_period', 26), signal_period=config.get(
                    'signal_period', 9), price_type=config.get('price_type',
                    'close'))
            elif indicator_type == 'bollinger_bands' or indicator_type == 'bbands':
                self.add_bollinger_bands_indicator(window_size=config.get(
                    'window_size', 20), num_std=config_manager.get('num_std', 2.0),
                    price_type=config_manager.get('price_type', 'close'))
            elif indicator_type == 'atr':
                self.add_atr_indicator(window_size=config.get('window_size',
                    14))
            else:
                self.logger.warning(f'Unknown indicator type: {indicator_type}'
                    )

    def add_sma_indicator(self, window_size: int, price_type: str='close'
        ) ->None:
        """
    ...
    def add_ema_indicator(self, window_size: int, price_type: str='close',
        smoothing: float=2.0) ->None:
        """
        ...
        """
        indicator = EMAState(window_size=window_size, price_type=price_type,
            smoothing=smoothing)
        self.indicator_manager.add_indicator(indicator)

    def add_rsi_indicator(self, window_size: int=14, price_type: str='close'
        ) ->None:
        """
    ...
    def add_macd_indicator(self, fast_period: int=12, slow_period: int=26,
        signal_period: int=9, price_type: str='close') ->None:
        """
        ...
        Args:
            ...
            price_type: Price type to use
        """
        indicator = MACDState(fast_period=fast_period, slow_period=
            slow_period, signal_period=signal_period, price_type=price_type)
        self.indicator_manager.add_indicator(indicator)

    def add_bollinger_bands_indicator(self, window_size: int=20, num_std:
        float=2.0, price_type: str='close') ->None:
        """
        ...
        Args:
            ...
            price_type: Price type to use
        """
        indicator = BollingerBandsState(window_size=window_size, num_std=
            num_std, price_type=price_type)
        self.indicator_manager.add_indicator(indicator)
    ...
    @with_exception_handling
    def process_tick(self, symbol: str, tick_data: Dict[str, Any]) ->Dict[
        str, Any]:
        """
        ...
        result = {'symbol': symbol, 'timestamp': tick_data['timestamp'], **
            tick_data, 'indicators': updated_indicators}
    ...
    def get_latest_indicators(self, symbol: Optional[str]=None) ->Dict[str, Any
        ...
        if symbol:
            if symbol in self.latest_ticks:
                return {'symbol': symbol, 'timestamp': self.latest_ticks[
                    symbol].get('timestamp', datetime.now()), 'indicators':
                    indicators}
            else:
                return {'symbol': symbol, 'indicators': indicators}
        else:
            return {'indicators': indicators, 'symbols': list(self.
                latest_ticks.keys())}
    ...
    def reset(self, symbol: Optional[str]=None) ->None:
        ...
        self.logger.info(
            f"Reset indicator state for {symbol if symbol else 'all symbols'}")
...
class WebSocketTickProcessor:
    ...
    @async_with_exception_handling
    async def connect_to_feed(self, url: str, symbol: str, api_key:
        ...
        try:
            ...
            return
        if symbol in self.websocket_tasks and not self.websocket_tasks[symbol
            ].done():
            self.logger.info(
                f'Already connected to {symbol}, disconnecting first')
            self.websocket_tasks[symbol].cancel()
        self.connection_info[symbol] = {'url': url, 'api_key': api_key}
    ...
    """
            ...
            while self.running:
                try:
                    self.logger.info(
                        f'Connecting to WebSocket feed for {symbol}: {url}')
                    async with websockets.connect(url, extra_headers=headers
                        ...
                        while self.running:
                            try:
                                ...
                                self.logger.warning(
                                    f'WebSocket connection closed for {symbol}'
                                    )
                                break
                    ...
                    self.logger.error(
                        f'WebSocket connection error for {symbol}: {str(e)}')
                if self.running:
                    self.logger.info(
    ...
    async def reconnect_all(self) ->None:
        ...
        for symbol, info in self.connection_info.items():
            ...
            await self.connect_to_feed(url=info['url'], symbol=symbol,
...
class LatencyMonitor:
    ...
    def __init__(self):
        ...
        self.latency_stats = {'tick_processing': [], 'indicator_updates': {
            }, 'total_pipeline': []}
    ...
    def record_indicator_latency(self, indicator_type: str,
    ...
    def get_latency_report(self) ->Dict[str, Any]:
        ...
        report = {'timestamp': datetime.now().isoformat(),
            'tick_processing': self._calculate_stats(self.latency_stats[
            'tick_processing']), 'indicator_updates': {}, 'total_pipeline':
            self._calculate_stats(self.latency_stats['total_pipeline'])}
        for indicator_type, samples in self.latency_stats['indicator_updates'
            ].items():
            report['indicator_updates'][indicator_type
                ] = self._calculate_stats(samples)
        return report
    ...
    def _calculate_stats(self, samples: List[float]) ->Dict[str, float]:
        ...
        if not samples:
            return {'min': None, 'max': None, 'avg': None, 'median': None,
                'p95': None, 'p99': None, 'samples': 0}
        import numpy as np
        return {'min': float(np.min(samples)), 'max': float(np.max(samples)
            ), 'avg': float(np.mean(samples)), 'median': float(np.median(
            samples)), 'p95': float(np.percentile(samples, 95)), 'p99':
            float(np.percentile(samples, 99)), 'samples': len(samples)}
