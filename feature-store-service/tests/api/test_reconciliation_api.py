...
from feature_store_service.main import app
...
@patch.object(ReconciliationService, "reconcile_feature_data")
async def test_reconcile_feature_data(mock_reconcile, mock_reconciliation_result):
    """Test the reconcile_feature_data endpoint."""
    # Configure the mock
    mock_reconcile.return_value = mock_reconciliation_result
    
    # Make the request
    response = client.post(
        "/api/v1/reconciliation/feature-data",
        json={
            "symbol": "EURUSD",
    ...
    assert call_args["features"] == ["feature1", "feature2"]
...
@patch.object(ReconciliationService, "reconcile_feature_version")
async def test_reconcile_feature_version(mock_reconcile, mock_reconciliation_result):
    """Test the reconcile_feature_version endpoint."""
    # Configure the mock
    mock_reconcile.return_value = mock_reconciliation_result
    
    # Make the request
    response = client.post(
        "/api/v1/reconciliation/feature-version",
        json={
