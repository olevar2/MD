    ...
    def get_info(cls):
        ...
        return {
            ...
            'parameters': [
                ...
                {
                    'name': 'price_source',
                    'description': 'The price column to use',
                    'type': 'string',
                    'default': 'close'
                }
            ]
    ...
    def get_info(cls):
        ...
        return {
            ...
            'parameters': [
                {
                    'name': 'period',
                    'description': 'The window size for the calculation',
                    'type': 'int',
                    'default': 14
                },
                {
...
class SimpleBBIndicator(BaseIndicator):
    ...
    def __init__(self, period: int = 20, stddev: float = 2.0, price_source: str = "close", **kwargs):
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        result['bb_middle'] = data[self.price_source].rolling(window=self.period).mean()
        
        # Calculate standard deviation
        result['bb_stddev'] = data[self.price_source].rolling(window=self.period).std()
        
        # Calculate upper and lower bands
        result['bb_upper'] = result['bb_middle'] + (result['bb_stddev'] * self.stddev)
        result['bb_lower'] = result['bb_middle'] - (result['bb_stddev'] * self.stddev)
    ...
    def get_info(cls):
        ...
        return {
            ...
            'description': 'Volatility bands that contract and expand based on price action',
            'category': cls.category,
            'parameters': [
                {
                    'name': 'period',
...
class TestMarketRegimeClassifier(unittest.TestCase):
    ...
    def _create_trending_bullish_data(self):
        ...
        # Strong uptrend
        price = np.linspace(100, 150, days)  # 50% increase over the period
        ...
        self.bullish_data = pd.DataFrame({
            ...
            'low': price * (1 - 0.01),
            'close': price + noise,
            'volume': 1000000 * (1 + 0.2 * np.random.randn(days))
        }, index=date_range)
    ...
    def _create_trending_bearish_data(self):
        ...
        # Strong downtrend
        price = np.linspace(150, 100, days)  # 33% decrease over the period
        noise = np.random.normal(0, 1, days)
    ...
    def _create_ranging_volatile_data(self):
        ...
        base = 120
        volatility = 5.0
        price = base + volatility * np.sin(np.linspace(0, 4*np.pi, days))  # Oscillating
        noise = np.random.normal(0, 2, days)  # Higher noise
        
        self.volatile_ranging_data = pd.DataFrame({
            'open': price + np.random.normal(0, 2, days),
            'high': price + np.random.uniform(1, 4, days),
            'low': price - np.random.uniform(1, 4, days),
            'close': price + noise,
            'volume': 1000000 * (1 + 0.3 * np.random.randn(days))
        }, index=date_range)
    ...
    def _create_ranging_tight_data(self):
        ...
        noise = np.random.normal(0, 0.5, days)  # Lower noise
        
        self.tight_ranging_data = pd.DataFrame({
            'open': price + np.random.normal(0, 0.5, days),
            'high': price + np.random.uniform(0.2, 1, days),
            'low': price - np.random.uniform(0.2, 1, days),
            'close': price + noise,
            'volume': 1000000 * (1 + 0.1 * np.random.randn(days))
        }, index=date_range)
    ...
    def _create_breakout_data(self):
        ...
        # Second half: strong breakout up
        ...
        volume[days//2:] *= 2.5  # Volume surge during breakout
        
        self.breakout_data = pd.DataFrame({
            'open': [p * (1 - 0.005) for p in price],
            'high': [p * (1 + 0.01) for p in price],
            'low': [p * (1 - 0.01) for p in price],
            'close': price,
            'volume': volume * (1 + 0.2 * np.random.randn(days))
        }, index=date_range)
    ...
    def _create_reversal_data(self):
        ...
        # Second half: reversal and downtrend
        ...
        volume = np.ones(days) * 1000000
        volume[days//2-2:days//2+3] *= 2.0  # Volume surge at reversal
        
        self.reversal_data = pd.DataFrame({
            'open': [p * (1 + np.random.uniform(-0.01, 0.01)) for p in price],
            'high': [p * (1 + 0.015) for p in price],
            'low': [p * (1 - 0.015) for p in price],
            'close': price + np.random.normal(0, 0.5, days),
            'volume': volume * (1 + 0.2 * np.random.randn(days))
        }, index=date_range)
    ...
    def test_breakout_classification(self):
        ...
        # Use only the breakout part of the data
        breakout_part = self.breakout_data.iloc[len(self.breakout_data)//2-5:]
    ...
    def test_reversal_classification(self):
        ...
        # Use data centered around the reversal point
        reversal_part = self.reversal_data.iloc[len(self.reversal_data)//2-5:len(self.reversal_data)//2+5]
...
class TestIndicatorPerformanceMetrics(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Add a clear trend for testing indicator performance
        ...
        # Create indicators with different characteristics
        self.ma_fast = SimpleMAIndicator(period=10)  # Fast MA, should be responsive
        self.ma_slow = SimpleMAIndicator(period=30)  # Slow MA, should lag
        self.rsi = SimpleRSIIndicator(period=14)  # RSI for oscillator
        ...
        # Generate some simple signals based on indicators
        self.data['ma_signal'] = np.where(
            self.data['sma_10'] > self.data['sma_30'], 
            1, np.where(self.data['sma_10'] < self.data['sma_30'], -1, 0)
        )
        
        self.data['rsi_signal'] = np.where(
            self.data['rsi_14'] > 70, 
            -1, np.where(self.data['rsi_14'] < 30, 1, 0)
        )
    ...
    def test_metrics_calculation(self):
        ...
        # Calculate metrics for MA crossover
        ma_metrics = self.metrics.calculate_metrics(
            self.data['sma_10'],
            self.data,
            self.data['ma_signal']
        )
        
        # Should have overall metrics and period metrics
        self.assertIn('overall', ma_metrics)
        self.assertIn(f'period_{self.metrics.lookback_periods[0]}', ma_metrics)
        
        # Calculate metrics for RSI
        rsi_metrics = self.metrics.calculate_metrics(
            self.data['rsi_14'],
            self.data,
            self.data['rsi_signal']
        )
    ...
    def test_prediction_power(self):
        ...
        # Fast MA should have better prediction power in a trending market
        ma_fast_metrics = self.metrics.calculate_metrics(
            self.data['sma_10'],
            self.data
        )
        
        ma_slow_metrics = self.metrics.calculate_metrics(
            self.data['sma_30'],
            self.data
        )
        
        # During the uptrend, the faster MA should be more aligned with price
        uptrend_data = self.data.iloc[:days//2]
        ma_fast_uptrend = self.metrics.calculate_metrics(
            uptrend_data['sma_10'],
            uptrend_data
        )
        
        ma_slow_uptrend = self.metrics.calculate_metrics(
            uptrend_data['sma_30'],
            uptrend_data
        )
        
        # Fast MA should have higher prediction power in short horizons
        # This checks that the prediction power calculation is working properly
        if ('prediction_power_1' in ma_fast_uptrend['overall'] and 
            'prediction_power_1' in ma_slow_uptrend['overall']):
            self.assertGreaterEqual(
                ma_fast_uptrend['overall']['prediction_power_1'],
                ma_slow_uptrend['overall']['prediction_power_1']
            )
    
    def test_signal_accuracy(self):
        """Test signal accuracy metrics."""
        # Calculate metrics for MA crossover signals
        ma_metrics = self.metrics.calculate_metrics(
            self.data['sma_10'],
            self.data,
            self.data['ma_signal']
        ...
        # Should have signal accuracy metrics
        for period_key in ma_metrics.keys():
            if period_key != 'overall':
                # Look for at least one period that has signal accuracy metrics
                if any('signal_accuracy' in k for k in ma_metrics[period_key].keys()):
                    break
        else:
...
class TestIndicatorSelectionEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # First 30 days: uptrend (trending bullish)
        for _ in range(30):
            price += 0.2 + np.random.normal(0, 0.3)
            prices.append(price)
        
        # Next 30 days: ranging (tight range)
        for _ in range(30):
            price += np.random.normal(0, 0.5)
            prices.append(price)
        
        # Last 40 days: downtrend (trending bearish)
        for _ in range(40):
            price -= 0.2 + np.random.normal(0, 0.3)
            prices.append(price)
        ...
        # Create indicator registry with various indicators
        self.indicators = {
            'ma_10': SimpleMAIndicator(period=10),
            'ma_20': SimpleMAIndicator(period=20),
            'ma_50': SimpleMAIndicator(period=50),
            'ma_200': SimpleMAIndicator(period=200),
            'rsi_7': SimpleRSIIndicator(period=7),
            'rsi_14': SimpleRSIIndicator(period=14),
            'bb_20_2': SimpleBBIndicator(period=20, stddev=2.0),
            'bb_20_3': SimpleBBIndicator(period=20, stddev=3.0)
        }
        
        # Create temporary files for testing
        self.temp_dir = tempfile.mkdtemp()
        self.performance_path = os.path.join(self.temp_dir, 'performance_history.json')
        self.config_path = os.path.join(self.temp_dir, 'selection_config.json')
        
        # Initialize selection engine
        self.selection_engine = IndicatorSelectionEngine(
            indicators_registry=self.indicators,
            performance_history_path=self.performance_path,
    ...
    def test_indicator_categorization(self):
        ...
        # Check that indicators were categorized
        self.assertEqual(
            self.selection_engine.indicator_categories['ma_10'],
            IndicatorCategory.TREND
        )
        
        self.assertEqual(
            self.selection_engine.indicator_categories['rsi_14'],
            IndicatorCategory.MOMENTUM
        )
        
        self.assertEqual(
            self.selection_engine.indicator_categories['bb_20_2'],
            IndicatorCategory.VOLATILITY
        )
    
    def test_regime_classification_and_selection(self):
        """Test indicator selection for different market regimes."""
        # Test selection for bullish trend period
        bullish_data = self.data.iloc[:30]
        bullish_indicators = self.selection_engine.select_indicators(bullish_data)
        
        # Should select some indicators
        self.assertGreater(len(bullish_indicators), 0)
        
        # Test selection for ranging period
        ranging_data = self.data.iloc[30:60]
        ranging_indicators = self.selection_engine.select_indicators(ranging_data)
        
        # Should select some indicators
        self.assertGreater(len(ranging_indicators), 0)
        
        # Test selection for bearish trend period
        bearish_data = self.data.iloc[60:]
        bearish_indicators = self.selection_engine.select_indicators(bearish_data)
        
        # Should select some indicators
        self.assertGreater(len(bearish_indicators), 0)
        
        # The selected indicators should differ between regimes
        self.assertNotEqual(set(bullish_indicators.keys()), set(ranging_indicators.keys()))
    ...
    def test_performance_tracking(self):
        ...
        signal = np.where(
            ma_result['sma_20'] > ma_result['close'].shift(1),
            1, -1
        )
        
        # Update performance
        self.selection_engine.update_performance(
            'ma_20',
            ma_result['sma_20'],
            self.data,
        ...
        # Check that performance was saved to file
    ...
    def test_optimize_selections(self):
        ...
        for name, indicator in self.indicators.items():
            ...
            if name.startswith('ma'):
                ...
                col_name = 'bb_middle'  # Just use middle band for testing
            
            # Create a dummy signal
            if col_name in result.columns:
                signal = np.where(
                    result[col_name] > result['close'].shift(1),
                    1, -1
                )
                
                # Update performance with varying success rates to create differences
                if 'ma_10' in name:
                    # Make this one perform well in trending markets
                    signal = np.where(
                        result[col_name] > result[col_name].shift(5),
                        1, -1
                    )
                elif 'rsi' in name:
                    # Make this one perform well in ranging markets
                    signal = np.where(
                        result[col_name] > 70,
                        -1, np.where(result[col_name] < 30, 1, 0)
                    )
        ...
        # Now optimize the selections
        self.selection_engine.optimize_selections()
        
        # Check that the config was updated
        self.assertTrue(os.path.exists(self.config_path))
        
        # The regime indicators should now contain preferred indicators for each regime
