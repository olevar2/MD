...
class TestChartPatternRecognizer(unittest.TestCase):
    ...
    def test_head_and_shoulders_detection(self):
        ...
        found = False
        for pattern in patterns['head_and_shoulders']:
            if 40 <= pattern['start_idx'] <= 60 and 120 <= pattern['end_idx'] <= 150:
                found = True
                break
        
        self.assertTrue(found, "Head and shoulders pattern not detected in the expected region")
