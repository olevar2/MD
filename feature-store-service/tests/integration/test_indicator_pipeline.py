...
class TestIndicatorPipeline(unittest.TestCase):
    ...
    def test_end_to_end_indicator_pipeline(self):
        ...
        # Prepare inputs for ML model using the adapter
        X, y = ml_feature_consumer.prepare_model_inputs(
            features=features,
            target_column=None,  # We'll pass the target separately
            model_id="test_model"
        )

        # Manually add the target since we created it separately
    ...
    def test_ml_feedback_integration(self):
        ...
        # 2. Extract features using the adapter
        ml_feature_consumer = MLFeatureConsumerAdapter()
        ...
        # 3. Prepare model inputs using the adapter
        X, _ = ml_feature_consumer.prepare_model_inputs(
            features=features_clean,
            model_id="test_model"
        )

        # Get feature importance feedback
        feature_importances = ml_feature_consumer.get_feature_importance_feedback(
            model_id="test_model",
