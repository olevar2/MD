...
class TestSystemIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.monitoring_service = ErrorMonitoringService(
            storage_dir=str(self.temp_dir / "monitoring")
        )
        ...
        self.test_data['high'] = self.test_data[['open', 'close']].max(axis=1) + 1
        self.test_data['low'] = self.test_data[['open', 'close']].min(axis=1) - 1
    ...
    def test_end_to_end_calculation_flow(self):
        ...
        # 2. Submit calculation with resource management
        calc_id = f"{indicator_name}_{datetime.utcnow().timestamp()}"
        
        try:
            # Start performance monitoring
            self.performance_optimizer.monitor.start_calculation(
                calc_id=calc_id,
                indicator_name=indicator_name,
                parameters=params
            )
            ...
            future = self.resource_manager.submit_calculation(
                calc_id=calc_id,
                calc_func=dummy_rsi_calculation,
                args=(self.test_data,),
                cache_key=f"{indicator_name}_{params['period']}",
                **params
            )
            ...
            if isinstance(e, (CalculationError, DataError, ParameterError)):
                ...
                if recovery_result:
                    self.logger.log_error(
                        ...
                        message=str(e),
                        details={'recovered': True, 'recovery': recovery_result}
                    )
                else:
                    self.logger.log_error(
                        indicator_name=indicator_name,
        ...
        # 3. Validate calculation result
        result_validation = self.validator.validate(
            'calculation',
            result,
            params=params
        )
        ...
        error_patterns = self.monitoring_service.get_error_patterns(
        ...
        self.assertEqual(len(error_patterns), 0)  # No errors expected
    ...
    def test_performance_optimization(self):
        ...
        # Analyze performance and get optimal parameters
        analysis = self.performance_optimizer.analyze_performance(indicator_name)
        optimal_params = self.performance_optimizer.optimize_parameters(
            indicator_name,
            {'period': 14}  # Current parameters
        )
    ...
    def test_resource_management(self):
        ...
        future1 = self.resource_manager.submit_calculation(
            calc_id="calc_1",
            calc_func=dummy_rsi_calculation,
            args=(self.test_data,),
            cache_key=f"{indicator_name}_{params['period']}",
        ...
        future2 = self.resource_manager.submit_calculation(
            calc_id="calc_2",
            calc_func=dummy_rsi_calculation,
            args=(self.test_data,),
            cache_key=f"{indicator_name}_{params['period']}",
    ...
    def test_system_monitoring(self):
        ...
        for i in range(5):
            try:
                if i % 2 == 0:
                    # Generate successful calculation
